...
function main() {
  ...
  if (unreachable.length > 0) {
    ...
    // 性能:backedUp ~4359 个 hash,单次 git cat-file --batch 从 stdin 批量 peel tree.
    const backedTrees = new Set()
    {
      const input = [...backedUp].map((h) => `${h}^{tree}`).join('\n') + '\n'
      try {
        const out = execSync(
          'git cat-file --batch="%(objectname)"',
          { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], input, timeout: LOCAL_GIT_TIMEOUT_MS * 10 },
        )
        for (const line of out.split('\n')) {
          const t = line.trim()
