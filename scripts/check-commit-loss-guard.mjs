...
function main() {
  ...
  if (unreachable.length > 0) {
    ...
    {
      // 注意:Windows shell:true 下 cmd 会破坏含 % 的 format 串,且 PowerShell 不展开
      // JS 模板变量——改用 execSync input 管道喂 ref 名给 for-each-ref --stdin.
      const refNames = run('git for-each-ref refs/tags/lost-commit refs/tags/backup --format="%(refname)"', { allowFail: true })
      const input = refNames.split('\n').filter(Boolean).join('\n') + '\n'
      let refOut = ''
      try {
        refOut = execSync('git for-each-ref --stdin --format="%(objectname) %(*objectname)"', {
          encoding: 'utf8',
