...
const checks = [
  ...
  // --- 73 (2026-09-23 新增,mobile-rn 深色模式前景/容器回归守门,blocking) ---
  // 背景(2026-09-23 Drawer/StudyBar/Profile 深色真机复核):mobile-rn 深色下 brand.DEFAULT
  //   翻成 #FFFFFF,任何「brand 背景 + surface.light/text.primary 前景」组合都会白底白字;
  //   「surface.light / α≥0.5 白 rgba / className bg-white 作容器背景」在深色下都是漏白容器。
  //   R1 为零豁免硬门;R2 按文件基线棘轮(scripts/brand-foreground-baseline.json),只减不增。
  {
    id: '73',
    ...
    onFailHint: [
      '',
      '  💡 品牌底(brand.DEFAULT)上的文字必须用 tokens.brand.foreground(深色自动翻黑)。',
      '     容器背景用 surface.card / surface.muted / surface.inputBg,禁止 surface.light / 白 rgba / bg-white。',
      '     收紧基线(人工确认后):node scripts/check-brand-foreground.mjs --update-baseline',
      '     自检:node scripts/check-brand-foreground.mjs --self-test',
      '     紧急跳过(不推荐):HUSKY_SKIP_BRAND_FOREGROUND=1 git commit ...',
      '',
    ].join('\n'),
  },
  // --- info (1 项) ---
