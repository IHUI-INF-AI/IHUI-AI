...
const checks = [
  ...
  // --- 批量写计数诚实性对账(1 项,blocking)---
  {
    ...
    onFailHint: [
      ...
      '     刻意不判的两型:布尔 deleted:true(无写链那一半是全仓惯例,改它属全 API 语义决策)',
      '     与读查询 count: rows.length(不是写)。两型的**现读数**由结论行喊出(不在这里钉数字,',
      '     钉了就是一笔假账);B1 判的是其中"同函数体有写链却不问库"的那一子集。',
      '     判不了的形状落「未判定」并报数,--strict 下有未判定即 exit 2(拒绝出合格证)。',
      '     ① 看清单:node scripts/check-batch-write-count-honesty.mjs [--json|--staged]',
      '     ② 自检:node scripts/check-batch-write-count-honesty.mjs --self-test',
      '     ③ 镜像:node --test scripts/tests/check-batch-write-count-honesty.test.mjs',
