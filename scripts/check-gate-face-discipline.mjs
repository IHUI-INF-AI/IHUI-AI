...
function selfTest() {
  ...
  //     **这一格由上面的构造面证明(qual 0 而终态 loose-fs),不靠仓里恰好有样本** ——
  //     原先写的是"唯一这么写的 `check-api-routes.mjs` 由合取兜住",而该文件已于
  //     2026-09-26(`1e7ed628e9b`,守门 8 扩三端)迁进取材层变成 `face`,那句"现读有样本"当场失真。
  //     教训与镜像 T22 同一条:**不得把仓库瞬时状态当恒定前提**,证一条排除的代价只能用构造面。
  //     人群侧的活体检查改由镜像 T22 承担:HEAD 上任何读点开头根级文件的门,终态不得是
  //     `no-content`(看不见),必须是 `loose-fs`(合取接住)或 `face`(已迁移)。
