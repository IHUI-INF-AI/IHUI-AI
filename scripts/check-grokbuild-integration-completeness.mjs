 ...
 *
 * 守门脚本:防止"grok-build 100% 整合"虚假声明再次出现。
 *
 * 校验目标:
 *   1. 全项目 grep `grokbuild|grok-build|GrokBuild|xai-cli|xai-org` 必须零匹配
 ...
 *
 * 集成方式:
 *   - 直接运行:`node scripts/check-grokbuild-integration-completeness.mjs`
...
const NOT_INTEGRATED_CLAIMS = [
  {
    id: 'NI-1',
    name: 'xai-grok-markdown (checkpoint-based freezing)',
    evidenceFile: 'scripts/audit-grokbuild-crates.md',
    requiredTokens: ['xai-grok-markdown', '不融合', '理念借鉴'],
  },
  {
    id: 'NI-2',
...
async function checkNaming(files) {
  ...
  const selfPath = path.resolve(PROJECT_ROOT, 'scripts/check-grokbuild-integration-completeness.mjs');
  // 守门脚本文件名(被 .husky/ 钩子等合法引用,文件名含 grokbuild 字符串是命名的必然)
  const selfScriptName = 'check-grokbuild-integration-completeness';
  for (const pattern of FORBIDDEN_PATTERNS) {
    const re = new RegExp(pattern, 'i');
    for (const f of files) {
      // 跳过守门脚本自身(否则脚本文件名/注释里的 grokbuild 字符串会误判为违规)
      ...
      for (let i = 0; i < lines.length; i++) {
        if (re.test(lines[i])) {
          ...
          // 白名单 2:引用守门脚本自身文件名(check-grokbuild-integration-completeness.mjs)
          // 适用场景:.husky/pre-commit 等钩子文件调用本守门脚本时,文件名必然含 grokbuild
          // 字符串(脚本命名是为了表达"防止 grokbuild 命名再次出现"的意图,非真实使用)
          if (lines[i].includes(selfScriptName)) {
            continue;
          }
          // 白名单 3:.husky/ 钩子文件中描述守门项的中性注释/label 字符串
          // (例如 "// 13. grok-build 整合完整性守门(PROJECT_PLAN.md 第 38 轮落地)")
          // 这是描述守门目标的中性表述,而非在源码中真实使用 grokbuild 命名
          if (
            isHuskyHook &&
            /(?:守门|检查 grok-build|整合完整性|grok-build 整合)/.test(lines[i])
          ) {
            continue;
          }
...
const CLAIMED_CAPABILITIES = [
  ...
  {
    id: 'CLAIM-P39-3',
    name: 'check-grokbuild 脚本"已声明但未实现"自动检测',
    ...
    fileCheck: ['scripts/check-grokbuild-integration-completeness.mjs'],
  },
  {
    id: 'CLAIM-P39-4',
...
//
// 通过验证的真实 claim(保留守门):
//   CLAIM-P39-3(check-grokbuild 脚本自身,文件存在)
...
async function main() {
  ...
  if (helpMode) {
    console.log(`
用法:node scripts/check-grokbuild-integration-completeness.mjs [选项]
...
守门项:
  1. 命名清零(grokbuild/grok-build/GrokBuild/xai-cli/xai-org 必须零匹配)
  2. 已整合能力 import 证据
  3. 已整合能力 call 证据
  4. 已整合能力 file 存在
...
`);
  ...
  console.log('🔍 check-grokbuild-integration-completeness');
  ...
  console.log('━━━ 守门项 1:命名清零(grokbuild/grok-build/GrokBuild/xai-cli/xai-org)━━━');
