...
import { existsSync, lstatSync, readFileSync, readdirSync, readlinkSync, renameSync, rmSync, statSync, mkdirSync, mkdtempSync, writeFileSync } from 'node:fs'
import { homedir, tmpdir } from 'node:os'
import { dirname, join, relative, resolve, sep } from 'node:path'
import { fileURLToPath, pathToFileURL } from 'node:url'
import { registryOf } from './check-home-junctions.mjs'
...
export function targetFor(srcPath, home, devEnv) {
  ...
  if (srcPath.startsWith(appdata + sep)) return join(devEnv, 'cache', 'userhome', `appdata-roaming-${base}`)
  if (srcPath.startsWith(local + sep)) return join(devEnv, 'cache', 'userhome', `appdata-local-${base}`)
...
export function repairOne(srcPath, dstPath, { dry = false, resetDst = false } = {}) {
  ...
  if (isLink(srcPath)) {
    if (existsSync(dstPath)) return { src: srcPath, action: 'link', ok: true, note: '已是指针且目标在位' }
    ...
    if (dry) return { src: srcPath, action: 'would-recreate-target', ok: true, note: `目标缺失,将重建空目录 ${dstPath}` }
    try {
      mkdirSync(dstPath, { recursive: true })
    } catch (e) {
      return { src: srcPath, action: 'recreate-failed', ok: false, note: `重建目标失败:${e.code || e.message}` }
    }
    return {
      src: srcPath,
  ...
  if (!existsSync(srcPath)) return { src: srcPath, action: 'absent', ok: true, note: '源不存在,无需改道' }
  if (!statSync(srcPath).isDirectory()) return { src: srcPath, action: 'file', ok: true, note: '同名文件,非目录' }

  const before = fingerprintTree(srcPath)
  if (dry)
    return { src: srcPath, action: 'would-move', ok: true, note: `${before.size} 个文件 / ${Math.round([...before.values()].reduce((s, n) => s + n, 0) / 1048576)}MB` }
  ...
  for (let round = 1; round <= 4; round++) {
    ...
    const rc = run('robocopy', [srcPath, dstPath, '/E', '/XJ', '/SL', '/COPY:DAT', '/R:1', '/W:1', '/NFL', '/NDL', '/NJH', '/NJS', '/NP'])
    if (rc.status >= 8 || rc.error)
      return { src: srcPath, action: 'copy-failed', ok: false, note: `robocopy 返回 ${rc.status}(≥8 视为失败,源未动)` }
    ...
    if (round === 4)
      return {
        ...
        ok: false,
        note: `4 轮仍不一致(源 ${snap.size} / 副本 ${dst.size};源缺 ${d.onlyB.length} 目标缺 ${d.onlyA.length} 字节不同 ${d.differ.length})` +
          (residueOnly ? ' ⇒ 目标是本工具上次失败留下的残留,用 --reset-dst 清掉再跑' : ' ⇒ 源在被持续写入或复制未成功,不建 junction、不删源'),
      }
  ...
  try {
    ...
    return { src: srcPath, action: 'rename-failed', ok: false, note: `改名失败(多半被进程占用):${e.code || e.message}` }
  }
  ...
  if (!isLink(srcPath)) {
    ...
    return { src: srcPath, action: 'mklink-failed', ok: false, note: `mklink 未生效(rc=${mk.status}),已回退改名` }
  }
  ...
  if (!sameFingerprint(snap, through)) {
    ...
    try {
      renameSync(stash, srcPath)
      return { src: srcPath, action: 'rolled-back', ok: false, note: '经 junction 回读不一致 ⇒ 断链并还原源,未删任何数据' }
    } catch {
      return { src: srcPath, action: 'STUCK', ok: false, note: `回读不一致且回退失败!源数据在 ${stash},请人工处理` }
    }
  }
...
function selfTest() {
  ...
  push('家目录项 → userhome/<原名>', targetFor(join(home, '.cargo'), home, dev) === join(dev, 'cache', 'userhome', '.cargo'))
  push('.ollama 特例 → cache/ollama', targetFor(join(home, '.ollama'), home, dev) === join(dev, 'cache', 'ollama'))
  push('APPDATA 项加 roaming 前缀', targetFor(join(process.env.APPDATA, 'npm'), home, dev) === join(dev, 'cache', 'userhome', 'appdata-roaming-npm'))
  push('LOCALAPPDATA 项加 local 前缀', targetFor(join(process.env.LOCALAPPDATA, 'pnpm-cache'), home, dev) === join(dev, 'cache', 'userhome', 'appdata-local-pnpm-cache'))
  ...
  try {
    ...
    push('指纹按相对路径 + 字节记录', fp.size === 1 && fp.get('b/f.txt') === 5, [...fp.entries()].join(','))
    ...
    push('absent 项判 ok 且不动盘', repairOne(join(root, 'nope'), join(root, 'dst')).action === 'absent')
    push('--dry 不建目标目录', (() => {
    ...
    const mk = spawnSync(GIT_BASH, ['/c', 'mklink', '/J', dkSrc, dkDst], { windowsHide: true, encoding: 'utf8' })
...
async function main() {
  ...
  for (const it of items) {
    if (apply && !noCooldown && cool[it.src] > now) {
      rows.push({ ...it, src: it.src, action: 'cooldown', ok: true, note: `冷却中(剩 ${Math.ceil((cool[it.src] - now) / 60000)} 分钟)` })
      continue
    }
  ...
  console.log(
    `[re-home-junctions] ${apply ? 'APPLY' : 'CHECK ONLY'}:登记 ${rows.length} / 已改道 ${rows.filter((r) => r.action === 'link' && r.ok).length} / 待改道 ${would.length + moved.length} / 已改道成功 ${moved.length} / 冷却中 ${rows.filter((r) => r.action === 'cooldown').length} / 异常 ${bad.length}`,
  )
  for (const r of rows) {
...
export const __test__ = { plan, targetFor, fingerprintTree, sameFingerprint, diffFingerprint, repairOne, readCooldown, cooldownPath }
// ⁠​‌​​‌​​‌‍‍​‌​​‌​​​‍‍​‌​‌​‌​‌‍‍​‌​​‌​​‌‍‍​​‌​‌‌​‌‍‍​‌​​​​​‌‍‍​‌​​‌​​‌‍‍‌​‌‌​‌‌‌‍‍‌‌​​‌‌​​‌‌‌‌​‌​‍‍‌‌​‌‌​​​‌​​​‌‌‌‍‍​‌​​​​​‌‍‍​‌​​‌​​‌‍‍‌​‌‌​‌‌‌‍‍‌‌​​‌‌‌​‌​​‌‌‌​‍‍‌‌​​‌‌​​​‌​​‌​‌‍‍‌​‌‌‌​‌‌‌​‌‌‌​‌‍‍‌​‌‌​‌‌‌‍‍​‌​​‌‌​​‍‍​‌​​​​‌‌‍‍‌​‌‌​‌‌‌‍‍​‌‌​​​​‌‍‍​‌‌​‌​​‌‍‍​‌‌‌‌​‌​‍‍​‌‌​‌​​​‍‍​‌‌‌​​‌‌‍‍​​‌​‌‌‌​‍‍​‌‌‌​‌​​‍‍​‌‌​‌‌‌‌‍‍​‌‌‌​​​​‍‍‌​‌‌​‌‌‌‍‍​‌​‌​​​​‍‍​‌​‌​​‌​‍‍​‌​​‌‌‌‌‍‍​‌​‌​‌‌​‍‍​‌​​​‌​‌‍‍​‌​​‌‌‌​‍‍​‌​​​​​‌‍‍​‌​​‌‌‌​‍‍​‌​​​​‌‌‍‍​‌​​​‌​‌‍‍​​‌​‌‌​‌‍‍​​‌‌​​‌​‍‍​​‌‌​​​​‍‍​​‌‌​​‌​‍‍​​‌‌​‌‌​⁠
