...
import { existsSync, lstatSync, readFileSync, readdirSync, renameSync, rmSync, statSync, mkdirSync, mkdtempSync, writeFileSync } from 'node:fs'
...
export function repairOne(srcPath, dstPath, { dry = false, resetDst = false } = {}) {
  ...
  if (isLink(srcPath)) {
    ...
    return { src: srcPath, action: 'link', ok, note: ok ? '已是指针且目标在位' : `指针目标缺失(DANGLING):${dstPath}` }
  }
