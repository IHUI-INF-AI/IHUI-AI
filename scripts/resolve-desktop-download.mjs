...
async function resolveFromGitee() {
  ...
  for (const asset of release.assets || []) {
    ...
    if (mapped) {
      ...
      try {
        const sigRes = await fetch(`${href}.sig`, { redirect: 'follow' })
        if (sigRes.ok) mapped.signature = (await sigRes.text()).trim()
      } catch {
        /* 无签名文件则省略 */
      }
      assets.push(mapped)
...
function serializeSnapshot(data) {
  const assetLines = data.assets
    .map((a) => {
      const sig = a.signature ? ` signature: '${a.signature}',` : ''
      return `    { href: '${a.href}', sizeBytes: ${a.sizeBytes}, format: '${a.format}', arch: '${a.arch}',${sig} },`
    })
    .join('\n')
