...
| scan-hardcoded-zh.mjs            | 扫描 web + ui-react + shared 下硬编码中文字符串(未走 t()/next-intl),按每文件额度对比基线棘轮拦截新增 | `scripts/hardcoded-zh-baseline.json` | `--json <out>` `--top N` `--staged` `--exit 1` `--update-baseline` |
...
| check-parent-pollution.mjs            | 项目父目录污染巡查(运行时守门;2026-09-23 起 `CREDENTIAL_DIR_NAMES` 8 项(secrets/secret/credentials/credential/密钥/certs/certificates/.pybcrypt)整目录不扫 —— `--auto-clean` 对"仅文件名强信号命中"直接 unlinkSync 实删文件且无二次确认(内容双信号命中只告警),曾会删掉用户凭据库) | 26 (blocking)  | 命中凭据目录时先补豁免再清理               |
| check-root-dir-clean.mjs              | 根目录整洁(一级目录白名单 blocking;另对"被 git 忽略的根级临时产物 .log/.html/cookies/截图/ad-hoc 脚本"只告警)                                                                                                                                                                     | 44 (blocking)  | 2026-09-23 补忽略产物告警面                |
