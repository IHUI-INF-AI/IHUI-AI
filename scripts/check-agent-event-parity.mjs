...
const WHITELIST = [
  ...
  {
    name: 'task_progress',
    reason: '看板进度事件,useAgentSSE.onmessage 泛型接收(setLastEvent)统一分发,无需逐名分支',
  },
  {
    name: 'worker_status',
    reason: 'worker 状态事件,useAgentSSE.onmessage 泛型接收,当前无逐名消费需求',
  },
  {
    name: 'dag_level_advanced',
    reason: 'DAG 层级推进事件,AgentSSEEvent 契约声明+预留,前端经 onmessage 泛型透传',
  },
  ...
  {
    name: 'start',
    reason: 'execute/stream 起始事件(resume_from 重连锚点),api-client onEvent 兜底统一分发',
  },
  { name: 'status', reason: 'langgraph 状态流转事件,api-client onEvent 兜底统一分发' },
  // P0-5(2026-09-13):agents.py /agents/tasks/stream "thinking.delta"→"thinking" 已由
  // use-agent-runtime addEventListener 逐名消费;langgraph execute/stream 的同名思考事件
  // 仍经 api-client onEvent 兜底统一分发(保留本条目覆盖该场景)
...
const FRONTEND_LEGACY_EVENTS = [
  {
    name: 'content',
    reason:
      'use-agent-runtime.ts onmessage 的历史契约分支:现 /agents/tasks/stream 仅发命名事件(agents.py 全部带 event: 字段,匿名分支运行时不可达);是否清理分支或补发 content 事件由业务侧决策',
  },
  {
    name: 'permission_request',
    reason:
      'api-client dispatchSSEEvent 兼容分支:现行审批契约为 hook tool.approval → SSE tool-approval,后端已无 permission_request 命名事件生产,保留分支仅为旧调用方兼容',
  },
]
...
function walk(dir, exts, out = []) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (exts.some((e) => entry.name.endsWith(e)) && !isTestFile(entry.name)) {
...
const TS_CONTRACT_RE = /'([a-z_]+)'/g
{
  ...
  if (start === -1) {
    errors.push(
      'packages/types/src/agent-runtime.ts 未找到 AgentSSEEvent 声明(契约类型被移动/改名,请同步本守门)',
    )
  } else {
...
const PY_MAPPING_RE = /"[a-z_.]+":\s*"([a-z_.-]+)"/g
{
  ...
  if (start === -1) {
    errors.push(
      'apps/ai-service/app/routers/agents.py 未找到 _map_hook_event_to_sse(SSE 命名映射被移动/改名,请同步本守门)',
    )
...
const SANITY_MIN = [
  ...
  {
    key: 'eventSourceFiles',
    min: 2,
    label: '前端 EventSource 文件(use-agent-runtime/useAgentSSE/tool-approval-dialog)',
  },
...
for (const { key, min, label } of SANITY_MIN) {
  ...
  if (actual < min) {
    errors.push(
      `扫描器疑似失效: ${label} 仅命中 ${actual} 处(阈值 ${min})—— 正则/路径需随代码同步更新,禁止空转变绿`,
    )
  } else {
...
for (const w of FRONTEND_LEGACY_EVENTS) {
  if (!frontendEvents.has(w.name)) {
    warnings.push(
      `FRONTEND_LEGACY_EVENTS 条目 "${w.name}" 已无前端监听点(分支已被清理), 条目过期请移除`,
    )
  }
}
