 ...
 *  2. **端到端不回归**:按生成器同一份扫描面(ALPHA_V3_SCAN_FACES,2026-09-25 收窄后登记表只由
 *     真 v3 消费端导出;miniapp 是 v4,其用量由 R6 报数、不进表)导出的表 == HEAD 里那张登记表(逐字节)。
 *     这一步是"自动生成没把既有表改坏"的唯一机械证明。表面与语料面必须是**同一个常量** ——
 *     测试自己再拼一遍扫描面,就是"生成器与守门各写一遍取值逻辑"那一型(本文件曾钉着收窄前的
 *     3 面语料 + 17 形态内容,985e8aff33 写它时成立、79ddc42163 收窄后结构性失效)。
...
test('T1 §22c 装车:源必须 export __test__ 且键齐全,测试必须真 import 它', () => {
  ...
  assert.match(
    ...
    '必须有 isDirectRun 守卫(§22d)',
  )
  assert.match(src, /if \(isDirectRun\)/, 'CLI 主流程必须挂在 isDirectRun 之后')
})

test('T2 真仓端到端:按生成器同一扫描面,HEAD 语料导出的表必须逐字节等于 HEAD 的登记表', async () => {
  ...
  const files = GATE.collectAlphaCorpus({ face: 'head', faces: GATE.ALPHA_V3_SCAN_FACES }).map(
    (c) => ({
      rel: c.rel,
      src: c.eff,
    }),
  )
  const h = S.harvestForms(files, {
    tiers,
    supportedKinds: Object.keys(plugin.ALPHA_UTILITY_KINDS),
  })
  const derived = S.buildUsageTable(h.forms, reg.usage)
  const rendered = S.spliceUsageBody(reg.faces.pluginTxt, derived)
  // 这一条是本票的核心结论:按**生成器自己的那份面**导出,复现 HEAD 已入库的表 ⇒ 接上生成器不改变产物。
  // 若它红,说明表与用量已经分叉(有人加了 v3 用法没登记,或表里有死规则),出路只有一条:跑生成器。
  assert.equal(
    rendered,
    reg.faces.pluginTxt,
  ...
  // 阳性对照改为构造面(旧写法钉 derived.primary.bg==='10' / muted==='[0.12]' —— 那是收窄前
  // 由 miniapp(v4)用量撑起来的表内容;登记表按 v3 面归零后,把内容当健康期望就成了移动量钉死,
  // 且真 v3 语料为空时"等式两边都空"会恒真。这里给同一份 harvest→build 链喂一条**真 v3 面路径**
  // 的合成用法:数值档逐字进表、任意值族不归并。判据一旦瞎(抽取失效/族归并),这一条当场红。
  const synth = S.harvestForms(
    [
      {
        rel: 'packages/app/src/__alpha-positive-control__.tsx',
        src: '<View className="bg-primary/10 bg-muted/[0.12]" />',
      },
    ],
    { tiers, supportedKinds: Object.keys(plugin.ALPHA_UTILITY_KINDS) },
  )
  const built = S.buildUsageTable(synth.forms, {})
  assert.deepEqual(built.primary.bg, ['10'], '合成 v3 用法必须进表(阳性对照)')
  assert.deepEqual(
    built.muted.bg,
    ['[0.12]'],
    '任意值形态必须以 [0.12] 原样登记,不是归并后的数值族',
  )
})
...
test('T3 判据有牙(真仓阳性对照):bg-muted/40 只活在注释里,既不在表里也不得进表', async () => {
  ...
  const masked = new Set(S.harvestForms(files, { tiers, supportedKinds: kinds }).forms.keys())
  ...
  for (const { rel, src } of files) {
    ...
    for (const t of GATE.extractAlphaUsages(src, { tiers, isCss, original: src }).tokens)
      if (t.onPresetTier && !t.exempt && kinds.includes(t.kind))
        unmasked.add(`${t.kind}-${t.tier}/${t.mod}`)
  }
  const commentOnly = [...unmasked].filter((k) => !masked.has(k))
  assert.ok(
    commentOnly.includes('bg-muted/40'),
    `真仓语料里必须仍能举出注释假用量这一型,bg-muted/40 实得:${JSON.stringify(commentOnly)}`,
  )
  assert.equal(masked.has('bg-muted/40'), false, 'bg-muted/40 不得进表(它就是当年被删的那条死规则)')
  assert.equal(
    Object.prototype.hasOwnProperty.call(reg.usage, 'muted') &&
      (reg.usage.muted.bg || []).includes('40'),
    false,
...
test('T5 原位写回的字节边界:表外一字节都不许动', async () => {
  ...
  assert.equal(
    ...
    reg.faces.pluginTxt.slice(0, bodyStart),
    '表体之前的字节被改写了',
  )
...
test('T9 写回闸:表外有未提交改动必须拒绝;只有表体不同必须放行', () => {
  ...
  const outsideTouched =
    'export const ALPHA_USAGE = {\n  primary: { bg: ["10"] },\n}\nexport const K = 2\n'
  assert.equal(
    S.assertTargetInSync({ worktreeText: tableOnly, faceText: host, rel: 'x', force: false }).ok,
    true,
    '只有表体不同必须放行(否则第一次写回就自我封锁)',
  )
  const guard = S.assertTargetInSync({
    worktreeText: outsideTouched,
    faceText: host,
    rel: 'x',
    force: false,
  })
  ...
  assert.equal(
    S.assertTargetInSync({ worktreeText: outsideTouched, faceText: host, rel: 'x', force: true })
      .ok,
    true,
    '--force 只降级为警告',
  )
...
test('T10 CLI:import 无写盘副作用,--self-test/--help 退出码正确,--face worktree 退 2', () => {
  ...
  const cli = (args) => {
    try {
      const out = execFileSync(
        process.execPath,
        [absTarget('scripts/sync-alpha-usage.mjs'), ...args],
        {
          cwd: ROOT,
          encoding: 'utf8',
          windowsHide: true,
  ...
  assert.equal(
    readFileSync(absTarget(S.PLUGIN_REL), 'utf8'),
    before,
    '测试期间目标文件被改写 ⇒ 只读承诺破裂',
  )
})
