/**
 * @file web 端 i18n 死 key 扫描器集成测试
 * @description 端到端覆盖 _i18n-scan-helpers.mjs 的 main() 入口,模拟 web 端
 *   (messagesPath=packages/i18n/messages/web + scanTargets=apps/web/src 等)
...
// web 端配置(对应 scripts/scan-dead-i18n-keys.mjs TARGETS.web)
const END_CONFIG = {
  name: 'web',
 ...
 * @param {Object} [opts.otherLocales] - { en: {...}, ja: {...}, ... } 其他 4 语言
...
// 静默 main() 的 console.log/warn 输出(测试不依赖日志)
...
describe('web 端 i18n 死 key 扫描器集成测试(main() 端到端)', () => {
  ...
  test("场景 3:t('key', { args }) 带参数调用被识别(2026-07-26 STATIC_T_RE 增强,P2 关键验证)", () => {
    // 增强前:t('about.title', { name: 'xxx' }) 不被 STATIC_T_RE 命中 → about.title 被判死 key
    // 增强后:STATIC_T_RE 支持 (?:,[^)]*)? 可选参数组 → about.title 被识别为引用
    setupFixture({
      zhCN: {
        about: { title: '关于', count: '计数' },
      },
      codeFiles: {
        'apps/web/src/page.tsx': [
          "import { useTranslations } from 'next-intl'",
  ...
  test('场景 5:dryRun=false + out=<path> 写报告到指定路径,内容含死 key 列表', () => {
    setupFixture({
      ...
      // 提供 4 语言完整翻译,避免 used.key 因翻译缺失出现在"翻译不完整"段干扰断言
      otherLocales: {
    ...
    // 精确断言:used.key 不应出现在死 key 列表段(可能出现在翻译不完整段,但本场景已提供 4 语言)
    // 死 key 列表段格式:`- \`<key>\``,而 used.key 被引用,不应以这种格式出现
