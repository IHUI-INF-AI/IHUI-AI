/**
 * @file miniapp-taro 端 i18n 死 key 扫描器集成测试
 * @description 端到端覆盖 _i18n-scan-helpers.mjs 的 main() 入口,模拟 miniapp-taro 端
 *   (messagesPath=packages/i18n/messages/miniapp-taro + scanTargets=apps/miniapp-taro/src)
...
// miniapp-taro 端配置(对应 scripts/scan-miniapp-taro-dead-i18n-keys.mjs)
const END_CONFIG = {
  name: 'miniapp-taro',
...
const CODE_FILE = 'apps/miniapp-taro/src/page.tsx'

let tmpDir
let runScan

before(async () => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'i18n-miniapp-scan-'))
 ...
 */
function setupFixture({ zhCN, otherLocales = {}, codeContent }) {
  ...
  if (codeContent !== undefined) {
    const full = path.join(tmpDir, CODE_FILE)
    fs.mkdirSync(path.dirname(full), { recursive: true })
    fs.writeFileSync(full, codeContent, 'utf8')
  }
}
...
describe('miniapp-taro 端 i18n 死 key 扫描器集成测试(main() 端到端)', () => {
