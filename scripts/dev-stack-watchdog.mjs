...
function install() {
  ...
  const vbsPath = path.join(ROOT, 'scripts', 'dev-stack-watchdog-task.vbs');
  const cmd = `"${process.execPath}" "${path.join(ROOT, 'scripts', 'dev-stack-watchdog.mjs')}"`;
  const vbs = [
    ...
    `shell.Run "${cmd.replace(/"/g, '""')}", 0, False`,
    '',
  ].join('\r\n');
  ...
  log(`计划任务 ${TASK_NAME} 已注册(每 2 分钟巡检,VBS 隐藏无弹窗)`);
}
