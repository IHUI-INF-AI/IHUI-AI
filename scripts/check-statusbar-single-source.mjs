 ...
 *     **S1 的边界(2026-09-24 真机实测,不必再试第二条路)**:那条带由 App 根节点绘制,屏幕内容
 *     在屏幕顶边(y=68px)被裁掉 —— 把 `marginTop:-34dp; paddingTop:94dp; backgroundColor:black`
 *     装进某屏的根 View 后,黑色只填到带**下方**(实测 y=68..190 为 (0,0,0)),带内
 *     (实测 y=8..60 仍为 (245,245,245) = surface.bg)。即"**单屏自己把底色铺进状态栏**"在端内
 *     做不到,只能由单点按当前路由改根背景。已知唯一受害者:`VideoPlayerScreen`(全屏纯黑播放器
 *     上方横一条浅灰带)—— 见到该形态不要再去改端内,那是此边界而非漏做的票。
 *  S2 不得再有第二个取值口 —— 两类形态:
