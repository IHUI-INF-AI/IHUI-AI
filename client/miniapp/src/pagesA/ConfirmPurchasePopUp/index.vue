...
async function createPayOrder() {
  try {
    ...
    if (!userInfo) throw new Error("请先登录")
    if (!userInfo.openId) throw new Error("用户信息不完整，请重新登录")
...
function handlePaySuccess() {
  ...
  updateUserVipStatus()
  emit("paySuccess", { productId: props.productId, amount: price.value })
  setTimeout(() => {
    isLoading.value = false
    emit("updateVisible", false)
  }, 1500)
...
async function checkOrderStatus() {
  try {
    ...
    const res = await uniCloud.callFunction({
      name: "payment",
      data: { action: "checkOrderStatus", params: { userId: userInfo._id } },
    })
...
async function updateUserVipStatus() {
  try {
    ...
    const res = await uniCloud.callFunction({
      name: "user",
      data: { action: "refreshUserInfo", params: { userId: userInfo._id } },
    })
...
watch(
  () => props.visible,
  (newVal) => { if (newVal) loadProductInfo() }
)

watch(
  () => props.productId,
  (newVal) => { if (props.visible && newVal) loadProductInfo() }
)
</script>
...
.popup {
  ...
  .button-area {
    ...
    .pay-button {
      ...
      font-weight: bold;
      border-radius: 30rpx;
      box-shadow: 0 0 15rpx rgba(5, 122, 255, 0.4);
      margin-bottom: 20rpx;
