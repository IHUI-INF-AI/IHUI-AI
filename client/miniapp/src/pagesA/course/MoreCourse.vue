...
function handleFeatureClick(type) {
	...
	if (type === 'ai-image') {
		uni.navigateTo({ url: '/pages/tools/ai_photo' })
		return
	}
	
	if (type === 'marketing') {
		uni.navigateTo({ url: '/pages/tools/ai-marketing' })
		return
	}
...
function handleViewAll(type) {
	uni.vibrateShort()
	uni.showToast({ title: '更多内容开发中', icon: 'none' })
}
</script>
