  ...
  <view class="personal-card">
    <view class="personal-card-title">
      <view style="
        ...
        ">
        <view class="nickname">
          <image src="https://file.aizhs.top/sys-mini/default/3_01.png" mode="heightFix"></image>
          <view class="nickname_text">{{ user.thirdPartyAccounts.nickname }}</view>
          <image src="https://file.aizhs.top/sys-mini/default/3_03.png" mode="heightFix"></image>
          
        </view>
        <image v-if="imageSrc" :src="imageSrc" mode="scaleToFill"
          style="width:74rpx;height: 74rpx;border-radius: 8rpx;" />
        <image v-else src="https://file.aizhs.top/sys-mini/daixaodiming.png" mode="scaleToFill"
          style="width:74rpx;height: 74rpx;" />
      </view>
      <view class="income-box" style="line-height: 46rpx">
        <view class="boxItem">公司总业绩:</view>
      ...
      <view class="button_max_box">
        <view style="line-height: 46rpx;text-align: center;display: flex;flex-direction: row;align-items: center;">
          <view class="boxItem">公司收入:</view>
          <view class="boxItemCount">{{ formatPrice(data.currentAmount) }}</view>
        </view>
        <view class="button_box" @click="withdrawalClick">
          <image src="https://file.aizhs.top/sys-mini/tixian.jpg" mode="scaleToFill"
            style="width: 120rpx; height: 100%;" />
        </view>
      </view>
    </view>
  </view>
...
const user = ref({})
const imageSrc = ref('')
const showIdCardPop = ref(false)

const { nickname, uuid } = uni.getStorageSync("data");
const openId = uni.getStorageSync("data").thirdPartyAccounts.openId;

user.value = uni.getStorageSync("data");
imageSrc.value = uni.getStorageSync("data").avatar;
...
const withdrawalClick = () => {
  if(props.data.currentAmount <= 0) return
  let userData
  userData = uni.getStorageSync('data')['authInfo']
  if (!userData['certificate'] && !userData['username']) {
    ...
    if (amount <= 0) {
      uni.showToast({
        title: '可提现金额为0，暂时无法提现',
        icon: 'none',
        duration: 2000
      });
      return;
    }
    showIdCardPop.value = true;
    uni.navigateTo({
      url: "/pagesA/withdrawal/index?currentAmount=" + props.data.currentAmount,
    });
  }
}

const handleClose = () => {
  showIdCardPop.value = false;
}
</script>
...
.personal-card {
  ...
  .bottom-button {
    ...
    box-shadow: 0 0 10rpx rgba(0, 0, 0, 0.15), inset 0 1px 0 rgba(255, 255, 255, 1);
  ...
  .nickname {
    ...
    position: relative;
    .nickname_text{
      background: url('https://file.aizhs.top/sys-mini/default/3_02.png') repeat-x;
