        ...
        <view class="plaza_tab fenlei_btn_list">
            <view v-if="showAll" class="tab_item flex_center fenlei_btn" :class="{ 'selected active': all }"
                ...
                <image class="icon fenlei_icon" v-if="all"
                    src="/static/images/sqb_20250816161049A277.png" />
                <image class="icon fenlei_icon" v-else
                    src="/static/images/qqb_20250816161046A276.png" />
                <text>全部</text>
            </view>

            <view class="tab_item flex_center fenlei_btn" v-for="(item, index) in tabList" :key="item.id"
                :class="{ 'selected active': viewIds.includes(item.id) }" @click="select(item)">
                <image class="icon fenlei_icon" :src="item.butUrl" v-if="viewIds.includes(item.id) && item.butUrl" />
                <image class="icon fenlei_icon" :src="item.field1" v-if="!viewIds.includes(item.id) && item.field1" />
                <text>{{ item.name }}</text>
            </view>
...
watch(tabValue, (newValue) => {
    if (newValue.length > 0 && props.showAll) {
        all.value = false
    }
    if (newValue.length == 0) {
        all.value = true
    }
    viewIds.value = newValue.map(item => {
        return item.id
    })
    emit('change', newValue)
}, { deep: true })

watch(() => props.values, (n, o) => {
    if (n && n != tabValue.value) {
        tabValue.value = n
