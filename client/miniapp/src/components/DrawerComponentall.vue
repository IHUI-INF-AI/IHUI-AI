      ...
      <view class="drawer_border" :class="{ 'drawer-open': tagWrapShow }">
          ...
          <view class="drawer_menu">
              ...
              <view class="drawer_menu_item" @click="gopage('/pages/table/square/index')">
                  <image class="drawer_menu_label" src="/static/images/drawer_menu2.png" mode="heightFix"></image>
              ...
              <!-- <image @click="gopage('/pages/tools/aigc/index')" class="drawer_menu_label" src="/static/images/drawer_menu4.png" mode="heightFix"></image> -->
              <view class="drawer_menu_item" @click="gopage('/pagesA/studyindex/index')">
                  <image class="drawer_menu_label" src="/static/images/kecheng.png" mode="heightFix"></image>
          ...
          <scroll-view class="drawer-menu" scroll-y :show-scrollbar="true">
              <view v-for="(modelGroup, modelIndex) in sortedGroupedData" :key="modelIndex" class="model-group">
                <view class="model-title">
                  <image class="model-logo" :src="getModelImg(modelGroup.modelName)" mode="aspectFill"></image>
                  {{ modelGroup.modelName }}
                </view>
                <view v-for="(dateGroup, dateIndex) in modelGroup.dateGroups" :key="dateIndex" class="date-group">
                  <view class="date-title">{{ dateGroup.date }}</view>
                  <view v-for="(item, itemIndex) in dateGroup.list" :key="itemIndex" class="menu-item" :class="modelIndex == sortedActiveIndices.modelIndex && dateIndex == sortedActiveIndices.dateGroupIndex && itemIndex == sortedActiveIndices.itemIndex ? 'menu-item_active':''" @click="handleShowFullList(item, itemIndex, modelIndex, dateIndex)" @touchstart="handleTouchStart($event, itemIndex, modelIndex, dateIndex)" @touchmove="handleTouchMove($event, itemIndex, modelIndex, dateIndex)" @touchend="handleTouchEnd($event, itemIndex, modelIndex, dateIndex)">
                    <text class="menu-text" :class="modelIndex == sortedActiveIndices.modelIndex && dateIndex == sortedActiveIndices.dateGroupIndex && itemIndex == sortedActiveIndices.itemIndex ? 'menu_text':''">{{ item.mark }}</text>
                    <image class="menu_remove" :style="{ transform: `translateX(${item.isShow ? '-101':'0'}rpx)` }" @click.stop="removeChat(item.id, itemIndex, modelIndex, dateIndex)" src="/static/images/close_chat.png" mode="widthFix" style="width: 48rpx;height: 48rpx;display: block;"></image>
                  </view>
                </view>
              </view>
          </scroll-view>
          <view class="back_index_btn_bor" v-if="showIndexBtn">
          ...
          <view class="bottom_userInfo">
            ...
            <view style="display: flex; align-items: center; gap: 20rpx;">
              <image @click.stop="handleSettingsClick" class="set_btn" src="/static/images/setting_icon.png" mode="widthFix"></image>
...
const sortedGroupedData = computed(() => {
  ...
  const result = []
  Object.keys(modelGroups).sort((a, b) => {
    if (a === '默认模型') return 1
    if (b === '默认模型') return -1
    return a.localeCompare(b, 'zh-CN')
  }).forEach(modelName => {
    const items = modelGroups[modelName]

    const dateGroups = {}
    items.forEach(item => {
      const date = item.create_time ? item.create_time.split(' ')[0] : '未知日期'
      if (!dateGroups[date]) {
        dateGroups[date] = []
      }
...
const sortedActiveIndices = computed(() => {
  if (!sortedGroupedData.value || sortedGroupedData.value.length === 0) {
    return { modelIndex: -1, dateGroupIndex: -1, itemIndex: -1 }
  }

  for (let modelIndex = 0; modelIndex < sortedGroupedData.value.length; modelIndex++) {
    const modelGroup = sortedGroupedData.value[modelIndex]

    if (!modelGroup.dateGroups || !Array.isArray(modelGroup.dateGroups)) {
      continue
    }

    for (let dateGroupIndex = 0; dateGroupIndex < modelGroup.dateGroups.length; dateGroupIndex++) {
      const dateGroup = modelGroup.dateGroups[dateGroupIndex]

      if (!dateGroup.list || !Array.isArray(dateGroup.list)) {
        continue
      }

      for (let itemIndex = 0; itemIndex < dateGroup.list.length; itemIndex++) {
        const item = dateGroup.list[itemIndex]

        if (item.id && props.active_date && item.id === props.active_date) {
          return { modelIndex, dateGroupIndex, itemIndex }
        }

        let globalIndex = 0
        for (let mi = 0; mi < sortedGroupedData.value.length; mi++) {
          for (let di = 0; di < sortedGroupedData.value[mi].dateGroups.length; di++) {
            for (let ii = 0; ii < sortedGroupedData.value[mi].dateGroups[di].list.length; ii++) {
              if (globalIndex === props.active_menu) {
                if (mi === modelIndex && di === dateGroupIndex && ii === itemIndex) {
                  return { modelIndex, dateGroupIndex, itemIndex }
                }
              }
              globalIndex++
...
function getPaddingTop(height) {
  if (!height && height !== 0) return '0px'
  if (typeof height === 'number') {
    return height + 'px'
  }
  if (typeof height === 'string' && (height.includes('px') || height.includes('rpx') || height.includes('vh') || height.includes('%'))) {
    return height
  }
  return height + 'px'
}

function getModelImg(modelName) {
  const model = props.modelList.find(item => item.name === modelName || item.source === modelName)
  return model && model.img ? model.img : '/static/images/mian_label.png'
}
...
function handleShowFullList(item, index, modelIndex, dateGroupIndex) {
  emit('show-full-list', item, index, modelIndex, dateGroupIndex)
}

function handleTouchStart(event, index, modelIndex, dateGroupIndex) {
  if (sortedGroupedData.value && sortedGroupedData.value[modelIndex] &&
      sortedGroupedData.value[modelIndex].dateGroups &&
      sortedGroupedData.value[modelIndex].dateGroups[dateGroupIndex] &&
      sortedGroupedData.value[modelIndex].dateGroups[dateGroupIndex].list &&
      sortedGroupedData.value[modelIndex].dateGroups[dateGroupIndex].list[index]) {
    const item = sortedGroupedData.value[modelIndex].dateGroups[dateGroupIndex].list[index]
    if (item.isShow === undefined) {
      item.isShow = false
    }
    item.startX = event.touches[0].clientX
  }
  emit('touch-start', event, index, modelIndex, dateGroupIndex)
}
...
function handleTouchMove(event, index, modelIndex, dateGroupIndex) {
  if (sortedGroupedData.value && sortedGroupedData.value[modelIndex] &&
    ...
    const offsetX = currentX - item.startX
    item.offsetX = offsetX
  }
  emit('touch-move', event, index, modelIndex, dateGroupIndex)
}
...
function handleTouchEnd(event, index, modelIndex, dateGroupIndex) {
  if (sortedGroupedData.value && sortedGroupedData.value[modelIndex] &&
    ...
    sortedGroupedData.value.forEach(modelGroup => {
      if (modelGroup.dateGroups) {
        modelGroup.dateGroups.forEach(dateGroup => {
          if (dateGroup.list) {
            dateGroup.list.forEach(listItem => {
  ...
  emit('touch-end', event, index, modelIndex, dateGroupIndex)
}

function removeChat(field1, index, modelIndex, dateGroupIndex) {
  emit('remove-chat', field1, index, modelIndex, dateGroupIndex)
}
