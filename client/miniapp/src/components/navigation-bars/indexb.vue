  ...
  <view v-if="viscosity === true" style="position: sticky; top: 0; z-index: 999; padding: 0 20rpx 0 20rpx;" class="tab-bar" :style="{ height: topBarHeight, background: backgroundColor || '#fff' }">
    ...
    <view style="display: flex; align-items: center; position: relative"
      ...
      <view v-else class="back-bak back-right-moved">
        ...
        <!-- 分类按钮，与 indexa 一致：用 view 包一层 padding: 20rpx 0 -->
      ...
      <view class="center-row-absolute">
        <view class="title"  style="margin-right: 80rpx;" @click="handlenavclicktitle(0)">每日资讯</view>
        <view class="title border_bottom" style="color: #8389FF;" @click="handlenavclicktitle(1)">排行榜</view>
      </view>
...
const emit = defineEmits(['active_nav', 'menu-click', 'nav-click', 'pack'])
...
watch(() => props.tagWrapShow, (newVal) => {
  if(newVal){
    fenLeiSrc.value = '/static/images/fl.pic.jpg';
  } else {
    fenLeiSrc.value = '/static/images/backf2.png';
  }
}, { immediate: true })
