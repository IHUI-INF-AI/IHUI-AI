<template>
  <view class="personal-card">
    <!-- <image src="https://file.aizhs.top/sys-mini/bjcspNew.png" mode="widthFix" style="width: 100%; height: 100%" /> -->
    <view class="personal-card-title">
      <view style="
        ...
        <image v-else src="/static/images/daixaodiming.png" mode="scaleToFill"
      ...
      <view class="income-box" style="line-height: 46rpx">
        ...
        <view class="boxItemCount">{{
          formatPrice(props.data.totalIncome)
        }}</view>
      </view>
      <view class="button_max_box">
        <view style="line-height: 46rpx;text-align: center;display: flex;flex-direction: row;align-items: center;">
          ...
          <view class="boxItemCount">{{ formatPrice(props.data.currentAmount) }}</view>
        </view>
        <view class="button_box" @click="withdrawalClick">
          <image src="/static/images/tixian.jpg" mode="scaleToFill"
            style="width: 120rpx; height: 100%;" />
        </view>
      </view>
    </view>
    <!-- <idCardIndex v-if="showIdCardPop" @close="handleClose" /> -->
  </view>
</template>

<script setup>
import { ref } from 'vue'
// import idCardIndex from "@/components/loginPopUp/idCardIndex.vue";
...
imageSrc.value = uni.getStorageSync("data").thirdPartyAccounts.avatar;
...
function withdrawalClick() {
  console.log(uni.getStorageSync("data")['authInfo']['certificate'],'----uni.getStorageSync("data")');
  console.log(uni.getStorageSync("data")['authInfo']['username'],'----uni.getStorageSync("data")');
  let userData
  userData = uni.getStorageSync('data')['authInfo']
  console.log(uni.getStorageSync('data')['authInfo']);
  if (!userData['certificate'] && !userData['username']) {
    ...
    console.log(props.data.currentAmount);
