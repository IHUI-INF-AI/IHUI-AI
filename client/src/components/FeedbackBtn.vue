...
<template>
  <button class="feedback-button" aria-label="反馈">
    <div class="outline" />
...
/* 豁免原因：局部 CSS 变量声明 + 装饰性渐变 border-box，属于合理硬编码 */
.feedback-button {
  --primary: var(--el-color-danger);
  --neutral-1: var(--el-bg-color);
  --neutral-2: var(--el-fill-color-light);
...
.feedback-button:hover p span {
  opacity: 1;
  animation: wave 0.5s ease forwards calc(var(--i) * 0.02s);
}

.feedback-button:focus p span {
  opacity: 1;
...
.feedback-button:hover .state--default .icon {
  transform: rotate(45deg) scale(1.25);
}

.feedback-button:focus .state--default svg {
  animation: takeOff 0.8s linear forwards;
}

.feedback-button:focus .state--default .icon {
...
.feedback-button:focus .state--default {
  position: absolute;
}

.feedback-button:focus .state--sent {
  display: flex;
}

.feedback-button:focus .state--sent span {
  opacity: 0;
  animation: slideDown 0.8s ease forwards calc(var(--i) * 0.2s);
}

.feedback-button:focus .state--sent .icon svg {
  opacity: 0;
