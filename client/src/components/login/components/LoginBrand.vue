<template>
  <div class="login-brand">
    <!-- Welcome SVG -->
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      fill="none"
      version="1.1"
      width="447"
      height="67"
      viewBox="0 0 447 67"
      class="welcome-svg"
    >
      <g>
        <g>
          <path
            d="M301.75,53L301.75,40.5L306.75,40.5L306.75,53L301.75,53ZM311.75,53L311.75,40.5L316.75,40.5L316.75,45.5L319.25,45.5L319.25,40.5L324.25,40.5L324.25,53L319.25,53L319.25,48L316.75,48L316.75,53L311.75,53ZM331.75,53L331.75,50.5L329.25,50.5L329.25,40.5L334.25,40.5L334.25,50.5L336.75,50.5L336.75,40.5L341.75,40.5L341.75,50.5L339.25,50.5L339.25,53L331.75,53ZM346.75,53L346.75,40.5L351.75,40.5L351.75,53L346.75,53ZM366.75,53L366.75,40.5L371.75,40.5L371.75,53L366.75,53ZM376.75,53L376.75,40.5L381.75,40.5L381.75,43L384.25,43L384.25,45.5L386.75,45.5L386.75,40.5L391.75,40.5L391.75,53L386.75,53L386.75,50.5L384.25,50.5L384.25,48L381.75,48L381.75,53L376.75,53ZM396.75,53L396.75,40.5L406.75,40.5L406.75,43L401.75,43L401.75,45.5L406.75,45.5L406.75,48L401.75,48L401.75,53L396.75,53ZM411.75,53L411.75,50.5L416.75,50.5L416.75,53L411.75,53ZM421.75,53L421.75,43L424.25,43L424.25,40.5L431.75,40.5L431.75,43L434.25,43L434.25,53L429.25,53L429.25,48L426.75,48L426.75,53L421.75,53ZM426.75,45.5L429.25,45.5L429.25,43.1L426.75,43.1L426.75,45.5ZM439.25,53L439.25,40.5L444.25,40.5L444.25,53L439.25,53Z"
            fill="var(--el-text-color-primary)"
            fill-opacity="1"
          />
        </g>
        <g class="welcome-text">
          <path
            d="M13.75,55L13.75,48.125L6.875,48.125L6.875,20.625L13.75,20.625L13.75,48.125L20.625,48.125L20.625,55L13.75,55ZM34.375,48.125L34.375,20.625L41.25,20.625L41.25,48.125L34.375,48.125ZM27.5,55L27.5,48.125L20.625,48.125L20.625,27.5L27.5,27.5L27.5,48.125L34.375,48.125L34.375,55L27.5,55ZM55,55L55,20.625L75.625,20.625L75.625,27.5L61.875,27.5L61.875,34.375L75.625,34.375L75.625,41.25L61.875,41.25L61.875,48.125L75.625,48.125L75.625,55L55,55ZM89.375,55L89.375,20.625L96.25,20.625L96.25,48.125L110,48.125L110,55L89.375,55ZM144.375,34.375L144.375,27.5L130.625,27.5L130.625,20.625L144.375,20.625L144.375,27.5L151.25,27.5L151.25,34.375L144.375,34.375ZM130.625,55L130.625,48.125L123.75,48.125L123.75,27.5L130.625,27.5L130.625,48.125L144.375,48.125L144.375,55L130.625,55ZM144.375,48.125L144.375,41.25L151.25,41.25L151.25,48.125L144.375,48.125ZM171.875,55L171.875,48.125L165,48.125L165,27.5L171.875,27.5L171.875,20.625L185.625,20.625L185.625,27.5L192.5,27.5L192.5,48.125L185.625,48.125L185.625,55L171.875,55ZM171.875,47.849998L185.625,47.849998L185.625,27.775L171.875,27.775L171.875,47.849998ZM206.25,55L206.25,20.625L213.125,20.625L213.125,27.5L220,27.5L220,34.375L226.875,34.375L226.875,41.25L220,41.25L220,34.375L213.125,34.375L213.125,55L206.25,55ZM233.75,55L233.75,34.375L226.875,34.375L226.875,27.5L233.75,27.5L233.75,20.625L240.625,20.625L240.625,55L233.75,55ZM254.375,55L254.375,20.625L275,20.625L275,27.5L261.25,27.5L261.25,34.375L275,34.375L275,41.25L261.25,41.25L261.25,48.125L275,48.125L275,55L254.375,55Z"
            fill="var(--el-text-color-primary)"
            fill-opacity="1"
          />
        </g>
      </g>
    </svg>

    <!-- 项目选择器 -->
    <ProjectSelectorBanner
      v-if="shouldShowProjectSelector"
      :available-projects="projectSelectorProps.availableProjects"
      :selected-project="projectSelectorProps.selectedProject"
      :select-text="projectSelectorProps.selectProjectText"
      @select="projectSelectorProps.selectProject"
    />
  </div>
</template>

<script setup lang="ts">
import { computed } from 'vue'
import ProjectSelectorBanner from '../ProjectSelectorBanner.vue'

interface LoginBrandProps {
  projectSelectorProps?: {
    isMobile: boolean
    currentSource: string | null
    selectProjectText: string
    availableProjects: Array<{ key: string; name: string }>
    selectedProject: string
    selectProject: (key: string) => void
  }
  isRegisterMode: boolean
}

const props = defineProps<LoginBrandProps>()

const shouldShowProjectSelector = computed(() => {
  return (
    props.projectSelectorProps &&
    !props.projectSelectorProps.isMobile &&
    !props.isRegisterMode
  )
})
</script>

<style scoped lang="scss">
.login-brand {
  text-align: center;
  margin-bottom: 32px;
}

.welcome-svg {
  max-width: 100%;
  height: auto;
  margin-bottom: 24px;

  .welcome-text path {
    fill: var(--el-text-color-primary);
  }
}

@media (width <= 768px) {
  .welcome-svg {
    width: 300px;
    margin-bottom: 16px;
  }
}

// 暗色模式适配
:global(html.dark) .welcome-svg .welcome-text path {
  fill: var(--el-text-color-primary);
}
</style>
