...
export default {
  ...
  methods: {
    ...
    expressionChange() {
      // Calculation starts - hide results
      this.isShow = false;
      ...
      let nums = 0;
      // Array used to temporarily store time rule results
      let resultArr = [];
      ...
      // Get possible year array, month array etc. for the next 100 years according to rules
      ...
      if (resultArr.length == 0) {
        ...
        if (resultArr.length !== 5) {
          this.resultList.push(
            t("cmpresult.cronResultTop") + resultArr.length + t("cmpresult.cronResultBottom"),
          );
        }
      }
      // 计算完成-显示结果
