    ...
    <div class="header">
      <el-form :inline="true" :model="searchParam" class="demo-form-inline">
        ...
        <el-form-item v-if="!isComponent">
          <el-button  type="primary" @click="add">创建公司</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="content">
      <div class="content-list">
        <el-table v-loading="dataLoading" :data="list"  style="width: 100%;" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="status" label="会员数量">
            <template #default="scope">
              {{scope.row.memberQty || 0}}
            </template>
          </el-table-column>
          <el-table-column prop="status" label="公司类型">
            <template #default="scope">
              {{scope.row.companyType && scope.row.companyType.name}}
            </template>
          </el-table-column>
          <el-table-column prop="status" label="最大会员数量">
            <template #default="scope">
              {{scope.row.companyType && scope.row.companyType.memberMaximum}}
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="150" v-if="!isComponent">
            <template #default="scope">
              ...
              <el-button link style="color:orange;" v-if="scope.row.status === 'normal'" @click="disable(scope.row)">禁用</el-button>
              <el-button link style="color:#0d9bff;" v-if="scope.row.status === 'invalid'" @click="enable(scope.row)">启用</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog title="编辑会员公司" v-model="showMemberCompanyFormDialog" :before-close="hideMemberCompanyForm">
      <el-form :model="memberCompany" :rules="memberCompanyRules" ref="memberCompanyRef">
        <el-form-item label="名称：" label-width="150px" prop="name">
          <el-input  v-model="memberCompany.name" placeholder="请输入名称" autocomplete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="排序：" label-width="150px" prop="sortOrder">
          <el-input  v-model="memberCompany.sortOrder" placeholder="请输入排序，数值越大越靠前" autocomplete="off"></el-input>
        </el-form-item>
