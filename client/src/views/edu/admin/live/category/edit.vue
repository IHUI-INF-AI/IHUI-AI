  ...
  <div class="app-container">
    <form ref="categoryRef" @submit.prevent>
      ...
      <div class="mb-4">
        ...
        <div>
          <Switch id="isShow" v-model="category.isShow" />
        </div>
      </div>
      <div class="mb-4">
        <label class="mb-1 block text-sm font-medium text-foreground">是否在首页显示</label>
        <div>
          <Switch id="isShowIndex" v-model="category.isShowIndex" />
        </div>
      </div>
    </form>
...
const { findCategoryList, toTree, getCategory, saveCategory, updateCategory } = liveApi
  ...
  export default {
    ...
    setup(props) {
      ...
      const uploadData = {
        url: '/api/v1/edu' + "/oss/channel/category/image",
        files: []
      }
      const rules = {
        ...
        categoryRef.value.validate(valid => {
          ...
          if (category.value.id) {
            updateCategory(category.value, (res) => {
              success("编辑成功")
              router.push({path: "/admin/edu/live/channel/category", query:{ id: res["id"]}});
