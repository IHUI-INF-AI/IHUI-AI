<template>
  <div class="content-container">
    <el-row class="row">
      <el-col :span="4">
        <member-menu active="follow"/>
      </el-col>
      <el-col :span="20" v-if="list">
        <div class="msg-list" v-loading="listLoading">
          <el-empty v-if="!(list && list.length)"/>
          <el-row :gutter="24" v-else>
            <el-col :span="6" v-for="item in list" :key="item.id">
              <div class="msg-item">
                <a class="msg-item-avatar" target="_blank">
                  <img :src="item.followMember.avatar" class="msg-item-avatar-item">
                </a>
                <div class="user-name">
                  {{item.followMember.name}}
                </div>
                <div class="button-group">
                  <el-button @click="follow(item)">{{item.each ? '互相关注' : '取消关注'}}</el-button>
...
import {getFollowMemberList, unfollowMember} from "@/api/edu/web/member";
...
export default {
  ...
  setup() {
    ...
    const loadList = function() {
      ...
      getFollowMemberList(params.value, res => {
        list.value = res.list;
    ...
    loadList()
    const follow = (item) => {
      unfollowMember(item.followMember.id, () => {
        success("取消关注成功")
        loadList()
      })
    }
    const privateLetter = (item) => {
      router.push({path: "/edu/message/private-letter", query: {memberId: item.followMember.id}})
    }
