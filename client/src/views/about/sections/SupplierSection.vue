    ...
    >
      ...
      <template #footer>
        <div class="dialog-actions">
          <button type="button" class="btn-reset" @click="handleReset">{{ t('becomeSupplier.form.reset') }}</button>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.error('[SupplierSection] 提交失败', e)
...
/* ---------- 暗色模式覆盖 ---------- */
:where(html.dark) .supplier-section .portal-hero {
  h2 { color: var(--el-text-color-primary); }
  .subtitle { color: var(--el-text-color-regular); }
}

:where(html.dark) .supplier-section .benefit-card {
  background: var(--app-surface-2);
...
:where(html.dark) .supplier-section .section-label::after {
  background: var(--el-border-color);
}

:where(html.dark) .supplier-section .step-card {
  background: var(--app-surface-2);
  border-color: var(--border-unified-color);
...
:where(html.dark) .supplier-section .apply-section .btn-apply-large {
...
:where(html.dark) .supplier-section .supplier-dialog {
  :deep(.el-dialog) {
    background: var(--el-bg-color);
    border: var(--unified-border);
  }
  ...
  :deep(.el-dialog__header) {
    ...
    .el-dialog__title { color: var(--el-text-color-primary); }
  }
...
:where(html.dark) .supplier-section :deep(.tech-form) {
...
:where(html.dark) .supplier-section .dialog-actions {
  .btn-submit {
    background: var(--el-bg-color);
    color: var(--el-text-color-primary);
