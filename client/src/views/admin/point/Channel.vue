<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('point.searchPlaceholder.channel')"
    :selectable="true"
    @search="onSearch"
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.pointChannel(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.pointChannelCreate(data),
  updateFn: (id, data) => adminApi.pointChannelUpdate(id, data),
  deleteFn: (id) => adminApi.pointChannelDelete(id),
  batchDeleteFn: (ids) => adminApi.pointChannelBatchDelete(ids),
  onSuccess: reload,
})
