<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('point.searchPlaceholder.list')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'name', dataKey: 'name', title: '商品名称', width: 220 },
  { key: 'points', dataKey: 'points', title: '积分', width: 100 },
  { key: 'stock', dataKey: 'stock', title: '库存', width: 100 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.pointList({ current: page.value, size: size.value, keyword: keyword.value })
