<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('point.searchPlaceholder.record')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'action', dataKey: 'action', title: '行为', width: 140 },
  { key: 'change', dataKey: 'change', title: '变更', width: 100 },
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.pointRecord({ current: page.value, size: size.value, keyword: keyword.value })
