<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('circle.searchPlaceholder.dynamic')"
    @search="onSearch"
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.circleDynamicList({ current: page.value, size: size.value, keyword: keyword.value })
