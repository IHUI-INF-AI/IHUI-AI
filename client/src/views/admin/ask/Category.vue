<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('ask.searchPlaceholder.category')"
    :selectable="true"
    @search="onSearch"
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.askCategoryList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.askCategoryCreate(data),
  updateFn: (id, data) => adminApi.askCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.askCategoryDelete(id),
  batchDeleteFn: (ids) => adminApi.askCategoryBatchDelete(ids),
  onSuccess: reload,
})
