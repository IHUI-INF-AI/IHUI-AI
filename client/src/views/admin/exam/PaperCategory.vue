<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('exam.searchPlaceholder.paperCategory')"
    :selectable="true"
    @search="onSearch"
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.examPaperCategory(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.examPaperCategoryCreate(data),
  updateFn: (id, data) => adminApi.examPaperCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.examPaperCategoryDelete(id),
  batchDeleteFn: (ids) => adminApi.examPaperCategoryBatchDelete(ids),
  onSuccess: reload,
})
