<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('exam.searchPlaceholder.questionCategory')"
    :selectable="true"
    @search="onSearch"
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.examQuestionCategoryCreate(data),
  updateFn: (id, data) => adminApi.examQuestionCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.examQuestionCategoryDelete(id),
