<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('exam.searchPlaceholder.questionJudgment')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'answer', dataKey: 'answer', title: '答案', width: 80 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.examQuestionJudgment({ current: page.value, size: size.value, keyword: keyword.value })
