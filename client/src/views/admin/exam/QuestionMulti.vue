<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('exam.searchPlaceholder.questionMulti')"
    @search="onSearch"
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.examQuestionMulti({ current: page.value, size: size.value, keyword: keyword.value })
