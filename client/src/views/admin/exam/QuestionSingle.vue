<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('exam.searchPlaceholder.questionSingle')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'title', dataKey: 'title', title: '题干', width: 300 },
  { key: 'difficulty', dataKey: 'difficulty', title: '难度', width: 80 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.examQuestionSingle({ current: page.value, size: size.value, keyword: keyword.value })
