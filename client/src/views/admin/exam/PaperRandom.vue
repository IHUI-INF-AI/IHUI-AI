<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('exam.searchPlaceholder.paperRandom')"
    @search="onSearch"
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.examPaperRandom({ current: page.value, size: size.value, keyword: keyword.value })
