<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('search.searchPlaceholder.hot')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'keyword', label: t('adminCommon.label.hotKeyword'), required: true, minLength: 1, maxLength: 100 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.searchHot(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.searchHotCreate(data),
  updateFn: (id, data) => adminApi.searchHotUpdate(id, data),
  deleteFn: (id) => adminApi.searchHotDelete(id),
  batchDeleteFn: (ids) => adminApi.searchHotBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'keyword', dataKey: 'keyword', title: t('adminCommon.label.hotKeyword'), width: 220 },
  { key: 'searchCount', dataKey: 'searchCount', title: '搜索数', width: 120 },
  {
    key: 'actions',
