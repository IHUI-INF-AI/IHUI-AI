<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('article.searchPlaceholder.category')"
    :selectable="true"
    @search="onSearch"
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.articleCategoryList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.articleCategoryCreate(data),
  updateFn: (id, data) => adminApi.articleCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.articleCategoryDelete(id),
  batchDeleteFn: (ids) => adminApi.articleCategoryBatchDelete(ids),
  onSuccess: reload,
})
