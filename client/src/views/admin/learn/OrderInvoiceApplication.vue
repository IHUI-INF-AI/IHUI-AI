<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('learn.searchPlaceholder.orderInvoiceApplication')"
    @search="onSearch"
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.learnOrderInvoiceApplication({ current: page.value, size: size.value, keyword: keyword.value })
