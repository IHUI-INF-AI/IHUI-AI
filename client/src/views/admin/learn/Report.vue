<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('learn.searchPlaceholder.report')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'completion', dataKey: 'completion', title: '完成率', width: 100 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.learnReportLesson({ current: page.value, size: size.value, keyword: keyword.value })
