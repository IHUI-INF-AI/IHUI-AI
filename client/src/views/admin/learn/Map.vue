<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('learn.searchPlaceholder.map')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: t('adminCommon.label.pathName'), required: true, minLength: 1, maxLength: 50 },
  { prop: 'lessonCount', label: t('adminCommon.label.lessonCount'), type: 'number', min: 0, max: 999 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.learnMapList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.learnMapCreate(data),
  updateFn: (id, data) => adminApi.learnMapUpdate(id, data),
  deleteFn: (id) => adminApi.learnMapDelete(id),
  batchDeleteFn: (ids) => adminApi.learnMapBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'name', dataKey: 'name', title: t('adminCommon.label.pathName'), width: 220 },
  { key: 'lessonCount', dataKey: 'lessonCount', title: t('adminCommon.label.lessonCount'), width: 100 },
  {
    key: 'actions',
