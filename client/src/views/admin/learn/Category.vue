<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('learn.searchPlaceholder.category')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  ...
  { prop: 'parentId', label: t('adminCommon.label.parentId'), type: 'number', min: 0, max: 9999999 },
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.learnCategoryList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.learnCategoryCreate(data),
  updateFn: (id, data) => adminApi.learnCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.learnCategoryDelete(id),
  batchDeleteFn: (ids) => adminApi.learnCategoryBatchDelete(ids),
  onSuccess: reload,
})
...
const columns: Column<unknown>[] = [
  ...
  { key: 'parentId', dataKey: 'parentId', title: t('adminCommon.label.parentId'), width: 100 },
