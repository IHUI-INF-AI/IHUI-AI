<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('learn.searchPlaceholder.orderInvoiceTitle')"
    @search="onSearch"
...
const columns: Column<unknown>[] = [
  ...
  { key: 'title', dataKey: 'title', title: '抬头', width: 220 },
  { key: 'taxNo', dataKey: 'taxNo', title: '税号', width: 180 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.learnOrderInvoiceTitle({ current: page.value, size: size.value, keyword: keyword.value })
