<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('learn.searchPlaceholder.lessonTrash')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'deletedAt', dataKey: 'deletedAt', title: '删除时间', width: 180 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.learnLessonTrash({ current: page.value, size: size.value, keyword: keyword.value })
