<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('resource.searchPlaceholder.category')"
    :selectable="true"
    @search="onSearch"
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.resourceCategory(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.resourceCategoryCreate(data),
  updateFn: (id, data) => adminApi.resourceCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.resourceCategoryDelete(id),
  batchDeleteFn: (ids) => adminApi.resourceCategoryBatchDelete(ids),
  onSuccess: reload,
})
