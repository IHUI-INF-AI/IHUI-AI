<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('resource.searchPlaceholder.tag')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: t('adminCommon.label.tagFullName'), required: true, minLength: 1, maxLength: 50 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.resourceTag(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.resourceTagCreate(data),
  updateFn: (id, data) => adminApi.resourceTagUpdate(id, data),
  deleteFn: (id) => adminApi.resourceTagDelete(id),
  batchDeleteFn: (ids) => adminApi.resourceTagBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'name', dataKey: 'name', title: t('adminCommon.label.tagFullName'), width: 180 },
  {
    key: 'actions',
