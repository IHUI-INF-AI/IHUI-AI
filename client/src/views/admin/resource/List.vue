<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('resource.searchPlaceholder.list')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'name', dataKey: 'name', title: '资源名称', width: 220 },
  { key: 'category', dataKey: 'category', title: '分类', width: 120 },
  { key: 'size', dataKey: 'size', title: '大小', width: 100 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.resourceList({ current: page.value, size: size.value, keyword: keyword.value })
