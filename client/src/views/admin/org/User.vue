<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('org.searchPlaceholder.user')"
    @search="onSearch"
...
const columns: Column<any>[] = [
  ...
  { key: 'department', dataKey: 'department', title: '部门', width: 180 },
  { key: 'position', dataKey: 'position', title: '岗位', width: 140 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.orgUserList({ current: page.value, size: size.value, keyword: keyword.value })
