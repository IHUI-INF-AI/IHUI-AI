<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('org.searchPlaceholder.department')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: t('adminCommon.label.departmentName'), required: true, minLength: 1, maxLength: 50 },
  { prop: 'manager', label: t('adminCommon.label.manager'), maxLength: 200 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.orgDepartmentList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.orgDepartmentCreate(data),
  updateFn: (id, data) => adminApi.orgDepartmentUpdate(id, data),
  deleteFn: (id) => adminApi.orgDepartmentDelete(id),
  batchDeleteFn: (ids) => adminApi.orgDepartmentBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'name', dataKey: 'name', title: t('adminCommon.label.departmentName'), width: 180 },
  { key: 'manager', dataKey: 'manager', title: t('adminCommon.label.manager'), width: 140 },
