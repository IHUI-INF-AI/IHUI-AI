<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('live.searchPlaceholder.channel')"
    @search="onSearch"
...
const columns: Column<unknown>[] = [
  ...
  { key: 'name', dataKey: 'name', title: '频道名称', width: 220 },
  { key: 'lecturer', dataKey: 'lecturer', title: '讲师', width: 140 },
...
const reload = async () => {
  loading.value = true
  try {
    const res = await adminApi.liveChannelList({ current: page.value, size: size.value, keyword: keyword.value })
