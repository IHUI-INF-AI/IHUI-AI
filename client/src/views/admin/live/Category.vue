<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('live.searchPlaceholder.category')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: '分类名称', required: true, minLength: 1, maxLength: 50 },
  { prop: 'sort', label: '排序', type: 'number', min: 0, max: 9999 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.liveCategoryList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.liveCategoryCreate(data),
  updateFn: (id, data) => adminApi.liveCategoryUpdate(id, data),
  deleteFn: (id) => adminApi.liveCategoryDelete(id),
  batchDeleteFn: (ids) => adminApi.liveCategoryBatchDelete(ids),
  onSuccess: reload,
})
...
const columns: Column<any>[] = [
  ...
  { key: 'sort', dataKey: 'sort', title: '排序', width: 100 },
  {
    key: 'actions',
    title: '操作',
    width: 180,
