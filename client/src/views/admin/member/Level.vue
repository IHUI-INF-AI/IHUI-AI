<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('member.searchPlaceholder.level')"
    :selectable="true"
    @search="onSearch"
...
const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.memberLevelList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.memberLevelCreate(data),
  updateFn: (id, data) => adminApi.memberLevelUpdate(id, data),
  deleteFn: (id) => adminApi.memberLevelDelete(id),
  batchDeleteFn: (ids) => adminApi.memberLevelBatchDelete(ids),
  onSuccess: reload,
})
