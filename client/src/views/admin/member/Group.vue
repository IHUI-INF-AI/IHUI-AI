<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('member.searchPlaceholder.group')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: t('adminCommon.label.groupName'), required: true, minLength: 1, maxLength: 50 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.memberGroupList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.memberGroupCreate(data),
  updateFn: (id, data) => adminApi.memberGroupUpdate(id, data),
  deleteFn: (id) => adminApi.memberGroupDelete(id),
  batchDeleteFn: (ids) => adminApi.memberGroupBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'name', dataKey: 'name', title: t('adminCommon.label.groupName'), width: 180 },
  { key: 'memberCount', dataKey: 'memberCount', title: t('adminCommon.label.memberCountInRole'), width: 100 },
  { key: 'createdAt', dataKey: 'createdAt', title: '创建时间', width: 180 },
  {
    key: 'actions',
