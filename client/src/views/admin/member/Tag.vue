<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('member.searchPlaceholder.tag')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: t('adminCommon.label.tagName'), required: true, minLength: 1, maxLength: 50 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.memberTagList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.memberTagCreate(data),
  updateFn: (id, data) => adminApi.memberTagUpdate(id, data),
  deleteFn: (id) => adminApi.memberTagDelete(id),
  batchDeleteFn: (ids) => adminApi.memberTagBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'name', dataKey: 'name', title: t('adminCommon.label.tagName'), width: 180 },
  { key: 'usage', dataKey: 'usage', title: '引用', width: 100 },
  {
    key: 'actions',
