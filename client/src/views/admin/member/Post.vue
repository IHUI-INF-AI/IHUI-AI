<template>
  <AdminTableV2
    ...
    :loading="loading"
    :keyword="keyword"
    :search-placeholder="t('member.searchPlaceholder.post')"
    :selectable="true"
    @search="onSearch"
...
const formFields: FormField[] = [
  { prop: 'name', label: t('adminCommon.label.postName'), required: true, minLength: 1, maxLength: 50 },
]

const { keyword, page, size, total, loading, list, reload, onSearch, onPageChange } = useAdminTable({
  fetchFn: (params) => adminApi.memberPostList(params),
})
...
const { dialogVisible, dialogMode, formData, submitting, onAdd, onEdit, onDelete, onBatchDelete, onSubmit, onSubmitContinue, batchEditVisible, batchEditRows, batchEditProgress, onBatchEdit, onBatchEditSubmit, onBatchEditRetry } = useAdminCrud({
  fields: formFields,
  createFn: (data) => adminApi.memberPostCreate(data),
  updateFn: (id, data) => adminApi.memberPostUpdate(id, data),
  deleteFn: (id) => adminApi.memberPostDelete(id),
  batchDeleteFn: (ids) => adminApi.memberPostBatchDelete(ids),
  onSuccess: reload,
})

const columns: Column<unknown>[] = [
  { key: 'id', dataKey: 'id', title: 'ID', width: 80 },
  { key: 'name', dataKey: 'name', title: t('adminCommon.label.postName'), width: 180 },
  { key: 'count', dataKey: 'count', title: '人数', width: 100 },
  {
    key: 'actions',
