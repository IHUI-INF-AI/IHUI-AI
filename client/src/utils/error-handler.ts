...
/**
 * 确认Error handled（显示确认对话框）
 */
export async function handleErrorWithConfirm(
  error: unknown,
  options: {
    title?: string
    message?: string
  ...
  const errorInfo = handleError(error, { showMessage: false, logError: true })
  const errorMessage = message || errorInfo.message

  try {
    await ElMessageBox.confirm(errorMessage, title, {
...
export function createErrorBoundaryHandler(componentName: string) {
  return (error: unknown, errorInfo?: { componentStack?: string }) => {
    ...
    ElMessage.error(t('errorBoundary.componentError', { component: componentName }) || `Component ${componentName} encountered an error, please refresh the page and retry`)
  }
}
