...
async function loadAll<T = unknown>(name: string): Promise<T[]> {
...
/** 通用分页查询 */
export async function querySeed<T = unknown>(
  name: string,
...
/** 单条查询 */
export async function getSeed<T = unknown>(name: string, id: number | string): Promise<T | null> {
  const all = await loadAll<T>(name)
  return all.find((it) => {
...
/** 配置类单对象查询 */
export async function getConfig<T = unknown>(name: string): Promise<T | null> {
  const res = await fetch(`${BASE}/${name}.json`)
...
): Promise<{ success: boolean; code: number; data: { list: unknown[]; total: number }; message: string; timestamp: number }> {
