...
describe('seedData', () => {
  ...
  it('querySeed filter', async () => {
    ...
    const r = await api.querySeed('items', { filter: (it: any) => it.v === 1 })
    expect(r.data.total).toBe(1)
  })

  it('querySeed fetch 失败返回空', async () => {
    ;(globalThis as any).fetch = vi.fn(() => Promise.resolve({ ok: false, json: () => Promise.resolve([]) })) as any
  ...
  it('getConfig 失败', async () => {
    ;(globalThis as any).fetch = vi.fn(() => Promise.resolve({ ok: false, json: () => Promise.resolve(null) })) as any
    const r = await api.getConfig('cfg')
    expect(r).toBeNull()
  })
