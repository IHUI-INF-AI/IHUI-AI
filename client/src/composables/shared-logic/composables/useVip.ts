...
export function useVip() {
  const { userInfo, isLoggedIn: _isLoggedIn } = useUser()
  ...
  async function checkOrderStatus(orderId: string) {
    const res = await request<{ orderId: string; status: string }>({ url: `/api/vip/order/status/${orderId}`, method: 'GET' })
    return res.data
  }

  async function purchaseVip(planId: string, _payMethod: 'wechat' | 'alipay' = 'wechat') {
