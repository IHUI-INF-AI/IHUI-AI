...
import * as api from '../distribution/subordinates'
...
describe('subordinates', () => {
  ...
  it('getSubordinates 应能正常调用', async () => {
    const fn = (api as any).getSubordinates
  ...
  it('getUserAndChildrenOrders 应能正常调用', async () => {
    const fn = (api as any).getUserAndChildrenOrders
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
