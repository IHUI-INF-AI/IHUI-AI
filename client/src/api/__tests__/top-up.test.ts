...
import * as api from '../payment/top-up'
...
describe('top-up', () => {
  ...
  it('createTopUpOrder 应能正常调用', async () => {
    const fn = (api as any).createTopUpOrder
  ...
  it('getTopUpStatus 应能正常调用', async () => {
    const fn = (api as any).getTopUpStatus
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
