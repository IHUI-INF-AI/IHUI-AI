...
describe('file-upload', () => {
  ...
  it('uploadFormFile 应能正常调用', async () => {
    const fn = (api as any).uploadFormFile
  ...
  it('uploadAgentAndCreateExamine 应能正常调用', async () => {
    const fn = (api as any).uploadAgentAndCreateExamine
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
