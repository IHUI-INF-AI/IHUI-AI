...
describe('agent-category-cache', () => {
  ...
  it('getCacheInfo 应能正常调用', async () => {
    const fn = (api as any).getCacheInfo
  ...
  it('reloadCache 应能正常调用', async () => {
    const fn = (api as any).reloadCache
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
  ...
  it('convertIdsToNames 应能正常调用', async () => {
    const fn = (api as any).convertIdsToNames
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
  ...
  it('getCategories 应能正常调用', async () => {
    const fn = (api as any).getCategories
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
