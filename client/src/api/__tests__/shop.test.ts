...
import * as api from '../distribution/shop'
...
describe('shop', () => {
  ...
  it('selectsGoods 应能正常调用', async () => {
    const fn = (api as any).selectsGoods
  ...
  it('getactivity 应能正常调用', async () => {
    const fn = (api as any).getactivity
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
