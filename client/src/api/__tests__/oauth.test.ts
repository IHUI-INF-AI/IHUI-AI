...
describe('oauth', () => {
  ...
  it('createAlipayOrder 应能正常调用', async () => {
    const fn = (api as any).createAlipayOrder
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
  ...
  it('createWeChatOrder 应能正常调用', async () => {
    const fn = (api as any).createWeChatOrder
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
  ...
  it('createCardPaymentOrder 应能正常调用', async () => {
    const fn = (api as any).createCardPaymentOrder
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
