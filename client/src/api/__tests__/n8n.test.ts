...
describe('n8n', () => {
  ...
  it('addN8nAgent 应能正常调用', async () => {
    const fn = (api as any).addN8nAgent
  ...
  it('processN8nAgent 应能正常调用', async () => {
    const fn = (api as any).processN8nAgent
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
