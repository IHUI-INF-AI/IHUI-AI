...
describe('aiModelInfo', () => {
  ...
  it('getAIModelList 应能正常调用', async () => {
    const fn = (api as any).getAIModelList
  ...
  it('addAIModel 应能正常调用', async () => {
    const fn = (api as any).addAIModel
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
  ...
  it('updateAIModel 应能正常调用', async () => {
    const fn = (api as any).updateAIModel
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
  ...
  it('deleteAIModel 应能正常调用', async () => {
    const fn = (api as any).deleteAIModel
    expect(typeof fn).toBe('function')
    try {
      const result = await fn()
      expect(result).toBeDefined()
