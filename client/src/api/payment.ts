/**
 * api/payment.ts - 智能体广场列表/分类/收藏/点赞接口
 *
...
function extractData<T>(response: unknown): { data?: T } {
...
function extractMessage(response: unknown): { message?: string } {
...
// ============ 支付订单查询/关闭/同步（统一入口，转发到 ali-pay / unified-wechat） ============
...
export async function checkPaymentStatus(orderNo: string): Promise<{ data?: { status?: string; paid?: boolean } }> {
  ...
  const body = (response as { data?: ApiEnvelope<{ status?: string; paid?: boolean }> })?.data
  return { data: body?.data }
}
...
export async function syncPaymentStatus(orderNo: string): Promise<{ data?: { verified?: boolean } }> {
  ...
  const body = (response as { data?: ApiEnvelope<{ verified?: boolean }> })?.data
  return { data: body?.data }
}
...
export async function verifyPaymentCallback(params: Record<string, unknown>): Promise<{ data?: { valid?: boolean } }> {
  ...
  const body = (response as { data?: ApiEnvelope<{ valid?: boolean }> })?.data
  return { data: body?.data }
}
