...
export const learnApi = {
  ...
  chapterList: (lessonId: string) =>
    http.get<ApiResponse<Chapter[]>>(`/learn/lesson/${lessonId}/chapter/list`),

  // 报名(sign-up)
  signUp: (lessonId: string) => http.post<ApiResponse<SignUp>>('/learn/sign-up', { lessonId }),
  cancelSignUp: (sid: string) =>
    http.put<ApiResponse<void>>(`/learn/sign-up/${sid}/cancel`),

  // 学习记录
  recordSave: (data: { lessonId: string; duration: number; progress: number }) =>
    http.post<ApiResponse<LearnRecord>>('/learn/record/upsert', data),
  recordUpdate: (rid: string, data: { progress: number; lastTime?: string }) =>
    http.put<ApiResponse<LearnRecord>>(`/learn/record/${rid}/complete`, data),
  ...
  // 作业
  homeworkList: (lessonId: string, params?: PaginationParams) =>
    http.get<ApiResponse<PaginationResponse<Homework>>>(`/learn/homework/lesson/${lessonId}`, { params }),
  ...
  // 学习地图
  mapList: () => http.get<ApiResponse<LearnCategory[]>>('/learn/learn-map/list'),
  mapDetail: (mid: string) => http.get<ApiResponse<LearnCategory>>(`/learn/learn-map/${mid}`),
}
