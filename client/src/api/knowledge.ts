...
eduApiClient.interceptors.request.use(
  (config) => {
    // 从 StorageManager 获取认证 token
    // 优先级：accessToken > user_token > token
    const token = String(StorageManager.getItem(STORAGE_KEYS.ACCESS_TOKEN) || '') ||
                  String(StorageManager.getItem(STORAGE_KEYS.USER_TOKEN) || '') ||
                  String(StorageManager.getItem(STORAGE_KEYS.TOKEN) || '')
    if (token) {
...
> {
  try {
    ...
    // 提取实际数据
    let actualData: { list?: any[], total?: number, current?: number, size?: number, pages?: number }
    ...
    const items: KnowledgeBase[] = rawItems.map((item: any) => {
...
> {
  ...
  if (response.success && response.data) {
    const data = response.data as { list?: any[]; rows?: any[]; total?: number }
...
export async function searchKnowledge(
  ...
  if (response.success && response.data) {
    const data = response.data as { list?: any[]; rows?: any[] }
    const rawItems = data.list || data.rows || (Array.isArray(response.data) ? response.data as unknown[] : [])
    
    const results: KnowledgeSearchResult[] = rawItems.map((item: any) => {
      const resource = item as Record<string, unknown>
      return {
