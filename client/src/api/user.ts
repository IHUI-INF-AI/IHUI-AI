import { COZE_PATHS, LOGIN_PWD_PATHS, ADMIN_LOGIN_PATHS } from '@/config/backend-paths'
...
export const getUserCoins = withApiResponseHandler(
  async (params: {
    userId: string
  }): Promise<ApiResponse<{ balance: number; frozen?: number; recharge_options?: unknown[] }>> => {
    const response = await request.get<{
      balance: number
      frozen?: number
      recharge_options?: unknown[]
...
/**
 * 后台管理员账号登录 (admin_user 表, 区别于普通用户 /auth/login 链路)
 *
 * 对应后端: POST /api/v1/login/username (Query 参数: username, password)
 * 返回字段为 snake_case, 此处映射为前端 LoginResponseData 期望的 camelCase 结构,
 * 使 stores/auth/index.ts 的 login() 主流程无需感知差异。
 *
 * 映射关系:
 *   access_token  -> token / accessToken / userToken
 *   refresh_token -> refreshToken
 *   user_id       -> id / uuid
 *   user_name     -> username
 *   nick_name     -> nickname
 *   roles         -> roles (注入 userStore, 触发 permissionsStore 放权)
 *   permissions   -> permissions (admin 通配 "*")
 *
 * 注意: 不使用 withApiResponseHandler 包装, 该包装会吞掉 throw 返回错误响应,
 * 导致上层拿到空 token 静默失败。此处失败时显式抛异常, 由上层 catch 给用户提示。
 */
export const adminLogin = async (data: {
  username: string
  password: string
}): Promise<ApiResponse<LoginResponseData>> => {
  // 2026-06-28 联调: 改 Query→Body, 对齐后端 username_login.py 的 Body(embed=True) 参数
  // (项目硬约束: API endpoints must use Body parameters instead of Query parameters)
  const response = await request.post<LoginResponseData>(ADMIN_LOGIN_PATHS.login, {
    username: data.username,
    password: data.password,
  })
  const normalized = normalizeApiResponse(response) as ApiResponse<LoginResponseData>
  // 后端 code=0 表示成功 (normalizeApiResponse 已识别), 失败时 code 非 0
  if (!normalized.success || !normalized.data) {
    throw new Error(normalized.message || '管理员登录失败')
  }
  // snake_case -> camelCase, 适配 buildUserFromLoginResponse 与 token 提取逻辑
  const raw = normalized.data as unknown as {
    access_token?: string
  ...
  const userIdStr = raw.user_id != null ? String(raw.user_id) : ''
  normalized.data = {
    ...normalized.data,
    // token 三件套 (tokenStore.setToken 提取优先级: token > accessToken > userToken)
    ...
    nickname: raw.nick_name || raw.user_name || '',
    avatar: raw.avatar || '',
    roles: raw.roles || [],
