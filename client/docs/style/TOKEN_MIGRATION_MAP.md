...
### 2.1 2026-06-24 暗色模式修复（封版前）

本次封版前暗色模式修复涉及的 token / 硬编码颜色工作，均沿用第二节映射规则将残留硬编码颜色迁移为 `--el-*` / `--app-*` 语义化 token：

| 修复范围 | 说明 |
|---|---|
| 暗色模式 P0/P1/P2 | 修复暗色模式下高优先级色块显示问题 |
| 硬编码颜色 | 将残留硬编码颜色迁移为 `--el-*` / `--app-*` 语义化 token |
| ECharts 图表 | 适配暗色模式，图表配色跟随主题切换 |
| VIP 弹窗 / 支付页 | 修复暗色模式下色块对比与可读性 |
