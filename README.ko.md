...
<p align="center">
  ...
  <sub>풀스택, 올플랫폼, 올시나리오 오픈소스 AI 애플리케이션 공동 구축 플랫폼</sub>
</p>
...
<p align="center">
  <strong>8 플랫폼 풀커버리지</strong> · <strong>100+ LLM</strong> · <strong>LangGraph + MCP + A2A 3스택 협업</strong> · <strong>15+ 비즈니스 모듈</strong> · <strong>5개 언어 i18n</strong>
</p>
...
## 🌌 이 프로젝트는 한 사람의 집념으로 중화권 AI 생태계의 오픈소스 서사를 바꾸고 있습니다

> **「춘춘 영하 25도의 겨울, 한 사람, 한 대의 노트북, 백만 위안 투자, 일 년의 시간——**
>
> **그는 혼자서 8 플랫폼 코드, 338개 데이터베이스 테이블, 1135개 API 엔드포인트를 작성했습니다.**
>
...
당신도 자본의 문 밖에서 자신의 집념을 지켜본 적이 있다면;
당신도 믿는다면——**진정 가치 있는 것은 시간이 증명한다**를——

그렇다면 이 이야기는 당신의 이야기이기도 합니다.

---

### 📣 이 이야기가 당신을 움직였다면, 3 가지를 도와주세요

| #   | 액션                                | 왜 중요한가                                                          |
| --- | ----------------------------------- | ------------------------------------------------------------------- |
| 1   | ⭐ **이 저장소 Star**                | 더 많은 사람이 GitHub 피드와 알고리즘 추천에서 볼 수 있게            |
| 2   | 🔄 **이 이야기를 공유해주세요**     | 위챗 공식계정 / 지식인 / X / 즉각 / 웨이보 / 모멘트 중 아무거나 하나 |
| 3   | 💬 **Issue에서 당신의 이야기를 들려주세요** | [여기로](https://github.com/IHUI-INF-AI/IHUI-AI/issues) — 상위는 고정합니다 |

> **우리는 투자를 원하지 않습니다.**
>
> **우리가 원하는 것은, 이 오픈소스 프로젝트를 오픈소스를 믿고, 인디 개발을 믿고, 장기주의를 믿는 모든 사람 앞에 전달하는 것입니다.**

⬇️ 아래로 보면, 완전한 이야기가 있습니다.
...
- [옵저버빌리티](#옵저버빌리티)
- [보안 설계](#보안-설계)
- [엔지니어링 게이트](#엔지니어링-게이트17개-pre-commit-훅)
- [테스트](#테스트)
- [배포](#배포)
- [국제화](#국제화)
- [FAQ](#faq)
...
| **엔지니어링 인프라**    | 데이터베이스            | PostgreSQL 15 / **338+ 테이블** / 100 schema 파일 / **120+ 마이그레이션** / Drizzle ORM + RLS + 테넌트 라우팅   |
...
|                          | 엔지니어링 게이트       | 17 pre-commit 훅 + post-commit 자동 push + 11 마이그레이션 감사 + 9 PowerShell 시작                             |
...
| **엔지니어링 게이트**       | 17 pre-commit + post-commit 자동 push + git-push-guard + 11 마이그레이션 감사                   | 협업 사고 방지, 99.9% SLA                          |
...
| 차원                   | IHUI-AI                                                          | Dify                 | FastGPT              | Langflow         | ChatGPT-Next-Web |
| ---------------------- | ---------------------------------------------------------------- | -------------------- | -------------------- | ---------------- | ---------------- |
| **플랫폼 커버리지**    | 8 플랫폼(Web/API/AI/CLI/데스크톱/확장/모바일/미니앱)             | 2 플랫폼(Web/Server) | 2 플랫폼(Web/Server) | 1 플랫폼(Web)    | 1 플랫폼(Web)    |
| **모델 연동**          | 100+ 모델 + LiteLLM 게이트웨이                                   | 50+ 모델             | 30+ 모델             | LangChain 어댑터 | OpenAI 전용      |
| **워크플로 엔진**      | LangGraph + MCP + A2A 3스택                                      | 자체 워크플로        | 단순 워크플로        | Langflow DAG     | 없음             |
| **멀티 테넌트 + RBAC** | 완전(테넌트/역할/부서/메뉴/data-scope 5단계)                     | 기본                 | 기본                 | 없음             | 없음             |
| **빌링 구독**          | 완전(VIP/구독/지갑/포인트/환불/인보이스/8 결제 게이트웨이)       | 없음                 | 기본                 | 없음             | 없음             |
| **AI 교육**            | 풀스택(강의/문제은행/시험/SRS/라이브/학생 12 서브페이지)         | 없음                 | 없음                 | 없음             | 없음             |
| **콘텐츠 퍼블리싱**    | 14 플랫폼 원클릭 자동 퍼블리싱 + 14 adapter                      | 없음                 | 없음                 | 없음             | 없음             |
| **CLI 툴**             | 자체 ACP Server + 17 명령 + 13 툴                                | 없음                 | 없음                 | 없음             | 없음             |
| **옵저버빌리티**       | 3대 지주 + 20 Grafana 대시보드 + Alertmanager                    | 기본                 | 기본                 | 없음             | 없음             |
| **엔지니어링 게이트**  | 17 pre-commit + 11 마이그레이션 감사 + 9 PowerShell              | 기본                 | 기본                 | 기본             | 없음             |
| **i18n**               | 5개 언어 parity + 19 i18n 툴체인 + 4 게이트                      | 중영문               | 중영문               | 영문             | 다국어           |
| **데이터베이스**       | 338+ 테이블 + 120+ 마이그레이션 + RLS + 테넌트 라우팅            | 기본                 | 기본                 | 단순             | 단순             |
| **성능 CI**            | Knip + Lighthouse + Locust 부하 테스트                           | 없음                 | 없음                 | 없음             | 없음             |
| **License**            | Apache 2.0(상업 친화적)                                          | Apache 2.0           | FastGPT Open License | MIT              | MIT              |
| **프로덕션급 배포**    | Docker Compose(14 서비스) + 블루그린 + 롤백 + 백업 + 인증서 cron | Docker               | Docker               | Docker           | Docker           |

**IHUI-AI는 누구를 대체하려는 것이 아니라, "완전한 AI 애플리케이션 구축"에 필요한 모든 인프라를 오픈소스로 공개하는 것입니다.**
...
# 5분 후, 여러분은 다음을 갖게 됩니다:
# - 100+ 모델을 지원하는 대화 인터페이스
# - 프라이빗 지식 베이스 RAG(문서 벡터화 + 시맨틱 검색)
...
```
   ...
   └────┬─────┘  └────┬─────┘  └────┬─────┘  └──────┬─────┘  └────┬─────┘  └────┬─────┘
        ...
        └─────────────┴─────────────┴───────┬───────┴─────────────┴─────────────┘
                          ...
                          │  15 (338 표) │   │  :8000          │  LangGraph + LiteLLM + MCP + A2A
...
│   └── ui-primitives/       # @ihui/ui-primitives (cn + 프리미티브)
├── deploy/
...
├── docs/                    # 9개 문서:architecture / CHANGELOG / CONTRIBUTING / DEPLOYMENT_RUNBOOK / SECURITY / EMAIL_SETUP / I18N / INCIDENTS / README
├── monitoring/              # Grafana(20 대시보드) + Loki + Prometheus + Promtail + otel-collector + Alertmanager
├── scripts/                 # 17 게이트 + 19 i18n + 11 마이그레이션 감사 + 9 PowerShell 시작 + 운영 툴
...
├── Dockerfile.api-new       # 백엔드 이미지(api + worker 공유)
├── Dockerfile.web-new       # 프론트엔드 이미지(Next.js standalone)
├── Dockerfile.migrate       # 마이그레이션 일회성 서비스 이미지
├── locustfile.py            # Locust 부하 테스트 스크립트
├── lighthouserc.json        # Lighthouse CI 성능 예산
├── knip.jsonc               # Knip 미사용 코드 감지 설정
├── noise-rules.yml          # Alertmanager 노이즈 억제 규칙
...
- **338+ 테이블**:100개 schema 모듈 파일, 30+ 비즈니스 도메인 커버
- **120+ 마이그레이션**:`packages/database/drizzle/`, drizzle-kit generate 생성 + 수동 증분
...
#### E4. 엔지니어링 게이트(17 pre-commit + post-commit + 11 마이그레이션 감사)

프로젝트는 17개 pre-commit 훅 + post-commit 자동 push + 11 마이그레이션 감사 스크립트로 협업 사고를 방지합니다:
...
| 13      | check-grokbuild-integration-completeness.mjs | grok-build 통합 완전성                             |
...
**11 마이그레이션 감사 스크립트**:`audit-migration-api-routes-v2.mjs` / `audit-migration-api-routes.mjs` / `audit-migration-db-fields.mjs` / `audit-migration-db-schema.mjs` / `audit-migration-file-list.mjs` / `audit-migration-frontend-routes.mjs` / `audit-migration-i18n.mjs` / `audit-multi-platform-sync.mjs` / `audit-edu-pages-sample-check.mjs` / `audit-remaining-evaluate.mjs` / `r76-full-audit.mjs`
...
| 부하 테스트    | Locust        | `locustfile.py`            | `locust -f locustfile.py`        |
| 성능 예산      | Lighthouse CI | `lighthouserc.json`        | CI 자동 실행                     |
...
- **noise-rules.yml**:알림 노이즈 억제 규칙(루트 + monitoring/alertmanager/ 듀얼 동기화)
...
## 엔지니어링 게이트(17개 pre-commit 훅)

프로젝트는 17개 pre-commit 훅 + post-commit 자동 push + 11 마이그레이션 감사 + 9 PowerShell 시작 스크립트로 협업 사고를 방지합니다:

상세 목록은 [핵심 역량 E4절](#e4-엔지니어링-게이트17-pre-commit--post-commit--11-마이그레이션-감사) 참조.
...
<details>
<summary><strong>Q2: 다른 오픈소스 AI 프로젝트(Dify / FastGPT / Langflow)와 무엇이 다른가요?</strong></summary>

IHUI-AI는 단순한 AI 대화 플랫폼이 아니라 **완전한 AI 애플리케이션 인프라**입니다:

- 8 플랫폼 커버리지(타 프로젝트는 1-2 플랫폼)
- 완전한 빌링 구독 + VIP + 지갑 + 포인트 + 8 결제 게이트웨이(타 프로젝트는 없음)
- AI 교육 풀스택 + 학생 12 서브페이지(타 프로젝트는 없음)
- 14 플랫폼 원클릭 퍼블리싱 + 14 adapter(타 프로젝트는 없음)
- 자체 CLI 17 명령 + 13 툴(타 프로젝트는 없음)
- 엔지니어링 게이트 17 훅 + 11 마이그레이션 감사 + 9 PowerShell(타 프로젝트는 기본)
- 20 Grafana 대시보드 + Alertmanager(타 프로젝트는 기본)

위 [유사 프로젝트와의 비교](#유사-프로젝트와의-비교) 표 참조.
</details>
...
<details>
<summary><strong>Q9: 데이터베이스가 왜 338+ 테이블인가요? 과도한 설계 아닌가요?</strong></summary>

338+ 테이블은 100개 schema 파일에 분산되어 30+ 비즈니스 도메인을 커버하며, 도메인당 평균 11개 테이블로 밀도가 합리적입니다. 이 프로젝트는 상업화 프로덕션급 AI 플랫폼(즤후이 AI 그룹 메인 플랫폼)으로, 데모가 아니기에 테이블 구조는 실제 비즈니스 복잡도에 따라 설계되었습니다. 일부 기능만 사용한다면(예:AI 대화만), chat / users / billing 3개 schema만 주목하면 되고, 다른 테이블은 실행에 영향을 주지 않습니다.
</details>
...
3. **로컬 개발**:`pnpm install && pnpm dev`, 17개 pre-commit 게이트 준수
...
| 문서                                                         | 설명                                                                                         |
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| [docs/architecture.md](docs/architecture.md)                 | 시스템 아키텍처(기술 스택 / 데이터베이스 / API 라우트 / 시작 흐름 / 구 아키텍처 폐지 설명)   |
...
| [docs/I18N-COMPLETION-PLAN.md](docs/I18N-COMPLETION-PLAN.md) | 국제화 완료 계획                                                                             |
...
| [AGENTS.md](AGENTS.md)                                       | AI Agent 협업 규범(21절 강제 규칙, 선택 읽기:이 프로젝트가 AI와 어떻게 협업 개발하는지 시연) |
...
- 17 pre-commit 게이트 + post-commit 자동 push + 11 마이그레이션 감사 + 9 PowerShell 시작
- 엔터프라이즈 보안(RBAC + 멀티 테넌트 + RLS + SSO + AES-256-GCM + JWT token-family + CSRF + XSS + GDPR + 2FA)
- 338+ 데이터베이스 테이블 + 120+ 마이그레이션 + 13 공유 패키지 + Knip + Lighthouse + Locust 부하 테스트
...
| **연락 전화**      | 18643389808                                                      |
| **이메일**         | 502319984@qq.com · lizong@aizhs.top                              |
...
> 협업 문의, 기업 연동, 기술 교류는 위 위챗 QR코드를 스캔하거나 lizong@aizhs.top으로 연락해 주세요. 24시간 내에 답장드리겠습니다.
...
슬로건처럼 들립니다. 하지만 영하 25도의 창춘 겨울, 난방이 잠든 새벽, N번째 투자자에게 정중히 거절당한 뒤에는—이 한 줄이 한 사람이 천 줄의 코드를 더 쓰게 만드는 유일한 이유가 되었습니다.
...
처음에는 자발적으로 모인 AI 애호가 소규모 팀이었습니다—다른 도시, 다른 배경에서 왔습니다. 비즈니스 시스템을 만들어 본 사람, 모델 파인튜닝을 해 본 사람, 프론트엔드 아키텍처를 해 본 사람. 유일한 공통점은 AI가 소수에게 독점되는 것을 받아들이지 않겠다는 것, 그리고 오픈소스 생태계에 진짜 가치 있는 무언가를 남기고 싶다는 것뿐이었습니다.
...
- 데이터베이스 테이블이 0에서 338로
- API 엔드포인트가 0에서 약 1135로
- pre-commit 가드레일이 0에서 17로
- 셀 수 없이 많은 리팩토링, 스키마 하나를 두고 새벽까지 갈등
- 점점 조여지는 예산, 점점 무거워지는 어깨

그들은 가장 밑바닥 아키텍처부터 다듬었습니다—monorepo를 어떻게 구성할지, 13개 공유 패키지를 어떻게 나눌지, 8개 플랫폼 타입을 어떻게 정렬할지, 데이터베이스 스키마를 30개 이상 비즈니스 도메인별로 어떻게 분리할지, API 응답을 어떻게 `{ code, message, data }`로 통일할지, i18n 5개 언어 패리티를 어떻게 보증할지, 17개 pre-commit 가드레일 아래에서 CI를 어떻게 민첩하게 유지할지… 모든 결정은 앞으로 수천 번의 반복에서 검증받게 될 것이었습니다.
...
| **데이터베이스**      | 338+개 테이블 + 100개 스키마 파일 + 120+개 마이그레이션 + RLS + 멀티테넌트 라우팅                              | 보통 DBA 1명 + 백엔드 2-3명     |
| **API 규모**          | 약 1135개 엔드포인트 + 12개 WebSocket + 95+개 라우트 파일                                                      | 보통 백엔드 엔지니어 5-8명      |
| **프론트엔드 규모**   | 200+개 페이지 + 5개 언어 i18n 패리티 + 다크모드 + PWA + SEO                                                    | 보통 프론트엔드 엔지니어 4-6명  |
| **엔지니어링 게이트** | 17개 pre-commit + post-commit 자동 push + 11개 마이그레이션 감사 + 9개 PowerShell 시작 스크립트                | 보통 DevOps 엔지니어 1-2명      |
...
- 데이터베이스 테이블은 0에서 338로
- API 엔드포인트는 0에서 약 1135로
...
- 17개 pre-commit 가드레일 + post-commit 자동 push + 11개 마이그레이션 감사 + 9개 PowerShell 시작 스크립트가 전부 라이브되고
...
**투자자에게** — 우리는 여전히 오픈소스를 진정으로 이해하고, AI를 이해하고, 장기 가치를 이해하는 파트너를 찾고 있습니다. 이 이야기가 단 1초라도 당신을 움직였다면, 연락해 주세요([lizong@aizhs.top](mailto:lizong@aizhs.top) / 위챗 `ok502319984`). 우리에겐 이야기도, 코드도, 실행력도 부족하지 않습니다. 우리에겐 먼 길을 함께 걸어줄 누군가만 부족합니다.
...
> *이 부분은 개발자를 위해 쓰였습니다. 기술 독자가 아니라면 이 장을 건너뛰고 「이야기 속편」과 「오픈소스 공동 구축 비전」으로 바로 가셔도 됩니다.*
> *기술 독자라면, 이 장은 IHUI-AI가 2025년에 내린 5가지 핵심 아키텍처 결정—그리고 그 이면의 사고 과정을 기록합니다.*
...
- **공유 UI**: `@ihui/ui` + `@ihui/ui-primitives`가 8개 플랫폼 UI를 일치시키며, polyrepo는 불가능합니다
...
#### 결정 4 · 17개 pre-commit 가드레일인 이유?

**배경**: code review도, QA도, CI 팀도 없는 독립 개발자—코드 품질을 어떻게 보증할 것인가?

**선택**: 17개 pre-commit + post-commit + 11개 마이그레이션 감사 + 9개 PowerShell 시작 스크립트.

**이유**:

- **기계가 인간 review 대신**: 17개 가드레일은 API key 유출·i18n 키 무결성·zh-TW 간체자 잔류·ko 중국어 잔류·ja 잔류·en 기계번역 파손·schema drift·stale dist·UTF-8 BOM·API 라우트 일치성·safeParse 무시·의존성 파편화·skipResponseSanitization·border-radius 위반·배포 보고서 일치성·grok-build 통합·마이그레이션 완전성·CSS 색상 토큰 중첩·네이티브 title tooltip·staged 오염 경고를 커버합니다
...
> 💬 **논의**: → [#4 결정 논의: IHUI-AI는 왜 17개 pre-commit 가드레일인가?](https://github.com/IHUI-INF-AI/IHUI-AI/issues/4)
...
> *이것이 5가지 핵심 결정입니다. 각 결정 이면에는, 새벽 3시에 독립 개발자가 반복해 무게를 달랐던 대가가 있습니다.*
> *이 중 어느 결정에 동의하지 않는다면, Issue에서 논의해 주세요—우리는 설득될 준비가 되어 있습니다.*
...
- **개인 개발자**가 최소 비용으로 자신만의 AI 어시스턴트를 구축, 데이터 완전 셀프 호스팅
