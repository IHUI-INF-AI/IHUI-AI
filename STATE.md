...
- browser_use 自验（管理员账号登录，web:8801）：
  - 登录 PASS（密码页签，无需验证码）
...
**tag 同步慢/假死根因与根治（2026-09-04 补记）**：

- 根因①：本地 4919 tag 中 `lost-commit/*` 历史归档占 4018，`git push --tags` 单次
  协商 5000 ref 且无进度输出 → 形如假死；gitcode 镜像落后缺 4558 tag 放大了耗时。
- 根因②：commit message 临时文件放仓库根目录 → 反复触发 check-root-dir-clean
  白名单churn。
- **根治①**：`scripts/sync-tags.mjs`（常驻运维工具）——命名空间策略 + 60/批分批
  推送 + 逐批日志。**同步策略：origin 为权威备份仓保护全部命名空间；gitee/gitcode
  镜像只同步活命名空间**（backup/ saved/ recovery/ unreachable-commits-backup/
  版本 tag），历史归档不同步。**今后禁止裸 `git push --tags`，一律走 sync-tags.mjs**。
- **根治②**：commit 消息文件放 `.git/CC-MSG.tmp`（`.git/` 内不进守门视野），
  `.git-commit-msg-tmp.txt` 白名单项已移除，根目录临时文件白名单 churn 终结。

**浅克隆遮盖事故与全量丢失审计（2026-09-05 凌晨收官）**：

- **发现**：本地 `.git/shallow` 存在、main 仅 97 commit（boundary=bc664a5599）——某会话
  `git fetch --depth` 把本地历史砍穿，守门 fsck/patch-id 比对全部失效。
- **修复**：origin 大包传输两次断流（GitHub 1.6MB/min 且冻结）→ 改 **gitee 源
  `git fetch --unshallow` 3 分钟完成**，main 恢复 5844 commit。残留 shallow.lock/
  tmp_pack 已清。**规约：本项目禁止对 main 做 fetch --depth；发现 .git/shallow
  立即用 gitee 源 unshallow。**
- **丢失审计（三路验证，结论：零内容丢失）**：
  ① reflog 全扫描：13:54 `reset HEAD~1` 丢的 d07d9e3ae9（39 文件过度清理）已 merge
  回归 + 8 项误删由 b8c127a6d2 revert 恢复（GAP-PLAN.md/RN 脚本/水印守门均实测在位）；
  21:29 reset 丢的 8529bc5296 经 `git cherry` 证实与已推送 391db9e373 等价。
  ② patch-id 全量比对（691 个近期备份 commit vs main 5844）：173 精确等价 + 518 中
  510 个为 WIP/index stash 快照（被最终版自然取代）+ 8 个实义 commit 逐一核实：
  第四刀=f83c953d82、第五刀=bc664a5599（修订版在 main）；记忆评测/bench 验证的
  4 个关键文件全在 main 且版本更新（+5 行水印）；preserve-untracked 为空占位。
  ③ .wt6 工作树 68 个暂存"改动"全为水印重复注入污染（0 功能行），已丢弃并移除该
  worktree。
- **垃圾清查**：跟踪文件 10080 个中垃圾模式命中 0；根目录 3 个日志残留
  （__gate_result.txt/gate-output.log/push_tags.log）已删；大文件均为合法字体/发行包。
