...
| 6 | P2 | knip 结果落库 + commit 时校验 | ✅ done (commit 9b6ca3c6) | knip.json 变更触发 scan:knip:hints (5-10s) |
| 7 | P3 | 拆分 admin views 接入 components/api/ | ✅ done (6 个 view + 6 个路由 + 4 语言 i18n) | AppCard/GroupCard/PackageCard/BillingRecordCard/UsageTopStats/ApiMethodSearch+RequestBuilder+ResponseViewer 接入 |
| 8 | P3 | knip 排除动态 import 误报 | ✅ done (commit 198a6139) | Unused files 249→105 (-58%), Unused exports 1657→293 (-82%) |
...
| 2 | PENDING_COMPONENTS.md 存在且含 18 个 component | 文件 grep | ✅ 文件含 19 个 component (5 组分类) |
| 3 | 完整 e2e 回归全通过 | `npx playwright test` | pending (耗时过长, 先做关键守门) |
| 4 | AGENTS.md 含 port-drift + line-endings 章节 | grep 关键字 | ✅ 包含 (commit 9b6ca3c6 恢复) |
| 5 | clean.mjs 集成 _archive 清理 | `node scripts/clean.mjs --dry-run` | ✅ 集成 (commit 488f00fd) |
| 6 | lint-staged 含 knip 校验 | grep "knip" package.json | ✅ knip.json 变更触发 scan:knip:hints |
| 7 | 6 项核心守门全过 | typecheck/i18n/tokens/contrast/dead-code/knip | pending (进行中) |
