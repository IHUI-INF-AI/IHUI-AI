...
export async function metricsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { slug: string } }>(
    ...
    async (request, reply) => {
      ...
      const [apiCalls, aiTokens, storage] = await Promise.all([
        ...
        promQuery(request.log, `sum(rate(litellm_tokens_total{tenant_slug="${slug}"}[1h]))`),
        // 存储:所有客户容器的 fs usage
        promQuery(
          request.log,
  ...
  app.get<{ Params: { slug: string } }>(
    ...
    async (request, reply) => {
      ...
      const [cpu, memory, networkRx, networkTx] = await Promise.all([
        ...
        promQuery(request.log, `sum(container_memory_usage_bytes{name=~"customer-${slug}-.*"})`),
        promQuery(
          request.log,
  ...
  app.get('/admin/api/metrics/summary', async (request) => {
    const [cpu, memory] = await Promise.all([
      ...
      promQuery(request.log, 'sum by (name) (container_memory_usage_bytes{name=~"customer-.*"})'),
    ])
