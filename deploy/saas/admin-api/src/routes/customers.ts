...
export async function customerRoutes(app: FastifyInstance): Promise<void> {
    ...
    async (request, reply) => {
      ...
      const backups = readdirSync(backupsDir)
        ...
        .map((timestamp) => {
          ...
          const age = ageDays > 0 ? `${ageDays} 天前` : ageHours > 0 ? `${ageHours} 小时前` : '刚刚'
          return {
            timestamp,
  ...
  app.delete<{ Params: { slug: string; timestamp: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!tsParse.success) {
        return reply.status(400).send({ error: 'InvalidTimestamp', message: tsParse.error.message })
      }
      ...
      const result = await runScript('delete-backup.sh', [slug, timestamp], request.log)
      if (result.code !== 0) {
        return reply.status(500).send({
  ...
  app.post<{ Params: { slug: string } }>(
    '/admin/api/customers/:slug/pause',
    async (request, reply) => {
      const parse = SlugSchema.safeParse(request.params.slug)
      if (!parse.success) {
        return reply.status(400).send({ error: 'InvalidSlug', message: parse.error.message })
  ...
  // ==================== 恢复 ====================
  app.post<{ Params: { slug: string } }>(
    '/admin/api/customers/:slug/resume',
    async (request, reply) => {
      const parse = SlugSchema.safeParse(request.params.slug)
      if (!parse.success) {
        return reply.status(400).send({ error: 'InvalidSlug', message: parse.error.message })
  ...
  // ==================== 备份 ====================
  app.post<{ Params: { slug: string } }>(
    '/admin/api/customers/:slug/backup',
    async (request, reply) => {
      const parse = SlugSchema.safeParse(request.params.slug)
      if (!parse.success) {
        return reply.status(400).send({ error: 'InvalidSlug', message: parse.error.message })
