...
| 5   | coze_websocket_chat(推断)          | coze_zhs_py/api/websocket.py                          | `apps/api/src/plugins/ws-ai.ts:452` → `GET /ws/coze/chat`(代码注释明确"等价自 coze_zhs_py/api/websocket.py",完整 Coze WS 协议 + 3s 心跳) | ✅ 已等价实现                                                   |
