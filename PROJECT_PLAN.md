...
- [ ] O20c 生产 `pg_hba.conf` 本地链路是 **trust**(127.0.0.1 免密即可以 superuser 连接)。这与 O13"非超级用户角色 + 应用闸"的方向直接对冲:应用角色隔离挡住的是"经应用连上来的路径",挡不住"本机任意进程直连 8810"。改成 `scram-sha-256` 会同时影响每日备份任务(`IHUI-PG-BACKUP`)、psql 运维脚本、部署循环,爆炸半径是**本机全部数据库消费方**,需 owner 决策后再动;本会话只读确认,未擅改 **凭据核验(2026-09-24 EncodedCommand 只读实查)**:① backup-db.sh / health-check.sh / restore-db.sh 凭据全走 PGPASSWORD 环境变量或 ~/.pgpass(零硬编码)——trust 下空凭据能连,改 scram 后调用环境(NSSM/计划任务)必须先配凭据,否则备份/健康检查/恢复全断;② ihui-deploy.ps1 零 PG 直连不受影响;③ backup-pg-local.ps1 从 apps/api/.env 的 DATABASE_URL 解析凭据,scram 下仍可跑;④ **顺带发现既存缺陷:backup-pg-local.ps1 用 D:/ihui-pg/16/bin/pg_dump.exe 对 PG18 服务端,违反「pg_dump 只能向下兼容」铁律,需升级 18 的 pg_dump**。实施步骤:备份 pg_hba.conf → 配三脚本凭据 → 改 scram-sha-256 → pg_ctl reload → 手跑备份验证;等用户确认窗口后执行。 **已实施生效(2026-09-24 12:5x,owner 全权授权)**:pg_hba.conf 六条 trust 全部替换为 scram-sha-256 并 reload,运行态四重验证全 PASS——① 带密码(w + PGPASSWORD,凭据从生产 .env 现场解析,不外传)psql 连 ihui 角色成功;② 空密码被拒(fe_sendauth no password supplied);③ api health HTTP 200(应用连接串本就带密码,前置核验 hasPassword=True);④ 文件 0 trust/10 scram 且无 -w 的 psql 转为交互索密(运行态生效铁证)。备份 pg_hba.conf.bak-20260924 在 pgdata。教训:远程 psql 验证必须带 -w(无 -w 在认证失败时交互索密会挂死 SSH 会话,首次实施脚本即因此超时)。三个 bash 脚本人工跑时需传 PGPASSWORD=xxx(脚本既有用法),无自动调度任务故无自动化断链。。
...
- [x] ✅(2026-09-24) **D41 Office/PDF 产物预览(G-49)**:docx/pptx(含讲者备注)/xlsx(sheet 切换 + 选区)/pdf(页码)preview + preview/源码切换 + 不可用/过大/过期三态降级(对标 Qoder `data-artifact-preview-kind`)。共享层优先:先查 `packages/ui-react` 与既有 FilePreview,不得端内重造。**验收**:四态(可用/过大/过期/不支持)用例 + 与 `canOpenInWorkPanel` 互不冲突 + 大文件不内联走懒加载 **侦察定档(2026-09-24)**:pdfjs-dist ^6.2.108 已在 web 依赖(PDF 页码预览低阻),但 docx/xlsx 解析需装新依赖(pnpm-lock.yaml 共享热点,并行会话下 lockfile 变更高危)、pptx 无成熟纯 JS 渲染库(讲者备注可经 jszip 解 XML 取文本,幻灯片渲染需降级为"文本+备注大纲"形态)——**依赖选型(docx-preview vs mammoth、xlsx 库体积)与 pptx 降级形态需 owner 拍板后一次装齐,避免 lockfile 多次冲突**;message-file-preview.tsx 空闲可承接。 **实施落地(2026-09-24,owner 授权选型后)**:docx-preview/xlsx(SheetJS)/jszip 已装齐(commit 2210959a7cb,lockfile 34 行变更),office-preview.tsx 四态+懒加载+pdf 页码跳转落地,pptx 为授权降级"文本+备注大纲";10 用例+19 回归过。**D91 四类批注坐标随之解锁**(按预览器能力接批注坐标)。
...
- [x] ✅(2026-09-24)  **D77 对话流业务表单卡(G-106)**:在消息流内完成业务动作的可填表单——邮件撰写卡(收件人/抄送/密送/主题/回复至/正文 + **批准操作/拒绝操作**成对动作)、日历创建/更新卡(创建/保存 + 时间区间与多出席人折叠)。数据面需新事件 `form_request`{kind,fields[],actions[]} 与 `form_response`(进 D34 契约双份);渲染走 `question-dialog` 同族弹层还是内联卡由方案定,但**必须复用 `packages/ui-react` 表单件**(共享层优先,禁止端内自绘)。端覆盖按 §9 矩阵执行,miniapp/mobile-rn 若需豁免必须显式写理由。**验收**:两表单端到端(填→批准→后端落→状态回显)+ 拒绝路径不产生副作用 + 五语言词表 __收口(2026-09-24):business-forms.ts(BUSINESS_FORM_KINDS email/calendarEvent + BusinessFormSchema 与 form_request 事件形状对齐 {kind,fields[],actions[]} + validateFormValues 判定层校验(email 必填/格式、日历开始<结束/出席人≥1) + **动作成对硬约束 FORM_ACTION_PAIRS approve↔reject,拒绝路径 submitInvoked 恒 false 零副作用(专门负例)** + applyFormAction 状态机穷尽 switch 零 default + attendeesFoldNeeded 折叠判定)+ business-form-card.tsx(**复用 packages/ui-react 表单件**,邮件卡+日历卡两形状,不取数 onAction 注入)+ ai.pane.businessForms 7 组键×5 语言。子代理报告因响应截断丢失,产物由主会话全量亲验(测试/tsc/词包)。__剩余__:form_request/form_response 事件契约与后端落库(D34 同批)另票,端到端验收相应后置;miniapp/mobile-rn 端覆盖待矩阵排期__
...
- [x] ✅(2026-09-24) **D91 四类文档批注锚点分型(G-124,扩展 D87)**:Qoder 的批注不是单一"选中文字",而是四种定位坐标——PDF `PDF 第 {page} 页`、PPTX `第 {slide} 张 · {element}` + `批注 {element}`、DOCX `文档第 {page} 页`、XLSX `{sheet} · {range}` + `已选择 {range}`;统一动作是 `描述希望 Agent 修改或检查的内容` → **添加到任务**,并支持 取消/删除。落点 `artifact-canvas` + 圈选事件族(D22 的 `ihui:add-text-reference` 同机制),**禁止**为四类各写一套批注状态机 **依赖定档(2026-09-24)**:圈选事件族(ihui:add-text-reference)与 D87 批注双向锚点已就绪,但四类坐标(PDF 页码/PPTX slide/DOCX 页码/XLSX range)依赖 D41 四类 Office 预览器先行——D41 因依赖选型+lockfile 时机待 owner(见其行内定档),本条随之阻塞;解阻顺序=D41 落地 → 本条按预览器能力逐类接批注坐标。 **更新(2026-09-24)**:D41 已落地(见其行内),本条解除阻塞,可开工;实施时按 OfficePreview 能力逐类接批注坐标(pdf 页码/xlsx 选区行列号/pptx slide 序号/docx 无分页概念需锚段落)。 **实施落地(2026-09-24 15:0x)**:XLSX {sheet·range} 与 DOCX 段落锚 两类全链落地(annotation-anchor.tsx 采集卡→ihui:add-text-reference 事件→message-input 兼容消费),复用前置底座 applyAnchorAction 零第二套状态机;PDF/PPTX 登记不做(解阻=D41 二期);14 用例+26 回归过(commit 4dfe493a064)。。**验收**:四坐标各一用例 + 回流成任务输入 + 删除/取消态**复核(2026-09-24)**:annotation-anchors.ts @HEAD + 测试过;commit 4dfe493a064 即本票(XLSX/DOCX 落地,PDF/PPTX 登记),补勾选。
