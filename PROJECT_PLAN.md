...
- [x] ✅(2026-09-20) **防回潮(工程约束自身增量,§24 豁免)**:`scripts/check-i18n-keys.mjs` 新增按 next-intl 解析语义递归检测含点 key,命中即 exit 1;复用既有 pre-commit 时机(2f-web / 2f-miniapp-taro / 2f-cli),不新增守门 id。正反向均实测 —— 注入 1 个 `"probeDotted.key"` → 报"发现 1 个含点键"并 exit 1(移除后复原字节级一致);当前 7 个 locale 目录(web/shared/cli/extension/miniapp-taro/mobile-rn/api)含点键全部为 0,规则上线不会挡任何在途工作。
- [x] ✅(2026-09-20) **验证**:`check-i18n-keys` 1434 文件 / 15462 键 / 5 语言 parity OK;`check-i18n-broken-en` 0 破碎英文;`check-i18n-namespace-passing` 609 文件 OK;`check-cli-i18n-parity` 5 locales × 12 键 OK;自研 dup 键检测 5 文件 0;含点键 walk 探针(与 `validateMessagesSegment` 同语义)HEAD 84 → 工作区 0;`createTranslator` 前后对比 = 原本在渲染的 8 条 foldPolicy 文案逐字未变、44 处(22×2 语言)回退键名转真文案、MISSING_MESSAGE 22 → 0;web vitest 全量 138 文件 / 1922 用例绿。**范围判定**:§9 = 单端文案 + 守门脚本(i18n 数据面,`packages/i18n` 五语齐改、无跨端样式/契约变化),desktop 随 web 壳自动覆盖,其余端不涉及;i18n 五语 parity 由守门保证;§21 README 豁免(纯 bug 修复,不改变对外能力清单)。
- [ ] **另记(不在本次范围)**:① `check-i18n-keys --target=miniapp-taro` 在干净 HEAD 上即因 `login.phone` 缺失键 exit 1(该文件无未提交改动,属 miniapp 端既有缺口);② `check-watermark-coverage --no-fix` 报 7 个已跟踪文件水印缺口(`apps/api/scripts/export-openapi.ts`、`apps/api/src/routes/oauth-*.ts` ×2、`docs/developer/*.md` ×3、`scripts/check-installer-assets.mjs`)—— 均为并发会话产物,留待其归属会话或下一轮自愈。
