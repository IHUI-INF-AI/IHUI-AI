...
- [ ] **37. 第一轮遗留并入**:小程序 AI 增强(原 12,miniapp-taro 原生对话/工具卡片渲染 + SSE 适配——整端重写量级,待单独立项)、Skill 市场产品化(原 14)。
...
- [x] ✅(2026-09-16) **44-阶段2 瓶颈高亮**(阶段1 拓扑/消息流经调研确认 AgentSwarmMonitor 308 行已有——列表+拓扑双视图,零重复建设):types.ts agentList 加可选 startedAt(向后兼容,缺省数据源不显示);agent-swarm-monitor.tsx 瓶颈计算(运行中且 startedAt 最早=耗时最久者,30s 节流 tick 驱动刷新)+ 行高亮(琥珀底)+ 徽章(data-bottleneck);i18n ai.swarmMonitor.bottleneck × 5 语言。验证:web tsc 零错误 + parity OK 14854 键。**留待**:阶段1 数据源补 startedAt(产生 agentList 的 swarm 事件流)+ 阶段3 中途插话改派(需 SSE 双向后端能力)。
