...
- [x] ✅(2026-07-28) **P2(下一步,自动化)继续 PR 到 7 个候选 awesome 列表** — 脚本就绪 `scripts/marketing/pr-to-awesome.sh`(一键 fork + edit + push + 创建 PR)+ 8 份完整 PR 文案就绪(`.trae-cn/tmp/marketing-2026-07-28/awesome-{llms-in-china,openai,langgraph,mcp,tauri,react-native,taro,fastify}-pr.md`,每份含 README 插入片段 + PR 标题/描述/标签 + 评估结论)+ `.github/workflows/marketing-awesome-pr.yml` CI workflow 就绪。**潜在曝光**:~50-100k stars(7 个目标列表 awesome-openai + awesome-tauri + awesome-fastify 通过率最高)
- [x] ✅(2026-07-28) **P2(下一步,自动化)自动化 GitHub Trending 推送** — 5 端 Release Notes 草稿就绪(`github-release-draft.md`)+ ProductHunt 完整表单就绪(`producthunt-submission.md`)+ HackerNews Show HN 完整帖就绪(`hackernews-show-hn.md`)+ V2EX 4 节点差异化文案就绪(`v2ex-promotion.md`)+ 微博 7 天系列推文就绪(`weibo-trending.md`)。3 个 shell 脚本就绪:`scripts/marketing/create-releases.sh`(`--dry-run` 已通过)/`submit-producthunt.sh`(`--dry-run` 已通过)/`post-hackernews.sh`(预检逻辑完整)
- [x] ✅(2026-07-28) **P2(下一步,自动化)IndexNow 批量推送 URL 到 Bing/Yandex** — 脚本就绪 `scripts/marketing/indexnow-push.mjs`(支持 `--from-local` 离线提取 + 自动回退到本地 sitemap.ts,实测 540 URL dry-run 通过)+ `.env.example` 新增 `INDEXNOW_KEY=` 段完整配置说明(5 步引导)+ `README.md` "高密度曝光度" 章节新增 IndexNow 推送引用。**待 site 上线后用户配置 key 即可推送**
- [x] ✅(2026-07-28) **P3(下一波)创建 Substack/Mirror 文章** — 完整 10 期通讯大纲就绪(`substack-newsletter-outline.md`)+ 10 篇 dev.to 文章草稿就绪(`devto-crosspost.md`)+ 10 篇 Medium Partner Program 改写版就绪(`medium-crosspost.md`)+ `scripts/marketing/crosspost-blog.mjs` 交叉发布脚本就绪(支持 dev.to / Medium / Substack,`--dry-run` 已通过,实测列出 20 篇可用文章)。**需用户登录 + 配置 API token**
- [x] ✅(2026-07-28) **P3(下一波)YouTube/B 站视频脚本** — 5 个 5 分钟短视频脚本完整就绪(架构解析 / Demo 实操 / 商业模式 / 6 大 AI 功能 / 5 分钟上手指南)+ B 站上传清单详细就绪(`bilibili-upload-checklist.md`,明确标注"AI 不可自动化"原因 + owner 工作量约 2.5 小时)+ 营销素材总目录 README 索引全部 18 份文案 + 6 个脚本 + 2 周执行时间线 + KPI 目标(`.trae-cn/tmp/marketing-2026-07-28/README.md`)
