...
- [ ] O14 SDK 真正发布（现 0 tag / brew sha256 占位）：npm/PyPI/Go/Maven + install 脚本校验 + `@ihui/api-client` 去 `private`  ⏳(2026-09-21 复核:发布链判定层已做成 fail-safe —— `release-sdk.yml` 新增 `gate` job(real 模式必须先用 `npm whoami` 真实鉴权调用证明凭据可用,不成立则 4 个发布 job 全部不执行;此前"空 mode 被印成 Real release"与"job 整体 skipped 仍全绿"两类假绿已堵)、四通道发布后**回读判红**(npm view / PyPI JSON API / repo1 pom / ls-remote tag sha)、`npm pack --dry-run` 产物干净度实测通过(files 76 / 无 .env 无 src / junk 命中 0);另修掉一个必然失败缺陷:`pypi-publish` 的 `cp ../../LICENSE` 层级差 1,该 job 此前在 dry-run 与 real 两种模式下都必红。**结论:仍不可发布**,唯一硬缺失是外部凭据(NPM_TOKEN / PYPI_TOKEN / MAVEN_* 均不在 repo secrets,本机也无;`git tag -l 'v*'` 与远端 tag 实测为 0)。剩余前置:打 `sdk-v*` tag、`@ihui/api-client` 需先补 build→dist + `files` + `publishConfig` 才能去 private、`deploy/homebrew/ihui.rb:13` sha256 仍是占位、.NET 无 NuGet 通道) **前置全部完成(2026-09-24 16:1x,owner 授权)**:api-client 已去 private(发布形态终态:exports/files/publishConfig/build 全就绪,npm pack 403 文件 junk=0,186/186 测试过);homebrew sha256 改 releases/latest/download 形态+一条 gh api digest 回填命令;NuGet 在 yml 注释登记待 owner;**唯一剩余=GitHub secrets 配 NPM_TOKEN/PYPI_TOKEN(owner UI 操作)**,配完打 sdk-v0.1.0 tag 即自动发布+回读判红。
...
- [x] ✅(2026-09-24) **计划表 ⇄ HEAD 差集审计(只读,62 条票逐条核)** —— 不是功能票,是给"照票面派单"这件事本身上一道防呆。四组结论必须留档：
  ...
  ① **最危险的一类不是"幽灵待办",是"票面记录与仓库相反"**：**D35 / D36 / D43** 三条都写着"已入库 + 实测 xx 例全绿 + barrel 已登记",而 `voice-note.ts` / `prompt-drafts.ts` / `history-projection.ts` **在全仓任何 ref 上都不存在**(磁盘也没有)。唯一权威证据是 HEAD 自留注释块 `packages/shared/src/chat/index.ts:69-78`：三条 `export *` 指向解析不到的路径会让 `@ihui/shared` 根出口整体不可用、28 个走根 barrel 的 web 文件无法构建(部署环卡在 `next build`),故被 `e77ca9e99fb` 注释掉。复跑法：`git grep -l turn_ordinal HEAD -- apps packages` = 0；`git log --all -- packages/shared/src/chat/{voice-note,prompt-drafts,history-projection}.ts` = 空。⇒ **这三条要按"重新落地"派,不能按"差最后一步"派**,否则就是第二次"以为在、其实在"的丢失。盘上未跟踪的 `packages/database/drizzle/20260924100000_chat_history_projection.sql` 是并行会话在途物,不属欠账。

  ② **可直接勾的幽灵待办**：`D58` 工具类目聚合层(`apps/web/src/components/ai/progress-sections/tool-category.ts` 自述唯一映射 + 两专测;未决仅"票面 20 类 vs 代码 18 类"措辞差)、`D41` Office/PDF 预览(`office-preview.tsx` 四态齐,`docx-preview`/`xlsx`/`jszip`/`pdfjs-dist` **都已在 `apps/web/package.json`** ⇒ 票面"依赖待 owner 拍板"过期;真缺的只有 preview/源码切换)、`D34` 事件契约(交付形态=两帧落地 + 两帧有意收回,收回理由在 `sse_contract.py:49-56`)、`D64①` Credits 热力图(卡被 `settings/billing/page.tsx:26,80` 真消费,`/credits-usage/daily` 在库)。**反向幽灵**：`D49` 勾了但只做完 ①,③ 验收要求降行数,实测 HEAD 不降反升(`MessageItem.tsx` 1486 / `message-input.tsx` 1431 / `ai-side-panel.tsx` 1503);`D78` 勾渲染面而票面自述的数据面缺口未动。

  ③ **"件在库 ≠ 装上车"是一整批**(判据 = `git grep -ln "<导出名>" HEAD` 只命中自身定义 + 自身测试)：`D77` `business-form-card.tsx` 零 importers **且 `form_request`/`form_response` 在两处 SSE 契约里都不存在**;`D81②-⑥` `tool-activity-line.tsx` 6 个渲染件在 `apps/web` 被引用 0 次;`D64⑤` 反馈问卷卡零消费点且端点载荷仍只有 `{messageId,rating}`;`D38` `queue-interaction-bar.tsx` 零 importers;`D73` `pane-split-container.tsx` 在 side-panel 里 0 引用;`D64③` `refsCount` 只有测试传、真调用点不传。⇒ **从此续做/新增类票的验收标准必须含一条反查消费点的 `git grep`**,否则交付的就是本仓最高频的那类死码(本票自己就是这么拦下 D14 的,见下条)。

  ④ **票面用词能把代码写死的两例**：`D19` 让接 `terminal_delta` 帧,但 HEAD 的 `SSE_EVENTS` 里没有这个事件名(只有 `terminal_start`/`terminal_end`)⇒ 按旧名接线产出永不命中的代码;`O20` 写"71 项 `host:'ai-service'`",实测 `capabilities.json` 只有 **6** 项。另有 `D21/D24/D25/D28/O17` 五个 ID **在 HEAD 计划里已无条目且不留归档占位**(属活文档事故面,与守门 71/100 同族),结论一律以实测实现物为准。

  ⑤ **机制化出口(登记为下一票,本票未施工)**：`check-task-claims.mjs` 只管认领、`check-plan-line-loss.mjs` 只管登记行不丢,**没有任何尺子看"票内声称的实现物是否真在 HEAD"** —— D35/D36/D43 的"全绿"因此在仓库里躺了一整天而无人可见。建议新门 `scripts/check-plan-claims-vs-head.mjs`：从**未完成条目**抽三类可机检声称(反引号 `path/file.ext` / `NNNN_*.sql` 迁移名 / 点名符号),逐条 `git cat-file -e HEAD:<path>` 与 `git grep -l <sym> HEAD` 判在不在,**只报红、绝不自动改活文档**;`--self-test` 必含"声称且确在→绿"与"D35 现状→红"成对;首跑报不出 D35/D36/D43 三处即判据失效,宁可不接线。
...
- [ ] D18 Agent SDK 对外开放(G-23)。**发布就绪性已上机器闸门(2026-09-24,本票)**:新建 `packages/sdk/tests/publish-ready.test.mjs` 8 用例并接进根 `package.json` 的 `check:sdk-publish` + 串入 `check:all`(否则就是本仓反复记过的"造好没装车")。判据刻意**复用既有权威入口不另造第二套**:`node scripts/check-pkg-installable.mjs packages/sdk` 实测 exit 0(真 npm pack + 解包 8 项),本测试补的是它结构性看不见的那几维 —— 它在本机工作树 pack,永远看不出"根 `LICENSE`/`NOTICE` 未入库 ⇒ CI 干净检出时 prepack 必失败";且完全不读 python/go/java 三份清单。新增钉死的四条:根 LICENSE/NOTICE 必须在 HEAD 且非空、包内 LICENSE/NOTICE **按设计不得入库**(`.gitignore` 显式忽略、由 prepack 从根复制,防法律文本漂移)双向锁死、npm/PyPI/Maven/Go 四通道坐标与 `release-sdk.yml` 里的**回读 URL 字面量**逐一同名、三处 version 全等(一条发布列车)。**.NET 反向守卫**:44 个 `.cs` 至今无 `.csproj`/`.nuspec` 即无发布出口,一旦有人补了工程文件却漏建 `nuget-publish` job 立即判红。**剩余全为人工或对外不可逆动作**(故本条不勾):GitHub secrets(NPM/PYPI/MAVEN)、npm `@ihui` org 与 trusted publisher、PyPI 项目归属、Maven Central groupId 域名验证、打 `sdk-v0.1.0` tag 推 origin(唯一触发器)、NuGet 通道整条待建、homebrew sha256 回填需真实 release 资产。⚠️ 顺带纠一处**票面前提已过时**:本票开头引用的 O14"api-client 仍 `private: true`"在同日 16:1x 已被别的会话完成(去 private + exports/files/publishConfig 齐、403 文件 junk=0),照旧文派单会重复劳动 —— 本票据此**没有**动 `packages/api-client`。
...
- [ ]（进行中）**D38 队列语义完整交互(G-42)**:拖拽重排 / 撤回 / 编辑队列项 / 「打断并执行」/ 队列模式可配(steer vs queue,对标 Codex `followUpQueueMode`)。复用 D28 侧问队列与 W2 abort 通道,不造第二套排队。**验收**:五动词各有 e2e + 与 /side 互不回归 + 重排后发送顺序断言
  - **D38 进度(2026-09-24,判定层+交互条+store 四动作+词包已入库,宿主替换未做)**:交互编排 `packages/shared/src/chat/queue-interactions.ts`(32 例) + 队列条 `apps/web/src/components/chat/queue-interaction-bar.tsx`(32 例;编辑框聚焦用 ref+effect 并断言 `document.activeElement`,不用 `autoFocus` —— `jsx-a11y/no-autofocus` 判红,首版就是被这条拦下的) + `apps/web/src/stores/chat.ts` 的 `requeue/removeQueued/editQueued/interruptAndRun/followUpQueueMode/setFollowUpQueueMode`(+87 行,只增不改既有 enqueue/remove/shift 与其消费点) + 词包 `ai.pane.queueOps` 14 键 × 5 语言。两条硬指标已由用例钉死:①**重排后发送顺序** = seed[a,b,c]→`requeue(2,0)`→逐条 `shiftSideQuestion` 得 `['c','a','b']` 与重排数组逐位相等;②**与 /side 互不回归** = 四动作后全 store 仅 `sideQueueByConversation` 一个顶层键变化。许可门一律复用 D69 `queueInteractionPerms`,被拒动作显式渲染 `denied.*`(不静默禁用);「打断并执行」只产出 `{stopFirst, thenRun}`,停流走 W2 既有 abort、队首仍读 `queue[0]`,未新增第三种队首语义。**三条未闭环(逐项点名归谁)**:① 宿主替换 `message-input.tsx:951-986` 的 D28 侧问队列块 —— 该文件此刻有并行会话 D36 草稿改造的未提交改动,按 §12b 不重写他人主体也不把其未提交行混进本票,hunk 与 props 取值来源已写 `.ihui-agent/tmp/d38-mount/README.md`,换 hunk 时须同批更新引用旧 testid(`side-queue-answer-*`/`side-queue-remove-*`)的用例;② 台账原文要求**五动词 e2e**,现只有组件级 vitest 用例,Playwright 五动词 spec 待 ① 落地后补;③ 全仓**尚无 `runtimeSupportsInterjection` 生产者**(D69 banner 自身也零挂载点),接线前宿主须诚实传 `false` ⇒ steer 回落 queue + 显式降级句,不得端内自建第二套能力判定。
...
- [ ]（进行中）**D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做
  - **D64 进度(2026-09-24,判定层 + ②③ 装车 + ①⑤ 前端卡已入库;①④⑤⑥ 各有余下主体)**:判定层 `packages/shared/src/chat/element-pack.ts`(28 例) + 词包 `ai.pane.elementPack` 49 键 × 5 语言。逐条实况:**①热力图** 卡 `billing/credits-heatmap-card.tsx` 已可用(5 例)并修掉两处硬伤——`return null` 早退写在两个 `useState` 之前(条件 hook,数据由有→无必炸)、日历格用 `title=` 原生提示(§4 禁)→ 改 `aria-label`;**但 web 侧无按日消耗时序**(实测:`api-client/endpoints/wallet.ts` 只有余额与收支明细;全仓唯一日聚合 `ai_relay_channel_daily_usage` 是 BYOK 渠道配额内部表且**无对外路由**,语义也不是用户积分;最接近的 `GET /points/transactions` 是分页流水无日聚合)⇒ 后端加日聚合查询属 §24 需 owner 确认,**未擅自开路由**,故该卡暂零消费点。**②图片预览** 已装车:`media/FilePreview.tsx` 的 ImagePreview 补翻页/第 N·M 张/缩放档位/保存与复制成败(判据全走 element-pack,`image-preview-pack.test.tsx` 5 例),单图调用行为逐字不变;多图 `gallery` 由调用方注入(渲染件不自行收集附件,避免第二套聚合),真实调用点在 `message-list/**`(在途)与 `media/message-file-preview.tsx`,hunk 见 `.ihui-agent/tmp/d64-mount/README.md`。**③思考卡双态** 组件层已双态入库(`progress-sections/thinking-section.tsx` 接 `thinkingTitleView`,9 例含双态各一),并同批改掉 **H22 反超**——自动展开 effect 现仅在"尚未展开"时接管,用户手动展开/localStorage 偏好不再被流结束收起。**但主 agent 19:5x 复核自己上一段结论时发现第二态在 web 宿主结构性不可达**:`MessageItem.tsx:821` 整块被 `{m.reasoning && (...)}` 门住,无思考时该卡根本不挂载,传不传 `refsCount` 都一样 —— 上一段写的"已装车"只对组件层成立,对宿主不成立,现就此更正。**解阻需 owner 定档而非自选**:第二态的卡体该显示什么(把引用列表收进卡内并抑制下方 `CitationBar` / 还是保持列表在下方而卡片只做计数标题),这是**新增交互形态**,`refsCount` 数据源本身没问题(`m.citations`,与 `CitationBar:1049` 同一集合,不需另数一遍)。**④后台子任务八态** 仅判定层:八态词汇表 + `stopFailed` 显式 + `retryStop` + `fromAgentStatus` 在层内;`side-task-lifecycle-card.tsx` 实测属 D75/G-102 侧边任务**四态**域且文件头明文"不得另写第二套",**未污染该卡**,真实落点指认 `sub-agent-task-tree.tsx`(已有三张状态表)。**⑤反馈问卷** 前端卡已装车(`chat/feedback-survey-card.tsx`,三选+可跳过+免打扰,5 例,不取数不落库);**落库缺口实证**:`/api/chat/messages/feedback` 只收 `{messageId,rating}`,三选/评论/跳过无处存 ⇒ 扩展该端点载荷属 §24 需 owner 确认。**⑥goal 卡** 按台账"先自证"纪律只出对照表(`.ihui-agent/tmp/d64-goal-compare/README.md`),唯一无争议差距=编辑入口(`setGoal` 已有),`budgetLimited/usageLimited`、token 计量、"时长三档"全部标**待取证**,未凭印象写任何竞品文案(H23)。**主 agent 复核时抓到的一处漏做**:i18n 子任务按当时模块状态落键,漏了 `imagePreview.{prev,next,zoomIn,zoomOut,save,copy}` 与 `feedbackSurvey.answer.{solved,partial,notSolved}` 共 9 键 × 5 语言(判定依据=按各组件 `useTranslations` 绑定的命名空间逐字面量解析取词路径,而非 grep 键名),已随本票补齐。
...
- **B4e-B4o 组 D65-D101 的覆盖声明(第 19 轮补齐;此前矩阵只写到 D64,是计划自身的漏格 —— 实施者若照旧会各自猜豁免,正是返工源)**:
  ...
  - **D38 / D64 / D73 的 H18 逐格矩阵(2026-09-24 实测,三端并行取证;每格结论都带可复测路径)**：
    - **公共前置(本格承重结论)**：`ai.pane.*` 共 976 叶**此前只存在于 `packages/i18n/messages/web/`**，`shared/miniapp-taro/mobile-rn/cli/extension` 各 **0**；而 miniapp(`src/i18n/index.tsx:14-16`)与 rn(`src/i18n/*` 第 11-13 行)合并的是 **shared + 各自端包**，web 才合并 shared+web ⇒ **任何非 web 端接共享词表都是裸键回显**。已由 `6a93e94ad` 把三批次的 79 叶(queueOps14/elementPack49/multiPane14/thinkingRefsTitle1/thinkingTitle1)**整体搬**到 shared 并重生小程序离线包；纯移动由"合并键值集 HEAD vs 工作树"证明(116860→116860，丢失 0/新增 0/落点错 0/未制造新重键)，`check-i18n-keys --target=shared|web --parity-only` 双绿。`thinkingTitle` 与 `thinkingRefsTitle` **必须同包**——拆开放会让"有思考"能渲染而"引用数"永远回显。
    - **cli**：D38 **已接线** —— 新增 `apps/cli/src/commands/queue-ops.ts`(端内唯一适配器，判定仍走 shared) + `prompt-queue.ts`(`reorderPending`/`editPendingText`) + `repl.ts`(`/queue move|edit|run|mode`，rm 过 undo 许可门，drain 队首一次性放行)；`tests/queue-ops.test.ts` 8 例 + `prompt-queue.test.ts` 47 例回归全绿，`tsc --noEmit` 0 错。D64①②③④⑤ 与 D73 = **平台独占豁免**，逐条证据在 `.ihui-agent/tmp/end-cli-ext/cli-d64-exemptions.md`、`cli-d73-multi-pane.md`(credits/kitty/chafa/reasoning/background/feedback 与 splitPane/PaneLeaf 各自 grep 0 命中 + element-pack 文件头数据面自证)。
    - **mobile-rn**：D64② **已接线** —— `src/components/ImagePreviewModal.tsx` 改消费 `pageImage/imageCounterView/zoomStep/imageTransferView`，`tests/image-preview-modal-element-pack.test.tsx` 7/7 绿(不 mock 判据，测真模块)；`vitest.config.ts` 加子路径别名且置于父路径前。D73 **平台独占豁免**(词边界 `\bPane\b`/`splitView` 0 命中，导航只有 native-stack + bottom-tabs，无分屏容器)；D38/③/④/⑤ = **宿主被占**，hunk 在 `.ihui-agent/tmp/end-rn/*.md`；D64① 该端**无积分页**(heatmap 0 命中，`credit` 3 命中全是 `CreditCard` 图标)，新开页属 §24，未擅自开。
    - **miniapp-taro**：D64② **已接线(逻辑半格)** —— 新 `src/lib/image-preview-pack.ts` 消费 `elementPackKey/imageCounterView/imageTransferView`，11 例绿，整端 `tsc` 0 错，style-parity/tokens-sync 两门绿；**渲染半格与③/④/⑤/D38 全部宿主被占**(`ChatMessageItem.tsx`、`src/api/index.ts` 并行在途)，hunk 见 `.ihui-agent/tmp/end-miniapp/`；D64① 与 D73 = **平台独占豁免**(本端无积分页：credits/heatmap 各 0 命中；小程序无多窗口与跨视图拖放，`PANE_CONVERSATION_DRAG_TYPE` 是 HTML5 DnD MIME)；②的"复制图片"半格差异已在文件头声明(小程序无二进制剪贴板 API，落为复制地址)。
    - **extension**：D64④ **已接线**(`AgentRuntimePanel.tsx` 状态色档改由 `backgroundTaskView(fromAgentStatus(·)).tone` 派发 + 2 例)但**未提交** —— 该文件在我接线期间被并行会话同时加入 D55②(`permissionDecisionWord`)改动，整文件提交会把他人未提交行混进本票(§12 污染事故)，按 §12b 等其收尾；D38/②③⑤ = 宿主被占(`ChatPage.tsx`/`MessageContent.tsx`/`chat-persistence.ts` 在途)，hunk 齐；①+D73 豁免(sidePanel 单列形态、paneAPI/credits 0 命中)。
    - **本票没做的两件，理由不是"忘了"**：① 三模块的 `packages/shared/src/chat/index.ts` barrel 出口**刻意不补** —— 四端均已用子路径 `@ihui/shared/chat/*` 直连且测试为证，补 barrel 只增加重名与打包面风险无收益；② D64⑥ 保持"只出对照表、不做"，未因有代理在场就翻案。
    - **仍欠的 H18 空格(逐条点名解阻判据)**：D38 web 宿主(`message-input.tsx` 待 D36 收尾)、D38 rn/miniapp/extension 宿主、D64③ rn/miniapp 宿主、D64④ miniapp(需共享 SSE 层先有子任务帧)、D64⑤ 三端 + 后端落库(待 chat-history-projection 迁移入库)、extension D64④ 提交。全部 hunk 已备在 `.ihui-agent/tmp/end-*/` 与 `d38-mount`/`d73-mount`/`d64-mount`，宿主一干净即可一轮替换 + 补 e2e。
...
- **进度(2026-09-24,mobile-rn 半边收口)**:三件事逐一核到 —— ①档名+后果说明**已在库**(`AiAssistantN8nScreen.tsx:1597-1611`,第 58/59 轮提交,数据源=消息盖章值 > 工作区默认档,皆缺整行隐藏);②审批**结果**展示已在(`AgentRuntimePanel.tsx:73-74` 已放行/已拒绝,专测三层覆盖);③真缺口是 `AgentRuntimePanel.tsx:79` **裸显英文枚举档名**(如 acceptEdits)—— 已修:改走 `permissionTierWordKeys(permission.mode).title`(档位词汇从 `packages/types/src/permission-mode.ts` 唯一真源派生,改动文件 PERMISSION_MODES 计数 0/硬编码档位数组 0),未知档落「未知模式」绝不 default。新增 `tests/agent-runtime-permission-mode.test.tsx` 5 例(五档 wire 拼写逐一渲染/camel 别名归一/历史别名归一/未知档绝不显示成默认档/词包五档互异);实跑 4 文件 **14/14 通过**、mobile-rn typecheck 本改动 0 错、eslint 0 问题。**整条仍不勾**:审批三键(允许一次/总是允许/拒绝)两端均无响应通道、pending 帧需 ai-service 侧新增 —— 属需 owner 拍板的新端能力(第 58 轮裁定不造假接通)。
...
- **仍未闭环(本会话不代做)**:`ChatScreen.tsx` 的 `shareBtn`/`shareBtnText` 同型主次不分,待该文件转干净后按同一正解改。
- **正解参照仓库既有实现**:`apps/mobile-rn/src/components/LoginPopUp.tsx:556-576`
  ...
  **不要**再自创第三档。
- **重开判据**:`git status --porcelain -- <这些文件>` 为空后,按上述正解改,并复跑守门 83
  (R1 会盯 `brand.DEFAULT` 背景配 `surface.light`/`text.primary` 前景;原登记写"守门 75",该门已于 O28 换号为 83)。
...
### O46 运维到人通道收口:第三方微信推送腿(Server酱)整体摘除,邮件为唯一通道且无总量封顶(2026-09-24,用户两次明确授权)
...
- **② `safe-commit` 的跳闸判据补齐(`c2627f1a` + `39eae6c4`)**:旧逻辑把 `git commit` 的**任何**非零退出都当成"pre-commit 判红 = 别人代码没过门",随即 `--no-verify` 重试成功还打印"commit 干净"。同日实测它就是建立在一次 `.git/index.lock` 瞬时争用上(钩子一次没跑,再看锁已消失)。现在:锁争用类(exit 128 / `index.lock` / `cannot lock ref`)⇒ 重试同一条不跳门的命令,最多 `IHUI_SAFE_COMMIT_LOCK_RETRIES`(默认 10)×3s,耗尽即失败并明写"这不是钩子判红";exit 129(git 用法错)⇒ 判"工具自己把参数拼坏",同样拒绝用跳门兜底;Step 1 `reset` 与 Step 2 `add` 与 Step 4 `commit` 共用同一条 `gitStep`(上一票只加固了 commit 步,下一次就死在 `reset`)。**有效性由它抓出我自己**:第一版 `commitArgs()` 漏写子命令 `commit`,新加的 129 通道第一发就点出来 —— 旧逻辑会把它洗成"钩子阻塞后跳门"。
- **③ 门 90 补"帧名出现位置"归因(`a03b0350` + README `bf6b4055`)**:矩阵按端聚合命中,"该端已覆盖某帧"看不出是哪块界面接的 —— RN 的 `onBudget` 只在 N8n 助手屏注册、主聊天屏一帧不接,矩阵上却与"全端已接"同形(这条盲区我自己登记过,当时只能靠人记)。现在命中图与归因图**共用同一次 `git grep`**(抽成 generator),`--report` 用 `◇仅此面` 点名全仓只有一处在接的帧:web 4 帧全在 `use-chat/send-message.ts`、extension 11 帧全在 `ChatPage.tsx`、miniapp 2 帧只在 `api/index.ts`、RN 7 帧(含 `onBudget`)只在 `AiAssistantN8nScreen`。镜像测试 ②b 钉死"各端文件并集逐帧等于命中集合",11/11 绿。**刻意不升成 blocking**:"新流式面零承接即拦"在 HEAD 实测产 13 处假阳(同族符号/纯注释/测试文件都算调用点,连被当线索的 `AiAssistantScreen.tsx:130` 也只是注释)。
- **④ 门 78 五条 workspace 链接补位(机器侧,不入提交树)**:`apps/web/node_modules/@ihui/{api-client,auth,i18n,types,ui-react}` 全缺。唯一正解是全量 `pnpm install`(§12e 禁 `--filter`),但本机 install 反复以 `ERR_PNPM_EPERM … @next+swc-win32-x64-msvc … rename` 中断 —— 生产 8801 的 next-server 正持着那枚 DLL,且同时有 ≥5 个并发 install 互踩(我跑完那一轮,5 条链接又被别人的 install 削回去)。最终按 pnpm 自己的布局手工建 5 条 junction,复测「扫 637 条,破损 0 条」。**留一句实话**:这种手工补位只在"install 不再互踩"时守得住,下次并发 install 可能重新覆盖,判据仍是门 78 本身。
- **⑤ 我自己的两次交付不实(必须同批登记,否则判据永远只落在别人头上)**:
...
### 第四十批(2026-09-24):契约键豁免清单的"过期即红"被自己的装车证明顶住 —— 把钉条目换成钉不变量

- **红点与归属**:`node scripts/scan-dead-i18n-keys.mjs --target all --exit 1`(`pnpm check:all` 第 31 段)在 HEAD 上 exit 1,诊断 `DECLARATION_UNNEEDED mobile-rn/permissionTier.label`。成因:`apps/mobile-rn/src/components/ChatDisclosure.tsx:188` 已直接 `t('permissionTier.label')`(HEAD 版即可见),而该清单 `_readme` 硬约束四把"键已被本端静态引用"定义为条目过期 ⇒ **豁免清单自己成了全链红源**。**先按 HEAD 取证再动手**:用 `git archive HEAD` 独立检出复跑,拿到同一条 `DECLARATION_UNNEEDED`,排除"红只在滞后的共享工作树"这一型假案。
- **改法不是只删条目**:`scripts/tests/scan-dead-i18n-keys.test.mjs` 原「装车证明」断言"清单里**必须登记** `permissionTier.label`" —— 与判据正面冲突:清单的语义就是条目会过期,钉住一枚具体条目必然在下一次过期时变成"要么恒红、要么逼人 `--no-verify`"。现拆成两条不会腐烂的断言:① 逐端校验声明形状合法 + 无未知端;② **凡真仓登记着的声明,必须被真仓扫描器认作成立**(以真仓为 `cwd` 跑 `--target <端> --dry-run`,输出里出现三种失效码即红)。清单为空时 ② 自然成立并如实打印「有登记的端 0 个」,不假装覆盖。
- **新断言的有效性用变异注入证明**(不是"改完就绿"):把那条过期豁免原样注入清单 ⇒ 新测试转红(34 例中恰好 1 红),还原 ⇒ 34/34。另取证:`--target all` exit 0、`apps/mobile-rn` 的 `permission-tier-pack.test.ts` 2/2(它钉的是词包形状,不依赖该豁免,实测未受影响)。
- **本票顺带消掉一个"缩扫描面"的动机**:O42/O45 残余 ③ 记着并发会话曾把 `check:all` 里的 `--target all` 收窄成 `--target web`(那正是本清单 `_readme` 反对的出路 —— "把其余四端的红点重新藏起来")。实测该收窄**从未进入提交树**:HEAD 与工作树的 `package.json` 第 31 段都仍是 `--target all`,且现在它真能跑绿。不得有人再以"反正 all 会红"为由重提收窄。
- **提交链现场**:`safe-commit` 首跑撞 `.git/index.lock`(并发会话持锁,age 61s,40.6s 后自行释放),按 §12d 只用 `existsSync` 轮询等锁、**未删锁**;第二轮跑完 130 项守门,唯一失败是 [29] `check-push-sync`(本地 ahead,属机器态而非本票代码),钩子按 §12 用户规则 `--no-verify` 落地 `7dc33250360`(`git show --name-only` 回读仅含声明的 2 个文件),随后 `node scripts/git-push-converge.mjs` = **ALREADY**。
- **顺带实测掉四处 phantom 债(供后续派单,不要再按旧文派)**:L5448 登记的"4 道 blocking 门红在 HEAD"**现已全部为绿** —— 门 52 生产代码 0 违规、门 7 通过、门 77「违规 0 处」、门 83 `R1=0 / R2/R3/R4 全部 ≤ 基线`;G-150 的界面层(`compression-divider.tsx` + `progress-sections/__tests__/compression-divider-ceiling.test.tsx`)已入库;守门 89 的 R4 文档缺口 0 枚;守门 90 `check-sse-dispatch-parity.mjs` 的跨取材面缺陷已由 `46fd852b316` 改为提交树双修订口径。**这些结论与那些 `- [ ]` 原文仍并存 —— 按旧行派单即白烧一轮。**
- **未闭环(不写作收口)**:① `check-i18n-keys` 在当前工作树上仍红 —— `chat.voiceNote` 命名空间 9 枚静态键 + `phase.*` / `errors.*` 两个动态前缀整体缺失(web 语音笔记界面此刻会回显键名)。**本票刻意不代补**:`apps/web/src/components/chat/voice-note.tsx` 与 `packages/shared/src/chat/voice-note.ts` 均为**未跟踪新文件**(并行会话 D43 在飞),而 §19 要求"词表与代码同票或词表先提交",由该会话一并落 5 语言才是正解;代补会造出第二套同族键表与它对撞。解阻判据 = 那两个文件进入提交树。② 上面四处 phantom 债的原文划勾属活文档**就地改写**,本批只登记实测结论、未回头改 L5448 / G-150 各行(避免与并发会话在同行对撞),划勾权归各该票持有人。

### 第四十批·续(2026-09-24,用户改口"补全语音笔记的 9 个词条"):词表先于代码落地,并把"补完词条仍不可用"的断因钉清

- **上面第四十批"未闭环①"已作废(同日按用户指令续做)**:`chat.voiceNote` 词表**已补齐**,`pnpm check:i18n-keys` 由 exit 1 转 **exit 0** —— 原文行按 §12 逐字保留,不就地改写(改写会让活文档对账器判"整行丢失",而 safe-commit 的 ⊇ HEAD 硬校验只认丢失计数,不认"我其实换了写法")。

- **做了什么**:5 个语言包(`packages/i18n/messages/web/{zh-CN,zh-TW,en,ja,ko}.json`)各插 30 行,`chat.voiceNote` 共 **22 枚叶子键**(title + 6 枚 action + 7 枚 errors + 8 枚 phase),形态对齐既有 `chat.compaction` 嵌套块 —— 顶层 `chat` 下无含点键,`t(\`phase.${x}\`)` 要求 `phase` 必须是对象节点。**不整体重序列化**:实测 `JSON.stringify(_,2)` 与磁盘每文件差 ~1.7KB,整体写回会把 5 个文件全行改写,在共享工作区里等价于给并发会话制造一次整文件回写;故按 `"chat": {` 锚点行做**纯插入**,提交前 `git diff --numstat` 自证 `30 / 0`(五文件删除数全为 0)。
- **取证(权威入口,不手搓判据)**:`pnpm check:i18n-keys` 由 exit 1 → **exit 0**(1566 文件 / 17745 键 / 5 语言 parity OK);`scan-i18n-zh-residue` zh-TW·ko·ja·en 四门绿;`check-i18n-broken-en` 0 处;`check-i18n-duplicate-namespaces` 无重复。另按 [[locale-batch-edits-need-symmetry-check]] 自证五语言 `chat.voiceNote` 键集**逐枚对称**(22=22=22=22=22,缺 0 多 0)。
- **词的出处与不猜的边界**:5 枚 `errors.*` 文案与组件注释「权限拒绝/无设备/启动失败/保存失败/识别中断」逐一对应;`action.insertToInput`=「插入对话」取自文件头自述的 draftInput 回插通道。**相位只补组件实际能渲染的 8 相** —— 台账写的是 **12 相判别联合**,余下 4 个名字的唯一来源是 `PROJECT_PLAN.md:2448` 记的"Qoder asar 字节偏移 63654950 逐字切出 recordingNote 12 键",而本机 `Program Files` / `AppData\Local\Programs` 下**没有 Qoder 安装**(find 实测仅 Codex/opencode/Quark 四枚 asar),`transitionVoiceNotePhase` 在全仓的唯一残留是 `.next/dev` 编译缓存,而那份缓存里写的正是 `Cannot find module` ⇒ **无源可取,不凭猜测补键**(猜出来的相位名会让模块重建者对不上)。
- **断因钉清(比"缺键"严重,且此前无人登记)**:`packages/shared/src/chat/voice-note.ts` 从未进入任何 ref —— `git log --all --oneline -- <该路径>` 零命中;组件 import 的 8 个具名符号 + 3 个类型全是空气;`packages/shared/src/chat/index.ts:69-74` 已把它连同 D36 `prompt-drafts` / D35 `history-projection` 一起注释掉出口,注释里写明理由("根 barrel 解析不到会让 28 个走 `from '@ihui/shared'` 的 web 文件整片无法构建,部署环卡在 next build")。计划 L2448 却记着"12 phase 状态机**已落地**、shared 18/18、web 4/4 实测通过" ⇒ 该模块只在未提交的工作区里活过一次,现已丢失,与 §5b 宿主清理层 / 并发整文件回写同型。
- **影响面已界定,不外溢到主线**:`git show HEAD:apps/web/src/components/ai/ai-side-panel.tsx` 对 `voice-note` **0 命中**,`git ls-tree -r HEAD` 里组件文件也不存在 ⇒ 断链关在他人未提交的工作区内,main 与部署环未受影响。因此本票**只交词表**(§19 允许词表先于代码),不代做模块 —— 重建 12×13 迁移矩阵属 D43 持有人职权,代做即§12 越权。解阻判据 = 该会话把 `voice-note.ts` + 组件 + `ai-side-panel` 接线三者同票落地,届时把 `index.ts` 那行注释去掉即可恢复出口。
...
- [x] ✅(2026-09-24)**P0 根因(2026-09-24 真机锁定,推翻本轮此前所有"广场页配色"归因;已按方案 A+C 收口,commit 9cbb2371)**:**mobile-rn 存在两套主题真相源,系统档位在冷启动后变化即产生同屏分裂**。**机理**:`apps/mobile-rn/src/theme/active-tokens.ts` 的 `applied = persistedMode() ?? systemMode()` 在**模块求值期一次定档**,86 个文件的模块级 `StyleSheet.create` 在那一刻把颜色**取成字符串常量**(1439 处 `tokens.*`);而 `ThemeContext.useTheme()` 的 `resolvedTheme` 走 `useColorScheme()`(**响应式**),共享层组件 `getTokens(colorScheme)` 每次渲染重算 ⇒ 系统翻档后**一半跟、一半不跟**。**实测证据链**(code11 真机,同一分钟内四页对照):我的 chrome=26 body=26 / AI应用商店 chrome=26 body=38 / 智汇AI chrome=26 body=26 / **广场 chrome=26 body=245 tab=26(正文浅色占比 96%)**;`adb shell cmd uimode night` = **no**、`settings get secure ui_night_mode` = **1**(系统现为浅色),而 App 偏好为"跟随系统"(设置页 ✓ 在"跟随系统"行)⇒ 冷启动时系统为深色(故 chrome/tab 冻结为深),之后系统翻浅色,共享层翻了、模块层没翻。**已排除的两个假因**(都查过才敢写):① 不是接线漏传 —— `apps/mobile-rn/src/screens/PlazaScreen.tsx:467/497` 确实传了 `colorScheme={resolvedTheme}`,守门 91 全仓 0 未接线为真;② 不是工作区旧基线 —— 两个 PlazaScreen 文件磁盘 blob 与 HEAD **逐字节相同**。**为什么没直接改**:release 包里 `reloadForTheme()` 是空实现(`DevSettings.reload` 仅 __DEV__),且本仓**未装** expo-updates / react-native-restart,`grep` 确认无 `Updates.reloadAsync` 通道 ⇒ 不存在"既一致又实时"的第三条路。三个候选各自改变产品语义,须用户拍板:**A** 让 `resolvedTheme` 改读 `currentRnTheme()`(与模块层同源)—— 消除分裂,代价是**显式切主题在冷启动前看似无反应**;**B** 把 86 文件模块级样式迁到 `makeStyles(tk)` —— 最正,等于重写全端样式层;**C** 引入 `expo-updates`(或 react-native-restart)让 release 真能重载 JS —— 既一致又实时,但加原生依赖 + 重出包。**已修(用户选定"装可重载依赖",实施为 A+C 两半)**:C = 引入 `react-native-restart@0.0.29`,`reloadForTheme()` 在 release 走 `RNRestart.restart('theme')`(进程重启 = JS 全量重求值,与冷启动同语义),`__DEV__` 仍用 `DevSettings.reload` 保留 Metro;A = 顺带查出并修掉**第二个更隐蔽的缺陷** —— 冷启动 `applied = persistedMode() ?? systemMode()` **无条件优先读文件**,而文件存的是"上次解析结果",偏好为 system 时它就是过期的系统档缓存,于是分裂**在重启之后依然成立**且没有 change 事件去纠正;现改为落盘**偏好**本身(带 `v2:` 版本前缀,旧格式一律不认 —— 把旧值当偏好会把"跟随系统"悄悄固化成"显式深色"),冷启动按 `resolveRnTheme(偏好)` 解析。无重启循环:`reloadForTheme` 只在真实换档事件里被调用,不在挂载路径上,且 `commitRnTheme` 结果未变时返回 false。真机复验(code13):系统浅+跟随系统 → chrome/body/tab=255/255/255;系统深+跟随系统 → 26/26/26;系统深下显式点「浅色」→ 255/255/255(证明确实触发了真重启,修复前只会翻共享层、chrome 留深色)。**一处如实保留的验证盲区**:运行中把系统翻深色时,本机 MIUI 不给运行中的 App 派发配置变化,Appearance 监听未触发,故"实时自动重启"这条路径在本机无法演示,只验证了冷启动侧与显式切换侧。依赖按 §12e 用全量 `pnpm add`(未用 `--filter`),装后复验根 node_modules 关键入口在位、守门 78 exit 0;APK dex 已确认打进 `RNRestart` 类。设备状态已复原(App 回「跟随系统」、`ui_night_mode` 回 1、stayon 关)。
  ...
  **方法论债(本轮第三次同形错误,必须固化)**:三次误判都是**探针量的不是我要断言的东西**,却据此下了结论 ——① 把 rgba(0,0,0,0.6) 遮罩压暗 40% 当成"颜色没落上";② 用"attrs 里有没有 colorScheme 字样"得出"漏传 0 处"(真实 118);③ 用 `grep -E "error TS" | head -3` 截断输出,把 budget-note 的 2 条 TS2305 看成"不存在",还写下"剩余报错均在我未触碰的文件"。②③ 还都不是"探针弱",是**探针根本没往那个方向看**。纪律已写入用户级记忆:凡给"0 处 / 没有 / 已全清 / 均非我属"这类否定或全称结论,必须先构造一个**已知应命中的正例**喂同一判据;截断输出(head / tail / grep -c)不得当作"没有"的证据。本轮 427 例全绿前,`budget-note` 那 3 条红测试单跑同样红 —— 我上一轮判它"序依赖"就是拿"grep 没命中"当"通过",纯属误判。
- [x] ✅(2026-09-23) **测试**:`apps/api/tests/notify-deploy-failure.test.ts` 39 例(参数解析/message 三级优先/severity 白名单降级/收件人三级优先级/env-file 绝不覆盖进程环境/From 三情形/**Resend payload 断言含 html+Authorization**/SMTP 失败→Resend 回落/--strict 退出码/dry-run 零网络,BOM 与无 BOM 各一例)+ PS 镜像测试 6 例(证明自拼传输 0 命中 + 六个契约 flag 在位 + 无 BOM 落盘 + 降级链路 + SCT 成功不发邮件)。相关 4 个 api 测试文件合跑 **146 passed**,`tsc --noEmit` 0 错误。
## O48 全队提交链三条硬故障根治:Gitee 配额自激环路 / 2n-web parity 读脏工作树 / 根 .bin 缺 shim
...
<!-- 合并回捞(2026-09-24):以下 19 行是 merge-file --union 未能保位/保住的原行,按原样补在块尾,一行未改 -->
...
- [ ] **D91 四类文档批注锚点分型(G-124,扩展 D87)**:Qoder 的批注不是单一"选中文字",而是四种定位坐标——PDF `PDF 第 {page} 页`、PPTX `第 {slide} 张 · {element}` + `批注 {element}`、DOCX `文档第 {page} 页`、XLSX `{sheet} · {range}` + `已选择 {range}`;统一动作是 `描述希望 Agent 修改或检查的内容` → **添加到任务**,并支持 取消/删除。落点 `artifact-canvas` + 圈选事件族(D22 的 `ihui:add-text-reference` 同机制),**禁止**为四类各写一套批注状态机 **依赖定档(2026-09-24)**:圈选事件族(ihui:add-text-reference)与 D87 批注双向锚点已就绪,但四类坐标(PDF 页码/PPTX slide/DOCX 页码/XLSX range)依赖 D41 四类 Office 预览器先行——D41 因依赖选型+lockfile 时机待 owner(见其行内定档),本条随之阻塞;解阻顺序=D41 落地 → 本条按预览器能力逐类接批注坐标。。**验收**:四坐标各一用例 + 回流成任务输入 + 删除/取消态
...
### 第三十九批·清账与 union 收敛(2026-09-24,owner 指令"执行到底、不留后续建议")

- **union 收敛落地**:本地 main 与 origin 分叉(本地独有 19 / 缺远端 21)由手工三向合并收口,合并提交 `e128be38efac`。活文档按行并集;`scripts/re-home-junctions.mjs` **不能用 union** —— 远端把 `if (!existsSync(src))` 早退挪到 `isLink` 之后(悬空 junction 的 existsSync 为 false),union 会把它放回原位、复活远端刚修掉的 bug,故手工合:repairOne/main 语义取远端 + `pruneStashes` 整块取本地。独立复跑:该文件自带 `--self-test` 24/24、并集镜像测试 14/14(两侧关键用例俱在)。**A1 防吞并对账(等价守门 100)实测两侧独有新增丢失 0 / 0**;文档零丢失 / 长行份数零膨胀 / 零冲突标记由装配器写盘前自证。
- **一类事故只补一条取证事实**:远端合并 `9a0f7610e9` 写着"双方每一行均存活"却整批吞掉 35 个路径(含我方 D42/D76/D97 八枚文件)—— 并发会话已找回全部文件并立守门 100 + `union-converge.mjs` 作修复出口。**教训的具体形态:判"交付是否还活着"要看远端树里文件在不在,`git status` 与 diff 行数全都看不出来。**
- **D76 两条残余做实**(`91beeee1772` + `76c688acdf4`):派生层下沉 `packages/shared/src/chat/artifact-turn.ts`(按锚点取下标,**不建第二份映射**),同轮多产物改为按被点产物精确聚焦;正文链接发起端接既有 `ihui:focus-artifact` 通道(无新事件名、无第二套载荷)。变异取证:旧实现在新用例上必红(实跑,非推演)。
- **D50 缩窄**:② 每会话 Tab 状态已落地(`a5df11d50b2`,store 分桶 + LRU 上限 20 桶 + 13 例单测,零网络、零第二持久化键);`③ WorkBuddy 取证` 子项**前提已过期** —— 第 54 轮 D110 已在本机实测打通 asar 取证并派生 G-150~G-158,该子项由 D110 替代完成;`① 多端遥控配对`(新表 + 新端能力)属 §24,须 owner 显式启动,本批未动。故本条实剩 ①。
- **D42 全链路 e2e**:台账原写"需要 e2e 基建"经实测为**半假** —— 基建已在(87 枚 spec + `e2e.yml`),缺的只是 CI 里没有 ai-service:8803。两档分别做实:mock 档 `4482169d3cc`(`annotation-flow.spec.ts` 5 例,私有 dev 端口真跑 7 passed,4 条变异各红各用例)+ 真画面档 CI 编排 `6451335ed01`(`e2e-browser-hub.yml` 三条 fail-fast 活性断言:8803 健康 / Chromium 引擎预检 / 无凭据探针必须 401)。
- **D55② extension 端收口**(`52fe609bc4c`):端内 `DECISION_KEY` 是**第二个真相源**,实测旧映射会把 `auto_skip_approval` 直显原始码、且 `deny` 词值与共享漂移(「已拦截」vs「已拒绝」);改为走共享唯一入口 `permissionDecisionWord()`,5 例渲染用例 + 三形态变异(直显枚举/猜语义/喷键名)全红。
- **D79② 据实回勾**:台账那句"三个入参只存在于并发会话未提交工作区"**已过期** —— 真实路径 `chat/message-list/message-item-parts.tsx`(HEAD blob `fa8b0e4`),`MessageItem.tsx:741-743` 三 prop 全传,既有 5 例真跑全绿,逐 prop 物理变异各红 4 例。
- **本批回勾**:D34(两帧落地 + 两帧收回已裁决,残余转 D106/D107 登记)、D37、D76(两条残余做实)、D108(第 48 轮已补完且全在 HEAD)、P0 顶部安全区(sha `39ed523e685`/`5b82ae18065`/`f5c199073f3`/`063e7676257`)。
- **仍开放项(逐条给主体与解阻判据,不伪装收口)**:
  - **D97 呈现侧 meta 键 `cloudChatActivity` 的生产者仍缺**:消费端在位(`timeline-event.tsx:165-167`),全仓无任何链路写它。主体文件 = `apps/ai-service/app/routers/llm.py` + `app/core/sse_contract.py`,本批改账时实测两文件均为**他人未提交在飞**(`git status` 脏),按 §16 不碰。**解阻判据**:两文件转 CLEAN 且持有会话提交后,按 D33 已验证四层套路(生产端 → `/api/ai/callback` zod → `packages/types` 契约键 → `history-message.ts` 读回)+ 守门 57 锚点补生产端;**与 D50① 无关,不必等配对方案**(上一轮把 D97 挂在 D50 上是错的记账)。
  - **extension 词包 `agent.decision{Allow,Ask,Deny}` ×5 语言自本批起无消费点**(死键),未删是因为 `packages/i18n/messages/**` 同窗口正被并行会话做 ai.pane 79 键搬迁;解阻判据 = 该迁移落地后按干净检出跑死键扫描器再清。
  - **D99 / D106**:机制主体在 HEAD,剩余是规模化与两端渲染期基建,属另一会话在飞票,本批不动。
