...
- [ ] **35. 流式 markdown 增量解析缓存**:稳定块 AST 缓存,只解析尾部活跃块。
- [ ] **36. 多模型并排对比**:同消息 2–3 模型并排生成对比卡(best_of_n 后端已有,接 UI)。
- [ ] **32. PDF/CSV 消息内富预览**;**34. 屏幕阅读器对话全流程 e2e**(aria-live 流式播报)。
- [ ] **37. 第一轮遗留并入**:小程序 AI 增强(原 12)、Skill 市场产品化(原 14)。
...
- [ ] **43. 成本协商代理(Cost-aware Negotiation)**:任务开始前预测 token 成本与时长,超预算时**与用户协商**(「预计 ¥X,精简/继续?」),流内实时消耗 vs 预测对比条。竞品现状:全部只有事后账单。
- [ ] **44. Agent 团队作战室(Team War Room)**:多 agent 协作实时可视化(拓扑+消息流+任务分派+瓶颈高亮),用户可中途插话改派任务。竞品现状:Qoder Experts 为黑盒并行,无过程可视干预。
- [ ] **45. 自愈工作区(Self-healing Workspace)**:对话检测环境故障(依赖损坏/索引过期/端口占用/磁盘满)自动修复并汇报,修复动作全部进对话流可审计。竞品现状:报错后等用户处理。
- [ ] **46. 实时语音协作(Realtime Voice Ops)**:语音双工对话 + agent 边说边干 + 屏幕理解联动(「帮我看看为什么红屏」→ 语音指导 + 实时截屏分析)。竞品现状:语音输入/TTS 单向,无双工协作。
...
- [ ] **54. 审计日志 + 提示词审计**(对标 /audit-logs + /prompt-audit):管理端操作留痕、提示词风险检测(涉敏/注入模式),接既有 audit 体系。
- [ ] **55. 用户自定义属性**(对标 /user-attributes):给用户挂 KV 属性,供分组/风控/运营筛选。
- [ ] **56. 数据管理**(对标 /data-management):日志与事件保留期、按条件批量清理/导出,防磁盘膨胀。
- [ ] **57. 运行时热更新**(对标 /runtime):配置热加载,改配置不重启服务。
- [ ] **58. 运营面板 WS 实时化**(对标 /ws):告警/渠道状态/用量走 WebSocket 推送,替代轮询。
- [ ] **59. 插件系统**(对标 /plugins):扩展点注册机制,便于后续能力外挂。

**差异化(做竞品做不到的,这才是"远超"的落点)**:
- [ ] **60. 全模态网关做深**:图/视频/3D/音频统一计费与编排(竞品仅 Grok 图片),补齐异步任务状态机与回调。
- [ ] **61. 企业合规闭环**:发票/合同/企业认证打通订单与订阅,出对公结算与开票自动化(竞品为个人订阅分发,此块完全缺)。
- [ ] **62. AI 运营助手**:基于用量/成本/错误数据自动给出调价建议、异常归因与容量预警(竞品无 AI 能力)。
