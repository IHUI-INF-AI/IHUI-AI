...
## design-tokens 同步守门统一 + 变量覆盖扩展(2026-07-28,P1-C + P1-F)

### [x] ✅(2026-07-28) P1-C 扩展守门脚本覆盖 --radius/--chart-* 等非 color 变量 + P1-F 三个 design-tokens 同步脚本合并为统一脚本

- [x] P1-C:变量覆盖从仅 --color-* 扩展到 7 类 design-tokens 变量(--color-* / --radius* / --chart-* / --font-* / --animate-* / --z-* / --shadow-*),原三个 check 脚本只校验 --color- 变量,其余 139 个变量无任何守门
- [x] P1-F:三个逻辑高度相似的脚本合并为统一脚本 `check-design-tokens-sync.mjs --target=<web|miniapp-taro|mobile-rn>`,消除维护成本
- [x] 新建 `scripts/check-design-tokens-sync.mjs`(460 行):两类校验模式(值比对 miniapp-taro/mobile-rn + @import 单源 web),支持 --target 必填 + --quiet/--staged/--check 向后兼容,注释剥离防误匹配,BOM 鲁棒,目标文件取首个 :root/.dark 块避免本地扩展块干扰,tokens.css 合并所有 @theme+:root 块
- [x] 新建 `scripts/tests/check-design-tokens-sync.test.mjs`(29 个测试):CLI 参数(--target/--quiet/--help)+ 变量同步(:root/.dark/@theme)+ P1-C 7 类变量覆盖(radius/chart/font/animate/z/shadow)+ 注释剥离 + BOM 鲁棒 + 边界(空文件/无变量/多块覆盖)+ 错误处理 + 输出格式
- [x] guardian-runner.mjs items 36/37 更新:script 改为 check-design-tokens-sync.mjs,args 改为 ['--target=miniapp-taro'] / ['--target=web'],label + onFailHint 更新提及 P1-C 7 类变量
- [x] .husky/pre-commit item 16c 更新:script 改为 check-design-tokens-sync.mjs --target=mobile-rn,label 更新提及 P1-C
- [x] 删除 3 个旧脚本 + 2 个旧测试:check-miniapp-tokens-sync.mjs / check-web-tokens-sync.mjs / check-rn-global-css-sync.mjs + tests/check-miniapp-tokens-sync.test.mjs / tests/check-rn-global-css-sync.test.mjs
- [x] AGENTS.md 守门脚本速查 UI/样式 类别更新:添加 37 + 注明 P1-C 7 类变量 + P1-F 统一脚本

验证:`node scripts/check-design-tokens-sync.mjs --target=miniapp-taro` exit 0(69 变量同步)/ `--target=mobile-rn` exit 0(50 变量同步)/ `--target=web` exit 0(@import OK + 0 顶层重复块)/ `node --test scripts/tests/check-design-tokens-sync.test.mjs` 29/29 pass / `node --check scripts/guardian-runner.mjs` syntax OK。

## 多端维护成本优化批次 P2/P3 状态(2026-07-28,4 subagent 并行)

### [x] ✅(2026-07-28) P2-A + P2-C 审查结论:当前架构下无需修改(4 subagent 并行,Subagent B)

- [x] P2-A(miniapp-taro style.ts 静态值对齐 web):**审查无修改** — `apps/miniapp-taro/src/constants/style.ts` 首行注释 `AUTO-GENERATED by scripts/sync-design-tokens.mjs`,COLORS 对象已用 hsl() 格式与 tokens.css @theme 块完全对齐(13 语义色 × light/dark);其他常量(SPACING/FONT_SIZES/FONT_WEIGHTS/RADII/SHADOWS/DURATIONS/Z_INDEX)是 tokens.css 无对应的合理静态值,RADII 对齐 AGENTS.md §4 圆角守门梯度(2/4/6/8/12/16px)非对齐 tokens.css `--radius-*`(rem 单位),是有意的端差异;`pages/setting/privacy.css` 与 `theme.css` 已全部使用 `var(--color-*)` 引用;`app.css` 本地扩展块已以 CSS 变量形式定义。验证:`node scripts/check-design-tokens-sync.mjs --target=miniapp-taro` exit 0(69 变量同步)+ `pnpm --filter @ihui/miniapp-taro typecheck` exit 0
- [x] P2-C(RN 端 hex 硬编码治理):**审查无修改** — mobile-rn StyleSheet 中 P1 阶段已全部消除 hex 字面量(grep `['"]#[0-9a-fA-F]{6}['"]` No matches found),残留的 `// TODO: custom color #XXX` 注释(11 文件 25 处)是 P1 过渡标记,代码实际用 `tokens.xxx` 引用;className 中 58 处 hex 残留(6 文件:DeveloperScreen/DevEnterScreen/ExamHistoryScreen/ExamResultScreen/ExamScreen/FeedbackDetailScreen)需扩展 `tailwind-preset.js` + `global.css` 才能消除,降级为 P3 任务。验证:`node scripts/check-design-tokens-sync.mjs --target=mobile-rn` exit 0(50 变量同步)

### [ ] P2-F + P3-A 架构性阻塞:packages/app RN-only,三端页面同名不同功能(Subagent C 暂停报告)

- [ ] P2-F(miniapp-taro 端共享组件桥接层):**架构性阻塞** — `packages/app/src/features/` 13 个共享组件全部 `from 'react-native'`(View/Text/StyleSheet/TouchableOpacity/FlatList/RefreshControl),`peerDependencies: react-native`;`apps/miniapp-taro/package.json` 不依赖 `@ihui/app`,用 `@tarojs/components`(Taro 原语)与 RN 组件运行时不兼容;`@ihui/ui-react` 是 Web 专属(`peerDependencies: react-dom` + `@radix-ui/*` + `lucide-react`)。需重构 packages/app 为 platform-agnostic(逻辑层 + 渲染层三套),工程量大,需单独立项
- [ ] P3-A(共享组件扩展 Agreement/Privacy/Help/Ranking):**前提不成立** — 三端候选页面是"同名不同功能":Agreement(mobile-rn 48 行 3 节极简版 vs miniapp-taro 532 行 13 节完整法律文本)/ Privacy(mobile-rn 48 行 vs miniapp-taro 558 行含 16 个第三方 SDK)/ Help(mobile-rn 98 行 FAQ vs miniapp-taro 201 行 FAQ+搜索+反馈表单)/ Ranking(mobile-rn 学习排行榜 vs miniapp-taro AI 工具榜单);i18n key 命名空间不重叠(mobile-rn `agreement.sectionTerms` vs miniapp-taro `about.protocol.s1.p1`)。需产品先统一定义再下沉,是产品决策非技术决策

### [x] ✅(2026-07-28) P2-E + P3-B 守门性能监控 + 测试覆盖率提升(Subagent D)

- [x] P2-E(守门项执行性能监控):新建 `scripts/measure-guardian-performance.mjs`(530 行)— 从 guardian-runner.mjs 源码动态提取 checks 数组(零配置漂移),CLI 支持 `--filter=<id>` / `--runs=<N>`(默认 3)/ `--json` / `--threshold=<ms>`(默认 5000)/ `--help`,跳过 blocking 模式守门项避免副作用,状态判断 OK(≤阈值)/ SLOW(>阈值)/ FAIL(崩溃/超时/exit≥2),输出表格模式 + JSON 模式(供 CI 消费)
- [x] P3-B(守门体系测试覆盖率提升):新建 3 个测试文件共 42 个测试用例 — ① `scripts/tests/check-cli-i18n-parity.test.mjs`(12 用例:CLI 标志 + 目录不存在 + parity 一致/不一致 + 边界);② `scripts/tests/check-solito-residue.test.mjs`(15 用例:CLI + 全量扫描 + --staged 跳过 + 4 类依赖检测 + pnpm.patchedDependencies + pnpm-workspace.yaml + patches/*.patch + import 语句 + 正则精度 + 注释跳过);③ `scripts/tests/measure-guardian-performance.test.mjs`(15 用例:CLI + JSON 输出结构 + 阈值判断 + runs 次数 + slowest 字段 + 表格汇总)

验证:`node --test scripts/tests/check-design-tokens-sync.test.mjs` 29/29 pass + `node --test scripts/tests/check-cli-i18n-parity.test.mjs scripts/tests/check-solito-residue.test.mjs scripts/tests/measure-guardian-performance.test.mjs` 42/42 pass(累计 71/71 全绿)+ `node scripts/check-design-tokens-sync.mjs --target={miniapp-taro|mobile-rn|web}` 全部 exit 0 + `npx eslint` 4 新文件 exit 0。

### P2/P3 剩余任务(本批次未执行,需单独立项)

- [ ] P2-B(sync 脚本接入 pre-commit 自动触发):Subagent A 已更新 pre-commit item 16c 调用新统一脚本,但 sync-design-tokens.mjs 的自动触发(改 token 后自动同步 app.css)未接入 pre-commit,需独立任务
- [ ] P2-D(排查 web 端 i18n parity 微漂):已有 commit `6593cc0fa4` 补全 611 处 pending 翻译,剩余微漂需独立排查
- [ ] P2-G(守门 warn→blocking 升级时间表统一):文档化任务,需统一规划 2f-mobile-rn / 2f-cli 等新 warn 项的升级时间表
- [ ] P2-H(守门项 id 命名空间重构):需改 guardian-runner.mjs 大量项 id,风险高,需独立任务
- [ ] P3-C(守门体系文档自动化):新建文档生成脚本,从 guardian-runner.mjs 自动生成守门项文档
- [ ] P3-D(miniapp-taro 本地 interface 分批下沉):改 miniapp-taro 类型文件,工作量大
- [ ] P3-E(types/api-client 包按业务域拆分):大重构,风险高,需独立任务
