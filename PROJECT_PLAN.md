...
- [x] ✅(2026-09-22) **P2-F.6 屏级适配器死代码清理 + 新增守门 64「未接线即拦」**:
  ...
  - **runner 是 fail-fast**(1768 行 blocking 失败即 `exit(1)`):意味着任一 blocking 门在途红时,其后所有门当轮不执行。端到端取证因此分三层落地——① 脚本级注入 `FakeColorProbe.taro.tsx` 含 `#ff0000` → style-parity 精确报出并 exit 1,撤销回 0;② 编排级 `--staged` + 临时索引 → `[64] … 失败,提交已阻止`;③ `--help` 清单证明 66 在数组内。前两层各被他人/探针在途失败挡过一次(id 6 sanitizer 无 skipEnv、id 30c 陈旧副本),探针改完即 `git restore` 还原(实测 ff0000 计数 0、零 diff)。
