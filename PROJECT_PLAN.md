...
- [ ] **D54 工具名本地化词表(G-65,先自证再定档)**:核 `apps/web/src/components/ai/tool-call-card.tsx` 是否把工具名送 i18n;若显英文原名,补 27 项通用工具 + 16 项浏览器动作的五语言词表(对标 Qoder `toolNames.*` 原文口径,但**用我方术语**)。**验收**:词表覆盖率脚本判据 100% + 五语言 parity 守门绿 + 未知工具名回落原文不误译
