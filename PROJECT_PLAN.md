...
- [ ] I2:typecheck + build 全绿;browser 验证 /developer/ide 可达 + IDELayout 渲染 + 文件树展开 + 终端连 WS + diff 显示真实 git 变更
- [ ] I3:Agent 面板深化(ide-layout.tsx activeTopTab='agent' 空壳 div → 真实面板),接入 ai-service agent_loop/agent_graph,复用 chat 能力,支持 AI 自主编码(读改文件+跑命令+迭代)
- [ ] I4:MCP 面板深化(ide-layout.tsx activeTopTab='mcp' 空壳 div → 真实面板),接入 ai-service mcp.py/mcp_server.py,展示 MCP server 列表/连接状态/工具调用
