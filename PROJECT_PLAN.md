...
- [ ] O19 第二段(3 条,前 2 条本会话正在推进):① 端点级属主鉴权落地(`approval-response` 跨用户 403、`tasks/stream` 按属主过滤且 fail-closed、`/agents/sessions*` 复用 `memory.py` 既有 P1-6 `user_id` 隔离参数 —— 该隔离原语早已存在,**这些调用方一个都没传**,于是全部落到 legacy 无前缀 key 等于绕过隔离);② **运行中的 8803 仍是旧代码 + 旧 .env**,须重启才对现网生效(该进程非本会话所起,未擅自 kill);③ `scripts/e2e-agent-access.mjs` 有一处**改动前既有**的 eslint 死变量 `probe()`(原 720 行,现 946 行),它使 lint-staged 对任何触碰该文件的提交必红 ⇒ 被迫 `--no-verify`(= 一次跳过约 80 项守门),删该死函数或加 `_` 前缀即可根治
