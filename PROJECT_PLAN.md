...
- [ ] **P0-3d admin 成本治理看板 — 数据填充验证** — `packages/database/seed/ai-cost-records-seed.ts` 补 2-3 个用户 × 3-5 个模型 × 7 天的测试 aiCostRecords + aiBudgets(用真实用户 ID),让 top-users/budget-alerts 端点有真实数据可展示;注册到 `packages/database/seed/index.ts` 第 11 步
- [ ] **P0-3e 预算告警 BullMQ 定时任务** — `apps/api/src/services/budget-alert-service.ts` 扫描 aiBudgets(scope='user')对比今日/本月消耗,80% warning/100% critical 触发 `notificationQueue` 推站内信 + `sendEmail` 发邮件(去重防重复告警,cooldown 6h);注册新 SCHEDULED_JOBS 项 `budget-alert-check` pattern `*/30 * * * *`(每 30 分钟);在 scheduler-worker.ts switch 加 case;NotificationType 枚举加 `BUDGET_ALERT` 类型;i18n 通知模板 5 语言补齐
