...
- [x] ✅(2026-09-13) **7. temperature/top_p/system prompt 参数面板(对标:CodeX/Qoder)**:模型选择器旁加"高级参数"抽屉(temperature/top_p/max_tokens/自定义 system prompt),按会话生效,后端透传 LLM 网关。**落地**:前端新增 stores/sampling-params.ts(全局默认 + 会话覆盖双层,清除走 delete 保证不被 undefined 遮蔽,persist 'ihui-sampling-params')+ components/chat/sampling-params-panel.tsx(输入区「高级参数」按钮 + Dialog 面板:temperature/top_p/top_k/max_tokens 数值输入、自定义 system prompt 文本域 8000 字计数、逐项清除/全部重置/设为所有会话默认);**链路补断点**——apps/api chatStreamSchema 与 chat-resumeSchema 补声明(此前 api-client 已支持但网关 zod strip 静默丢弃,与 workspaceContext/mode 同型坑)→ snake_case 透传 → ai-service LLMCompleteRequest 补 top_p/top_k/system_prompt + _inject_custom_system_prompt 置顶注入(独立实现,不依赖并行改动);并修复流式路径断链——complete_stream 的 _native_fc_kwargs 此前**完全不传 temperature/max_tokens**(Web 流式对话下采样参数被静默忽略),现补四项透传;send-message/send-answer/resume-stream 三处发送入口统一快照下发;测试:ai-service test_advanced_params_passthrough 9/9(流式/非流式透传 + 未传不注入 + system prompt 合并/插入/空值)、web sampling-params store 9/9;i18n chat.sampling 23 keys×5 locale parity 通过(死 key 扫描 0)。
- [x] ✅(2026-09-13) **8. Qoder Repo Wiki 对应物——项目知识库自动生成**:利用既有五维索引 + `context_engine.py`,给每个工作区自动生成"项目百科"页(目录结构/核心模块/依赖图/关键决策),AI 对话自动引用,可手动触发更新。**落地**:后端 repo-wiki-service 重写为真实索引采集——`collectRepoFiles`(目录遍历 + .gitignore 过滤 + 压缩预算 60000 字符 budgetRepoFiles)+ `generateRepoWiki`(五章节 markdown:概览/目录结构/核心模块/依赖图/关键决策,LLM 摘要降级纯结构版)+ 持久化 `repo_wiki_docs` 表(version 累加 + kind=overview/module);routes/repo-wiki.py 全套 CRUD(POST generate/GET latest/GET history/DELETE);**AI 对话自动引用**——repo-wiki-context.ts `loadRepoWikiContext`(overview + repoName 匹配 + userId 本人/公共,generatedAt 倒序取最新,8000 字符换行边界截断 + 截断提示,全程吞异常不阻塞主链路),ai-chat-stream/chat-resume 双入口 `repoName` schema 声明(防 zod strip)透传,ai-service llm.py `_inject_repo_wiki`(`<repo_wiki repo>` XML 隔离 + 属性转义 + marker 去重,不依赖并行改动);**手动触发更新**——前端 repo-wiki 页详情卡「重新生成」按钮(收集文件后可用,生成中禁用);**依赖图**——extractImportEdges 纯函数(TS/JS import/export-from 正则 + Python 仅 .py 生效防假边,模块归属与 budgetRepoFiles 一致,MAX_GRAPH_EDGES 60,mermaid 节点 id/表格竖线转义)renderDependencyGraph 挂 overview 文档;测试:pytest 17/17 + vitest 29/29 + api/types tsc 0 错 + web tsc 仅 4 条既有基线错;真实库集成验证 5/5(命中/换行边界截断/取最新/kind 过滤/空白 repoName 不打库)。
- [x] ✅(2026-09-13) **9. FIM 专用模型 + 接受率闭环(对标:Trae CUE Tab)**:FIM 现混用对话模型。做:模型目录立"补全专用"档位(接轻量代码模型);`window.__ihuiFimMetrics` 埋点接管理看板,接受率 <30% 自动告警。**落地**:**模型目录"补全专用"档位**——model_catalog.py `_FIM_MODEL_RULES`(codestral/codegemma/starcoder/tabby/stable-code/qwen-coder/deepseek-coder/\bfim\b 正则)+ `is_fim_model` + annotate_models 回填 fim 标记 + `pick_fim_model` 纯函数(显式请求透传/auto 取首个 fim/畸形元素跳过);fim.py `_resolve_fim_model` 选型接线(优先级 env `FIM_PREFERRED_MODEL` > 用户显式 model > 候选首个 fim 模型 > 'auto',`_load_fim_candidates` 与 /llm/models 同源基线 + 300s TTL 双检缓存 + 全程吞异常绝不阻塞补全);types `ModelCatalogFields.fim?: boolean` 正交标记;**接受率闭环**——前端 CodeEditor 增量差值上报(fimMetrics 累计值 − 基线 = 本次增量,5s 防抖 POST /api/llm/fim/metrics,accepted/dismissed 计数接入 Monaco 内容变更检测);后端进程内按 model 聚合(threading.Lock + 延迟样本上限 500),GET /metrics/summary 返回 acceptanceRate/p50/p95/alert(**alert = rate<0.30 且 suggestions≥20**,样本不足不告警、无建议不除零);管理看板 ai-metrics 页 FIM 卡片消费 summary,告警条目高亮;测试:pytest 80/80(pick_fim_model 5 类用例 + _resolve_fim_model 接线用例 + metrics 增量聚合/分位数/告警阈值),web/types tsc 仅既有基线错,eslint 0,i18n fim* 10 keys×5 locale parity 1902、死 key 0。
