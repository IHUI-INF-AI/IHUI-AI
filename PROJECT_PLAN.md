...
- [ ] G5 生产调用点接入 — `mcp_server.py` 新增 `_tool_knowledge_lookup(arguments)` 函数(包装 `knowledge_lookup()`)+ 注册到 `_TOOLS`(MCPTool schema)+ 注册到 `_TOOL_HANDLERS`(handler 调度表);LLM 可通过 MCP 协议直接调 `knowledge_lookup` 工具查代码库/RAG/历史会话三源;单测覆盖空 query / 三源成功 / 各源失败降级 / 参数透传;不加入 `_ADMIN_ONLY_TOOLS`(查询类,所有用户可用,类比 search_codebase)
