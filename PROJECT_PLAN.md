...
> 配套 user_profile 规则:测试账号强制使用 admin(username=admin / password=[REDACTED-PW] / email=[REDACTED-EMAIL] / phone=[REDACTED-PHONE]),禁止创建新测试账号。

### 目标

为 admin 账号(email=[REDACTED-EMAIL] / phone=[REDACTED-PHONE])在测试环境下启用固定验证码 123456,无需收真实验证码即可完成登录/注册/换绑手机等流程的自动化测试与 E2E 验证。
...
- [ ] **P2(草稿就绪,等用户操作)自动化 GitHub Trending 推送** — AI 已完成全部物料:① GitHub 侧已就位(20 topics / 222 字符 description / aizhs.top homepage / 7 Releases 最新 v1.2.0 / Discussions 开启 / 17 Issues 含 5 决策讨论 + #22 Welcome + #13/#14 功能建议 + Q&A);② 5 个 cross-publish 脚本 dry-run 全部验证通过(dev.to/Hashnode 15 篇博客 payload + ProductHunt 54 字符 tagline + V2EX 992 字符正文 + Reddit 2652 字符正文);③ V2EX 草稿 `.trae-cn/tmp/v2ex-post.md` + Reddit 草稿 `.trae-cn/tmp/reddit-post.md` + ProductHunt 草稿 `.trae-cn/tmp/producthunt-launch.md`(脚本可解析)已创建;④ 用户操作一键清单 `.trae-cn/tmp/user-action-checklist.md`(按 ROI 排序 11 个平台操作步骤)。**等用户操作**:V2EX 扫码登录 + Reddit 登录后复制粘贴,或注册 dev.to/Hashnote 获取 API key 后跑脚本
- [ ] **P3(草稿就绪,等用户操作)ProductHunt 提交 + HackerNews "Show HN" 实际发布** — AI 已完成全部物料:① Show HN 草稿 `.trae-cn/tmp/marketing-2026-07-28/hackernews-show-hn.md`(290 字英文 + 7 个常见问题答案 + 提交时间策略);② ProductHunt 草稿 `.trae-cn/tmp/producthunt-draft.md`(9 项要素齐全:Name/Tagline 53 字符/Description 960 字符/Topics/Gallery 6 张图建议/Maker Comment/Pricing/Website/Demo URL)+ `.trae-cn/tmp/producthunt-launch.md`(脚本可解析版);③ Launch Day 行动清单 + 风险预案(套壳质疑/demo 挂掉应对)。**等用户操作**:注册 ProductHunt maker 账号 + 绑定 Twitter + 提前 7 天预约 launch date + 提交产品;HackerNews 登录后复制粘贴 title+url
- [ ] **P3(草稿就绪,等用户操作)创建 Substack/Mirror 文章** — AI 已完成全部物料:① Substack 10 期 newsletter 大纲 `.trae-cn/tmp/marketing-2026-07-28/substack-newsletter-outline.md`(每期 1500-3000 字 + 订阅表单文案 + 邮件模板 + 增长策略 + 付费订阅阈值);② dev.to 交叉发布脚本 `scripts/cross-publish-devto.mjs`(15 篇博客 dry-run 通过,带 canonical_url 回指 aizhs.top);③ Hashnode 交叉发布脚本 `scripts/cross-publish-hashnode.mjs`(15 篇博客 dry-run 通过);④ Medium 交叉发布沿用 dev.to 内容(改 canonical_url 即可)。**等用户操作**:注册 Substack + 创建 publication + 按大纲每 2 周 1 期写作;dev.to/Hashnode 注册账号获取 API key 后跑脚本自动发布 15 篇博客
...
用户要求:"继续按你的建议去做执行,最多agent并行开发最大化效率,要求完美细致完整毫无遗漏"。**测试账号统一 admin(真实凭证:username=`admin` / password=`[REDACTED-PW]` / email=`[REDACTED-EMAIL]`;原误记为 `admin@ihui.ai`,该邮箱不存在,2026-08-01 已根治),禁止创建测试账号**(已写入用户规则 + AGENTS.md 配套)。
...
- [x] **浏览器验证(admin/[REDACTED-PW])**:Ctrl+Shift+J 打开 Pane / minimize 按钮 / Timeline tab / Overview tab / ResourceBudget / 搜索 Ctrl+F / dark mode 全部验证通过(0 hydration errors)
