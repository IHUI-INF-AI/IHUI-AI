...
Docker Desktop 启动 + db(8810)+ redis(8811)容器启动 + api(8802)+ web(8801)+ ai-service(8803)全链路就绪。补加 users 表 two_factor 列(0130_two_factor.sql) + 临时 DISABLE system_admin 触发器重置 admin 密码([REDACTED-PW])后验证。
...
| **登录获取 token**                     | POST `/api/auth/login` {admin/[REDACTED-PW]}                                                             | ✅ HTTP 200,返回 JWT token(Bearer ...)                                                                                                                                                                                                                                         |
...
- L6 ✅ 多模态记忆:agent_multimodal_memory 表持久化 + MultimodalEmbedder(image/audio/video/document 四模态 embed,三级降级 LLM→caption→hash 伪向量 128 维)+ MultimodalMemory(store/search/delete/update_importance/load_all_for_user,纯 Python cosine + importance 加权 + content_hash 去重)(2026-07-25,75 测试全绿)
- L7 ✅ 联邦学习:agent_federated_lessons 表持久化 + DifferentialPrivacy(Laplace/Gaussian 噪声 + apply_to_count/score + anonymize_user_id/text + k_anonymity_filter)+ FederatedLearner(aggregate_user_lessons 跨用户聚类 + DP 噪声 + PII 脱敏 + UPSERT + lifespan hydrate + build_system_prompt_snippet 注入)(2026-07-25,60 测试全绿)
- L8 ✅ 长程一致性:agent_session_summary + agent_working_memory_compression 双表持久化 + SessionSummarizer(summarize_session LLM JSON + compress_working_memory 三策略 summary/sliding_window/hybrid + list_user_summaries + search_relevant_summaries cosine)+ LongTermMemory(recall_cross_session + build_context_for_new_session + extract_key_facts 聚合去重 + update_importance + load_recent_summaries)(2026-07-25,96 测试全绿)
- L9 ✅ 元认知:agent_metacognition_log 表持久化 + ActiveForgetter(scan_stale_memories 跨 episodic/semantic/procedural 三表扫描 + forget_memory + demote_memory + merge_duplicates + cleanup_orphans)+ Metacognition(reflect_on_memories LLM JSON + 启发式降级 + detect_conflicts 关键词匹配 + get_reflection_history + build_system_prompt_snippet 注入)(2026-07-25,59 测试全绿)
