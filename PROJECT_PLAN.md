...
- [ ] G4 完整迁移 — ① `RAGService.retrieve_only()` 公有方法(替代 `_retrieve` 私有);② `knowledge_lookup.py` 迁移到 `retrieve_only()`;③ 新 `app/services/agent_tools.py` 提供 `make_knowledge_lookup_tool()` 工厂,让 `AgentLoopV2` 调用方一行接入 knowledge_lookup 工具;④ 单测覆盖 retrieve_only + 工厂 + 工具执行;⑤ README G4 章节从 PoC 升级为完整迁移
