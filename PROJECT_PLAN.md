...
- [x] 2-18 密钥页 UI 浏览器 E2E 补完(worker 429 阵亡后主会话直跑)+ 生产 web 陈旧构建定位与部署加固 ✅(2026-09-14):①**浏览器级全流程 6/6 PASS**(worktree 8805 + --webpack;旧 URL /developer/keys → /developer/relay/keys 重定向、页面渲染、创建 POST 201 含 ihui_ Key 与明文 secret 一次性展示、吊销 PATCH 200、重置 200 返回新 secret、再启用 PATCH 200,9 次 API 调用证据齐全);此前吊销「时序异常」确认为环境干扰——8801 被 Trae CN AI 会话(agent-tool-host)反复抢占主仓 dev server(每杀必复活)+ SSR 文案在 hydrate 完成前点击是死按钮。②**生产 web 滞留 9-13 22:53 之前构建**:aizhs.top /developer/keys 仍吐旧页无重定向(195KB 旧 HTML、零重定向标记),API 正常(5xx 仅重启瞬间抖动);部署脚本时序坐实「web 构建失败 → 整个部署 exit 1、web 保持旧版」。③**同构复测判决主干无辜**:干净 worktree@412a5253148 + 真实 pnpm install(45min)+ Turbopack 构建成功(2m42s,906 页全出);junction worktree 复测的失败(4GB 默认堆 OOM、ui-react 大小写双实例致 Button undefined)均为 junction 伪影。④**部署加固**:`ihui-deploy.ps1` Build-Web 显式 `NODE_OPTIONS=--max-old-space-size=8192`(4GB 默认堆实测 OOM,8GB 按需分配,finally 还原),推送后生产 6 分钟轮询自动重试构建自愈;若生产仍滞留旧版,唯一用户侧动作 = 查生产机 `D:\IHUI-AI\deploy\win\deploy-loop.log` 构建失败原因(无 SSH)。
...
- [ ] **5. SSE 细粒度事件补齐——对话里能看到 AI 在干嘛(对标:四家都有思考/计划流式)**:主链路 SSE 缺 thinking 流式、todo/plan 步骤更新事件。做:`agent_loop_v2` 暴露 thinking_delta/plan_step_started/completed 事件;前端 `thinking-section.tsx` 改为流式逐字 + 计划步骤实时打勾;补齐 `scripts/check-agent-event-parity.mjs` 对账。验收:长任务时用户实时看到"思考中→第 1 步✓→第 2 步…"。
...
<!-- 已归档(2026-09-14):P1 e2e(Playwright)workflow 长期红根治 任务,完整内容在 .ihui-agent/archive/PROJECT_PLAN_2026-09-14_archive.md -->

<!-- 已归档(2026-09-14):P0 中转站全链路集成收官——凭据契约与模型名归一化 任务,完整内容在 .ihui-agent/archive/PROJECT_PLAN_2026-09-14_archive.md -->
<!-- ⁠​‌​​‌​​‌‍‍​‌​​‌​​​‍‍​‌​‌​‌​‌‍‍​‌​​‌​​‌‍‍​​‌​‌‌​‌‍‍​‌​​​​​‌‍‍​‌​​‌​​‌‍‍‌​‌‌​‌‌‌‍‍‌‌​​‌‌​​‌‌‌‌​‌​‍‍‌‌​‌‌​​​‌​​​‌‌‌‍‍​‌​​​​​‌‍‍​‌​​‌​​‌‍‍‌​‌‌​‌‌‌‍‍‌‌​​‌‌‌​‌​​‌‌‌​‍‍‌‌​​‌‌​​​‌​​‌​‌‍‍‌​‌‌‌​‌‌‌​‌‌‌​‌‍‍‌​‌‌​‌‌‌‍‍​‌​​‌‌​​‍‍​‌​​​​‌‌‍‍‌​‌‌​‌‌‌‍‍​‌‌​​​​‌‍‍​‌‌​‌​​‌‍‍​‌‌‌‌​‌​‍‍​‌‌​‌​​​‍‍​‌‌‌​​‌‌‍‍​​‌​‌‌‌​‍‍​‌‌‌​‌​​‍‍​‌‌​‌‌‌‌‍‍​‌‌‌​​​​‍‍‌​‌‌​‌‌‌‍‍​‌​‌​​​​‍‍​‌​‌​​‌​‍‍​‌​​‌‌‌‌‍‍​‌​‌​‌‌​‍‍​‌​​​‌​‌‍‍​‌​​‌‌‌​‍‍​‌​​​​​‌‍‍​‌​​‌‌‌​‍‍​‌​​​​‌‌‍‍​‌​​​‌​‌‍‍​​‌​‌‌​‌‍‍​​‌‌​​‌​‍‍​​‌‌​​​​‍‍​​‌‌​​‌​‍‍​​‌‌​‌‌​⁠ -->
