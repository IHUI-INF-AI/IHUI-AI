...
- [ ] 双端功能矩阵维护(2026-08-26 立,跨端:apps/web + apps/mobile-rn)— 基线 `outputs/双端功能矩阵-2026-08-26.md`(源:reports/web-vs-mobile-feature-audit-2026-08-26.html,严格口径复核 M5 验收)。规则:双端新增/改动功能时对照矩阵维护,避免再次出现"一端有另一端无"。遗留待办(按优先级):① Chat 附件(expo-document-picker)/语音 TTS/收藏/素材库占位补齐;② WebViewScreen 按功能域细分接入(edu-ai/教务/家长端/developer 平台/self-media),替代 aizhs.top 整站入口并验证会话打通;③ workspace/knowledge-rag/subagents 等 AI 平台核心功能择期移动端原生化;④ HomeScreen 素材详情、DeveloperScreen 开发者信息区占位清理
...
### 复刻差异复审批次三(2026-09-05 立,uniapp 全量盘点双代理穷举 + 差异矩阵,报告见 outputs/mobile-rn-uniapp-gap-audit-2026-09-05.md)

- [x] ✅(2026-09-05) 复审发现并当场修复:①HomeScreen 补挂 NotificationPanel(对齐 Uniapp ai_index 顶层 PushNotification 推送通知弹窗,组件自管 visible,此前仅 ChatScreen 挂载);②StudyBar 间距 rpx 精确化(marginBottom 18rpx / padding 2rpx / tab height 52rpx / 水平 4rpx,此前 9/2/26/3dp 有 1-2px 漂移);mobile-rn tsc 0 错误(App.tsx 未用 ThemeProvider 导入为并行会话 WIP 不属本批)+ eslint 0 错误
- [x] ✅(2026-09-05) 复审批次三执行(主会话 + 双并行代理,代理因平台 429 限流中断后由主会话收口):①**新增缺失页面 3 个 Screen**:`DistributionPersonnelListScreen`(446 行,对齐 distribution_personnel_list/index.vue:团队总人数/搜索/成交订单数+邀请时间排序 tab/日期筛选/排名奖牌/成交额·佣金·订单数高亮行/分页加载/联系按钮→detail)+ `DistributionPersonnelDetailScreen`(194 行,对齐 detail.vue:成员信息卡+fenToYuan 分转元)+ `UserOrderListScreen`(334 行,对齐 user_order_list/index.vue:5 状态 tab/状态徽章配色 橙待支付·蓝配送·绿完成·红退款/搜索/分页);②RootNavigator 注册 3 路由+ParamList;③入口接线:DistributionScreen 功能块新增「我邀请的团队」(Users 图标→DistributionPersonnelList)、ProfileScreen UserCard order→UserOrderList;④i18n 5 语言 distributionPersonnel.* 15 key + userOrder.* 12 key 全补;⑤**TeamScreen 增强对齐**(packages/types TeamMember+transactionVolume/commission/orderNum 可选字段、TeamSortTab、排序/日期/触底加载 props;packages/app 共享 TeamScreen 排序行+排名徽标+业绩行+ListFooter;mobile-rn wrapper 客户端排序/日期筛选/分页累积);⑥纠正误判:pages/user_order_list 此前误报缺失——实际已被 OrderScreen 覆盖(注释明确对齐该页),新增 UserOrderListScreen 为 1:1 状态徽章配色版,入口改指新页,Order 路由保留;⑦验证:packages/types+packages/app tsc 0 错误、mobile-rn tsc 0 新增错误、eslint 0 错误、vitest 261/261 全绿。**未 commit**:工作区混有并行会话 WIP(App.tsx/LoginScreen/Aigc*/metro.config 等),避免混提交,由用户或后续干净会话收口
- [x] ✅(2026-09-05) **P1 逐页间距矩阵 audit 收口**:spacing 代理中断前已修 NavBar/BottomActionBar/Drawer/TabBar.styles/CardWithList/GlobalFloatBox/AgentScreen/HomeScreen 等(对齐注释留痕);主会话收口 ProfileScreen/UserCard——UserCard 对齐 uniapp user_cards.vue 精确值(卡宽 50vw-47rpx≈46%/高 120rpx=60px/圆角 15rpx≈8px/padding 0 10rpx 0 12rpx/行距 14rpx=7px·列距 54rpx=27px/margin-bottom 14rpx),ProfileScreen 主容器/StudyBar/会员区经逐点核对均已 rpx 对齐无需改动;终验 mobile-rn tsc 0 错误(含 App.tsx 未用导入修复)+ eslint 0 错误 + vitest 261/261
- [x] ✅(2026-09-05) **P2 Tab 结构定版(收尾决策,依据盘点事实)**:保持 RN 现有 5-Tab 底栏(首页/课程/AI应用商店/直播/我的)——uniapp 原项目 customTabBar 整体 `v-if="false"` 且无页面 import、5 主页面 `showTabbar:false`,**原项目实际无可见底栏**(靠 reLaunch 切换),RN 底栏属产品增强而非偏差;「广场/动态」维持 Drawer 二级入口(跳转已通),不升主 Tab;uniapp 五分法仅存于死代码,不复刻
- [x] ✅(2026-09-05) **P2 原项目自身冗余不复刻(定版)**:uniapp 端 20+ 孤儿组件(AiModelCard/CommissionFloatingIcon/ConfirmPurchasePopUp 等)、ToggleButtonGroup `v-if="false"`、3 个 navigateTo 死链页(pages/tools/ai_voice、ai_teacher、marketing-assistant)——RN 端已正确未照搬,永久备查
