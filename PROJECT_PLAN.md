...
### P0 安全与核心架构债清零
- [ ] 修复 `csdn_publish.py` 中的 `CSDN_APP_SECRET` 硬编码问题，迁移至环境变量。
- [ ] 补全 `admin-missing-routes.ts` 和 `missing-user-routes.ts` 中的 51 条 API 空桩（Mock data 替换为真实业务逻辑）。

### P1 深度代码质量治理
- [ ] 清理测试环境硬编码密钥（`tbox.test.ts`, `embedding-provider.test.ts` 等），迁移至 Mock 或环境变量。
- [ ] 修复代码库中的 149 处 `@ts-ignore` / `eslint-disable`（重点：`terminal-panel.tsx` 等）。
- [ ] 补充 `mcp_server.py` 及其他核心模块的缺失测试用例（消除 `TODO: test`）。
...
- [ ] 修复 `apps/extension/lib/config.ts` 等文件中的弃用 API 调用。
...
- [ ] 消除脚本中的绝对路径硬编码（如 `C:\`, `D:\`, `G:\`），改用项目相对路径或动态推导。
