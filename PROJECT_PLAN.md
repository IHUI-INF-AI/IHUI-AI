...
- [ ] 修复代码库中的 149 处 `@ts-ignore` / `eslint-disable`（重点：`terminal-panel.tsx` 等）— **2026-07-26 重新精确统计**:实际 145 处(非 149),分布 100 文件,Top 命中:`apps/web/src/components/ui/dropdown-menu.tsx` 9 处 + `apps/web/src/components/rules/rules-manager.tsx` 9 处 + `apps/web/tests/visual/sidebar-height-verify.spec.ts` 16 处 + `apps/web/app/(main)/admin/ai-metrics/page.tsx` 5 处 + `apps/web/app/(main)/registry/page.tsx` 3 处;按目录分布:scripts/ ~30 处(多为守门脚本合法抑制)+ apps/extension/sidepanel/pages/ ~20 处 + apps/web/ ~40 处 + apps/api/ ~10 处 + packages/ ~5 处;后续按文件分批处理(每个高频文件单独 subagent)
