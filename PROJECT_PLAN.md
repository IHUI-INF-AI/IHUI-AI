...
- [ ] 数据库:教师时间表(edu_teacher_schedule)+ 排课规则表(edu_scheduling_rule)
- [ ] API:自动排课(基于教师时间/教室/班级约束)、冲突检测、调课申请/审批
- [ ] 前端:排课管理界面(拖拽/快速排课)、冲突提示、调课审批流程
- [ ] 侧边栏新增导航入口 + i18n 翻译补充
- [ ] typecheck + lint + build 全绿 + 页面 HTTP 200

### 批次5:作业管理(P2)

- [ ] 数据库:作业提交记录表(edu_homework_submission，关联 learnHomework)
- [ ] API:作业CRUD、学生提交(附件/文本)、教师批改(评分/评语)、完成率统计
- [ ] 前端:作业布置(教师)/提交(学生)/批改(教师)/统计看板
- [ ] 侧边栏新增导航入口 + i18n 翻译补充
- [ ] typecheck + lint + build 全绿 + 页面 HTTP 200

### 批次6:招生管理(P2)

- [ ] 数据库:线索表(edu_lead)+ 试听预约表(edu_trial_booking)+ 报名记录表(edu_enrollment)
- [ ] API:线索管理(创建/跟进/转化)、试听预约/确认、报名处理、转化分析
- [ ] 前端:招生看板、线索管理(表格+筛选)、试听排期、报名处理
- [ ] 侧边栏新增导航入口 + i18n 翻译补充
- [ ] typecheck + lint + build 全绿 + 页面 HTTP 200

### 批次7:财务管理(P3)

- [ ] 数据库:学费标准表(edu_tuition_fee)+ 缴费记录表(edu_payment_record)+ 退费记录表(edu_refund_record)
- [ ] API:学费配置、缴费/退费、欠费提醒、财务汇总报表
- [ ] 前端:学费设置、缴费页面(学生)、退费申请、财务报表(图表+表格)
- [ ] 侧边栏新增导航入口 + i18n 翻译补充
- [ ] typecheck + lint + build 全绿 + 页面 HTTP 200

### 批次8:现有功能优化

- [ ] 课程表:批量复制上周课表(一键快速排课)、教师冲突检测(同教师同时间段告警)、导出/打印课程表
- [ ] 菜谱:营养分析展示(热量/蛋白质/碳水可视化图表)、食材采购清单自动生成、菜品图片上传预览
- [ ] 学习计划:进度可视化(甘特图/进度条)、教师审核学生备注、计划完成率统计报表
- [ ] typecheck + lint + build 全绿 + 页面 HTTP 200
