...
- [x] ✅(2026-09-01) H12:界面语言问题排查结论 — **代码层无英文问题**:SSR `request.ts` 硬编码 `zh-CN`;客户端 `I18nProvider` 读 `useLanguageStore`(localStorage `ihui-language`,默认 zh-CN);`zh-CN.json` 707KB 抽查 key 全中文;`GlobalTopBar.tsx` 全部走 i18n 无硬编码英文;无任何 Tauri 自动设英文逻辑。**判定**:最新安装包英文界面属安装包/测试环境问题(旧包缓存 / 系统 locale 影响 WebView2 默认),待新构建安装包实证
