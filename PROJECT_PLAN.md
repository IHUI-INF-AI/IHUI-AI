...
- [ ] **58. 运营面板 WS 实时化**(**4-3 唯一未落地项**;保留:告警/渠道状态已有 60s 轮询与手动刷新,WS 为体验增强项)(对标 /ws):告警/渠道状态/用量走 WebSocket 推送,替代轮询。**2026-09-17 核验**:全仓 relay 无任何 WS 端点与广播(现有 `ws-*` 插件仅覆盖 chat / ai / terminal / 客服),轮询链路(告警规则 5 分钟评估 + 渠道状态 60s 刷新 + 手动刷新)可用,故按体验增强项保留,不计入"已落地"。
...
- [x] ✅(2026-09-17) **4-4-12 运营控制台 4 新模块全栈落地(lottery/points-mall/promotion-rules/tax)**:4-4-11 豁免的 4 条「无表」外部模板页按「完整开发、不混乱」要求转正——①`packages/database` 新增 4 表(lotteries/points_mall_products/promotion_rules/billing_tax_rates,迁移 `20260917100000` 幂等,本地 PG 实跑通过);②`apps/api/src/routes/admin/console-modules.ts` 用 `registerCrud` 工厂 4 组 × 5 端点(list/:id/POST/PUT/DELETE+batch);③前端抽共享 `crud-resource.tsx`(useCrudResource hook + CrudFormDialog + i18n 化列头/表单/删除确认),4 页 `/admin/lottery|points-mall|promotion-rules|tax` 全部真实读写;④5 语言 i18n parity。删 4 条豁免。
- [x] ✅(2026-09-17) **4-4-13 剩余 7 条移植壳契约适配(签到规则/敏感词/排课×3)**:侦察后按各页真实数据层分别处置——①`signin-rules`:后端仅 PUT,补全为 registerCrud CRUD(`extraReward` jsonb `{description}`);②`sensitive-words`:后端 CRUD 已有,唯前端错调 `/security/sensitive-words`(误触发豁免注册)+ 缺 word 搜索——前端改调真实 `/admin/sensitive-words`,查询 schema 补 `word` ilike;③`scheduling`:后端 hyphen 路径(`/scheduling-rule|/teacher-schedule|/schedule-change`)本已有完整 CRUD,slash 版 POST/PUT/DELETE 是补的桥接而前端 GET 却调了 slash 版——删除 6 条桥接 + 前端整体改调原生 hyphen 路径,`check-conflicts` 重写为全学期冲突扫描(termId → 教师/教室时间重叠人读串),新增 `GET /teachers`(RBAC teacher 角色 distinct);④`/admin/roles` 授权页:后端新增 `GET /roles/:id/users`(已授权,username/phone 搜索)、`GET /roles/:id/unallocated-users`(notInArray 子查询)、`POST|DELETE /roles/:id/users` 批量授权/取消、`DELETE /roles/:id/users/:userId` 单个取消,前端 4 文件字段对齐(`userName/phonenumber`→`username/phone`);⑤`/admin/visit-trend`:新端点 `GET /stats/visit-trend` 基于 visit_logs 真实字段聚合(day/week/month 桶:PV/UV 趋势、新增 UV、覆盖城市、来源域名、热门页面),**裁剪 duration/bounceRate 无数据源假指标**,前端 StatCard 换 新增访客/覆盖城市。删 4 条豁免。
- [x] ✅(2026-09-17) **4-4-14 移植壳收口验证与合并(豁免 12→3,全绿)**:两批提交 `134c565dbd0`(4-4-12)+ `f0ec2661598`(4-4-13)→ merge origin/main(desktop CI 链 10 提交,merge-tree 冲突 0)→ `51aae8c7a38` 推 feature 分支 + main(ls-remote 复核双分支一致)。守门 strict 全仓 EXIT=0(后端 4801 条路由注册,前端 1847 处调用,豁免仅剩 3 条:favorites×2 为归一化误报如实登记 + 1 条历史占位);api/web/database tsc 全绿;i18n 5 语言 parity OK/死键 0/破碎英文 0;schema drift 0;全量 typecheck 25 包通过。**教训:多 Edit 并行写同一文件会相互覆盖丢改动**(crud-resource/edu-ai-management 实证)→ 单文件编辑必须串行,复杂批量改动用 Python 脚本落盘 `.ihui-agent/tmp/` 兜底。

- [x] ✅(2026-09-18) **4-4-15 4-4-13 实装后的四处回归根治 + 两处隐藏缺陷**(验证链全绿、真实 DB 端到端冒烟 23/23):上批实装暴露的问题逐条根治——①**FST_ERR_DUPLICATED_ROUTE 事故根因**:`admin-extended/role-routes.ts` 与 `admin/stats.ts` 重复注册 `POST|DELETE /admin/roles/:id/users(:userId)`,Fastify 启动即抛错、服务反复崩起;去重后 role-routes 为唯一实现,stats 只留 GET(注释固化「此处不得重复注册」)。②**批量取消授权被 410 打穿**:`DELETE /admin/roles/:id/users` 原被 `deprecated-routes.ts` 的 gone stub 占用,而前端 `cancelAllMut` 正在调用——补真实 `{userIds[]}` 实现(`removeUserRoleBatch` 返回实际删除行数),授权侧兼容 `{userId}` 与 `{userIds}`(`addUserRoleBatch` onConflictDoNothing 幂等),删 gone stub。③**knowledge-card 双重 send 500**:本地 `requireLogin` 作 preHandler 失败时仅 `return false`,Fastify async hook 返回假值**不中断生命周期** → handler 二次 send 抛 FST_ERR_REP_ALREADY_SENT;新增 `requireLoginHook`(失败 `return reply`)替换 5 处 preHandler,内联布尔版保留(仓库既有正解见 require-permission.ts requireAuth 的 P1 修复)。④**registerCrud 参数硬化**:`idParamSchema` 是松散 `z.string()`(`/ai-model-config/:id` 等数值 id 端点在用,不可直接改),另立 `crudIdParamSchema`(`z.uuid()`);`fields()` 对 `Number()` 强转产物 NaN 在 POST/PUT 拦为 400(否则 NaN 写 PG 数值列变 500);批量 `?ids=` 逐个验 UUID。**另修两处隐藏缺陷**:⑤`listAtomicCheckpoints` 按目录名倒序而快照 id 只含**秒级**时间戳+随机后缀 → 同秒两个快照顺序随机翻转(实测 1/6 抖动),改按 `meta.createdAt` 倒序(连跑 8/8 稳定);⑥**生产 P0:`buildServer` 的 `pluginTimeout` 放宽到 120_000(不限环境)**——adminMissingRoutes 内部动态 import 十余子路由,注册耗时随 import 图增长;超 avvio 默认 10s 即抛 AVV_ERR_PLUGIN_EXEC_TIMEOUT,**进程不退出(仅 unhandled rejection)但也永不 `server.listen()`**,服务「活着却不监听」。生产与开发都跑 TS(生产 NSSM `run-api.ps1` 走 tsx loader),两条链路同样中招:9/18 生产实测 8802 无监听、日志 AVV_ERR × 3、web 反代 /api/* 全 500 持续数小时(节点进程 07:10Z 启动后零 listening),本地冷启动 8821 亦复现一次;本仓测试对同一插件早已统一 120_000。**验证链**:api tsc 0 错误 → 守门 strict EXIT=0(4798 路由 vs 1847 调用,豁免 2 条不变)→ apps/api 全量 vitest **396 文件 / 6414 用例全绿**(同步修 2 处滞后测试:monitor/alerts 用非 UUID 占位 id、signin-rules 旧 published/draft 枚举;并把「非 UUID id → 400」用例的 body 改成合法值,否则会被 NaN 校验提前拦成 400 而测不到 UUID 守门)→ 8822 冷启动日志零 FST_ERR/AVV_ERR → 真实 DB 冒烟 **23/23 PASS**(冷启动 + 建角色 + 批量授权 2 + 幂等重放不重复 + 已授权/未授权列表 + 批量取消清空 + 单个取消 + 空数组 400;registerCrud 非 UUID/NaN → 400、合法 UUID → 200/201 且真实落库并删除)。冒烟脚本 `.ihui-agent/tmp/smoke-rbac-2026-09-18.mjs`(免密签发 admin JWT:`JWT_SECRET` + HS256 手工签名 + `sub` 用真实 admin UUID,避免改本地 admin 密码干扰并行会话)。
