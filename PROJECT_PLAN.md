...
- [ ] O19 该门首跑暴露的**真实 schema 漂移**:7 表 13 列存在于迁移后的库而 `packages/database/src/schema` 零命中(`daily_call_limit`/`tpm_limit`/`alias` 等)。现以 `KNOWN_SCHEMA_HOLES` 基线**只豁免"多列"方向、逐条注明原因、修好后打印 stale 提醒**(防基线变僵尸豁免);要把这 13 列正式并回 schema 或删掉迁移侧残留才算清账 —— "多出来的列无人负责"是运维地雷
...
- [ ] batch-58 合入 main(用户已拍板,机械被阻):merge-tree 判定**唯一**冲突文件 = `apps/web/src/components/agents/UnifiedTaskDashboard.tsx`,其余 78 个 b58 变更文件可自动合并;但主工作区有 5 个文件既被并行安全会话改脏又在 b58 变更集内(llm_gateway.py / mcp_server.py / network_approval.py / session_store.py / test_mcp_server.py,最后写入 10:45),`git merge batch-58` 已实测**零副作用中止**(HEAD 未动、零文件触碰)。**解阻条件:那 5 个文件的会话 commit 后,在 main 直接 `git merge batch-58` 并解决 UnifiedTaskDashboard.tsx 一处冲突即可**。解阻后立即执行的遗留清单(已由只读盘点定死,证据=整词 grep 消费方统计,主会话已复核):① `path_guard.py` 为敏感目录黑名单单一权威源(3 消费方),`image_saver.py:99 validate_save_path` 的 `_FORBIDDEN_PARTS`(7 项,缺 `.next`,与权威源 8 项已漂移)应改复用权威源;② `image_saver.py`(生产 0 消费)、`dispatch_helper.py`(0 消费,且缺 mcp_server `_tool_dispatch_subagent` 的 progress_callback/嵌套深度护栏)为未接线等价副本,按 §7 收敛;③ `automation_executor.py` **确认无等价替代实现**(mcp_server 的 configure_automation_task 只支持 schedule/dispatch/webhook,不跑 steps),独有多步编排功能,保留待接线;④ `mcp_marketplace.py`(545 行,含独有 CapabilityRegistry/install kit)与 `session_handoff.py`(582 行,HandoffPacket 聚合,与 session_relay 定位重叠)全仓含测试零引用 —— 独有功能按 §7 不删,登记为待接线,待产品拍板接线排期
...
- [ ] O13c 自助创建的 OAuth 应用**删不掉**:`DELETE /api/auth/oauth/apps/:clientId` 对刚由 `POST /api/auth/oauth/apps/create` 创建、同一 owner 的应用返回 **400**(生产实测;创建返回 200)。需定位是参数 schema 还是所有权判定。生产现存一条测试残留 `oauth_apps` 行(名称前缀 `oauthchan-`),待本条修复后清理 —— 记在此处以免被当成已清理干净。
