...
- [x] ✅(2026-09-24) **查清"镜像恒红"的真正机理不是等 GC 而是**自我喂养**(提交 `317acc479b4`)**:逐轮取证 run #3570-#3575 —— 远端内部备份标签已删净(`git ls-remote` 实测远端仅剩 20 枚标签、内部标签 0 枚),本轮只推 main + 7 枚标签,服务端仍回 `Repo size: 1603.148MB, exceeds quota 1024MB` 并把 **main 与删除操作一起拒**(pre-receive 仓库级判定)。体积为何反增:`09-23 21:55 = 1060.676MB` → `09-24 00:17 = 1603.148MB`,中间正好 4 轮 ≈ **135MB/轮**,与我们每轮发送的整仓 pack 同量级 —— receive-pack 是"先收对象、后跑钩子",被拒的包留在服务端等 housekeeping ⇒ ***/20 分钟的定时重试本身就是把仓库推离配额的那只手**。修法:全量推送前先跑 **0 字节配额探针**(把服务端已有的 main sha 推到一个新 ref 名:对象全在服务端、传输 0 字节,但 pre-receive 照样执行),命中配额即本轮短路并点名"等 GC";探针 ref 建成即删。取证脚本 `.ihui-agent/tmp/20260924-rootfix/mirror-keylines.mjs`(整份 job 日志按决定性关键词取行 —— 按步骤切片会命中 fetch 阶段的 `[new tag]` 噪音,看不到真因)。
- [ ]（进行中）**上述探针的 CI 实证未闭环**:`workflow_dispatch` 已受理(run **#3576**,03:41:46Z 起),步骤 1-4 全成功、步骤 5「镜像到 Gitee」到 04:2x 仍 `in_progress`(**21 分钟,已超历史最长一轮 22min 的门槛**)。两种解释都要脸:① 探针未拦住 —— Gitee 的体积判定可能**只在真收到新对象时才触发**,那么"0 字节探针"这个设计前提就不成立,须改为"直接比对上轮 job 日志的配额判词再决定是否推";② 配额已释放、这轮真在推全量(那就是恢复)。回读判据(一条命令):`node .ihui-agent/tmp/20260924-rootfix/mirror-keylines.mjs` ⇒ 若结论行含 `配额探针通过` 且推送成功=②;若仍见 `exceeds quota` 且探针段打印的是"通过"=①,须按①改判据。**责任归属:本会话已把判据与两条出路写死在这里,下一个接手的人不必重新发现。**
- [x] ✅(2026-09-24) **堵掉 `2n-web` 的两处洞(提交 `16f9741dd`)**:① 旧注释假设"文件不在暂存区 ⇒ 工作区与 HEAD 一致",共享工作区里不成立 —— 此刻磁盘上并发会话未暂存的 WIP 就是活证据(web 的 en/ja/ko 三包删掉 `admin.announcements.maintenanceNotice` 整块 17 键,而 **HEAD 里五语言齐全**),于是一次与 i18n 毫无关系的提交被 blocking 拦红;现改为**未暂存一律读 `HEAD:<path>`、暂存过的读 `:<path>`**(全量模式仍读工作树,那是 CI/人工审计该看的口径)。② 同轮被变异测试挖出的**假绿**:两处 `catch {}` 静默丢弃读失败的语言包,parity 少比一门却照样打印"通过" ⇒ 新增 `unreadablePacks` 记账,任一语言包读不出即点名文件与原因并 exit 1。四路实跑:`--staged` 绿且真比对 **5 语言 × 23043 键路径**;全量口径仍红并点名 17 键(未变瞎);**阳性对照**(临时索引把脏 en.json 标为已暂存)⇒ rc=1;反向 A/B(revspec 故意改坏成 `::path`)⇒ 输出「❌ 1 个语言包读不出来 ⇒ parity 实际只比对了 4 门语言,拒绝当作通过」,还原后复绿。对照脚本 `.ihui-agent/tmp/20260924-rootfix/ctrl-mutation.sh`。
- [x] ✅(2026-09-24) **根 `node_modules/.bin` 缺 shim 这一型:§12e 的既定 remedy 无效,已补上唯一生效的一步**:本会话连吃三次 pre-commit 红,真因是 `.bin/` 里 **eslint / tsc / tsserver / rimraf 四个 shim 不见**(包本体完好)→ lint-staged 第一步报 `'eslint' 不是内部或外部命令` → 每次 commit 必红 → 各会话按 §12 合法 `--no-verify` → **约 110 道守门对全队同时失效**。而 `pnpm install` 与 `pnpm install --force` 都在 **1.2 秒内**回 "Already up to date"(pnpm 认为树是好的;本机根 `.modules.yaml` 甚至不存在),**不会重新生成 shim** ⇒ 新增 `scripts/repair-node-bin-links.mjs`(提交 `067f1dc41f9`,幂等 + `--check` 零副作用),AGENTS.md §12e 同步改写并把判据落成"实测版本号"。实测:`--check` 修前点名缺 4 个,修复后 `eslint --version = v10.8.1`、`tsc --version = 5.9.3`、**经 cmd 宿主跑 `node_modules\.bin\eslint.CMD` 同样出号**(lint-staged 就走这条),随后一次真实提交的 hook 日志出现 `✔ eslint --fix ...` 与 `✔ Done running tasks for staged files!`(此前是 `✖`)⇒ 阻塞面已清。
- [x] ✅(2026-09-24) **我自己造成并立刻回补了一次共享文档回退(提交 `94a67e88ca7` → 回补 `29d8feb451c`,+15/-0)**:`94a67e88ca7` 的 blob 取自"几分钟前读的 HEAD",期间并发落了 10 枚提交,于是那票实际写成 `10 insertions / 16 deletions` —— 把他人刚入库的 §26 三段规则行顶掉了,其中一行恰好就是**「提交活文档前必须做工作树 ⊇ HEAD 行级对账」**。教训固化成两条硬动作:① **读 HEAD 与落笔之间不得隔任何工具调用**;② 纯新增不够,**提交后必须再回读一次 `HEAD:<file>` 是否仍 ⊇ donor**(本次就是靠这条回读发现 a03b03509b1 只顺带走了部分行、仍缺 15 行)。工具 `.ihui-agent/tmp/20260924-rootfix/restore-agents-lines.mjs`(HEAD 为骨 + donor 锚点纯新增,四断言:HEAD 每行仍在 ∧ donor 每行仍在 ∧ 目标他人行确被找回 ∧ 我自己的行未丢)。
- **点名他人持有、本会话不代改的两项**:① 共享**索引**里长时间挂着一份陈旧 `PROJECT_PLAN.md`(门 84 `check-stale-revert --staged` 因此反复红,连无关提交一起被拦 —— 谁 staged 的谁来收敛,处置是把它前向提交或 `git restore --staged` 后由作者重做);② `apps/web/node_modules/vitest` 被指向 **`C:\Users\Administrator\.workbuddy\tmp\d41-vitest-fix\node_modules\.pnpm\vitest@4.1.10_...`** 这个临时实验目录(门 78 因此在"破损 0 条 / 1 条"之间闪断,同步盘或清理一动手就变成 web 端测试全体起不来)—— 正解是 `pnpm install` 重链接或改回仓库内 store,由该实验的作者做。
- **残余(不写作收口)**:本票三条里前两条已各自有实测闭环,**唯一未闭环是 Gitee 探针的 CI 实证**(#3576 结论未回读),判据与两条出路已钉在上面那条;另有并发会话同时在修 node_modules(其记忆条目点名"被在跑进程锁住的平台包只能等重启窗口"),故依赖树深层残留归他们,不重复动。
