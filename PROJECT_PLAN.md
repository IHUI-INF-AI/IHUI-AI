...
- [x] ✅(2026-07-26) LLM provider 字典化阶段 3 — 主工作暂不执行(触发条件:第 N+1 版本发布后,所有用户已升级 .env 格式),**前置工作已完成 3/3**:① `scripts/migrate-llm-providers.mjs`(阶段 2 commit `3210dbf01` 落地,阶段 3 增强 `--backup`/`--strip-flat`/`--redact` flags 待他 agent 提交);② `scripts/check-llm-provider-schema.mjs` warn-only 守门(commit `84c164268` 落地,297 行 / 7 条校验规则 / 31 provider 白名单);③ `docs/llm-provider-stage3-changelog.md` 6 章节发布说明(commit `84c164268` 落地,385 行);另:迁移/守门 npm scripts 已在另一 agent 的 `package.json` 修改中(`check:llm-provider-schema` + `migrate:llm-providers`,待其 commit);`config.py` 已加 `DeprecationWarning` 监控 fallback 命中(为阶段 3 触发条件观测埋点);主体工作待触发条件满足后执行,详见 `docs/llm-provider-stage3-changelog.md` §3
- [ ] G4 知识查询统一门面 PoC — `apps/ai-service/app/services/knowledge_lookup.py`:`knowledge_lookup(query, *, user_id, repo_id, session_id, top_k_per_source, source_priority)` 并发查 codebase_indexer / RAG / long_term_memory 三源,聚合为 `KnowledgeLookupResult(hits, errors, duration_ms)`,按 `source_priority` 排序,IO 失败降级返回空(`§3` 最小化 PoC:门面 + 单测 + README,**不接入调用点**,迁移留后续 task);单测覆盖三源 fallback / priority 排序 / 空结果 / 错误降级
