...
## O50 C 盘"怎么占了这么多"全量对账:34.75GB 逐项验身份后回收,并把三类系统自产残骸接进每日维护(2026-09-24 立并完成 ✅,单端工程治理:scripts + 机器配置)
...
- [x] ✅(2026-09-24) **每日维护脚本 `scripts/c-drive-auto-maintain.ps1` 从 4 段扩到 6 段**,把"不是我们的产物、但系统每天在产且无人回收"的三类接进去:第 5 段 5a 打印队列(只删超 3 天的队列文件,**刻意不停 Spooler** —— 凌晨重启服务会取消用户过夜排队的真实作业,被占用文件走 `[FAIL]` 下一轮重试)、5b 内核转储(超 7 天 **或** 超出 1024MB 上限按最旧先删,保留最新一条供事后排查)、5c 孤儿浏览器构建(声明根与默认位置不一致即判孤儿);第 6 段是**封禁而非删除**:写 `HKCU:\SOFTWARE\Policies\Google\Chrome\OptimizationGuideModelDownloadingEnabled=0`(Chrome 静默下载 4GB `weights.bin`,2026-07 删过一次又长回来,只删不封等于每天回到原点)。所有删除仍只走 `ForceDelete` 这一个出口 ⇒ `-DryRun` 天然覆盖新段;汇总行同步补 4 个新计数。**副作用如实登记**:存在 Chrome 策略键后设置页会显示「浏览器由所属组织管理」,撤销=删该 DWORD。段编号 `[n/4]`→`[n/6]` 已全部改齐无残留,`Parser::ParseFile` 语法 0 错,受影响的 `check-c-drive-pollution.test.mjs` + `seal-c-root-stray.test.mjs` + `check-pwsh-version.test.mjs` **23/23 绿**(含"重解析点只断链绝不递归删"与"量 pagefile 必须用 `AllocatedBaseSize`"两条护栏断言)。
...
- **O50 残余(不写作收口)**:① **Chrome 策略是否真被 Chrome 认,还差消费者自证** —— 注册表侧已四口径确认在位、政策名与外部报道一致,但最终判据是下次打开 Chrome 看 `chrome://policy` 是否列出该项;未做是因为那要启动用户的浏览器。② **AGENTS §26 的"功能 ①②③④"清单落后于脚本实际的 6 段** —— 该文件今夜被并发会话整文件回写多次(见 O49⑤),按 §12 的活文档对账流程单独处理,不夹带在本票里。③ **`C:\Windows\Temp` 里有 5 个我们的一次性脚本**(`clean-o7b.cjs`/`verify-o7b.cjs`/`oauthchan.cjs`,09-21,§25 违规面),体积仅 2.2MB 且本票已把它们收掉,但**污染守门看不见这一类**:`check-c-drive-pollution.mjs` 按名字白名单只认 `ihui-*`/`probe-*` 等前缀,`clean-*.cjs` 既不匹配"我们的"也不进"未识别清单" ⇒ 判据有洞,补法要先想清楚"无名字特征的东西如何归因",不当本次顺手改。④ **DriverStore 里两份 `iigd_dch.inf`(2.21+1.70GB)旧显卡驱动包未动** —— 需 `pnputil /enum-drivers` 精确点名被取代的那份再删,误删当前版等于把 GPU 驱动打坏,收益 1.7GB 不值得冒这个风险盲做。⑤ 飞书 `Service Worker` 缓存**没有应用侧上限**,还会回潮;真要根治只能在飞书设置里限缓存,不归本仓管。⑥ Adobe CameraRaw 2.23GB、Trae SOLO CN 4.14GB、`Local\Programs\Python`(torch)1.83GB 属"应用数据/在用环境",按 [[feedback-cleanup-verify-before-delete]] 一律未动,只登记。
