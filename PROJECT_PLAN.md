...
- [x] ✅(2026-07-15) **P1-1 expert_courses 数据补全**:`packages/database/seed/ai-tutorials.ts` 追加 11 大类专家课程清单(吴恩达/李宏毅/李沐/Stanford CS229+CS231n/3Blue1Brown/Karpathy/fast.ai/DeepSeek/LangChain/RAG/LLM 综合 + 国家平台 + 官方文档),共 ~30 条记录,source 字段标记 `expert_courses.json`。typecheck ✅ exit 0。
- [x] ✅(2026-07-15) **P1-2 ask 模块前端封装**:`apps/web/src/lib/community-api.ts` 新增 11 个函数 — updateAsk/deleteAsk + AskAnswer CRUD(getAnswers/createAnswer/updateAnswer/deleteAnswer) + getMyAsks/getMyAnswers + 批量查询getAsksByIds/getAnswersByIds + 统计 countMyQuestions/countMyAnswers。typecheck ✅ exit 0。
- [x] ✅(2026-07-15) **P1-3 exam 模块前端封装**:`apps/web/src/lib/exam-api.ts` 新增 12 个函数 — getExamChapters + ExamSignUp CRUD(getSignUp/saveSignUp/cancelSignUp) + getMySignUps/getMyRecords/checkSubmitted + getFavoriteExams/getRecommendExams/getHotExams/getExamsByIds。typecheck ✅ exit 0。
- [x] ✅(2026-07-15) **P1-4 token 自动刷新**:`apps/web/src/stores/auth-token.ts` 实现 scheduleRefresh — 过期前 5 分钟(REFRESH_THRESHOLD_MS=5min)自动 POST `/api/auth/refresh`,成功后 setToken 续期,clear 时清理 timer。typecheck ✅ exit 0。
- [x] ✅(2026-07-15) **P1-5 menu 动态路由消费**:新增 `apps/web/src/hooks/use-admin-routers.ts`(调用 `/api/admin/menu/getRouters` + 模块级 cached 缓存);`AdminNav.tsx` 接入 useAdminRouters,动态路由优先(visible≠0 + sort 排序),按 path 匹配 ADMIN_NAV 合并图标,fallback 到硬编码。typecheck ✅ exit 0。

### goal 深度架构迁移完整性审计(2026-07-15)— achieved
