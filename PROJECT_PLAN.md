...
- [ ] 2-12 迁移记账守门脚本(防再现污染):新增校验 ①`drizzle/meta/_journal.json` 的 entries 与 `drizzle/*.sql` 文件名一一对应(无孤儿/无缺失) ②`drizzle.__drizzle_migrations` 行数 == journal 条数 且 hash 全为合法 sha256(禁 `NOFILE:`/`manual_` 伪值与重复) ③journal 的 `when` 单调递增、且账本 `max(created_at)` 必须小于未来 `when`(否则 `drizzle-kit migrate` 静默空转) ④迁移 `.sql` 禁止写入零宽水印字符(会被 sha256 通道判为内容变更)。落地位置:`scripts/` + pre-commit 第 14 项;验收:注入一条孤儿迁移文件能让守门失败。
