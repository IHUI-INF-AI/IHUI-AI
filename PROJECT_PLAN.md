...
- [ ] **P3(下一步,需用户配合)ProductHunt 提交 + HackerNews "Show HN" 实际发布** — Show HN 草稿已就绪(`.trae-cn/tmp/show-hn-post.md`),ProductHunt 提交草稿已就绪(`.trae-cn/tmp/producthunt-draft.md`,9 项要素齐全:Product Name / Tagline / Description / Topics / Gallery 建议 / Maker Comment / Pricing / Website / Demo URL),需用户注册 maker 账号 + 提交产品
...
- [ ] **P3-4.6 Server-Driven UI 草稿+预览模式(P3-4.5 增强)**— admin 编辑改动不立即生效,先存为草稿(`home_schema_draft` key),预览确认后再发布到生产 schema。实现:① 后端复用现有 /api/admin/configs + /api/configs,新增 `home_schema_draft` key(isPublic=true 用于预览页加载);② api-client 加 getHomeSchemaDraftConfig fetcher;③ use-home-schema.ts 加 useHomeSchemaPreview hook(读 ?preview=draft query param,加载 draft schema);④ (marketing)/page.tsx + (main)/home/page.tsx 接入 preview hook,?preview=draft 时渲染草稿;⑤ admin/home-schema/page.tsx 改造:加载时同时加载生产 + 草稿(draft 不存在时 fallback 生产),编辑改的是草稿,"保存草稿"按钮保存到 draft key,"预览"按钮新窗口打开 `/?preview=draft`,"发布"按钮把草稿拷贝到生产 schema,"丢弃草稿"按钮删除 draft key 重置为生产;⑥ browser 验证 4 状态(默认/编辑草稿/预览/发布)
