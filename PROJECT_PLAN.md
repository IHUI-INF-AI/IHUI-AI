...
### 全端数据埋点补齐(2026-08-31 00:07-01:25 收尾)

- [x] ✅(2026-08-31) **埋点现状审计 + 全端补齐** — 审计结论:web+后端链路完整(visit_logs/analytics_events/api_logs/llm_call_logs/crash_reports/audit 链/metrics),mobile-rn/extension 零埋点、小程序仅崩溃、CLI 默认关。**补齐**:① 后端 `log-retention-daily` 定时任务(每日05:30,LOG_RETENTION_DAYS 默认90天,分批5000删 api_logs/visit_logs/analytics_events,0=安全阀;service+单测4/4);② mobile-rn 新建 src/lib/analytics.ts(访问/行为/崩溃,AsyncStorage会话)+ App.tsx 路由 onStateChange + ErrorUtils 全局崩溃捕获;③ miniapp-taro 新建 src/utils/analytics.ts(Taro.onRouteChange 运行时特性检测)+ app.tsx;④ extension 新建 src/lib/analytics.ts + SidepanelApp 路由埋点 + background 生命周期事件;⑤ desktop 无需改码(frontendDist=../../web/out 自动生效);⑥ CLI telemetry 默认开启 + endpoint 缺省派生 `${apiUrl}/api/analytics/track`;⑦ web 登录/注册显式埋点。**业务事件接入**(4 agent 并行,文件域隔离):web chat_send/conversation_create/payment_success/publish_success/search 5 处;mobile-rn login_success(3方式)/register_success/chat_send/conversation_create/payment_success 5 文件;miniapp trackLogin(wechat×4+phone×3)/chat_send/payment_success 6 文件;extension login_success/chat_send/conversation_create;CLI commander postAction hook → command_run。**端到端实测**:POST /api/analytics/track + /visit-tracking/visit-log + /crash-reports 三路模拟上报 → psql 查库三表均落库 ✓。**验证**:6 端 typecheck 全绿 + mobile-rn 261 测试 + CLI telemetry 39 测试 + lint 0 违规 + miniapp weapp 编译成功。

### 全库表治理执行(2026-08-31 02:27-03:10)

- [x] ✅(2026-08-31) **表审计结论全量执行** — ① **补表**：codebase_chunks 已建（pgvector 未装，embedding 临时 TEXT 占位，装后 ALTER TYPE vector(1536)+HNSW，insert 冒烟通过）；② **删死表 131 张**（668→537）：先全仓 git grep -F 二次核验（发现 agent 自动扫描 27% 误判，48 张在用表如 sys_*/t_article/t_favorite/notification_channel/zhs_course/user_token_balance 全部保护保留）→ 全库 pg_dump 备份(66MB) → 分批 DROP → ORM 定义同步删 131 块(28 文件) → 误删 5 张(zhs_crew_*/zhs_knowledge_doc，常量名≠表名核验漏检) 已从备份 pg_restore 恢复，行数一致(3/10/10/2/7)；③ **audit_logs 自动分区**：新建 audit-partition-service.ts + scheduler 注册 audit-partition-daily(每月1日,Asia/Shanghai 建下月分区,边界连续防 default 冲突)；④ **audit_chain_entries DROP migration 已执行**；⑤ **整合评估降级**：message_system_notice↔notifications、message_private_letter↔messages、zhs_ai_model_info↔ai_model_config_models、ai_cost_records↔llm_call_logs 四组经结构核实均语义不同（全站广播vs用户通知/email串vs uuid/市场目录vs配置子表/平台内部账本vs中转站账本），**正确降级为保留+注释**，仅删 4 张零引用 message_* 死表。**验证**：database/api/mobile-rn/miniapp/extension/cli/api-client typecheck 全绿；web 唯一阻塞为预存损坏文件 apps/web/app/(main)/a2a/page.tsx（667 行被压缩破坏，非本次改动，待用户确认 git checkout 恢复）。
