...
### [x] ✅(2026-07-22) P3 深化:§22 README 同步规则机制守门集成(平台独占:仅守门脚本 + 文档,2026-07-22 立)

**触发**:承接 commit `4f9ca4f`(P3 深化 + AGENTS.md §22 新规则 + check-readme-sync.mjs 守门脚本就绪)。§22 规则要求"集成位置:`.husky/pre-commit` 第 22 项(warn-only)",但实际 pre-commit 第 22 项是 `check-staged-files.mjs`,新脚本未挂载,机制守门闭环未完成。

**范围**(平台独占:仅 .husky/pre-commit + README.md + AGENTS.md,守门脚本本身,不触及运行时能力):
- `.husky/pre-commit` 新增第 22 项 `check-readme-sync.mjs`(warn-only),原第 22 项 `check-staged-files.mjs` 顺延为第 23 项
- `README.md` E4 守门脚本速查表补齐 6 项缺漏(#10 openapi-check / #19 staged-pollution / #20 tailwind-class-conflict / #21 multi-end-sync / #22 readme-sync / #23 staged-files)
- `README.md` 全局统一 pre-commit 钩子计数(22 → 23 / 21 个 → 23 个),含首页能力矩阵 + 工程守门章节 + 决策 4 演化注记 + 三重门禁 + 贡献流程 + 客观承认不足等 11 处
- `README.md` 决策 4 追加"演化(2026-07-22)"小节,记录从 21 → 23 的 4 项新增决策理由
- `AGENTS.md` 守门脚本速查表章节标题更新(1-19 项 → 1-23 项)+ 补齐 #10 / #23 缺项

**约束边界**:
- 平台独占豁免(§9):仅守门脚本 + 文档改动,不触及 apps/* / packages/* 运行时代码,不影响跨端调用链路
- 不改 §22 规则原文(规则已 commit `4f9ca4f` 落地,本次只做机制集成 + README 同步)
- 不归档历史决策 4(保留"21 个 pre-commit"原文作为初始决策快照,仅追加演化注记)

**完成证据**:
- `node scripts/check-readme-sync.mjs --staged` exit 0 ✅(warn-only,正确识别其他 agent 改动的功能文件并发 warn)
- `node scripts/check-readme-sync.mjs` exit 0 ✅(working tree 模式正常)
- README.md 全局 grep 残留 = 3 处(均为决策 4 历史快照 + GitHub issue 链接,保留不动)✅
- AGENTS.md 速查表条目数 = 29(含 16/16b/17-post 共 3 项后置条件闸门 + post-commit)✅

**Git 同步证据**:
- 本地 commit: `80de1e351`
- origin commit: `80de1e351`
- 同步状态: local == remote ✅
- 守门脚本: `node scripts/git-push-guard.mjs` 自动 push + 验证 local == remote ✅
- push 过程:post-commit 钩子触发 → 首次 push 因 pre-push typecheck 失败(packages/api-client/src/client.ts 其他 agent 代码 TS6133 错误)→ 按用户规则"--no-verify 跳过其他 agent 代码 hook 失败"重试成功
...
8. **前端 iteration 徽章**(commit 4fb6e7c77):web ToolCall 接口加 iteration 字段,use-chat.ts 解析 SSE iteration 并透传,ToolCallCard iteration>1 时显示"第N轮"徽章,message-list.tsx 传递 prop
9. **AGENT_CONTROL_INTERNAL_SECRET 时序 bug 修复**(commit bbc110b87):mcp_server.py 模块级变量 _AGENT_CONTROL_SECRET 在 main.py 同步 os.environ 之前求值 → 永远为空 → MISSING_SECRET。改为函数 _get_agent_control_secret() 动态从 settings 读取,config.py 加字段,main.py 同步 os.environ
...
- python -m py_compile apps/ai-service/app/core/config.py apps/ai-service/app/main.py apps/ai-service/app/services/mcp_server.py exit 0 ✅
- SSE 端到端实测(test-sse-multiturn.py):tool-call-start + tool-result 各 1 个,iteration=1 ✅;errorCode 从 MISSING_SECRET → EXECUTION_FAILED(secret 已加载,因 api 未启动连接失败,符合预期)✅;LLM 无幻觉,正确报告"工具执行失败"✅;stub=False,LLM API key 解析正确 ✅
- 代码层验证全绿(注:web/api 因其他 agent 代码问题 Internal Server Error,不在本任务范围)
