...
## 四、中严重度缺失（R35 验证：M-1/M-2/M-3/M-4/M-5/M-6/M-7/M-8/M-10/M-11/M-12/M-13/M-14/M-15/M-18 已修复；M-9 部分修复；M-16/M-17 未修复；M-19/M-20/M-21/M-22/M-23/M-24/M-25/M-26/M-27/M-28/M-29 未修复；R18 安全基础设施缺失仍未修复）
...
- **R35 更新**：✅ noise-rules.yml 已迁移为 [monitoring/alertmanager/noise-rules.yml](file:///g:/IHUI-AI/monitoring/alertmanager/noise-rules.yml)（未追踪文件），8 条抑制规则完整迁移（PGInstanceDown→PGConnectionsHigh/PGReplicationLagHigh/PGRollbackHigh/PGBloatHigh + PGConnectionsExhausted→PGConnectionsHigh + PGDeadlockDetected→PGRollbackHigh + PGLongTransactionCritical→PGLongTransaction + PGCacheHitLow→PGConnectionsHigh）
- **严重程度**：低（N+1 检测 + Prompt 注入防护已生效；Saga 需业务接入；noise-rules.yml 已迁移但未追踪 commit）
...
| 状态        | M 项                                                                   | 数量  |
| ----------- | ---------------------------------------------------------------------- | ----- |
| ✅ 已修复   | M-1/M-2/M-3/M-4/M-5/M-6/M-7/M-8/M-9/M-10/M-11/M-12/M-13/M-14/M-15/M-18 | 16 项 |
| ⚠️ 部分修复 | 无                                                                     | 0 项  |
| ❌ 未修复   | M-16/M-17/M-19/M-20/M-21/M-22/M-23/M-24/M-25/M-26/M-27/M-28/M-29       | 13 项 |

**R34 修复项**（2026-07-11）：

- M-9 noise-rules.yml 补建：monitoring/alertmanager/noise-rules.yml，9 条 Alertmanager inhibit_rules 迁移自旧架构 server/noise-rules.yml
- web React 19 类型问题解决：pnpm install 重新解析依赖后 typecheck 0 errors（原 130 个 TS2786 错误已消失）
- 临时文件清理：11 个 tmp_old_*.txt 文件删除
- drizzle 快照技术债记录：0040_snapshot.json 仅含 1 表(orders)，33 个快照文件缺失(0007-0037/0041-0042)，但 drizzle-kit generate 功能正常，重建风险高，标记为已知技术债

**结论**：全部阻断级(B-1~~B-4)和高严重度(H-1~~H-15)缺失已修复。中严重度 16/29 已修复、0/29 部分修复、13/29 未修复（均为非阻断项）。**R34 将 M-9 从部分修复升级为已修复（noise-rules.yml 补建），M-6/M-10/M-12 在 R29 已升级为已修复**。未修复 13 项需产品确认是"未迁移"还是"已废弃"。

### 13 项待确认产品决策清单（R34）

| M 项 | 缺失内容                 | 影响范围                                                | 产品决策问题                            |
| ---- | ------------------------ | ------------------------------------------------------- | --------------------------------------- |
| M-16 | Web Worker fileWorker.ts | 文件处理后台任务                                        | 新架构是否需要浏览器端文件处理 Worker？ |
| M-17 | 客户端服务层 25 个       | client/src/services/ 40+ 文件中 25 个无对应             | 这些服务是否已由服务端 API 覆盖？       |
| M-19 | Composables 35+          | client/src/composables/ 80+ 文件中 35+ 无对应           | 这些组合式函数是否已由 hooks 覆盖？     |
| M-20 | API 模块 17 个           | tools/ranking/checkin/developer API keys 等 17 个无路由 | 这些 API 是否已废弃或合并到其他路由？   |
| M-21 | 开放平台/Feature Center  | Dashboard + APIs/Agents/Documents/Models/SDKs Hub       | 开放平台是否还在产品规划中？            |
| M-22 | 散点缺失 10 个           | user-model-chat/admin-faq 等 4 API + 3 数据 + 3 配置    | 这些散点功能是否还需要？                |
| M-23 | pricing 计算引擎         | calculateCost() AI 定价计算                             | 新架构是否需要前端定价计算？            |
| M-24 | 测试覆盖 244+183→72      | server/tests/ 244 + client/e2e/ 183 → 72 文件           | 测试覆盖大幅缩减是否可接受？            |
| M-25 | S3 lifecycle/Locust/部署 | S3 分层+Locust 压测+Helm/K8s                            | 新架构部署方案是否已变更？              |
| M-26 | Storybook 组件开发环境   | client/.storybook/ 4 stories                            | 新架构是否需要 Storybook？              |
| M-27 | 专项 CI/CD 7 个          | server/.github/workflows/ 18 个中 7 个无对应            | 这些专项工作流是否还需要？              |
| M-28 | 业务可观测性指标 26 项   | zhs_biz_* 前缀 26 个指标                                | 新架构是否需要这些业务指标？            |
| M-29 | 基础设施指标             | DB 池 6 项+SQL+WS+活跃连接                              | 新架构是否需要这些基础设施指标？        |
