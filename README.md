...
- 窗口失焦非活动态:窗口失去系统焦点时按钮降亮,对标 Windows caption(`data-window-inactive` + `group/wc` + `group-data-[window-inactive=true]/wc:` 变体挂点三件套)
...
| 27         | check-z-index-guard.mjs                                                  | **z-index 层叠防护(blocking,2026-09-22 加固:补 skipEnv + onFailHint + 新增第 5 项两组契约):5 项判据 — tokens/globals 禁 !important、layout.tsx inline script `--z-*` setProperty 齐全、dialog.tsx 遮罩禁 open 态 fade-in、GlobalTopBar 窗口控制两组契约(等效压暗层 `data-window-controls`+`data-window-controls-dim` / 失焦非活动态 `data-window-inactive`+`group/wc`+`group-data-[window-inactive=true]/wc:` 变体);判据有效性自查 `--self-test`。跳过:`HUSKY_SKIP_Z_INDEX_GUARD=1 git commit ...`** |
...
| 57         | check-chat-element-coverage.mjs                                          | **AI 对话流元素覆盖清单(blocking)**:`scripts/data/chat-flow-elements.json` 登记 106 条(88 条 G-ID 差距 + 18 条已实现锚点),含 G-70 反向清单(我方领先项,禁止"补齐");`entryCountBaseline` 只挡倒退不挡增长,`status: planned` 只计数不判定                                                                                                                                                                                                                                                               |
