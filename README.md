...
| 小程序(Taro)         | `miniapp_ui` / `miniapp`             | describe / read / navigate / invoke                         | WXML 无同源 DOM，故无 click/fill/submit；导航仅放行 `app.config.ts` 生成的页面白名单，`invoke` 仅放行显式登记的命令，**退出登录永不暴露**；可调用命令面 = 主题三档 + 五个 tab 切换 + 回到页面顶部 + 五种界面语言（语言切换走 `I18nProvider` 挂载时注册的 setter，拿不到 setter 如实返回 `EXECUTION_FAILED`，不报假成功）                                                                                                                                        |
| RN(Expo)             | `app_ui` / `rn`                      | describe / read / navigate / invoke                         | 协议与工具族(`mobile_ui_*`)已就绪,端侧桥已落地:无同源 DOM 故无 click/fill/submit;`navigate` 仅放行 204 条 Screen 白名单(生成器解析 `RootNavigator` 已登录分支 + `RootStackParamList` + `linking.ts` 的 `:param`);`invoke` 只登记 4 条显式命令(主题三档 + 回首页),**退出登录/注销/支付/清缓存一律不登记**；可调用命令面 = 主题三档 + 五个 Main tab + 五种界面语言（同走 Provider 注册的 setter，未挂载即如实失败）;RN 无 URL,`screen` 字段语义是"根到叶激活路径" |
