...
| 57         | check-chat-element-coverage.mjs                                          | **AI 对话流元素覆盖清单(blocking)**:`scripts/data/chat-flow-elements.json` 登记 110 条(89 条 G-ID 差距 + 21 条已实现锚点),含 G-70 反向清单(我方领先项,禁止"补齐");`entryCountBaseline` 只挡倒退不挡增长,`status: planned` 只计数不判定                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 77         | check-radius-single-source.mjs                                           | **全 8 端圆角单一源头对账(blocking,2026-09-23 立)**:档位表唯一真相源 `packages/design-tokens/src/radius.js`(xs2/sm4/md6/lg8/xl12/2xl16)。判据 A:radius.js ↔ tokens.css `--radius-*` ↔ miniapp-taro app.css ↔ tailwind-preset 四处逐档同值(preset 必须引用 `RADIUS_REM`,不得重新内联)——历史上 v3 preset 的 `rounded-sm`=2px 与 web v4 的 4px 同名不同值,是"手机上所有容器圆角与全局设定不一致"的根因。判据 B:端内取用必须引用档位(RN `rnRadius.lg` / CSS `var(--radius-lg)` / 类名 `rounded-lg`),数字字面量、`rpx()` 算圆角、每文件 `*_RADIUS` 常量、`rounded-[任意值]` 一律红;真圆/胶囊(头像/装饰点/半高胶囊)不得方档化,须 `size/2` 或同行 `radius-exempt: 原因` 显式声明。存量走基线棘轮只减不增。判据不锚定行首(`width:16px; border-radius:50%` 这类同行多声明、以及 cli 分享页/设计模板/扩展 content script 里 **TS 模板字面量生成的 CSS** 同样可见,后者须用 `${RADIUS_CSS_PX.md}` 插值而非写死)。`--self-test` 20 例(含真实表端到端对账),镜像测试 `node --test scripts/tests/check-radius-single-source.test.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
只登记、不定性、不清理(清理类任务的铁律:先验明身份)。另单独判一条 **TEMP 漂移** ——
