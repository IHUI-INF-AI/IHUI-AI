...
| 77         | check-radius-single-source.mjs                                           | **全 8 端圆角单一源头对账(blocking,2026-09-23 立)**:档位表唯一真相源 `packages/design-tokens/src/radius.js`(xs2/sm4/md6/lg8/xl12/2xl16)。判据 A:radius.js ↔ tokens.css `--radius-*` ↔ miniapp-taro app.css ↔ tailwind-preset 四处逐档同值(preset 必须引用 `RADIUS_REM`,不得重新内联)——历史上 v3 preset 的 `rounded-sm`=2px 与 web v4 的 4px 同名不同值,是"手机上所有容器圆角与全局设定不一致"的根因。判据 B:端内取用必须引用档位(RN `rnRadius.lg` / CSS `var(--radius-lg)` / 类名 `rounded-lg`),数字字面量、`rpx()` 算圆角、每文件 `*_RADIUS` 常量、`rounded-[任意值]` 一律红;真圆/胶囊(头像/装饰点/半高胶囊)不得方档化,须 `size/2` 或同行 `radius-exempt: 原因` 显式声明。存量走基线棘轮只减不增。判据 B 覆盖五种取用形态:B1 数值与**字符串形态**(`borderRadius: '8px'` / `"6px 6px 0 0"`)、B2 本地圆角常量(含名字不带 RADIUS 的 `BAR_RX`)、B3 CSS px/rpx 字面量、B4 `rounded-[任意值]`、B5 **SVG `rx`/`ry`**;数字字面量一律红,几何圆须显式 `radius-exempt` 标记。判据不锚定行首(`width:16px; border-radius:50%` 这类同行多声明、以及 cli 分享页/设计模板/api `swagger-theme` 里 **TS 模板字面量生成的 CSS** 同样可见,后者须用 `${RADIUS_CSS_PX.md}` 插值而非写死)。`--self-test` 34 例(含真实表端到端对账、静态/内联 SVG 两种语义、NaN 防回归),按文件自验 `--files <a> <b>`,镜像测试 `node --test scripts/tests/check-radius-single-source.test.mjs` |
