...
| **模型可用性过滤**          | `apps/ai-service/app/services/model_availability.py`          | ModelAvailabilityService,启动时 + 每 5 分钟后台 ping 所有已配置 key 的 provider:① 优先查 balance_endpoint(OpenRouter/DeepSeek/SiliconCloud/StepFun)→ 余额=0 标 DOWN+PAYMENT_REQUIRED,余额>0 标 HEALTHY;② 降级发 max_tokens=1 推理 ping,402→PAYMENT_REQUIRED,401→INVALID_KEY,403→FORBIDDEN,429→DEGRADED,超时→TIMEOUT。`/llm/models` 端点调用 `is_model_available()` 过滤,账户没钱/key 失效/接不通的 provider 模型不进用户模型列表。PENDING(启动初期)宽松显示,DEGRADED(限流/延迟高)仍显示 |
| **Provider 余额健康 admin** | `/admin/providers-health` + `GET /llm/providers/availability` | 13 个 provider 实时健康面板:状态(healthy/degraded/down/local/zero_cost/pending)+ 余额(余额>0 绿色,=0 红色)+ 延迟 + 错误类型(payment_required/forbidden/invalid_key/timeout/network_error)+ 错误详情。账户没钱的 provider 显示"去充值"按钮(跳转 OpenRouter/DeepSeek/SiliconCloud 等 recharge_url),30s 自动刷新。用户规则:账户没钱的 provider 不应进模型列表,管理端需可视化 + 跳转充值                                                                                            |
