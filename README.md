...
| **B. 端侧 UI 动作桥接**        | `web_ui_*` 七工具经 `agent-control` 通道(category=`ui`)下发到用户浏览器，前端执行后回传结果；RN/小程序各七工具走 `app_ui`/`miniapp_ui` | 站内导航 / 按钮点击 / 表单填写 / 表单提交 / 页面读取 / 命令面板与模式调用（RN、小程序无 DOM，click/fill/submit 打在端内**控件注册表**上：组件挂载时交出 `onPress`/写入通道才可操作，没交出就如实 `UNSUPPORTED_ACTION`，不存在"回了 ok 而界面没动"） | `ui_action_bridge.py` + `apps/web/src/lib/ui-action-registry.ts` + `apps/{mobile-rn,miniapp-taro}/src/lib/{ui-action-registry,ui-field-registry}.ts` |
...
| 小程序(Taro)         | `miniapp_ui` / `miniapp`             | describe / read / navigate / invoke / click / fill / submit | 七动词与 web 同形,但真机是 WXML、无可枚举同源 DOM,故 click/fill/submit **打在控件注册表上**(`src/lib/ui-field-registry.ts`):组件挂载时交出 `setValue`/`onPress`/`onSubmit`,没交出的控件 `writable/pressable=false`,fill/click 一律如实 `UNSUPPORTED_ACTION`,**不存在"回了 ok 而界面没动"**;id 序号全局单调且永不复用 ⇒ 用旧快照猜 id 只会失败,不会误改别的控件;密码/验证码连快照都不出现,删除/支付/提现/分享类文案一律拒绝入表。导航仅放行 `app.config.ts` 生成的页面白名单,`invoke` 仅放行显式登记的命令,**退出登录永不暴露**;可调用命令面 = 主题三档 + 五个 tab 切换 + 回到页面顶部 + 五种界面语言(语言切换走 `I18nProvider` 挂载时注册的 setter,拿不到 setter 如实返回 `EXECUTION_FAILED`)                                                                                                                                                                                                                                                |
| RN(Expo)             | `app_ui` / `rn`                      | describe / read / navigate / invoke / click / fill / submit | 与小程序同一套机理:`@ihui/ui-native` 的 `Input`/`Button` 挂载时经 globalThis 约定键把写/触发通道交给端内 `src/lib/ui-field-registry.ts`(下层包不得反向依赖 app,故走约定键;宿主不在就返回 `null` 照常渲染)。**受控 `Input` 只有在父组件给了 `onChangeText` 时才标 `writable`** —— 那正是用户键盘输入走的同一条 path;没给就如实不可写。协议与工具族(`mobile_ui_*`)已就绪;`navigate` 仅放行 204 条 Screen 白名单(生成器解析 `RootNavigator` 已登录分支 + `RootStackParamList` + `linking.ts` 的 `:param`);`invoke` 只登记 4 条显式命令(主题三档 + 回首页),**退出登录/注销/支付/清缓存一律不登记**;可调用命令面 = 主题三档 + 五个 Main tab + 五种界面语言;RN 无 URL,`screen` 字段语义是"根到叶激活路径"。**接入量是量出来的**:机制已通,但 RN 侧走 `@ihui/ui-native` 的输入框目前只有 1 处(`AgentRuntimePanel`),另有 25 处裸 `<TextInput>`(14 文件)与 476 处裸 `Pressable/TouchableOpacity` 尚未接入 —— 逐屏改样式需真机回归,不做无验证的批量改写 |
...
| RN / 小程序输入框   | click/fill/submit 打在端内**控件注册表**上(无同源 DOM 就不查节点) | 已落地(2026-09-21):组件挂载时交出 `setValue`/`onPress`/`onSubmit` 才可操作,没交出就如实 `UNSUPPORTED_ACTION`;**接入量是量出来的** —— 逐屏采纳仍在推进,见 PLAN 遗留待办 B12 |                                                                                                                           |
...
> 三端各自 endpoint/`category`，互不抢指令；三端**动作集相同(七动词)**,但 web 靠同源 DOM 定位元素,
> RN 与小程序无 DOM ⇒ click/fill/submit 打在端内控件注册表上(组件挂载时交出通道才可操作,否则如实失败)
...
- **LLM 中转站造血**:OpenAI 兼容 v1 API + Key 池轮转 + FallbackRouter 故障转移 + Redis 响应缓存(60% 命中)
