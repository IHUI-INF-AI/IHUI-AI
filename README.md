...
| 77         | check-radius-single-source.mjs                                           | **全 8 端圆角单一源头对账(blocking,2026-09-23 立)**:档位表唯一真相源 `packages/design-tokens/src/radius.js`(xs2/sm4/md6/lg8/xl12/2xl16)。判据 A:radius.js ↔ tokens.css `--radius-*` ↔ miniapp-taro app.css ↔ tailwind-preset 四处逐档同值(preset 必须引用 `RADIUS_REM`,不得重新内联)——历史上 v3 preset 的 `rounded-sm`=2px 与 web v4 的 4px 同名不同值,是"手机上所有容器圆角与全局设定不一致"的根因。判据 B:端内取用必须引用档位(RN `rnRadius.lg` / CSS `var(--radius-lg)` / 类名 `rounded-lg`),数字字面量、`rpx()` 算圆角、每文件 `*_RADIUS` 常量、`rounded-[任意值]` 一律红;真圆/胶囊(头像/装饰点/半高胶囊)不得方档化,须 `size/2` 或同行 `radius-exempt: 原因` 显式声明。存量走基线棘轮只减不增。判据不锚定行首(`width:16px; border-radius:50%` 这类同行多声明、以及 cli 分享页/设计模板/扩展 content script 里 **TS 模板字面量生成的 CSS** 同样可见,后者须用 `${RADIUS_CSS_PX.md}` 插值而非写死)。`--self-test` 41 例(含真实表端到端对账、静态/内联 SVG 两种语义、NaN 防回归、**判据 C 新端未归类必红**),按文件自验 `--files <a> <b>`,镜像测试 `node --test scripts/tests/check-radius-single-source.test.mjs`。**判据 C 覆盖面对账**:全仓任何含圆角的跟踪文件必须落在 `SCAN_DIRS` 或 `OUT_OF_SCOPE`(带理由)二者之一,新端/新目录静默逃逸即红;`.py` 亦枚举(ai-service 的发布 HTML 渲染器就写在 Python 里),当前已声明范围外 43 个(美术 33 / 内容渲染 4 / 报告生成器 1 / docs 1 / 表自身 4)并在结论行如实报数 |
