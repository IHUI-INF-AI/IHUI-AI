...
| 大类              | 模块                | 关键能力                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AI 对话与模型** | 多模型对话          | 176 模型 / 智能路由 / 60% 缓存命中 / 流式 SSE + WebSocket / 对话收藏 / 历史记录 / 分享 / 模板                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
```
   ...
   └────┬─────┘  └────┬─────┘  └────┬─────┘  └──────┬─────┘  └────┬─────┘  └────┬─────┘
        ...
        └─────────────┴─────────────┴───────┬───────┴─────────────┴─────────────┘
            ...
            │  ├─ FTS5 全文检索           │   │  ┌─ 17 provider + 16 IM 渠道  │  + 14 publish adapter
...
.\scripts\cleanup-external-junk.ps1     # 清理 G:\ 根目录外部工具污染(16 目录 + 31 文件,支持 -Force 自动模式)
.\scripts\cleanup-memory-topics.ps1     # 清理 memory topics
```

### G:\ 根目录外部工具污染防护(2026-07-24 立)

`G:\` 根目录会因三类原因累积项目外垃圾:

1. **外部 Qt 工具污染**:`微信支付商户API证书工具 V1.4.exe` 等 Qt 应用在 G:\ 根目录双击运行时,Qt 框架自动释放 `platforms/` / `iconengines/` / `imageformats/` / `styles/` / `bearer/` / `translations/` 插件目录 + Qt5*.dll + `CA/cert/WXCertUtil` 证书工作目录到当前工作目录。
2. **其他 IDE 配置异常**:QoderCN 等 IDE 的 venv 命令可能把 `C:\` 错写成 `g:\c\`,导致 Python 在 G:\ 下创建完整路径链 `c\Users\Administrator\.workbuddy\...`;路径解析失败时还会创建 `nonexistent-root/` 兜底目录,JDK 探测缓存写到 `.appdata/`。
3. **早期 agent 临时文件违规**:2026-07-23 v2 守门立规前,agent 把 hover 截图 / sidebar .vue / .diff / .log 等临时文件随手写到 `G:\tmp\`。

**根治方案**:

- **清理**:`powershell -ExecutionPolicy Bypass -File g:\IHUI-AI\scripts\cleanup-external-junk.ps1 -Force`(agent 自动模式,16 目录 + 31 文件清单,不用通配符,不触碰其他应用目录)
- **预防**:
  - 外部 Qt 类工具(微信支付证书工具 / .exe 安装包 / 解压即用工具)禁止在 G:\ 根目录运行,必须放到 `G:\tools\` 或项目子目录
  - pnpm 命令禁止在 G:\ 根目录执行(会创建 `.pnpm-store` v11 与项目内 v3 冲突)
  - 使用 QoderCN 等 IDE 时,若发现 `G:\c\Users\Administrator\.workbuddy\` 被创建,需检查 IDE Python venv 路径配置
  - agent 创建临时文件必须用 `.trae-cn/tmp/`(由 `scripts/check-workspace-hygiene.mjs` v2 blocking 守门,pre-commit 第 25 项)

详见 [AGENTS.md §15 G:\ 根目录外部工具污染防护](AGENTS.md)。
