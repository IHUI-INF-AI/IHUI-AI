...
| 26         | check-parent-pollution.mjs                                               | **项目父目录污染巡查(blocking:agent 在项目外直接创建文件)**。2026-09-23 加固:新增 `CREDENTIAL_DIR_NAMES` 8 项目录名 `secrets`/`secret`/`credentials`/`credential`/`密钥`/`certs`/`certificates`/`.pybcrypt`(比较按 `toLowerCase()`,大小写不敏感) **整目录不扫** —— 因 `--auto-clean` 对「文件名强信号命中」直接 `unlinkSync` 实删文件且无二次确认(内容双信号命中仅告警不删),此前会把用户凭据库 `D:/DevEnv/secrets/ihui-app-password.txt` 连 agent 垃圾一起删掉(同类陷阱第二次,前例 `ihui-release.keystore.说明.txt`)。**纪律:命中项落在凭据目录内时,先补豁免再清理,顺序不可反**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
