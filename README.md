...
<p align="center">
  <strong>542 张表 · 4393 API 路由 · 12 WebSocket 通道 · 87 守门脚本 · 56+ pre-commit 项 · 719 测试文件 / ~14839+ 测试用例 · 67 e2e spec · 40 CI workflows · 14 AI 引擎 GEO 文件</strong><br/>
...
核心数字: 8 端 / 176 模型 / 542 表 / 4393 API / 24 MCP 工具 / **38 发布平台** / **反风控五层防线 37+ 检测点** / **AI 对话可视化 inline 到消息气泡** / 87 守门脚本 / 719 测试文件 / ~14839+ 测试用例 / 5 语言 i18n(7 端 35 JSON)/ 109 万行代码
...
- **运维**:Prometheus + Grafana · 40 CI workflows · 87 守门脚本 · 蓝绿部署
...
| **代码规模** | 8 端代码 / 205 schema 文件 / 542 数据库表 / drizzle-kit push / 4393 API 路由 / 250+ Web 页面 / 16 共享包 / 87 守门脚本 + 56+10 pre-commit 项 / 5 语言 i18n parity                                                   |
...
8. **87 守门脚本 + 56+10 pre-commit 项 + drizzle-kit push 模式 + post-commit 自动 push**:从机制上减少协作事故 — 开源 AI 项目中较为少见的把工程守门做到机制级(Dify/FastGPT 仅有基础 lint)
...
| **工程守门**       | **87 守门脚本 + drizzle-kit push 模式 + 自动 push**              | -              | 基础        | 基础             | 基础       | -              | 无          | 无          | 无             | -              | -             |
...
| 26         | check-parent-pollution.mjs                                               | **项目父目录污染巡查(blocking:agent 在项目外直接创建文件)**。2026-09-23 加固:新增 `CREDENTIAL_DIR_NAMES` 对 `secrets`/`密钥`/`credentials`/`certs`/`.pybcrypt` **整目录不扫** —— 因 `--auto-clean` 对「文件名强信号命中」直接 `unlinkSync` 且无二次确认,此前会把用户凭据库 `D:/DevEnv/secrets/ihui-app-password.txt` 连 agent 垃圾一起删掉(同类陷阱第二次,前例 `ihui-release.keystore.说明.txt`)。**纪律:命中项落在凭据目录内时,先补豁免再清理,顺序不可反**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 61         | check-installer-assets.mjs                                               | 桌面安装器位图三方对账(引用 ↔ File 打包 ↔ 5 档 DPI 落盘)+ GetOptions 前缀误匹配免疫 + 三条跨文件几何/集合不变量(CTA/取消洞必须与按钮矩形逐像素等大,否则透出按钮面色刷成"方形白边";百分比控件中心必须与位图双环环心同心;Section 埋点集合必须等于轨道刻度集合),注入式回归见 scripts/tests/check-installer-assets-geo.test.mjs(基线全绿 + 4 条违规全部被咬)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **反自动化**     | IP + 用户双轨频率统计(用户维度经验签取身份)+ 扫描器模式即时封禁 + CAPTCHA 挑战可自助解除自动封禁 + 机器人 UA 检测;封禁线 600 次/分钟(原 200,因前端扇出 bug 会误伤真实用户而上调),监控探针与自助解封面不参与封禁 |
...
### 新增守门示例:第 77 项「提交内容含 Git 冲突标记」(2026-09-23)

**第 77 项 `check-no-conflict-markers.mjs`(blocking)** —— 补的是一个**已经漏过一次**的缺口。
...
8. **87 守门脚本 + drizzle-kit push 模式 + 9 PowerShell + post-commit 自动 push**(在开源 AI 项目中较为少见)
...
- 87 守门脚本 + 56+10 pre-commit 项 + post-commit 自动 push + drizzle-kit push 模式 + 9 PowerShell 启动
