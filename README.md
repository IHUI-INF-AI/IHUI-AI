...
| 77         | check-radius-single-source.mjs                                           | **全 8 端圆角单一源头对账(blocking,2026-09-23 立)**:档位表唯一真相源 `packages/design-tokens/src/radius.js`(xs2/sm4/md6/lg8/xl12/2xl16)。判据 A:radius.js ↔ tokens.css `--radius-*` ↔ miniapp-taro app.css ↔ tailwind-preset 四处逐档同值(preset 必须引用 `RADIUS_REM`,不得重新内联)——历史上 v3 preset 的 `rounded-sm`=2px 与 web v4 的 4px 同名不同值,是"手机上所有容器圆角与全局设定不一致"的根因。判据 B:端内取用必须引用档位(RN `rnRadius.lg` / CSS `var(--radius-lg)` / 类名 `rounded-lg`),数字字面量、`rpx()` 算圆角、每文件 `*_RADIUS` 常量、`rounded-[任意值]` 一律红;真圆/胶囊(头像/装饰点/半高胶囊)不得方档化,须 `size/2` 或同行 `radius-exempt: 原因` 显式声明。**棘轮锚点 = 该文件 HEAD 版本自身的违规数**(2026-09-24 换锚:并行会话的索引层重建曾把 309 个路径整文件回写成迁移前的旧基线、HEAD 积累 1179 处,而静态清单对此完全绿灯;`radius-single-source-baseline.json` 降为人工兜底且现须为空),全量审计改判 **HEAD blob 而非工作树**(滞后的旧草稿不该记成本仓债务 —— 一道与真实改动无关的红门只会逼人 `--no-verify`)。另有判据 C 覆盖面对账:含圆角的跟踪文件要么落在 SCAN_DIRS,要么在 `OUT_OF_SCOPE` 写明原因。判据不锚定行首(`width:16px; border-radius:50%` 这类同行多声明、以及 cli 分享页/设计模板/扩展 content script 里 **TS 模板字面量生成的 CSS** 同样可见,后者须用 `${RADIUS_CSS_PX.md}` 插值而非写死)。`--self-test` 45 例(含真实表端到端对账、NaN 防回归、HEAD 锚点四方向正反例),镜像测试 `node --test scripts/tests/check-radius-single-source.test.mjs`(4 例,含"锚点必须是 HEAD 而非静态清单"装车证明) |
