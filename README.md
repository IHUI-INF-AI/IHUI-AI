...
| RN(Expo)             | `app_ui` / `rn`                      | describe / read / navigate / invoke                         | 协议与工具族(`mobile_ui_*`)已就绪,端侧桥已落地:无同源 DOM 故无 click/fill/submit;`navigate` 仅放行 204 条 Screen 白名单(生成器解析 `RootNavigator` 已登录分支 + `RootStackParamList` + `linking.ts` 的 `:param`);`invoke` 只登记 4 条显式命令(主题三档 + 回首页),**退出登录/注销/支付/清缓存一律不登记**;RN 无 URL,`screen` 字段语义是"根到叶激活路径" |
