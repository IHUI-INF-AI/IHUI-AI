...
| 91         | check-theme-prop-wiring.mjs                                                  | **主题接线对账(blocking,2026-09-24 立)**:`packages/app` 213 个 theme-driven 组件形参默认 `colorScheme = 'light'`,调用方漏传或写字面量 ⇒ 整棵子树静默按该档案渲染,端内 chrome 却跟真主题走 ⇒ 同一屏幕两套档案(真机实测 #1a1a1a vs #f5f5f5)。首轮全量命中 118 处 / 117 文件,已修 108。组件清单由源码自动推导(不维护手工名单);`{...props}` 归 spread-unknown 而非 ok;元素边界花括号深度扫描。余 9 处冻结基线。镜像测试 6 例含装车证明与编号唯一。紧急跳过 `HUSKY_SKIP_THEME_PROP_WIRING=1` |
...
| 90         | check-cross-end-tokens.mjs                                               | **跨端色值同源对账(blocking,2026-09-24 装车)**:`packages/design-tokens/src/rn-tokens.ts`(RN)↔ `src/styles/tokens.css`(web/小程序)逐位对账,亮/暗各算(`.dark` 未覆盖的变量按 cascade 回退亮色参与暗色比对);比对前归一(HEX 小写、`hsl(0 0% 0%)`→`#000000`、`rgba` 去空白)。三条判据 —— R1 已声明映射同值、**R2 品牌键覆盖**(`brand` 命名空间每个键必须有映射声明或在 `RN_ONLY_BRAND_KEYS` 写明"web 无对应变量"的理由,豁免项失效同样红)、**R3 悬空引用**(`tokens.brand.<key>`/`tk.brand.<key>` 必须命中已声明键;只认 token 袋前缀,局部变量也叫 brand 的不得假红)。补 R2/R3 的起因:`brand.ctaFill`/`ctaText` 这对混血档(浅色=web primary、深色=brand-accent)存在一整天,而本门当时只核"已声明的 8 条映射"—— `8/8 in sync` 与 `1 个端内分叉档`同时为真;该档已于 2026-09-24 删除,品牌 CTA 统一 `brand.DEFAULT + brand.foreground`(≡ `--color-primary` + `--color-primary-foreground`),要调暗色对比度改 tokens.css 一处、三端同动。此前本门只挂在 `pnpm check:all`,从未进 pre-commit —— 也是它没能及时拦住分叉的原因之一。自检 `--self-test`(8 例正反对照)+ 镜像测试 `node --test scripts/tests/check-cross-end-tokens.test.mjs`(6 例,含"必须注册为 blocking 且 id 不撞号"装车证明);紧急跳过 `HUSKY_SKIP_CROSS_END_TOKENS=1`                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
