...
**本任务已归档收尾**(任务已完整闭环,所有用户决策点已落地,所有补开发已验证通过,所有代码已 push 留存)。

剩余 28 个 API 设计风格差异 + 234 个前端非真实缺失 + 预存 i18n parity 问题均不属本任务范围,如需推进请新开任务。

---

## §26 任务范围外剩余项调研结论归档(2026-07-20)
...
- D 盘历史项目核查(`D:\历史项目存档\`)
- 守门脚本源码逻辑分析(`scripts/check-i18n-*.mjs` + `scripts/scan-i18n-*.mjs`)
...
- "236 真实缺失" → 减去 2 已补开发 + 4 类非真实缺失 = **0 真实需要补开发**
...
- auth-sso 类(5 个):D 盘 5 入口 → IHUI-AI 单 SSO 端点更简洁
- 支付回调类(3 个):D 盘独立 controller → IHUI-AI payment-gateway 统一
- 文件上传类(5 个):D 盘 `uploadChunkedFile/*` → IHUI-AI 语义化命名
- 命名规范化类(~58 个):feedbacks / invoices / notifications / follows / workflows / buy / settlement / vip / review / permissions / content / reports / system-login-logs / system-operation-logs / like / visit-tracking / homework/record / agreement 等

**结论**:所有 86 个设计风格差异端点当前实现都已符合 RESTful + kebab-case 规范,功能等价或更优,**无需任何重构,无需多端同步改动,工作量评估为 0 文件**。
...
### 26.6 后续审计改进建议(可选 P3,无业务驱动)

1. `scripts/audit-migration-frontend-routes.mjs` 增加:
   - 重复路径去重逻辑(edu client 与 code/edu 同源)
   - Dialog 模式识别(列表页 + Dialog 等价于独立 edit 页)
   - 动态路由收敛识别(`[type]` 覆盖多个独立路由)

2. `scripts/audit-migration-api-routes.mjs` 增加:
   - 路径风格等价识别(RESTful query 参数 `/members?mobile=` 等价于 `/by-mobile/{phone}`)

3. `scripts/check-i18n-broken-en.mjs` 增强:
   - 单单词机翻检测(捕获 `huawei: Why` 类错误)

4. 新增 `scripts/check-i18n-broken-ko.mjs` 和 `check-i18n-broken-ja.mjs`(参考 broken-en 模式)
...
**本批调研已归档收尾**(4 项剩余项已全部关闭,审计改进建议为可选 P3 优先级,无业务驱动)。
