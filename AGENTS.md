...
- **覆盖面对账(判据 C,声明而非漏掉)**:全仓任何含圆角取用的跟踪文件只有两种合法归宿 —— ① 落在守门 `SCAN_DIRS` 内受 B 约束;② 落在 `OUT_OF_SCOPE` 并**写明为什么不适用**;落进第三类(新端、新目录、改名)即红。当前已声明范围外 43 个:静态美术资产 33(svg 的 `rx` 是图形轮廓本身)、内容渲染分支 4(ai-service 发布到第三方平台的文章 HTML,同 §4「文档/营销正文」豁免口径)、开发期报告生成器 1、docs 产物 1、档位表自身 4。`.py` 也在枚举之列 —— ai-service 的圆角就写在 Python 里,不枚举等于让这条豁免靠漏掉而非靠声明。
- **守门**:`scripts/check-radius-single-source.mjs`(guardian 第 **77** 项,blocking)三判据 —— A 档位表四处对账(改一处忘改另一处即红)、B 端内取用必须引用档位(B1 数值与字符串形态 / B2 本地常量 / B3 CSS 字面量 / B4 任意值类 / B5 SVG rx-ry;存量走 `scripts/radius-single-source-baseline.json` 棘轮只减不增)、C 覆盖面对账(见上条)。判据**不锚定行首**,故 `width:16px; border-radius:50%` 同行多声明、TS 模板里生成的 CSS 一样可见。按文件自验:`--files <a> <b>`;紧急跳过 `HUSKY_SKIP_RADIUS_GUARD=1`;自检 `--self-test`(41 例,含真实表端到端对账、静态/内联 SVG 两种语义、NaN 防回归、C 判据新端必红),镜像测试 `node --test scripts/tests/check-radius-single-source.test.mjs`。容器纯圆违规仍由 `scripts/check-rounded-full.mjs`(第 11 项)管,两条互补不互替。
...
<!-- 合并归并说明:本行下方两条登记分属两个会话同日新增的闸门(78 workspace 依赖链接对账 / 77 提交内容含冲突标记),两侧均保留,不构成互斥。 -->
