...
**守门集成**:pre-commit 第 2f-web 项(blocking),仅当 staged 涉及 `apps/web/messages/zh-CN.json` 时检测,有 pending 则阻塞 commit 直到翻译完成。多 agent 并行时其他 agent 改 target locale 文件不会触发阻塞(避免误伤)。
