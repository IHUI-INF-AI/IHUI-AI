...
- **主题接线对账**(91):check-theme-prop-wiring.mjs(blocking,2026-09-24 立)—— `packages/app` 里 213 个组件形参默认 `colorScheme = 'light'` 并据此 `getTokens(colorScheme)`, 调用方**漏传或写字面量**,整棵子树就静默按该档案渲染,而端内 NavBar/TabBar 跟着真主题走 ⇒ 同一屏幕两套档案 (真机实测:广场页顶栏底栏 #1a1a1a、正文 #f5f5f5)。首轮全量命中 **118 处 / 117 文件**,已修 108 处。 组件清单**由源码自动推导、不维护手工名单**(手工名单正是上一版判据看不见东西的原因);两类红 = 漏传 / 写死字面量,`{...props}` 归 `spread-unknown`(判不出就如实说判不出,不静默放行成 ok);元素边界用花括号深度扫描,不被 `onPress={() => a > b}` 里的尖括号截断。**教训:用"元素文本里有没有 colorScheme 字样"统计会得出"0 处漏传"的假结论** —— 它既看不见 `{...props}` 展开转发,也没意识到 wrapper 的 props 里根本没有这个键。 余 9 处冻结在 `scripts/theme-prop-wiring-baseline.json`(棘轮只减不增):7 个屏系他人 M 在制不代收,2 处在 study-publish —— 该文件 14 处写死 `getTokens('light')`、其中 8 处在模块级 `StyleSheet.create` 里(结构上不可能跟随主题),属整文件主题化改造,不半修。 取证:`--self-test` 全通过 + `node --test scripts/tests/check-theme-prop-wiring.test.mjs` 6 例(含**装车证明**:runner 必须真有 id 91 + blocking + skipEnv;及编号唯一断言,防同日撞号)。紧急跳过 `HUSKY_SKIP_THEME_PROP_WIRING=1`
...
- **它的删除面是"按名字"的,不是"按目录整片"**:盘根只认 `IHUI-*` / `.empty-tmp*` / `.pnpm-store`;`C:\tmp` 与 `C:\temp` 内只删 `ihui-*` / `IHUI-*` / `next-backup-*` / `probe-*` / `wb-ext-debug.log`;另有 Chrome 缓存与「Temp 中 mtime>3 天的目录」两段。**注册前当天 `-DryRun` 全量命中仅 1 项**(`C:\Windows\Temp\Installer*.tmp`),合计释放 0 MB。(本条曾被排版批处理把 `ihui-*` 吃成 `ihui-_`、并把 `C:\tmp` 与 `C:\temp` 两个 code span 粘成一个 —— 文档里的通配符被吞掉就等于把判据说错了,发现即修。)
- **仍未闭环的一条**:TEMP 漂移 —— HKCU `TEMP` 已指 `D:\DevEnv\Temp`,但活着的宿主/终端进程仍持 `C:\Users\Administrator\AppData\Local\Temp`(实测 `node -p os.tmpdir()` 即旧值),所以走 `os.tmpdir()` 的脚本会继续落 C 盘;新开终端/重启宿主后自愈。判据与污染可见性由守门 **92** `check-c-drive-pollution.mjs`(只读、永不删)承担。
