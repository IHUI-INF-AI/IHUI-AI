...
- **任务状态一致性(2026-09-26 立,守门 130 配套)**:计划文档被当台账用而**没有主键约束**,这是"完成的为什么还在这里、还有重复"的唯一根因 —— §1 上一条让每个 agent 追加登记,同一任务编号被不同批次各写一份,做完只翻自己那一批那份,其余副本永久烂成"未勾选";而既有防护链(门 71 防丢行 / 84 防写回旧版 / 100 防合并吞文件 / 工作区自愈)**全部单向防丢,没有一道判"状态分叉"**,并集合并又是"每行重数取 max",所以改写必然产出两份。三条规矩:
  1. **派单口径只认一个入口**:`node scripts/plan-tasks.mjs --open`(未勾选 − 同题已完成副本 − 自带作废声明的行 − 同一件事的第二条待办副本;四条判据 F1/F2/F3/F4 都以"编号 + 标题前缀逐字等值"的复合主键为准,不依赖行号顺序也不依赖相似度)。`grep '^- \[ \]'` 不是待办清单(2026-09-26 一次实测的**不重叠**分解:未勾选 205 = 已认领 44 + 其余排除 28 + 真待办 133;同刻 F1/F2/F3/F4 读数均为 0 —— **这些数是当次现读,派单前必须重跑,勿照抄**;报告里其余排除项的明细计数彼此可重叠,只能当诊断看,不能拿来减法核账)。归并出口:`node scripts/plan-tasks-merge.mjs --heal --commit`(F1/F2 翻勾并写明归并到谁;**F4 只加"重复登记副本"指针、绝不动勾选** —— 两条都是没做完的事,把没做的记成做过的比原病更响)。
  2. **一个编号只能有一行"当前状态"**:新登记同一件事时必须把旧行翻勾或就地改写,不得在别处再抄一份。判据是复合主键(编号 + 标题前缀**逐字**等值),不是相似度 —— 相似度只能报数,不配判红。
  3. **证据指针禁止写行号**:行号在任何一次 append 后都会挪位。实测 HEAD 面 27 处 `存活于 L<行号>` 式指针**复核通过率 0/27**(目标行已不是条目行)——上一轮登记"已识别、只标注未翻勾"的那批账,今天已无法按它自己给的证据复核。必须用内容锚点(编号 + 标题原文)。
  - 问责:`pnpm check:plan-task-state`(`--strict`,存量判红);提交链上跑的是**差值棘轮**(只拦"这次提交让分叉变多",存量永不拦 —— 与改动无关的恒红门只会逼人 `--no-verify`,连带废掉全部守门,§12e 同型)。修法只有**归并**(副本翻勾并注记归并到谁),§1 的"禁止无声删除"对已完成行仍然成立。
...
- **自动归档**:`scripts/archive-completed-tasks.mjs` 扫描完成 ≥7 天的条目,post-commit 钩子自动 `--auto-commit`。归档 commit 设 `IHUI_ARCHIVE_COMMIT=1` 防递归。
  ...
  - **归档锚点必须受版本控制**(2026-09-25 G-183):`.gitignore` 对 `.ihui-agent/` 的处理是"忽略其内容 + `!.ihui-agent/archive/` 例外",**不得改回整目录忽略** —— §1 的"完整内容在 `.ihui-agent/archive/PROJECT_PLAN_*.md`"和守门 71 的"archive 里找得到 ⇒ 不算丢"都以它为事实基础。旧状态(整目录忽略)下:历史那次 `b599edbba` 归档 11 条的 commit **只含 PROJECT_PLAN.md 一个路径**,即副本从未入库,而"A 机归档过"可以授权"B 机删行"。守门 71 曾仍读磁盘副本 ⇒ 尚未入库的本机归档文件可以授权另一次提交删行,**该洞已于 2026-09-25 由 G-184 关闭**:`archivedCopy()` 的归档清单与内容现在**同面**取(`--staged` 走 `git ls-files` 索引、全量走 `git ls-tree HEAD`),面里没有的路径不构成删行凭据;`runCheck` 必须把当次判定面喂给豁免(`archiveExemptFor(isStaged)`)。取证:自测四条组合 + 镜像端到端 **A/B**(同一份归档正文、同样删掉 `## O42`,唯一差别是一次 `git add`:未入库侧必 exit 1、已入库侧必 exit 0),外加一条反向回归锁(函数体内不得再出现 `readdirSync(` / `readFileSync(`,对 HEAD 旧版该锁 5 条断言全红)。**G-183 当时登记的"被门 118 结构性棘轮挡住"这个前提实测已不成立** —— 并行会话已把门 71 的内容面迁到 `scripts/lib/face-reader.mjs`,门 118 见 face-reader 导入即判合规,所以这里不存在"必须先整道迁移"的阻塞;登记这条是为了说明**沿用旧理由会挡住一件已经可以直接做的事**。
  ...
  - **归档占位点名的文件必须真在审面里(守门 13c 的 A1/A2/A3,2026-09-25 立)**:`<!-- 已归档(…):X,完整内容在 .ihui-agent/archive/<FILE> -->` 是一句**可核验的指针**,不是修辞。A1 = 盘上有 `PROJECT_PLAN_*.md` 而审面(全量 HEAD 树 / `--staged` 索引)没有 ⇒ 红;A2 = 占位点名的具体文件不在审面里 ⇒ 红(写成 `PROJECT_PLAN_*.md` 通配的占位不点名,不参与判定)。**A3(同日 G-192)= 同一把尺子伸进归档件内部** —— 元归档层(归档文件正文里再写"完整内容在 archive/<另一个>")过去完全无人看守,实测 `2026-09-12` 那份里嵌着 **218 条**占位、点名 13 个文件,其中 12 个从未入库。A3 与 A2 共用"缺失⇒红 / 已登记⇒只报数 / 已入库仍挂台账⇒判清单腐烂"三段规矩,标签不串门,同一名字被 A2 与 A3 点到只计一次债;blob 取不到那一层 ⇒ 结论行喊"未判定",不静默通过。立项即抓到两条实测:`PROJECT_PLAN_dedup-2026-09-23.md` 只躺在本机(已随本票入库),以及 **8 个被 52 条占位点名的归档文件在仓库与本机两侧都不存在、其路径也从未出现在历史里** —— 即 §1 那句承诺对这批条目一直是空的。这 8 项进 `LOST_ANCHOR_LEDGER` **只报数**:它们是已发生的债,当场判红就是一台与任何提交都无关的恒红门(§12e 同型);台账是**待偿清单不是豁免清单** —— 修好了仍挂着 ⇒ 判"清单腐烂"红,删了行却没修好 ⇒ 照常红,两个方向各有一条自检钉死。按条目从计划文档历史版本逐字找回另计一票。
  - **台账已第一次真清偿(2026-09-25 G-191,8 项减到 4 项)**:找回的办法是**从 git 历史逐字取回,不是重写历史**。每批归档都由"把正文从计划文档搬走"的那枚提交产生,所以那枚提交**删掉的内容块**就是归档正文;落地判据只有一条 —— 该正文逐字存在于所引提交的父版本 `PROJECT_PLAN.md`,不成立就不落地(实测有 2 条 `2026-07-26` 的占位是在 `3a5b737bf8` 里**凭空新增**的纯 `+` 行,按裸标题 `git log -S` 证明那两个条目从未存在过 ⇒ 从别的提交"补"出来就是编造,已确认为找不回,不当干)。落地 4 份 / **28 块 / 567 行实质正文、0 行无出处**,取证用两把**互相独立**的尺子(逐块对应 23/23 + 格式无关的全量行级比对 567/567 —— 后者专门防"重建脚本自己给自己发合格证")。每份文件里保留逐条 `recovered from <sha>` 注释 + 一句"这是找回件,不是当时原写"的性质说明:**不写进文件,后来人就会把重建文本当一手记录。**
    **取出处时必绕过一个坑**:`b129e09482` 做过一次 `.trae-cn/archive/` → `.ihui-agent/archive/` 的**占位文本搬家**(它不删除任何内容),按"最旧一次 `-S` 命中"取出处会选中搬家那枚而非真正的删除,必须沿搬家链回走。
  - **递归那一格已做完(2026-09-25 G-192),并把看守面扩到归档层自身**:上面"218 条嵌在元归档里的占位"共点名 **13 个**文件,三类处置 —— ① **4 个真存在过**(躺在 `.trae-cn/archive/` 下,搬家时漏带),按 `git cat-file blob <出处>^:.trae-cn/archive/<名>` **原字节**落盘,判据两把:注入水印**之前** `git hash-object` 与出处 blob **同 SHA**,注入**之后**正文仍须逐行全等(合计 2,210 行 / 170,608 B);② **8 个从未作为文件存在过**(`git log --all -- <任意前缀>/<名>` 对 8 个名字全部 0 命中)⇒ 占位被写过、文件没被写过,结构上找不回,进台账只报数;③ 1 个由 G-191 已入库。**为什么必须两把尺子**:找回脚本自己写的是"剥水印→逐行全等",它剥窄或剥宽都会让自己的断言跟着错(§22c"镜像测试只复读实现就是复读机"同型),所以独立复核换成异形判据 —— "blob 每一行必须在落地文件里**同序、连续、逐字**出现",水印层天然落在窗口之外。过程中真抓到一次不等:出处带 **UTF-8 BOM** 而 `watermark.mjs inject` 重写时会去掉 ⇒ 容差只开在 BOM 这一处(字节同值已由注入前的 SHA 判据证明),并写进文件里的出处注释,不让它读成"逐字节永远同值"。**A3 是本格的真正产出**:归档件**内部**的占位同样点名,A1/A2 过去只扫计划文档,这一层零看守。台账 4→12 项(**全部只报数**,所以扩 A3 不产生新恒红面)。
  - **13c 自身的取材口径(同日迁移)**:此前 `ROOT = process.cwd()` + 裸 `git show` 用 `execSync` 拼字符串(无 timeout)+ 占位合规性看 `git diff` 文本,三处都与本仓现行口径相反(工作树滞后即换结论 / 服务账户 PATH 不通 / 别人同时改这文件时合规性跟着盘走)。现全部经 `scripts/lib/face-reader.mjs`,对照面是 `--staged`:HEAD→索引、全量:HEAD^→HEAD、`--worktree` 仅人工;整面取不到 ⇒ **exit 2 无法判定**,不冒红也不记绿。归档件正文也**同面**取(A3 用),单份 blob 取不到 ⇒ 在结论行喊"那一层未判定",不静默通过。另有两条是本门第一次自跑咬出来的:CRLF/LF 的同一条标题不得被读成"删除"(HEAD blob 是 LF、共享工作树常是 CRLF,不剥 `\r` 会把整批已完成条目误报成被删);**点名对象的归属必须拿该目录在审面上的"全集"判**,而不是只看 `PROJECT_PLAN_*` 形状 —— 归档目录里也放非该形状的跟踪件,按形状筛就会把"已入库"读成"落空"。
  - **本门自己修掉的两个"拦了却不说话"型缺陷(2026-09-25 G-192,都值得记住形态)**:① A0 阻塞分支写成 `if (deletedHeadings.length > 0 && !del.addedPlaceholders)`,而**空数组是真值** ⇒ 该分支永不执行,门对"直接删已完成条目"`exit 1` 且**零输出** —— 调用方只看到提交被阻止,看不到原因(现走 `del.compliant`,由 length 决定)。② 门按自身位置推导 `ROOT` 之后,镜像测试靠 `cwd` 定位夹具的那套调用**结构上失效了**:13 例里 11 例其实在审真仓,账面全绿而结论与夹具无关(守门 70 同型)。夹具改经显式 `--root <dir>` 通道进入,落点改回 `scripts/lib/scratch-dir.mjs`(§26,不再往 `os.tmpdir()` 写),并由两条**源码级反向锁**钉住(不得再出现"只靠 cwd"的调用形态、不得回到裸 `mkdtempSync`)—— 这类失效只有源码锁能防,加断言会跟着一起漂绿。取证现为 `--self-test` **17** 条 + 镜像 **20** 例(含 A1/A2/A3 端到端与"点名已入库的对象不该红"的反向对照,后者防的是把整层一律判红那种恒红门)。**本门没有应急跳过通道**(runner 条目无 `skipEnv`、脚本不读 env);登记这一事实,是因为本仓多次出现"文档写了一个跑不通的出路"。
...
- **唯一写法(2026-09-26 回翻定稿)**:主按钮、选中胶囊、悬浮加号这类"品牌实底 + 其上文字",RN/共享包写 `brand.cta`(底)+ `brand.ctaForeground`(文字)**成对**;CSS/类名侧用 `--color-cta` + `--color-cta-foreground`(小程序 `CategoryBar`/FAB 同值)。**这一档黑/白反转**:亮=纯黑底(`#000000`)/ 纯白字(`#FFFFFF`),暗=纯白底(`#FFFFFF`)/ 纯黑字(`#000000`),与 `--color-primary` 同一明暗行为,`.dark` 显式覆盖。对账由「跨端色值同源对账」守门负责(编号以 `scripts/guardian-runner.mjs` 现值为准,勿照抄文档)。
- **为什么保留独立档名而不并进 `--color-primary`**:primary 在 web 端兼任墨色(`text-primary` 1803 处),改它等于给全站正文染色;CTA 保持独立档名、取值与同主题 primary 全等,语义 = "品牌实底 + 其上文字"。(2026-09-24 曾取 `#4A7A96` 明暗同值以避开"大色块与页面反极",2026-09-26 用户明确要求回翻为黑/白反转,该设计已废。)
...
- **禁止端内自立 CTA 档**:`brand.ctaFill` / `ctaText` 这类"浅色等于 web、深色另取一档"的**混血**键不得再加回 —— 当年删它不是因为"不许有 CTA 档",而是因为**它在端里自立、且两档取值互不相同**(那才是"手机上改了 web 没改"的成因)。`--color-cta` 走的是相反路径:先落 `tokens.css` 源头(2026-09-26 起 `.dark` 显式覆盖、黑/白反转)、再进同源对账门的映射表,所以它合法而旧键仍属禁止。要调主按钮观感就改 `tokens.css` 的 `--color-cta`(亮档)与 `.dark --color-cta`(暗档)**同一处**,三端一起动;不得在某一端单独覆盖。
- **成对即合规,不得再当债务**:主 CTA 的唯一写法是 `brand.cta` 实底 + `brand.ctaForeground` 文字(旧档 `brand.DEFAULT` + `brand.foreground` 已废,见上),所以这套配对**必须**被所有前景/容器对账判据认作合法 —— 按规矩写就红的门只有一个结局:逼人 `--no-verify`,连带废掉全部守门。拦的是**没有配对前景**的实底卡片与**跨档错配**(底取 `brand.cta`、字却取 `text.primary`/`surface.light` —— 前者黑/白反转下两态与底同色纯隐形,后者按档位法则本就非配对前景)。守门 83 的 R1_BG 已同时认 `brand.DEFAULT|brand.cta`,就是为了"判据必须覆盖门自己产出的形态"。
...
- **覆盖面(同日两次补,现行口径以这条为准)**:R8 判**三种书写形态**——RN style 对象(`border\w*:` 取墨档)、Tailwind 类名(`border-primary`/`ring-primary`)、**CSS 声明**(`border*: … var(--color-primary)`,含 TSX 内联字符串形态)。文件面**不再维护端名单**:`R8_DIRS = ['apps','packages']` 整面取,只靠 `e2e/tests/__tests__` 与 `*.test/spec.*` 排除叙述行;扩展名含 `tsx|jsx|ts|js|css|scss`。立项时前两种形态各自漏过一块(extension 不在 R5 端表里 → 类名形态在扩展端失明;.css 不在扩展名里 → **19 处** CSS 声明整面隐身),两处都已随同笔提交清零,基线 `inkBorderCounts` 仍为**空 = 零容忍**。**给守门扩面必须同批改两半**:内容判据的 scope 函数 + 枚举/预筛的目录表与扩展名;本条的第一版就只改了 scope,`git ls-files` 仍用旧表,结果**整块改动静默失效而门照报绿**。防线两条:① 拿真仓 HEAD 喂新判据,它必须点名已知存量(阳性对照);② 镜像测试锁源码——枚举面必须引用新表、旧表不得再出现、预筛常量必须含新形态。
...
- **其余三端的同一型(2026-09-25 收口;现值一律按当次实测取,勿照本行数字派单)**:`packages/app/src/features/**` + `apps/mobile-rn/src/screens/**` + `apps/extension` 的页头返回已全部收进 RN 侧唯一实现 **`packages/app/src/components/BackChevron.tsx`**(经 `@ihui/rn-app` 导出;lucide-react-native `ChevronLeft`,命中块 36×36 对齐 web `TopBarBackButton`)。**`colorScheme` 是必填 prop 而非 `= 'light'` 默认值** —— 守门 91 只把"带默认值的形参"认作静默脱主题开关,必填改由 `tsc` 强制,比门更严。HEAD 面实测 `<Text>{t('common.back')}</Text>` 仍有 42 处、`{'‹'}` 0 处:那 42 处是**错误态/空态卡片按钮、翻页、弹窗关闭** —— 那里「返回」是文案不是箭头,换裸箭头反而不表意,逐条带 `back-label-exempt: <原因> until YYYY-MM-DD`(该族已登记进守门 108 的 `FAMILY_LIFETIME_DAYS`,取 365 天:它是结构性定性不是待偿债务,短周期到期只会逼人删标记、删了又被 GA4 判红,两道门互咬)。**这两处"已知残余"已于同日由后续票收掉(2026-09-26 按 HEAD 复验;照旧执行会去找不存在的洞)**:① S0 现登记 5 条,含 RN 共享层那份 `packages/app/src/components/BackChevron.tsx`(该门注释原文即"只登记端内那一份时,共享层被摘线无人喊红");② 混合形态 `← 返回` 由新增判据 **GA5** 纳管,两处真站点的落地形态是 `ModelConfigDialog.tsx` **保留字形 + 带原因 `back-label-exempt`**(实测 `←`=1 / 豁免=1 / `ChevronLeft`=0 —— 正确处置是带理由豁免而非矢量化:该组件作用域内既无 `t` 也无 `tokens`,换矢量要改别人的组件签名)与 `Selecter.tsx` 删字形留文字 + 豁免注明"i18n 与矢量通道双缺"。

<!-- ⚠️ 本行是上面那条的**改写前原文**(2026-09-25 由提交 259fb1262d9 收口后已失效:其中"packages/app 223 处/168 文件""mobile-rn 5 处""共享层两个 NavBar 与 PayResultScreen 用 ‹"四项现值全变),照它派单会把已做完的 180 处重做一遍。保留只为不丢行(§12),勿照它执行。 -->
...
- **单一真相源**:design-tokens 在 `packages/design-tokens/src/styles/tokens.css`(`@theme` + `:root` + `.dark`)。miniapp-taro 的 `app.css :root/.dark` 由 `scripts/sync-miniapp-tokens.mjs` 自动同步(**禁止手改 app.css 的 token 块**;改 token 改源头 + 跑同步。2026-09-25 该生成器由端内 `apps/miniapp-taro/scripts/sync-design-tokens.mjs` **搬到根 `scripts/`**(旧路径已删,`pnpm --filter @ihui/miniapp-taro sync-tokens` 入口不变)—— 搬的理由是架构门 103 逼出来的:守门 36 与生成器必须共用**一份**取源实现,而取源住在 `scripts/lib/design-token-blocks.mjs`;端内脚本向上摸工具层是 D2 反向依赖(且 tooling 声明 `exported:false`),把实现下沉进 contract 包又让根工具层按路径穿透包实现细节(D3),而工具层**按包名解析不到 workspace 链接**(实测给根 package.json 加 `@ihui/design-tokens` 后 pnpm 写出的根链接是悬空的,门 78 当场判红,已回退)。摆在工具层 ⇒ 四个端的同名机制全在一处,零穿透零豁免。旧文档那句"根 `scripts/` 下没有同名文件"就此作废,照它跑 `node scripts/sync-design-tokens.mjs` 现在仍会报模块找不到 —— 新名字是 `sync-miniapp-tokens.mjs`)。另有 `scripts/check-miniapp-taro-design-tokens.mjs` 与 `scripts/check-miniapp-tokens-sync.mjs`(后者为 guardian-runner 第 36 项实际调用项)校验同步一致性。
...
- **端内 CSS/色值副本一律是派生态,禁止手改、也禁止"只加拦红不写回"**(2026-09-25 立,由实测逼出):`packages/design-tokens/src/styles/tokens.css` 是唯一色值源;所有 v3 端副本(`apps/miniapp-taro/src/app.css` 的受管块、`apps/mobile-rn/global.css`、`packages/design-tokens/src/rn-tokens.ts`、`tailwind-alpha-plugin.js` 的 `ALPHA_USAGE`)必须由 `scripts/lib/design-token-blocks.mjs`(取 tokens.css 的**唯一实现**)+ 各自的 `scripts/sync-*.mjs` 派生,并挂在 `scripts/lib/pre-commit-hook.js` 的 `TOKEN_SYNC_TARGETS` 表上按**各自触发面**自动写回(token 类看 tokens.css,用量类看 v3 三端源码)。两条不可违反的写法:**① 原位写回,禁止整块替换** —— 副本里非受管的端内档(如 `global.css` 的 `.dark` 那批 `--rn-*`)与解释性注释必须逐字留在原位,值已等价时连字节都不改(这才是幂等);实测旧写法整块替换一次就抹掉 13 行在用深色档 + 6 段注释,那正是 RN 侧自动同步长期只拦红不回写的真实原因(接上必炸)。**② 取源只能有一份实现** —— 生成器与守门各写一遍取值逻辑必然不同形(实测:生成器只取 `@theme` 首个非贪婪块,漏掉后续 `:root` 块里的 `--color-*-rgb` 三元组,一次"同步"就删掉守门 93 R6 要求必备的 alpha 通道)。**新增跨端档位的正确顺序**:改 tokens.css 一处 → 跑对应 sync 脚本(或让提交链自动跑)→ 由门复核;不得在任何端手写色值、色名常量或第二张登记表。
- **小程序端有第 6 个受管区 = `app.css` 的 `@theme` 色档注册区**(2026-09-25 立,`f17b5fbdda`):它不是"又一份色值表",而是 Tailwind v4 的**注册动作** —— v4 只把 `@theme` 里的 `--color-*` 认作主题档,色值若只落在普通 `:root/.dark` 上,`bg-muted`/`text-foreground` 这一整族 utility **一条都不产出**(实测真构建 0 落地)。档位仍**唯一派生自** `deriveMiniappManaged(tokens.css, 'light')` 再剔掉 `-rgb` 三元组 —— `-rgb` 进 `@theme` 会长出 `.bg-muted-rgb` 这类脏档(v4 见 `--color-*` 就当颜色档)。三条不得照抄的形态选择:**只用普通 `@theme { }`**;`@theme inline` 会把亮档值烘进每条规则 ⇒ 深色档整族失效;`@theme reference` 会给每条规则附一份亮档 fallback(更贵,且把"没定义"洗成"默默用亮档")。守门 36 现同时核 `:root` / `.dark` / `@theme` 三面(无缺档 + 逐位同值 + 反向拦 `-rgb` 脏档)。**同批判死一条判据缺陷**:`check-miniapp-css-landing` 的 C1 原拿源类名直比产物类名,而 weapp-tailwindcss 会把类名标点转写(`[→_b ]→_B /→_f :→_c (→_p )→_P ,→_m .→_d +→_u`),于是 505 条"缺项"里 474 条是比对噪声;现按同表**正向**转写后再比,且分"原名直中/转写后中"两态计数(合计会把"表漏一条"藏起来)。**判落地的门如果比错形态,症状不是差一点,而是把真信号淹在噪声里** —— 上一轮正是靠"缺项恰好成族"才定位到色档整族没落地。
...
- ⚠️ **上面那条 `/alpha` 判据目前到不了小程序产物**(2026-09-25 实测,登记于 PROJECT_PLAN O62附②/附③):源码用到的 **687 个 Tailwind utility / 11,105 处用法**,在真实构建(`pnpm --filter @ihui/miniapp-taro build`,exit 0)的 154 个 wxss 里 **0 个产出规则**(`.flex{` `.items-center{` `.rounded-xl{` `.bg-muted{` `.text-sm{` 逐条为 0);而 **base/preflight 层是在的**(`border:0 solid;box-sizing:border-box;margin:0;padding:0`)⇒ 症状精确到"**只有 utilities 层为空**"。阳性对照:同一份 dist 里 `app.css` 手写类名 75/98 可查到规则。**机制不是** `config/index.ts` 那行 `tailwindcss: { enable: true, config: {} }` —— 实测把它改成真实配置路径后 A/B/C 三次构建产物**逐字节相同**(该键不是开关);真因指向 weapp-tailwindcss 5.2.9 **自带一份 vendored tailwind**(`dist/tailwindcss-*.js`)并由它自己的 generator 出 CSS,项目 `tailwind.config.ts` 的 content globs 没有喂进那条链。**后果**:本条 R6 判据守的是"源码用量 ↔ 登记表 ↔ 插件产出"三者一致,这三者都在**项目侧**;小程序实际产物由第三方 generator 决定,R6 的绿灯**不等于**到端生效。改这行配置的爆炸半径见 O62附③(主包余量 42,401 B 而 utilities 约 42,392 B,以及 `text-card` 同名双义会被注入白底白字)。**⚠️ 本条"0 个产出规则"的前提已于同日作废**(2026-09-25 深夜,`edb225c909` + `f17b5fbdda`):真产物落地率现测 **740/771 = 96.0%**,命名档与项目色档都已产出,主包余量 **113,504 B**;当年"整端一起变"的半径已被拆成两步分别验证。现行事实与判据见本节末"v4 色档注册区"条与 PROJECT_PLAN O62 附⑧/附⑨,勿照本行执行。
- ⚠️ **v3 端禁止裸 rpx 长度写进 `text-` / `border-` 任意值**(2026-09-25 立,守门 93 新增 **R7** 是它唯一的判据):`text-[28rpx]` / `border-[2rpx]` 会被 Tailwind v3 解析成**颜色属性**(`color: 28rpx` / `border-color: 2rpx`),整条声明无效且不报错。决定对错的是**单位不是前缀** —— `px`/`rem`/`em` 在 v3 下本就正确落 `font-size`/`border-width`,**不得**给它们加 `length:`(纯噪音);只有 rpx 这一族坏,因为 v3 的类型推断不认 rpx。正确写法:`text-[length:28rpx]`。颜色形态 `text-[var(--color-x)]` / `text-[color:…]` 不受本条约束。存量 1,331 处已清零,故 R7 在 HEAD 上是零容忍。
- ⚠️ **但 `apps/miniapp-taro` 实际跑的是 Tailwind v4.3.3,不是端内 `package.json` 声明的 v3**(2026-09-25 实测,详见 PROJECT_PLAN O62附⑤):`weapp-tailwindcss@5.2.9` 引用的是 **`@tailwindcss/postcss`**(v4 专属入口 —— v4 的 main 导出会**直接报错拒绝**被当 postcss 插件用),产物 base 层也带 v4 指纹(`border:0 solid` 合并写法、`--tw-gradient-position`,且无 v3 独有的 `--tw-bg-opacity`)。**取版本别只读 `package.json`** —— 那样会得到相反结论(本会话就错过一次:用 `resolve('tailwindcss/package.json')` 探到 `MODULE_NOT_FOUND` 就否掉了正确说法,真实依赖名是 `@tailwindcss/postcss`)。两条后果:① 上面 R7 拦的裸 rpx 错属性在 **v3 与 v4 上行为一致**(已用 v4 + 真实源码逐字复测),所以判据与那 1,341 处改写**不因版本换人而作废**;② 但"miniapp 的 `/alpha` 靠 `tailwind-alpha-plugin`"这个前提**尚未证实** —— v4 走 `color-mix` 是另一条路,插件的确定价值在 **mobile-rn(NativeWind v3)**。带真 theme 复测前,**不要把 R6 对 miniapp 那一侧的登记要求当成已证事实扩严**。
- ⚠️ **上面两条 `/alpha` 相关规则的实际适用范围已收窄,读的时候带上这条**(2026-09-25 实测,详见 PROJECT_PLAN O62附⑥):守门 R6 的"必须登记进 `ALPHA_USAGE`"只对**真 v3 消费端**(`apps/mobile-rn/src` + `packages/app/src`)生效 —— `apps/miniapp-taro` 跑的是 v4(见上一条),而 **v4 对 `/alpha` 是原生的**(带真 `@theme` 复测 7/7 命中,含项目自定义色与 `[0.12]` 这类任意值,声明体 `color-mix(in srgb, …)`)。收窄后量到的结论要如实说:**`tailwind-alpha-plugin` 至今零真实使用者**(全仓 64 处 `/alpha` 用量全在 miniapp,登记表已按用量归零为 `{}`),连 `--color-*-rgb` 三元组变量也**只有注释在引用、产物 0 处使用**。**别把它当已交付能力引用** —— 它是等 mobile-rn 真写 `/alpha` 时才生效的机制。要扩面先跑 `node scripts/sync-alpha-usage.mjs`(表由它导出,**不要手改**);R6 报"未登记"时的修复出口也是这条命令,不是"去 ALPHA_USAGE 加一行"。
...
- **钩子失败的归因是量出来的,不是抄来的(2026-09-25 立,`scripts/lib/commit-gate-attribution.mjs`)**:旧版首次 commit 一失败就打印「按用户规则"hook 失败因其他 agent 代码 → --no-verify 重试"」并整批跳门 —— 它**从未计算过归因**。实测一轮 134 道门 111 通过 / 1 失败,红的是 `check-push-sync`(判远端态、与提交内容无关),措辞仍是"他人代码"。现流程:解析 runner 的失败门清单 → **逐道复跑** → 该门这次的输出**点名本次声明的文件 ⇒ 判"本任务自己的红",拒绝 `--no-verify` 并 exit 1**(必须修);一个都没点名 ⇒ 允许跳,但只说量到的话;批没跑完 / 解析不到 / 复跑全不可用 ⇒ 明写「未归因」,按应急路径落地并留痕于 `.workbuddy/safe-commit-attestation.jsonl`。**"批一道都没跑"从 2026-09-26 起不再是中性事实,而是一个必须先补证据的分支**:`scripts/lib/pre-commit-hook.js` 里 lint-staged 跑在守门批**之前**,于是一次 lint/prettier 失败 = 156 道门全不跑 = 归因永远 `unattributed` = 照样跳门,而账面读起来像"跑过了、只是与本次无关"(实测 `acaf8407cc` 与 `d379215126` 前后各一次;这正是"判据失效的表现永远是安静"那一型,守门 70/76/81 同族)。现由 `safe-commit` 在判到 `unattributed ∨ ranFullBatch===false` 时**自己把 `guardian-runner --staged` 跑一遍**,再把那一轮的输出喂回**同一条铰链**(`classifyHookFailure`)分流:点名本次文件 ⇒ `mine` ⇒ 拒绝跳门;一个都没点名 ⇒ 可跳但措辞必须写明"取证来自 safe-commit 自跑的那一轮,不是钩子内那一轮";自跑也没成功 ⇒ 照旧落地,而 `unattributed` 的措辞必须带上具体原因(runner 不在位 / 派生失败码 / 超时)。**刻意不把 `unattributed` 改成拒绝跳门** —— 那会让"别人把我挡在 lint 外"的应急路径消失,逼人手工 `git commit` 跳得更彻底且不留痕。**一条如实登记的覆盖面上限**:自跑发生在首次 commit 失败之后,而 lint-staged 失败时会回滚它自己动过的暂存区 ⇒ 新增文件此刻已退回未跟踪,自跑那一轮看到的暂存集可能少于声明集(与既有"逐道复跑"同一刻同一索引态,两份证据因此可互相比较)。应急提速:`IHUI_SAFE_COMMIT_BATCH_TIMEOUT_MS`(极小值 ⇒ 自跑判"未成功"并带原因入库,**不是**静默开关)。取证:镜像 13 例(含"汇总块在位时绝不自跑"的反向锁与装车证明)。**红在守门批「之外」也要点名,不得把已量到的批内结论丢掉(2026-09-25 补,G-185)**:钩子在 runner **之后**还跑若干独立 blocking 步(实测 `i18n 死 key 扫描`),它一红就 `exit 1` —— 那一轮 154 道门**全部跑完、blocking 失败 0**,而旧归因把这种组合写成「汇总里没列出失败门 ⇒ 归因未计算」,随后照样 `--no-verify` 落地(提交 `46a4c18ed` 即此型),等于**把已经拿到的守门结论整块丢弃**,账面读起来像"没跑守门"。现规则:解析到批外步骤名(`outsideBatchStep`)后仍按同一条铰链分流 —— 该步结论点名本次声明的文件 ⇒ 判 `mine`(拒绝跳门,必须修);一个都没点名 ⇒ 判 `not-ours` 但措辞必须是「批内 0 失败,红在批外这一步」并点名是哪一步、提醒随后清偿。**失效方向仍是"多要一次定向说明"**,不是"多放一次跳门",也不是"干脆不落地"。取证:C0/C0b/C1/C2/C3 五例(含"批内确有失败门时不得走这一支"的反向对照,否则批外步骤会替批内那道门洗责)。**刻意不建"机器态门 id 清单"** —— 豁免清单必然腐烂(见 §4 对 `RN_ONLY_BRAND_KEYS` 的教训)。 —— 豁免清单必然腐烂(见 §4 对 `RN_ONLY_BRAND_KEYS` 的教训)。取证:镜像测试 5/5(含装车证明 + 真实 `hook-logs/pre-commit.log` 取样校验夹具)+ 三条变异各自变红(ANSI 剥色那条登记为**等价变异**,不造假具硬凑红)。**因此:agent 不得再假设"钩子红了就能跳"** —— 被拒时那句红点名了哪个文件,就去修哪个文件;确属误判才可按 §16 显式说明后手工提交。
...
- **parity 绿不等于内容语种对(2026-09-26 立,由 D20 实测逼出;同日已配尺子 = 守门 133)**:上面五门判的是**键集**与"**有没有**中文残留",所以"某一块整体落成另一语言"这一型结构上看不见。实测抓到三种形态,当时全部门禁都是绿的:① `aiChat.org` 的 **ja 装了繁体、zh-TW 装了谚文**;② `chat.exportMenu.exportPdf` 是 **ja←zh-TW←ko←ja 的三向轮转**(判据必须按**码位**量,按字形肉眼判会把谚文 `을 익스포트` 与假名 `をエクスポート` 看成同一种东西);③ `aiChat.toast.orgSaved` 被机械写成 `orgSaved:{orgSaved:"…"}` **自套一层**,`t('toast.orgSaved')` 取到对象 ⇒ 功能成功但提示永远不响。**批量落 i18n 后必须做两件事**:逐语种量"这块内容是不是这门语言",以及全量扫"同名自套一层"。**尺子**:`scripts/check-i18n-locale-content-language.mjs`(唯一码位族表在 `scripts/lib/i18n-script-families.mjs`,五族区间两两不相交由 `rangesAreDisjoint()` 机器证明)。**三条能力边界要照抄进判断**:汉字族内部**简繁码位判不了**(日本新字体与中国简化字同码位:気/会/図/点),所以只在"族级矛盾"上判红;`en` 整门 L1 刻意跳过(品牌原文必须留汉字);超过 4000 叶的大块计"未判定"而非通过 —— `--strict` 因此**恒 exit 2 直到未判定被清**,那是它拒绝出合格证,不是仓库故障。**定级**:L1 默认档只报数、`--strict` 判红(建门实测候选 16 处,当场 blocking 就是恒红门 ⇒ 逼人跳门连带废掉全部守门,§12e 同型);L2 按每文件 HEAD 自身存量棘轮(建门实测**存量 5 处,不是本节先前写的"现仓 0 处"** —— 该数字已被下面这条实测推翻并就地改写,照旧文派单会去找一个不存在的零存量)。问责入口 `pnpm check:i18n-locale-content-language`(默认档,已串进 `check:all`)/ `pnpm check:i18n-locale-content-language:strict`。**建门当天顺带抓到两件事**:① 门自己第一版把"块级 4000 叶上限"误套在**文件**级 ⇒ `web/*.json` 22,702 叶只扫 4,000,连真事故都看不见;② 上面①②③三处内容**我曾按 `b4d2305c6` 改对过一次,被并发提交 `9ccfd31a8` 拿旧基线整文件写回,五道 i18n 门与部署环一路绿灯**,直到这道新门复验才现形(内容由 `55e772b78` 重新改对)。⇒ **通用教训:语言包这类多会话共写文件,"我改过了"不构成事实,判据必须在被审面上现读。**
...
- **名字承诺/实现兑现对账**(guardian id 以 `scripts/guardian-runner.mjs` 现值为准 —— 本枚取号器落地时给到 **137**，并发会话可能挪号；blocking，`stagedTriggers=apps/ packages/ scripts/ deploy/ monitoring/ sdks/`，紧急跳过 `HUSKY_SKIP_DIGEST_NAME_REALITY=1`)：`scripts/check-digest-name-reality.mjs` —— 拦的是**「函数或字段的名字承诺了摘要/脱敏，实现却没兑现」这一型**：读判据的人相信名字，而名字可以是张空支票。立项因由是本会话同轮量到的 4 处：`cross_account_guard._fingerprint_hash()` 文档写「计算指纹哈希」而实现是 `return "|".join(parts)`（明文，内含 UA/时区/伪经纬度），被原样持久化成设备图谱的跨账号比较键；`apps/cli/src/doom-loop-detector.ts` 的 `hashInput()` 字段全程叫 `inputHash`、实现是 `JSON.stringify(整个工具入参)`，随 `pattern` 明文 POST 进服务端长期落库；`deploy/scripts/ai-diagnose.mjs` 注释承诺「发送前一律抹掉」而正则只盖三族；`packages/types/src/device.ts` 写「指纹不可逆」而实现是有界字典上的 FNV-1a。**三条与门缺一不可**（只看"没有哈希调用"会把 Redis 的 hash 结构键名、显示档 `mask*`、JWT 的 `signature` 段名、内存内比较键满天误报）：① 命名承诺（**词元匹配非子串**，`hashtags` 不得被读成 `has`+`tags`=hash 承诺）② 可达返回/赋值面内无任何真散列调用 ③ 值流向持久化/出网出口或被当等值标识用。判不出的一律落 **未判定** 并报数（`digest` 在「散文摘要」与「密码学摘要」两义同形，机械分不开 —— 本仓最高频的失效型是"把没判写成判过了"，所以 `--strict` 下有未判定即 **exit 2，拒绝出具合格证**）。口径同 70/77/83/98/103/118：全量判 **HEAD blob**、`--staged` 判**索引 blob**、`--worktree` 仅人工、两面旗同给 exit 2、枚举到 0 个候选判死；`--staged` 棘轮锚点 = 该文件 HEAD 自身存量。**行内出口 `digest-name-exempt: <原因>` 必须带原因、且只认声明行与其紧邻上一行**（写两行注释把标记顶到上上行 = 豁免不生效，本门落地当天主会话自己踩过并改正），已挂守门 108 的 30 天存活期档 —— 命名债一律**改名收口**而不是用豁免遮（`oauth2.ts` 那处 `hashed` 列表里混着明文 legacy secret，就是改名为 `storedCandidates` 而非豁免）。**假阳性清单如实登记**：首跑 3 处命中经逐条复核，`fingerprints`（哈希值的集合）与 `hashIdx`（URL 片段 `#` 的位置下标）是同词不同义、带原因豁免；判据有牙的证明是**阳性对照** —— 把修复前那枚父提交（`69a3d72f2`）的 `cross_account_guard.py` 与 `doom-loop-detector.ts` 原文喂同一判据 ⇒ 命中 2/2 文件，而修复后的 HEAD ⇒ 命中 0。取证 `--self-test` 35 条 + §22c 镜像测试（含"从注册表按大括号配对取出本门那一条再判 blocking+skipEnv"的装车证明、"摘线不得被读成已装车"的反向对照、"预筛必须是判据关键字超集"、"真散列形态必须落放过"）。手动问责入口 `pnpm check:digest-name`（走 `--strict`）。**两条写门过程中由主会话接手修掉的缺陷（F 代理撞轮次上限死在半途，记下来防再犯）**：① `JS_ASSIGN_RE` 定义了却从未接进 `findUnits` ⇒ `const digest = 拼接` 整型隐身，既不判红也不记未判定（自检 23 抓到的是"静默放过"），接上后候选声明 351→496；② `git grep` 的 `--cached` 被放在模式**之后** ⇒ `--staged` 档 `fatal: unable to resolve revision: --cached`，而 runner 给每道门追加 `--staged`，**接上就是每一次提交恒红**。
- **后台任务类型三面 parity**(guardian id 以 `scripts/guardian-runner.mjs` 现值为准 —— 本枚落地时取到 **136**,并发会话可能挪号;blocking,`stagedTriggers=apps/ai-service/app/services/` + `apps/ai-service/app/api/`,紧急跳过 `HUSKY_SKIP_BG_TASK_TYPE_PARITY=1`):`scripts/check-background-task-type-parity.mjs` —— V3 #51 立。钉的是"**声明了任务类型却没有执行器**"和"**有执行器却没人接线**"两型:声明面(`run_in_background` 广告给模型的 task_type 集合)↔ 实现面(`task_executors.py` 注册表)↔ 接线面(生产调用点)三面必须一致,任一面多出即红;回显型 executor(sleep/echo 冒充执行)直接判红。**存量按"在账未接线"报数不判红**(落地当天现读:六类中仅 2 类接线、6 类在账)—— 当场判红就是一台与任何提交都无关的恒红门,唯一结局是逼人 `--no-verify` 连带废掉全部守门(§12e 同型)。取证 `--self-test` 30 条 + §22c 镜像 6 例(其中 T2 按**大括号配对取出本门那一条注册项**再判 `mode: 'blocking'+skipEnv+stagedTriggers —— 上一版取「脚本名前后各 400 字符」当条目范围,会跨进邻门:别人有 blocking 就算我有;它原先还断言 `blocking: true`,而本仓 runner 167 处全是 `mode: 'blocking'`,那条断言对任何正确注册的门都恒红)。 **注册曾被并发会话按旧副本整块写回**:落地当天 `grep -c check-background-task-type-parity scripts/guardian-runner.mjs` 由 1 变 0,而 AGENTS 仍点名它 ⇒ 门在、判据对、无人调度,且没有一道门会喊(它头注没自称已接线所以 R1/R2 不响,镜像 T2 的「未注册」分支也照样绿)。教训:接完门必须在 **HEAD 面**复现 grep 计数,不能只看落地器打印的成功;编号一律现读,不得照抄本文。**尚无 §22c 镜像测试,已登记为残余**。

- **批量写计数诚实性对账**(guardian id 以 `scripts/guardian-runner.mjs` 现值为准 —— 本枚落地时取到 134,并发会话可能挪号;`blocking`,`stagedTriggers=apps/api/src/routes/` + `apps/api/src/db/`,紧急跳过 `HUSKY_SKIP_BATCH_WRITE_COUNT_HONESTY=1`):`scripts/check-batch-write-count-honesty.mjs` —— 拦的是本仓 2026-09-26 一整批同型静默失真的**形状**:`db.delete/update ... where inArray(ids)` 之后把**请求侧数组的 `.length`** 当 `affected` / `deleted` / `restored` 回给前端。那一型的问题是"改了 0 行"与"改成功"在响应上完全同形 —— `where` 带 `eq(roleId)` / `eq(memberId)` 过滤时,传进来的 id 一个都不命中也照样回 `deleted: 3`,后台显示"已删除 3 项"而库里一行没动。当轮人肉清出 8 处(`chat.ts` batch、`admin-sys/role-routes.ts` cancelAll+selectAll、`admin-demand-square.ts`、`admin/_shared.ts` 的 `registerCrud` 批量删〔**19 处 admin 路由复用同一个工厂**〕、`message.ts` 批量删、`workspace.ts` batch-delete/batch-restore),统一出口是 `apps/api/src/utils/batch-outcome.ts`(`dedupeIds` + `batchWriteOutcome(requested, confirmed)` ⇒ `{requestedIds, affected, missedIds}`),db 层一律 `.returning({id})` 回报**库确认集合**,响应新增 `missedIds` 逐条点名(既有键名与 status code 逐字不变)。**四条放过通道,每条都有正反用例**:① 同一写链带 `.returning(`;② 计数根标识符 ≤2 跳可追到归属预查询(`business-card-routes.ts` 那种"先查 owned 再删"的**正确写法**不得判红);③ 该文件 import 了唯一出口;④ 行内 `batch-count-exempt: <原因>`(须带原因,裸标记不计)。**刻意不判的两型**:布尔 `deleted: true`(全仓惯例,HEAD 面实测 257 处、10+ 条用例当契约钉,要改属全 API 语义决策、交人定,不是这道门能顺手做的)与读查询 `count: rows.length`(不是写)。**判不了的形状如实登记**:写链在 db 层而计数在 handler(`workspace.ts` 那一型收口前看不见)、`send` 在嵌套箭头里、`deleted: (await …).length` 非点号链形态 —— 这些落 `未判定` 并报数,**`--strict` 下有未判定即 exit 2**(拒绝出合格证)。**口径同 70/77/83/98/103/118**:全量判 HEAD blob、`--staged` 判索引 blob、`--worktree` 仅人工、两面旗同给 exit 2、取不到判"无法判定"、枚举到 0 个候选判死;棘轮锚点 = 该文件 HEAD 自身违规数,所以存量未清时判红不会变成人人跳门。**两处遮噪方向不同**(本门自己踩过):判写链要"剥注释 + 保留字符串"(标识符落在字符串里的形态要看得见),判 `import` 说明符则连字符串一起抹会直接失明。取证 `--self-test`(35 条 = 构造面 27 + 临时 git 仓端到面 8)+ 镜像 10 例(含"禁 `process.cwd()` 定根 / 禁自派生 git 读内容 / 默认档必须判 HEAD"三条反向锁)。立门前阳性对照用**修复前的 HEAD blob** 喂同一判据:`_shared.ts ⇒ 342:deleted:idList.length`、`message.ts ⇒ 348:deleted:body.ids.length`,证明判据真能命中本仓实际产出的形态。
...
- **桌面端事件链路接线守门**(无 id,hook :606): `scripts/check-desktop-event-wiring.mjs`(blocking) —— Tauri `.emit("事件名")` → `use-desktop.ts` listen+case+`new CustomEvent` → 前端 `addEventListener`/事件注册表三层对账,任一层断链 exit 1(立因托盘主题/设置点击无反应);**它在 `guardian-runner` 与 `check:all` 的清单之外**(钩子直调、无 id)—— 所以"全量审计 0 红"不覆盖它,反过来它红了也只会表现为"每次提交都要跳门";紧急跳过 `HUSKY_SKIP_DESKTOP_EVENT_WIRING=1`(hook :602);另在 `pnpm check:all` 链内。
  - **G4 的"就绪点亮"按可达性判,不按字面量所在函数体判(2026-09-26 修,登记 G-205)**:该判据原先只取 `take_pending_deep_links` 的函数体找 `target_ready = true`,而 `15573c449` 把状态迁移抽成了纯函数 `apply_take_ready`(为了 Rust 侧单测能直接断言)—— 赋值换了地方,门就红了,于是**全队每次提交被逼 `--no-verify`、约 158 道门对每次提交作废**。现判据 = 命令体内没有时,**沿命令体真的调用到了的函数再找一跳**(不追传递闭包、不做全文件搜索)。正反自检两条:赋值提交给同文件 helper ⇒ 不得判红(门必须认自己产出的形态)/ 命令体与 helper 都不置就绪 ⇒ **必红**(允许一跳不等于放过)。**同格判据由两条会话同日独立改到,落地的是走得更远的那一版**(它同时新增 **G8** 整页导航复位对账,并含"导航面找不到任何发起点 ⇒ 判失明、不记通过");本会话那版作废 —— 记这一句是为了让后来人知道**最终生效的是哪一份实现**,两条登记并存就会有人按没落地的改法去推下一步。**通用规矩:凡"某处必须出现 X"的判断,先问"X 被抽进被调函数还算不算";按文本位置判 red 的代价从来不是误报一次,而是大家学会跳过钩子。**
...
- **mobile-rn global.css token 同步守门**(无 id,由 `scripts/lib/pre-commit-hook.js` 的 16c 步条件 blocking 调用): `scripts/check-rn-global-css-sync.mjs`(blocking) —— `apps/mobile-rn/global.css` 的 `--color-*` 值与 `packages/design-tokens/src/styles/tokens.css` 逐变量对账(NativeWind 手抄副本,漂移即视觉 bug),不一致 exit 1,staged 触及两文件之一才触发。**取材面 2026-09-26 收口**(与 36/124/93 同口径):默认判 **HEAD blob**、`--staged` 判**索引 blob**、`--worktree` 只作人工逃生舱、两面旗同给 exit 2、任一面取不到**不回落**另一个面 ⇒ exit 2。旧形态两处缺陷叠在一起:① 默认判磁盘;② `--staged` 索引取不到时悄悄回落 HEAD —— **回落就是把"没判"写成"判过了"**,而自洽的假绿比可见的红难查。**同批给钩子调用点补上 `--staged`**(这条不是顺手改:本门收口后默认档判 HEAD,钩子若不传面旗就等于在审上一提交态,生成器刚写回、已 `git add` 的新值它结构上看不见 —— 而它是 `TOKEN_SYNC_TARGETS` mobile-rn 那一行写回之后的**唯一**复核闸)。取证:镜像测试 22 例(含 F1–F5:纯函数四态锁、临时仓三面互异、CLI 三面成对、无提交判死、F5 形状锁禁止 `gitShow` / `readFileSync(join(root` / `node:fs`),变异自证 `def:'worktree'` 后 4 例红 ⇒ "默认判 HEAD"不是恒真);真仓 HEAD 与索引两面 exit 0(187 受管档逐位同值、26 个端内自立 `--color-*` 档只报数)。仍无应急通道(hook 未设 skip env)。
...
<!-- ⚠️ 本行与上方同名条目(第 1339 行)同体,是同日两次改写被并集留下的旧副本;现行以第 1339 行为准。其中"10 例/12 例"之类的镜像测试例数、以及任何具体收口块数,都按当次实测取,勿照本行派单。保留本行只为不丢行(AGENTS §12)。 -->- **架构契约对账**(103):`scripts/check-architecture-policy.mjs`(blocking,2026-09-25 立)—— 与其余全部守门**方向相反**的一道:其余是"发现一类违规 → 写一条判据"(打地鼠),本门读 `config/architecture-policy.yaml` 这张**声明表**,从声明反查违规。四类红:**T1** 表与现实脱节(模块改名/目录搬走而表没跟上 —— 表一过期,依赖它的所有判断都在对着空气打分)/ **C1** 单文件行上限(阈值 6000,**取自 HEAD 实测最大文件 5258 行之上**,依据写在 yaml 注释里)/ **D1 D2** 依赖方向(包 import 端、或 `requires` 未声明就 import;以及反向按 rank 更深一层)/ **D3** 深导入(绕过 `public_entrypoints` 直接摸别的包内部 —— 它是"端内重复实现"的入口)。**渐进收口是设计前提**:存量模块一律 `managed: false` ⇒ 只报数不判红(立项实测全量 exit 0、报数 3 处、`managed:true` 0 个)(⚠️ 这几个数字是**立项状态、不是现值**;现值一律跑 `node scripts/check-architecture-policy.mjs` 看 `managed:true` 行),翻 true 前先用 `--managed-trial <id>` 看条数,**不得为消红去改阈值或把不该对外的模块标成 exported**。行内豁免 `arch-exempt: <原因>`(须带原因)。口径同 70/77/83/98/101:全量判 **HEAD blob**、`--staged` 判索引 blob。**策略表自身的取材按档定向**(`policyFaceOrder`):全量档 HEAD 优先、**`--staged` 档索引优先**,两者都再降级到工作树;取不到判"无法判定",不冒红也不记绿。⚠️ 这里 2026-09-25 修掉一处**本门对自身改动的盲视**:原实现两个面都 HEAD 优先,于是"修改策略表自身"的那枚提交完全不进本门审查 —— 实测往索引版 `apps/cli.requires` 注入一条 `apps/api`(端应用 `exported:false`,T1 必判红)后全量与 `--staged` 双双 exit 0,而 `--staged` 输出照旧打印旧表的 `managed:true` 清单。这个洞偏偏落在"规矩 2:翻 `managed:true` 前先试跑"所要求的动作上(提交链是唯一无验的一环),且源码旧注释"表是输入不是被审对象"正是它的设计理由,已就地推翻。取证 `--self-test` 51 例(成对正反 + 两面口径差异 + 解析器坏了必须大声失败)+ 镜像测试 13 例(含 runner 装车证明、"同一代码 managed 两侧结论不同"、T12 的**纯函数+构造面**取材面证明、以及 T12b 的尺子反例;变异回旧顺序只有 T12 红)。**本门自身踩过的判据教训(勿再犯)**:这里曾写过一条"索引表≠HEAD 表 ⇒ 两档收口集合必异形"的条件断言,两条前提都不成立(改注释/requires 都会让表不同而集合不变;尺子若吞到行尾则两档因"扫描 N 文件"天然不等)⇒ 一支误红、一支无牙,已删。**证明取材面这类行为只能用纯函数+构造面**,不得依赖仓库瞬时状态。**收口进度不写进文档**:某模块是否 `managed:true` 一律按当次实测取(`--staged` 输出行即现值),理由与本门刚修的那类缺陷同源 —— 登记过期数字会替人做出"已经收口"的判断。编号说明:任务书原定 102,落地前已被 `check-glyph-arrow-icon` 占用,按"后来者改号"顺延 103。紧急跳过 `HUSKY_SKIP_ARCH_POLICY=1`。
...
- **"服务身份能不能跑到这一步"要用它那份环境验,不得用我的终端验(2026-09-25 实测)**:上面那条讲"哪份文件",这条讲**那份文件跑在谁的身份下**。nssm 服务全是 `LocalSystem`,它读的是**机器级 PATH**,而本机机器级 PATH 里那串 `D:\nodejs\` 是**死路径**(目录根本不存在),node 真身 `D:\DevEnv\runtimes\node\node.exe` 只挂在**用户级 PATH** —— 于是 `Get-Command node.exe` 在人手里命中、在服务手里落空,凡是"取凭据/发通知要先找 node"的分支会**静默跳过并回落兜底档**,现象是"手动跑是对的、自动跑用的是旧账号",而日志只有一行 WARN。处置口径:① 取可执行程序一律走 `Resolve-NodeExe`(`deploy/win/ihui-deploy.ps1:120-131`,绝对路径候选已内建),不得只写 `Get-Command`;② 兜底全落空必须**喊出来**(Write-Host WARN/ERROR + 非零),严禁 `if (Get-Command …) { … }` 无 else 静默降级 —— 那与 §5e"失败必须响"是同一条禁令;③ 验证要剥掉环境再跑**服务真正执行的那份副本**:`$env:Path` 去掉含 `node`/`DevEnv` 的条目后跑 `deploy/prod-bundle/*.ps1`,看输出行报的是哪个角色,而不是在我终端里跑 `deploy/win/*`。实例与取证见 `deploy/win/ihui-pg-backup.ps1` 的凭据段注释与 PLAN 第四十九批·续十。
  ...
  - **认领升级为租约(2026-09-25 立,守门 109 `check-task-claims.mjs --check-gate`)**:裸 `（进行中）` 只是文本 —— 没有持有者、没有时间戳、不判年龄,于是 agent 一死标记就永久留在 HEAD,下一个派单会话按上一条扫描**要么看不见那件活、要么把它当"别人正在做"而按住不做**(本仓记录里已反复出现,不需要构造)。**新格式**:`- [ ]（进行中@YYYY-MM-DD/持有者短名）`,三要素齐备才算一次认领;裸 `（进行中）` 仍被解析但**只计数不判红**(实测存量 36 条全是裸标记,当场判红就是一台恒红门)。年龄 > 72h(`IHUI_CLAIM_LEASE_TTL_HOURS` / `--ttl-hours` 可调)判红并点名行号与持有者;写了 `@日期` 却缺 `/持有者` 同样判红 —— 半个租约比没有租约更危险,它看起来像被管过。同一行既"进行中"又 `[x]` 判红(协议自相矛盾 = 释放失败的现场)。**门绝不自动摘除任何人的认领**(那属 §16 越权),出路只有三条:续租(改日期)/ 完成翻勾并摘牌 / 显式让渡给某持有者。派单前除扫进行中项,还要跑一次三轴基线自检 `node scripts/check-baseline-freshness.mjs --preflight`(见 §守门脚本速查)。
...
- **候选序解析器必须同时提供「出处」出口**(2026-09-25 立,A19):`scripts/lib/key-dir.mjs` 的 `resolveKeyDirDetailed()` 返回 `{path, triedCandidates[], winnerIndex}` 三态 —— 一个根都没命中(盘没挂上 / 根缺失 = **无法判定**,不得报成「凭据失效」)/ 根在而子目录不在(先建目录再放口令)/ 命中。`resolveKeyDir()` 的签名与行为**一字未动**(它是 12+ 个脚本的依赖面,detailed 版是它的超集,旧版由新版降格投影而来,等价性由单测钉住)。新增任何候选序解析器都要照此配对,调用方**禁止再自己抄一份 F→D→E→G→C 候选表** —— 那正是本节记过的「读不到被下游报成失效」的成因。**但这条是散文约束、没有门**:判「有没有真的回答出处」结构上判不了,为它立门只会逼人写空调用去应付(如实登记,别误以为这里有尺子)。
...
- **第三方来源台账对账**(guardian id **107**,blocking,紧急跳过 `HUSKY_SKIP_PROVENANCE_LEDGER=1`):`scripts/provenance-ledger.mjs` 把"哪些内容不是我们写的"变成机器可查事实 —— 三本账(嵌入 / 复制 / 覆盖)+ 一本机制来源账,数据在 `config/third-party-provenance/*.json`,许可原文按内容哈希自证(读盘不联网)。四类红:P1 `roots` 指向不存在的路径 / P2 盘上有 vendored 目录或第三方归属头而台账无条目 / P3 已登记条目 hash 漂移 / P4 revision 不可考却没显式 `null` + 说明 + P8 **归属反噬**(2026-09-25 同日补):已登记的第三方内容上不得出现我方水印横幅/版权主张,出现即红 —— 起因是实测到 `apps/web/public/pdfjs/pdf.worker.min.mjs` 第 1 行是我们的横幅(等于把 Mozilla 的作品声明成我们的)。水印层的排除源唯一 = 台账 roots(经 `scripts/lib/third-party-roots.mjs`,`watermark.mjs` 与 `check-watermark-coverage.mjs` 两个消费者共用),**这些文件不是改成看不见,而是改由 P8 审计**。口径同 70/77/83/98/101/103(全量判 HEAD blob、`--staged` 判索引、取不到判"无法判定")。**刻意不挂 `stagedTriggers`**:新登记一条 vendored 内容可以发生在任何路径。立项两记否证值得记:(a) 规格称 `vendor/` 是唯一 vendored 件 —— 实测另有 PDF.js worker、`gradlew{,.bat}`、lucide svg 三处未登记;(b) §5c 的零宽水印门禁与"许可原文不得改动格式"**结构冲突**,那些文件至今幸免只因 `SKIP_DIRS` 里恰有一条 `'vendor'`,是巧合不是设计 —— 处置写进台账注释,**不得用豁免遮**。
  - **可审计锚点必须受版本控制(P5 的 specFile 由"工作树存在"改成"按被审面存在",2026-09-25 立,G-173 换来的)**:旧判据是裸 `existsSync(join(root, e.specFile))`,而台账 `$schemaNote` 当时明写"specFile 允许是 gitignore 的临时规格"——**"允许临时"与"blocking 判存在"结构互斥**:gitignored 路径不进任何检出,而本仓 post-commit 的 `--auto-clean` 自己会清掉它,结果就是作者机常绿、别的检出上**每次提交都被逼 `--no-verify`**(实测 145 项里它一项红)。**通用规矩:凡被 blocking 门要求"必须存在的文件",其路径要先过面判据(`reader.has`)或至少过 `git check-ignore`;写进 `.ihui-agent/` 的引用不叫证据,叫机器-local 巧合。** 取证是自检 `8e` 两条成对用例 —— 夹具故意让锚点**在工作树上存在但未提交**,新判据必须红;旧写法在这里报绿,所以这条同时是"新判据有牙"的证明,不许用"改文案"把它退化成恒红。数据侧 `specFile` 已重指到受版本控制的 `AGENTS.md` 本条,并用 `specNote` 如实声明"上游研究笔记未幸存、内容无从找回,**不得把该字段读成原文仍在**"——恢复不了的东西不要伪造。
...
- **任务认领租约对账**(guardian id **109**,blocking,`stagedTriggers=PROJECT_PLAN.md`,紧急跳过 `HUSKY_SKIP_TASK_CLAIM_LEASE=1`):`scripts/check-task-claims.mjs --check-gate` 把 §1 的认领标记升级成带归属与寿命的锁(见 §1 第二条)。CL1 年龄超阈值(默认 72h)/ CL2 有 `@日期` 无 `/持有者` / CL3 同一行**在勾选框位置**既已 `[x]` 又挂着租约形态 `（进行中@…）`(正文叙述里的 `[x]` 不算 —— 判据把叙述当状态,就会替另一会话持有的行造出恒红)。**CL3 自 2026-09-26 起套「HEAD 自身存量」棘轮**:锚点是矛盾行的**行文本多重集**,不是"前 N 条"计数 —— 计数式切法会让新塞进文档前面的一颗顶掉一颗旧残留的名额,两者都得绿(镜像测试 T17 当场抓到第一版实现就是这样)。归并器"翻勾不摘牌"已在**生产者侧**收口(`scripts/plan-tasks-merge.mjs` 的 `rewriteFork()` 翻勾前先摘牌,配 T6 与变异对照),HEAD 里那批残留(逐条带 `**[归并]**` 出处标记)按存量只报数,**换个文案的新增一律判红**;`--worktree` / `--plan` 两条人工与取证面**不套棘轮**,仍全量判。锚点取不到 ⇒ 显式喊"未判定"并列出条数,**不冒绿**。棘轮锚点取 HEAD 而不是手工清单,是为了让它只会自己收紧:谁把残留清进 HEAD,红线就跟着降。**向后兼容第一**:存量裸 `（进行中）` 一律只计数不判红,否则上线当天即恒红门。门**绝不自动摘除任何人的认领**(§16 越权)。
...
- **测试收集存续性对账**(guardian id **114**,**warn**,紧急跳过 `HUSKY_SKIP_TEST_COLLECTION=1`):`scripts/check-test-collection-runs.mjs` 判的是"**收集阶段**就失败的套件数必须为 0"。立因是实测到的结构性失明:`apps/mobile-rn` 一度 `Test Files 15 failed | 37 passed`,其中 14 个是 transform / import 解析期失败,内含 131 条 it 声明(展开 137 枚用例)**一条都没跑** —— 端内约三分之一覆盖被静默削掉,而提交链里 130+ 道门没有一道跑 vitest(`check-staged-typecheck` 走 tsc,结构上看不见解析期失败),CI 会红但本机可以永远看着绿。**定级 warn 而非 blocking 是设计前提**:本门判的是端内测试能否被收集,一次提交完全可能与它无关,与改动无关的恒红门只会逼人 `--no-verify` 并连带废掉全部门(§12e 同型);要问责跑 `--strict`(CI / 巡检)。**三条不可动摇的口径**:① 零测试文件不判绿(空扫就是它要防的同一型故障);② 断言失败与收集失败绝不混计 —— `Test Files 1 failed` 两种故障都会出现,故文件级计数不能作判据,只认 `Failed Suites` 区块与带方括号的套件级形态;③ 只接 vitest,pytest 的收集期故障是另一套语法(`ERROR … - ModuleNotFoundError` + `no tests ran`),照抄判据必然空转,非 vitest 入口一律判"未判定"并写明原因,不冒绿。`stagedTriggers` 取 `apps/mobile-rn/` + `packages/` —— 能打断这份收集的只有 RN 端自身与它吃的共享包。
...
- ⚠️ **中文出参会被控制台码页吃掉(2026-09-25 实测,和上一条同为"码页"但方向相反)**:上面说中文当 **argv** 传出去无损;但 node 往 stdout 打的**路径**含中文(`密钥/...`)时,PowerShell 按 **GBK 码页**解码 ⇒ 拿回的是乱码字符串 ⇒ `Test-Path` 判"不存在" ⇒ 脚本静默退回兜底凭据,表现得像"口令文件没建"。**凡"把一个值读回来继续用"的场合(路径/文件名/URL),调 node 前必须显式 `[Console]::OutputEncoding = UTF8` 并在 finally 还原**;只在控制台显示乱码的是"输出面",这条不成立 —— 值一旦参与判据就不是显示问题。同类修法也适用于 §5e 说的"多行中文正文走 --message-file"。
- **凭据目录现状**(2026-09-25 起):`db-backup/ihui-backup.txt` = 只读备份账号 `beifen` 的口令;`IHUI生产账号/pgsql管理员-postgres-*.txt` = 该库 `postgres` 的口令(此前**从未设过**,详见 `deploy/win/ihui-pg-backup-role.sql` 顶部的执行记录)。两份都在仓库外、都在 §5d 权威目录里,都不进日志/聊天/提交。
- **发给 provider 的 function parameters 必须由校验面单向投影**(2026-09-25 立,A13):统一出口是 `@ihui/types` 的 `projectToolInputSchema` / `toolsToProviderSchema` (`packages/types/src/schema-projection.ts`),**禁止端内手搓 `parameters` 对象或自拼第二份 JSON Schema** —— "运行时怎么校验"与"模型被告知怎么填"必须是同一份描述的两个投影,否则两边各自漂移而 typecheck 全绿。等价性由 158/158 个真工具对象的同瞬间 A/B 证明(线字节 + 属性名集合 + required 集合三项全等)。 **⚠️ 那句"158/158"当场不可复核,现已换成可重跑判据(2026-09-25)**:158 出自提交 `dd5142f2255` 的一次性 A/B 脚本,而那个脚本**没入库**、**口径也不同** —— 它按 `文件#name` 计数,把 re-export 的同一工具重复计了 56 次,所以 158 ≠ 门 111 的 104。更糟的是那条 A/B **今天再跑仍报 158/158,而它是恒真式**:它比的"旧侧" `toolsToProviderSchema` 在 `dd5142f2255` 之后自己就改成调投影器了,等于拿新实现和它自己比 ⇒ "已证明"三个字当时成立、事后失效,而账面什么都看不出来。现在这句话的唯一入库载体是 `apps/cli/tests/a13-projection-equivalence.test.ts`:旧侧取 `git show dd5142f2255^` 的**历史源码逐字快照**(`tests/fixtures/toJsonProperty.legacy.ts`,头注禁止演进),断言①门数到的工具集合与运行时枚举到的**逐名相等**、②逐工具三项全等(线字节 / properties 键集 / required 集)、③两条变异对照各红在对应判据、④枚举到 0 个工具判死不判绿。**登记一条已知局限**:运行时侧读磁盘而门侧判 HEAD ⇒ 他人在飞新增的真工具会让"逐名相等"点名它,红可能来自别人未提交的文件(§77/§83 同型)。
- **压缩分母对账**(112):`scripts/check-compaction-denominator.mjs`(blocking,2026-09-25 立)—— 钉的是 A30 那条实测缺口:provider 的 context window 是 **input 与 output 共享的同一个窗口**,所以"什么时候该压缩"的分母必须**先扣掉本轮留给输出的预留**;不扣就是拿"占满整个共享窗口的 88%"当阈值,而同一窗口里模型还要生成回复 —— 故障形态是**越接近上限越容易炸,现象却写成"压缩判过了还是 400/overflow",极难归因到分母**。立门前先确认"没有等价实现"再确认"没有这个名字"(两步都跑:本仓实测 `grep -rniE "outputReserve|effectiveContextWindow" apps/cli/src packages/context-compaction/src` = 0)。唯一出口 `effectiveContextWindow({contextWindow, maxOutputTokens, buffer, enabled})`,预留封顶 `MAX_OUTPUT_RESERVE_TOKENS`;凡「token 数 ÷ `contextLimit`/`contextWindow`」而分母未经该出口 ⇒ 红。**只从包主入口导出,不得新建子路径** —— 架构契约表没登记 `packages/context-compaction` 的子入口,深导入会被守门 103 判 D3 红(本仓踩过)。**行为后果如实登记**:分母只会变小 ⇒ **压缩更早触发**,这是用户可感知的变化,回退有两条路(构造参数 `outputReserveEnabled:false` / 环境变量 `IHUI_COMPACTION_OUTPUT_RESERVE=0`);存量 8 文件/12 处进 `scripts/compaction-denominator-baseline.json` 只减不增,其中 **api 端 `/chat/stream` 的溢出面尚未收口** —— 那是留下的账,不得读成"已修完"。口径同 77/83/98/103:全量判 HEAD blob、`--staged` 判索引 blob、取不到 ⇒ **exit 2 无法判定**、枚举到 0 个文件判死不记绿。取证 `--self-test` 15 条(成对正反例 + 空枚举反向对照)+ §22c 镜像测试;紧急跳过 `HUSKY_SKIP_COMPACTION_DENOMINATOR=1`。
...
- **入参校验装车对账**(115):`scripts/check-tool-arg-validation-wired.mjs`(blocking,`stagedTriggers=apps/cli/src/tools/` + `packages/types/src/`,2026-09-25 立) —— 堵的是 A36 查出来的那一格:`apps/cli/src/tools/argument-validator.ts` 校验器写得挺全(field/expected/actual、嵌套 `items[0]` 路径都有),**但生产面零调用方** ⇒ 工具入参从来没被校验过,`required` 只拿去生成提示文案和 provider schema,模型少传/传错类型时由 104 枚 handler 各自兜 ⇒ 同一类错误 104 种表现。**只判两条**:① `validateToolArguments(` 必须有非测试调用方(**注释里的提及不算** —— 本仓最高频失效型就是"看起来有、其实没装车",全仓三处 `argument-validator` 的出现位置全是注释);② 模式开关必须在位、**默认 off**、`shadow` 档存在 —— `enforce` 未实现前默认绝不能是 enforce,因为那些 `parameters` 描述**从未被执行过、准确度未知**,直接打开拒绝就是运行时版恒红事故(与 §12e/守门 77/83 同一条理由)。影子档只记账:不改 `call.arguments`、不改返回值、不拦调用,遥测**不落任何入参值**(`ValidationError.actual` 在 `enum_mismatch` 分支装的就是用户原值,`expected` 装枚举表 ⇒ 双双排除)。已知覆盖面缺口如实登记:`hubEnabled && hubResolver` 分支在拿到 Tool 对象之前就 return,那条路径影子不生效。三步顺序(①影子 → ②用影子数据修描述 → ③才谈默认 enforce + 逐条回灌 `{字段路径,期望,实得}`)登记在 PROJECT_PLAN 第八波。口径同 77/83/98/103/112/113(全量判 HEAD blob、`--staged` 判索引、取不到 exit 2、枚举 0 判死);取证 `--self-test` 14 条 + §22c 镜像 5 例;紧急跳过 `HUSKY_SKIP_TOOL_ARG_VALIDATION_WIRED=1`。
  **本票新建的两把常驻工具(量算/归并出口,不是守门 —— 勿因"没接 guardian-runner"就当作没装车)**:
  `scripts/c-disk-breakdown.mjs` —— 全盘**只读**量算仪,junction 安全(刻意不跟随重解析点,否则把 D 盘的量算成
  C 盘的债),强制对账「遍历 + 特殊文件 + 未解释 = 已用」,未解释 >2GB 或为负都当场喊话。报磁盘状态先跑它,
  不要跑 `df`(§26 已记过一次它给出相反百分比)。用法 `node scripts/c-disk-breakdown.mjs [--root C:/] [--depth N] [--json]`。
  ...
  `scripts/plan-union-merge.mjs` —— 活文档台账被撞号或整段回写时的人工归并出口(临时索引 + commit-tree + CAS,
  落地前自证"本侧/对侧路径零丢失"),镜像测试 4 例在位。两者**都不做任何删除**。
- **出站事实对账**(116):`scripts/check-egress-facts.mjs`(blocking,2026-09-25 立) —— 钉 A34:一趟请求**实际**用的代理/CA/NO_PROXY 配置必须是**响应上的返回值**,不是日志行。成因不是假想:AGENTS §5b 连着记过"git 网络时通时不通""钩子进程不继承 shell env""服务身份与交互账户的 `safe.directory` 互不相通",每次都是人肉现读 `git config --local`,因为没有任何一次调用把"我这趟用了哪份配置"带回来。唯一出口 `apps/api/src/utils/proxy-dispatcher.ts` 的 `proxiedFetch`,响应挂**不可枚举** `egress` 字段(不可枚举是为防它被 `JSON.stringify(res)` 意外带进日志/响应体 —— 凭据面同族),用 `readEgressFacts(res)` 取;`isProxiedUrl` 已改成该事实的投影 ⇒ **决策与事实共用一份判据**(两处算同一件事必漂移,本仓记过多次)。厂商域名**不硬编码**,由 `VENDORS.baseUrl` + `DEFAULT_PROXY_DOMAINS` 两张真表推导(实测 100 个;推不出 ⇒ exit 2 不记绿)。覆盖面如实登记 = `apps/api/src` + `apps/cli/src/provider`;`packages/api-client` 打自家后端不入面,Python 侧只登记。**两条"没做"同样在案**:① 错误分流(被策略拦 vs 网络错两个码)—— 本仓 TS 侧没有任何出口会在传输前拒发,造两个码就是永远不响的门;② `NO_PROXY` 只读回成事实、未让它生效(改路由波及全部厂商调用,另票)。存量 6 处/5 文件进 `scripts/egress-facts-baseline.json` 每文件棘轮,只减不增;口径同 77/83/98/103/112/113/115。取证 `--self-test` 28 条 + §22c 镜像 5 例(含真表真挂装车证明);紧急跳过 `HUSKY_SKIP_EGRESS_FACTS=1`。
- **对话流元素覆盖**(57):**五类**判据(blocking)—— ① 锚点漂移(`anchor-missing-file`/`anchor-missing-marker`)② SSE 契约双端不齐 ③ 清单条目倒退(`entryCountBaseline`)④ **锚点存续性倒退**(`anchorCountBaseline` 的 `perElement` 逐条目 + `total`,含"该字段整块不见 = 台账被旧基线回写"的指纹;搬到同条目另一端是合法重构、总数不变即绿)⑤ **注释式摘线**(匹配面是**剥注释后的代码面**,字符串感知状态机;存量出口 `commentOnlyAnchorBaseline` 只报数不判红、只减不增,首轮 3 处存量已于 2026-09-25 按「mustMatch 重指向宿主代码面真实标识」清偿归零 ⇒ 新增该型直接判红,且**禁止**反向删宿主注释或补登记来消账);**判仓库内容而非共享工作区快照** —— 已暂存取索引 blob、仅工作树脏取 HEAD blob,别人未提交的重写不得钉红无关提交(2026-09-24,与守门 77 同取向)
- **测试收集存续性对账**(114,`scripts/check-test-collection-runs.mjs`,**warn 级**,紧急跳过 `HUSKY_SKIP_TEST_COLLECTION=1`)—— 堵"测试到底有没有在跑"这一维:mobile-rn 实测曾有 14 个套件在**收集阶段**整体失败、内含 131 条 it 声明(展开 137 枚用例)一条都没跑,而头条写成"2 failed"看着近乎全绿;更根本的是提交链百余道门**无一跑测试**(`check-staged-typecheck` 走 tsc,结构上看不见 transform / 解析期失败),CI 会红而提交链不拦。判据只认 `Failed Suites` 区块头与 `FAIL <file> [ <file> ]` 方括号形态两信号取大值(文件级计数在两种故障下长得一样,不作判据),断言失败另计并明写不计入本门;`No test files found` 或 `Test Files …(0)` 判红"空扫 ⇒ 覆盖归零";取不到结论(无 test script / 入口非 vitest / 入口不可解析 / 超时 / 无汇总行 / 空输出)一律**未判定**并点名原因,绝不记为通过。**刻意 warn 级**:该数字随他人工作树在 2→0→43 间跳,挂 blocking 等于每台每次逼人 `--no-verify`,一次绕过约 140 道门全废(§12e 同型);问责出口是 `--strict` / `pnpm check:test-collection`(CI 侧可换 `--require-idle` 型判红)。取证 `--self-test` 18 例(mkScratch 真造临时端、真派生)+ 镜像 17 例(含 T1 装车 / T2 摘线必判未接线 / T3 定级被改成 blocking 必读得出 / T4 全 runner id 不得重复 / T12 反向钉死不得 shell:true)。**已知边界**:只接 JS 系(vitest);pytest 侧是另一套版式、单列一票;`Failed Suites` 版式在 vitest 3.x(miniapp)是否逐字相同**未取证**,故扩端顺序是先 `--end apps/web` 连测零红再接提交链;被吞 it 的精确条数只能估算。
...
- **子代理权限继承对账**(119):`scripts/check-subagent-permission-inherited.mjs`(blocking,`stagedTriggers=apps/cli/src/`,2026-09-25 立) —— 立门前先否证了票面前提:`spawn-subagent.ts` / `subagent-registry.ts` / `permission-engine.ts` 三路径在工作树+索引+HEAD **三件套均不存在**,全仓把 bypassPermissions 写成默认档 **0 处** ⇒ "派生默认绕过权限闸"不成立。真缺陷是**反方向那一格**:HEAD 面实测 **7 处 / 4 文件**的派生点**整个不下传** permissionMode 与 permissions,子代理落兜底档后父会话 `--disallowed-tools` 的显式 deny 在子代理里查不到(实测不传规则返回 ask、传规则返回 deny,已由 vitest 钉住)。三判据:**P1** 字面量 bypassPermissions 作默认档(零容忍、不吃豁免)/ **P2** 构造调用必须下传两键(**棘轮锚点 = 该文件 HEAD 自身计数**,存量 7 处只报数,绝不造恒红门)/ **P3** 调用之外读到权限键却没喂进去(半个继承)。**继承落点的代码改动因全在实现票清单外而未做**,补丁方案与越权点写在交付报告里 —— 登记事实,不代裁。自检 39 例(含"把 import 语句里的子串误判成读到键"的反向锁)+ 镜像 12 例;紧急跳过 `HUSKY_SKIP_SUBAGENT_PERMISSION_INHERITED=1`。
- **名单类判据正向证明对账**(120):`scripts/check-list-predicate-has-positive-proof.mjs`(blocking,**不挂** `stagedTriggers` —— 名单漂移可发生在任意路径,2026-09-25 立) —— 第八批三份独立取证收敛到同一个形态缺陷:本仓大量门是"名单驱动"(拦名单里的标识符、或放过白名单成员),而它们的自测只有**反向证明**(拦到坏值才红),从没证明**名单里某一条真能命中**;名单于是可以是张死表而门一路报绿。本门要求每条登记名单在其自测/镜像测试里至少有一条断言的输入**取自名单本身**;判不出 ⇒ 记「未判定」并点名,**不记为通过**。真仓 HEAD 面实测 3 条有真证(门 52 的 43 成员 / 门 80 的 22 / 门 107 的 6),尚未入库的名单判"未判定"而非红 ⇒ 不出生即红。自检 15 条 + 镜像 6 例;紧急跳过 `HUSKY_SKIP_LIST_POSITIVE_PROOF=1`。
- **声明策略消费者对账**(121):`scripts/check-declared-policy-has-consumer.mjs`(blocking,`stagedTriggers=apps/,packages/`,2026-09-25 立) —— 判五类"声明"必须有生产面消费者(带链 import 且取用):MAX_AGE/TTL/RETENTION 策略常量、`*BudgetContract*`、packages/types 的 `*Contract` 导出类型与情态谓词、`(prune|cleanup|purge|expire|sweep)` 导出清理函数。注释/字符串/纯 re-export/测试面/scripts 层都不算消费者。立票实测:`apps/cli/src/sessions/state-store.ts` 的 `pruneOldSessions`(7 天保留)全仓零调用方 ⇒ 已挂到 `saveSession` 写前(60s 节流 + `IHUI_SESSION_PRUNE_DISABLED` 出口 + 裁剪异常不冒泡,"宁可不删");`ToolResultBudgetContract` 的唯一合理消费点在 `tools/index.ts` 的 executor 边界,该文件在飞 ⇒ 未接线登记,摘除会砸坏守门 111 的 CONTRACT_GROUPS(§7 有承接功能不得删)。全量现测:候选 131 / 未接线 32 处 29 文件只报数(subagents/state-store.ts 与 apps/api cleanup 族 9 处同型,清偿另计批)。自检 22 例 + 镜像 8 例;紧急跳过 `HUSKY_SKIP_DECLARED_POLICY_CONSUMER=1`。
- **文件写盘安全对账**(122):`scripts/check-file-write-safety.mjs`(blocking,`stagedTriggers=apps/cli/src/tools/`,2026-09-25 立) —— 工具层写文件必须走原子写出口(同目录临时文件 + `rename` 替换 + Windows rename 目标已存在会 EPERM/ENOENT 带重试 + **不跟随符号链接/junction**(§26 递归穿透事故同族)+ **读后写 stale 校验**:编辑前读到的内容与落盘前磁盘不一致时拒绝而非覆盖 —— 多会话共享工作区的真实风险)。裸 `writeFileSync` 存量走棘轮(实测 `file-edit.ts` 2 处只报数),新增即红。自检 24 例 + 镜像 13 例;紧急跳过 `HUSKY_SKIP_FILE_WRITE_SAFETY=1`。
- **工具族注册对账**(guardian id **131**,**warn**,紧急跳过 `HUSKY_SKIP_TOOL_FAMILY_REGISTERED=1`):`scripts/check-tool-family-registered.mjs` 问的是另一句话 —— tools/** 里 export 的每一族在生产面有没有把它挂进注册表的调用点。它补的是 `check-tool-registry-projection`(131 号那条近邻门)结构性看不见的那一格:**没注册 ⇒ 两侧都没有 ⇒ 那道门一致而绿**。三条到达路径(跨文件具名导入且落注册参数位 / 同文件一跳委托但须证明 owner 在生产面被调 / 随父族组合传播);注释与字符串先抹掉再判(否则门给自己发合格证);动态拼接、`export *`、命名空间转发、同名本地符号一律「未判定」—— 不记绿也绝不判红。定级 warn + **HEAD 存量棘轮**(只拦新出现的零注册族与"本次把注册点摘掉"),问责 `pnpm check:tool-family`(`--strict`),**升 blocking 前置 = 未注册存量归零**;现值一律跑该命令看末行(立项读数:族 18 / 已注册 17 / 未注册 1 = `DEBUG_TOOLS`,即 10 枚 DAP 工具与那句"30 分钟自动清理"对模型不可达 —— 是否注册它属 §24 产品决策,不是清账动作)。

- **工具执行预算对账**(123):`scripts/check-tool-exec-budget.mjs`(blocking,`stagedTriggers=apps/cli/src/tools/`,2026-09-25 立) —— 钉"工具执行无界"三源:`ToolContext` 无 `signal` 字段(取消无通道)、执行点无超时、`util/exec.ts` 无 `maxBuffer`(node 默认 1MB,超限抛错并丢已缓冲输出)。S1 系判**机制在位**(常量三件套 `TOOL_EXEC_BUDGET_{DEFAULT,MAX,SETTLE_GRACE}_MS` / 唯一解析出口 `resolveToolExecBudgetMs` 且内含 `Math.min` 封顶 / `linkAbortSignal` / `executeWithinExecBudget` 在代码位出现"自身定义+至少一次调用");S2 判**注入点**(未传 signal 的 ctx 构造逐处点名)。取材面=HEAD/索引,取不到 exit 2。自检 26 例 + 镜像 9 例;紧急跳过 `HUSKY_SKIP_TOOL_EXEC_BUDGET=1`。
...
- **钩子失败的归因是量出来的,不是抄来的(2026-09-25 立,`scripts/lib/commit-gate-attribution.mjs`)**:旧版首次 commit 一失败就打印「按用户规则"hook 失败因其他 agent 代码 → --no-verify 重试"」并整批跳门 —— 它**从未计算过归因**。实测一轮 134 道门 111 通过 / 1 失败,红的是 `check-push-sync`(判远端态、与提交内容无关),措辞仍是"他人代码"。现流程:解析 runner 的失败门清单 → **逐道复跑** → 该门这次的输出**点名本次声明的文件 ⇒ 判"本任务自己的红",拒绝 `--no-verify` 并 exit 1**(必须修);一个都没点名 ⇒ 允许跳,但只说量到的话;批没跑完 / 解析不到 / 复跑全不可用 ⇒ 明写「未归因」,按应急路径落地并留痕于 `.workbuddy/safe-commit-attestation.jsonl`。**"批一道都没跑"从 2026-09-26 起不再是中性事实,而是一个必须先补证据的分支**:`scripts/lib/pre-commit-hook.js` 里 lint-staged 跑在守门批**之前**,于是一次 lint/prettier 失败 = 156 道门全不跑 = 归因永远 `unattributed` = 照样跳门,而账面读起来像"跑过了、只是与本次无关"(实测 `acaf8407cc` 与 `d379215126` 前后各一次;这正是"判据失效的表现永远是安静"那一型,守门 70/76/81 同族)。现由 `safe-commit` 在判到 `unattributed ∨ ranFullBatch===false` 时**自己把 `guardian-runner --staged` 跑一遍**,再把那一轮的输出喂回**同一条铰链**(`classifyHookFailure`)分流:点名本次文件 ⇒ `mine` ⇒ 拒绝跳门;一个都没点名 ⇒ 可跳但措辞必须写明"取证来自 safe-commit 自跑的那一轮,不是钩子内那一轮";自跑也没成功 ⇒ 照旧落地,而 `unattributed` 的措辞必须带上具体原因(runner 不在位 / 派生失败码 / 超时)。**刻意不把 `unattributed` 改成拒绝跳门** —— 那会让"别人把我挡在 lint 外"的应急路径消失,逼人手工 `git commit` 跳得更彻底且不留痕。**一条如实登记的覆盖面上限**:自跑发生在首次 commit 失败之后,而 lint-staged 失败时会回滚它自己动过的暂存区 ⇒ 新增文件此刻已退回未跟踪,自跑那一轮看到的暂存集可能少于声明集(与既有"逐道复跑"同一刻同一索引态,两份证据因此可互相比较)。应急提速:`IHUI_SAFE_COMMIT_BATCH_TIMEOUT_MS`(极小值 ⇒ 自跑判"未成功"并带原因入库,**不是**静默开关)。取证:镜像 13 例(含"汇总块在位时绝不自跑"的反向锁与装车证明)。**stdout 没汇总而要去钩子日志取第二输入源时,该汇总块必须先过"轮次绑定"这道锁(2026-09-25 补,G-179)**:钩子日志是多轮追加的,而旧锁只要求"该汇总之前的窗口里出现过本次声明的**任一**文件名"—— 那段窗口实测 48KB–202KB,里头**必然**提到 `PROJECT_PLAN.md` / `AGENTS.md`(守门 71/13c 的结论行就点名它们),于是"我自己的红"可以借别人那一轮的汇总被洗成 `not-ours`(实测提交 `48ac2c03e`:红在 eslint `no-unused-vars`,lint-staged 失败使守门批根本没跑 ⇒ 152 道门被一次误归因跳掉,HEAD 里真留下那枚 lint 错误)。现规则:**该汇总之前那次 `staged 文件清单` 回显必须完整包含本次声明集**才允许用于归因 —— 回显由钩子入口 `auditStagingFiles()` 全量打印,是把一段日志绑到具体某枚提交的唯一内容证据(日志没有时间戳、也没有轮次号,实测按 `^YYYY-MM-DD` 抓零命中)。取"包含"不取"等值":回显反映磁盘实际暂存,并发会话同窗口 staged 的东西会造成超集,而它们结构上进不了本枚提交(commit 带 pathspec)。**绑定不成 ⇒ 落 `unattributed`,照旧能提交,只是措辞不得声称"因他人代码"—— 失效方向必须是"多要一次定向说明",绝不能是"多放一次跳门"**(与本节末那条 `findingLines` 教训同一条禁令)。真仓双向证明(不是夹具):合法那一轮绑定成功并取到 `总检查数: 152(已执行 152)` 的汇总,事故那一轮被拒且点名缺的 2 个文件。**stdout 没汇总时,第二输入源必须先过"轮次绑定"这道锁**(2026-09-25 补,G-179):钩子日志是多轮追加的,而旧锁只要求"该汇总块之前的窗口里出现过本次声明的**任一**文件名"—— 那段窗口实测 48KB–202KB,里头**必然**提到 `PROJECT_PLAN.md` / `AGENTS.md`(守门 71/13c 的结论行就点名它们),于是"我的红"可以借别人那一轮的汇总被洗成 `not-ours`(实测提交 `48ac2c03e`:红在 eslint `no-unused-vars`,而 lint-staged 失败使守门批根本没跑 ⇒ 152 道门被一次误归因跳掉)。现规则:该汇总块之前那次 `staged 文件清单` 回显必须**完整包含**本次声明集(回显由钩子入口 `auditStagingFiles()` 全量打印,是唯一能把一段日志绑定到具体某枚提交的信号 —— 日志没有时间戳、也没有轮次号);缺任何一项 ⇒ 判"不是本轮" ⇒ 落到 `unattributed`。**失效方向刻意是"多要一次定向说明",绝不是"多放一次跳门"**。真仓双向证明:合法那一轮 `绑定成功` 并取到 `总检查数: 152(已执行 152)` 的汇总块,事故那一轮 `拒绝` 且点名缺的 2 个文件(4 项声明 vs 4 项回显,重名 2 项 —— 正是旧锁放过的形态)。**刻意不建"机器态门 id 清单"** —— 豁免清单必然腐烂(见 §4 对 `RN_ONLY_BRAND_KEYS` 的教训)。取证:镜像测试 5/5(含装车证明 + 真实 `hook-logs/pre-commit.log` 取样校验夹具)+ 三条变异各自变红(ANSI 剥色那条登记为**等价变异**,不造假具硬凑红)。**因此:agent 不得再假设"钩子红了就能跳"** —— 被拒时那句红点名了哪个文件,就去修哪个文件;确属误判才可按 §16 显式说明后手工提交。
- **守门接线层对账**(89): `scripts/check-gate-wiring.mjs`(blocking) —— 本门是其余全部守门的"装车检",查接线层四类结构缺陷:**R1/R2** 脚本头部或 AGENTS.md 声称"已接 pre-commit/pre-push/CI 必跑/第 N 项"而五处权威点(`guardian-runner` 的 `script:` ∪ `scripts/lib/pre-commit-hook.js` ∪ `.husky/*` ∪ 根 `package.json` ∪ `.github/workflows`+cert)全部零命中 → 拦;**R4**(2026-09-24 由"仅报数"升档,前置=真仓缺口 48→0 已清零)门已接线但 AGENTS.md/README.md 通篇未点名 → 拦(判据刻意宽松:出现去后缀同名即算,故只会漏报不会误拦);**R5** 同一 `id` 登记两道门 → 拦(串 skipEnv 与失败归属);**R7** 台账 `type=dispatcher` 的依据文件不存在或文件里没提该脚本 → 拦。**R8**(2026-09-25 补)某道门的 `stagedTriggers` 只要会让归一层 `scripts/lib/guardian-triggers.mjs` **当场抛错**即拦 —— runner 的 `for (const check of effectiveChecks)` 没有 catch,一次抛错=整条 pre-commit 中止(同型崩点实测让 3 枚提交被迫 `--no-verify`,152 道门对那 3 枚全废)。**红条件委托那道 lib 的契约而不是本门自写形状规则**:自己写必然与 lib 漂移,而漂移的代价固定是"一道对合法注册恒红的 blocking 门 ⇒ 逼人跳门 ⇒ 全部守门作废"。据此三类**只分类报数不判红**:裸字符串=「已救回」(lib 有意归一,再判红就是两道门互相矛盾)、falsy 字面量=「死注册」(runner 的守卫走不到归一层,它结构上崩不了)、值不是字面量(变量/模板串)=「判不出」(不猜,也绝不静默算通过)。取材面**与 R1/R2 刻意不同**:全量档 HEAD、`--staged` 档优先索引 —— 注册表自身就是被审对象,"同一枚提交里既修注册形态又新立判据"若被自己的新判据判红,就等于本门亲手制造跳门(与 R2/R4 的 HEAD∪索引例外同一理由,两面一绿一红时才算证明过判据有牙)。R3(五处零命中且无肯定式声称)/R6(共用 skipEnv)/弱接线(仅 CI)只如实报数不判红。**取材口径分两面**:接线面(哪些脚本被登记为已接)一律按 **HEAD** 判 —— 共享工作区的工作区文件会滞后,产出相反结论;而**文档面**(AGENTS.md / README.md 有没有点名)按 **HEAD ∪ 索引**判(2026-09-24 改),因为"同一枚提交里既注册新门又补点名行"是正确姿势,只读 HEAD 会让这枚提交被自己判红 ⇒ 逼人 `--no-verify` ⇒ 全部守门作废;索引取不到退回 HEAD,口径在结论行如实报出,且**绝不读工作区**(只躺在工作区没 `git add` 的文档行不算点名,已由自检 M9d/M9e 一红一绿的可逆对照钉死)。;台账 `scripts/gate-wiring-allowlist.json` 只能救 R3,凡声称已接线者台账不救。⚠️ 本门对自身 `SELF_EXEMPT`(创建当期 HEAD 里还没有它),所以"它自己被摘线"由镜像测试 T13 钉死。真跑 `node scripts/check-gate-wiring.mjs` exit 0(已接线 143 / 台账豁免 13 / R4 0 枚,数字以命令末行现测为准);自检 `--self-test`(例数以末行为准,含 M8a/M8b 的 R4 双向端到端证明 + M9a-M9e 的文档取材口径五形态对照)+ `node --test scripts/tests/check-gate-wiring.test.mjs`(例数以末行为准,含 T13 装车证明与 T14/T15 取材对照);紧急跳过 `HUSKY_SKIP_GATE_WIRING=1`。
- **记忆端点属主绑定对账**(117,**warn**,紧急跳过 `HUSKY_SKIP_MEMORY_OWNER_BINDING=1`):`scripts/check-memory-owner-binding.mjs` 钉的是**"认证不等于授权"**那一层 —— `apps/ai-service/app/api/memory.py` 把 `user_id` 当请求体/Query 参数收,全文件**没有一处**与令牌主体对齐;`/api/memory` 不在 `PUBLIC_PATHS` ⇒ 匿名进不来,但**任何已登录用户填别人的 UUID 就能读/改/删别人的记忆**。判洞要问"有没有把请求里的 id 与令牌里的 id 比对",不是"有没有全局中间件"。两条 Python 语法锚点必须同时认:`user_id: str = Query(...)` 与 Pydantic `Field(...)` —— **只认 Query 会让整类 POST 端点隐身**;对齐出口只写在注释里不算(自称已拆、实际没拆那一型)。**定级 warn 是设计前提而非偷懒**:未对齐存量:立项读数 2 个文件;现值一律 `node scripts/check-memory-owner-binding.mjs` 自取 —— `app/api/memory.py` 已于同日 `3b0170fa176` 九端点统一 `require_request_user_id` 归零,剩下的是**此前无人枚举过的** `app/routers/rules.py`,而"怎么对齐"取决于 **v1 对外 API 能不能代表他人访问记忆** —— 实测 `apps/api/src/routes/v1-knowledge-tools.ts` 有 2 条转发是 `jsonInit(parsed.data)` **原样透传客户端自报身份**(本门把这类列成"上游透传清单"只报数,就是给选型看爆炸半径用的)。收紧成 403 与"忽略并覆盖"两种改法**都会让本门归零**,故在选型落地前它是"存量报数、新增判红"的棘轮;当场 blocking = 与任何提交都无关的恒红门,唯一结局是逼人 `--no-verify` 连带废掉全部守门(§12e 同型)。**升 blocking 的前置条件 = 未对齐存量归零**。口径同 70/77/83/98/101/103(全量判 HEAD blob、`--staged` 判索引、枚举到 0 个文件判"无法判定"而非记绿 —— 判据失明不是通过)。仓里现成出口 = `app/core/jwt_auth.py` 的 `require_request_user_id`(带 `DEV_ANONYMOUS_PRINCIPAL` 开发降级,以免 ASGI in-process 的既有测试整片 401 —— 那不构成安全结论)。取证 `--self-test` 17 例(正反成对) + §22c 镜像 14 例(含"未注册时不得被判定为已装车"的方向性对照);手动问责档 `pnpm check:memory-owner`(走 `--strict`)。

<!-- ⚠️ 本行是守门 102 的**四类判据原文**(2026-09-25 第二轮收口前),已被下方"六类判据"条就地取代:缺 GA5 混写与 GA6 双箭头两条,S0 也只登记 4 个机制。照它派单会以为 RN 共享层那份 BackChevron 有人看守。保留原文只为不丢行(AGENTS §12),勿照它执行。 -->

- **守门**:`scripts/check-glyph-arrow-icon.mjs`(guardian 第 **102** 项,blocking)六类判据:**GA1** 整格字符箭头 `›»→》‹←`(或 `{'>'}`)在可证 affordance 语境(自身或 ≤8 层祖先带 `on*Press/Click/Tap` 或 `role="button"`,或祖先为 `Link/Pressable/Touchable*/Button`)当图标 —— **左向 `‹`/`←` 于 2026-09-25 补入**(立项时只有右向四字,于是小程序 NavBar 与 7 个页面头拿 `‹` 当返回图标长期零判据);**GA2** 「更多」类标签与**同词干** `*Arrow*` 样式配对后箭头字号 > 标签字号(单位不同不判红,计入 undetermined);**GA4**(同日新增)整格子内容是「返回」类文案(`t('common.back')` 这类键名末段为 `back`/`back<数字>` 的 i18n 调用,或字面量 `返回`)且处在可证 affordance 语境 ⇒ 即"把返回两个字当返回箭头";**带宾语的标签刻意不纳**(`backHome`/`backLogin`/`prevMonth` —— 那些是按钮文案,换成裸箭头反而不表意);**GA5**(同日新增)「返回」与字形的**混写**两型:`← 返回` 整格(GA1 要整格只有字形、GA4 要整格只有文案,混排两条都不纳),以及字形与返回调用**分居同一元素的两个子节点**(`← {t('common.back')}` —— `walkAffordanceChildren` 走 `loneChildText`,多子元素的元素根本不入选 ⇒ 前三条结构上全盲);**GA6**(同日新增,跨文件判据)小程序页面渲染页内返回键(`BackChevron` 或 `<NavBar/>`)却是**原生导航栏** ⇒ 同屏两个箭头 + 双层 chrome,读 Taro 页面 config、页内未写则继承 `app.config.ts` 的全局 window,两处都取不到 ⇒ 计入「未判定」点名(不记绿也不冒红);**S0** 共享唯一实现被摘线或无人 import(防"造好没装车"),现登记 5 条:三端「更多」箭头 + 页头返回键的**两份同名实现**(小程序端内与 RN 共享层 `packages/app` —— 只登记一份时另一份被摘线无人喊红)。GA1 与 GA4 **共用同一遍遍历**(`walkAffordanceChildren`),两条对"可证 affordance"的定义必须同形 —— 各写一遍必然在栈深/祖先窗口上漂移。口径同 77/83:全量判 HEAD blob、`--staged` 判索引、**棘轮锚点 = 该文件 HEAD 自身违规数**(行尾字符箭头存量 70 处只报数不拦,免得恒红逼人绕过钩子连带废掉全部门);豁免**三条互不串门的通道**:`glyph-arrow-exempt: <原因>`(GA1/GA2,只认命中同行)、 `back-label-exempt: <原因>`(GA4/GA5,认命中行、可点元素起始行、及其紧邻上行)、**文件级** `nav-chrome-exempt: <原因>`(GA6 —— 它的"错"是页面配置 × 页面渲染的组合,不落在某一行上,逐行豁免无意义)。三个族都已登记进守门 108 的存活期表(`back-label`/`nav-chrome` 取 365 天:结构性定性而非待偿债务,短周期到期逼人删标记、删了又被判红,两道门互咬)。前两条**宽严刻意不对称**且各由自检钉住:GA1 若也按块放行,一个标记就能救整棵子树,"一行救不了别处"那条反向锁就没了;GA4 若只认同行,人按直觉把标记写在可点块上就永远不生效(初版即如此,四处真实站点全部落空 ⇒ 恒红)。**原因不得由注释闭合符冒充**(裸标记 + `*/` 曾被读成"带了原因"⇒ 整行免检;该洞自 §22c 立项起就在 GA1 里,由 GA4 的自检反手抓出,现两条通道同锁,全仓零裸标记存量故改严无债务)。取证 `--self-test` 81 例(含阳性对照、棘轮四向、预筛必须是判据字面量**超集**的对账 —— GA5 的 `«` 与 GA6 的 `BackChevron`/`NavBar`/`navigationStyle` 三个标识符少一个,门就在自己立项那一型上失明;GA6 的继承链三态、GA5-B 的"去掉判据即翻绿"存在性证明)+ 镜像测试 16 例(含"GA1/GA4 豁免宽严不得悄悄对齐"、"预筛漏字形=门对该形态全盲"、**"GA5/GA6 必须真挂在 `scan()` 上"**(守门 70/76/81 同型:函数在、自检过,但 `scan()` 没调它 = 提交链上一路绿灯)、"GA6 不得按磁盘读 config"、"新增豁免族必须进 108 存活期表")。紧急跳过 `HUSKY_SKIP_GLYPH_ARROW_ICON=1`。
- **GA1 存量已清零(2026-09-25 第三轮;取代上一行末「行尾字符箭头存量 70 处」那个读数)**:
  70 处 / 31 文件的行尾 `› » → 》` 字符箭头全部换成各端矢量 —— 小程序 53 处 → `LineIcon chevron-right`
  (尺寸统一 24rpx = §4 的小程序光学档,色一律沿用该元素原 token),web 8 处 → lucide-react 的
  `ChevronLeft`/`ChevronRight`/`ArrowRight`(跟随原字号),共享屏层 9 处 → lucide-react-native 的
  `ChevronRight`(**沿用原 fontSize,只换载体,不顺手改观感**)。随之失去使用者的样式规则一并删
  (`member-arrow`/`pf-arrow`/`vs-share-arrow`/`cd-aigc-arrow`/`card-entry-arrow`/`ask-create-picker-arrow`
  与 `itemArrow`、`modelBarArrow` 的 `fontSize`),仍被别处引用的(`py-coupon-arrow`/`income-stat-value`)保留。
  **棘轮锚点仍是「该文件 HEAD 自身违规数」** —— 存量清零不等于判据变严,判据一个字没改。
  **路上踩到两条,记为规矩:**① 补 import 不得用 `/^import /` 找「最后一行」——多行 import 的首行也匹配,
  于是新 import 被插进别人 import 的中间,当场写坏文件(eslint 报 `Parsing error: Identifier expected`);
  要按「以 `from …` 收尾的完整 import 行」定位。② 样式工厂里的局部色变量不得直接搬到组件 JSX 作用域
  (`arrowColor` 定义在 makeStyles 内、JSX 在组件内 ⇒ 改用组件作用域的 `tk.text.tertiary`),
  否则 lint 判 unused、运行时取到 undefined。
- **守门 102 的 HEAD 存量已全清 + 覆盖面换成实测(2026-09-26,终读:S0/GA1/GA2/GA4/GA5/GA6 全部 0)**。三件事值得单独留成规矩:
  ① **"全端已覆盖"不能建立在"恰好没有这种文件"上**:原 `TSX_RE = /\.tsx$/` 让 GA1/GA2/GA4/GA5 只在 `.tsx` 上判。
  实测受管 12 目录里 `.js/.html` 共 13 个、喂门自己的 `auditFile` 违规 0(命中全是注释与 config),全仓 `.jsx` 存量 0,
  CSS `content:` 用字符箭头 0 处 —— 数字上"不扩今天不漏",但仍把 `TSX_RE` 扩到 `\.(tsx|jsx)$`(`SRC_RE` 同步),
  并加自检锁"同一条违规在 `.jsx` 与 `.tsx` 上必须同判"。**为什么**:本门立项的理由就是"一条门只管自己立项那一型"
  (左向 `‹` 不在字符集里 ⇒ 9 个真站点长期零判据);一枚新 `.jsx` 就能让这句话静默作废而账面仍全绿。
  锁的有牙证明靠变异:`TSX_RE` 改回 `/\.tsx$/` ⇒ 自检 82/83 且红的正是这一条,还原后 83/83 并与备份 `diff` 逐字一致。
  ② **留在工作树里等 owner 提交的回退,必须当场证明"提交它会被拦"**:最后 8 处所在四文件被并发会话持有
  (工作树各有 19–613 行在飞改动),本票按「`git show HEAD:<path>` ⊕ 锚点精确替换(计数断言)→ 私有索引 →
  commit-tree + CAS」落地,不交工作树版本。其中 `SettingsScreen.tsx` 的工作树副本把 HEAD 的 `<BackChevron/>`
  换回「返回」两个字 —— 把那份 blob 只放进 `GIT_INDEX_FILE` 指的探针索引、按 `--staged` 喂本门 ⇒
  **exit 1 并点名 GA4**。"延后"与"放开一条回退通道"的差别,就是这条证明。
  **另:commit-tree 之后必须逐路径把主索引对齐到新 blob** —— 否则那四个路径留在"索引 == 旧 HEAD"的 `M ` 形态
  (§12d 第三层描述的形态),此后一次不带 pathspec 的普通提交就把修法写回旧版。
  ③ **GA5 那 1 处的正确处置是带理由豁免,不是矢量化**:它在 `ModelConfigDialog.tsx` 的 `RatioSelector` 内,
  实测该组件作用域里没有 `t`、也没有 `tokens`(换矢量要动组件签名)—— **不得为消红去改别人组件的 API**;
  与 `packages/app/src/components/Selecter.tsx:403` 同一处置(同为步骤回退标签,`back-label-exempt` 带原因与到期日)。
...
- **自动归档**:`scripts/archive-completed-tasks.mjs` 扫描完成 ≥7 天的条目,post-commit 钩子自动 `--auto-commit`,归档 commit 设 `IHUI_ARCHIVE_COMMIT=1` 防递归。
  - ⚠️ **本条的前半天读数已被同日晚间的实测成反向,现值以这一格为准(2026-09-25 深夜)**:链已由 `edb225c909` 打开,而**起效的载体是 `apps/miniapp-taro/src/app.css` 里的 `@source "./**/*.{ts,tsx}"`**,不是 `config/index.ts` 那行 `tailwindcss: {…}`(附⑧的归因更正)。落地后果实测:命名档 utility 已进产物、C1 类名覆盖 0.79% → **32.15%**、主包**净省 61,654 B**(旧"余量 42,401 vs utilities 42,392,开链会撑爆"这组数**方向是反的**,作废)、余量 114,379 B;`text-card` 同名双义已因改名 `.aigc-text-card` 消除。**仍然成立只剩一点**:项目**色**档(`bg-*`/`text-*`/`border-*` 含整个 `/alpha` 族)依旧 0 落地 —— v4 只认 `@theme`,而副本色值写在普通 `(:root/.dark)`,所以"R6 绿灯 ≠ 到端生效"这句对本条**不变**,色档接线是 O62 **附⑧补** 的在账项,不得读成已收口。
- ⚠️ **但 `apps/miniapp-taro` 实际跑的是 Tailwind v4.3.3,不是端内 `package.json` 声明的 v3**(2026-09-25 实测,详见 PROJECT_PLAN O62附⑤):`weapp-tailwindcss@5.2.9` 引用的是 **`@tailwindcss/postcss`**(v4 专属入口 —— v4 的 main 导出会**直接报错拒绝**被当 postcss 插件用),产物 base 层也带 v4 指纹(`border:0 solid` 合并写法、`--tw-gradient-position`,且无 v3 独有的 `--tw-bg-opacity`)。**取版本别只读 `package.json`** —— 那样会得到相反结论(本会话就错过一次:用 `resolve('tailwindcss/package.json')` 探到 `MODULE_NOT_FOUND` 就否掉了正确说法,真实依赖名是 `@tailwindcss/postcss`)。
  - ⚠️ **同日晚间顺着解析图复核,这条结论仍成立、但当时用的依据是错的,不得照它推论**:"weapp-tailwindcss 引用 `@tailwindcss/postcss`"在依赖图里**不成立** —— miniapp importer 的 weapp-tw peer 链的是 `tailwindcss@3.4.19`,而 `@tailwindcss/postcss@4.3.3` 是 **apps/web importer** 的依赖(从 miniapp 锚 resolve 得 MODULE_NOT_FOUND)。v4 的真身是 weapp-tailwindcss **自带 vendored 引擎**(其 dist 导出 `loadTailwindV4DesignSystem`)。判"这端到底跑哪个引擎"的两条可靠证据是:**产物指纹**(`--tw-leading`/`--tw-gradient-position` 在、`--tw-bg-opacity`/`--tw-text-opacity` 0)与**包内 vendored 路径**,不是"谁的 package.json 里出现过后者" —— 后者会把别的 importer 的依赖读成本端的(本仓 §5b/§15b 记过同型:"凡四类机器事实,每次按当次实测取值")。两条后果:① 上面 R7 拦的裸 rpx 错属性在 **v3 与 v4 上行为一致**(已用 v4 + 真实源码逐字复测),所以判据与那 1,341 处改写**不因版本换人而作废**;② 但"miniapp 的 `/alpha` 靠 `tailwind-alpha-plugin`"这个前提**尚未证实** —— v4 走 `color-mix` 是另一条路,插件的确定价值在 **mobile-rn(NativeWind v3)**。带真 theme 复测前,**不要把 R6 对 miniapp 那一侧的登记要求当成已证事实扩严**。
- **豁免到期账**(guardian id **108**,blocking,紧急跳过 `HUSKY_SKIP_EXEMPTION_EXPIRY=1`):`scripts/check-exemption-expiry.mjs` 钉"豁免只有出生、没有死亡"。实测全仓 **12 类**行内豁免 / 252 处 / 107 文件,**带真到期日 0 处**(这三个数是**立项读数、不是现值** —— 现值一律跑 `node scripts/check-exemption-expiry.mjs --json` 看 `totals`)。2026-09-25 补第 13 族 **`back-label-exempt`**(守门 102 GA4 的合法例外通道),在 `FAMILY_LIFETIME_DAYS` 给它 **365 天**而非同门 `glyph-arrow-exempt` 的 30:它是**结构性定性**(该位置的「返回」是按钮文案而非导航箭头),位置性质不随时间改变;短周期到期只会逼人删标记,删了又被 GA4 判红,**两道门互咬**。E1 新增不带到期日 ⇒ 红;E2 已过期仍生效 ⇒ 红;E3 lint 抑制面(`eslint-disable` / `@ts-ignore`)只报数。**三重防恒红**:存量进 `scripts/exemption-expiry-baseline.json` 只报数、E1 锚点 = 该文件该族在 **HEAD 自身的存量数**(棘轮)、`grandfatherUntil` 前不追存量 —— 与改动无关的 blocking 红只会逼人 `--no-verify` 连带废掉全部守门。到期**只判红只点名,绝不自动摘除任何人的豁免**。**记账面 = 被豁免的那一侧**:`scripts/**` 里的标记是"门在描述自己的出口"(判据正则 / 头注 / 镜像测试夹具),只报数不入账(实测 HEAD 面 372 条里 125 条在那一面);不收窄的话,**任何新增带豁免出口的门都会在自己立项那枚提交上被 E1 判红**——锚点是该文件 HEAD 自身的 0,而标记不写出来门就不工作,等于没有合法出口。这条规矩写在 `mergeBaseline` 而不是手工清一次:旧基线被一次回退带回来时,下一次 `--update-baseline` 自行收干净。副产品:第 12 族 `r3-cta-exempt` 3 处是从来没生效过的悬空豁免(守门 83 只认 `r5-cta-exempt`),归属该门持有者。
- **miniapp 原生 chrome 两份副本已收口为派生态 + 专属门**(2026-09-25 立):`apps/miniapp-taro/src/theme.json`(微信 darkmode 配置,`app.config.ts` 以 `@变量` 读取的编译期源)与 `apps/miniapp-taro/src/lib/theme.ts` 的 `THEME_CHROME`(运行期 `setNavigationBarColor`/`setTabBarStyle`)此前是手抄且**零生成器、零守门**(`grep -rl theme.json scripts/*.mjs` 立项时零命中)。现由 `scripts/sync-miniapp-chrome.mjs` 派生(原位写回、幂等;同色不同字节如 `#A3A3A3` 保留文件原字节,首跑不产伪 diff),挂进 `TOKEN_SYNC_TARGETS`(trigger `tokens`,failMode `block`),由守门 `scripts/check-miniapp-chrome.mjs`(guardian-runner **id 以 runner 现值为准** —— 本条初登 122,落地前该号被另一路的 `check-file-write-safety` 占用,按"后来者改号"顺延为 **124**;blocking,紧急跳过 `HUSKY_SKIP_MINIAPP_CHROME=1`)复核。取材面与 36/77/83/93/98/103 同:全量判 **HEAD blob**、`--staged` 判索引 blob、`--worktree` 只是人工/验生成器写回的逃生舱,两面旗同给 ⇒ exit 2;取不到判"无法判定",**不回落**另一个面。**登记分流是记录在案的决策,不是漂移**:dark `navBg`/`windowBg` = `#262626` 出自 commit `d2d80c1b23`(2026-09-06)明文「对齐 rn gray.800」,而源头 `.dark --color-background` = `hsl(0 0% 14%)` = `#242424`;该分歧登记在生成器的 `CHROME_DECLARED_DIVERGENCE` 且带 `nearToken` 机判 —— 源头一旦重新同值即判"登记表腐烂"红。**不得**把 #262626「同步」回 #242424,那是回滚那枚决策;`tabSelected` 的 #8fb8cc/#a3c4d6 即 `--color-brand-accent` 两档(2026-09-14 定稿的跨端强调色)。
- **派生面登记表必须自证自洽(守门 `scripts/check-token-sync-registry.mjs`,编号以 runner 现值为准,紧急跳过 `HUSKY_SKIP_TOKEN_SYNC_REGISTRY=1`)**:`scripts/lib/pre-commit-hook.js` 的 `TOKEN_SYNC_TARGETS` 是"改 `tokens.css` ⇒ 各端副本自动写回"的唯一登记表,每行三段 **触发文件 / 写回命令 / 复核门(`check:` 字段)**。四种腐烂过去无人看守:触发文件改名 ⇒ 该行**永不触发**(副本从此不跟随);写回脚本被删或改名 ⇒ 提交链跑到那步才崩;复核门被摘线或降级成 warn ⇒ 照样写回却没人判对错;**新加一行忘了配门 ⇒ 新派生面零覆盖** —— 最后一种最贵,表里挂着一行没人配门的行,比根本没有这行更糟,它会替人做出"已经收口了"的判断。本门按被审判的面逐行核这四条,取不到判"无法判定"。**新增一端派生副本的正确顺序**:先在 `tokens.css` 落源头 → 加生成器 → 在表里加一行(含 `check:`)→ 配那道复核门并注册进 runner ⇒ 四段同笔提交,门当场就能问出缺哪一段。修复口径是**改表与配门**,不得为消红放宽判据、更不得删表行(删行等于关掉那一端的自动同步)。
- **小程序端有第 6 个受管区 = `app.css` 的 `@theme` 色档注册区**(2026-09-25 立,`f17b5fbdda`):它不是"又一份色值表",而是 Tailwind v4 的**注册动作** —— v4 只把 `@theme` 里的 `--color-*` 认作主题档,色值若只落在普通 `:root/.dark` 上,`bg-muted`/`text-foreground` 这一整族 utility **一条都不产出**(实测真构建 0 落地)。档位仍**唯一派生自** `deriveMiniappManaged(tokens.css, 'light')` 再剔掉 `-rgb` 三元组 —— `-rgb` 进 `@theme` 会长出 `.bg-muted-rgb` 这类脏档(v4 见 `--color-*` 就当颜色档)。三条不得照抄的形态选择:**只用普通 `@theme { }`**;`@theme inline` 会把亮档值烘进每条规则 ⇒ 深色档整族失效;`@theme reference` 会给每条规则附一份亮档 fallback(更贵,且把"没定义"洗成"默默用亮档")。守门 36 现同时核 `:root` / `.dark` / `@theme` 三面(无缺档 + 逐位同值 + 反向拦 `-rgb` 脏档)。**同批判死一条判据缺陷**:`check-miniapp-css-landing` 的 C1 原拿源类名直比产物类名,而 weapp-tailwindcss 会把 **wxss 选择器**里的标点转写(`[→_b ]→_B /→_f :→_c (→_p )→_P ,→_m .→_d +→_u !→_e`,10 条全部逐字取自同一次真构建),于是 505 条"缺项"里 474 条是比对噪声。⚠️ **但"按转写表把名字认回来"这一步本身同日被我改错、已推翻(附⑩)**:转写只发生在 CSS 侧,产物 JS/WXML 侧仍是**源名**,DOM 上实测连方括号都被当空白吃掉 ⇒ 那些转写规则没有任何元素挂得上,是**死规则**不是命中。现判据是 **C4 可达性**:一个档要算"到端可用",必须**同时在 wxss 里有对应选择器、且在产物运行时 class 语料(js/wxml/wxs)里挂得上**;只在 CSS 出现的进 `deadRuleKinds` 且**不进 pct**。**但"运行时挂不上"也不是一句端常态** —— 2026-09-26 同一份 `config/index.ts` 连测三档构建,得到三种完全不同的产物形态:
  ...
  任何"可达率"读数都必须**先认档再说数**,脱离 C5/C6 的单个百分比没有意义。**三条教训叠在一起**:① 判落地的门比错形态,症状不是差一点,而是把真信号淹在噪声里;② 但在把两侧名字"对上"之前,必须先问**挂载侧到底写了什么** —— "CSS 里有这条规则"与"这个样式生效了"之间隔着一维,只读声明侧的绿灯结构上排除不了"规则存在、没人挂得上"。
- ⚠️ **上面那条 `/alpha` 判据目前到不了小程序产物**(2026-09-25 实测,登记于 PROJECT_PLAN O62附②/附③):源码用到的 **687 个 Tailwind utility / 11,105 处用法**,在真实构建(`pnpm --filter @ihui/miniapp-taro build`,exit 0)的 154 个 wxss 里 **0 个产出规则**(`.flex{` `.items-center{` `.rounded-xl{` `.bg-muted{` `.text-sm{` 逐条为 0);而 **base/preflight 层是在的**(`border:0 solid;box-sizing:border-box;margin:0;padding:0`)⇒ 症状精确到"**只有 utilities 层为空**"。阳性对照:同一份 dist 里 `app.css` 手写类名 75/98 可查到规则。**机制不是** `config/index.ts` 那行 `tailwindcss: { enable: true, config: {} }` —— 实测把它改成真实配置路径后 A/B/C 三次构建产物**逐字节相同**(该键不是开关);真因指向 weapp-tailwindcss 5.2.9 **自带一份 vendored tailwind**(`dist/tailwindcss-*.js`)并由它自己的 generator 出 CSS,项目 `tailwind.config.ts` 的 content globs 没有喂进那条链。**后果**:本条 R6 判据守的是"源码用量 ↔ 登记表 ↔ 插件产出"三者一致,这三者都在**项目侧**;小程序实际产物由第三方 generator 决定,R6 的绿灯**不等于**到端生效。改这行配置的爆炸半径见 O62附③(主包余量 42,401 B 而 utilities 约 42,392 B,以及 `text-card` 同名双义会被注入白底白字)。**⚠️ 本条"0 个产出规则"的前提已于同日作废**(2026-09-25 深夜,`edb225c909` + `f17b5fbdda`):命名档与项目色档**都已产出且运行时可达**(`.bg-muted{background-color:var(--color-muted)}` 这类),主包余量 **113,504 B**。但可达性只到命名档为止 —— **含标点的那一整族(arbitrary value / `!` important / 分数)是死规则**:weapp 只转写了 wxss 选择器(`z-[1001]` → `.z-_b1001_B`),运行时 className 仍是源名、DOM 上实测连方括号都被当成空白吃掉 ⇒ 那是**三档构建模式中的第 ① 档**(JS 转写腿整轮空转),不是端常态:同一配置另有两档分别读到 93.77% 与 99.35%。旧稿此处还误写过 96.0%(把死规则计成了命中)、后又误把 34.89% 当端常态 —— 两个方向都错过,现值一律"先认档(C5/C6)再说数"(`附⑩`/`附⑪`)。现行事实见本节末"v4 色档注册区"条与 PROJECT_PLAN O62 附⑧/附⑨/**附⑩**,勿照本行执行。
...
- **提交活文档前必须做"工作树 ⊇ HEAD"行级对账(2026-09-24 立,一夜三次自伤换来的)**:`README.md` / `AGENTS.md` / `PROJECT_PLAN.md` 这类多会话共写的文件,工作树副本**常年滞后于 HEAD**(实测分别少 57 / 48 / 54 行),而 `safe-commit` 的 Step ④ 是 `git commit -- <pathspec>` —— 按路径取**工作树**版本,于是"只 add 本任务文件"的规范提交照样把别人已入库的行整批写回旧态,`git status`、diff 行数、typecheck、守门 71 全都看不出来。动作:① 提交前 `node scripts/merge-live-doc.mjs --file <该文档>`(exit 1 = 存在真丢失);② 需归并时 `--apply`(它按锚点把 HEAD 缺失块插回工作树,并区分"被吃掉"与"被就地改写",后者不插回以免新老并存);③ 归并后必须报"仍判 lost = 0 且长行重复新增 = 0"。**但提交前的报告有时效边界**(2026-09-24 实测:`merge-live-doc` 报 0 丢失之后、`safe-commit` 落地之前,并发会话又推进了一轮 ⇒ 那次提交仍丢掉 2 条他人登记行)。所以顺序必须是四步:**报告 → 提交 → 立刻 `git show <新提交>^..<新提交>` 做行级对账 → 有丢失就地回补**(发现手段只能是提交后的 diff,`git status` 与提交前那次报告都看不见)。判据是**字符二元组** Jaccard ≥ 0.6 —— 前缀与按空格切词在中文里都会漏(工具自己栽过一次,README 里一度同时留下 `**第 93 项 X**` 与 `**守门 X**` 两行),已由 `--self-test` 8 例钉死。登记条目见 PLAN 的 O46④/O46⑩。**⚠️ 但"`lost=0`"从来不等于"工作树 ⊇ HEAD"(2026-09-25 实测并修工具)**:superseded 这一类的定义恰恰是"HEAD 的那一行文字不在工作树里",所以工具在 `lost=0` 时印的 `✅ 工作树 ⊇ HEAD,可安全提交` 是一句**做不到的承诺**,而本会话就在那句绿灯下把 20 条别人已翻勾/已追加注记的登记行提交成了旧形态。现分三态:`lost`(插回)/ `superseded`(有人改写,不插回)/ **`stale`(工作树停在旧形态:剥掉勾选状态后 WT 是 HEAD 的严格前缀 + 唯一候选 + HEAD 已勾 ⇒ 也不插回,但报告必须喊"这几行会被退回旧态,要人工取 HEAD 形态")**。为什么 `stale` 不自动回退:实测近 60 枚提交里有 **241** 次"合法缩短"(其中 **73** 次根本不带勾选状态变化),它们与"副本滞后"在内容上同形 —— 自动回退等于连人真删的字一起复原,那比原病更响。唯一性是生命线(两条同前缀候选 ⇒ 不猜)。工具报告现在只在**逐字包含全部 HEAD 行**时才声称 ⊇,否则按三态报数并给出出口;取证 `--self-test` 16→20 例 + 镜像 3→4 例(镜像 A 例的断言方向已反过来:有非逐字存活行时**不得**出现"⊇ HEAD"字样)。
- **钩子失败的归因是量出来的,不是抄来的(2026-09-25 立,`scripts/lib/commit-gate-attribution.mjs`)**:旧版首次 commit 一失败就打印「按用户规则"hook 失败因其他 agent 代码 → --no-verify 重试"」并整批跳门 —— 它**从未计算过归因**。实测一轮 134 道门 111 通过 / 1 失败,红的是 `check-push-sync`(判远端态、与提交内容无关),措辞仍是"他人代码"。现流程:解析 runner 的失败门清单 → **逐道复跑** → 该门这次的输出**点名本次声明的文件 ⇒ 判"本任务自己的红",拒绝 `--no-verify` 并 exit 1**(必须修);一个都没点名 ⇒ 允许跳,但只说量到的话;批没跑完 / 解析不到 / 复跑全不可用 ⇒ 明写「未归因」,按应急路径落地并留痕于 `.workbuddy/safe-commit-attestation.jsonl`。**红在守门批「之外」也要点名,不得把已量到的批内结论丢掉(2026-09-25 补,G-185)**:钩子在 runner **之后**还跑若干独立 blocking 步(实测 `i18n 死 key 扫描`),它一红就 `exit 1` —— 那一轮 154 道门**全部跑完、blocking 失败 0**,而旧归因把这种组合写成「汇总里没列出失败门 ⇒ 归因未计算」,随后照样 `--no-verify` 落地(提交 `46a4c18ed` 即此型),等于**把已经拿到的守门结论整块丢弃**,账面读起来像"没跑守门"。现规则:解析到批外步骤名(`outsideBatchStep`)后仍按同一条铰链分流 —— 该步结论点名本次声明的文件 ⇒ 判 `mine`(拒绝跳门,必须修);一个都没点名 ⇒ 判 `not-ours` 但措辞必须是「批内 0 失败,红在批外这一步」并点名是哪一步、提醒随后清偿。**失效方向仍是"多要一次定向说明"**,不是"多放一次跳门",也不是"干脆不落地"。取证:C0/C0b/C1/C2/C3 五例(含"批内确有失败门时不得走这一支"的反向对照,否则批外步骤会替批内那道门洗责)。**刻意不建"机器态门 id 清单"** —— 豁免清单必然腐烂(见 §4 对 `RN_ONLY_BRAND_KEYS` 的教训)。 —— 豁免清单必然腐烂(见 §4 对 `RN_ONLY_BRAND_KEYS` 的教训)。取证:镜像测试 5/5(含装车证明 + 真实 `hook-logs/pre-commit.log` 取样校验夹具)+ 三条变异各自变红(ANSI 剥色那条登记为**等价变异**,不造假具硬凑红)。**因此:agent 不得再假设"钩子红了就能跳"** —— 被拒时那句红点名了哪个文件,就去修哪个文件;确属误判才可按 §16 显式说明后手工提交。
...
- **钩子失败的归因是量出来的,不是抄来的(2026-09-25 立,`scripts/lib/commit-gate-attribution.mjs`)**:旧版首次 commit 一失败就打印「按用户规则"hook 失败因其他 agent 代码 → --no-verify 重试"」并整批跳门 —— 它**从未计算过归因**。实测一轮 134 道门 111 通过 / 1 失败,红的是 `check-push-sync`(判远端态、与提交内容无关),措辞仍是"他人代码"。现流程:解析 runner 的失败门清单 → **逐道复跑** → 该门这次的输出**点名本次声明的文件 ⇒ 判"本任务自己的红",拒绝 `--no-verify` 并 exit 1**(必须修);一个都没点名 ⇒ 允许跳,但只说量到的话;批没跑完 / 解析不到 / 复跑全不可用 ⇒ 明写「未归因」,按应急路径落地并留痕于 `.workbuddy/safe-commit-attestation.jsonl`。**stdout 没汇总而要去钩子日志取第二输入源时,该汇总块必须先过"轮次绑定"这道锁(2026-09-25 补,G-179)**:钩子日志是多轮追加的,而旧锁只要求"该汇总之前的窗口里出现过本次声明的**任一**文件名"—— 那段窗口实测 48KB–202KB,里头**必然**提到 `PROJECT_PLAN.md` / `AGENTS.md`(守门 71/13c 的结论行就点名它们),于是"我自己的红"可以借别人那一轮的汇总被洗成 `not-ours`(实测提交 `48ac2c03e`:红在 eslint `no-unused-vars`,lint-staged 失败使守门批根本没跑 ⇒ 152 道门被一次误归因跳掉,HEAD 里真留下那枚 lint 错误)。现规则:**该汇总之前那次 `staged 文件清单` 回显必须完整包含本次声明集**才允许用于归因 —— 回显由钩子入口 `auditStagingFiles()` 全量打印,是把一段日志绑到具体某枚提交的唯一内容证据(日志没有时间戳、也没有轮次号,实测按 `^YYYY-MM-DD` 抓零命中)。取"包含"不取"等值":回显反映磁盘实际暂存,并发会话同窗口 staged 的东西会造成超集,而它们结构上进不了本枚提交(commit 带 pathspec)。**绑定不成 ⇒ 落 `unattributed`,照旧能提交,只是措辞不得声称"因他人代码"—— 失效方向必须是"多要一次定向说明",绝不能是"多放一次跳门"**(与本节末那条 `findingLines` 教训同一条禁令)。真仓双向证明(不是夹具):合法那一轮绑定成功并取到 `总检查数: 152(已执行 152)` 的汇总,事故那一轮被拒且点名缺的 2 个文件。**stdout 没汇总时,第二输入源必须先过"轮次绑定"这道锁**(2026-09-25 补,G-179):钩子日志是多轮追加的,而旧锁只要求"该汇总块之前的窗口里出现过本次声明的**任一**文件名"—— 那段窗口实测 48KB–202KB,里头**必然**提到 `PROJECT_PLAN.md` / `AGENTS.md`(守门 71/13c 的结论行就点名它们),于是"我的红"可以借别人那一轮的汇总被洗成 `not-ours`(实测提交 `48ac2c03e`:红在 eslint `no-unused-vars`,而 lint-staged 失败使守门批根本没跑 ⇒ 152 道门被一次误归因跳掉)。现规则:该汇总块之前那次 `staged 文件清单` 回显必须**完整包含**本次声明集(回显由钩子入口 `auditStagingFiles()` 全量打印,是唯一能把一段日志绑定到具体某枚提交的信号 —— 日志没有时间戳、也没有轮次号);缺任何一项 ⇒ 判"不是本轮" ⇒ 落到 `unattributed`。**失效方向刻意是"多要一次定向说明",绝不是"多放一次跳门"**。真仓双向证明:合法那一轮 `绑定成功` 并取到 `总检查数: 152(已执行 152)` 的汇总块,事故那一轮 `拒绝` 且点名缺的 2 个文件(4 项声明 vs 4 项回显,重名 2 项 —— 正是旧锁放过的形态)。**刻意不建"机器态门 id 清单"** —— 豁免清单必然腐烂(见 §4 对 `RN_ONLY_BRAND_KEYS` 的教训)。取证:镜像测试 5/5(含装车证明 + 真实 `hook-logs/pre-commit.log` 取样校验夹具)+ 三条变异各自变红(ANSI 剥色那条登记为**等价变异**,不造假具硬凑红)。**因此:agent 不得再假设"钩子红了就能跳"** —— 被拒时那句红点名了哪个文件,就去修哪个文件;确属误判才可按 §16 显式说明后手工提交。
...
- **门脚本取材面纪律对账**(118):`scripts/check-gate-face-discipline.mjs`(blocking,`stagedTriggers=scripts/`,2026-09-25 立) —— **门数与分档读数一律现读,本文不钉数字**(跑 `node scripts/check-gate-face-discipline.mjs --json` 看 `counts`;2026-09-26 那次实测是 face 28 / loose 74(含半接线 1)/ unknown 35 / no-content 49 —— 写死它下次收紧就变成假账)。按磁盘判的门的结构性缺陷:共享工作树常年滞后 HEAD ⇒ 同一份 HEAD 代码在"恒红"与"假绿"之间来回跳,并把错数写回棘轮基线(守门 83 的 R3 登记一天内被整文件回退三次即此型)。**判据锚在"本次改动动过的门"上**(结构棘轮,不留会腐烂的白名单),全量档只报数 —— 一次性把 74 道判红就是逼各会话合法 `--no-verify`、连带废掉全部守门。**2026-09-26 收紧:"引了 `face-reader`" 不再构成合规** —— 旧判序第一刀是 `FACE_IMPORT_RE` 命中即判 `face`,于是"import 了层、内容却仍由自己 `git show` / 磁盘 `readFileSync` 取"这种**半接线**被系统性放过。实证不是假想:门 36 与门 124 都 import 了 `face-reader`(只用 `gitRaw`),却都**默认按磁盘判**;把两者改成 `catBatch` 判 HEAD 之后,本门分类读数一字未变 —— 一道自称守取材面纪律的门,对它立项要防的那一型完全失明。现判序:① 真调用层的**读取入口**(`catBatch` / `readWorktreeFile`,含 `as` 别名与 `import * as ns` 命名空间形态)⇒ `face`;② 引了层却没用它读内容、而内容来自自派生 git 或磁盘 ⇒ **`half-wired`(提交档判红)**;③ 其余照旧 `loose-git` / `loose-fs`。**只有非读取导出不算**:`selectFace` / `gitBinary` / `gitErrText` / `catBatchOids` / `catBatchSizes` 都拿不到正文,把它们当读取凭证就等于没收紧。反向护栏同等重要:别名与多行导入必须认(否则新判据对合法写法产假阳,`T10` 钉死);`usesLayerRead` 导出给镜像用,**禁止**在测试里再抄一份判据(§22c)。两层遮噪方向不同,别混用:认 face-reader 导入与 git 动词必须"只遮注释、保留字符串"(模块说明符和 `cat-file` 本身就是字符串),认 `readFileSync` 调用必须"注释与字符串都遮"(`markHidden` 那一档连字符串一起抹,直接套它会让 F1/F2 双双假绿)。取不到 ⇒ exit 2,全量面枚举到 0 道门判死。自检与镜像例数以命令末行为准(本枚落地时 24 例 / 镜像 11 例,含 F16b 与 T9–T11 的收紧锁);紧急跳过 `HUSKY_SKIP_GATE_FACE_DISCIPLINE=1`。
  ...
  - **13c 自身的取材口径(同日迁移)**:此前 `ROOT = process.cwd()` + 裸 `git show` 用 `execSync` 拼字符串(无 timeout)+ 占位合规性看 `git diff` 文本,三处都与本仓现行口径相反(工作树滞后即换结论 / 服务账户 PATH 不通 / 别人同时改这文件时合规性跟着盘走)。现全部经 `scripts/lib/face-reader.mjs`,对照面是 `--staged`:HEAD→索引、全量:HEAD^→HEAD、`--worktree` 仅人工;整面取不到 ⇒ **exit 2 无法判定**,不冒红也不记绿。取证 `--self-test` 10 条,含两条本门自己踩出来的:CRLF/LF 同一条标题不得读成"删除";**A2 的归属必须看该目录在审面上的全集**而不是只看 `PROJECT_PLAN_*` 形状 —— 本门第一次自跑就是被这条假红咬到的(已跟踪的 `orphan-capabilities-equivalence-2026-09-24.md` 被点名"落空")。
- **宿主级提示块与第三方内容的边界唯一出口**(强制,2026-09-26 立,第九轮 ZCode 吸收 A9D-1/A9D-4):CLI 侧一切"以宿主名义进提示"的块必须经 `apps/cli/src/utils/prompt-boundary.ts` —— `frameSystemReminder`(带 `kind` 的结构块,kind 是封闭集、空正文拒发)/ `neutralizeBoundaries`(非宿主内容进提示前必须过)/ `buildSkillPromptSection`(技能名单条与总量两道字节预算)。**`[系统提醒]` 这类裸文本前缀不得再被任何产出点复用**:前缀是纯文本,工具输出里出现同样字样时模型无从分辨"宿主在说话"与"第三方内容在冒充",这是结构缺陷不是措辞偏好(反第二真相由 `apps/cli/tests/prompt-boundary.test.ts` 末组钉死:源码面除唯一出口外不得再出现该字样)。第三方内容(技能正文/工具输出/fs 事件)**被省略必须在输出里留计数行** —— 静默变短等于伪造完整性,与 §5e"失败必须响"、守门 77"绝不静默成看起来全绿"是同一条禁令。不得为过测试去放宽中和判据或把预算调到 0。
- **shared 包非 Node 宿主纯度对账**(guardian id **126**,blocking,紧急跳过 `HUSKY_SKIP_SHARED_NONDE_PURITY=1`):`scripts/check-shared-nonde-node-purity.mjs` 判的是"**本地 typecheck 全绿、端上构建才炸**"那一族(与守门 72/78 同型):`packages/shared` 被 web / 小程序 / RN / `packages/app` 这些**非 Node 宿主**共同消费,而入口闭包可达面内一旦出现 `node:fs`/`node:dns` 等内建导入,`tsc` 结构上看不见(它走 tsconfig paths,不看打包器)。判据按**可达性**而非"文件里有没有 `node:`":从 `src/index.ts` + `package.json` 的**非通配** exports 做值边传递闭包(`import type` 不计,内联 `export { type X }` 保守算值边),闭包内出现内建导入即红并打印链路;**宿主直接 import 那条子路径**(绕过 barrel 的第二个入口)同样判红 —— 那正是"barrel 没导出所以巧合安全"会被一次顺手改动打破的路。三态如实:「含内建但闭包不可达」与「动态拼接/解析不到」**只报数且必须打印**(否则读报告的人会以为 shared 是纯的),取不到判"无法判定" exit 2,枚举到 0 个 shared 候选或 0 个宿主候选判死不记绿。修法唯一:把平台特有依赖改成 adapter 注入或移出共享面(§3 工厂模式),**不得**为消红放宽判据、也不得把可达文件改判成不可达;现状实测 HEAD 面不可达 1 处(`packages/shared/src/utils/ssrf-guard.ts` 的 `node:dns`,属待处置源码面)。
- **跨语言出站路由声明对账**(guardian id **127**,blocking,紧急跳过 `HUSKY_SKIP_DECLARED_OUTBOUND_ROUTES=1`):`scripts/check-declared-outbound-routes.mjs` 钉"代码里写死了要打的自家路由,而那条路由两侧都不存在" —— 声明面(Python hub 路径表 + TS `egress-call` 字面量)对注册面(Fastify 沿 `register(prefix)` 图走通 + FastAPI `include_router`/`APIRouter(prefix=)` 拼接)按归一路径段集合比对,参数段 `{x}`/`:x`/模板插值同视。**三态是这门的全部价值**:命中 / 未匹配 / **不透明前缀下的未判定**(子树挂载、同文件 const 插件、`app.mount` 一律承认看不见,不得把"看不见"洗成"确信没有")。默认档只报数、`--strict` 才判红 —— 实测 12 条存量(6 条 hub 死支柱 + 6 条 api 自转发)若当场 blocking 就是一台与任何提交都无关的恒红门,唯一结局是逼人 `--no-verify` 连带废掉全部守门(§12e 同型);问责入口 `pnpm check:outbound-routes`。豁免 `route-declare-exempt: <原因>` 只救本行且须带原因,**禁止**用基线文件消红。口径同 70/77/83/98/101/103/126(全量判 HEAD blob、`--staged` 判索引、两面旗同给 exit 2、注册集为 0 判死不记绿)。取证 `--self-test` 51 例 + §22c 镜像 6 例(含 T1"接线成套性:已接则必须 blocking + skipEnv 齐备"、T4"无提交判死"、T6 取材面形状锁)。
- **工具名本地化**(55/56):check-tool-name-display-coverage(注册表 `_TOOLS` × 词表 × 五语言 taskStatus 三方比对,blocking,拦"新增工具不补功能名");check-tool-display-resolvable(91 个功能名在 shared + 5 端合并视图 + 小程序离线包逐语言解析,拦两类静默失败:端内取词缺键回显 `toolReadFile`、忘跑 `pnpm gen:i18n` 致离线包过期;`node --test scripts/tests/check-tool-display-resolvable.test.mjs` 自检)。紧急跳过 `HUSKY_SKIP_TOOL_NAME_COVERAGE=1` / `HUSKY_SKIP_TOOL_DISPLAY_RESOLVABLE=1` **判定面 2026-09-26 收口(同 36/124/93 口径)**:默认 **HEAD blob** / `--staged` **索引 blob** / `--worktree` 逃生舱 / 两面旗同给 exit 2 / 取不到 ⇒ exit 2 **不回落**;34 个输入在同一次 `cat-file --batch` 里**同面同轮**读满(旧写法把清单来自磁盘、内容来自 `git show` 混着取,并发会话推进的瞬间会产出自洽却错位的尺子)。`--self-test` 也改跑在被审面上。**反向锁的去处**:本门此前用"只报数"冒充"扫过了",现在未判定与通过是两件事 —— 缺语言包等 4 个必需输入取不到即判死。镜像测试新增 `scripts/tests/check-tool-display-resolvable.test.mjs` 的 F1–F4(四态 / 临时仓索引≠磁盘 / 形状锁),并把原"装车证明"从磁盘面重指到 HEAD 面(断言本体一字未改)。
- **i18n JSON 同层重复键守门**(guardian 条目 `2e-dupns`,blocking,紧急跳过 `HUSKY_SKIP_I18N_DUP_NS=1`): `scripts/check-i18n-duplicate-namespaces.mjs` —— 字符级深度栈扫 `packages/i18n/messages/**/*.json` 找同一对象层重复键(JSON.parse last-wins 静默遮蔽),发现即 exit 1。**判定面 2026-09-26 收口**:默认判 **HEAD blob**、`--staged` 判**索引 blob**、`--worktree` 只作人工/夹具逃生舱、两面旗同给 exit 2、任一面取不到 ⇒ **exit 2 无法判定且不回落**;清单(`ls-tree` / `ls-files`)与正文(**一次** `cat-file --batch`)**同面同轮**;枚举到 0 个 `.json` **判死而不记绿**。收口前的真实代价已量化:它按磁盘判而 runner 一律追加 `--staged`(它不认识),于是并行会话在 `packages/i18n/messages/**` 里的半编辑语料**把当天每一次提交都钉红**(实测一轮 4 次 `--no-verify` 归因里都有它)—— 一次绕过约等于全部 156 道门对该提交作废(§12e 同型)。同批修掉 `const ROOT = process.cwd()`(扫哪棵树由调用者站哪决定 ⇒ 守门 70 的镜像测试 13/14 恒红那一型),ROOT 改由脚本自身位置推导;`--root <dir>` 保留为**测试通道**且**只在 `--worktree` 档有效**(换根却仍按 HEAD/索引读 = 双根分裂,直接判死)。镜像 7 例:R1/R2 定根、R3 通道三态、R4 形状锁(收口后方向反转:必须引 `face-reader` 且必须出现 `catBatch(`,否则视为退化)、R5 面四态纯函数、**R6 同一棵临时仓三面三答**(HEAD 干净 / 索引脏 / 磁盘脏,且索引摘除后不得借 HEAD 内容凑数)、R6b 空目录必须给可诊断结论、禁打"✅ 无重复"。
...
> ⚠️ **上面这段里两个数在 2026-09-26 的三路并行审计中被测错,现值以这一格为准**:渲染位「返回」是 **47 处**
> (不是 49 —— 多出的 2 条,一条是 JSDoc 里逐字引用了 JSX,一条是 `ariaLabel=` 驼峰属性不被 `aria-`/`label=` 滤网命中);
> `‹` 字符是 **2 行**(不是 10;`{'‹'}` 在 HEAD 面 0 处,第 3 处只存在于被并发会话污染的工作树里)。
> 并且"GA4 = 0"要带一条限定:门 102 现有 **1 处自报盲区**(`--all` 点名
> `packages/app/src/features/course-screen/CourseScreen.tsx:113`)—— 跨行自闭合标签会让该处之后的遍历栈失配。
> **所以那个 0 读作"没有已判定的违规",不读作"这一族文件都被看过"**;盲区现在由 `backBlind` 探针点名,不再静默。
...
- **对账**:`scripts/check-cross-end-ui-parity.mjs`(guardian 第 **128** 项,blocking,紧急跳过 `HUSKY_SKIP_CROSS_END_UI_PARITY=1`)。同名配对组件的单位归一后比几何档集合(`72rpx` ≡ `36px` ≡ `36dp`;Tailwind `h-9` = 36、`text-sm` = 14),差档即点名;**棘轮锚点 = `scripts/cross-end-ui-parity-baseline.json`**(立项读数:19 对同名组件 / 18 对有差异 / 174 处差异档),所以它只拦"把两端差异加大",不会把存量债变成人人跳门(§12e)。圆角与色档**不在本门射程**(守门 77 / 93 各管一段,一台尺子只量一段)。
- **差异必须显式登记**:确属平台导致的(原生 chrome、键盘避让、系统字号缩放路径不同)写进台账 `waivers.<组件>.reason` 并由守门 108 管到期;**静默不同形**与**为消红给单端补数字**都算违规。
- **不得做的事**:不得为让本门变绿去改判据或调台账数字;不得把"两份实现"读成"两份真相"。

- **可见真值仍只有一处**:色档 `packages/design-tokens/src/styles/tokens.css`、圆角 `packages/design-tokens/src/radius.js`、**几何档 `packages/design-tokens/src/geometry.js`**(2026-09-26 O81 票② 立:`GEOMETRY_PX` 一张表 → `rnGeometry`(dp) / `taroGeometry`(已折 2 倍 rpx) 两个投影,与 `radius.js` 同形)。两端的 `global.css` / `app.css` / `rn-tokens.ts` 一律是**派生态**(见本节"端内 CSS/色值副本一律是派生态"条),**禁止手改、禁止在端内抄第二个数字**。命中区/图标墨迹这类尺寸**不得**再写 `const *_SIZE = <数字>` 当端内既定档 —— 两份"唯一实现"的返回键就是那样各差 2px 的。
- **守门**:`scripts/check-prompt-injection-registry.mjs`(guardian 第 **129** 项,blocking,紧急跳过 `HUSKY_SKIP_PROMPT_INJECTION_REGISTRY=1`;手动入口 `pnpm check:prompt-injection`)。三条判据,零豁免清单(判据输入全部由被审面自身推导):**R1** 每条登记的 `producer` 文件必须在**被审面**上存在、其代码面出现该 id、且有一处记账出口调用(注释里的提及不算装车);**R2** `renderInjectionNotice(` 在生产面(非测试)至少一个调用点,且登记表与唯一出口在面上解析得到、导出仍在位;**R3** 出现 `[系统提醒]` / `[系统提示]` 这类裸宿主前缀而该文件既不调用也不定义唯一出口 ⇒ 违规,**锚点 = 该文件 HEAD 自身的违规数**(棘轮,只拦新增;把存量当场判红就是一台与任何提交无关的恒红门,唯一结局是逼人 `--no-verify` 连带废掉全部守门,§12e 同型)。口径同 70/77/83/98/103/118:全量判 **HEAD blob**、`--staged` 判**索引 blob**、`--worktree` 仅人工逃生舱、两面旗同给 exit 2、取不到判"无法判定"、枚举到 0 个候选判死。**接线史(两型同体,值得记住)**:该门立项时只有 `package.json` 一个脚本、**不在提交链**,而头注自称"guardian 第 128 项"与 runner 现值不符(128 归 `check-cross-end-ui-parity`)—— "造好没装车"与"编号谎报"同时存在,由守门 89 的 R4(已接线而文档通篇不点名)暴露;2026-09-26 按空闲号 129 补装进 `scripts/guardian-runner.mjs`,并在本节点名。**编号一律以 runner 现值为准,不得照文档派单;头注与 runner 不一致时,以 runner 为真值去改头注,而不是让后人按谎报的编号找门。**

- **parity 绿不等于内容语种对(2026-09-26 立,由 D20 实测逼出)**:上面五门判的是**键集**与"**有没有**中文残留",所以"某一块整体落成另一门语言"这一型结构上看不见。实测抓到三种形态,全部门禁当时都是绿的:① `aiChat.org` 的 **ja 装了繁体、zh-TW 装了谚文**;② `chat.exportMenu.exportPdf` 是 **ja←zh-TW←ko←ja 的三向轮转**(判据必须按**码位**量,按字形肉眼判会把谚文 `을 익스포트` 与假名 `をエクスポート` 看成同一种东西);③ `aiChat.toast.orgSaved` 被机械写成 `orgSaved:{orgSaved:"…"}` **自套一层**,`t('toast.orgSaved')` 取到对象 ⇒ 功能成功但提示永远不响。**批量落 i18n 后必须做两件事**:逐语种量"这块内容是不是这门语言"(CJK 三族互判),以及全量扫"同名自套一层"(`{"x":{"x":…}}`)—— 后一型现仓 0 处,新增即应判红。补正式门禁另计一票(现无门判这两型),在此之前不得把 parity 通过当成语种通过。
- **RN 函数形态 style 被 cssInterop 吃掉**(guardian id **131**,blocking,`stagedTriggers=apps/mobile-rn/ + packages/app/`,紧急跳过 `HUSKY_SKIP_RN_INTEROP_FN_STYLE=1`):`scripts/check-rn-interop-fn-style.mjs` 拦"函数形态 `style={({pressed}) => …}` 落在被 `cssInterop` 注册过的 RN 组件上"。真机 A/B 定案(PROJECT_PLAN「O83 登记④」):interop 对非数组声明执行 `{ ...declaration }`,而 `{ ...函数 }` === `{}`,且 `applyStyles` 先把 `state.props` 清成 `{}` 再合并 ⇒ **该元素整份内联 style 静默消失**,退回默认 column + stretch(实测把「更多」折成两行)。两个设计点:① **注册表现读**,不维护手工清单 —— 每次从 `node_modules/react-native-css-interop/…/components.js` 抽 `cssInterop(...)` 组件名,读不到判"未判定"(内置兜底表只用于"包没装时仍能报数",且表里有现读没有的名字 ⇒ 判"清单腐烂"红);反直觉事实:**`Modal` 不在注册表里**,`StatusBar` 在。正则必须认 CJS 转译形态 `(0, api_1.cssInterop)(react_native_1.Pressable, …)` —— 按 ESM 直写形态写的正则在真包上恒空,而恒空表现为"什么都没注册",门对整型缺陷全盲却一路报绿(§22c"判据失效的表现永远是安静"同型)。② 棘轮锚点 = **该文件 HEAD 自身存量数**(与 77/83/98/102/113 同族):立项实测存量 118 处 / 58 文件,当场判红就是恒红门(§12e)。行内出口 `interop-style-exempt: <原因>`(**必须带原因**,裸标记不放行;已进守门 108 存活期表 30 天)。回溯不到 JSX 开标签的命中计"未判定"并报告,**不静默丢弃**。口径同 70/77/83/98/101/103/118(全量判 HEAD blob、`--staged` 判索引 blob、两面旗同给 exit 2、取不到 exit 2)。取证 `--self-test` 11 条 + §22c 镜像 9 例(T1 装车 / T2 摘线不得被读成已装车 / T4 取材面形状锁 / T5 真仓阳性对照"看不见存量不算通过" / T6 注册表现读且兜底表是其子集)。
- **RN 函数形态 style 被 cssInterop 吃掉**(guardian id **131**,blocking,`stagedTriggers=apps/mobile-rn/ + packages/app/`,紧急跳过 `HUSKY_SKIP_RN_INTEROP_FN_STYLE=1`):`scripts/check-rn-interop-fn-style.mjs` 拦"函数形态 `style={({pressed}) => …}` 落在被 `cssInterop` 注册过的 RN 组件上"。真机 A/B 定案(PROJECT_PLAN「O83 登记④」):interop 对非数组声明执行 `{ ...declaration }`,而 `{ ...函数 }` === `{}`,且 `applyStyles` 先把 `state.props` 清成 `{}` 再合并 ⇒ **该元素整份内联 style 静默消失**,退回默认 column + stretch(实测把「更多」折成两行)。两个设计点:① **注册表现读**,不维护手工清单 —— 每次从 `node_modules/react-native-css-interop/…/components.js` 抽 `cssInterop(...)` 组件名,读不到判"未判定"(内置兜底表只用于"包没装时仍能报数",且表里有现读没有的名字 ⇒ 判"清单腐烂"红);反直觉事实:**`Modal` 不在注册表里**,`StatusBar` 在。正则必须认 CJS 转译形态 `(0, api_1.cssInterop)(react_native_1.Pressable, …)` —— 按 ESM 直写形态写的正则在真包上恒空,而恒空表现为"什么都没注册",门对整型缺陷全盲却一路报绿(§22c"判据失效的表现永远是安静"同型)。② 棘轮锚点 = **该文件 HEAD 自身存量数**(与 77/83/98/102/113 同族):立项实测存量 118 处 / 58 文件,当场判红就是恒红门(§12e)。三站点已清(见 PROJECT_PLAN「G-211 更正」),现值一律跑本门读末行,不得引用文档数字。行内出口 `interop-style-exempt: <原因>`(**必须带原因**,裸标记不放行;已进守门 108 存活期表 30 天)。回溯不到 JSX 开标签的命中计"未判定"并报告,**不静默丢弃**。口径同 70/77/83/98/101/103/118(全量判 HEAD blob、`--staged` 判索引 blob、两面旗同给 exit 2、取不到 exit 2)。**判据面先剥注释与字符串**(等长遮罩,行号不变 —— 本门按行回溯 JSX 开标签,删字符会错位;豁免标记写在注释里,所以判据看遮罩面而豁免看原文):本门第一次自跑就是被自己写的说明咬到的 —— 给 CategoryInlineBar 补一句"早先这里写 style=函数形态"之后,`--worktree` 读数从预期的 115 变成 116,**门把解释自己的散文判成了违规**(守门 70 的 URL 假注释态同型)。取证 `--self-test` 14 条(新增"注释里的该形态不得计入"+"同一形态换成真代码必须命中"成对) + §22c 镜像 10 例 —— 其中 **T10 是端到端双向锁**:往临时索引里注入一处真代码函数形态 ⇒ 必红,把同一形态只写进注释 ⇒ 必绿,两条同时成立才算"遮罩关掉的是误报、不是判据"(只留后者就是替"门瞎了"背书)。它的牙是量出来的:把判据面从遮罩面改回原文面(`RE.test(line)`),T10 的 B 臂当场翻红,红语即"注释形态不得判红"(T1 装车 / T2 摘线不得被读成已装车 / T4 取材面形状锁 / T5 真仓阳性对照"看不见存量不算通过" / T6 注册表现读且兜底表是其子集)。
  1. **派单口径只认一个入口**:`node scripts/plan-tasks.mjs --open`(未勾选 − 同题已完成副本 − 自带作废声明的行 − 同一件事的第二条待办副本;五条判据 F1/F2/F3/F4/F6 都以"编号 + 标题前缀逐字等值"的复合主键为准,不依赖行号顺序也不依赖相似度)。`grep '^- \[ \]'` 不是待办清单(2026-09-26 一次实测的**不重叠**分解:未勾选 205 = 已认领 44 + 其余排除 28 + 真待办 133;同刻 F1/F2/F3/F4 读数均为 0 —— **这些数是当次现读,派单前必须重跑,勿照抄**;报告里其余排除项的明细计数彼此可重叠,只能当诊断看,不能拿来减法核账)。归并出口:`node scripts/plan-tasks-merge.mjs --heal --commit`(F1/F2 翻勾并写明归并到谁;**F4 只加"重复登记副本"指针、绝不动勾选** —— 两条都是没做完的事,把没做的记成做过的比原病更响)。
     - **F6 是"块"级量纲,行级四条判不到它**:一整块多行登记被并发 union 追加两遍时,每一行都"只是又一个孪生行",F1–F4 与门 71 同时全绿(本会话 2026-09-26 真实自伤:探针登记块从 1 份涨到 3 份而账面什么都没喊)。判据 `findDupBlocks` 只认 **≥3 行且每行 ≥40 字符的连续 bullet 块逐字相同**(低于阈值会把台账里天然成对的短行全数成百地报出来,噪声淹掉信号);修复出口 `node scripts/plan-tasks-merge.mjs --dedupe-blocks`(先不带 `--commit` 出报告,确认四条零损失断言后再 `--commit`),它**只删第 2..N 份逐字相同的副本**,漂移副本(首行同而正文不同)一份都不动 —— 机器折半即有损,那型交人工。与 `--heal` 的关键区别是**不进 post-commit 自动档**:删行是活文档上最危险的动作。
     - **F6 是"块"级量纲,行级四条判不到它**:一整块多行登记被并发 union 追加两遍时,每一行都"只是又一个孪生行",F1–F4 与门 71 同时全绿(本会话 2026-09-26 真实自伤:探针登记块从 1 份涨到 3 份而账面什么都没喊)。判据 `findDupBlocks` 只认 **≥3 行且每行 ≥40 字符的连续 bullet 块逐字相同**(低于阈值会把台账里天然成对的短行全数成百地报出来,噪声淹掉信号);修复出口 `node scripts/plan-tasks-merge.mjs --dedupe-blocks`(先不带 `--commit` 出报告,确认四条零损失断言后再 `--commit`),它**只删第 2..N 份逐字相同的副本**,漂移副本(首行同而正文不同)一份都不动 —— 机器折半即有损,那型交人工。与 `--heal` 的关键区别是**不进 post-commit 自动档**:删行是活文档上最危险的动作。**合并落地闸要跑两把尺子**(同日由一次真实的"已付的账原地复活"逼出):`git-sync-converge` 的**无冲突** merge-tree 分支原先零看守 —— 本地把 F6 从 8 清偿到 0、远端一份陈旧计划文档带着那 8 块进来,merge-tree 判"无冲突 ✅ 推进",账就复活了;而 `union-converge` 的判据是"相对 max(两侧)",这一型里 max(两侧) 恰好等于合并结果 ⇒ **第一把尺子结构上看不见它**。现落地闸同时跑 ① 相对两侧上限 ② 相对入库基线 `scripts/plan-task-state-baseline.json`(维度清单仍只有一份 = `plan-tasks.mjs` 导出的 `probe()`,别处不得再抄),两把各有正反对照(PS-1 同形而基线收紧必点名 / PS-2 基线与实态一致不得判红 —— 第二把若恒红就等于把所有合并堵死,与恒红门同罪)。
- **对象空间落地与活文档编辑的常驻出口 = `scripts/object-space-land.mjs` / `scripts/live-doc-edit.mjs` / `scripts/gate-registry-insert.mjs`(+ 共用 plumbing `scripts/lib/bypass-git.mjs`),不是守门**(2026-09-26 立;镜像测试 `node --test scripts/tests/{object-space-land,live-doc-edit,gate-registry-insert,bypass-git}.test.mjs` = 36 例)。为什么这条要进 AGENTS:本仓在共享工作区里做旁路提交是**常态**(活文档与注册表的工作树副本常年滞后 HEAD,按 pathspec 交工作树就是把别人已入库的行整批写回旧态),而 2026-09-26 那一轮同一套 plumbing(临时 `GIT_INDEX_FILE` + `hash-object` + `update-index` + `write-tree` + `commit-tree` + CAS `update-ref`)在同一会话里被手写了 **6 份**、全躺在 `.ihui-agent/tmp/` 不受版本控制,并且已经互相漂开:索引可动性判据两种写法、零损失判据三种写法 —— 其中一份拿"重复行计数"当零损失判据,被 `guardian-runner.mjs` 里天然存在的 `  },` 与空串行顶出"凭空多出 1322 条"而**一次也没落地成功**,换成结构等值判据才通过。不入库的代价是确定的:下次会话要么再手写一遍(带着同一个 bug 的新版本),要么图省事走 pathspec。**三条判据照抄别重新发明**:① CAS 循环内**每次重读 HEAD** 且"任一目标路径被别人改过即停"(不是抢锁,是拒绝覆盖);② 零损失判据 = **结构等值**(`new == 前缀 ⊕ 本块 ⊕ 后缀`),不是重复行计数;③ 落地后 `git show --name-only` 回读声明路径,并把**共享主索引**逐路径对齐到新 blob —— **只在** 索引 blob 等于父提交 blob 或索引里没有这条时动,否则点名"归属他人"不动(§12d 第三层那型:一次不带 pathspec 的普通提交就会把旁路交付写回旧版)。取号类判据(`gate-registry-insert`)必须同认 `id: '134'` 与 `id: "134"` 两种形态 —— 双引号 id 对一切按单引号解析注册表的判据(含守门 89 的撞号 R5)隐身,这是本轮真实踩过并用夹具钉死的坑。
- **共享层重复**(40):端内重新实现 shared hook/util 检测(阻塞,防端内独立实现回升,白名单:web/useChat + web/useAuth + web/useAgentRuntime + web/useClipboard + web/useNotificationStore + mobile-rn/useAuth + mobile-rn/useNotificationStore(后二者均为 re-export wrapper + 平台 adapter,属 §3 允许形态;守门的 re-export 判定只看 export 那一行有无 `from`,函数型 wrapper 必须显式登记))。〔归并补(副本独有说法,现行判据以上文为准):-**共享层重复实现守门**(40):`scripts/check-shared-layer-duplication.mjs`(blocking)——apps/*/src的export名与packages/shared/srcexport名求交集(`collectExports:21`,`scanAppDir:85`),端内重新实现即exit1(`:152`);(原 L1422) / 无应急通道(runner无skipEnv;(原 L1422) / re-exportwrapper豁免逻辑在白名单)。(原 L1422)〕
- **品牌邮件通道对账**(81):check-brand-email-channel.mjs(blocking,2026-09-23 立)—— 拦绕过品牌模板层的 ops 自发纯文本邮件("能发出去、typecheck/lint 全绿,用户收到的邮件没样式",与守门 72/78 同族):R1 `Send-MailMessage` 调用语句缺 `-BodyAsHtml` / R2 `api.resend.com/emails`(含 host+path 分行形态)同一发送上下文无 `html` 字段 / R3 有发信动作(`createTransport`/`sendMail`/`api.resend.com`)却不引用 `email-templates` 也不调用 `notify-deploy-failure`;范围 `deploy/**`+`scripts/**`(不含 tests)+workflows `*.yml`,注释行/裸域名不判(宁漏不误报),行内豁免 `brand-mail-exempt: <原因>`,存量走 `scripts/brand-email-channel-baseline.json` 只减不增(**2026-09-24 实测基线 `counts` 已为 `{}`** —— 建门时揪出的第三条通道 `scripts/check-credential-health.mjs` 已迁到同一派发器,未用 `brand-mail-exempt:` 糊过去)。**范围已两度扩面**(2026-09-24):原只扫 `deploy/**`+`scripts/**`,而告警邮件正文的实际出口在 `monitoring/alertbridge/*.cjs` —— 目录与扩展名**双重不可见**;现含 `monitoring/**` + `.cjs/.mts` + `apps/api/scripts/**`,并新增两条判据:**R3b** 接了唯一出口却自带一份 HTML 版式(三段与门:邮件版式标记 ∧ ±10 行样式指纹 ∧ 本文件确在邮件语境,注释行不计)—— R1/R2/R3a 只看"有没有走模板",对这一型全盲;**R4 告警接收面** `monitoring/alertmanager/**` 的 yml/yaml/tmpl 出现 `email_configs`/`smtp_*`(AM 原生邮件 = 无版式纯文本)、IM 中转 receiver(名字与 host/路径**都**认,`feishu-copy` 改名躲不过)、或 `webhook_configs` 里 url ≠ `BRIDGE_URL` ⇒ 红;注释行不判红但 `amCommentLiterals` 如实计数。**全仓不扫 yml**(根 compose / prometheus / loki 的历史注释里就写着 `dingtalk-webhook`,全仓扫必假红 —— 面外写进断言)。`stagedTriggers` 与扫描面**必须同步扩**,否则"只改 bridge/模板的提交"根本不唤起本门(判据存在而永不调用 = 没有,守门 70/76 同型;该字段今晚被并发整文件回写抹掉**三次**,由镜像测试"装车证明"抓回)。**修法的唯一姿势:ops 邮件一律经 `apps/api/scripts/notify-deploy-failure.ts`(版式由 `email-templates.ts` 单点决定),不得端内自拼传输层;Alertmanager 出口只有 bridge 一条**。取证 `--self-test` 61 例(正反成对+临时仓双取材面)+ 镜像测试 20 例(含装车证明与 `BRIDGE_URL` 与渲染器逐字等值的漂移即红断言);`--staged` 暂存集为空/取不到 → 退化全量(守门 70 教训);紧急跳过 `HUSKY_SKIP_BRAND_MAIL_GUARD=1`。〔归并补(副本独有说法,现行判据以上文为准):范围`deploy/**`+`scripts/**`(不含tests)+workflows`*.yml`,注释行/裸域名不判(宁漏不误报),行内豁免`brand-mail-exempt:<原因>`,存量走`scripts/brand-email-channel-baseline.json`只减不增(建门实测:ps1自拼通道已被并行会话清除,副产品揪出第三条通道`scripts/check-credential-health.mjs`已入基线待迁移)。(原 L1395) / **修法的唯一姿势:ops邮件一律经`apps/api/scripts/notify-deploy-failure.ts`(版式由`email-templates.ts`单点决定),不得端内自拼传输层**。(原 L1395) / 取证`--self-test`30例(正反成对+临时仓双取材面)+镜像测试8例(含装车证明);(原 L1395)〕
- **主题接线对账**(91):check-theme-prop-wiring.mjs(blocking,2026-09-24 立)—— 堵"顶栏深色 + 正文浅色"这类**同屏双色档案**:`packages/app` 213 个 theme-driven 组件的形参默认值是 `colorScheme = 'light'`,调用方漏传就静默脱主题,类型系统完全不红(`colorScheme?:` 可选)。三判据 —— **missing**(渲染点未传该属性)/ **literal**(写死 `'light'` 或 `'dark'`)/ **spread-unknown**(仅 `{...props}` 展开且回溯不出来源,判不出即如实报数,不静默放行)。建门实测:此前一轮用"JSX 元素文本里有没有 `colorScheme` 字样"统计出"漏传 0 处",**该结论是错的** —— 换成花括号深度扫描 + 组件清单自动推导后真实命中 **118 处 / 117 文件**,即 115 个屏一直在静默锁死浅色档案。**盲区已补(比漏修更值得记)**:写死的 `'light'` 有 5 处藏在**对象构造里**而非 JSX 属性上(`const props = { colorScheme: 'light' }; return <SharedX {...props} />`),"字面量"判据只解析 attrs 故从不判红,"未接线"判据见到 spread 就笼统归"判不出" —— 这 5 屏以"待人工核"的名义静默锁死浅色,**门一直是瞎的**。现由 `resolveSpreadThemeValue()` 回溯对象构造再判:对象里有字面量 → 判红;对象里确实没这个键 → 判 missing;props 来自函数形参且本文件无对象字面量 → 仍承认判不出,不猜。**基线 `scripts/theme-prop-wiring-baseline.json` 现须为空 `{counts:{}}`(零容忍)**,`--strict` 口径为"0 未接线 / 0 字面量 / 0 判不出"。行内豁免 `theme-wiring-exempt: <原因>`(须带原因)。取证 `--self-test`(**含阳性对照**:注释里写 `colorScheme: 'light'` 不得误判、不传 `src` 时行为与旧版一致)+ §22c 镜像测试 `scripts/tests/check-theme-prop-wiring.test.mjs`(含**装车证明**:runner 里必须真有 id 91 + blocking + skipEnv,且本门编号在 runner 中恰好出现一次)。**内容口径(2026-09-25 补齐,此前它是全链最后一道按磁盘判的主题门)**:全量判 **HEAD blob**、`--staged` 判**索引 blob**、`--worktree` 仅人工逃生舱;枚举与内容同面同轮;取不到 ⇒ **exit 2「无法判定」**(不冒红也不记绿),`--staged --worktree` 同给判死,全量枚举到 0 个 .tsx 判死;git 面必须先 `prefetch`(一次 `cat-file --batch`)再 `read`,未预取即 read 抛错而非静默跳过。立项当场即由此把他人**未提交**的 `ChatScreen.tsx` 摘 prop 判成本仓违规(HEAD 其实合规)。取证 `--self-test` 的 F1–F4 在 `mkScratch` 临时仓里造"索引≠磁盘"现场,并由**三条变异**分别证明 F1/F4 不是恒真;紧急跳过 `HUSKY_SKIP_THEME_PROP_WIRING=1`。〔归并补(副本独有说法,现行判据以上文为准):-**跨端主题透线对账**(91):`scripts/check-theme-prop-wiring.mjs`(blocking)——packages/app的主题驱动组件(形参解构含`colorScheme`且一律带`='light'`默认值)必须在每个JSX渲染点被**真的传参**:默认值就是"静默失效开关"，写死或漏传都不报错、typecheck也不红。(原 L1458) / 拦两类真事故形态——L1调用方写死`colorScheme="light"`(实测导航栏深色而页面体浅色，同一屏两套档案)、L2漏传(当前0处，本门把它钉死在0；(原 L1458) / 刻意不把213处形参改成必填:无收益且会与并行会话在213个文件上对撞)。(原 L1458) / 组件集**不靠手工清单**(清单过期正是上一道门漏判的原因)，改为扫`packages/app/src`导出组件名+**import解析**(含alias/默认导入/命名空间成员)，只有解析到`@ihui/rn-app`或包内相对路径才算共享主题组件——端内同名自绘版(NavBar/TabBar/Carousel/UserInfoCard)因此不会误判；(原 L1458) / `{...spread}`转发的渲染点单列「不确定」如实报数(不静默放过、也不误判为红)。(原 L1458) / 实测真仓`--staged`exit0；(原 L1458) / 应急跳过`HUSKY_SKIP_THEME_PROP_WIRING=1`。(原 L1458)〕
- **品牌实底前景/容器对账(RN + 共享包 + web 类名面)**(83,原 75):check-brand-foreground.mjs(blocking,2026-09-23 立)—— 五条判据,`brand.DEFAULT` 与 `brand.cta` **同形认**(只认 DEFAULT 会让迁移后的主实底整片隐身 —— 判据必须覆盖门自己产出的形态):**R1** 零豁免,同块内 `backgroundColor: (tokens|tk).brand.(DEFAULT|cta)` × `surface.light`/`text.primary` 作前景;**R4** 同一关系拆到**兄弟 key**(`retryBtn` × `retryText`)按命名配对判,走棘轮(现 127 处);**R2** 拦"浅色当容器底"(`surface.light` / α≥0.5 白 rgba / 无 `dark:` 变体的 `bg-white`),棘轮现 24 处;**R3** 数品牌实底填充/描边,**按档配对**免债:DEFAULT 底 ↔ `brand.foreground`、cta 底 ↔ `brand.ctaForeground`,**跨档配对仍计**(DEFAULT 底 × ctaForeground 深色档案下=白底白字),棘轮现 235 处;**R5** web/ui-react 的 **Tailwind 类名形态**(R1–R4 全解析 RN style 对象,对 `bg-primary`+`text-primary-foreground` 这一 §4 已废配对原本整侧盲视),同行成对才算,变体前缀(`data-[state=checked]:bg-primary`)与染色底(`bg-primary/10`)不算,`r5-cta-exempt: 原因` 豁免,基线 `webClassPairCounts` **现为空 = 零容忍**(立项实测 22 处/14 文件,含 `@ihui/ui-react` Button 的 6 个 variant,当日由并行提交 `4e0b24689a` 迁完)。**R5 两条已知限制如实登记**:跨行配对不计(宁窄不误)、变体前缀不算实底(排除前缀的字符类与排除 `/90` 的是同一个,放宽任一即破坏口径)。**内容口径**:全量与 `--update-baseline` 判 **HEAD blob**,`--staged` 判暂存 —— 共享工作树滞后 HEAD,按磁盘算会恒红/假绿来回跳并把错数写回基线(2026-09-24 一天内 R3 登记被整文件回退三次的直接成因)。基线 6 键:`counts`/`ctaCounts`/`r4Counts`/`webClassPairCounts` + 两个文档性注记键(`pairedCtaNotVisibleToRule`/`ctaFillRenameLedger`)—— `--update-baseline` **必须保留注记键**,旧写法整文件重写会把他人审计台账冲掉(本轮并入远端时实测踩过)。取证:`--self-test` **94 条断言**(含 3 条阳性对照 + 变异对照)+ `node --test scripts/tests/check-brand-foreground.test.mjs` **13 例**;`--staged` / `--update-baseline` / `--self-test`;紧急跳过 `HUSKY_SKIP_BRAND_FOREGROUND=1`。〔归并补(副本独有说法,现行判据以上文为准):-**mobile-rn深色前景/容器对账**(83,原75):check-brand-foreground.mjs(blocking,2026-09-23立)——四条判据:**R1**零豁免,同一style块内`tokens.brand.DEFAULT`作背景×`surface.light`/`text.primary`作前景(深色档案下brand.DEFAULT=纯白⇒白底白字);(原 L1382) / **R4**同一关系被拆到**兄弟key**(`retryBtn`×`retryText`)时按命名配对判,走基线棘轮;(原 L1382) / **R2**`surface.light`背景/α≥0.5白rgba/无`dark:`变体的`bg-white`作容器底,每文件计数对`scripts/brand-foreground-baseline.json`只减不增;(原 L1382) / **R3**数brand.DEFAULT填充/描边但**不计§4成对CTA**(同块或兄弟键用`brand.foreground`作前景)——见本节上方「品牌CTA/主按钮色同源」,按规矩写就红的门只会逼人`--no-verify`;(原 L1382) / **无配对的白卡片照旧计**(自检里两条阳性对照钉住)。(原 L1382) / **内容口径**:全量与`--update-baseline`判**HEADblob**,`--staged`判暂存——共享工作树滞后HEAD,按磁盘算会在恒红/假绿间来回跳并把错数写回基线(2026-09-24一天内R3登记被整文件回退三次的直接成因)。(原 L1382) / -**mobile-rn深色前景/容器对账**(83,原75):check-brand-foreground.mjs(blocking,2026-09-23立)——R1零豁免:同一style块内`tokens.brand.DEFAULT`作背景×`surface.light`/`text.primary`作前景(深色档案下brand.DEFAULT=纯白⇒白底白字);(原 L1384) / R2基线棘轮:`surface.light`背景/α≥0.5白rgba/无`dark:`变体的`bg-white`作容器底,每文件计数对`scripts/brand-foreground-baseline.json`(现13文件24处,均为媒体上的合法浮层或已带`dark:`变体)只减不增。(原 L1384) / `--staged`/`--update-baseline`/`--self-test`(11例);(原 L1384)〕
- **反回退对账**(84,原 76):check-stale-revert.mjs(blocking,2026-09-23 立)—— 堵共享工作区**静默回滚**:§12d converge 用 merge-tree/commit-tree 只推进 HEAD+index、**不 checkout**,工作区落后 HEAD 时 `git add <file>` 交的是旧基线(实测 503 文件落后 486 提交),等于把别人该路径的后续改动静默回滚,而 diff 只显"改了几行"。判据 R1 = 暂存 blob != HEAD blob **且字节级等于该路径某祖先提交版本** → 拦并点名回到的 commit;真新编辑不可能恰好等于历史 blob,故误报极低。三条护栏:merge 上下文**不再整轮豁免**(2026-09-25 收窄,见下)、暂存删除只 warn(`git rm` 合法,机器分不清就被删)、判定文件 >300 跳过(性能护栏,防逼人 --no-verify 连带关掉全部守门;**乘数级路径不吃这条护栏**,2026-09-24 立)。**合并期窄判据 R1m**:某路径上 `ours == theirs`(两父逐字节一致)⇒ 合并对它无事可做 ⇒ 索引里与两父都不同的内容**不可能来自本次合并**,再要求它恰等于该路径某历史版本才判红 —— 第三个条件是必须的,因为"人工解冲突写进新内容"同样满足前两条,而那是正当形态。`cherry-pick / revert / rebase` 仍整轮豁免(取历史内容本就是其语义)。立因(第四十二批未闭环④):旧口径那句"merge 上下文整轮豁免"是**本门自己的盲区**,当夜 30c 判红、84 沉默,合并期把旧基线写进索引完全无人看守。取证 `--self-test` **12 例**(新增 5a/5b/5c/5d/6b 五例合并对照:两父不同⇒绿、两父一致而等于历史版本⇒红、等于新写内容⇒绿、cherry-pick⇒仍绿)+ §22c 镜像测试 7 例(含"整轮豁免那句写法不得回来"的**反向回归锁** —— 收窄是判据不是临时关闭,必须能被机器发现)+ 临时 index 端到端演练 3/3(2026-09-23 原始取证,保留);**有意回退一律改用 `git revert` 生成前向提交**。**一条本门看不见的时间窗(2026-09-26 实测)**:84 判的是**索引**里的 blob,而"工作树副本 == 祖先版本、但尚未 `git add`"这一格它结构上碰不到 —— 偏偏危险就在这格里:任何人一次 `git add <该文件>` 或一条不带 pathspec 的 `git commit -a` 就会把它交上去。当天实测 `scripts/guardian-runner.mjs` 的工作树副本等于祖先 `4c1610662`(09-25 19:34),**比 HEAD 少 3 道门(124/125/126)**,而 `git status` 只显示一个 ` M`、13c/71/84 全都不响。量这一型的尺子是 `node scripts/heal-worktree-tracked.mjs --align-drift`(判据:索引==HEAD 且工作树==该路径某祖先 ⇒ 才动,可证零独有数据),手工等价动作是 `git checkout-index -f -- <path>`;**先看判据再动手,不满足就不许对齐** —— 它也可能是别人真在写的现场。紧急跳过 `HUSKY_SKIP_STALE_REVERT_GUARD=1`。〔归并补(副本独有说法,现行判据以上文为准):三条护栏:merge/cherry-pick/revert上下文整轮豁免、暂存删除只warn(`gitrm`合法,机器分不清就被删)、判定文件>300跳过(性能护栏,防逼人--no-verify连带关掉全部守门)。(原 L1385) / 取证`--self-test`8例(含"写回v1必判红"阳性对照)+临时index端到端演练3/3;(原 L1385)〕
- **硬编码中文基线棘轮**(70):scan-hardcoded-zh.mjs(blocking,2026-09-22 接线)—— 扫 `apps/web/{app,src/components,src/hooks}` + `packages/{ui-react,shared}/src` + 2026-09-24 补的三端(`apps/mobile-rn/src`、`apps/extension/{entrypoints,src,lib}`、`apps/cli/src`),排除注释/metadata/useTranslations 行/admin/测试。**每文件命中数与基线额度比较**(`scripts/hardcoded-zh-baseline.json`):只拦"比基线更多",存量债(886 文件 / 13090 行)冻结、新增即拦;清理后 `--update-baseline` 下调额度(必须人工确认后再跑,禁止为过门而调高)。该脚本 2026-07-20 即存在但从未接入守门链,是"造好没装车"的第二个实例(第一个是同一批的 69)。`--staged` 由 runner 自动下发(runner 传 `--exit 1`,不带该参数时**永远 exit 0** —— 集成测试踩过),暂存集为空时回退全量防"空暂存恒绿"。**两处同日修**:① 14 例镜像测试自 §15「ROOT 由脚本自身位置推导」起 **13/14 恒红** —— 测试靠 `cwd` 定位夹具而脚本按定义忽略 cwd,14 例全在扫真仓(不是判据错,是调用方式失效);加 `--root <dir>` 显式测试通道(生产不带,语义不变)后 15/15 绿。② 命中判定漏剥**行尾 `//`** ⇒ PriceChart / TerminalTab / TerminalStatusIndicators 各多出 2/1/1 处假阳,把它们顶过基线额度、把本门钉成"谁碰这三个文件谁被拦";同时修 `'https://x/*'` 这类串内 `/*` 骗进块注释状态机的假绿。**遗留已消除(2026-09-25 实测复核)**：全量模式的取材口径**已改为判 HEAD blob** —— `scan-hardcoded-zh.mjs` 现有"磁盘/索引/HEAD 三种取材共用一份判据"(约 :204 行注释与实现)，实测 `node scripts/scan-hardcoded-zh.mjs` 全量**越线文件数 = 0**。本段此前写的"全量模式按共享工作树判 ⇒ 同一份 HEAD 代码干净检出报 1 个、本机工作树报 281 个"是**已修状态**，保留原话只为说明那一型为什么会发生；**下一个接手者不必再做这件事**，若要复核判据请跑上面那条命令看末行现值。（另记 `heal-worktree-tracked` 的刷新面仍在扩大，两件事互补不互替。）紧急跳过 `HUSKY_SKIP_HARDCODED_ZH_GUARD=1`。〔归并补(副本独有说法,现行判据以上文为准):**遗留(已量化,未修)**:全量模式按共享工作树判,而共享工作树对约2064个路径落后HEAD——同一份HEAD代码,临时worktree干净检出报**1**个文件越线(`packages/shared/src/chat/handoff-package.ts`49处/基线0),本机工作树报**281**个。(原 L1379) / 要么让全量模式改判HEADblob(守门77已验证的口径),要么把`heal-worktree-tracked`的刷新面扩大;(原 L1379) / 前者是判据口径、后者是环境自愈,都不该靠"反正本地红着习惯就好"糊过去。(原 L1379)〕
- **架构契约对账**(103):`scripts/check-architecture-policy.mjs`(blocking,2026-09-25 立)—— 与其余全部守门**方向相反**的一道:其余是"发现一类违规 → 写一条判据"(打地鼠),本门读 `config/architecture-policy.yaml` 这张**声明表**,从声明反查违规。四类红:**T1** 表与现实脱节(模块改名/目录搬走而表没跟上 —— 表一过期,依赖它的所有判断都在对着空气打分)/ **C1** 单文件行上限(阈值 6000,**取自 HEAD 实测最大文件 5258 行之上**,依据写在 yaml 注释里)/ **D1 D2** 依赖方向(包 import 端、或 `requires` 未声明就 import;以及反向按 rank 更深一层)/ **D3** 深导入(绕过 `public_entrypoints` 直接摸别的包内部 —— 它是"端内重复实现"的入口)。**渐进收口是设计前提**:立项时 24 块一律 `managed: false`(只报数不判红),逐块翻正时才按判红口径问责。⚠️ "**存量一律 false**"描述的是**立项状态,不是现值** —— 现值一律按当次实测取,见下。翻 true 前先用 `--managed-trial <id>` 看条数,**不得为消红去改阈值或把不该对外的模块标成 exported**。行内豁免 `arch-exempt: <原因>`(须带原因)。口径同 70/77/83/98/101:全量判 **HEAD blob**、`--staged` 判索引 blob。**策略表自身的取材按档定向**(`policyFaceOrder`):全量档 HEAD 优先、**`--staged` 档索引优先**,两者都再降级到工作树;取不到判"无法判定",不冒红也不记绿。⚠️ 这里 2026-09-25 修掉一处**本门对自身改动的盲视**:原实现两个面都 HEAD 优先,于是"修改策略表自身"的那枚提交完全不进本门审查 —— 实测往索引版 `apps/cli.requires` 注入一条 `apps/api`(端应用 `exported:false`,T1 必判红)后全量与 `--staged` 双双 exit 0,而 `--staged` 输出照旧打印旧表的 `managed:true` 清单。这个洞偏偏落在"规矩 2:翻 `managed:true` 前先试跑"所要求的动作上(提交链是唯一无验的一环),且源码旧注释"表是输入不是被审对象"正是它的设计理由,已就地推翻。取证 `--self-test` 51 例(成对正反 + 两面口径差异 + 解析器坏了必须大声失败)+ 镜像测试 13 例(含 runner 装车证明、"同一代码 managed 两侧结论不同"、T12 的**纯函数+构造面**取材面证明、以及 T12b 的尺子反例;变异回旧顺序只有 T12 红)。**本门自身踩过的判据教训(勿再犯)**:这里曾写过一条"索引表≠HEAD 表 ⇒ 两档收口集合必异形"的条件断言,两条前提都不成立(改注释/requires 都会让表不同而集合不变;尺子若吞到行尾则两档因"扫描 N 文件"天然不等)⇒ 一支误红、一支无牙,已删。**证明取材面这类行为只能用纯函数+构造面**,不得依赖仓库瞬时状态。**收口进度不写进文档**:某模块是否 `managed:true` 一律按当次实测取(`--staged` 输出行即现值),理由与本门刚修的那类缺陷同源 —— 登记过期数字会替人做出"已经收口"的判断。编号说明:任务书原定 102,落地前已被 `check-glyph-arrow-icon` 占用,按"后来者改号"顺延 103。紧急跳过 `HUSKY_SKIP_ARCH_POLICY=1`。 **E1/E2 两条同日补**:E1 公开入口存在性对账 —— `public_entrypoints` 声明的入口必须在**被审判的同一取材面**里解析得到(HEAD 面实测 75 条声明入口全部在位 ⇒ 本条可即时问责,不像 E2 那样带存量)/ E2 契约工件齐备性 —— 纳管模块必须有策略表字段点名的自述与契约文件(**清单取表里的字段,不在代码里硬编码第二份名字**),默认档**只报数**:HEAD 实测 16 块未齐备,当场判红就是一台恒红门(§12e 同型),要问责跑 `--strict`。退出码口径:判红只算「C1/T1 全仓即时 + 已收口模块的契约违规」,E1/E2 的红仅按 `managed:true` 问责。**同批给策略表配了只读出口** `scripts/module-context.mjs`(pnpm `module:context`):按模块 id 打印「是否纳管 / 声明依赖 / 本模块契约文件 / 依赖模块契约文件 / 公开入口 / 边界条款」,`--files` 直接吐派单用的文件清单 —— §11 那张「受影响文件」格过去全靠派单人手写,而 §11 自己就记着这张清单写错会造成越权/污染。未知模块 id **exit 2 并点名**(实测 `node scripts/module-context.mjs nosuch-module-xyz` → rc 2);`--json` 必须可 `JSON.parse`;E2 未齐备既打印 missing **也进退出码** —— 上游那一版只打印不判,这一格我们补上了。〔归并补(副本独有说法,现行判据以上文为准):口径同70/77/83/98/101:全量判**HEADblob**、`--staged`判索引blob,策略表自身按HEAD→索引→工作树降级取且**退到工作树会大声提示**;(原 L1400) / 取证`--self-test`51例(成对正反+两面口径差异+解析器坏了必须大声失败)+镜像测试10例(含runner装车证明与"同一代码managed两侧结论不同"的判据有牙证明)。(原 L1400) / 〔归并补:取证`--self-test`51例(成对正反+两面口径差异+解析器坏了必须大声失败)+镜像测试12例(含runner装车证明、"同一代码managed两侧结论不同"、以及T12的**取材面双向条件不变量**——索引表≠HEAD表则两档结论必不同形、相同则必同形;(原 L1944) / (原L1946)/**渐进收口是设计前提**:存量模块一律`managed:false`⇒只报数不判红(立项实测全量exit0、报数3处、`managed:true`0个)(⚠️这几个数字是**立项状态、不是现值**;(原 L1944) / **渐进收口是设计前提**:存量模块一律`managed:false`⇒只报数不判红(立项实测全量exit0、报数3处、`managed:true`0个)(⚠️这几个数字是**立项状态、不是现值**;(原 L1954)〕。〔归并补(副本独有说法,现行判据以上文为准):-**为什么"产物存在"不等于"能力在线"(本门立论,值得推广到别的生成器)**:这四个生成器里此前**只有i18n那个进了构建链**,其余三个既无构建入口也无门禁⇒"造好没装车"只能靠人记得手动跑;(原 L2137) / 而dev链不跑i18n生成器,意味着**开发时离线语言包常年是旧的**,只有release才被发现。(原 L2137) / 同类盲区见守门64(适配器接线)/70(硬编码中文)/81(邮件通道):**判据必须在真跑它的那一刻才成立,否则等于没有。(原 L2137)〕
  - **为什么"产物存在"不等于"能力在线"(本门立论,值得推广到别的生成器)**:这四个生成器里此前**只有 i18n 那个进了构建链**,其余三个既无构建入口也无门禁 ⇒ "造好没装车"只能靠人记得手动跑;而 dev 链不跑 i18n 生成器,意味着**开发时离线语言包常年是旧的**,只有 release 才被发现。同类盲区见守门 64(适配器接线)/ 70(硬编码中文)/ 81(邮件通道):**判据必须在真跑它的那一刻才成立,否则等于没有。**。〔归并补(副本独有说法,现行判据以上文为准):-**小程序端「源↔派生产物」存续性对账**(105):`scripts/check-miniapp-generated.mjs`(**warn,刻意不blocking**)——管四件"生成器造好了但没人跑"的派生产物:i18n离线包`remote-locales.gen.ts`、`static/images/icons/*.svg`、`LineIcon/icons.ts`、`assets/tabbar/*.png`。(原 L2057) / **升blocking的前置条件是G1存量清零**。(原 L2057)〕
  任何"可达率"读数都必须**先认档再说数**,脱离 C5/C6 的单个百分比没有意义。**三条教训叠在一起**:① 判落地的门比错形态,症状不是差一点,而是把真信号淹在噪声里;② 但在把两侧名字"对上"之前,必须先问**挂载侧到底写了什么** —— "CSS 里有这条规则"与"这个样式生效了"之间隔着一维,只读声明侧的绿灯结构上排除不了"规则存在、没人挂得上"。〔归并补:现测**hit269/771=34.89%(原名直中266+转写可达0+复合首族3)/死规则485/确定缺失6**。(原 L2094) / **两条教训叠在一起**:①判落地的门比错形态,症状不是差一点,而是把真信号淹在噪声里;(原 L2094)〕
- ⚠️ **上面那条 `/alpha` 判据目前到不了小程序产物**(2026-09-25 实测,登记于 PROJECT_PLAN O62附②):`apps/miniapp-taro/config/index.ts` 的 `tailwindcss: { enable: true, config: {} }` 让端内 `tailwind.config.ts`(content globs + `presets:[@ihui/design-tokens/tailwind-preset]` + 本插件)在真实构建中**从未被加载** —— 实测真实 build 后 dist 的 154 个 wxss 里,源码用到的 **687 个 Tailwind utility / 11,105 处用法有 0 个产出规则**,`.flex{` 与 preflight 指纹双双缺失;阳性对照是同一份 dist 里 `app.css` 手写类名 75/98 可查到规则,故不是检索姿势问题。**后果**:改这条配置等于让整端 11,105 处用法同时开始产出 CSS,而现状很可能是手写 CSS 已补偿过(自有类名 2,228 个),开启后好坏只能由微信开发者工具真机渲染判定 ⇒ 属未决决策项,不是已收口项。**同时提醒同源对账门的边界**:门 36/37/93/`check-miniapp-taro-style-parity` 全部只核源码与 token 源头,**没有一道看产物**;它们五道全绿而到端 CSS 为零,是可能且已发生的。新增跨端同源门时,请先问自己判的是"写的同源"还是"生效的同源"。**⚠️ 本条的"0 产出"现值已作废**(同上,`edb225c909` + `f17b5fbdda`):`app.css` 现有一个由 `sync-miniapp-tokens.mjs` 派生的 `@theme` 注册区,命名档与项目色档已产出**且运行时可达**;但含标点那一族(arbitrary value 等)是"CSS 有规则、运行时挂不上"的死规则,真实可达率**必须先认构建档位**再读(同一配置实测三档:34.89% / 93.77% / 99.35%,见下下条与 PROJECT_PLAN 附⑩/附⑪)。"没有一道同源门看产物"这半句仍然成立 —— 看产物的是 `check-miniapp-css-landing.mjs`(手动/CI,未接提交链),它现在连**运行时挂不挂得上**也一起看。〔归并补:**⚠️本条的"0产出"现值已作废**(同上,`edb225c909`+`f17b5fbdda`):真产物落地率740/771=96.0%,`app.css`现有一个由`sync-miniapp-tokens.mjs`派生的`@theme`注册区;(原 L1958) / 但"没有一道同源门看产物"这半句仍然成立——看产物的是`check-miniapp-css-landing.mjs`(手动/CI,未接提交链)。(原 L1958) / 但含标点那一族(arbitraryvalue等)是"CSS有规则、运行时挂不上"的死规则,全端真实可达率现测**269/771=34.89%**(见下下条与PROJECT_PLAN附⑩)。(原 L2096)〕
- **记忆端点属主绑定对账**(117,**blocking**,2026-09-25 升档;紧急跳过 `HUSKY_SKIP_MEMORY_OWNER_BINDING=1`):`scripts/check-memory-owner-binding.mjs` 钉的是**"认证不等于授权"**那一层 —— `apps/ai-service/app/api/memory.py` 把 `user_id` 当请求体/Query 参数收,全文件**没有一处**与令牌主体对齐;`/api/memory` 不在 `PUBLIC_PATHS` ⇒ 匿名进不来,但**任何已登录用户填别人的 UUID 就能读/改/删别人的记忆**。判洞要问"有没有把请求里的 id 与令牌里的 id 比对",不是"有没有全局中间件"。两条 Python 语法锚点必须同时认:`user_id: str = Query(...)` 与 Pydantic `Field(...)` —— **只认 Query 会让整类 POST 端点隐身**;对齐出口只写在注释里不算(自称已拆、实际没拆那一型)。**定级史(照抄旧档会判错)**:立项时 warn —— 当时未对齐存量 2 个文件;现值一律 `node scripts/check-memory-owner-binding.mjs` 自取 —— `app/api/memory.py` 已于同日 `3b0170fa176` 九端点统一 `require_request_user_id` 归零,剩下的是**此前无人枚举过的** `app/routers/rules.py`,而"怎么对齐"取决于 **v1 对外 API 能不能代表他人访问记忆** —— 实测 `apps/api/src/routes/v1-knowledge-tools.ts` 有 2 条转发是 `jsonInit(parsed.data)` **原样透传客户端自报身份**(本门把这类列成"上游透传清单"只报数,就是给选型看爆炸半径用的)。收紧成 403 与"忽略并覆盖"两种改法**都会让本门归零**,故存量未清时判红就是恒红门,唯一结局是逼人 `--no-verify` 连带废掉全部守门(§12e 同型)。**2026-09-25 前置已满足并升 blocking**:`752c6eb110b` 收掉最后一个未对齐面(`app/routers/rules.py`)后 HEAD 面实测"未对齐 0 个",runner 侧同时带 `args:['--strict']`(本门默认档只报数,不带它升档是假的)。**⚠️ 上面那句"HEAD 面实测未对齐 0 个"当次即作废,是本节里一个真实的假绿灯(2026-09-25 同日更正)**:`3b0170fa176` 把参数一律改写成 `user_id: str | None = Query(...)`,而本门两条正则只认 `: str =` ⇒ **一个端点都扫不到** ⇒ 输出"收 user_id 的端点文件 0 个 / 未对齐 0 个"并 exit 0 —— 一道刚被升到 blocking 的安全门变成永远绿灯的尺子,它既看不见当时的敞口,也看不见"明天有人把 `require_request_user_id` 删掉"。补宽正则(可选标注两档都认)后 HEAD 面实测:**收 user_id 的端点文件 2 个 / 已对齐 1 / 未对齐 1**,那一个就是 `app/routers/usage.py` 的 `/stats`+`/quota`+`/record` —— 写法是 `uid = user_id or request.state.user_id`(**自报身份优先于令牌主体**),而参数说明里的"(管理员用)"在该文件内没有任何对应校验 ⇒ 任何已登录用户带 `?user_id=<别人>` 即可读他人用量/配额。已按 `require_request_user_id` + `roleId ≥ 1` 才可代表他人收口(实测 `apps/web` 两处调用方都只传 `days`、`usage/record` 全仓零调用方 ⇒ 不影响在跑链路),回归 `apps/ai-service/tests/test_usage_authz.py` 10 例。**本条的通用教训:改被审代码的写法,必须同时改审它的正则** —— 否则"升级成 blocking"这个动作本身就是在给一台瞎掉的尺子背书。取证 `--self-test` 17→**21 例**(新增 S17–S20:可选标注的 Query/Field 各一对正反例,夹具文本逐字取自真实文件)。**降回 warn 等于重新放开新增敞口**;若 HEAD 面再报未对齐(例:有人跳过钩子塞进去),先清偿再提别的票,不得削判据。口径同 70/77/83/98/101/103(全量判 HEAD blob、`--staged` 判索引、枚举到 0 个文件判"无法判定"而非记绿 —— 判据失明不是通过)。仓里现成出口 = `app/core/jwt_auth.py` 的 `require_request_user_id`(带 `DEV_ANONYMOUS_PRINCIPAL` 开发降级,以免 ASGI in-process 的既有测试整片 401 —— 那不构成安全结论)。取证 `--self-test` 17 例(正反成对) + §22c 镜像 14 例(含"未注册时不得被判定为已装车"的方向性对照);手动问责档 `pnpm check:memory-owner`(走 `--strict`)。〔归并补(副本独有说法,现行判据以上文为准):**定级warn是设计前提而非偷懒**:HEAD实测未对齐存量**2个文件**(`app/api/memory.py`与**此前无人枚举过的**`app/routers/rules.py`),而"怎么对齐"取决于**v1对外API能不能代表他人访问记忆**——实测`apps/api/src/routes/v1-knowledge-tools.ts`有2条转发是`jsonInit(parsed.data)`**原样透传客户端自报身份**(本门把这类列成"上游透传清单"只报数,就是给选型看爆炸半径用的)。(原 L2019) / 收紧成403与"忽略并覆盖"两种改法**都会让本门归零**,故在选型落地前它是"存量报数、新增判红"的棘轮;(原 L2019) / 当场blocking=与任何提交都无关的恒红门,唯一结局是逼人`--no-verify`连带废掉全部守门(§12e同型)。(原 L2019) / **升blocking的前置条件=未对齐存量归零**。(原 L2019) / **定级warn是设计前提而非偷懒**:未对齐存量:立项读数2个文件;(原 L2022)〕
  - **13c 自身的取材口径(同日迁移)**:此前 `ROOT = process.cwd()` + 裸 `git show` 用 `execSync` 拼字符串(无 timeout)+ 占位合规性看 `git diff` 文本,三处都与本仓现行口径相反(工作树滞后即换结论 / 服务账户 PATH 不通 / 别人同时改这文件时合规性跟着盘走)。现全部经 `scripts/lib/face-reader.mjs`,对照面是 `--staged`:HEAD→索引、全量:HEAD^→HEAD、`--worktree` 仅人工;整面取不到 ⇒ **exit 2 无法判定**,不冒红也不记绿。取证 `--self-test` 10 条,含两条本门自己踩出来的:CRLF/LF 同一条标题不得读成"删除";**A2 的归属必须看该目录在审面上的全集**而不是只看 `PROJECT_PLAN_*` 形状 —— 本门第一次自跑就是被这条假红咬到的(已跟踪的 `orphan-capabilities-equivalence-2026-09-24.md` 被点名"落空")。〔归并补(副本独有说法,现行判据以上文为准):2026-09-26那次实测是face28/loose71(其中half-wired1)/unknown38/no-content49——写死它下次收紧就变成假账。(原 L2010) / **那1道半接线同日收口了**:`check-rn-global-css-sync`换成走层的`catBatch`三面取材,收口后现读`half-wired`应为0——这条判据落地当天就产出了第一次真实修复,不是只多了一批红)。(原 L2010) / **同日两处精度修正(都是量出来的假阳,不是审美)**:①"读内容"与"枚举"必须分开——`cat-file-e`(存在性)、`ls-tree--name-only`(路径清单)、`rev-list`/`merge-base`不产生blob正文,把它们算成"散写读内容"会把只做枚举的门钉成半接线(它一处blob都没读);(原 L2010) / **假阳比漏报更贵**——它指使人去"修"没坏的东西,还把判据口径说歪成"数字很多"。(原 L2010) / ②仓库锚点必须落在**路径函数实参**里:旧写法的`|\bROOT\s*,`一支会把`functionaudit(limit=400,cwd=ROOT)`读成"以仓库根拼路径"。(原 L2010) / ③借层的transport自己读内容(`gitRaw(['show',spec])`)同样算半接线——管子共用不等于面共用。(原 L2010) / **已知空档如实登记**:锚点只认大写`ROOT`,故`join(root,…)`这类小写模块根写法落进`unknown`(不判红)。(原 L2010) / 试过放宽(74→93),其中两格是假阳(一格读`.workbuddy`运行台账、一格读按设计只存在于部署机的gitignore副本),**已收回**:判据分不清"读被审内容"与"读运行态",就不该假装分得清并据此判红;(原 L2010) / 补它要认识gitignore语义(参照`check-prod-bundle-shadow`的S2),不是一行正则的事。(原 L2010) / -**门脚本取材面纪律对账**(118):`scripts/check-gate-face-discipline.mjs`(blocking,`stagedTriggers=scripts/`,2026-09-25立)——本仓159道门里只有**24**道走`scripts/lib/face-reader.mjs`,散写git读内容或按磁盘`readFileSync(join(ROOT,…))`判的有**74**道,另32道"读文件但找不到仓库锚点"(疑临时夹具,结构上判不了,只报数)。(原 L2073) / **两层遮噪方向不同,别混用**:认face-reader导入与git动词必须"只遮注释、保留字符串"(模块说明符和`cat-file`本身就是字符串),认`readFileSync`调用必须"注释与字符串都遮"(`markHidden`那一档连字符串一起抹,直接套它会让F1/F2双双假绿)。(原 L2073)〕
- **对象空间落地与活文档编辑的常驻出口 = `scripts/object-space-land.mjs` / `scripts/live-doc-edit.mjs` / `scripts/gate-registry-insert.mjs`(+ 共用 plumbing `scripts/lib/bypass-git.mjs`),不是守门**(2026-09-26 立;镜像测试 `node --test scripts/tests/{object-space-land,live-doc-edit,gate-registry-insert,bypass-git}.test.mjs`,**例数以该命令末行为准,本文不钉数字**)。为什么这条要进 AGENTS:本仓在共享工作区里做旁路提交是**常态**(活文档与注册表的工作树副本常年滞后 HEAD,按 pathspec 交工作树就是把别人已入库的行整批写回旧态),而 2026-09-26 那一轮同一套 plumbing(临时 `GIT_INDEX_FILE` + `hash-object` + `update-index` + `write-tree` + `commit-tree` + CAS `update-ref`)在同一会话里被手写了 **6 份**、全躺在 `.ihui-agent/tmp/` 不受版本控制,并且已经互相漂开:索引可动性判据两种写法、零损失判据三种写法 —— 其中一份拿"重复行计数"当零损失判据,被 `guardian-runner.mjs` 里天然存在的 `  },` 与空串行顶出"凭空多出 1322 条"而**一次也没落地成功**,换成结构等值判据才通过。不入库的代价是确定的:下次会话要么再手写一遍(带着同一个 bug 的新版本),要么图省事走 pathspec。**三条判据照抄别重新发明**:① CAS 循环内**每次重读 HEAD** 且"任一目标路径被别人改过即停"(不是抢锁,是拒绝覆盖);② 零损失判据 = **结构等值**(`new == 前缀 ⊕ 本块 ⊕ 后缀`),不是重复行计数;③ 落地后 `git show --name-only` 回读声明路径,并把**共享主索引**逐路径对齐到新 blob —— **只在** 索引 blob 等于父提交 blob 或索引里没有这条时动,否则点名"归属他人"不动(§12d 第三层那型:一次不带 pathspec 的普通提交就会把旁路交付写回旧版)。取号类判据(`gate-registry-insert`)必须同认 `id: '134'` 与 `id: "134"` 两种形态 —— 双引号 id 对一切按单引号解析注册表的判据(含守门 89 的撞号 R5)隐身,这是本轮真实踩过并用夹具钉死的坑。
- **批量写计数诚实性对账**(guardian id 以 `scripts/guardian-runner.mjs` 现值为准 —— 落地时取到 134,并发会话可能挪号;`blocking`,`stagedTriggers=apps/api/src/routes/` + `apps/api/src/db/`,紧急跳过 `HUSKY_SKIP_BATCH_WRITE_COUNT_HONESTY=1`):`scripts/check-batch-write-count-honesty.mjs` —— 拦的是 2026-09-26 一整批同型静默失真的**形状**:`db.delete/update ... where inArray(ids)` 之后把**请求侧数组的 `.length`** 当 `affected` / `deleted` / `restored` 回给前端。那一型的问题是"改了 0 行"与"改成功"在响应上完全同形 —— `where` 带 `eq(roleId)` / `eq(memberId)` 过滤时,传进来的 id 一个都不命中也照样回 `deleted: 3`,后台显示"已删除 3 项"而库里一行没动。当轮人肉清出 8 处(`chat.ts` batch、`admin-sys/role-routes.ts` cancelAll+selectAll、`admin-demand-square.ts`、`admin/_shared.ts` 的 `registerCrud` 批量删〔**19 处 admin 路由复用同一个工厂**〕、`message.ts` 批量删、`workspace.ts` batch-delete/batch-restore;后两者的 db 层原本返回 `void`,**结构上无法回报命中集**),统一出口是 `apps/api/src/utils/batch-outcome.ts`(`dedupeIds` + `batchWriteOutcome(requested, confirmed)` ⇒ `{requestedIds, affected, missedIds}`),db 层一律 `.returning({id})` 回报**库确认集合**,响应新增 `missedIds` 逐条点名(既有键名与 status code 逐字不变)。**四条放过通道,每条都有正反用例**:① 同一写链带 `.returning(`;② 计数根标识符 ≤2 跳可追到归属预查询(`business-card-routes.ts` 那种"先查 owned 再删"的**正确写法**不得判红);③ 该文件 import 了唯一出口;④ 行内 `batch-count-exempt: <原因>`(须带原因,裸标记不计,并已挂进守门 108 的 30 天到期档 —— 豁免不得只出生不死亡)。**刻意不判、但每次现读并报数的两型**(不计红、不改退出码,写进门自身的观测面,免得"放过"与"没看见"在账面上长得一样):布尔 `deleted: true`(全仓惯例、既有用例当契约钉,要不要改成 `found`/404 语义属全 API 决策、交人定 —— **处数一律跑本门看末行现值,不得照任何文档抄数**)与读查询 `count: rows.length`(不是写)。**判不了的形状如实登记**:写链在 db 层而计数在 handler、`send` 在嵌套箭头里、`deleted: (await …).length` 非点号链形态 ⇒ 落 `未判定` 并报数,`--strict` 下有未判定即 exit 2(**拒绝出合格证**)。**口径同 70/77/83/98/103/118**:全量判 HEAD blob、`--staged` 判索引 blob、`--worktree` 仅人工、两面旗同给 exit 2、取不到判"无法判定"、枚举到 0 个候选判死;棘轮锚点 = 该文件 HEAD 自身违规数,所以存量未清时不会变成人人跳门。**两处遮噪方向不同**(本门自己踩过):判写链要"剥注释 + 保留字符串"(标识符落在字符串里的形态要看得见),判 `import` 说明符则连字符串一起抹会直接失明。**取证例数一律以命令末行现测为准,本文不钉数字**(本门落地当天就从 35 条长到 50 条、镜像从 10 例长到 14 例 —— 钉死的那份立刻变成假账):`node scripts/check-batch-write-count-honesty.mjs --self-test` / `node --test scripts/tests/check-batch-write-count-honesty.test.mjs`;阳性对照用**修复前的 HEAD blob** 喂同一判据,点名 `_shared.ts` 与 `message.ts` 两处真实存量,证明判据真能命中本仓实际产出的形态。

- **同一把尺子的另外三维(2026-09-27 补,读数一律现读)**:① **具名档**——数字一旦收编进 `packages/shared/src/ui/*-spec.ts` 或 `geometry.js`,组件里只剩标识符,**必须仍然读得出数值**(否则"收进单一源"= 从尺子上消失,这是用户实拍"两端还是不一样"而当次门报绿的直接成因)。**一条血的教训**:该判据 2026-09-26 就写进了 `readGeometry` 并配了自检,而 `main()` 的调用点漏传第四个参数,**在提交链上生效次数为 0** —— 自检恒绿是因为它手工喂档表给函数,证明的是"函数会给答案"而不是"有人问它";现由源码级装车锁(㊵)+ 镜像 T10 钉死。② **SL 单侧具名档** —— 一张 spec 档只被一条腿引用 = 另一条腿还没接线,与"同一元素两端不同值"处置动作不同,所以**只列名字、不折进几何集合**,与 IC 同形走 HEAD 棘轮(存量只报数,新增才红)。③ **G 几何表 ↔ 它自己的 `geometry.d.ts`** 两侧对账:有引用而类型不认 ⇒ TS2339(吵),**没人引用则两边都不红**,而本门正是按运行时键集读表的 ⇒ 判据信任了类型层不承认的档;落地前工作树正发生前者(`controlBox`/`controlGlyph` 入表未入类型 ⇒ miniapp 8 条 TS2339),而提交链上无一覆盖这一格。④ **拆对声明 `pairingRejects`** —— 同名不同物(实测 `FloatBox`:小程序 = 右下角三按钮功能盒,RN = 顶部自动消失的 toast)不是"两端分叉",继续计账只会诱导下一个人去收敛两个不同的东西;三条都判红(声明须带 reason + 未到期 `until` / 拆掉的族不得再挂 `counts` 或 `waivers` / 面上找不到该对则声明须了结),且 `emitBaseline` 必须原样带走它(它是判据输入不是存量数字,重写冲掉的后果与守门 83 那条同型)。**该族差异数从账上消失必须由"配对错了"这个证据换来,不得由"数字太大"换来。**
- **ApiResult 失败分支身份对账**(guardian id **135**,blocking,`stagedTriggers=apps/ + packages/`,紧急跳过 `HUSKY_SKIP_API_FAILURE_THROW=1`):`scripts/check-api-failure-throw.mjs` 拦 `if (!X.success) throw new Error(X.error)` —— `ApiResult` 失败分支本来带 `status`/`errorCode`(`packages/types/src/api.ts`),只取 message 等于把"这个错误的身份"丢掉;而 `toUserFriendlyMessage` 判序是 **errorCode → HTTP status → 文案正则**,前两档一丢,401 的 `Invalid or expired token` 只能落到参数类正则 `/invalid|missing|required|must be|expected/i`,用户看到「提交的信息有误,请检查后重试」。**立因是真机实测**:进 App「广场」tab 一进来就弹这句 —— 它把"去重新登录"引导成"回去改表单",不是措辞偏好而是**修复方向被指错**。唯一出口 `packages/shared/src/utils/error-messages.ts` 的 `apiFailureToError(res)`;出口被摘线时本门判"没有出路"并**参与退出码**(只打印不改退出码 = 下一次没人看)。三条设计点:① **判据面先剥注释与字符串**,遮罩实现只有一份(`scripts/lib/code-mask.mjs`,守门 131 同日改引它 —— 两处实现必漂移是本仓记过最多次的失败型,由镜像 T7 钉死"任一门不得留本地副本");② 棘轮锚点 = **该文件 HEAD 自身存量**(立项实测 958 处 / 576+ 文件,当场判红就是恒红门,§12e);③ 行内出口 `api-error-exempt: <原因>`(须带原因,已挂守门 108 的 30 天到期档)。**刻意不判的第二半必须记住**:catch 里写 `showFloat(e.message)` / `setError(String(e))` 的,即使 throw 换了出口仍然丢身份 —— 所以迁移余量必须逐处读 catch 路径,批量替换会产出"看起来改了、实际没好"的提交。取证 `--self-test`(含真仓 HEAD 阳性对照"看不见存量不算通过"、"HEAD 全量档不得因存量判红")+ §22c 镜像 8 例(T1 装车 / T2 摘线不得被读成已装车 / T4 取材面形状锁 / T5 真仓存量 / **T6 用纯函数 + 构造面证明"没 export 的出口等于不存在"** / T7 遮罩不得有第二份 / T8 自检端到端);另有一次**私有索引注入**的现场复跑:往 `ArticleListScreen.tsx` 的索引副本加一处新违规 ⇒ `--staged` exit 1 并点名该文件。
    任何"可达率"读数都必须**先认档再说数**,脱离 C5/C6 的单个百分比没有意义。**三条教训叠在一起**:① 判落地的门比错形态,症状不是差一点,而是把真信号淹在噪声里;② 但在把两侧名字"对上"之前,必须先问**挂载侧到底写了什么** —— "CSS 里有这条规则"与"这个样式生效了"之间隔着一维,只读声明侧的绿灯结构上排除不了"规则存在、没人挂得上"。〔归并补:现测**hit269/771=34.89%(原名直中266+转写可达0+复合首族3)/死规则485/确定缺失6**。(原 L2094) / **两条教训叠在一起**:①判落地的门比错形态,症状不是差一点,而是把真信号淹在噪声里;(原 L2094)〕
