...
- **提示注入登记对账**(guardian id **128**,blocking,`stagedTriggers=apps/cli/src/`,紧急跳过 `HUSKY_SKIP_PROMPT_INJECTION_REGISTRY=1`):`scripts/check-prompt-injection-registry.mjs` 钉 `apps/cli/src/utils/prompt-injection-registry.ts` 这张表的三种「说了没做」——**R1** 登记的 `producer`(`<路径>#<符号>`)必须①文件在被审面上、②代码面出现该 id、③真调记账出口(**注释里的提及不算**,那是「看起来有、其实没装车」的本仓最高频失效型)/ **R2** `renderInjectionNotice(` 在生产面至少一个调用点,且表与唯一出口模块在面上解析得到(拦「造好没装车」;测试面调用不算)/ **R3** 以宿主名义的裸 `[系统提醒]`/`[系统提示]` 前缀而该文件既不调用也不定义出口 ⇒ 红,**锚点 = 该文件 HEAD 自身违规数**(棘轮,存量只报数:当场把存量判红就是与任何提交都无关的恒红门,唯一结局是逼人 `--no-verify` 连带废掉全部守门,§12e 同型)。**零豁免清单**是设计前提——「接了出口」由代码自身推导,不登记文件名。取材面同 70/77/83/98/101/118(全量 HEAD blob / `--staged` 索引 blob / 两面旗同给 exit 2 / 枚举 0 判死 / 取不到 ⇒ exit 2 不回落)。已知盲区如实登记:`apps/cli/src/commands/agent.ts` 两处 `[系统提示]`(fs 事件段、工具连续失败反思段)是真实产出点、尚未改接出口,由 R3 的 HEAD 棘轮兜住「不得再加」,改接属实现票;同型地 `apps/cli/tests/prompt-boundary.test.ts` 的反第二真相锁只匹配 `[系统提醒]`,扩字面量集合必须与那两处迁移同批落地,否则造一台出生即红的测试。取证 `--self-test`(16 条,含「登记无生产者必红」「裸 `[系统提示]` 旁路必红」两条阳性对照 + HEAD 存量不判红反向锁)+ §22c 镜像 13 例(含 runner 装车成套性与 `.d.ts` 排除锁);问责入口:`node scripts/check-prompt-injection-registry.mjs`(根 `package.json` 当日被并发会话占着暂存,`check:prompt-injection` 脚本行未落,交主会话补)。
