...
- **案例 1**:cli CLI 融合迁移第十一轮,主线程串行完成 17 项 todo,耗时较长。用户反馈"多 agent 做 效率最大化",要求写入本规则强制后续并行。
...
- 第 1 节(唯一计划文档) > 第 8 节(删除安全) > 第 9 节(goal 红线) > 第 10 节(多端同步) > 第 11 节(报告一致性) > 第 12 节(subagent git 隔离) > **第 14 节(本规则,多会话隔离)** > 第 13 节(并行效率) > 第 3 节(做减法)。
