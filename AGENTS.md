...
- **守门**:`scripts/check-workspace-hygiene.mjs` + pre-commit 第 24 项(warn-only,扫描 `.trae-cn/tmp/` 下脚本是否引用项目外路径)。
- **历史教训**:2026-07-23 扩展验证时,agent 把构建产物复制到 `C:\temp\ihui-ext2`,用户质问"为什么不放项目内"。根因:§15 原版只说"临时文件放 `.trae-cn/tmp/`",没明确禁止项目数据写到项目外,导致 agent 习惯性沿用早期诊断脚本的 `C:\temp` 路径。本规则从机制上根治。
...
| 25  | check-workspace-hygiene.mjs                  | **项目外路径违规(warn-only,扫描 .trae-cn/tmp/ 脚本)** |
