...
- **凭据外泄对账**(67):非 2xx 响应**不经** `response-sanitizer`(其 496 行 `statusCode<200||>=300` 直接 return payload),故把上游响应体 `JSON.stringify` 进 error message 是脱敏旁路。本门仅在 4xx/5xx 构造上下文内、且被 stringify 的实参具备凭据语义(变量名/声明右侧/对象 key/message 字面量命中 `token|secret|credential|password|api_?key|bearer|access_token|refresh_token|id_token`)时 BLOCK,并覆盖"经一层声明间接外泄";其余非凭据实参只进低置信候选不计失败(全仓此类历史写法 35 处 / 13 文件,一律拦即假阳性)。成因:已提交进 main 的真实事故 `7384c92ed0`(IMS OAuth2 令牌端点整个响应体进 502)。修这类缺陷**严禁**用加 `skipResponseSanitization` 消红——那是关掉脱敏保护。紧急跳过 `HUSKY_SKIP_CREDENTIAL_LEAK_IN_MESSAGE=1`,自检 `node scripts/check-credential-leak-in-message.mjs --self-test` + `node --test scripts/tests/check-credential-leak-in-message.test.mjs`
