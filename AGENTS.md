...
- **工具注册表完整性守门**(无 id,hook :~665): `scripts/check-tool-registry-integrity.mjs`(blocking,V3 #49 立) —— fs 工具有两个不等的可达面(本地注册 `mcp_server._TOOL_HANDLERS` / 浏览器委托 `apps/web/src/lib/workspace-tool-executor.ts`),本门把 `_FS_DEPENDENT_TOOLS` ⇄ `_DELEGATE_ONLY_TOOLS` ⇄ `_DELEGATE_ONLY_HINTS` ⇄ `_TOOL_ALIASES` 四面互相咬死:J1 双落点皆无即真幽灵 / J2 别名值域 ⊆ 本地注册表 / J3 别名键不得抢占真工具名 / J4 别名不得碰委托专有 / J5 委托专有三条定义 / J6 前端实现不可成死代码 / J7 提示表双向同步,任一破 exit 1;hook 以 `--quiet` 跑,真跑全量 rc=0(0s,fs 11 / 委托专有 4 / 别名 26 条 / 本地注册 86 / 前端实现 12);`--self-test` rc=0 ✅ 10 例(含「注释里的名字不得被误吸」变异验证——守门必须剥 Python 行注释取材,`_FS_DEPENDENT_TOOLS` 上方注释里写着已移出的 `list_files`);取材由脚本位置推导 + `--root <dir>` 通道供夹具,不靠 cwd;本门**无应急跳过之外的旁路**,紧急跳过 `HUSKY_SKIP_TOOL_REGISTRY_INTEGRITY=1`。
- **能力矩阵对账守门**(无 id,hook :~690): `scripts/check-capability-matrix.mjs`(blocking,V3 #57 立) —— ai-service 的 76 个 feature env(开关类 56 / 灰度类 7 / 门控类 13)以 `apps/ai-service/app/core/capability_matrix.py` 为台账单一事实源,本门扫描 app/ 全部 `os.environ.get(` 收集默认关 env 全集,与台账对账:**新增默认关 env 漏登即红**;取材剥 Python 行注释(注释里的同名 env 字符串不得误吸,同 #49 铁律);`--self-test` rc=0 ✅(红/绿夹具咬合);启动时 main.py lifespan 自动打印矩阵 + `GET /api/admin/capabilities` 查询(role_id≥1);真跑 rc=0(台账 76 / 扫描 553 py / 默认关 61 全对账);紧急跳过 `HUSKY_SKIP_CAPABILITY_MATRIX=1`。
