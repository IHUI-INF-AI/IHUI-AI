...
-- changeset bill:202103182
create table t_check_in_record
(
    id          bigint auto_increment comment '主键id',
    member_id   bigint      not null comment '会员id',
    type        varchar(20) not null comment '签到类型',
    create_time timestamp default current_timestamp comment '创建时间',
    update_time timestamp default current_timestamp on update current_timestamp comment '最后修改时间',
    primary key (id)
) comment '会员签到记录';

-- changeset bill:202104231
create table t_follow
(
    id               bigint auto_increment comment '主键id',
    member_id        bigint       not null comment '会员id',
    follow_member_id bigint       not null comment '关注的会员id',
    status           varchar(100) not null default 'follow' comment '状态',
    create_time      timestamp             default current_timestamp comment '创建时间',
    update_time      timestamp             default current_timestamp on update current_timestamp comment '最后修改时间',
    primary key (id)
) comment '会员签到记录';

-- changeset bill:202304231
alter table t_member add expire_time timestamp null comment '过期时间';
