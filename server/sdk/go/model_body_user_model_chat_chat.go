/*
ZHS Platform

ZHS Platform API - AI Agent Marketplace & Educational Platform

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zhs_api

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the BodyUserModelChatChat type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BodyUserModelChatChat{}

// BodyUserModelChatChat struct for BodyUserModelChatChat
type BodyUserModelChatChat struct {
	Model *string `json:"model,omitempty"`
	Messages []interface{} `json:"messages"`
	Temperature *float32 `json:"temperature,omitempty"`
	MaxTokens *int32 `json:"max_tokens,omitempty"`
	Stream *bool `json:"stream,omitempty"`
}

type _BodyUserModelChatChat BodyUserModelChatChat

// NewBodyUserModelChatChat instantiates a new BodyUserModelChatChat object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBodyUserModelChatChat(messages []interface{}) *BodyUserModelChatChat {
	this := BodyUserModelChatChat{}
	var model string = "gpt-4o-mini"
	this.Model = &model
	this.Messages = messages
	var temperature float32 = 0.7
	this.Temperature = &temperature
	var maxTokens int32 = 2048
	this.MaxTokens = &maxTokens
	var stream bool = false
	this.Stream = &stream
	return &this
}

// NewBodyUserModelChatChatWithDefaults instantiates a new BodyUserModelChatChat object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBodyUserModelChatChatWithDefaults() *BodyUserModelChatChat {
	this := BodyUserModelChatChat{}
	var model string = "gpt-4o-mini"
	this.Model = &model
	var temperature float32 = 0.7
	this.Temperature = &temperature
	var maxTokens int32 = 2048
	this.MaxTokens = &maxTokens
	var stream bool = false
	this.Stream = &stream
	return &this
}

// GetModel returns the Model field value if set, zero value otherwise.
func (o *BodyUserModelChatChat) GetModel() string {
	if o == nil || IsNil(o.Model) {
		var ret string
		return ret
	}
	return *o.Model
}

// GetModelOk returns a tuple with the Model field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BodyUserModelChatChat) GetModelOk() (*string, bool) {
	if o == nil || IsNil(o.Model) {
		return nil, false
	}
	return o.Model, true
}

// HasModel returns a boolean if a field has been set.
func (o *BodyUserModelChatChat) HasModel() bool {
	if o != nil && !IsNil(o.Model) {
		return true
	}

	return false
}

// SetModel gets a reference to the given string and assigns it to the Model field.
func (o *BodyUserModelChatChat) SetModel(v string) {
	o.Model = &v
}

// GetMessages returns the Messages field value
func (o *BodyUserModelChatChat) GetMessages() []interface{} {
	if o == nil {
		var ret []interface{}
		return ret
	}

	return o.Messages
}

// GetMessagesOk returns a tuple with the Messages field value
// and a boolean to check if the value has been set.
func (o *BodyUserModelChatChat) GetMessagesOk() ([]interface{}, bool) {
	if o == nil {
		return nil, false
	}
	return o.Messages, true
}

// SetMessages sets field value
func (o *BodyUserModelChatChat) SetMessages(v []interface{}) {
	o.Messages = v
}

// GetTemperature returns the Temperature field value if set, zero value otherwise.
func (o *BodyUserModelChatChat) GetTemperature() float32 {
	if o == nil || IsNil(o.Temperature) {
		var ret float32
		return ret
	}
	return *o.Temperature
}

// GetTemperatureOk returns a tuple with the Temperature field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BodyUserModelChatChat) GetTemperatureOk() (*float32, bool) {
	if o == nil || IsNil(o.Temperature) {
		return nil, false
	}
	return o.Temperature, true
}

// HasTemperature returns a boolean if a field has been set.
func (o *BodyUserModelChatChat) HasTemperature() bool {
	if o != nil && !IsNil(o.Temperature) {
		return true
	}

	return false
}

// SetTemperature gets a reference to the given float32 and assigns it to the Temperature field.
func (o *BodyUserModelChatChat) SetTemperature(v float32) {
	o.Temperature = &v
}

// GetMaxTokens returns the MaxTokens field value if set, zero value otherwise.
func (o *BodyUserModelChatChat) GetMaxTokens() int32 {
	if o == nil || IsNil(o.MaxTokens) {
		var ret int32
		return ret
	}
	return *o.MaxTokens
}

// GetMaxTokensOk returns a tuple with the MaxTokens field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BodyUserModelChatChat) GetMaxTokensOk() (*int32, bool) {
	if o == nil || IsNil(o.MaxTokens) {
		return nil, false
	}
	return o.MaxTokens, true
}

// HasMaxTokens returns a boolean if a field has been set.
func (o *BodyUserModelChatChat) HasMaxTokens() bool {
	if o != nil && !IsNil(o.MaxTokens) {
		return true
	}

	return false
}

// SetMaxTokens gets a reference to the given int32 and assigns it to the MaxTokens field.
func (o *BodyUserModelChatChat) SetMaxTokens(v int32) {
	o.MaxTokens = &v
}

// GetStream returns the Stream field value if set, zero value otherwise.
func (o *BodyUserModelChatChat) GetStream() bool {
	if o == nil || IsNil(o.Stream) {
		var ret bool
		return ret
	}
	return *o.Stream
}

// GetStreamOk returns a tuple with the Stream field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BodyUserModelChatChat) GetStreamOk() (*bool, bool) {
	if o == nil || IsNil(o.Stream) {
		return nil, false
	}
	return o.Stream, true
}

// HasStream returns a boolean if a field has been set.
func (o *BodyUserModelChatChat) HasStream() bool {
	if o != nil && !IsNil(o.Stream) {
		return true
	}

	return false
}

// SetStream gets a reference to the given bool and assigns it to the Stream field.
func (o *BodyUserModelChatChat) SetStream(v bool) {
	o.Stream = &v
}

func (o BodyUserModelChatChat) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BodyUserModelChatChat) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Model) {
		toSerialize["model"] = o.Model
	}
	toSerialize["messages"] = o.Messages
	if !IsNil(o.Temperature) {
		toSerialize["temperature"] = o.Temperature
	}
	if !IsNil(o.MaxTokens) {
		toSerialize["max_tokens"] = o.MaxTokens
	}
	if !IsNil(o.Stream) {
		toSerialize["stream"] = o.Stream
	}
	return toSerialize, nil
}

func (o *BodyUserModelChatChat) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"messages",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varBodyUserModelChatChat := _BodyUserModelChatChat{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varBodyUserModelChatChat)

	if err != nil {
		return err
	}

	*o = BodyUserModelChatChat(varBodyUserModelChatChat)

	return err
}

type NullableBodyUserModelChatChat struct {
	value *BodyUserModelChatChat
	isSet bool
}

func (v NullableBodyUserModelChatChat) Get() *BodyUserModelChatChat {
	return v.value
}

func (v *NullableBodyUserModelChatChat) Set(val *BodyUserModelChatChat) {
	v.value = val
	v.isSet = true
}

func (v NullableBodyUserModelChatChat) IsSet() bool {
	return v.isSet
}

func (v *NullableBodyUserModelChatChat) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBodyUserModelChatChat(val *BodyUserModelChatChat) *NullableBodyUserModelChatChat {
	return &NullableBodyUserModelChatChat{value: val, isSet: true}
}

func (v NullableBodyUserModelChatChat) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBodyUserModelChatChat) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


