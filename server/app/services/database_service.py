...
class DatabaseService:
    ...
    def update_operation_status(
        ...
        if op:
            op.status = status  # type: ignore[assignment]
            op.output_file = output_file  # type: ignore[assignment]
            op.error_message = error_message  # type: ignore[assignment]
            op.processing_time = processing_time  # type: ignore[assignment]
            op.completed_at = datetime.utcnow()  # type: ignore[assignment]
            db.commit()
            db.refresh(op)
        return op
    ...
    def revoke_certificate(db: Session, serial_number: str) -> bool:
        ...
        if cert:
            cert.is_revoked = True  # type: ignore[assignment]
            db.commit()
            return True
        return False
...
class UploadService:
    ...
    def update_upload_chunks(db: Session, upload_id: str, chunk_index: int) -> UploadRecord | None:
        ...
        if record:
            chunks: list[Any] = record.uploaded_chunks or []  # type: ignore[assignment]
            if chunk_index not in chunks:
                chunks.append(chunk_index)
                record.uploaded_chunks = chunks  # type: ignore[assignment]
                db.commit()
                db.refresh(record)
        return record
...
class ShareService:
    ...
    def increment_downloads(db: Session, share_id: str) -> ShareRecord | None:
        record = db.query(ShareRecord).filter(ShareRecord.share_id == share_id).first()
        if record:
            record.current_downloads += 1  # type: ignore[assignment]
            db.commit()
            db.refresh(record)
        return record
...
class FileVersionService:
    ...
    def rollback_to_version(db: Session, version_id: str) -> FileVersionRecord | None:
        ...
        version.is_current = True  # type: ignore[assignment]
        db.commit()
        db.refresh(version)
        return version
...
class UserService:
    ...
    def update_password(db: Session, user_id: int, password_hash: str) -> UserRecord | None:
        ...
        if user:
            user.hashed_password = password_hash  # type: ignore[assignment]
            db.commit()
            db.refresh(user)
        return user
