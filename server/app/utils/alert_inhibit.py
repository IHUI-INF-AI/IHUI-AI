            ...
            for inh in self._inhibits.values():
                if self._match(inh.target_labels, labels):
                    for a in active_alerts or []:
                        ...
                        a_severity = a.get("severity") or a_labels.get("severity")
                        if a_severity in ("CRITICAL", "HIGH") and self._match(inh.source_labels, a_labels):
                            ok = True
                            for k in inh.equal:
                                if labels.get(k) != a_labels.get(k):
                                    ok = False
                                    break
                            if ok:
