...
class Diff:
    ...
    main: Any
    shadow: Any
    ts: float = field(default_factory=time.time)  # type: ignore[operator]
...
class ShadowTable:
    ...
    def reconcile(self, pk: str, prefer_main: bool = True) -> dict[str, Any]:
        ...
        with self._lock:
            self._main[pk] = dict(truth)  # type: ignore[arg-type]
            self._shadow[pk] = dict(truth)  # type: ignore[arg-type]
        return dict(truth)  # type: ignore[arg-type]
