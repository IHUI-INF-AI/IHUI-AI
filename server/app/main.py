...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            from app.metrics_business import APP_CLOCK_TZ_EXPECTED, APP_CLOCK_TZ_OFFSET  # type: ignore[attr-defined]
    ...
    try:
        ...
        with _engine2.connect() as _conn:
            _cols = {c["name"] for c in _sa_inspect(_conn).get_columns("users")}
            if "email" not in _cols:
                _conn.execute(_sa_text("ALTER TABLE users ADD COLUMN email VARCHAR(100) NULL"))
                _conn.execute(_sa_text("CREATE INDEX IF NOT EXISTS ix_users_email ON users (email)"))
                _conn.commit()
                logger.info("Migration: added 'email' column to users table")
            else:
                logger.debug("Migration: users.email column already exists")
    except Exception as e:
        logger.warning(f"Email column migration skipped: {e}")

    # 启动本地 SMTP 服务器 (aiosmtpd, 免费无账号, 用于邮箱登录验证码投递)
    # 仅在 SMTP_HOST 未配置时启用 (生产环境配置 SMTP_HOST 后自动跳过)
    try:
        from app.config import settings as _settings

        if not (_settings.SMTP_HOST and _settings.SMTP_USER and _settings.SMTP_PASSWORD):
            from app.utils.local_smtp_server import start_local_smtp

            start_local_smtp()
            logger.info("Local SMTP server started (free mode, no account needed)")
        else:
            logger.info("SMTP configured, skip local SMTP server")
    except Exception as e:
        logger.warning(f"Local SMTP server start skipped: {e}")

    # Start cached expiration monitor + canary monitor bridge
    # canary_monitor_bridge 依赖 monitor_manager, 必须在 monitor 启动后启动
    try:
        from app.services.monitor_startup import monitor_manager

        await monitor_manager.start_all_monitors()
        logger.info("Monitor manager started (cached_expiration_monitor)")

        from app.services.canary_monitor_bridge import start_canary_monitor_bridge

        await start_canary_monitor_bridge()
        logger.info("Canary monitor bridge started")
    except Exception as e:
    ...
    try:
        from app.utils.local_smtp_server import stop_local_smtp
    ...
    # Stop canary monitor bridge + cached expiration monitor
    try:
        from app.services.canary_monitor_bridge import stop_canary_monitor_bridge

        await stop_canary_monitor_bridge()
    except Exception:
        pass
    try:
        from app.services.monitor_startup import monitor_manager

        await monitor_manager.stop_all_monitors()
    except Exception:
        pass
...
def create_app() -> FastAPI:
    ...
    try:
        ...
        try:
            ...
            # compat_routes 内路径已是完整路径 (/api/v1/..., /api/login/pwd/...),
            # 不能再加 prefix="/api/v1", 否则变成 /api/v1/api/v1/... 双重前缀导致 404
    ...
    # original paths.
    # Legacy /api/<domain>/ paths - 每个路由单独 try/except,
    # 避免一个 import 失败导致同 try 块内其余路由全部跳过 (历史 P0 bug:
    # audit.routes 模块不存在, 导致 9 个 legacy 路由全部未注册, 被 mock catch-all 拦截)
    _legacy_routers = [
        ("app.api.v1.pdf.pdf_routes", "/api/pdf", "Legacy PDF"),
        ("app.api.v1.upload.routes", "/api/upload", "Legacy Upload"),
        ("app.api.v1.version.routes", "/api/version", "Legacy Version"),
        ("app.api.v1.rbac.routes", "/api/rbac", "Legacy RBAC"),
        ("app.api.v1.system.audit", "/api/audit", "Legacy Audit"),
        ("app.api.v1.customer_service.customer_service_routes", "/api/customer-service", "Legacy CS"),
        ("app.api.v1.customer_service.customer_service_ws", "/api/customer-service", "Legacy CS WS"),
        ("app.api.v1.customer_service.ticket_routes", "/api/zhs_api_ticket", "Legacy Tickets"),
        ("app.api.v1.agent.routes", None, "Legacy Agent"),
        ("app.api.v1.agents.upload", None, "Legacy Agent Upload"),
        ("app.api.v1.docs.routes", "/api/docs", "Legacy Docs"),
    ]
    ...
    for _mod, _prefix, _tag in _legacy_routers:
        try:
            ...
            logger.error(f"Failed to mount legacy router {_mod}: {_e}")
    logger.info(f"Legacy /api/<domain> routers mounted ({_legacy_ok}/{len(_legacy_routers)})")
    ...
    # 2026-06-29 联调: file_upload_router 单独挂载到根路径 (/cozeZhsApi/file/*)
    # 必须在 mock catch-all 之前注册, 否则 /cozeZhsApi/file/upload/* 会被 mock 拦截
    # 前端 file.service.ts 调用 /cozeZhsApi/file/upload/{form|base64|octet}, 不带 /api/v1 前缀
    # 同时保留 api_router 中的挂载 (/api/v1/cozeZhsApi/file/*), 兼容两种路径
    try:
        from app.api.v1.content.file_upload import router as file_upload_root_router
        app.include_router(file_upload_root_router)
        logger.info("File upload router registered at root (/cozeZhsApi/file/upload/*)")
    except Exception as e:
        logger.error(f"Failed to register file_upload router at root: {e}")

    # AI 世界站点列表 (/bot/sites/kind) - 公开接口, 必须在 mock catch-all 之前注册
    # 前端 AiWorld.vue 通过 /api-kou/bot/sites/kind 调用, Vite proxy 原样转发到后端
    try:
        from app.api.v1.bot_sites import router as bot_sites_router
        app.include_router(bot_sites_router)
        logger.info("Bot sites router registered (/bot/sites/kind)")
    except Exception as e:
        logger.error(f"Failed to register bot_sites router: {e}")
