...
async def call_callback(
    ...
    with get_session() as db:
        try:
            ...
            db.add(log)
            db.flush()
            log.response_body = '{"code":0,"message":"ok"}'  # type: ignore[assignment]
            log.process_time = int((datetime.utcnow() - start).total_seconds() * 1000)  # type: ignore[assignment]
...
async def log_detail(lid: int):
    with get_session() as db:
        try:
            entry = db.query(CallBackLog).filter(CallBackLog.id == lid).first()
            if not entry:
            ...
            return success(
                {
                    ...
                    "create_time": entry.created_at.isoformat() if entry.created_at else None,
                }
            )
        except Exception as e:
