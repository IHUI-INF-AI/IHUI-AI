...
async def create_paper_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def publish_paper_endpoint(paper_id: int, user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def get_paper_endpoint(paper_id: int, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
    from app.services.edu_exam import get_paper
    result = get_paper(db, paper_id=paper_id)
    return success(data=result)

@router.get("/papers", summary="List papers")
async def list_papers_endpoint(page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
    from app.services.edu_exam import list_papers
    result = list_papers(db)
    return success(data=result)

@router.post("/questions", summary="Add question")
async def add_question_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def list_questions_endpoint(paper_id: int, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
...
async def start_exam_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def submit_exam_endpoint(record_id: int, user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def get_exam_record_endpoint(record_id: int, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
...
async def add_wrong_question_endpoint(question_id: int, user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def mark_mastered_endpoint(wrong_book_id: int, user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
