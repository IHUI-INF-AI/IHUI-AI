...
async def upload_resource_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def delete_resource_endpoint(resource_id: int, user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def get_resource_endpoint(resource_id: int, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
...
async def increment_download_endpoint(resource_id: int, payload: dict = {}, db: Session = Depends(_get_db)):
...
async def list_resources_endpoint(page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
