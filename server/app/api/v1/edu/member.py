...
async def create_member_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def update_member_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def get_member_by_id_endpoint(member_id: int, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
...
async def add_points_endpoint(user_id: int, payload: dict = {}, db: Session = Depends(_get_db), current_user_id: int = Depends(get_current_user_id)):
...
async def deduct_points_endpoint(user_id: int, payload: dict = {}, db: Session = Depends(_get_db), current_user_id: int = Depends(get_current_user_id)):
...
async def list_members_endpoint(page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
    from app.services.edu_member import list_members
    result = list_members(db)
    return success(data=result)

@router.post("/parents", summary="Bind parent")
async def bind_parent_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def unbind_parent_endpoint(user_id: int = Depends(get_current_user_id), payload: dict = {}, db: Session = Depends(_get_db)):
...
async def list_parent_children_endpoint(parent_user_id: int, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), db: Session = Depends(_get_db)):
