...
def image_text_watermark(
    ...
    if not input_path or not text:
        return {"code": 400, "message": "缺少 input_path 或 text", "data": None}
    ...
    if not data:
        return {"code": 500, "message": "添加水印失败", "data": None}
    return {"code": 0, "message": "ok", "data": {"size": len(data)}}
...
def image_logo_watermark(payload: dict = Body(...)):
    ...
    if not base_path or not logo_path:
        return {"code": 400, "message": "缺少 base_path 或 logo_path", "data": None}
...
def video_text_watermark(payload: dict = Body(...)):
    ...
    if not result:
        return {"code": 500, "message": "添加视频水印失败", "data": None}
    return {"code": 0, "message": "ok", "data": {"output_path": result}}
...
def video_logo_watermark(payload: dict = Body(...)):
    ...
    if not input_path or not logo_path:
        return {"code": 400, "message": "缺少 input_path 或 logo_path", "data": None}
    result = add_image_watermark_video(
        input_path=input_path, logo_path=logo_path, output_path=output_path,
        position=position,
    )
    if not result:
        return {"code": 500, "message": "添加视频 logo 水印失败", "data": None}
    return {"code": 0, "message": "ok", "data": {"output_path": result}}
