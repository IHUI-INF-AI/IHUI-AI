...
def dashboard_forecast(request: Request):
...
def dashboard_subscriptions_create(request: Request):
    return success(data={"id": ""})


@router.delete("/api/v1/dashboard/subscriptions/{sub_id}", summary="仪表盘订阅删除")
def dashboard_subscriptions_delete(sub_id: str):
...
def login_pwd_sms_verify(phone: str = Query(None), code: str = Query(None)):
...
def login_pwd_verify(phone: str = Query(None), code: str = Query(None)):
    from app.utils.sms_util import verify_sms_code
    ok = verify_sms_code(phone or "", code or "")
    return success({"valid": ok})
