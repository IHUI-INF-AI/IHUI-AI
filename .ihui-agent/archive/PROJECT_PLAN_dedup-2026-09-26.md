...
# PROJECT_PLAN.md 去重:被删的 done 侧重复登记逐字归档(2026-09-26)

每段都是计划台账里被删掉的重复登记行**原文,逐字未改**。存活条目已把其中仍然有效的独有内容并入正文
(行尾「归并补 …」)。本件同时是守门 71 的豁免面("archive 里找得到 ⇒ 不算丢"),必须与主文档同枚提交。

### 被删副本 @ 原第 6040-6040 行(族主键 O39#O39三枚提交;存活副本 @ 第 486-486 行)
...
### 被删副本 @ 原第 652-652 行(族主键 D29#D29团队级知识引擎;存活副本 @ 第 653-653 行)
...
### 被删副本 @ 原第 9870-9870 行(族主键 D29#D29团队级知识引擎;存活副本 @ 第 653-653 行)
...
### 被删副本 @ 原第 10133-10133 行(族主键 D29#D29团队级知识引擎;存活副本 @ 第 653-653 行)

- [x] ✅(2026-09-26) D29 团队级知识引擎:记忆/Repo Wiki/知识卡云端共享+成员修正+过程审计(对标 Qoder 1.0,官方实证输入 token -40%)(G-35) 〔2026-09-26 摘牌:本票已落地(详见台账内 ✅ 详情行 —— schema 四表 + 迁移 idx=291 + 服务层 max(成员/团队/创建者) 且 restricted 不认团队默认档 + 12 条显式路径零公开面 + api-client/web 页面 + 注册对账 5 例 + 66 键×5 语,测试 36+5 全绿)。本行是派单租约/裸副本,保留编号不删行〕

### 被删副本 @ 原第 657-657 行(族主键 D31#D31设计稿转码;存活副本 @ 第 655-655 行)
...
### 被删副本 @ 原第 658-658 行(族主键 D31#D31设计稿转码;存活副本 @ 第 655-655 行)
...
### 被删副本 @ 原第 10135-10135 行(族主键 D31#D31设计稿转码;存活副本 @ 第 655-655 行)
...
### 被删副本 @ 原第 654-654 行(族主键 D30#D30无人值守修复闭环;存活副本 @ 第 656-656 行)
...
### 被删副本 @ 原第 10134-10134 行(族主键 D30#D30无人值守修复闭环;存活副本 @ 第 656-656 行)

- [x] ✅(2026-09-26) D30 无人值守修复闭环:GitHub issue/代码扫描告警/失败测试→automations 定时认领修复→PR 回帖(对标 QoderWake;与 D14/D15 协同)(G-36) 〔2026-09-26 摘牌:本票已落地(状态机 + 迁移审计 + Redis 锁互斥认领、无 Redis 即 fail-closed + 回帖只经 D15 App 通道 + 定时认领环;15+9 例全绿,api typecheck 0 错。运行前提:GITHUB_APP_ID+私钥且 installation 授 issues:write/pull_requests:write,IHUI_AUTOMATIONS_ENABLED=true)。本行是派单租约/裸副本,保留编号不删行〕

### 被删副本 @ 原第 2505-2505 行(族主键 O13b#O13b第二段;存活副本 @ 第 2503-2503 行)
...
### 被删副本 @ 原第 2507-2507 行(族主键 O13b#O13b第二段;存活副本 @ 第 2503-2503 行)

- [x] ✅(2026-09-25) O13b 第二段(收敛本身,5 条可核算):① 34 个白名单文件逐个迁移到集中封装并**删条目**;② 删掉 2 处本地重定义 `requireAdmin`(`earnings-routes.ts:137`、`security.ts:74`);③ `internalUserRoleId` 通道并入同一封装并补提权断言;④ 第 53 项升 blocking;⑤ `admin.ts:124` 统一 preHandler 收编进 `require-permission`。另:部署机需运维 `ALTER ROLE ihui_app PASSWORD` + 配 `DATABASE_APP_URL`,之后才评估 `ENABLE ROW LEVEL SECURITY` 〔2026-09-25 翻勾:5 条可核算项经代理逐条以代码现值复核,已由 cd2d8f8f32d 等 5 批先序落地;守门53 全量 exit 0;残余债务(idor-guard 叶子模块/常量形态散落)已各自登记为独立票〕

### 被删副本 @ 原第 6817-6817 行(族主键 O13b#O13b第二段;存活副本 @ 第 2503-2503 行)

- [x] ✅(2026-09-25) O13b 第二段(收敛本身,5 条可核算):① 34 个白名单文件逐个迁移到集中封装并**删条目**;② 删掉 2 处本地重定义 `requireAdmin`(`earnings-routes.ts:137`、`security.ts:74`);③ `internalUserRoleId` 通道并入同一封装并补提权断言;④ 第 53 项升 blocking;⑤ `admin.ts:124` 统一 preHandler 收编进 `require-permission`。另:部署机需运维 `ALTER ROLE ihui_app PASSWORD` + 配 `DATABASE_APP_URL`,之后才评估 `ENABLE ROW LEVEL SECURITY` 〔2026-09-25 翻勾:5 条可核算项经代理逐条以代码现值复核,已由 cd2d8f8f32d 等 5 批先序落地;守门53 全量 exit 0;残余债务(idor-guard 叶子模块/常量形态散落)已各自登记为独立票〕

### 被删副本 @ 原第 7114-7114 行(族主键 O13b#O13b第二段;存活副本 @ 第 2503-2503 行)

- [x] ✅(2026-09-25) O13b 第二段(收敛本身,5 条可核算):① 34 个白名单文件逐个迁移到集中封装并**删条目**;② 删掉 2 处本地重定义 `requireAdmin`(`earnings-routes.ts:137`、`security.ts:74`);③ `internalUserRoleId` 通道并入同一封装并补提权断言;④ 第 53 项升 blocking;⑤ `admin.ts:124` 统一 preHandler 收编进 `require-permission`。另:部署机需运维 `ALTER ROLE ihui_app PASSWORD` + 配 `DATABASE_APP_URL`,之后才评估 `ENABLE ROW LEVEL SECURITY` 〔2026-09-25 翻勾:5 条可核算项经代理逐条以代码现值复核,已由 cd2d8f8f32d 等 5 批先序落地;守门53 全量 exit 0;残余债务(idor-guard 叶子模块/常量形态散落)已各自登记为独立票〕

### 被删副本 @ 原第 2511-2511 行(族主键 O21#O21资金链与文件版本面的属主谓词补齐;存活副本 @ 第 2512-2517 行)
...
### 被删副本 @ 原第 2525-2525 行(族主键 O20c#O20c生产pghba;存活副本 @ 第 2523-2523 行)

- [x] ✅(2026-09-26) O20c 生产 `pg_hba.conf` 本地链路是 **trust**(127.0.0.1 免密即可以 superuser 连接)。这与 O13"非超级用户角色 + 应用闸"的方向直接对冲:应用角色隔离挡住的是"经应用连上来的路径",挡不住"本机任意进程直连 8810"。改成 `scram-sha-256` 会同时影响每日备份任务(`IHUI-PG-BACKUP`)、psql 运维脚本、部署循环,爆炸半径是**本机全部数据库消费方**,需 owner 决策后再动;本会话只读确认,未擅改。 〔2026-09-26 翻勾(生产机 ssh 实测,三条环回路径逐一验):`psql -w -U postgres -h 127.0.0.1|-h ::1|-h localhost -p 8810 -d postgres` **三条全部** `fe_sendauth: no password supplied` ⇒ "免密即可以 superuser 连接"这一前提已不成立;另测应用角色本身 `role=ihui super=f createrole=f`(全库仅 1 个 superuser = postgres),且 `show hba_file`/`show data_directory` 对它 `permission denied to examine`(要 pg_read_all_settings)⇒ 连"用应用凭据读配置"都不行。**未做的**:"收紧文件内容"这一步不存在了(行为已达标),但 `local` 行与 Windows 上同名服务的 `--pg-data` 落点我**没能读到文件本体**(非超级用户无权 examine),所以"文件里到底还有没有 trust 行"是**行为证否 + 文件未核**两半,登记为已闭环的依据是攻击面(免密 TCP 环回)已消失。〕 〔2026-09-26 追补,把"文件未核"那一半也翻实:经 `sc qc IHUI-PG` 量到服务是 `pg_ctl.exe runservice -D "D:DevEnvdatapgdata"`,读该目录下 `pg_hba.conf` 全文**有效规则共 6 条,认证方法逐条都是 `scram-sha-256`**(local all / host all 127.0.0.1/32 / host all ::1/128 + replication 同三条)⇒ **零 trust 行**,与行为测试互为印证;上一句"local 行未核"作废,不改写原文只在此推翻。生产上留下的 5 个只读探针脚本已当场删除(`dir /b D:DevEnvTempprod-*.ps1` 复跑无输出)。〕

### 被删副本 @ 原第 2527-2527 行(族主键 O20f#O20f并行会话tree重置事件;存活副本 @ 第 2528-2528 行)
...
### 被删副本 @ 原第 7565-7566 行(族主键 O20f#O20f并行会话tree重置事件;存活副本 @ 第 2528-2528 行)
...
### 被删副本 @ 原第 8340-8340 行(族主键 O20f#O20f并行会话tree重置事件;存活副本 @ 第 2528-2528 行)
...
### 被删副本 @ 原第 10502-10502 行(族主键 D15#D15GitHubApp;存活副本 @ 第 2565-2565 行)
...
### 被删副本 @ 原第 2642-2642 行(族主键 D37#D37内联系统注入条+上下文装配查看器;存活副本 @ 第 2639-2639 行)
...
### 被删副本 @ 原第 2782-2782 行(族主键 D39#D39错误重试可观测+额度耗尽处置动作族;存活副本 @ 第 2781-2781 行)
...
### 被删副本 @ 原第 6906-6906 行(族主键 D39#D39错误重试可观测+额度耗尽处置动作族;存活副本 @ 第 2781-2781 行)
...
### 被删副本 @ 原第 7569-7569 行(族主键 D39#D39错误重试可观测+额度耗尽处置动作族;存活副本 @ 第 2781-2781 行)
...
### 被删副本 @ 原第 8342-8342 行(族主键 D39#D39错误重试可观测+额度耗尽处置动作族;存活副本 @ 第 2781-2781 行)
...
### 被删副本 @ 原第 2807-2807 行(族主键 D48#D48本地会话数据主权与加密;存活副本 @ 第 2808-2812 行)
...
### 被删副本 @ 原第 2814-2814 行(族主键 D48#D48本地会话数据主权与加密;存活副本 @ 第 2808-2812 行)

- [x] ✅(2026-09-24) **D48 本地会话数据主权与加密(G-56)**:桌面端本地缓存加密(对标 Trae SQLCipher;我方优势:导入侧已有 redact_secrets 实测 0.07% 命中)。**验收**:静态盘 grep 明文会话为 0 + 解锁失败降级可读空态不崩 + 密钥不落仓(§5d)

### 被删副本 @ 原第 6907-6907 行(族主键 D48#D48本地会话数据主权与加密;存活副本 @ 第 2808-2812 行)
...
### 被删副本 @ 原第 2835-2835 行(族主键 D55#D55机器代批决策条;存活副本 @ 第 2836-2846 行)
...
### 被删副本 @ 原第 6908-6908 行(族主键 D55#D55机器代批决策条;存活副本 @ 第 2836-2846 行)
...
### 被删副本 @ 原第 6909-6909 行(族主键 D55#D55机器代批决策条;存活副本 @ 第 2836-2846 行)
...
### 被删副本 @ 原第 7571-7571 行(族主键 D55#D55机器代批决策条;存活副本 @ 第 2836-2846 行)
...
### 被删副本 @ 原第 2854-2854 行(族主键 D58#D58工具类目聚合层;存活副本 @ 第 2855-2855 行)
...
### 被删副本 @ 原第 2866-2866 行(族主键 D62#D62语音字幕与讨论纪要;存活副本 @ 第 2865-2865 行)
...
### 被删副本 @ 原第 6910-6910 行(族主键 D62#D62语音字幕与讨论纪要;存活副本 @ 第 2865-2865 行)
...
### 被删副本 @ 原第 2871-2871 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)
...
### 被删副本 @ 原第 9606-9606 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)
...
### 被删副本 @ 原第 9607-9607 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 9608-9608 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 9609-9609 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 9610-9610 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 9634-9634 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)
...
### 被删副本 @ 原第 9635-9635 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔PROGRESS 2026-09-25: ②图片预览器生产挂载/③思考卡双态标题/⑥goal对照 完成(commit 同日入库,web vitest 44/44);①Credits热力图待后端日聚合(§24)、④八态待SSE子任务帧、⑤问卷卡待扩端点(§24)〕 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 9636-9636 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔PROGRESS 2026-09-25: ②图片预览器生产挂载/③思考卡双态标题/⑥goal对照 完成(commit 同日入库,web vitest 44/44);①Credits热力图待后端日聚合(§24)、④八态待SSE子任务帧、⑤问卷卡待扩端点(§24)〕 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 9637-9637 行(族主键 D64#D64小元素包;存活副本 @ 第 2870-2870 行)

- [x] ✅(2026-09-26) **D64 小元素包(G-72/75/77/79/82/83)**:①Credits 热力图(单日消耗 + 会话/热力切换);②图片预览器补翻页/第 N·M 张/缩放比例/保存与复制成败;③思考卡双态标题(有思考→「思考过程」,无思考→「使用了 N 个引用」);④后台子任务八态与"停止失败"文案;⑤反馈问卷化(把 D49①的 toast 兜底升级为「这次回复有没有帮你解决问题?」结构化落库);⑥**goal 卡先自证再定档**——逐字段比对我方 `ai/goal-card.tsx` 与 Trae/Qoder 五态·操作·时长格式,**未核对前不列差距**(第 5 轮已因此拦下一条幻影差距)。**验收**:每项独立用例;⑥必须先产出对照表再决定做/不做 〔PROGRESS 2026-09-25: ②图片预览器生产挂载/③思考卡双态标题/⑥goal对照 完成(commit 同日入库,web vitest 44/44);①Credits热力图待后端日聚合(§24)、④八态待SSE子任务帧、⑤问卷卡待扩端点(§24)〕 〔【归并】重复登记副本(2026-09-26):同主键的另一条登记在 L2866,派单以那条为准,本行不再单独派单。〕 〔【归并】D64 落账:复测 2026-09-26: 六项均有实现且生产面挂载(热力图在 settings/billing/page.tsx:26 被 import;反馈问卷卡挂在 MessageItem;后台子任务八态含 stopFailed;goal 卡对照在 docs/plan-audit)。〕

### 被删副本 @ 原第 3050-3050 行(族主键 D66#D66编辑并重新发送=文件回退组合操作;存活副本 @ 第 3047-3047 行)
...
### 被删副本 @ 原第 3051-3051 行(族主键 D67#D67额度归属分型与折扣倒计时;存活副本 @ 第 3048-3048 行)
...
### 被删副本 @ 原第 3052-3052 行(族主键 D67#D67额度归属分型与折扣倒计时;存活副本 @ 第 3048-3048 行)

- [x] ✅(2026-09-24)  **D67 额度归属分型与折扣倒计时(G-90,与 D56 合并)**:额度错误按**归属**分四类标题+对应动作(个人今日/免费模型今日/团队·需管理员/计费组·Credits 上限)+ 三动作族 + `低峰折扣进行中`/`{{time}}后进入低峰折扣` 倒计时。**判据用已复现原文**;实施前须与"不充值可用心智"边界对表(免费档可用时不得弹诱导)。**验收**:四型各一用例 + 倒计时纯函数测试 __收口(2026-09-24):quota-ownership.ts(四型归属穷尽 switch 零 default(personalDaily/freeModelDaily/teamAdmin/billingGroupCredits,后两类 escalate=true) + 三动作族 viewUsage/switchFreeModel/upgradeOrAdmin + **discountCountdown 左闭右开**含恰好开始/恰好结束/跨午夜 23:00→次日01:00/非法区间/NaN 全边界 + formatDurationHuman 单位词可替换 + **「不充值可用心智」机器判据:shouldShowOwnershipCard 仅当次因额度被拒才显示(预防性展示一律 false)、isInducementRisk 免费档可用×personalDaily 判诱导且判定层剔除付费动作(非渲染层自觉)** + fromErrorCode 与 D71 error-catalog **两道闸协同**(先过 resolveErrorCatalog 防陈旧映射、再过窄映射白名单;任一不过返回 null 不硬塞;RATE_LIMITED 等非归属码刻意不入))+ quota-ownership-card.tsx 纯展示 + ai.pane.quotaOwnership 13 键×5 语言。shared 32 + web 18 全绿。__剩余__:宿主接线(错误卡挂载,动作对接 D39 既有 /points /vip /models/usage 通道)、团队/计费组 errorCode 待后端产出、折扣窗口数据面来源__

### 被删副本 @ 原第 6818-6818 行(族主键 D67#D67额度归属分型与折扣倒计时;存活副本 @ 第 3048-3048 行)
...
### 被删副本 @ 原第 6911-6911 行(族主键 D67#D67额度归属分型与折扣倒计时;存活副本 @ 第 3048-3048 行)
...
### 被删副本 @ 原第 6912-6912 行(族主键 D67#D67额度归属分型与折扣倒计时;存活副本 @ 第 3048-3048 行)
...
### 被删副本 @ 原第 7573-7573 行(族主键 D67#D67额度归属分型与折扣倒计时;存活副本 @ 第 3048-3048 行)
...
### 被删副本 @ 原第 6927-6927 行(族主键 D69#D69输入区文案族补齐;存活副本 @ 第 3055-3055 行)
...
### 被删副本 @ 原第 3059-3059 行(族主键 D71#D71统一Turn状态词汇表+错误分类族;存活副本 @ 第 3061-3062 行)
...
### 被删副本 @ 原第 3064-3064 行(族主键 D72#D72Worktree生命周期对话流卡;存活副本 @ 第 3063-3063 行)
...
### 被删副本 @ 原第 644-644 行(族主键 D73#D73多任务窗格;存活副本 @ 第 3065-3067 行)
...
### 被删副本 @ 原第 7564-7564 行(族主键 D73#D73多任务窗格;存活副本 @ 第 3065-3067 行)

- [x] ✅(2026-09-25) **D73 多任务窗格(G-100)**:向右/向下拆分、最大化还原、**联动调整相邻窗格**、空窗格"从侧栏拖入一个任务"、Fork 失败提示。落点在既有 `ai-side-panel` + `work-panel` 之上做分屏容器,**禁止**新建第二套会话承载体系(与 D52/D68 协同)。**验收**:拆分/拖入/Fork 失败三用例 + 拖拽复用 D22 已建的 `application/x-ihui-conversation` 通道 〔2026-09-25 翻勾:五项(拆分/最大化/联动调整/拖入/Fork失败)经代理逐项核验已由先序全量落地,multi-pane 35/35 + web 51/51 全绿,parity OK〕

### 被删副本 @ 原第 7602-7602 行(族主键 D73#D73多任务窗格;存活副本 @ 第 3065-3067 行)

- [x] ✅(2026-09-25)**D73 多任务窗格(G-100)**:向右/向下拆分、最大化还原、**联动调整相邻窗格**、空窗格"从侧栏拖入一个任务"、Fork 失败提示。落点在既有 `ai-side-panel` + `work-panel` 之上做分屏容器,**禁止**新建第二套会话承载体系(与 D52/D68 协同)。**验收**:拆分/拖入/Fork 失败三用例 + 拖拽复用 D22 已建的 `application/x-ihui-conversation` 通道 〔2026-09-25 翻勾:五项(拆分/最大化/联动调整/拖入/Fork失败)经代理逐项核验已由先序全量落地,multi-pane 35/35 + web 51/51 全绿,parity OK〕

### 被删副本 @ 原第 7730-7730 行(族主键 D73#D73多任务窗格;存活副本 @ 第 3065-3067 行)

- [x] ✅(2026-09-25) **D73 多任务窗格(G-100)**:向右/向下拆分、最大化还原、**联动调整相邻窗格**、空窗格"从侧栏拖入一个任务"、Fork 失败提示。落点在既有 `ai-side-panel` + `work-panel` 之上做分屏容器,**禁止**新建第二套会话承载体系(与 D52/D68 协同)。**验收**:拆分/拖入/Fork 失败三用例 + 拖拽复用 D22 已建的 `application/x-ihui-conversation` 通道 〔2026-09-25 翻勾:五项(拆分/最大化/联动调整/拖入/Fork失败)经代理逐项核验已由先序全量落地,multi-pane 35/35 + web 51/51 全绿,parity OK〕

### 被删副本 @ 原第 3071-3071 行(族主键 D75#D75侧边任务生命周期;存活副本 @ 第 3070-3070 行)
...
### 被删副本 @ 原第 651-651 行(族主键 D76#D76产物归属turn与产物面板分型;存活副本 @ 第 3072-3072 行)
...
### 被删副本 @ 原第 3080-3080 行(族主键 D78#D78连接器授权卡;存活副本 @ 第 3079-3079 行)
...
### 被删副本 @ 原第 6913-6913 行(族主键 D78#D78连接器授权卡;存活副本 @ 第 3079-3079 行)
...
### 被删副本 @ 原第 3084-3084 行(族主键 D80#D80两条待自证定档;存活副本 @ 第 3083-3083 行)
...
### 被删副本 @ 原第 6914-6914 行(族主键 D80#D80两条待自证定档;存活副本 @ 第 3083-3083 行)
...
### 被删副本 @ 原第 3098-3098 行(族主键 D82#D82就地润色与失败保稿;存活副本 @ 第 3095-3095 行)
...
### 被删副本 @ 原第 3141-3141 行(族主键 D89#D89输入源与队列小项打包;存活副本 @ 第 3142-3142 行)
...
### 被删副本 @ 原第 3143-3143 行(族主键 D89#D89输入源与队列小项打包;存活副本 @ 第 3142-3142 行)

- [x] ✅(2026-09-24 渲染面) **D89 输入源与队列小项打包(G-119/G-121/G-122)**:①**智能快照**(`附加 {appName}`、`启用智能快照` + 首次使用引导 + 失败态)与**添加远程文件/照片**分流;②队列与引导**命令化**(`将提示加入队列`/`引导提示` 作为命令项 + 命令描述)与 **Undo**(`已恢复队列中的消息`/`已恢复排队的消息`),补进 D38;③**记忆引用计数条**(`{count} 条记忆引用` + tooltip`引用的记忆`)与 `已在 {totalTime} 内达成目标` 的 goal 成就耗时条。**验收**:各三态用例;②须与 W27 预备消息/侧问队列语义不冲突 **进度(2026-09-24)**:②命令化=command-registry 加 queuePrompt/steerPrompt 声明与三映射(接线点:ui-action-registry 三处 switch 闭合约约+W27 FIFO/steer 既有通道),Undo 二键进词表动作待接线;③goal 成就耗时条已渲染(3 用例),记忆引用计数条登记数据面缺失(引用类字段全仓 0 命中);①智能快照 blocked(截屏原语通道不对+per-app 枚举缺失,解阻判据=composer 截图直插或 desktop attach_screenshot 桥+窗口枚举)。词表键已插工作区随词表收口会话收编。

### 被删副本 @ 原第 3153-3153 行(族主键 D90#D90预览降级三态与"文件已更新"提示;存活副本 @ 第 3146-3146 行)
...
### 被删副本 @ 原第 3159-3159 行(族主键 D90#D90预览降级三态与"文件已更新"提示;存活副本 @ 第 3146-3146 行)

- [x] ✅(2026-09-23) **D90 预览降级三态与"文件已更新"提示(G-123)**:对标 Qoder `工具记录内容` / **`无法读取当前文件，已展示工具记录中的内容。`** / `这次文件变更没有记录完整内容。` / `没有可预览内容。` 四级降级,加 `文件已更新`+`刷新以查看最新内容`+关闭提示。与 D33 同批(降级依据正是"工具记录里存了什么")。**验收**:四级各一用例 + 断言降级时**明确告知展示的是历史快照而非当前文件**(防用户误以为看到最新)

### 被删副本 @ 原第 6918-6918 行(族主键 D90#D90预览降级三态与"文件已更新"提示;存活副本 @ 第 3146-3146 行)
...
### 被删副本 @ 原第 6922-6922 行(族主键 D90#D90预览降级三态与"文件已更新"提示;存活副本 @ 第 3146-3146 行)
...
### 被删副本 @ 原第 7578-7578 行(族主键 D90#D90预览降级三态与"文件已更新"提示;存活副本 @ 第 3146-3146 行)
...
### 被删副本 @ 原第 8346-8346 行(族主键 D90#D90预览降级三态与"文件已更新"提示;存活副本 @ 第 3146-3146 行)
...
### 被删副本 @ 原第 3154-3154 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 3160-3160 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)

- [x] ✅(2026-09-24)  **D91 四类文档批注锚点分型(G-124,扩展 D87)**:Qoder 的批注不是单一"选中文字",而是四种定位坐标——PDF `PDF 第 {page} 页`、PPTX `第 {slide} 张 · {element}` + `批注 {element}`、DOCX `文档第 {page} 页`、XLSX `{sheet} · {range}` + `已选择 {range}`;统一动作是 `描述希望 Agent 修改或检查的内容` → **添加到任务**,并支持 取消/删除。落点 `artifact-canvas` + 圈选事件族(D22 的 `ihui:add-text-reference` 同机制),**禁止**为四类各写一套批注状态机。**验收**:四坐标各一用例 + 回流成任务输入 + 删除/取消态 __收口(2026-09-24):自证 D87 reply-annotation 是**回复文本选区**批注、四类文档坐标全仓零形状;annotation-anchors.ts(四类坐标 anchorLabel 逐字对齐原文(PDF 第{page}页/第{slide}张·{element}/批注{element}/文档第{page}页/{sheet}·{range}/已选择{range}) + **四类共用单一状态机**(反证:四类走同一动作序列状态轨迹逐点一致,deleted 上五动作原地不动) + toTaskInput 回流「描述希望 Agent 修改或检查的内容」→ 添加到任务 + PPTX 无 element/XLSX 无 range 退化键防 undefined)+ annotation-anchor-label.tsx 纯展示 + ai.pane.annotationAnchors 14 键×5 语言。shared 19 + web 16 全绿。__剩余__:artifact-canvas 接线(onAddToTask 已留回调)与 PPTX/XLSX 坐标提取数据面待另票__

### 被删副本 @ 原第 3161-3161 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)

- [x] ✅(2026-09-24)  **D91 四类文档批注锚点分型(G-124,扩展 D87)**:Qoder 的批注不是单一"选中文字",而是四种定位坐标——PDF `PDF 第 {page} 页`、PPTX `第 {slide} 张 · {element}` + `批注 {element}`、DOCX `文档第 {page} 页`、XLSX `{sheet} · {range}` + `已选择 {range}`;统一动作是 `描述希望 Agent 修改或检查的内容` → **添加到任务**,并支持 取消/删除。落点 `artifact-canvas` + 圈选事件族(D22 的 `ihui:add-text-reference` 同机制),**禁止**为四类各写一套批注状态机。**验收**:四坐标各一用例 + 回流成任务输入 + 删除/取消态 __收口(2026-09-24):自证 D87 reply-annotation 是**回复文本选区**批注、四类文档坐标全仓零形状;annotation-anchors.ts(四类坐标 anchorLabel 逐字对齐原文(PDF 第{page}页/第{slide}张·{element}/批注{element}/文档第{page}页/{sheet}·{range}/已选择{range}) + **四类共用单一状态机**(反证:四类走同一动作序列状态轨迹逐点一致,deleted 上五动作原地不动) + toTaskInput 回流「描述希望 Agent 修改或检查的内容」→ 添加到任务 + PPTX 无 element/XLSX 无 range 退化键防 undefined)+ annotation-anchor-label.tsx 纯展示 + ai.pane.annotationAnchors 14 键×5 语言。shared 19 + web 16 全绿。__剩余__:artifact-canvas 接线(onAddToTask 已留回调)与 PPTX/XLSX 坐标提取数据面待另票__

### 被删副本 @ 原第 6819-6819 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 6820-6820 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 6917-6917 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 6919-6919 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 6921-6921 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 6923-6923 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 7577-7577 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 7579-7579 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 8345-8345 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 8347-8347 行(族主键 D91#D91四类文档批注锚点分型;存活副本 @ 第 3148-3148 行)
...
### 被删副本 @ 原第 3157-3157 行(族主键 D92#D92插件/MCP视图失败分类学;存活副本 @ 第 3149-3149 行)
...
### 被删副本 @ 原第 3169-3169 行(族主键 D92#D92插件/MCP视图失败分类学;存活副本 @ 第 3149-3149 行)
...
### 被删副本 @ 原第 7055-7055 行(族主键 D92#D92插件/MCP视图失败分类学;存活副本 @ 第 3149-3149 行)

- [x] ✅(2026-09-23) **D92 插件/MCP 视图失败分类学(G-125)**:Qoder 有 **15 种**插件视图失败文案(资源未找到/运行时异常/未注册启动入口/入口无效/依赖模块未提供/资源超限/环境初始化失败/已停用/后端超时/后端退出/未提供所需能力/崩溃测试)+ `错误码:{errorCode}` + `重新加载插件视图` 统一恢复动作。我方 MCP 面板现在只会笼统"加载失败"→ 建立**错误码→分类标题→建议动作**表(与 D71 错误分类族共用一张表,不另起),**验收**:15 类映射 + 恢复按钮始终可用 + 未知码回落通用态不误报

### 被删副本 @ 原第 7056-7056 行(族主键 D92#D92插件/MCP视图失败分类学;存活副本 @ 第 3149-3149 行)

- [x] ✅(2026-09-24) **D92 插件/MCP 视图失败分类学(G-125)**:Qoder 有 **15 种**插件视图失败文案(资源未找到/运行时异常/未注册启动入口/入口无效/依赖模块未提供/资源超限/环境初始化失败/已停用/后端超时/后端退出/未提供所需能力/崩溃测试)+ `错误码:{errorCode}` + `重新加载插件视图` 统一恢复动作。我方 MCP 面板现在只会笼统"加载失败"→ 建立**错误码→分类标题→建议动作**表(与 D71 错误分类族共用一张表,不另起),**验收**:15 类映射 + 恢复按钮始终可用 + 未知码回落通用态不误报

### 被删副本 @ 原第 3178-3178 行(族主键 D94#D94失败诊断脱敏交接包;存活副本 @ 第 3176-3176 行)
...
### 被删副本 @ 原第 3182-3182 行(族主键 D96#D96对话内写作块;存活副本 @ 第 3181-3181 行)
...
### 被删副本 @ 原第 7057-7057 行(族主键 D100#D100计费自助状态机;存活副本 @ 第 3198-3198 行)
...
### 被删副本 @ 原第 7058-7058 行(族主键 D102#D102对话移交工作树;存活副本 @ 第 3209-3209 行)
...
### 被删副本 @ 原第 3237-3237 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 3238-3238 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 6821-6821 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 6822-6822 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 7059-7059 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-24) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。 **对账改判(2026-09-24,HEAD 取证)**:四端 citations|onSteer 命中 extension 11 / miniapp 26 / mobile-rn 31 / cli 8,且各端有显式 D106 落点注释(推翻本条"四端 0 命中")。

### 被删副本 @ 原第 7115-7115 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-24) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。**进度(2026-09-24)**:① **三端 onSteer 消费落地**(cli/miniapp-taro/mobile-rn,各自 streamChat 调用点注册 + 渲染"引导已生效"交代,词表 15 文件直入正仓 packages/i18n/messages/{cli,miniapp-taro,mobile-rn} 五语言、译法与 web steerNoticeBar 逐字同源,端内 override 已摘除);测试 cli 4/4 + miniapp 7/7 + rn 9/9 全绿,三端文件域 tsc 0 错误;`onSteer` 命中 cli/miniapp/rn 由 0 变非 0。② **extension 已补齐(2026-09-24 第三轮,前述"无通道"结论系分母路径错误:extension 代码在 entrypoints/ 非 src/,该端早有 onCitations/onInjectionApplied/onRetryScheduled 消费)**:ChatPage 注册 onSteer(逐字段承接/空文本防御/8 条封顶)、MessageContent 渲染 steer-notice 交代条、词表五语言 steerNoticeTitle(与 web steerNoticeBar 同源)、@ihui/types ChatMessage 加 steerNotices 字段,steer-notice.test.tsx 4/4 过、tsc 0 错误。③ **守门 57 已闭合**:steer-injection-disclosure 条目入清单(implemented 32→33,13 锚点:ai-service 收集点/api schema/api-client 回调/五端消费与渲染),check-chat-element-coverage.mjs 实跑 EXIT 0(清单 125 条一致)。④ **历史灌回三端闭合(2026-09-24 第四轮)**:web readSteerAppliedFromMetadata(第一轮)+ miniapp backfillSteerNoticesFromMetadata(types.ts 守卫同 web/8 封顶/全坏不写,chat.tsx 两处历史恢复点接入)+ mobile-rn readSteerAppliedFromMetadata(chat-render-model 纯函数,双入口历史加载接入;顺带修复 ChatScreen toChatScreenMessage 不透传 steerNotices 导致 live 渲染死代码的缺陷);测试 miniapp 17/17 + rn 16/16,两端文件域 tsc 0。miniapp 注意:该端无服务端会话消息拉取(历史走本地存储),跨端 metadata 读回需先接服务端历史接口(读回函数已备好,行带 metadata 进来即可消费)。 **对账改判(2026-09-24,HEAD 取证)**:四端 citations|onSteer 命中 extension 11 / miniapp 26 / mobile-rn 31 / cli 8,且各端有显式 D106 落点注释(推翻本条"四端 0 命中")。

### 被删副本 @ 原第 7603-7603 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 7604-7604 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-25) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。**进度(2026-09-24)**:① **三端 onSteer 消费落地**(cli/miniapp-taro/mobile-rn,各自 streamChat 调用点注册 + 渲染"引导已生效"交代,词表 15 文件直入正仓 packages/i18n/messages/{cli,miniapp-taro,mobile-rn} 五语言、译法与 web steerNoticeBar 逐字同源,端内 override 已摘除);测试 cli 4/4 + miniapp 7/7 + rn 9/9 全绿,三端文件域 tsc 0 错误;`onSteer` 命中 cli/miniapp/rn 由 0 变非 0。② **extension 已补齐(2026-09-24 第三轮,前述"无通道"结论系分母路径错误:extension 代码在 entrypoints/ 非 src/,该端早有 onCitations/onInjectionApplied/onRetryScheduled 消费)**:ChatPage 注册 onSteer(逐字段承接/空文本防御/8 条封顶)、MessageContent 渲染 steer-notice 交代条、词表五语言 steerNoticeTitle(与 web steerNoticeBar 同源)、@ihui/types ChatMessage 加 steerNotices 字段,steer-notice.test.tsx 4/4 过、tsc 0 错误。③ **守门 57 已闭合**:steer-injection-disclosure 条目入清单(implemented 32→33,13 锚点:ai-service 收集点/api schema/api-client 回调/五端消费与渲染),check-chat-element-coverage.mjs 实跑 EXIT 0(清单 125 条一致)。④ **历史灌回三端闭合(2026-09-24 第四轮)**:web readSteerAppliedFromMetadata(第一轮)+ miniapp backfillSteerNoticesFromMetadata(types.ts 守卫同 web/8 封顶/全坏不写,chat.tsx 两处历史恢复点接入)+ mobile-rn readSteerAppliedFromMetadata(chat-render-model 纯函数,双入口历史加载接入;顺带修复 ChatScreen toChatScreenMessage 不透传 steerNotices 导致 live 渲染死代码的缺陷);测试 miniapp 17/17 + rn 16/16,两端文件域 tsc 0。miniapp 注意:该端无服务端会话消息拉取(历史走本地存储),跨端 metadata 读回需先接服务端历史接口(读回函数已备好,行带 metadata 进来即可消费)。 **2026-09-25 本会话 HEAD 复跑改判**:四端 `onSteer` 命中实测 extension 2 / miniapp-taro 4 / mobile-rn 6 / cli 3(全非 0),`steer-injection-disclosure` 13 锚点在册且 `node scripts/check-chat-element-coverage.mjs` exit 0 ⇒ 票面验收四条件齐,原"四端 0 命中"前提已过期。

### 被删副本 @ 原第 8302-8302 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 8308-8308 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)
...
### 被删副本 @ 原第 9434-9434 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-26) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。 〔2026-09-26 翻勾(票面验收"citations 与 steer 在四端命中数由 0 变非 0"按 HEAD 实测达成,且判的是**落点**不是命中数):extension = `entrypoints/sidepanel/pages/ChatPage.tsx:528 onCitations` 注册(端内枚举式合并,不显式写就等于静默丢帧) + `components/MessageContent.tsx:733` 渲染引用列表;miniapp = `src/api/index.ts:471 case 'citations'` 派发 + `pkg-ai/ai/ChatMessageItem.tsx:565` 传引用卡;mobile-rn = `screens/AiAssistantN8nScreen.tsx:754 <CitationList>`;cli = `commands/agent.ts:885,1325` 透传 + `commands/repl.ts:52 steerNoteText` 打印;web = 16 文件。台账锚点 `scripts/data/chat-flow-elements.json` 的 `context-injection-disclosure` 标题已写"五端已接"。**残余另计(不属本票验收项)**:mobile-rn 的引用位落在 N8n 助手屏,主对话页 `ChatScreen.tsx` 仍无引用渲染(与 D19 的 terminal_delta 同一型残余),该端主对话页归后续接线票。〕

### 被删副本 @ 原第 9448-9448 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-26) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。**进度(2026-09-24)**:① **三端 onSteer 消费落地**(cli/miniapp-taro/mobile-rn,各自 streamChat 调用点注册 + 渲染"引导已生效"交代,词表 15 文件直入正仓 packages/i18n/messages/{cli,miniapp-taro,mobile-rn} 五语言、译法与 web steerNoticeBar 逐字同源,端内 override 已摘除);测试 cli 4/4 + miniapp 7/7 + rn 9/9 全绿,三端文件域 tsc 0 错误;`onSteer` 命中 cli/miniapp/rn 由 0 变非 0。② **extension 已补齐(2026-09-24 第三轮,前述"无通道"结论系分母路径错误:extension 代码在 entrypoints/ 非 src/,该端早有 onCitations/onInjectionApplied/onRetryScheduled 消费)**:ChatPage 注册 onSteer(逐字段承接/空文本防御/8 条封顶)、MessageContent 渲染 steer-notice 交代条、词表五语言 steerNoticeTitle(与 web steerNoticeBar 同源)、@ihui/types ChatMessage 加 steerNotices 字段,steer-notice.test.tsx 4/4 过、tsc 0 错误。③ **守门 57 已闭合**:steer-injection-disclosure 条目入清单(implemented 32→33,13 锚点:ai-service 收集点/api schema/api-client 回调/五端消费与渲染),check-chat-element-coverage.mjs 实跑 EXIT 0(清单 125 条一致)。④ **历史灌回三端闭合(2026-09-24 第四轮)**:web readSteerAppliedFromMetadata(第一轮)+ miniapp backfillSteerNoticesFromMetadata(types.ts 守卫同 web/8 封顶/全坏不写,chat.tsx 两处历史恢复点接入)+ mobile-rn readSteerAppliedFromMetadata(chat-render-model 纯函数,双入口历史加载接入;顺带修复 ChatScreen toChatScreenMessage 不透传 steerNotices 导致 live 渲染死代码的缺陷);测试 miniapp 17/17 + rn 16/16,两端文件域 tsc 0。miniapp 注意:该端无服务端会话消息拉取(历史走本地存储),跨端 metadata 读回需先接服务端历史接口(读回函数已备好,行带 metadata 进来即可消费)。 〔2026-09-26 翻勾(票面验收"citations 与 steer 在四端命中数由 0 变非 0"按 HEAD 实测达成,且判的是**落点**不是命中数):extension = `entrypoints/sidepanel/pages/ChatPage.tsx:528 onCitations` 注册(端内枚举式合并,不显式写就等于静默丢帧) + `components/MessageContent.tsx:733` 渲染引用列表;miniapp = `src/api/index.ts:471 case 'citations'` 派发 + `pkg-ai/ai/ChatMessageItem.tsx:565` 传引用卡;mobile-rn = `screens/AiAssistantN8nScreen.tsx:754 <CitationList>`;cli = `commands/agent.ts:885,1325` 透传 + `commands/repl.ts:52 steerNoteText` 打印;web = 16 文件。台账锚点 `scripts/data/chat-flow-elements.json` 的 `context-injection-disclosure` 标题已写"五端已接"。**残余另计(不属本票验收项)**:mobile-rn 的引用位落在 N8n 助手屏,主对话页 `ChatScreen.tsx` 仍无引用渲染(与 D19 的 terminal_delta 同一型残余),该端主对话页归后续接线票。〕

### 被删副本 @ 原第 9486-9486 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-26) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。 **[O60r 判:裸副本]** 本行正题与同编号已勾登记同题(判据 = 剥状态前缀后字符二元组 Jaccard ≥0.6 或逐字包含,与活文档对账同一把尺),不重复计账、勿照本行派单;现行判定以当次 HEAD 复跑该票点名的实现面为准。 〔2026-09-26 翻勾(票面验收"citations 与 steer 在四端命中数由 0 变非 0"按 HEAD 实测达成,且判的是**落点**不是命中数):extension = `entrypoints/sidepanel/pages/ChatPage.tsx:528 onCitations` 注册(端内枚举式合并,不显式写就等于静默丢帧) + `components/MessageContent.tsx:733` 渲染引用列表;miniapp = `src/api/index.ts:471 case 'citations'` 派发 + `pkg-ai/ai/ChatMessageItem.tsx:565` 传引用卡;mobile-rn = `screens/AiAssistantN8nScreen.tsx:754 <CitationList>`;cli = `commands/agent.ts:885,1325` 透传 + `commands/repl.ts:52 steerNoteText` 打印;web = 16 文件。台账锚点 `scripts/data/chat-flow-elements.json` 的 `context-injection-disclosure` 标题已写"五端已接"。**残余另计(不属本票验收项)**:mobile-rn 的引用位落在 N8n 助手屏,主对话页 `ChatScreen.tsx` 仍无引用渲染(与 D19 的 terminal_delta 同一型残余),该端主对话页归后续接线票。〕

### 被删副本 @ 原第 9487-9487 行(族主键 D106#D106消息级"交代帧"跨端消费缺口;存活副本 @ 第 3241-3247 行)

- [x] ✅(2026-09-26) **D106 消息级"交代帧"跨端消费缺口(G-148;实测量化,不是推测)**:多落点 grep(`--include=*.ts --include=*.tsx`,排除 node_modules)得 **extension / miniapp-taro / mobile-rn / cli 四端对 `citations` 与 `onSteer` 均 0 命中,只有 web 消费**;对照 `compaction` 四端各有落点(extension 1 / miniapp-taro 3 / mobile-rn 2 / cli 15)→ 证明**不是"这些端不接 SSE 交代帧",而是逐帧漏接**,同一类缺口第 N 次出现(D34 的 `injection_applied` 我一开始也只接了 web,即本条的又一实例)。**根因层 = C 客户端通道(api-client 已给 `onInjectionApplied` / `onRetryScheduled` / `onCitations` 回调,端内没人注册)+ P 呈现(各端无对应组件)**。**做法纪律**:① 表现层组件沉 `@ihui/ui-react`(取词函数由 props 注入,不得在组件内 `useTranslations`,否则又变成 web 独占);② 每端注册回调时必须**逐字段显式承接**(各端 store 都是枚举式合并,未知字段会被静默丢掉 —— 与 D40 截断字段完全同因);③ 交代类文案一律走各端命名空间词表,**禁止把后端 `collapsed` 中文当界面文本**(第 42 轮已为此把 kind 定为取词键);④ 端内不得再抄一份渲染模型(mobile-rn 的 `utils/chat-render-model.ts` 属既有违例,收编另立任务)。**验收(可机检)**:守门 57 的 `context-injection-disclosure` 元素锚点从"仅 web"扩到 **web + extension + miniapp-taro + mobile-rn**;每端至少 1 条"帧字段进了 store、界面出本地化文案、未知 kind 回退 collapsed"的用例;`citations` 与 `steer` 在四端的命中数由 0 变非 0(脚本可复算,分母用四端目录)。**进度(2026-09-24)**:① **三端 onSteer 消费落地**(cli/miniapp-taro/mobile-rn,各自 streamChat 调用点注册 + 渲染"引导已生效"交代,词表 15 文件直入正仓 packages/i18n/messages/{cli,miniapp-taro,mobile-rn} 五语言、译法与 web steerNoticeBar 逐字同源,端内 override 已摘除);测试 cli 4/4 + miniapp 7/7 + rn 9/9 全绿,三端文件域 tsc 0 错误;`onSteer` 命中 cli/miniapp/rn 由 0 变非 0。② **extension 已补齐(2026-09-24 第三轮,前述"无通道"结论系分母路径错误:extension 代码在 entrypoints/ 非 src/,该端早有 onCitations/onInjectionApplied/onRetryScheduled 消费)**:ChatPage 注册 onSteer(逐字段承接/空文本防御/8 条封顶)、MessageContent 渲染 steer-notice 交代条、词表五语言 steerNoticeTitle(与 web steerNoticeBar 同源)、@ihui/types ChatMessage 加 steerNotices 字段,steer-notice.test.tsx 4/4 过、tsc 0 错误。③ **守门 57 已闭合**:steer-injection-disclosure 条目入清单(implemented 32→33,13 锚点:ai-service 收集点/api schema/api-client 回调/五端消费与渲染),check-chat-element-coverage.mjs 实跑 EXIT 0(清单 125 条一致)。④ **历史灌回三端闭合(2026-09-24 第四轮)**:web readSteerAppliedFromMetadata(第一轮)+ miniapp backfillSteerNoticesFromMetadata(types.ts 守卫同 web/8 封顶/全坏不写,chat.tsx 两处历史恢复点接入)+ mobile-rn readSteerAppliedFromMetadata(chat-render-model 纯函数,双入口历史加载接入;顺带修复 ChatScreen toChatScreenMessage 不透传 steerNotices 导致 live 渲染死代码的缺陷);测试 miniapp 17/17 + rn 16/16,两端文件域 tsc 0。miniapp 注意:该端无服务端会话消息拉取(历史走本地存储),跨端 metadata 读回需先接服务端历史接口(读回函数已备好,行带 metadata 进来即可消费)。 **[O60r 判:裸副本]** 本行正题与同编号已勾登记同题(判据 = 剥状态前缀后字符二元组 Jaccard ≥0.6 或逐字包含,与活文档对账同一把尺),不重复计账、勿照本行派单;现行判定以当次 HEAD 复跑该票点名的实现面为准。 〔2026-09-26 翻勾(票面验收"citations 与 steer 在四端命中数由 0 变非 0"按 HEAD 实测达成,且判的是**落点**不是命中数):extension = `entrypoints/sidepanel/pages/ChatPage.tsx:528 onCitations` 注册(端内枚举式合并,不显式写就等于静默丢帧) + `components/MessageContent.tsx:733` 渲染引用列表;miniapp = `src/api/index.ts:471 case 'citations'` 派发 + `pkg-ai/ai/ChatMessageItem.tsx:565` 传引用卡;mobile-rn = `screens/AiAssistantN8nScreen.tsx:754 <CitationList>`;cli = `commands/agent.ts:885,1325` 透传 + `commands/repl.ts:52 steerNoteText` 打印;web = 16 文件。台账锚点 `scripts/data/chat-flow-elements.json` 的 `context-injection-disclosure` 标题已写"五端已接"。**残余另计(不属本票验收项)**:mobile-rn 的引用位落在 N8n 助手屏,主对话页 `ChatScreen.tsx` 仍无引用渲染(与 D19 的 terminal_delta 同一型残余),该端主对话页归后续接线票。〕

### 被删副本 @ 原第 3248-3248 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)

- [x] ✅(2026-09-24) **D107 交代帧的"端内注册层"与"阶段标签"缺口(第 44 轮实测新立)**:守门 63 只对齐到 parser 层,**帧到了各端 dispatch 表仍会二次静默丢弃**,本条覆盖剩下两层。

### 被删副本 @ 原第 3256-3256 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)

- [x] ✅(2026-09-24) **D107 交代帧的"端内注册层"与"阶段标签"缺口(第 44 轮实测新立)**:守门 63 只对齐到 parser 层,**帧到了各端 dispatch 表仍会二次静默丢弃**,本条覆盖剩下两层。

### 被删副本 @ 原第 6926-6926 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)
...
### 被删副本 @ 原第 7060-7061 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)
...
### 被删副本 @ 原第 7062-7062 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)

- [x] ✅(2026-09-24) **D107 交代帧的"端内注册层"与"阶段标签"缺口(第 44 轮实测新立)**:守门 63 只对齐到 parser 层,**帧到了各端 dispatch 表仍会二次静默丢弃**,本条覆盖剩下两层。

### 被删副本 @ 原第 7580-7580 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)
...
### 被删副本 @ 原第 8348-8348 行(族主键 D107#D107交代帧的"端内注册层"与"阶段标签"缺口;存活副本 @ 第 3257-3271 行)
...
### 被删副本 @ 原第 8341-8341 行(族主键 D110#D110WorkBuddy一手证据已打通→对话流;存活副本 @ 第 3275-3304 行)
...
### 被删副本 @ 原第 7394-7394 行(族主键 D111#D111移动端完全没有权限模式可见性;存活副本 @ 第 3364-3364 行)
...
### 被删副本 @ 原第 3133-3133 行(族主键 O10#O10对外run语义;存活副本 @ 第 5375-5390 行)
...
### 被删副本 @ 原第 3134-3136 行(族主键 O10#O10对外run语义;存活副本 @ 第 5375-5390 行)
...
### 被删副本 @ 原第 9616-9616 行(族主键 O10#O10对外run语义;存活副本 @ 第 5375-5390 行)

- [x] ✅(2026-09-26 复核) O10 对外 run 语义：幂等 run 创建（`Idempotency-Key`）、外部 run 句柄（不依赖 IHUI session_id）、通用幂等层、游标分页规范  ⏳(幂等重放保护已入库(af96921c95);run 句柄与游标分页另列 O10b)（✅2026-09-26 复核完成:run 句柄+游标分页已由 15e4f1f742e 落库(/api/agent-runs 三端点+cr1_ 游标),本票全量核实 38/38 绿、无需重做;余 api-client 接线与该提交自登记尾巴）

### 被删副本 @ 原第 9879-9880 行(族主键 O10#O10对外run语义;存活副本 @ 第 5375-5390 行)

- [x] ✅(2026-09-25) O10 对外 run 语义：幂等 run 创建（`Idempotency-Key`）、外部 run 句柄（不依赖 IHUI session_id）、通用幂等层、游标分页规范  ⏳(幂等重放保护已入库(af96921c95);run 句柄与游标分页另列 O10b) 〔2026-09-25 翻勾:四件(幂等创建/外部句柄/通用幂等层/游标分页)经代理逐件核验已由 15e4f1f742e 落库,O10 测试 98/98 全绿;六条尾巴各自属主/需§24确认,已在其条登记〕
  被 bash 的转义规则吃掉了正则里的 `\b` ⇒ 尺子失效而读数看着合理。加**阳性对照**（一批已知答案的样本，含一条必须为 0 的反例）

### 被删副本 @ 原第 9884-9884 行(族主键 O10#O10对外run语义;存活副本 @ 第 5375-5390 行)

- [x] ✅(2026-09-26 复核) O10 对外 run 语义：幂等 run 创建（`Idempotency-Key`）、外部 run 句柄（不依赖 IHUI session_id）、通用幂等层、游标分页规范  ⏳(幂等重放保护已入库(af96921c95);run 句柄与游标分页另列 O10b)

### 被删副本 @ 原第 6714-6714 行(族主键 O45#O45①门为什么看不见;存活副本 @ 第 6072-6072 行)
...
### 被删副本 @ 原第 6715-6715 行(族主键 O45#O45②同批拆掉这条门另外两处假绿灯+一条跑不通;存活副本 @ 第 6073-6073 行)
...
### 被删副本 @ 原第 6716-6716 行(族主键 O45#O45③写入侧根治;存活副本 @ 第 6074-6074 行)
...
### 被删副本 @ 原第 6717-6717 行(族主键 O45#O45④"该在那吗"的另一半;存活副本 @ 第 6075-6075 行)
...
### 被删副本 @ 原第 6282-6282 行(族主键 O40#O40①用户授权后注册计划任务;存活副本 @ 第 6097-6097 行)
...
### 被删副本 @ 原第 6283-6283 行(族主键 O40#O40②删除面复核;存活副本 @ 第 6098-6098 行)
...
### 被删副本 @ 原第 6284-6284 行(族主键 O40#O40③把"C盘还剩多少未定性条目"从71校正到;存活副本 @ 第 6099-6099 行)
...
### 被删副本 @ 原第 6285-6285 行(族主键 O40#O40④剩余6项定性结论;存活副本 @ 第 6100-6100 行)
...
### 被删副本 @ 原第 6681-6681 行(族主键 O46#O46⑤把那道check;存活副本 @ 第 6122-6122 行)
...
### 被删副本 @ 原第 6682-6682 行(族主键 O46#O46⑥编号相撞的机制化收口;存活副本 @ 第 6123-6123 行)
...
### 被删副本 @ 原第 6683-6683 行(族主键 O46#O46⑦机器侧三处残留分三类处置;存活副本 @ 第 6124-6124 行)
...
### 被删副本 @ 原第 6684-6684 行(族主键 O46#O46⑧收口后又被查出两处"入库的并行口径";存活副本 @ 第 6125-6125 行)
...
### 被删副本 @ 原第 6685-6685 行(族主键 O46#O46⑨check;存活副本 @ 第 6126-6126 行)
...
### 被删副本 @ 原第 6686-6686 行(族主键 O46#O46⑩把"提交前对账活文档"升成入库工具scr;存活副本 @ 第 6127-6127 行)
...
### 被删副本 @ 原第 6130-6130 行(族主键 O46#O46⑬上一票的回补被并发合并再次顶掉⇒复验点必;存活副本 @ 第 6129-6129 行)
...
### 被删副本 @ 原第 6689-6689 行(族主键 O46#O46⑬上一票的回补被并发合并再次顶掉⇒复验点必;存活副本 @ 第 6129-6129 行)
...
### 被删副本 @ 原第 6690-6690 行(族主键 O46#O46⑬上一票的回补被并发合并再次顶掉⇒复验点必;存活副本 @ 第 6129-6129 行)

- [x] ✅(2026-09-24) **O46⑬ 上一票的回补被并发合并再次顶掉 ⇒ 复验点必须移到"每次收敛/合并之后"**:O46④ 那次我把守门 70 的 `i18n-content-exempt-file:` 两行原样补回 `packages/shared/src/chat/handoff-package.ts`(提交 `9e01f6aa9`,该提交内 `git show 9e01f6aa9:<该文件>` 实测命中 1);几轮并发收敛之后 `git show HEAD:<该文件>` **又是 0 命中**。更值得记的是取证路径:`git log -S'i18n-content-exempt-file'` 只剩 `e7d1121e6`(加)与 `cfe8f65e4`(删)两条 —— **回补那次被历史简化规则藏住,光看文件历史查不出是谁顶掉的**,只能拿"HEAD 里有没有这一行"当唯一判据。⇒ 结论写进 AGENTS §12 新子条:活文档与共享源文件的回补,**提交前对账 + 提交后 diff 两道都不够**(吃掉它的不是回补那次提交),复验必须发生在每次收敛/合并之后,一行命令即可 `git show HEAD:<f> | grep -c <稳定前缀>`。同一分钟实测:守门 70 对工作树版判"49 处按声明放行 + 逐文件列出理由"(绿),而 HEAD 版若被下一个会话原样提交会把该文件判红 —— **红点不会静默,但会误伤下一个人**,所以每次发现即回补;本轮工作树已再次补回并随本提交入库。

### 被删副本 @ 原第 6691-6691 行(族主键 O46#O46⑭三条pnpm入口已落package;存活副本 @ 第 6131-6131 行)
...
### 被删副本 @ 原第 6692-6692 行(族主键 O46#O46⑫守门30a抓到并已备份一枚真悬空comm;存活副本 @ 第 6132-6132 行)
...
### 被删副本 @ 原第 6693-6693 行(族主键 O46#O46④本票自己制造并修好的两处回退;存活副本 @ 第 6133-6133 行)
...
### 被删副本 @ 原第 6694-6694 行(族主键 O46#O46③第三处Server酱残留;存活副本 @ 第 6134-6134 行)
...
### 被删副本 @ 原第 6695-6695 行(族主键 O42#O42③第三处Server酱残留;存活副本 @ 第 6135-6135 行)
...
### 被删副本 @ 原第 6728-6728 行(族主键 O45#O45④本票自己制造并修好的两处回退;存活副本 @ 第 6638-6638 行)
...
### 被删副本 @ 原第 6729-6729 行(族主键 O45#O45③第三处Server酱残留;存活副本 @ 第 6639-6639 行)
...
### 被删副本 @ 原第 6128-6128 行(族主键 O46#O46⑪守门81补R4「告警接收面」——把AM旁;存活副本 @ 第 6687-6687 行)
...
### 被删副本 @ 原第 6688-6688 行(族主键 O46#O46⑪守门81补R4「告警接收面」——把AM旁;存活副本 @ 第 6687-6687 行)

- [x] ✅(2026-09-24) **O46⑪ 守门 81 补 R4「告警接收面」—— 把 AM 旁路从"跑渲染才红"提到"改了提交就红"**:⑧ 那次的结构对账做在渲染器里,而渲染是**人工动作** —— 谁往 `monitoring/alertmanager/**` 的 yml/tmpl 加回 `email_configs` 或 IM receiver 并直接 commit,全链没有一道门会红(81 原判据只扫代码扩展名,`.yml/.yaml/.tmpl` 根本不进面)。现 R4 判三类:`email_configs`/`smtp_*` 键形态、IM 中转(名字与 host/路径**两级**都认 ⇒ `feishu-copy` 这种改名躲不掉)、`webhook_configs` 内 url ≠ `BRIDGE_URL`;`BRIDGE_URL` 提成导出常量并由镜像测试断言它与 `render-alertmanager-config.mjs:20` 那个值**逐字等值**(两道门取向不一致,就退回"提交绿、跑渲染才红"这个本门要消灭的落差);整行注释不计红但 `amCommentLiterals` 如实计数。**面刻意不扩到全仓 yml**:根 `docker-compose.yml` / `prometheus.yml` / `loki` 的历史注释里就写着 `dingtalk-webhook`,全扫必假红 —— 这条"为什么不扩"由断言钉死,防后来者"顺手补全"。改前/改后判定文件数 **431 → 434**(同一取材面、HEAD 版脚本另处实跑对照,工作树零改动),反空转锚取 `amCommentLiterals=2` 这个**非零**计数(把面摘掉它就归零,不靠"行数看着像"自证 —— 守门 52 恒报 0 的教训)。取证:全量 exit 0 / `--self-test` 61 例 / `--staged` exit 0 / 镜像测试 20 例(原 12);注入对照由主 agent 独立复验(往真模板插一行 `    email_configs:` ⇒ 全量 exit 1 且点名 `alertmanager.yml.tmpl:62`,随后 `git hash-object` 逐字节还原、`git status` 对该目录为空);基线仍为空且测试继续钉 `counts === {}`,**未新增任何白名单目录**。

### 被删副本 @ 原第 3129-3129 行(族主键 D85#D85自动审查统计条;存活副本 @ 第 6920-6920 行)
...
### 被删副本 @ 原第 3130-3130 行(族主键 D85#D85自动审查统计条;存活副本 @ 第 6920-6920 行)

- [x] ✅(2026-09-24 复核) **D85 自动审查统计条(G-116,与 D55 合批)**:在 D55 决策徽章之上加**聚合**——`自动审查统计`、`已接受 N / 已拒绝 N`、`命令历史` 展开、**`自动审查未提供理由`** 显式缺省(Trae 有代批无统计、Codex 有统计无逐条理由文案,我方一次做完可同超两家)。**验收**:统计计数与逐条徽章同源(不许两套数)+ 无理由缺省用例

### 被删副本 @ 原第 6916-6916 行(族主键 D85#D85自动审查统计条;存活副本 @ 第 6920-6920 行)
...
### 被删副本 @ 原第 7576-7576 行(族主键 D85#D85自动审查统计条;存活副本 @ 第 6920-6920 行)
...
### 被删副本 @ 原第 642-642 行(族主键 D38#D38队列语义完整交互;存活副本 @ 第 8682-8688 行)
...
### 被删副本 @ 原第 2643-2643 行(族主键 D38#D38队列语义完整交互;存活副本 @ 第 8682-8688 行)

- [x] ✅(2026-09-25)**D38 队列语义完整交互(G-42)**:拖拽重排 / 撤回 / 编辑队列项 / 「打断并执行」/ 队列模式可配(steer vs queue,对标 Codex `followUpQueueMode`)。复用 D28 侧问队列与 W2 abort 通道,不造第二套排队。**验收**:五动词各有 e2e + 与 /side 互不回归 + 重排后发送顺序断言 〔2026-09-25 翻勾:五动词经代理逐项核验已由先序落地(打断按 D69 口径诚实降级,不支持插话时显式被拒);本批补 store 单测 6 例 + e2e 发送顺序断言,87/87 绿〕

### 被删副本 @ 原第 7562-7562 行(族主键 D38#D38队列语义完整交互;存活副本 @ 第 8682-8688 行)

- [x] ✅(2026-09-25) **D38 队列语义完整交互(G-42)**:拖拽重排 / 撤回 / 编辑队列项 / 「打断并执行」/ 队列模式可配(steer vs queue,对标 Codex `followUpQueueMode`)。复用 D28 侧问队列与 W2 abort 通道,不造第二套排队。**验收**:五动词各有 e2e + 与 /side 互不回归 + 重排后发送顺序断言 〔2026-09-25 翻勾:五动词经代理逐项核验已由先序落地(打断按 D69 口径诚实降级,不支持插话时显式被拒);本批补 store 单测 6 例 + e2e 发送顺序断言,87/87 绿〕

### 被删副本 @ 原第 7600-7600 行(族主键 D38#D38队列语义完整交互;存活副本 @ 第 8682-8688 行)

- [x] ✅(2026-09-25)**D38 队列语义完整交互(G-42)**:拖拽重排 / 撤回 / 编辑队列项 / 「打断并执行」/ 队列模式可配(steer vs queue,对标 Codex `followUpQueueMode`)。复用 D28 侧问队列与 W2 abort 通道,不造第二套排队。**验收**:五动词各有 e2e + 与 /side 互不回归 + 重排后发送顺序断言 〔2026-09-25 翻勾:五动词经代理逐项核验已由先序落地(打断按 D69 口径诚实降级,不支持插话时显式被拒);本批补 store 单测 6 例 + e2e 发送顺序断言,87/87 绿〕

### 被删副本 @ 原第 7728-7728 行(族主键 D38#D38队列语义完整交互;存活副本 @ 第 8682-8688 行)

- [x] ✅(2026-09-25) **D38 队列语义完整交互(G-42)**:拖拽重排 / 撤回 / 编辑队列项 / 「打断并执行」/ 队列模式可配(steer vs queue,对标 Codex `followUpQueueMode`)。复用 D28 侧问队列与 W2 abort 通道,不造第二套排队。**验收**:五动词各有 e2e + 与 /side 互不回归 + 重排后发送顺序断言 〔2026-09-25 翻勾:五动词经代理逐项核验已由先序落地(打断按 D69 口径诚实降级,不支持插话时显式被拒);本批补 store 单测 6 例 + e2e 发送顺序断言,87/87 绿〕

### 被删副本 @ 原第 8957-8957 行(族主键 G-174#G-174门108的棘轮"只下调"配上静态存量数;存活副本 @ 第 9139-9139 行)
...
### 被删副本 @ 原第 9080-9084 行(族主键 O76#本票未做的两件;存活副本 @ 第 9206-9207 行)
...
### 被删副本 @ 原第 9164-9164 行(族主键 O76#本票未做的两件;存活副本 @ 第 9206-9207 行)
...
### 被删副本 @ 原第 2566-2566 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)

- [x] ✅(2026-09-25) D16 多模型智能路由(任务类型分类器+成本感知选模+预算降级)(G-21) 〔2026-09-25 孪生旧副本翻勾:同题已勾于 L2566〕

### 被删副本 @ 原第 2567-2567 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)
...
### 被删副本 @ 原第 2568-2568 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)

- [x] ✅(2026-09-24) D16 多模型智能路由(任务类型分类器+成本感知选模+预算降级)(G-21)—— 本行是 union 归并留下的旧副本,三点口径的实测判定与预算降级取证见紧邻的下一条,勿照本行再派单。

### 被删副本 @ 原第 2569-2569 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)
...
### 被删副本 @ 原第 7599-7599 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)
...
### 被删副本 @ 原第 9322-9322 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)
...
### 被删副本 @ 原第 9327-9327 行(族主键 D16#D16多模型智能路由;存活副本 @ 第 9433-9433 行)
...
### 被删副本 @ 原第 2592-2592 行(族主键 D20#D20会话文件夹/标签/置顶+导出PDF;存活副本 @ 第 10504-10504 行)

- [x] ✅(2026-09-26) D20 会话文件夹/标签/置顶+导出 PDF(G-11)。**TTS 朗读已存在**(2026-09-19 晚 V2 复核:voice-stream-speaker.tsx+MessageItem TTS 朗读按钮),从本项剔除 〔PROGRESS 2026-09-26: 文件夹/标签(编辑对话框复用 shared 归一化)+侧栏筛选展示+打印通道导出 PDF 已落地(主会话接管补完 printConversationPdf 与 11 键 i18n,parity 过);置顶与导出余项未做,另批〕〔O76 判 2026-09-26 主会话实测:**置顶子项已端到端在库,勿再派** —— db `pinned`/`pinnedAt` 列(schema/chat.ts:42,2026-08-30 立)+置顶优先排序(chat-queries.ts:106-110)+PATCH 校验与响应透传(routes/chat.ts:138/261-280)+api-client `setConversationPinned`(endpoints/chat.ts:193)+列表乐观更新(conversation-list.tsx:237-247)+TagsView 侧栏过滤(15 处命中)+专项 `apps/api/tests/chat-pinned.test.ts` **10/10 绿(本会话实跑)**;旧注"置顶未做"系读数过期。真正余量只剩:①共享层 `packages/shared/src/chat/conversation-org.ts`(文件夹/标签后端持久化)仍在其 owner 未跟踪面,待其入库;②"导出余项"具体形态 owner 未注明,动工前先问〕 〔2026-09-26 翻勾:union 复活旧副本,文件夹/标签+PDF 导出入库(主会话补完 printConversationPdf 与 11 键×5 语);置顶未做另批〕

### 被删副本 @ 原第 8203-8203 行(族主键 D19#D19复测后仍按住;存活副本 @ 第 10525-10525 行)
...
### 被删副本 @ 原第 11122-11122 行(族主键 O82#O82续三落账三路守门票全部入库;存活副本 @ 第 11018-11018 行)
