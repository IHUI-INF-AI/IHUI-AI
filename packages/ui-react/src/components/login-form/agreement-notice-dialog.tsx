...
export interface AgreementNoticeDialogProps {
  /** i18n 翻译函数 */
 ...
 * (next/link 在扩展端会出错)。
 */
export function AgreementNoticeDialog({
  t,
  open,
  onAgree,
  onCancel,
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onCancel()}>
      <DialogContent
        className={cn(
          ...
          //   该类通过 packages/ui-react/src/styles/login-form.css 第 55 行
          //   `.login-form-scope { position: relative; isolation: isolate }` 规则
      ...
      >
        <div className="flex flex-col items-center text-center">
          ...
          <DialogTitle className="text-base font-semibold leading-none tracking-tight">
            {t('auth.agreementNoticeTitle')}
          </DialogTitle>
          <DialogDescription className="mt-2 text-xs leading-relaxed text-muted-foreground">
            {t('auth.agreementNoticeDesc')}
          </DialogDescription>
        </div>

        {/* 两条协议卡片(普通 <a> 不用 next/link,扩展端兼容) */}
        ...
        <div className="mt-4 flex items-center gap-1.5 rounded-md bg-muted/40 px-2.5 py-1.5 text-[11px] leading-none text-muted-foreground">
          ...
          <span>{t('auth.agreementNoticeSafe')}</span>
        </div>
        ...
        <div className="mt-5 flex items-center gap-2">
          ...
          >
            <CornerDownLeft className="h-3.5 w-3.5" strokeWidth={2.25} aria-hidden="true" />
            {t('auth.agreementNoticeAgree')}
          </Button>
        </div>
        <p className="mt-3 text-center text-[11px] leading-none text-muted-foreground">
          {t('auth.agreementNoticeEnterHint')}
        </p>
      </DialogContent>
    </Dialog>
  )
