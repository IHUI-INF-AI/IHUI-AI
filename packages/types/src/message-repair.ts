 ...
 *      ⚠️ 仅适用于会话历史持久化修复(如 CLI repairSessionHistory)。
 *      LLM 请求链路(API /chat/stream、ai-service llm_gateway)必须传
 *      `{ keepTrailingUser: true }`,否则正在发送的最新 user 输入会被误删,
 *      导致模型只看到历史上下文、回复旧内容。
 *
...
export interface RepairOptions {
  /**
   * 保留末尾的 user 消息(不执行 Rule 5 的移除)。
   * LLM 请求链路必须传 true:messages 末尾的 user 消息是"当前正在发送的输入",
   * 而非 interjection 残留,移除会导致模型收到的是旧上下文。
   * 默认 false,保持会话历史持久化修复(如 CLI /repair)的既有语义。
   */
  keepTrailingUser?: boolean
}
 ...
 * 不修改原始数组(返回新数组与新对象)。
 */
export function repairMessages<T extends RepairableMessage>(
  messages: T[],
  ...
  // LLM 请求链路必须传 keepTrailingUser: true,否则当前输入会被误删
  if (
    !options.keepTrailingUser &&
    cleaned.length > 0 &&
