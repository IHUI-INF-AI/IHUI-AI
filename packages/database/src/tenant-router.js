 ...
 */
export function setDefaultDatabase(db) {
    defaultDb = db;
}
/**
 ...
 */
function normalizeTenantIdForEnv(tenantId) {
 ...
 */
function getTenantDbUrl(tenantId) {
...
export function getTenantDatabase(tenantId) {
    ...
    try {
        ...
        console.warn(`[tenant-router] 租户 ${tenantId} 数据库创建失败，降级到默认库:`, err.message);
        if (!defaultDb) {
            throw new Error('[tenant-router] 默认 Database 未注入，请先调用 setDefaultDatabase()');
        }
        return defaultDb;
...
/** 获取当前已缓存的租户连接数量（监控用）。 */
export function getTenantPoolSize() {
