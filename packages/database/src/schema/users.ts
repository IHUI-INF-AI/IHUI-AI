...
export const users = pgTable(
  'users',
  {
    ...
    identityType: varchar('identity_type', { length: 32 }).default('normal').notNull(), // normal/vip/trader,操盘手=trader(对齐原 Uniapp 操盘手身份)
...
/**
 * 用户多邮箱表(2026-09-20 立):余额不足邮件通知等多收件人场景。
 * users.email 仍为主邮箱;本表存放该用户的全部附加收件邮箱,
...
export const userEmails = pgTable(
  ...
  (t) => ({
    userIdIdx: index('user_emails_user_idx').on(t.userId),
  }),
)
