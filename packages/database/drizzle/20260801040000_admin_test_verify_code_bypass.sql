...
-- ============================================================================
-- 20260801040000: 测试验证码 bypass(开发环境固定验证码)
--
-- 目标:
--   本地/CI 环境给指定开发账号配置固定验证码,无需收真实验证码
--   真实账号标识请通过运维流程写入(勿提交到仓库)
--
-- 安全:
--   - 仅 NODE_ENV !== 'production' 时生效(应用层守卫,见 apps/api/src/utils/code-store.ts)
--   - 生产环境永远走真实验证码流程,且生产库此表应保持 is_active=false
...
-- 2) Seed 占位开发账号 bypass 规则(真实账号标识由运维在本地自行 INSERT)
INSERT INTO "test_verify_code_bypass" ("identifier", "fixed_code", "is_active", "note")
VALUES
  ('admin@ihui.local', '123456', true, 'local dev admin email bypass'),
  ('13000000000', '123456', true, 'local dev admin phone bypass')
