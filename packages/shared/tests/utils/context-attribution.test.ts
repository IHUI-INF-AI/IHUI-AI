...
describe('按构成来源分解', () => {
  ...
  it('占比之和(可观测段)等于 1,且各段 tokens 之和等于 attributedTokens', () => {
    ...
    const shareSum = a.segments.filter((s) => s.tokens > 0).reduce((s, seg) => s + seg.share, 0)
  ...
  it('provider 真值只校准总量并给出未归类差额,不改变各段自身', () => {
    ...
    expect(findSegment(without, 'roleUser')?.tokens).toBe(
      findSegment(withProvider, 'roleUser')?.tokens,
    )
...
describe('明细与稳定性', () => {
  ...
  it('明细标签只放数据或 @哨兵,不放内联文案', () => {
    const a = computeContextAttribution({
      ...
      messages: [
        msg({ role: 'assistant', toolCalls: [{ toolName: 'run_command', args: { cmd: 'ls' } }] }),
      ],
    })
  ...
  it('args 键序不同不影响工具调用段的 token', () => {
    ...
    expect(findSegment(one, 'roleToolCall')?.tokens).toBe(
      findSegment(other, 'roleToolCall')?.tokens,
    )
  })
})
