...
export function createUserStore<TProfile>(
  ...
  const persistStorage: PersistStorage<UserStoreState<TProfile>> = {
    getItem: async (name) => {
      if (!transport) return null
      const raw = await transport.getItem(name)
      if (!raw) return null
      try {
        return JSON.parse(raw) as { state: UserStoreState<TProfile>; version: number }
      } catch {
        return null
      }
    },
    setItem: async (name, value) => {
      if (!transport) return
      await transport.setItem(name, JSON.stringify(value))
    },
  ...
  const useStore = create<UserStoreState<TProfile>>()(
    persist(() => initialState, {
      ...
      partialize: partialize as ((state: UserStoreState<TProfile>) => Partial<UserStoreState<TProfile>>) | undefined,
      version,
    }),
  )
