...
function eventFrame(event: string, payload: Record<string, unknown>, id?: number) {
  return { jsonrpc: '2.0', method: 'thread/event', params: { threadId: 'thr_1', event, payload }, ...(id ? { id } : {}) }
}
...
function makeFetch(
  ...
  const calls: Array<{ method: string; params: Record<string, unknown>; headers: Record<string, string> }> = []
...
describe('createAgent 协议对接', () => {
  ...
  it('首个 run 自动 thread.start,再发 thread.prompt;两次调用 id 递增且带 Bearer 头', async () => {
    const { fetchFn, calls } = makeFetch([
      ...
      {
        kind: 'sse',
        frames: [eventFrame('tool.before', { tool: 'read_file' }), { jsonrpc: '2.0', id: 2, result: runResult() }],
      },
    ])
...
describe('createAgent 流式事件', () => {
  ...
  it('run 的 options.onEvent 覆盖构造期 onEvent(两者不叠加)', async () => {
    const { fetchFn } = makeFetch([
      { kind: 'json', body: START_RESULT },
      { kind: 'sse', frames: [eventFrame('tool.before', {}), { jsonrpc: '2.0', id: 2, result: runResult() }] },
    ])
  ...
  it('HTTP 非 2xx → AgentEngineError(码为状态码取负)', async () => {
    const { fetchFn } = makeFetch([{ kind: 'json', body: {}, status: 401, statusText: 'Unauthorized' }])
    const agent = createAgent({ token: 't', baseUrl: 'http://test.local', fetch: fetchFn })
...
describe('createAgent 宿主工具回传', () => {
  ...
  it('handler 抛错 → tools.result 回传 error 字段(不中断事件流)', async () => {
    ...
    const steps: Array<
      { kind: 'sse'; frames: unknown[] } | { kind: 'json'; body: unknown }
    > = [
      { kind: 'json', body: START_RESULT },
        ...
        frames: [
          {
            ...
            params: { requestId: 'ap_1', threadId: 'thr_1', toolName: 'rm_rf', dangerLevel: 'high' },
          },
          { jsonrpc: '2.0', id: 2, result: runResult() },
        ],
      },
...
describe('createAgent 控制面', () => {
  it('interrupt / state / resume / close 的方法名与参数形态正确', async () => {
    const { fetchFn, calls } = makeFetch([
      { kind: 'json', body: START_RESULT },
      { kind: 'json', body: { jsonrpc: '2.0', id: 2, result: { interrupted: true, mode: 'pause' } } },
      { kind: 'json', body: { jsonrpc: '2.0', id: 3, result: { threadId: 'thr_1', prompts: 1 } } },
      { kind: 'sse', frames: [{ jsonrpc: '2.0', id: 4, result: runResult({ stopReason: 'resumed' }) }] },
  ...
  it('cost / models / listTools 走差异化管理端点', async () => {
    ...
    expect(calls.map((c) => c.method)).toEqual(['thread.start', 'cost.report', 'models.list', 'tools.list'])
