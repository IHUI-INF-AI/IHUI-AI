...
function trendLabel(trend: AiCareerTrend, t: TFunction, tk: AppThemeTokens): { text: string; color: string; bg: string } {
  if (trend === 'up') return { text: t('aiCareer.trendUp'), color: tk.brand.DEFAULT, bg: tk.success.light }
  if (trend === 'new') return { text: t('aiCareer.trendNew'), color: tk.purple.DEFAULT, bg: tk.purple.light }
...
export function AiCareerScreen({
  ...
  return (
    ...
    >
      ...
      {loading && items.length === 0 ? <Text style={styles.loadingText}>{t('common.loading')}</Text> : null}
          ...
          >
            <View style={styles.careerHead}>
              ...
              <View style={styles.matchRing}>
                <Text style={[styles.matchScore, { color: scoreColor(c.match, tk) }]}>{c.match}</Text>
