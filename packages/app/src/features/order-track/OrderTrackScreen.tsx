    ...
    <View style={styles.container}>
      ...
      {loading && items.length === 0 ? (
        ...
        <FlatList<OrderTrackItem>
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={styles.muted}>{t('orderTrack.empty')}</Text>
            </View>
          }
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              <Text style={styles.cardMeta}>
                {t('orderTrack.location')}: {item.location}
              </Text>
              {item.desc ? (
...
function createStyles(tk: AppThemeTokens) {
  return StyleSheet.create({
    ...
    cardTitle: { fontSize: 16, fontWeight: '600', color: tk.success.DEFAULT },
    cardTime: { fontSize: 11, color: tk.text.tertiary },
    cardMeta: { marginTop: 8, fontSize: 14, color: tk.text.secondary },
    cardDesc: { marginTop: 8, fontSize: 14, color: tk.text.tertiary },
  })
}
