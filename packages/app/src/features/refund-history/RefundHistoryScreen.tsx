...
/**
 * 退款历史共享屏 — props 注入式跨端组件
 *
 * 平台无关:负责渲染 header(返回 + 标题)+ 错误提示(可选)+ loading 态
 * + 退款卡片列表(amount[红] + status + reason + createdAt,可点击)
 * + 下拉刷新 + 空态。
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{t('refundHistory.title')}</Text>
      </View>
      ...
      {loading && items.length === 0 ? (
        ...
        <FlatList<RefundHistoryItem>
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={styles.muted}>{t('refundHistory.empty')}</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
            ...
            >
              ...
              <Text style={styles.cardTime}>
                {t('refundHistory.createdAt')}: {item.createdAt}
              </Text>
            </TouchableOpacity>
          )}
        />
...
function createStyles(tk: AppThemeTokens) {
  return StyleSheet.create({
    ...
    cardAmount: { fontSize: 15, fontWeight: '600', color: tk.danger.DEFAULT },
    cardStatus: { fontSize: 12, color: tk.success.DEFAULT },
    cardReason: { marginTop: 4, fontSize: 12, color: tk.text.secondary },
