            ...
            <View style={styles.cardMain}>
              ...
              {(item.useCount !== undefined || item.rating !== undefined) ? (
                <Text style={styles.meta}>
                  {item.useCount !== undefined ? `${t('agentScreen.useCount', { count: item.useCount })}` : ''}
                  {item.useCount !== undefined && item.rating !== undefined ? ' · ' : ''}
                  {item.rating !== undefined ? `${t('agentScreen.rating', { score: item.rating.toFixed(1) })}` : ''}
                </Text>
              ) : null}
            </View>
          </Pressable>
...
function createStyles(tk: AppThemeTokens) {
  return StyleSheet.create({
    ...
    btn: { marginTop: 12, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8, backgroundColor: tk.brand.DEFAULT },
    ...
    title: { paddingHorizontal: 16, paddingTop: 8, paddingBottom: 8, fontSize: 20, fontWeight: '700', color: tk.text.primary },
    ...
    avatar: { width: 48, height: 48, borderRadius: 8, backgroundColor: tk.surface.muted },
    avatarFallback: {
      width: 48,
      height: 48,
      borderRadius: 8,
    ...
    avatarText: { fontSize: 18, fontWeight: '600', color: tk.text.secondary },
    cardMain: { flex: 1, marginLeft: 12 },
    nameRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
    name: { flex: 1, fontSize: 15, fontWeight: '600', color: tk.text.primary },
    vipBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: tk.warning.DEFAULT },
    ...
    meta: { marginTop: 6, fontSize: 11, color: tk.text.tertiary },
  })
}
