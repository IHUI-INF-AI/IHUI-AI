...
import type { MessageGroupItem, MessageGroupScreenProps } from '@ihui/types'

/** 群聊列表/Props 类型 re-export(单一来源 @ihui/types) */
export type { MessageGroupItem, MessageGroupScreenProps }

/**
 * 群聊列表共享屏 — props 注入式跨端组件
 *
 * 平台无关:负责渲染 header(返回 + 标题)+ 错误提示 + loading 态
 * + 群聊卡片列表(groupName + 未读红色 badge + lastMessage[2 行] + lastMessageTime)
 * + 下拉刷新 + 空态。不显示 "from:" 前缀。
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{t('messageGroup.title')}</Text>
      </View>
      ...
      {loading && items.length === 0 ? (
        ...
        <FlatList<MessageGroupItem>
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={styles.muted}>{t('messageGroup.empty')}</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
