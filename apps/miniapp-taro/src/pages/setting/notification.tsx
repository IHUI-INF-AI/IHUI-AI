...
export default function NotificationPage() {
  ...
  const onSubscribeWx = useCallback(async () => {
    ...
    if (accepted) {
      Taro.showToast({ title: tt('setting.notification.subscribed', '已开启提醒订阅'), icon: 'success' })
    } else {
      logger.info('setting/notification', 'subscribe-rejected', '用户未同意订阅或未配置模板')
      Taro.showToast({ title: tt('setting.notification.subscribeFailed', '未开启订阅,可稍后重试'), icon: 'none' })
    }
  }, [tt])
            ...
            >
              <View className="mr-[16rpx] flex-1">
                ...
                <Text className="mt-[6rpx] block text-[24rpx] leading-[1.5] text-[color:var(--color-text-tertiary)]">
                  {tt('setting.notification.wxSubscribeDesc', '订阅后可在微信接收学费催缴提醒(每次订阅可收一条)')}
                </Text>
              </View>
