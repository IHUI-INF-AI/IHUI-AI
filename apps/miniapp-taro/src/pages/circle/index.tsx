...
export default function CircleIndexPage() {
  ...
  return (
    <View className="page">
      <View className="tabs">
        <Text className={`tab${tab === 'recommend' ? ' active' : ''}`} onClick={() => switchTab('recommend')}>推荐</Text>
        <Text className={`tab${tab === 'follow' ? ' active' : ''}`} onClick={() => switchTab('follow')}>关注</Text>
        ...
        <View className="list">
          {list.map(c => (
            <View key={c.id} className="item" onClick={() => goDetail(c.id)}>
              <View className="user">
                <Image className="avatar" src={c.avatar || '/static/default-avatar.png'} mode="aspectFill" />
