    ...
    <ThemeRoot>
      <View className="min-h-screen bg-background">
        ...
        <View className="mx-[20rpx] mt-[20rpx] rounded-xl bg-[var(--color-success-light)] py-[28rpx] px-[28rpx] text-center">
          <Text className="block text-[length:28rpx] text-[var(--color-success-deep-text)]">
            {tt('member.integral.current', '当前积分')}
          </Text>
          <Text className="block mt-[16rpx] text-[length:60rpx] font-bold text-[var(--color-success-deep-text)]">
            {total}
          </Text>
        </View>
        <View className="p-[20rpx] pb-[64rpx]">
          {list.map((it) => (
            ...
            >
              <View className="flex justify-between items-center">
                <Text className="flex-1 mr-[16rpx] text-[length:32rpx] font-semibold text-foreground">
                  {it.type}
                </Text>
                <Text
                  className={`text-[length:32rpx] font-semibold ${it.amount > 0 ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]'}`}
                >
                  {it.amount > 0 ? '+' : ''}
                  {it.amount}
                </Text>
              ...
              <View className="mt-[16rpx] flex justify-end">
                <Text className="text-[length:22rpx] text-[var(--color-text-tertiary)]">{it.time}</Text>
              </View>
            </View>
          ))}
          {loading && !list.length ? (
            <View className="flex flex-col items-center py-[96rpx] text-muted-foreground text-[length:28rpx]">
          ...
          {!loading && !list.length && !error ? (
            <View className="text-center py-[96rpx] text-[length:28rpx] text-muted-foreground">
