      ...
      <View className="lli lli-list1 flex w-full" onClick={onClick}>
        {data.thumbnail && (
          <View className="lli-thumb" style={{ width: '250rpx', height: '174rpx' }}>
            <Image src={data.thumbnail} mode="aspectFill" style={{ width: '100%', height: '100%' }} lazyLoad />
          </View>
        )}
        <View className="lli-content flex-1 min-w-0 flex flex-col justify-around" style={{ marginLeft: '20rpx' }}>
          <Text className="text-[28rpx] font-bold text-foreground truncate">{data.title}</Text>
          {data.subtitle && <Text className="text-[24rpx] text-muted-foreground truncate">{data.subtitle}</Text>}
          {vipVisible && (
          ...
          <View className="flex items-center">
            {data.duration && <Text className="text-[24rpx] text-muted-foreground mr-[20rpx]">{data.duration}</Text>}
            {data.likes !== undefined && (
              <Text className="text-[24rpx] text-muted-foreground font-bold">{data.likes}人已学习</Text>
            )}
          </View>
        </View>
      </View>
  ...
  if (variant === 'list2') {
    return (
      <View className="lli lli-list2 flex w-full" onClick={onClick}>
        {data.thumbnail && (
          <View className="lli-thumb lli-thumb-overlay" style={{ width: '250rpx', height: '174rpx' }}>
            <Image src={data.thumbnail} mode="aspectFill" style={{ width: '100%', height: '100%' }} lazyLoad />
  ...
  if (variant === 'carousel') {
    return (
      <View className="lli lli-carousel flex flex-col" onClick={onClick}>
        ...
        {data.subtitle && <Text className="text-[24rpx] text-muted-foreground mt-[10rpx] truncate">{data.subtitle}</Text>}
