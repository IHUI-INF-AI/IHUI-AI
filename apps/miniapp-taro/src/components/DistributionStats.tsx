    ...
    <View className="bg-card mx-3 my-3 rounded-xl p-4">
      ...
      <View className="flex items-baseline mb-3">
        ...
        <Text className="text-xs text-muted-foreground ml-2">
          {tt('distribution.totalRevenue', '累计收益')}
        </Text>
      </View>
      ...
      <View className="mb-3">
        <View className="flex justify-between mb-1">
          <Text className="text-xs text-muted-foreground">
            {tt('distribution.monthRevenue', '本月收益')}
          </Text>
          <Text className="text-xs text-muted-foreground">
            {tt('distribution.monthlyTarget', '目标')} ¥{monthlyTarget}
          </Text>
        </View>
      ...
      <View className="grid grid-cols-3 gap-2 pt-3">
        <View>
          <Text className="block text-xs text-muted-foreground">
            {tt('distribution.withdrawable', '可提现')}
          </Text>
        ...
        <View>
          <Text className="block text-xs text-muted-foreground">
            {tt('distribution.pendingSettle', '待结算')}
          </Text>
          <Text className="text-sm font-medium text-warning">¥{pendingSettlement.toFixed(2)}</Text>
        </View>
        <View>
          <Text className="block text-xs text-muted-foreground">
            {tt('distribution.withdrawn', '已提现')}
          </Text>
