  ...
  return (
    ...
    >
      <View className="flex items-center gap-3">
        {avatar ? (
          <Image src={avatar} mode="aspectFill" className="w-12 h-12 rounded-md bg-muted" />
        ) : (
          <View className="w-12 h-12 rounded-md bg-primary/10 flex items-center justify-center">
            <Text className="text-base font-medium text-primary">{nickname.charAt(0) || '?'}</Text>
          </View>
        )}
        <View className="flex-1 min-w-0">
          ...
          <View className="flex items-center gap-2 mt-1">
            ...
            {desc && <Text className="text-xs text-muted-foreground truncate">{desc}</Text>}
          </View>
        </View>
      </View>
    </View>
