...
export async function fetchPushTemplates(): Promise<string[]> {
  ...
  try {
    ...
    logger.warn('push', 'fetch-templates-fail', msg || 'unknown')
  }
  return cachedTmplIds
}
...
export function initPushSubscription(): void {
  void fetchPushTemplates()
    .then((ids) => {
      if (ids.length === 0) {
        logger.info('push', 'no-templates', '未配置推送模板,跳过')
        return
      }
      logger.info('push', 'ready', `已加载 ${ids.length} 个推送模板,等待用户触发`)
    })
    .catch(() => {})
}
...
export async function requestPushSubscription(): Promise<boolean> {
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e)
    logger.warn('push', 'subscribe-fail', msg || 'unknown')
    return false
  }
}