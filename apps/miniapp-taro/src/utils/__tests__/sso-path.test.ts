...
describe('miniapp-taro SSO 路径修复', () => {
  it('exchangeSsoCode 请求 URL 不含双 /api 前缀', async () => {
    ...
    // 完整 URL 应为 http://localhost:8801/api/auth/sso/exchange
    expect(url).toBe('http://localhost:8801/api/auth/sso/exchange')
  })
  ...
  it('validateToken 请求 URL 不含双 /api 前缀', async () => {
    ...
    expect(url).not.toContain('/api/api/')
    expect(url).toBe('http://localhost:8801/api/auth/sso/validate')
  })
  ...
  it('ssoLogout 请求 URL 不含双 /api 前缀', async () => {
    ...
    expect(url).not.toContain('/api/api/')
    expect(url).toBe('http://localhost:8801/api/auth/sso/logout')
  })
  ...
  it('所有 SSO 端点路径使用 BASE_URL(已含 /api)+ 单 /auth/sso/* 拼接', async () => {
    ...
    for (const call of fetchCalls) {
      expect(call.url).toMatch(
        /^http:\/\/localhost:8801\/api\/auth\/sso\/(exchange|validate|logout)$/,
      )
    }
  })
})
