...
const DEFAULT_MODELS = (tt: TtFn): ModelEntry[] => [
  {
    ...
    category: 'chat',
    uses: 2300,
    route: '/pkg-ai/ai/chat?model=gemini-2.5-flash',
    featured: true,
  },
  {
    key: 'httpmodel',
...
export default function SpecialModelsPage() {
    ...
    async (reset = false) => {
      ...
      try {
        ...
        const agentModels: ModelEntry[] = (res.list || []).map((a) => ({
          ...
          route: `/pkg-ai/ai/agent-detail?id=${a.id}`,
        }))
        setModels((prev) => {
  ...
  const goHistory = useCallback(() => {
    Taro.navigateTo({ url: '/pkg-ai/ai/history' }).catch(() => {
