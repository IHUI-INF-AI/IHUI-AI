...
export default function DeveloperSubscribePage() {
    ...
    try {
      ...
      if (profileRes) {
        setDev({
          id: devId,
          name: toStr(
            profileRes.name ?? profileRes.nickname,
            tf('developer.subscribe.devName', '开发者'),
          ),
  ...
  const handleUseModel = (m: DevModel) => {
    Taro.navigateTo({ url: `/pkg-ai/ai/agent-detail?id=${m.id}` })
  }
  ...
  return (
    <ThemeRoot>
      <View className="min-h-screen bg-background p-[24rpx] pb-[48rpx]">
        {/* 头部 — 对齐 RN TeacherDetailScreen header(头像 rpx(128) 圆形 + 姓名 18 600 + 关注胶囊按钮) */}
