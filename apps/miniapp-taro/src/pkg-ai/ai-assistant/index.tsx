...
export default function AiAssistantPage() {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (u?.userMargin?.tokenQuantity && u.userMargin.tokenQuantity < 50000) {
      ...
      if (r.confirm) Taro.navigateTo({ url: '/pkg-shop/wallet/recharge/index' })
      return
    }
  ...
  useShareAppMessage(() => {
    ...
    return {
      ...
      path: `/pkg-ai/ai-assistant/index?souce=share&question=${encodeURIComponent(item?.question || '')}&content=${encodeURIComponent(item?.answer || '')}`,
    }
  })
