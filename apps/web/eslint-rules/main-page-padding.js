                  ...
                  const scan = (n) => {
                    ...
                    for (const k in n) {
                      ...
                      if (v && typeof v === 'object') {
                        if (Array.isArray(v))
                          v.forEach((c) => c && typeof c.type === 'string' && scan(c))
...
function makeRule(context) {
  ...
  const unwrap = (arg) => {
    ...
    while (
      a &&
      (a.type === 'TSAsExpression' || a.type === 'AsExpression' || a.type === 'ChainExpression')
    )
      a = a.expression
    // A && <div/> 模式:检查右侧 JSX
  ...
  const resolveFunction = (node) => {
    ...
    if (node.type === 'CallExpression')
      return node.arguments.map(resolveFunction).find(Boolean) ?? null
