...
test.describe('分享功能验证', () => {
  ...
  test('完整分享流程验证', async ({ browser }) => {
    ...
    const shareContent = await sharePage.evaluate(
    ...
    await sharePage.screenshot({
      path: path.join(SCREENSHOT_DIR, 'share-page.png'),
      fullPage: true,
    })
    ...
    const isSuccess = toastTexts.some(
      (t) => t.includes('已复制') || t.includes('copied') || t.includes('成功'),
    )
    ...
    await mainPage.screenshot({
      path: path.join(SCREENSHOT_DIR, 'toast-check.png'),
      fullPage: true,
    })
    ...
    console.log(
      `    剪贴板: ${clipboardContent ? clipboardContent.substring(0, 200) + '...' : '(空)'}`,
    )

    // 截图
    await mainPage.screenshot({
      path: path.join(SCREENSHOT_DIR, 'clipboard-check.png'),
      fullPage: true,
    })

    // 最终截图
    await mainPage.screenshot({
      path: path.join(SCREENSHOT_DIR, 'final-result.png'),
      fullPage: true,
    })
    ...
    console.log(
      `2. 对话选择:        ${conversationTitle ? `"${conversationTitle}" ✓` : '无可用对话 ✗'}`,
    )
    console.log(
      `3. 分享按钮点击:    ${shareBtnFound ? '✓ 已点击' : '⚠ 未找到（可能需手动 hover）'}`,
    )
    console.log(
      `4. Toast 提示:     ${toastTexts.length > 0 ? JSON.stringify(toastTexts) : '未显示'}`,
    )
