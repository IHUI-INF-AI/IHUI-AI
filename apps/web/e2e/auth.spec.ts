...
test.describe('认证流程', () => {
  test('登录表单验证', async ({ page }) => {
    ...
    // 空表单提交应显示错误
    await page.getByRole('button', { name: /登录|登 录|sign in/i }).click().catch(() => {})
    // 等待可能的错误提示或表单验证
    await page.waitForTimeout(1000)

    // 填写表单
    const accountInput = page.locator('input[name="phone"], input[name="account"], input[type="text"]').first()
