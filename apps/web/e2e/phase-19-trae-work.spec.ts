...
import { test as base, expect as baseExpect } from './fixtures'
...
test.describe('Phase 19 v17 adminPage 深度化(20 个测试)', () => {
  ...
  test('v17.1 拖拽:水平 60px 拖拽后 localStorage 位置 x/y 均被持久化为 number', async ({
    adminPage,
  }) => {
    ...
    if (saved && typeof saved === 'object' && 'x' in saved && 'y' in saved) {
      baseExpect(typeof saved.x).toBe('number')
      baseExpect(typeof saved.y).toBe('number')
      // x 应该 >= 0(clamp 后)
      baseExpect(saved.x as number).toBeGreaterThanOrEqual(0)
      baseExpect(saved.y as number).toBeGreaterThanOrEqual(0)
    } else {
  ...
  test('v17.2 拖拽中 body cursor 切换为 grabbing,释放后还原为空字符串', async ({ adminPage }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(bodyCursorDrag).toBe('grabbing')
    ...
    baseExpect(bodyCursorAfter).toBe('')
  })

  // ── v17.3 拖拽:user-select 切换 ───────────────────────────────
  test('v17.3 拖拽中 body user-select=none(避免拖拽过程中选中文字),释放后还原', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(userSelectDuring).toBe('none')

    await adminPage.mouse.up()
    await adminPage.waitForTimeout(150)

    const userSelectAfter = await adminPage.evaluate(() => document.body.style.userSelect)
    baseExpect(userSelectAfter).toBe('')
  })

  // ── v17.4 拖拽排除:子元素 mousedown 不会触发 data-dragging ────
  test('v17.4 拖拽排除子元素:pane-minimize 按钮 mousedown 不会触发 data-dragging=true', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(await pane.getAttribute('data-dragging')).toBeNull()
  ...
  test('v17.5 快捷键:聚焦 chat 输入框后按 ?(Shift+/)不应打开 help panel', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(afterHelpCount).toBe(initialHelpCount)
  })

  // ── v17.6 快捷键 Esc 优先级:help 打开 → Esc 仅关 help,pane 仍在 ─
  test('v17.6 快捷键 Esc 优先级:help 打开时按 Esc 仅关闭 help,pane 仍可见', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    await toggle.click()
    await adminPage.waitForTimeout(200)
    baseExpect(await adminPage.locator('[data-testid="pane-help-panel"]').count()).toBe(1)
    ...
    // 3) help 关闭
    baseExpect(await adminPage.locator('[data-testid="pane-help-panel"]').count()).toBe(0)
    // 4) pane 仍可见
    baseExpect(await adminPage.locator(PANE_TESTID).first().isVisible()).toBe(true)
  })

  // ── v17.7 快捷键 Esc 在 unpin 状态:help 关闭后按 Esc 关闭 pane ─
  test('v17.7 快捷键 Esc 二级:help 关闭后再按 Esc(unpin 状态)会关闭整个 pane', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
  ...
  test('v17.8 快捷键 ↑/↓:聚焦折叠子区 header 后按 ArrowDown 焦点移到下一个 header', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(before).not.toBeNull()
    ...
    baseExpect(after).not.toBeNull()
  })

  // ── v17.9 庆祝横幅文本 + role/aria-live 完整三联验证 ─────────
  test('v17.9 庆祝横幅:含 text-emerald-300/700(emerald 暗/亮模式)+ 文本 "全部任务完成" 或等价 key', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    // role + aria-live
    baseExpect(await banner.getAttribute('role')).toBe('status')
    baseExpect(await banner.getAttribute('aria-live')).toBe('polite')

    // emerald 颜色类
    const bannerClass = (await banner.getAttribute('class')) ?? ''
    baseExpect(bannerClass).toContain('emerald')

    // 文本非空
    const text = (await banner.textContent()) ?? ''
    baseExpect(text.trim().length).toBeGreaterThan(0)
  })

  // ── v17.10 庆祝横幅 SVG (Sparkles lucide-react) 验证 ──────────
  test('v17.10 庆祝横幅:含 lucide Sparkles SVG + animate-pulse class', async ({ adminPage }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(hasSparkles || hasPulse).toBeTruthy()
  })

  // ── v17.11 Timeline 跳转:messageId → ihui:scroll-to-message ───
  test('v17.11 Timeline 跳转:含 messageId 的事件 click 后派发 ihui:scroll-to-message', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
  ...
  test('v17.12 Timeline 跳转:含 planStepId 的事件 click 后派发 ihui:scroll-to-plan-step', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    void received
    baseExpect(dispatched).not.toBeNull()
    baseExpect(dispatched?.planStepId).toBe('plan-1')
  })

  // ── v17.13 Timeline 跳转:toolCallId → ihui:scroll-to-tool-call ─
  test('v17.13 Timeline 跳转:含 toolCallId 的事件 click 后派发 ihui:scroll-to-tool-call', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(captured).not.toBeNull()
    baseExpect(captured?.toolCallId).toBe('tool-1')
  })

  // ── v17.14 Timeline 事件:无 messageId/planStepId/toolCallId 时 disabled ─
  test('v17.14 Timeline 事件:无 messageId/planStepId/toolCallId/children 的 button disabled=true', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(disabledCount).toBeGreaterThanOrEqual(0)
  })

  // ── v17.15 跨组件:消息 hover 触发 ProgressJumpStore.hoveredPlanStep ─
  test('v17.15 跨组件联动:hover AI 消息后,ProgressJumpStore.hoveredPlanStep/hoveredMessage 同步', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(typeof classAfter).toBe('string')
  })

  // ── v17.16 跨组件:TimelineStore.addEvent 注入新事件后可见 ────
  test('v17.16 跨组件:TimelineStore.addEvent 注入事件后,timeline-event-row 数量 +1', async ({
    adminPage,
  }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(storeApi.total).toBe(storeApi.unique)
    void before
  })

  // ── v17.17 i18n 切换:zh-CN 状态下 Pane tab 显示 "对话" + "时间线" ──
  test('v17.17 i18n:zh-CN 状态下 Pane tab 显示 "对话" / "时间线"', async ({ adminPage }) => {
    if (!(await openPaneAdmin(adminPage))) return
    ...
    baseExpect(inlineText).toContain('对话')
    baseExpect(timelineText).toContain('时间线')
  })

  // ── v17.18 i18n 切换:en 状态下 Pane tab 显示 "Inline" / "Timeline" ──
  test('v17.18 i18n:en 状态下 Pane tab 显示 "Inline" / "Timeline"', async ({ adminPage }) => {
    ...
    const inlineText = (await inlineTab.textContent()) ?? ''
    const timelineText = (await timelineTab.textContent()) ?? ''
    baseExpect(inlineText).toContain('Inline')
    baseExpect(timelineText).toContain('Timeline')
  })

  // ── v17.19 i18n 切换:ja 状态下 Pane tab 显示 "会話" / "タイムライン" ──
  test('v17.19 i18n:ja 状态下 Pane tab 显示 "会話" / "タイムライン"', async ({ adminPage }) => {
    ...
    const timelineText = (await timelineTab.textContent()) ?? ''
    // 验证含日语字(可能部分词被截断)
    baseExpect(/[ぁ-んァ-ヶ一-龯]/.test(inlineText)).toBeTruthy()
    baseExpect(/[ぁ-んァ-ヶ一-龯]/.test(timelineText)).toBeTruthy()
  })

  // ── v17.20 i18n 切换:zh-TW 状态下 Pane aria-label 含 "Agent 任務進度面板" ─
  test('v17.20 i18n:zh-TW 状态下 Pane aria-label 含繁体中文(任務進度)', async ({
    adminPage,
  }) => {
    ...
    // zh-TW 含繁体字形(任務而非任务)
    const hasTradAria = /任務|進度/.test(ariaLabel)
    baseExpect(hasTradAria).toBeTruthy()
  })
})
