...
test.describe('支付结账链路', () => {
  test('未登录访问 /payment 跳转登录', async ({ page }) => {
    ...
    const url = page.url()
    expect(url.includes('/login') || url.includes('/register') || url.includes('/payment')).toBeTruthy()
  })
  ...
  test('未登录访问 /payment/checkout 跳转或显示提示', async ({ page }) => {
    ...
    const url = page.url()
    expect(url.includes('/login') || url.includes('/register') || url.includes('/checkout')).toBeTruthy()
  })
