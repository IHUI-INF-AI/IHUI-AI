...
import { CheckCircle2, Loader2, RotateCw, UserPlus } from 'lucide-react'
...
export default function ReferrerPage() {
  const t = useTranslations('referrer')
  ...
  const bindQ = useMutation({
    ...
    onSuccess: () => {
      ...
      setCode('')
      void qc.invalidateQueries({ queryKey: ['user', 'referrer'] })
    },
    onError: (e: Error) => {
      toast.error(e.message || t('bindFailed'))
    },
  })
  ...
  return (
    <div className="mx-auto w-full max-w-md px-4 py-6">
      <div className="mb-4 flex items-center justify-between">
        <BackButton fallbackHref="/distribution" />
            ...
            <CardContent className="p-4">
              ...
              {info.referrerName ? (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>
                    {info.referrerName}
                    {info.referrerCode ? ` (${info.referrerCode})` : ''}
                  </span>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">{t('noReferrer')}</p>
              )}
              <p className="mt-3 text-xs text-muted-foreground">
                {t('myInviteCode')}: {info.code}
              </p>
            </CardContent>
          </Card>
          ...
          {!info.referrerName && (
            <Card>
              <CardContent className="space-y-3 p-4">
                ...
                <Button
                  className="w-full"
                  disabled={bindQ.isPending || !code.trim()}
                  onClick={() => bindQ.mutate(code.trim())}
                >
                  {bindQ.isPending ? (
