...
const STATUS_STYLES: Record<string, string> = {
  ...
  sleep: 'bg-amber-500/10 text-amber-600',
}
const CMD_STATUS_STYLES: Record<string, string> = {
  pending: 'bg-amber-500/10 text-amber-600',
  sent: 'bg-sky-500/10 text-sky-600',
  ack: 'bg-emerald-500/10 text-emerald-600',
  failed: 'bg-rose-500/10 text-rose-600',
}
const COMMANDS = [
  { cmd: 'reboot', Icon: Power },
  { cmd: 'lock', Icon: Lock },
  { cmd: 'unlock', Icon: Unlock },
  { cmd: 'upgrade', Icon: Upload },
] as const

async function api<T>(url: string, options?: RequestInit): Promise<T> {
  const r = await fetchApi<T>(url, options)
  if (!r.success) {
    const e = new Error(r.error) as RequestError
    e.status = r.status
    throw e
  }
  return r.data
...
export default function TboxPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['tbox-devices'],
    queryFn: () => api<Device[]>('/api/tbox/devices'),
  })
  const detailQuery = useQuery({
    queryKey: ['tbox-devices', 'detail', selectedId],
    queryFn: () => api<Device>(`/api/tbox/devices/${selectedId}`),
    enabled: selectedId !== null,
  })
  const commandsQuery = useQuery({
    queryKey: ['tbox-devices', 'commands', selectedId],
    queryFn: () => api<CommandRecord[]>(`/api/tbox/devices/${selectedId}/commands`),
    enabled: selectedId !== null && detailOpen,
  })
  const registerMutation = useMutation({
    mutationFn: (v: RegisterForm) =>
      api<Device>('/api/tbox/devices', {
        method: 'POST',
        body: JSON.stringify({
          deviceNo: v.deviceNo.trim(),
          deviceName: v.deviceName.trim() || undefined,
          deviceType: v.deviceType.trim() || undefined,
        }),
      }),
    onSuccess: () => {
      setForm(EMPTY_FORM)
      qc.invalidateQueries({ queryKey: ['tbox-devices'] })
    },
  })
  const commandMutation = useMutation({
    mutationFn: (cmd: string) =>
      api<CommandRecord>(`/api/tbox/devices/${selectedId}/command`, {
        method: 'POST',
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (form.deviceNo.trim()) registerMutation.mutate(form)
  }
  ...
  const badge = (s: string, styles: Record<string, string>) =>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 gap-4 min-[640px]:grid-cols-3">
              {field('deviceNo', 'deviceNo')}
              {field('deviceName', 'deviceName')}
              {field('deviceType', 'deviceType', 'tbox')}
            </div>
            {registerMutation.error && (
              <Alert
                variant="danger"
                title={t('error')}
                description={(registerMutation.error as Error).message}
              />
            )}
                ...
                <CardContent className="flex min-w-0 flex-1 flex-col gap-2 p-4">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    <span className="truncate text-xs text-muted-foreground">
                      {d.deviceNo} · {d.deviceType}
                    </span>
                    <span className="flex shrink-0 items-center gap-1 text-xs text-primary">
                      {d.lastOnlineAt ? formatShortDateTime(d.lastOnlineAt, locale) : '-'}
        ...
        <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 pr-8">
              <Cpu className="h-4 w-4 text-primary" />
              {detail?.deviceName || detail?.deviceNo || t('devices')}
            </DialogTitle>
            <DialogDescription>
              {detail ? `${t('deviceNo')}: ${detail.deviceNo}` : t('loading')}
            </DialogDescription>
          </DialogHeader>
          {detailQuery.isLoading ? (
            ...
            <div className="space-y-4 text-sm">
              <div className="space-y-2">
                {infoRow(
                  'status',
                  <span className={badge(detail.status, STATUS_STYLES)}>{t(detail.status)}</span>,
                )}
                ...
                {infoRow(
                  ...
                  <span className="flex items-center gap-1">
                    <Battery className="h-3.5 w-3.5" />
                    {detail.battery !== null ? `${detail.battery}%` : '-'}
                  </span>,
                )}
                {infoRow(
                  'location',
                ...
                {infoRow(
                  'lastOnline',
                  <span>
                    {detail.lastOnlineAt ? formatShortDateTime(detail.lastOnlineAt, locale) : '-'}
                  </span>,
                )}
              </div>
              <div className="space-y-2">
                <h3 className="flex items-center gap-1.5 font-medium">
                  <Power className="h-4 w-4 text-primary/70" />
                  {t('sendCommand')}
                </h3>
                ...
                {commandMutation.isError && (
                  <Alert
                    variant="danger"
                    title={t('error')}
                    description={(commandMutation.error as Error).message}
                  />
                )}
              </div>
              <div className="space-y-2">
                    ...
                    {commands.map((c) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-muted-foreground">
                          ...
                          {c.ackedAt && (
                            <span className="ml-2">
                              {t('ackedAt')}: {formatShortDateTime(c.ackedAt, locale)}
                            </span>
                          )}
                        </div>
                      </div>
