...
export default function CostDashboardPage() {
  ...
  const load = React.useCallback(async (gran: Granularity) => {
    ...
    const tRes = await fetchApi<CostTimeseries>(
      `/api/cost-ledger/timeseries?granularity=${gran}`,
    )
    setLoading(false)
        ...
        <>
          ...
          <div className="mt-4 rounded-xl border p-4">
            <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold">
              <TrendingUp className="h-4 w-4" /> 成本 / Token 走势({granularity === 'day' ? '按日' : '按小时'})
            </h2>
            {series.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {series.map((b) => (
                  <div key={b.bucket} className="grid grid-cols-[7rem_1fr_1fr_5rem] items-center gap-2 text-xs">
                    <span className="truncate text-muted-foreground" title={b.bucket}>
                      {b.bucket}
                    </span>
                    <div className="flex items-center gap-1.5">
                      <div className="h-3 rounded bg-primary/70" style={{ width: `${pct(b.cost, maxCost)}%` }} />
                      <span className="shrink-0 text-muted-foreground">{fmtUsd(b.cost)}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="h-3 rounded bg-emerald-500/70" style={{ width: `${pct(b.tokens, maxTokens)}%` }} />
                      <span className="shrink-0 text-muted-foreground">{b.tokens.toLocaleString()} tok</span>
                    </div>
