...
export function ScanLoginDialog({
  ...
  const [chromePort, setChromePort] = React.useState<number | null>(null)
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  if (typeof result === 'number' && result > 0) {
                    setChromePort(result)
                    setPhase('polling')
                    startTimeRef.current = Date.now()
                    startChromePolling(result, platform)
                    onOpenChange(false)
                    toast.success('已打开 Chrome,登录后将自动保存账号')
                  } else {
                    // web 端降级:仍按原提示走
                    toast.info('当前为 web 端,请登录后手动粘贴 Cookie 保存')
                  }
                }}
              >
