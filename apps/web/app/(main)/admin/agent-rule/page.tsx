...
export default function AgentRulePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin', 'agent-rule', currentPage],
    queryFn: () => api<{ list: AgentRule[] }>('/api/admin/agent-rules'),
  })
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-left text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className={th}>{t('colParams')}</th>
            ...
            {isLoading ? (
              ...
              list.map((item) => (
                <tr key={item.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-2.5 text-muted-foreground">{item.params}</td>
