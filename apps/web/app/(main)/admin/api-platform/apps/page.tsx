...
export default function AdminApiPlatformAppsPage() {
  ...
  const { data: apps = [], isLoading } = useQuery({
    ...
    queryFn: () => api<{ list: ApiApp[] }>('/api/admin/api-platform/apps').then((d) => d.list ?? []),
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        ...
        <Button size="sm" onClick={() => { setForm(EMPTY); setErr(null); setCreateOpen(true) }}>
                ...
                <TableRow key={a.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      ...
                      <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => copy(a.appId)}>
                  ...
                  <TableCell>
                    {a.status === 1 ? (
                      <span className="inline-flex items-center gap-1 rounded bg-emerald-500/10 px-1.5 py-0.5 text-xs text-emerald-600">启用</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded bg-muted px-1.5 py-0.5 text-xs text-muted-foreground">停用</span>
                    )}
                  </TableCell>
      ...
      <Dialog open={createOpen} onOpenChange={(o) => (o ? null : (setCreateOpen(false), setErr(null)))}>
        <DialogContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="ap-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="输入应用名称" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="ap-perms">权限（逗号分隔）</Label>
              <Input id="ap-perms" value={form.permissions} onChange={(e) => setForm({ ...form, permissions: e.target.value })} placeholder="例如：read:users,write:orders" />
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setCreateOpen(false)} disabled={createMut.isPending}>取消</Button>
          ...
          {created && (
            <div className="space-y-3">
              <div className="space-y-1">
                <Label>AppID</Label>
                <code className="block rounded-md bg-muted/50 px-3 py-2 text-xs">{created.appId}</code>
              </div>
              <div className="space-y-1">
                <Label>AppSecret</Label>
                <code className={cn('block rounded-md bg-amber-500/10 px-3 py-2 text-xs')}>{created.appSecret}</code>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button type="button" onClick={() => setCreated(null)}>确认</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
