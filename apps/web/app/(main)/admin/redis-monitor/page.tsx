...
const TB: Record<RedisMonitorResponse['topKeys'][number]['type'], string> = {
  string: 'bg-sky-500/10 text-sky-600',
  list: 'bg-emerald-500/10 text-emerald-600',
  hash: 'bg-amber-500/10 text-amber-600',
  set: 'bg-rose-500/10 text-rose-600',
  zset: 'bg-purple-500/10 text-purple-600',
  ...
  const delMut = useMutation({
    mutationFn: async (key: string) => {
      const r = await fetchApi<{ ok: boolean }>(
        `/api/admin/monitor/redis/${encodeURIComponent(key)}`,
        { method: 'DELETE' },
      )
      if (!r.success) throw new Error(r.error)
      return r.data
  ...
  return (
    <div className="space-y-4 px-4 py-6">
      ...
      <div className="grid grid-cols-1 gap-3 min-[640px]:grid-cols-4">
        ...
        <StatCard
          title="内存"
          value={`${nf.format(o.usedMemory)} MB`}
          icon={HardDrive}
          loading={isLoading}
        />
            ...
            <table className="mt-3 w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted-foreground">
                  ...
                  <th className="px-3 py-2 font-medium text-right">TTL</th>
                ...
                {k.map((it) => (
                  <tr key={it.key}>
                    ...
                    <td className="px-3 py-2">
                      <span className={`rounded px-2 py-0.5 text-xs ${TB[it.type]}`}>
                        {it.type}
                      </span>
                    </td>
