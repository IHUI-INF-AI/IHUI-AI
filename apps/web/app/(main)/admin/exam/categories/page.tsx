...
export default function AdminExamCategoriesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin', 'exam', 'categories', 'all'],
    queryFn: () => api<{ list: Category[] }>(`/api/admin/exam/categories`).then((d) => d.list ?? []),
  })
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      ...
      if (editing) {
        return api<{ category: Category }>(`/api/admin/exam/categories/${editing.id}`, {
          method: 'PUT',
          body: JSON.stringify(body),
        })
      }
      return api<{ category: Category }>(`/api/admin/exam/categories`, {
        method: 'POST',
        body: JSON.stringify(body),
      })
    },
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['admin', 'exam', 'categories'] })
      closeDialog()
    },
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => api(`/api/admin/exam/categories/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
  ...
  function handleDelete(cat: Category) {
    if (!window.confirm(t('categoriesDeleteConfirm'))) return
      ...
      <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : closeDialog())}>
        <DialogContent>
          <form onSubmit={submit} className="space-y-4">
            <DialogHeader>
              <DialogTitle>{editing ? t('categoriesEditTitle') : t('categoriesCreateTitle')}</DialogTitle>
            </DialogHeader>
            {err && (
