...
interface TForm { name: string; description: string; status: boolean }
...
export default function EduCertificateTemplatesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => eduApi<PageData<Template>>(`/api/admin/certificates/templates${buildQs({ page, pageSize: PAGE_SIZE })}`),
  })

  const saveMut = useMutation({
    mutationFn: () => {
      const body = { name: form.name.trim(), description: form.description.trim() || null, status: form.status ? 1 : 0 }
      if (editing) return eduApi(`/api/admin/certificates/templates/${editing.id}`, { method: 'PUT', body: JSON.stringify(body) })
      return eduApi(`/api/admin/certificates/templates`, { method: 'POST', body: JSON.stringify(body) })
    },
    onSuccess: () => { toast.success(editing ? '更新成功' : '创建成功'); qc.invalidateQueries({ queryKey: ['edu', 'cert', 'templates'] }); closeDialog() },
    onError: (e: Error) => setErr(e.message),
  })
  const deleteMut = useMutation({
    mutationFn: (id: string) => eduApi(`/api/admin/certificates/templates/${id}`, { method: 'DELETE' }),
    onSuccess: () => { toast.success('删除成功'); qc.invalidateQueries({ queryKey: ['edu', 'cert', 'templates'] }) },
    onError: (e: Error) => toast.error(e.message),
  })

  function openCreate() { setEditing(null); setForm(EMPTY); setErr(null); setOpen(true) }
  function openEdit(t: Template) { setEditing(t); setForm({ name: t.name, description: t.description ?? '', status: t.status === 1 }); setErr(null); setOpen(true) }
  ...
  return (
    <div className="space-y-4">
      <div><h1 className="text-2xl font-bold tracking-tight">证书模板</h1><p className="mt-1 text-sm text-muted-foreground">管理证书模板样式</p></div>
      ...
      <div className="overflow-x-auto rounded-lg border">
        <Table>
          <TableHeader className="bg-muted/50"><TableRow>
            <TableHead className="px-4 py-2.5">名称</TableHead><TableHead className="px-4 py-2.5">描述</TableHead>
            <TableHead className="px-4 py-2.5">状态</TableHead><TableHead className="px-4 py-2.5">创建时间</TableHead>
          ...
          <TableBody className="divide-y">
            ...
            ) : error ? (<TableRow><TableCell colSpan={5} className="px-4 py-10 text-center text-destructive">{(error as Error).message}</TableCell></TableRow>
            ) : rows.length === 0 ? (<TableRow><TableCell colSpan={5} className="px-4 py-10 text-center text-muted-foreground"><FileText className="mx-auto mb-2 h-8 w-8 opacity-40" />暂无模板</TableCell></TableRow>
            ) : rows.map((t) => {
              const enabled = t.status === 1
              return (
                <TableRow key={t.id} className="hover:bg-muted/30">
                  <TableCell className="px-4 py-2.5 font-medium">{t.name}</TableCell>
                  <TableCell className="px-4 py-2.5"><div className="max-w-xs truncate text-xs text-muted-foreground">{t.description ?? '-'}</div></TableCell>
                  ...
                  <TableCell className="px-4 py-2.5 text-xs text-muted-foreground">{new Date(t.createdAt).toLocaleDateString()}</TableCell>
      ...
      <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : closeDialog())}>
        <DialogContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-2"><Label htmlFor="tpl-name">名称</Label><Input id="tpl-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
            <div className="space-y-2"><Label htmlFor="tpl-desc">描述</Label><textarea id="tpl-desc" className={textareaClass} rows={3} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></div>
            <div className="flex items-center gap-2"><Switch id="tpl-status" checked={form.status} onCheckedChange={(v) => setForm({ ...form, status: v })} /><Label htmlFor="tpl-status">启用</Label></div>
