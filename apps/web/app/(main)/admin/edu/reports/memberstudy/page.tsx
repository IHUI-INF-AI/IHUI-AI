...
export default function EduReportsMemberStudyPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['edu', 'reports', 'memberstudy', debounced, page],
    queryFn: () =>
      eduApi<PageData<MemberStudyReport>>(
        `/api/admin/reports/member-study${buildQs({ page, pageSize: PAGE_SIZE, search: debounced })}`,
      ),
    retry: false,
  })
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="flex items-center gap-3 p-5">
            ...
            <div>
              <div className="text-sm text-muted-foreground">本页学习课程</div>
              <div className="mt-0.5 text-xl font-semibold">{totalCourses}</div>
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="flex items-center gap-3 p-5">
            ...
            <div>
              <div className="text-sm text-muted-foreground">本页学习时长</div>
      ...
      <div className="overflow-x-auto rounded-lg border">
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow>
              <TableHead className="px-4 py-2.5">会员</TableHead>
              <TableHead className="px-4 py-2.5">学习课程</TableHead>
              <TableHead className="px-4 py-2.5">已完成</TableHead>
              <TableHead className="px-4 py-2.5">学习时长</TableHead>
              <TableHead className="px-4 py-2.5">最后学习</TableHead>
            </TableRow>
          </TableHeader>
              ...
              rows.map((r) => (
                <TableRow key={r.id} className="hover:bg-muted/30">
                  ...
                  <TableCell className="px-4 py-2.5 text-xs text-muted-foreground">
                    {r.lastStudyAt ? dateFmt.format(new Date(r.lastStudyAt)) : '-'}
                  </TableCell>
                </TableRow>
              ))
            )}
