...
export default function EduLearnMapsPage() {
  ...
  const [editing, setEditing] = React.useState<Map | null>(null)
  const [form, setForm] = React.useState(EMPTY)
  const [err, setErr] = React.useState<string | null>(null)

  const { data, isLoading, error } = useQuery({
    queryKey: ['edu', 'learn', 'maps', page],
    queryFn: () =>
      eduApi<PageData<Map>>(`/api/admin/learn/maps/list${buildQs({ page, pageSize: PAGE_SIZE })}`),
    retry: false,
  })
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      ...
      if (editing)
        return eduApi(`/api/admin/learn/maps/${editing.id}`, {
          method: 'PUT',
          body: JSON.stringify(body),
        })
      return eduApi(`/api/admin/learn/maps`, { method: 'POST', body: JSON.stringify(body) })
    },
    onSuccess: () => {
      toast.success(editing ? t('updateSuccess') : t('createSuccess'))
      qc.invalidateQueries({ queryKey: ['edu', 'learn', 'maps'] })
      closeDialog()
    },
    onError: (e: Error) => setErr(e.message),
  })
  const deleteMut = useMutation({
    mutationFn: (id: string) => eduApi(`/api/admin/learn/maps/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <MapsFilter onCreate={openCreate} />
      <MapsTable
        rows={rows}
        isLoading={isLoading}
