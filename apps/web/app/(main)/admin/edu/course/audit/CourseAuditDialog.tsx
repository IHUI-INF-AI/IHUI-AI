...
function CompareRow({ label, before, after }: { label: string; before: unknown; after: unknown }) {
  ...
  const diff = b !== a
  return (
    <div className="grid grid-cols-2 min-[640px]:grid-cols-3 gap-2 border-b border-muted/40 py-1.5 text-sm">
  ...
  return (
    ...
    >
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {t('dialog.compareTitle', { type: t(TYPE_KEY[compareType] ?? 'type.unknown') })}
          </DialogTitle>
        </DialogHeader>
        {loadingCompare ? (
