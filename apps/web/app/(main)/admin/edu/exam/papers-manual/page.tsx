...
interface Question { id: string; type: string; title: string; score: string; sortOrder: number }
...
export default function EduExamPapersManualPage() {
  ...
  const assembleMut = useMutation({
    mutationFn: () => {
      ...
      return eduApi(`/api/admin/edu/exam/papers/${paperId}/assemble`, { method: 'POST', body: JSON.stringify({ questionIds: ids, mode: 'manual' }) })
    },
    onSuccess: () => { toast.success('手动组卷完成'); qc.invalidateQueries({ queryKey: ['edu', 'exam', 'questions', paperId] }); setSelected(new Set()) },
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="w-full max-w-sm">
          <Select value={paperId} onValueChange={(v) => { setPaperId(v); setSelected(new Set()) }}>
            <SelectTrigger className={selectClass} aria-label="选择试卷"><SelectValue placeholder="选择试卷" /></SelectTrigger>
            <SelectContent>{papers.map((p) => <SelectItem key={p.id} value={p.id}>{p.title}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <Button onClick={() => assembleMut.mutate()} size="sm" className="ml-auto" disabled={!paperId || selected.size === 0 || assembleMut.isPending}>
          {assembleMut.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Wand2 className="h-4 w-4" />}
          组卷 ({selected.size})
        </Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border">
        <Table>
          ...
          <TableBody className="divide-y">
            {!paperId ? (
              <TableRow><TableCell colSpan={4} className="px-4 py-10 text-center text-muted-foreground">请先选择试卷</TableCell></TableRow>
            ) : isLoading ? (
              ...
              <TableRow><TableCell colSpan={4} className="px-4 py-10 text-center text-muted-foreground">暂无题目，请先在题库添加</TableCell></TableRow>
            ) : (
              (questions ?? []).map((q) => (
                <TableRow key={q.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => toggle(q.id)}>
                  <TableCell className="px-4 py-2.5"><input type="checkbox" checked={selected.has(q.id)} onChange={() => toggle(q.id)} className="h-4 w-4" /></TableCell>
                  <TableCell className="px-4 py-2.5"><span className="rounded-full bg-muted px-2 py-0.5 text-xs">{q.type}</span></TableCell>
      ...
      <div className="text-sm text-muted-foreground">
        ...
        <Link href="/admin/edu/exam/questions" className="ml-1 text-primary hover:underline">题库管理</Link>
        添加新题目。
      </div>
    </div>
  )
