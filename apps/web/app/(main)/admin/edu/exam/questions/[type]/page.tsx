...
const TYPE_LABEL: Record<string, string> = {
  single: '单选题', multi: '多选题', judgment: '判断题',
  fill: '填空题', subjective: '简答题', programming: '编程题',
}
const TYPE_API: Record<string, string> = {
  single: 'single_choice', multi: 'multi_choice', judgment: 'judgment',
  fill: 'fill_blank', subjective: 'subjective', programming: 'programming',
}

interface Paper { id: string; title: string; isPublished: boolean }
interface Question {
  id: string; paperId: string; type: string; title: string
  options: unknown; score: string; sortOrder: number; answer?: unknown; analysis?: string
}
interface QForm { title: string; score: string; sortOrder: string; options: string; answer: string; analysis: string }
const EMPTY: QForm = { title: '', score: '5', sortOrder: '0', options: '', answer: '', analysis: '' }
...
function TypeQuestionsContent() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => eduApi<{ list: Question[] }>(`/api/admin/exam/papers/${paperId}/questions`).then((d) => (d.list ?? []).filter((q) => q.type === apiType)),
    enabled: !!paperId,
  })
  ...
  function buildBody() {
    const body: Record<string, unknown> = { type: apiType, title: form.title.trim(), score: form.score, sortOrder: Number(form.sortOrder) || 0 }
  ...
  function openEdit(q: Question) {
    setEditing(q)
    setForm({
      title: q.title, score: q.score, sortOrder: String(q.sortOrder),
  ...
  function closeDialog() { if (createMut.isPending || updateMut.isPending) return; setOpen(false); setEditing(null); setErr(null) }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button asChild variant="ghost" size="sm">
          <Link href="/admin/edu/exam/questions"><ChevronLeft className="h-4 w-4" />返回题库</Link>
        </Button>
        <div className="w-full max-w-sm">
          <select className={selectClass} value={paperId} onChange={(e) => onPaperChange(e.target.value)} aria-label="选择试卷">
            <option value="">请选择试卷</option>
            {papers.map((p) => <option key={p.id} value={p.id}>{p.title}{!p.isPublished ? '（未发布）' : ''}</option>)}
          </select>
        </div>
        <Button onClick={openCreate} size="sm" className="ml-auto" disabled={!paperId}><Plus className="h-4 w-4" />新建{label}</Button>
      </div>
      <div className="overflow-x-auto rounded-lg border">
        <Table>
          ...
          <TableBody className="divide-y">
            {!paperId ? (
              <TableRow><TableCell colSpan={4} className="px-4 py-10 text-center text-muted-foreground"><ListChecks className="mx-auto mb-2 h-8 w-8 opacity-40" />请先选择试卷</TableCell></TableRow>
            ) : isLoading ? (
              ...
              <TableRow><TableCell colSpan={4} className="px-4 py-10 text-center text-destructive">{(error as Error).message}</TableCell></TableRow>
            ) : questions.length === 0 ? (
              <TableRow><TableCell colSpan={4} className="px-4 py-10 text-center text-muted-foreground"><ListChecks className="mx-auto mb-2 h-8 w-8 opacity-40" />暂无{label}</TableCell></TableRow>
            ) : (
              questions.map((q) => (
                <TableRow key={q.id} className="hover:bg-muted/30">
                  ...
                  <TableCell className="px-4 py-2.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => { if (window.confirm('确定删除？')) deleteMut.mutate(q.id) }} title="删除" className="text-destructive hover:text-destructive" disabled={deleteMut.isPending}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))
      ...
      <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : closeDialog())}>
        <DialogContent className="max-w-2xl">
          <form onSubmit={submit} className="space-y-4">
            <DialogHeader><DialogTitle>{editing ? `编辑${label}` : `新建${label}`}</DialogTitle></DialogHeader>
            {err && <div className="rounded-md bg-destructive/10 px-3 py-2 text-sm text-destructive">{err}</div>}
            <div className="space-y-2">
              <Label htmlFor="t-title">题干</Label>
              <textarea id="t-title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} rows={2} className={cn(textareaClass, 'font-sans')} placeholder="请输入题干内容" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label htmlFor="t-score">分值</Label><Input id="t-score" type="number" min="0" value={form.score} onChange={(e) => setForm({ ...form, score: e.target.value })} /></div>
              <div className="space-y-2"><Label htmlFor="t-sort">排序</Label><Input id="t-sort" type="number" min="0" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: e.target.value })} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label htmlFor="t-options">选项 (JSON)</Label><textarea id="t-options" value={form.options} onChange={(e) => setForm({ ...form, options: e.target.value })} rows={4} className={textareaClass} placeholder='[{"key":"A","text":"选项一"}]' /></div>
              <div className="space-y-2"><Label htmlFor="t-answer">答案 (JSON)</Label><textarea id="t-answer" value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })} rows={4} className={textareaClass} placeholder='"A" / ["A","B"] / true' /></div>
            </div>
            <div className="space-y-2"><Label htmlFor="t-analysis">解析</Label><Input id="t-analysis" value={form.analysis} onChange={(e) => setForm({ ...form, analysis: e.target.value })} placeholder="解析(选填)" /></div>
            <DialogFooter>
