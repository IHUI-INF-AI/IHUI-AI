...
interface Category { id: string; name: string; pid: string | null; sort: number; status: number }
...
export default function EduExamCategoriesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => eduApi<{ list: Category[] }>(`/api/admin/exam/categories`).then((d) => d.list ?? []),
  })

  const saveMut = useMutation({
    mutationFn: () => {
      const body = { name: form.name.trim(), sort: Number(form.sort) || 0, status: form.status ? 1 : 0 }
      if (editing) return eduApi(`/api/admin/exam/categories/${editing.id}`, { method: 'PUT', body: JSON.stringify(body) })
      return eduApi(`/api/admin/exam/categories`, { method: 'POST', body: JSON.stringify(body) })
    },
    onSuccess: () => { toast.success(editing ? '更新成功' : '创建成功'); qc.invalidateQueries({ queryKey: ['edu', 'exam', 'categories'] }); closeDialog() },
    onError: (e: Error) => setErr(e.message),
  })
  const deleteMut = useMutation({
    mutationFn: (id: string) => eduApi(`/api/admin/exam/categories/${id}`, { method: 'DELETE' }),
    onSuccess: () => { toast.success('删除成功'); qc.invalidateQueries({ queryKey: ['edu', 'exam', 'categories'] }) },
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">考试分类</h1>
        <p className="mt-1 text-sm text-muted-foreground">管理试卷分类</p>
      </div>
        ...
        <Table>
          ...
          <TableBody className="divide-y">
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={4} className="px-4 py-10 text-center text-muted-foreground"><FolderTree className="mx-auto mb-2 h-8 w-8 opacity-40" />暂无分类</TableCell></TableRow>
            ) : (
              categories.map((c) => (
                <TableRow key={c.id} className="hover:bg-muted/30">
                  ...
                  <TableCell className="px-4 py-2.5">
                    <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium', c.status === 1 ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-500' : 'bg-muted text-muted-foreground')}>
                      <span className={cn('h-1.5 w-1.5 rounded-full', c.status === 1 ? 'bg-emerald-500' : 'bg-muted-foreground')} />
                      {c.status === 1 ? '启用' : '禁用'}
                    </span>
                  </TableCell>
      ...
      <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : closeDialog())}>
        <DialogContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-2"><Label htmlFor="c-name">名称</Label><Input id="c-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="分类名称" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2"><Label htmlFor="c-sort">排序</Label><Input id="c-sort" type="number" min="0" value={form.sort} onChange={(e) => setForm({ ...form, sort: e.target.value })} /></div>
              <div className="space-y-2"><Label htmlFor="c-status">状态</Label><div className="flex h-9 items-center gap-2"><Switch id="c-status" checked={form.status} onCheckedChange={(v) => setForm({ ...form, status: v })} /><span className="text-sm text-muted-foreground">{form.status ? '启用' : '禁用'}</span></div></div>
            </div>
