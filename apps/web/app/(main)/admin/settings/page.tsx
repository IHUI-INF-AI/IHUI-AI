...
import { Card, CardHeader, CardTitle, CardContent, Input, Label, Button, Select, SelectTrigger, SelectContent, SelectItem, SelectValue } from '@ihui/ui'
          ...
          {tab === 'site' && (
            <>
              ...
              <Field label={t('site_description')}>
                <Input value={site.description} onChange={(e) => setSite({ ...site, description: e.target.value })} />
              </Field>
          ...
          {tab === 'email' && (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label={t('email_pass')}>
                <Input type="password" value={email.pass} onChange={(e) => setEmail({ ...email, pass: e.target.value })} />
              </Field>
              <div className="sm:col-span-2">
                <Field label={t('email_from')}>
                  <Input value={email.from} onChange={(e) => setEmail({ ...email, from: e.target.value })} placeholder="noreply@example.com" />
                </Field>
              </div>
            </div>
          )}
          ...
          {tab === 'storage' && (
            <>
              <Field label={t('storage_type')}>
                <Select value={storage.type} onValueChange={(v) => setStorage({ ...storage, type: v as typeof storage.type })}>
              ...
              {storage.type !== 'local' && (
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label={t('storage_endpoint')}>
                    <Input value={storage.endpoint} onChange={(e) => setStorage({ ...storage, endpoint: e.target.value })} />
                  </Field>
                  <Field label={t('storage_accessKey')}>
                    <Input value={storage.accessKey} onChange={(e) => setStorage({ ...storage, accessKey: e.target.value })} />
                  </Field>
                  <Field label={t('storage_secretKey')}>
                    <Input type="password" value={storage.secretKey} onChange={(e) => setStorage({ ...storage, secretKey: e.target.value })} />
                  </Field>
                </div>
              )}
            </>
          ...
          {tab === 'security' && (
            <>
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <Switch checked={security.allowRegister} onChange={(v) => setSecurity({ ...security, allowRegister: v })} />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={t('security_maxLoginAttempts')}>
                  <Input value={security.maxLoginAttempts} onChange={(e) => setSecurity({ ...security, maxLoginAttempts: e.target.value })} type="number" min={1} />
                </Field>
                <Field label={t('security_jwtExpires')}>
                  <Input value={security.jwtExpires} onChange={(e) => setSecurity({ ...security, jwtExpires: e.target.value })} placeholder="7d" />
                </Field>
              </div>
            </>
          )}
