...
export default function AdminTicketPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await fetchApi<TicketListData>(`/api/admin/support/tickets?${qs}`)
  ...
  const statusMut = useMutation({
    mutationFn: ({ id, s }: { id: string; s: TicketStatus }) =>
      fetchApi(`/api/admin/support/tickets/${id}/status`, {
        method: 'PUT',
  ...
  const replyMut = useMutation({
    mutationFn: ({ id, body }: { id: string; body: TicketReplyBody }) =>
      fetchApi(`/api/admin/support/tickets/${id}/reply`, {
        method: 'POST',
    ...
    <div className="space-y-4">
              ...
              list.map((t: Ticket) => (
                <tr key={t.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-2.5 text-muted-foreground">
                    {t.updatedAt ? dtf.format(new Date(t.updatedAt)) : '-'}
                  </td>
      ...
      {reply && (
        <ReplyForm
          ...
          onSubmit={(content) =>
            replyMut.mutate({ id: reply.id, body: { content, isAdmin: true } })
          }
...
function ReplyForm({
  ...
  return (
    <form
      onSubmit={form.handleSubmit((v) => onSubmit(v.content))}
