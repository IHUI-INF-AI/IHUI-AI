    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-lg border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? <tr><td colSpan={6} className="px-4 py-10 text-center text-muted-foreground">加载中...</td></tr>
      ...
      {reply && (
        <div className="flex items-center gap-2 rounded-lg border bg-card p-3">
          <Input value={reply.content} onChange={(e) => setReply({ ...reply, content: e.target.value })} placeholder="输入回复内容" className="flex-1" />
          <Button size="sm" disabled={!reply.content.trim() || replyMut.isPending} onClick={() => replyMut.mutate({ id: reply.id, body: { content: reply.content, isAdmin: true } })}><Send className="h-4 w-4" />发送</Button>
          <Button size="sm" variant="ghost" onClick={() => setReply(null)}>取消</Button>
        </div>
      )}
    </div>
  )
