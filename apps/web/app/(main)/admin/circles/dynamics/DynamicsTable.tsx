...
import { MessageSquare, Trash2, Loader2, FileText } from 'lucide-react'
...
export function DynamicsTable({ list, isLoading, deletePending, onComments, onDelete }: Props) {
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        item.status === 'published' ? 'bg-emerald-500' : 'bg-muted-foreground/50',
                      )}
                    />
                    {item.status === 'published' ? t('statusPublished') : t('statusDeleted')}
                  </span>
                </TableCell>
                <TableCell className="px-4 py-2.5 text-sm text-muted-foreground">
