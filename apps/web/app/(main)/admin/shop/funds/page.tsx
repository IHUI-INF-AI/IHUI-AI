...
import { Loader2, Wallet, TrendingUp, TrendingDown, ArrowUpRight, ArrowDownRight } from 'lucide-react'
...
const TYPE_LABEL: Record<FundFlow['type'], string> = { recharge: '充值', consume: '消费', refund: '退款', withdraw: '提现' }
...
export default function AdminShopFundsPage() {
  ...
  const { data: accounts = [], isLoading: accLoading } = useQuery({
    ...
    queryFn: () => api<{ list: FundAccount[] }>('/api/admin/shop/funds/accounts').then((d) => d.list ?? []),
  ...
  const { data: flows = [], isLoading: flowLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return api<{ list: FundFlow[] }>(`/api/admin/shop/funds/flows?${qs.toString()}`).then((d) => d.list ?? [])
    },
    enabled: tab === 'flows',
  })
  ...
  const cards = [
    ...
    { label: '账户数', value: accounts.length, icon: TrendingUp, cls: 'text-emerald-600', raw: true },
  ]

  const tabCls = (active: boolean) => cn('rounded-md px-3 py-1.5 text-sm font-medium transition-colors', active ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground')
    ...
    <div className="space-y-4">
            ...
            <Card key={c.label}>
              ...
              <CardContent>
                <div className={cn('text-2xl font-bold', c.cls)}>
                  {c.raw ? c.value.toLocaleString() : `¥${(c.value / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
                </div>
              </CardContent>
            </Card>
          )
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex gap-1 rounded-lg border bg-muted/30 p-1">
          <button onClick={() => setTab('accounts')} className={tabCls(tab === 'accounts')}>账户</button>
          <button onClick={() => setTab('flows')} className={tabCls(tab === 'flows')}>流水</button>
        </div>
        {tab === 'flows' && (
          ...
          <Table>
            ...
            <TableBody>
              {accLoading ? (
                ...
                <TableRow><TableCell colSpan={6} className="py-10 text-center text-muted-foreground">暂无账户</TableCell></TableRow>
              ) : (
                accounts.map((a) => (
                  <TableRow key={a.id}>
                    <TableCell className="font-medium">{a.user}</TableCell>
                    <TableCell className="font-medium text-primary">¥{(a.balance / 100).toFixed(2)}</TableCell>
                    <TableCell className="text-amber-600">¥{(a.frozen / 100).toFixed(2)}</TableCell>
                    <TableCell className="text-emerald-600">¥{(a.totalRecharge / 100).toFixed(2)}</TableCell>
          ...
          <Table>
            ...
            <TableBody>
              {flowLoading ? (
                ...
                <TableRow><TableCell colSpan={6} className="py-10 text-center text-muted-foreground">暂无流水</TableCell></TableRow>
              ) : (
                flows.map((f) => (
                  <TableRow key={f.id}>
                    <TableCell className="font-medium">{f.user}</TableCell>
                    <TableCell>
                      <span className="rounded bg-muted px-1.5 py-0.5 text-xs">{TYPE_LABEL[f.type]}</span>
                    </TableCell>
                    <TableCell className={cn('font-medium', f.direction === 'in' ? 'text-emerald-600' : 'text-red-600')}>
                      <span className="inline-flex items-center gap-0.5">
                        {f.direction === 'in' ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                        {f.direction === 'in' ? '+' : '-'}¥{(f.amount / 100).toFixed(2)}
                      </span>
                    </TableCell>
                    <TableCell>¥{(f.balance / 100).toFixed(2)}</TableCell>
                    <TableCell className="max-w-[200px] truncate text-xs text-muted-foreground">{f.remark || '-'}</TableCell>
