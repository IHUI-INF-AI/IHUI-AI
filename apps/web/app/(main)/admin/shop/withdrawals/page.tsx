...
const CHANNEL_LABEL: Record<Withdrawal['channel'], string> = { alipay: '支付宝', wechat: '微信', bank: '银行卡' }
const STATUS_LABEL: Record<Withdrawal['status'], string> = {
  pending: '待审核', approved: '已通过', rejected: '已驳回', completed: '已完成', failed: '已失败',
}
...
export default function AdminShopWithdrawalsPage() {
  ...
  const { data: list = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return api<{ list: Withdrawal[] }>(`/api/admin/shop/withdrawals?${qs.toString()}`).then((d) => d.list ?? [])
    },
  })
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={7} className="py-10 text-center text-muted-foreground">暂无提现申请</TableCell></TableRow>
            ) : (
              list.map((w) => (
                <TableRow key={w.id}>
                  ...
                  <TableCell className="font-mono text-xs text-muted-foreground">{w.account}</TableCell>
                  <TableCell>
                    <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs', STATUS_STYLE[w.status])}>
                  ...
                  <TableCell className="text-right">
                    {w.status === 'pending' && (
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="ghost" disabled={auditMut.isPending} onClick={() => auditMut.mutate({ id: w.id, action: 'approve' })}>
                          <Check className="h-3.5 w-3.5 text-emerald-600" />通过
                        </Button>
                        <Button size="sm" variant="ghost" disabled={auditMut.isPending} onClick={() => auditMut.mutate({ id: w.id, action: 'reject' })}>
                          <X className="h-3.5 w-3.5 text-red-600" />驳回
                        </Button>
                      </div>
                    )}
                  </TableCell>
