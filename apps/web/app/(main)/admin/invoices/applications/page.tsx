...
const STATUS_ORDER: InvoiceAppStatus[] = ['pending', 'approved', 'rejected', 'invoicing', 'invoiced', 'canceled']
...
export default function AdminInvoiceApplicationsPage() {
  ...
  function openStatus(app: InvoiceApplication) { setTarget(app); setNewStatus(app.status); setErr(null) }
  function closeDialog() { if (mut.isPending) return; setTarget(null); setErr(null) }
        ...
        {STATUS_TABS.map((tb) => (
          <button
            ...
            className={cn(
              ...
              status === tb.value ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground',
            )}
          >
            {tb.label}
          </button>
          ...
          <TableBody className="divide-y">
            {isLoading ? (
              ...
              rows.map((a) => {
                ...
                const typeLabel = a.invoiceType === 'company' ? '企业' : a.invoiceType === 'personal' ? '个人' : a.invoiceType
                return (
                  <TableRow key={a.id} className="hover:bg-muted/30">
                    <TableCell className="px-4 py-2.5">
                      <span className="inline-flex items-center rounded-full bg-muted px-2 py-0.5 text-xs font-medium">{typeLabel}</span>
                    </TableCell>
                    <TableCell className="px-4 py-2.5 font-medium">{currencyFmt.format(Number(a.amount))}</TableCell>
                    <TableCell className="px-4 py-2.5 text-muted-foreground">{a.email ?? '—'}</TableCell>
                    <TableCell className="px-4 py-2.5">
                      <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium', sc.cls)}>{sc.label}</span>
                    </TableCell>
                    <TableCell className="px-4 py-2.5 text-xs text-muted-foreground">{dateFmt.format(new Date(a.createdAt))}</TableCell>
                    <TableCell className="px-4 py-2.5 text-right">
                      <Button size="sm" variant="outline" onClick={() => openStatus(a)}>审核</Button>
                    </TableCell>
                  </TableRow>
                )
              })
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {target && (
              <div className="rounded-md bg-muted/40 px-3 py-2 text-sm">
                ...
                <div className="mt-0.5 text-xs text-muted-foreground">{target.invoiceType} · {STATUS_CFG[target.status]?.label}</div>
              </div>
            )}
            <div className="space-y-2">
              <Label htmlFor="inv-status">状态</Label>
              <Select value={newStatus} onValueChange={(v) => setNewStatus(v as InvoiceAppStatus)}>
                <SelectTrigger className={selectClass} id="inv-status"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={closeDialog} disabled={mut.isPending}>取消</Button>
              <Button type="submit" disabled={mut.isPending}>{mut.isPending && <Loader2 className="h-4 w-4 animate-spin" />}保存</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
