...
import { Loader2, LayoutDashboard, Server, Cpu, Activity, Database, Brain, CheckCircle2, XCircle } from 'lucide-react'
...
export default function AdminMonitorDashboardPage() {
  const { data: services = [], isLoading: svcLoading } = useQuery({
    ...
    queryFn: () => api<{ list: ServiceItem[] }>('/api/admin/monitor/services').then((d) => d.list ?? []),
  })
  ...
  const perfCards = [
    ...
    { label: '内存', value: perf?.memory ?? 0, unit: '%', max: 100, cls: 'text-blue-600' },
    ...
    { label: '平均响应', value: perf?.avgResponse ?? 0, unit: 'ms', max: 0, cls: 'text-amber-600', raw: true },
  ]
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between text-base">
              <span className="flex items-center gap-2"><Server className="h-4 w-4" />服务状态</span>
