...
export default function BiDashboardPage() {
  ...
  const trendOption: EChartsOption = {
    ...
    xAxis: { type: 'category', data: d.trend.map((t: BiTrend) => t.date), boundaryGap: false },
    yAxis: [
    ...
    series: [
      {
        ...
        type: 'line',
        smooth: true,
        data: d.trend.map((t: BiTrend) => t.revenue),
      ...
      },
      {
        ...
        yAxisIndex: 1,
        smooth: true,
        data: d.trend.map((t: BiTrend) => t.orders),
  ...
  const categoryOption: EChartsOption = {
    ...
    xAxis: { type: 'category', data: d.categories.map((c: BiCategory) => c.name) },
    yAxis: { type: 'value' },
    series: [
      {
        type: 'bar',
        data: d.categories.map((c: BiCategory) => c.value),
    ...
    series: [
      {
        ...
        label: { show: false },
        data: d.sources.map((s: BiSource) => ({ name: s.name, value: s.value })),
