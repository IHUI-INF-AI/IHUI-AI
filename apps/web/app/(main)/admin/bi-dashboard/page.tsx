...
export default function BiDashboardPage() {
  ...
  const trendOption: EChartsOption = {
    ...
    xAxis: { type: 'category', data: d.trend.map((t: any) => t.date), boundaryGap: false },
    yAxis: [
    ...
    series: [
      {
        ...
        type: 'line',
        smooth: true,
        data: d.trend.map((t: any) => t.revenue),
      ...
      },
      {
        ...
        yAxisIndex: 1,
        smooth: true,
        data: d.trend.map((t: any) => t.orders),
  ...
  const categoryOption: EChartsOption = {
    ...
    xAxis: { type: 'category', data: d.categories.map((c: any) => c.name) },
    yAxis: { type: 'value' },
    series: [
      {
        type: 'bar',
        data: d.categories.map((c: any) => c.value),
    ...
    series: [
      {
        ...
        label: { show: false },
        data: d.sources.map((s: any) => ({ name: s.name, value: s.value })),
