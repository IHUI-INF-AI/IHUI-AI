...
export default function AdminResourceCategoriesPage() {
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      ...
      if (editing) {
        return api<{ category: Category }>(`/api/admin/resources/categories/${editing.id}`, {
          method: 'PUT',
          body: JSON.stringify(body),
        })
      }
      return api<{ category: Category }>(`/api/admin/resources/categories`, {
        method: 'POST',
        body: JSON.stringify(body),
      })
    },
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => api(`/api/admin/resources/categories/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <ResourceCategoryFilter onCreate={openCreate} />
