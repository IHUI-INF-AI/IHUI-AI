...
import { Loader2, Plus, Download, Search, CreditCard } from 'lucide-react'
...
const RESOURCE = '/api/admin/auth-find-info'
const PERM = 'auth:auth_find_info'
const EMPTY = { userUuid: '', card: '', belong: '', title: '', message: '', createdAt: '' }
...
export default function AuthFindInfoPage() {
  ...
  const [search, setSearch] = React.useState({ userUuid: '', card: '' })
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api<{ list: AuthFindInfo[]; total: number }>(`${RESOURCE}?${new URLSearchParams(params)}`),
  })
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border p-4">
        ...
        <div className="space-y-1">
          ...
          <Input
            className="h-9 w-48"
            value={search.card}
            onChange={(e) => setSearch({ ...search, card: e.target.value })}
            placeholder="搜索卡号"
          />
        </div>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={form.belong}
                  onChange={(e) => setForm({ ...form, belong: e.target.value })}
                />
              </div>
              <div className="space-y-1.5">
                <Label>标题</Label>
