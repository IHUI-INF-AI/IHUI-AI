...
// ---- 进度条 ----
function Bar({ percent, severity }: { percent: number; severity: 'warning' | 'critical' | 'normal' }) {
  const cls =
    severity === 'critical'
  ...
  return (
    <div className="h-1.5 overflow-hidden rounded-sm bg-muted">
      <div className={cn('h-full rounded-sm', cls)} style={{ width: `${Math.min(percent, 100)}%` }} />
    </div>
  )
}
...
export function BudgetAlertsSection() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api<BudgetAlert[]>('/api/admin/ai/cost/budget-alerts').catch(() => [] as BudgetAlert[]),
    retry: false,
  })
            ...
            {data.map((a) => (
              <div key={a.userId} className="space-y-1.5 rounded-md border border-border p-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="truncate font-mono text-xs">{displayName({ nickname: null, email: null, username: a.scopeKey })}</span>
                  <span
                    className={cn(
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className="tabular-nums">
                      {fmtNum(a.dailyTokenUsed)} / {fmtNum(a.dailyTokenLimit)} ({a.dailyTokenPercent}%)
                    </span>
                  </div>
                  <Bar percent={a.dailyTokenPercent} severity={a.dailyTokenPercent >= 100 ? 'critical' : 'warning'} />
                </div>
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>{t('monthlyCost')}</span>
                    <span className="tabular-nums">
                      {curFmt.format(a.monthlyCostUsed / 100)} / {curFmt.format(a.monthlyCostLimit / 100)} ({a.monthlyCostPercent}%)
                    </span>
                  </div>
                  <Bar percent={a.monthlyCostPercent} severity={a.monthlyCostPercent >= 100 ? 'critical' : 'warning'} />
                </div>
              </div>
            ))}
          </div>
                  ...
                  <tr key={v.id} className="border-b last:border-0">
                    <td className="py-2 pr-3">
                      <div className="flex items-center gap-1.5">
                        <span className="rounded bg-muted px-1.5 py-0.5 text-xs">L{v.levelValue}</span>
                        <span className="text-xs">{v.levelName}</span>
                      </div>
                    </td>
                    <td className="py-2 pr-3 text-right tabular-nums">{fmtNum(v.activeUsers)}</td>
                    <td className="py-2 pr-3 text-right tabular-nums">{fmtNum(v.aiBudgetDefaults?.dailyTokenLimit ?? 0)}</td>
