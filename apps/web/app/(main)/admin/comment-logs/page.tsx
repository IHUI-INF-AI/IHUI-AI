...
export default function CommentLogsPage() {
  const t = useTranslations('admin.commentLogs')
  const [currentPage] = React.useState(1)

  const { data, isLoading } = useQuery({
    queryKey: ['admin', 'comment-logs', currentPage],
    queryFn: () => api<{ list: CommentLog[] }>('/api/admin/comment-logs'),
  })
            ...
            {isLoading ? (
              ...
              list.map((item) => (
                <tr key={item.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-2.5 text-muted-foreground">{item.target}</td>
                  <td className="px-4 py-2.5">{item.status === 1 ? t('normal') : t('hidden')}</td>
