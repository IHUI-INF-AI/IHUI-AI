...
export default function AdminLearnTopicCategoryPage() {
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      ...
      const url = editing
        ? `/api/learn/topics/categories/${editing.id}`
        : '/api/learn/topics/categories'
      return api<{ category: Category }>(url, {
  ...
  function openEdit(item: Category) {
    setEditing(item)
    setForm({ name: item.name, sort: String(item.sort), status: item.status === 1 })
    setErr(null)
    setOpen(true)
  }
    ...
    return list.map((item) => (
      <TableRow key={item.id} className="hover:bg-muted/30">
        ...
        <TableCell className="px-4 py-2.5">
          <StatusBadge enabled={item.status === 1} />
        </TableCell>
