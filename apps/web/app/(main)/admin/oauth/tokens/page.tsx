...
const STATUS_LABEL: Record<Token['status'], string> = { active: '有效', expired: '已过期', revoked: '已撤销' }
...
export default function AdminOAuthTokensPage() {
  ...
  const { data: list = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return api<{ list: Token[] }>(`/api/admin/oauth/tokens?${qs.toString()}`).then((d) => d.list ?? [])
    },
  })
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative w-full max-w-xs">
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索用户/应用" className="h-9 pl-3" />
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={8} className="py-10 text-center text-muted-foreground">暂无 Token</TableCell></TableRow>
            ) : (
              list.map((t) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell>
                    <span className={cn('inline-flex rounded px-1.5 py-0.5 text-xs', t.tokenType === 'access' ? 'bg-blue-500/10 text-blue-600' : 'bg-purple-500/10 text-purple-600')}>
                      {TYPE_LABEL[t.tokenType]}
                    </span>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {(t.scopes ?? []).map((s) => <span key={s} className="rounded bg-muted px-1.5 py-0.5 text-xs">{s}</span>)}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-xs text-muted-foreground">{t.lastUsedAt ? new Date(t.lastUsedAt).toLocaleString() : '—'}</TableCell>
                  <TableCell className="text-right">
                    {t.status === 'active' && (
                      <Button size="sm" variant="ghost" disabled={revokeMut.isPending} onClick={() => revokeMut.mutate(t.id)}>
