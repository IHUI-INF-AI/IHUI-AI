...
const STATUS_LABEL: Record<OAuthApp['status'], string> = { active: '正常', disabled: '已禁用', pending: '待审核' }
...
export default function AdminOAuthAppsPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'oauth', 'apps'] }); setOpen(false); setForm(EMPTY); setErr(null) },
    onError: (e: Error) => setErr(e.message),
  })

  const toggleMut = useMutation({
    mutationFn: (a: OAuthApp) =>
      api(`/api/admin/oauth/apps/${a.id}/status`, { method: 'PATCH', body: JSON.stringify({ status: a.status === 'active' ? 'disabled' : 'active' }) }),
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'oauth', 'apps'] }); setDelTarget(null) },
    onError: (e: Error) => setErr(e.message),
  })

  const submit = (e: React.FormEvent) => {
    e.preventDefault(); setErr(null)
    if (!form.name.trim()) { setErr('请输入应用名称'); return }
    if (!form.ownerId.trim()) { setErr('请输入所属用户ID'); return }
    createMut.mutate()
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={7} className="py-10 text-center text-muted-foreground">暂无应用</TableCell></TableRow>
            ) : (
              apps.map((a) => (
                <TableRow key={a.id}>
                  <TableCell className="font-medium">{a.name}</TableCell>
                  <TableCell className="font-mono text-xs">{a.clientId}</TableCell>
                  <TableCell className="text-xs text-muted-foreground">{a.ownerName || a.ownerId}</TableCell>
                  <TableCell className="max-w-[200px]">
                    <div className="space-y-0.5 text-xs text-muted-foreground">
                      {(a.redirectUris ?? []).slice(0, 2).map((u) => <div key={u} className="truncate">{u}</div>)}
                      {a.redirectUris.length > 2 && <div className="text-xs">+{a.redirectUris.length - 2}</div>}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {(a.scopes ?? []).slice(0, 3).map((s) => <span key={s} className="rounded bg-muted px-1.5 py-0.5 text-xs">{s}</span>)}
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs', STATUS_STYLE[a.status])}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button size="sm" variant="ghost" onClick={() => toggleMut.mutate(a)} disabled={toggleMut.isPending}>
                        <Power className="h-3.5 w-3.5" />
                        {a.status === 'active' ? '禁用' : '启用'}
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => { setErr(null); setDelTarget(a) }}>
        ...
        <DialogContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="oa-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="oa-owner">所属用户ID</Label>
              <Input id="oa-owner" value={form.ownerId} onChange={(e) => setForm({ ...form, ownerId: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="oa-redirect">回调地址（逗号分隔）</Label>
              <Input id="oa-redirect" value={form.redirectUris} onChange={(e) => setForm({ ...form, redirectUris: e.target.value })} placeholder="https://example.com/callback" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="oa-scopes">权限（逗号分隔）</Label>
              <Input id="oa-scopes" value={form.scopes} onChange={(e) => setForm({ ...form, scopes: e.target.value })} placeholder="user:read,user:write" />
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={createMut.isPending}>取消</Button>
      ...
      <Dialog open={!!delTarget} onOpenChange={(o) => (o ? null : (setDelTarget(null), setErr(null)))}>
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setDelTarget(null)} disabled={delMut.isPending}>取消</Button>
            <Button type="button" variant="destructive" disabled={delMut.isPending} onClick={() => delTarget && delMut.mutate(delTarget.id)}>
