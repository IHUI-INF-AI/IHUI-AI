...
const STATUS_LABEL: Record<AuthRecord['status'], string> = { success: '成功', denied: '拒绝', revoked: '已撤销' }
...
export default function AdminOAuthAuditPage() {
  ...
  const { data: list = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return api<{ list: AuthRecord[] }>(`/api/admin/oauth/audit?${qs.toString()}`).then((d) => d.list ?? [])
    },
  })
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative w-full max-w-xs">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索用户/应用" className="h-9 pl-8" />
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={7} className="py-10 text-center text-muted-foreground"><ScrollText className="mx-auto mb-2 h-8 w-8 opacity-40" />暂无授权记录</TableCell></TableRow>
            ) : (
              list.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {(r.scopes ?? []).map((s) => <span key={s} className="rounded bg-muted px-1.5 py-0.5 text-xs">{s}</span>)}
                    </div>
                  </TableCell>
                  <TableCell className="font-mono text-xs text-muted-foreground">{r.ip}</TableCell>
                  <TableCell>
                    <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs', STATUS_STYLE[r.status])}>
                      <span className="h-1.5 w-1.5 rounded-full bg-current" />
                      {STATUS_LABEL[r.status]}
                    </span>
                  </TableCell>
                  <TableCell className="text-xs text-muted-foreground">{new Date(r.createdAt).toLocaleString()}</TableCell>
                  <TableCell className="text-xs text-muted-foreground">{r.expiresAt ? new Date(r.expiresAt).toLocaleString() : '永久'}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
