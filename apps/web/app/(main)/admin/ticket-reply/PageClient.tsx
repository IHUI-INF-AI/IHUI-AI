...
export default function AdminTicketReplyPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await fetchApi<TicketReplyListData>(
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        工单 ID:{ticketId || '—'} · 共 {total} 条回复
      </p>
      <div className="overflow-x-auto rounded-lg border bg-card">
        <table className="w-full text-sm">
          <thead className="text-left text-xs uppercase text-muted-foreground">
            <tr>
              <th className="px-4 py-2.5 font-medium">回复人</th>
              ...
              list.map((r: TicketReply) => (
                <tr key={r.id}>
                  ...
                  <td className="px-4 py-2.5">
                    <span
                      className={`rounded px-2 py-0.5 text-xs ${r.isAdmin ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}
                    >
                      {r.isAdmin ? '客服' : '用户'}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-muted-foreground">
                    {r.createdAt ? dtf.format(new Date(r.createdAt)) : '-'}
                  </td>
                </tr>
              ))
            )}
