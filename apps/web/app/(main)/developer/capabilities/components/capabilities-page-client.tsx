...
import { MyKeysPanel } from './my-keys-panel'

/**
 * 能力目录面板(O15)。整页只有三块:红线说明 → 目录浏览 → 我的 key。
 * TooltipProvider 在此统一挂载(徽章 hover 说明依赖它),不再逐层包裹。
 */
export default function CapabilitiesPageClient(): React.JSX.Element {
      ...
      <div className="mx-auto w-full max-w-5xl space-y-4 px-4 py-4">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight">
            ...
            <CountBadge count={CAPABILITY_CATALOG.length} />
          </h1>
          <p className="mt-0.5 text-sm text-muted-foreground">{t('capabilities.subtitle')}</p>
        </div>
        <CapabilityLegend />
        <CapabilityBrowser />
        <MyKeysPanel />
