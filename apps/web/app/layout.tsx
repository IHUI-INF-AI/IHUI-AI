        ...
        */}
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){var d=document.documentElement;d.style.setProperty('--z-base','1');d.style.setProperty('--z-sticky','990');d.style.setProperty('--z-modal','2000');d.style.setProperty('--z-popover','2001');d.style.setProperty('--z-notification','9999');d.style.setProperty('--z-max','10003');d.style.setProperty('--z-0','0');d.style.setProperty('--z-header','100');d.style.setProperty('--z-dropdown','1000');d.style.setProperty('--z-overlay','1000');d.style.setProperty('--z-loading','10000');try{var sw=localStorage.getItem('sidebar-width');if(sw){var sn=parseInt(sw,10);if(sn>=160&&sn<=180){d.style.setProperty('--sidebar-width',sn+'px');}else{d.style.setProperty('--sidebar-width','160px');}}else{d.style.setProperty('--sidebar-width','160px');}}catch(e){d.style.setProperty('--sidebar-width','160px');}try{if(localStorage.getItem('sidebar-collapsed')==='true'){d.style.setProperty('--sidebar-width','60px');}}catch(e){}try{var raw=localStorage.getItem('ihui-ai-panel');if(raw){var p=JSON.parse(raw);var w=p&&p.state&&p.state.width;if(typeof w==='number'&&w>=320&&w<=720){d.style.setProperty('--ai-panel-width',w+'px');d.style.setProperty('--ai-panel-occupy',(w+6)+'px');return;}}}catch(e){}d.style.setProperty('--ai-panel-width','380px');d.style.setProperty('--ai-panel-occupy','386px');try{var stored=localStorage.getItem('theme');if(stored!=='light'&&stored!=='dark'){var isTauri=('__TAURI_INTERNALS__' in window)||('__TAURI__' in window);if(isTauri){d.classList.add('dark');d.style.colorScheme='dark';}}}catch(e){}})();`,
          }}
        />
        {/*
