...
describe('TimelineStore — upsertEvent', () => {
  ...
  it('已存在 id + upsertEvent → events 长度不变,内容被 merge 更新', () => {
    useTimelineStore.getState().addEvent(
      makeTimelineEvent({ id: 'up-2', title: '原始标题', status: 'running' }),
    )
    expect(events().length).toBe(1)
    useTimelineStore.getState().upsertEvent(
      makeTimelineEvent({ id: 'up-2', title: '更新标题', status: 'done' }),
    )
    expect(events().length).toBe(1)
  ...
  it('upsertEvent 后再 upsertEvent 同一 id → 长度不变,内容为第二次 merge', () => {
    useTimelineStore.getState().upsertEvent(
      makeTimelineEvent({ id: 'up-4', title: '第一次', status: 'running' }),
    )
    useTimelineStore.getState().upsertEvent(
      makeTimelineEvent({ id: 'up-4', title: '第二次', status: 'done' }),
    )
    expect(events().length).toBe(1)
  ...
  it('upsertEvent 与 addEvent 共存(先 addEvent 再 upsertEvent 同一 id → merge)', () => {
    useTimelineStore.getState().addEvent(
      makeTimelineEvent({ id: 'up-7', title: 'addEvent', status: 'running' }),
    )
    useTimelineStore.getState().upsertEvent(
      makeTimelineEvent({ id: 'up-7', title: 'upsertEvent', status: 'done' }),
    )
    expect(events().length).toBe(1)
  ...
  it('upsertEvent 与 updateEvent 共存(先 upsertEvent 再 updateEvent 同一 id → merge)', () => {
    useTimelineStore.getState().upsertEvent(
      makeTimelineEvent({ id: 'up-8', title: 'upsertEvent', status: 'running' }),
    )
...
describe('TimelineStore — upsertEvent 边界场景', () => {
  ...
  it('upsertEvent 与 setEvents 共存:setEvents 覆盖后 upsertEvent 仍可工作', () => {
    ...
    // upsertEvent 更新其中一个
    useTimelineStore.getState().upsertEvent(
      makeTimelineEvent({ id: 'edge-3a', title: '更新 A', status: 'done' }),
    )
    expect(events().length).toBe(2)
  ...
  it('upsertEvent children 字段:新 children 覆盖旧 children', () => {
    ...
    useTimelineStore.getState().upsertEvent(makeTimelineEvent({ id: 'edge-5', children: newChildren }))
