...
describe('ConversationImportPanel — 外部会话导入四步流', () => {
  ...
  it('场景2: 选中来源后显示上传区,accept 按来源过滤', () => {
    renderPanel()

    pickSourceAndFileBeforeChange('claude_code')
    expect(
      (screen.getByTestId('import-file-input') as HTMLInputElement).getAttribute('accept'),
    ).toBe('.jsonl,.json')
    pickSourceAndFileBeforeChange('cursor')
    expect(
      (screen.getByTestId('import-file-input') as HTMLInputElement).getAttribute('accept'),
    ).toBe('.json,.vscdb')
