 ...
 *   - jsdom 在某些错误路径上构造异常大的 stack 字符串(内嵌 sourceMap data),
 *     触发 V8 内置 stack parser 中 `/at (?:(.+)\s+)?\(?(?:...)\)?/g` 类正则
 *     在大字符串上回溯爆炸 → `RangeError: Maximum call stack size exceeded`
 ...
 *   - jsdom 内部抛错的 stack 变得简洁,日志可读性提升
 */
...
// 注入后会包一层,但 vite 是直接覆盖 Error.prepareStackTrace,所以我们
