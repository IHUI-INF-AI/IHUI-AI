...
import { filterByCapabilities, groupByCategory, splitByTier, type ModelOption } from '@/components/chat/model-tier-utils'
...
describe('filterByCapabilities — 历史区能力过滤(2026-09-19 D23 立)', () => {
  ...
  it('单键过滤:仅 capabilities[key] === true 保留,键缺失按 false', () => {
    ...
    expect(filterByCapabilities(list, ['vision']).map((m) => m.value)).toEqual([
      'vision-model',
    ])
  })
  ...
  it('多键 AND:全部激活能力满足才保留', () => {
    ...
    expect(filterByCapabilities(list, ['vision', 'fim']).map((m) => m.value)).toEqual([
      'both',
    ])
  })
  ...
  it('非布尔值按 false 处理而非崩溃', () => {
    const list = [
      opt('weird', 'chat', 'legacy', { vision: 'yes' as unknown as true }),
    ]
