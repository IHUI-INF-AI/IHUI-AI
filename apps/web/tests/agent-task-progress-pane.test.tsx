...
describe('AgentTaskProgressPane — v13 深度优化', () => {
  ...
  it('PlanStepItem status icon 含 transition-colors duration-300 类(状态切换动画)', () => {
    useAgentProgressPaneStore.getState().openPane()
    ...
    // 检查每个 step 至少有一个 icon 含 transition-colors 类
    const allStepRows = container.querySelectorAll('[data-testid^="plan-step-"]')
    allStepRows.forEach((row) => {
      // icon className 应含 transition-colors(在 row 内部的 [data-testid="lucide-icon"] 上)
      const icon = row.querySelector('[data-testid="lucide-icon"]')
      if (icon) {
        expect(icon.className).toContain('transition-colors')
      }
    })
  })
