...
// ─── @radix-ui/react-tooltip mock:为 TimelineTab/TimelineEventRow 内的 <Tooltip> from '@/components/feedback' 提供 Provider 替身 ──
...
describe('Timeline i18n 化', () => {
  ...
  it('9. i18nParams 正确传递(iteration/tool)', () => {
    ...
    const oUpdate = mapProgressToTimelineUpdate(makeProgressEvent({ phase: 'output_ready' }))!
...
describe('Timeline 事件快捷筛选', () => {
  ...
  it('20. filteredEvents 响应式(store 更新 events 后列表自动更新)', () => {
    ...
    useTimelineStore
      .getState()
      .setEvents([makeEvent({ id: 'p1', type: 'plan', title: 'Plan 1', status: 'done' })])
    useTimelineStore.getState().setActiveTab('timeline')
    const { container } = render(<TimelineTab />)
    // 初始:1 个 plan
