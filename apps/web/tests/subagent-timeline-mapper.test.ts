...
describe('mapProgressToTimelineUpdate', () => {
  ...
  it('phase="tool_result" + tool="search" + ok=true → description 含 "工具返回" + "search" + "ok" 或 "成功"', () => {
    ...
    // 接受 "ok" 或 "成功" 两种实现
    expect(
      result!.updates.description!.match(/(ok|成功)/i),
    ).not.toBeNull()
  })
  ...
  it('phase="tool_result" + tool="search" + ok=false → description 含 "failed" 或 "失败"', () => {
    ...
    // 接受 "failed" 或 "失败" 两种实现
    expect(
      result!.updates.description!.match(/(failed|失败)/i),
    ).not.toBeNull()
  })
  ...
  it('meta 正确设置(phase / iteration / tool / ok / outputPreview)', () => {
    // thinking
    const r1 = mapProgressToTimelineUpdate(
      makeProgressEvent({ phase: 'thinking', iteration: 3 }),
    )!
