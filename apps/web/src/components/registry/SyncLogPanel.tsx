...
const STATUS_STYLE: Record<RegistrySyncStatus, { label: string; cls: string }> = {
  ...
  skipped: {
    label: '跳过',
    cls: 'bg-slate-100 text-slate-600 dark:bg-slate-900 dark:text-slate-400',
  },
  running: {
    label: '运行中',
            ...
            <ul className="space-y-1 p-3">
              {logs.map((log) => {
                ...
                return (
                  <li
                    key={log.id}
                    className="flex items-center gap-3 rounded-md px-2 py-1.5 text-xs hover:bg-accent/30"
                  >
                    <span className={cn('shrink-0 rounded-md px-1.5 py-0.5 font-medium', st.cls)}>
                      {st.label}
                    </span>
                    <span className="w-12 shrink-0 text-muted-foreground">
                      {TYPE_LABEL[log.sourceType] ?? log.sourceType}
                    </span>
