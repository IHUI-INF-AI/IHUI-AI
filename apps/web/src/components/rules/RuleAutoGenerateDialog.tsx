  ...
  return (
    ...
    >
      <div className="flex max-h-[85vh] w-full max-w-lg flex-col space-y-3 rounded-lg border border-border bg-card p-3 shadow-lg">
        ...
        <p className="text-[10px] text-muted-foreground">{t('autoGenerateHint')}</p>

        {!result && !loading && !error && (
          <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
            <Sparkles className="h-8 w-8 text-muted-foreground" />
            <Button size="sm" onClick={handleGenerate}>
              {t('startAnalyzeBehavior')}
            </Button>
          </div>
        )}
          ...
          <div className="thin-scroll space-y-2 overflow-y-auto">
            {result.degraded && (
              <div className="rounded-sm bg-yellow-500/10 px-2 py-1 text-[10px] text-yellow-600">
                {result.message ?? t('degradedNoCandidates')}
              </div>
            )}
            {result.behaviorCount !== undefined && (
            ...
            {result.candidates.length === 0 ? (
              <p className="py-4 text-center text-xs text-muted-foreground">{t('noCandidates')}</p>
            ) : (
              result.candidates.map((candidate, idx) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {t('confidencePercent', { pct: (candidate.confidence * 100).toFixed(0) })}
                    </span>
                  </div>
        ...
        {result && result.candidates.length > 0 && (
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-muted-foreground">
              {t('selectedOfTotal', { selected: selected.size, total: result.candidates.length })}
            </span>
            <div className="flex items-center gap-2">
              ...
              >
                {creating ? t('creating') : t('batchCreateCount', { n: selected.size })}
              </Button>
            </div>
          </div>
        )}
