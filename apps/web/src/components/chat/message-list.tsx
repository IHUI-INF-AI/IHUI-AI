...
import { Sparkles, AlertCircle, Loader2, ChevronDown, ShieldCheck, ShieldAlert, Hand, MessageSquare, ListTree } from 'lucide-react'
...
import { MessageContextMenu, plainTextForClipboard, markdownForClipboard } from '@/components/ai/progress-sections/message-context-menu'
...
export function MessageList({
  ...
  const derivedEvents = React.useMemo<TimelineEvent[]>(() => {
    ...
    for (const m of messages) {
      ...
      for (const tc of m.toolCalls ?? []) {
        events.push({
          ...
          title: tc.toolName,
          status:
            tc.status === 'success' ? 'done' : tc.status === 'error' ? 'failed' : 'running',
          messageId: m.id,
          toolCallId: tc.id,
        })
      }
    ...
    for (const sa of subAgentActivities) {
      events.push({
        ...
        status:
          sa.status === 'completed' ? 'done' : sa.status === 'failed' ? 'failed' : 'running',
  ...
  React.useEffect(() => {
    ...
    return () =>
      window.removeEventListener('ihui:scroll-to-message', onScrollTo as EventListener)
  }, [flashHighlight])
