 ...
 * 非预览模式时不渲染任何内容。
 *
 * 固定定位(top-9 = GlobalTopBar 高度 h-9=36px),z-sticky 与 AISidePanel 同级。
 */
export function PreviewBanner(): React.ReactElement | null {
  const isPreviewDraft = useIsPreviewDraft()
  if (!isPreviewDraft) return null
  return (
    <div className="fixed left-0 right-0 top-9 z-sticky flex items-center justify-center gap-1.5 bg-amber-500/95 px-4 py-1.5 text-center text-xs font-medium text-white shadow-sm">
