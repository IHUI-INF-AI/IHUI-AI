  ...
  return (
    <div className="font-mono text-xs leading-tight" data-testid="tasks-list">
      {sorted.map((step, idx) => {
        ...
        return (
          // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions -- 鼠标点击选 cursor;键盘用户通过全局 j/k 移动 cursor + Enter 展开提供等价交互
          <div
            key={step.id}
            className={cn(
          ...
          sub.status === 'running'
            ...
            : sub.status === 'done'
              ...
              : sub.status === 'failed'
                ...
                : colorCls
        return (
          // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions -- 鼠标点击选 cursor + 双击展开;键盘用户通过全局 j/k + Enter 提供等价交互
          <div
            key={sub.id}
            className={cn(
              'cursor-pointer px-1 py-0.5 transition-colors hover:bg-accent/30',
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              {/* P2-2 修复:移除冗余 [status] 文字(已有颜色 + 图标 + 状态行) */}
        ...
        return (
          ...
          >
            ...
            {showFull && term.output && (
              <pre className="mt-0.5 ml-7 max-h-48 overflow-y-auto whitespace-pre-wrap break-all bg-muted/30 p-1 text-[10px] leading-relaxed">
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-muted-foreground">
          ...
          <div><span className="text-foreground">PgUp</span> page up</div>
          <div><span className="text-foreground">Enter</span> expand</div>
          <div><span className="text-foreground">dbl-click</span> expand(item)</div>
          <div><span className="text-foreground">dbl-click</span> reset(resize)</div>
...
export function AgentTaskProgressPane() {
  ...
  // P1-1 修复:handleApprove 实际调用 toast 反馈(后端 approval API 待实现)
  const handleApprove = React.useCallback((id: string, approve: boolean) => {
    toast.info(approve ? `已批准 ${id}` : `已拒绝 ${id}`, { description: '子代理审批链路待后端实现' })
  }, [])

  // 鼠标点击条目 → setCursor
  const handleSelectCursor = React.useCallback((idx: number) => {
    useAgentProgressPaneStore.getState().setCursor(idx)
  }, [])

  // 双击 resize handle → 重置默认高度
  ...
  const searchMatchCount = React.useMemo(() => {
    if (!searchQuery) return { match: 0, total: 0 }
    const filter = (text: string) => matchesQuery(text, searchQuery)
    if (activeColumn === 'tasks') {
      const total = planSteps.length
      const match = planSteps.filter(
        (s) => filter(s.step) || filter(s.explanation ?? ''),
      ).length
      return { match, total }
    }
    if (activeColumn === 'subagents') {
      ...
      const match = archived.filter(
        (s) =>
          filter(s.handle) ||
          filter(s.nickname) ||
          filter(s.currentTask ?? '') ||
          filter(s.role ?? ''),
      ).length
      return { match, total }
    }
    ...
    const total = archived.length
    const match = archived.filter(
      (t) => filter(t.command) || filter(t.output ?? ''),
    ).length
    return { match, total }
  }, [searchQuery, activeColumn, planSteps, subagents, terminals, showArchived])
  ...
  React.useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // 焦点在输入控件时不拦截(INPUT/TEXTAREA/SELECT/contentEditable)
      // P1-3 修复:此检查必须在 showHelp 之前,否则帮助面板打开时输入框无法输入
      ...
      // 帮助面板打开时,任意键关闭(输入框已 return,不影响输入)
      ...
      // PgUp:向上翻页(5 项)
      if (e.key === 'PageUp' && !ctrl && !shift && !alt) {
        e.preventDefault()
        moveCursor(-5, visibleCount)
        return
      }
  ...
  // P1-2 修复:sortMode 变化时重置 cursor 到 0(避免排序后 cursor 指向错项)
  React.useEffect(() => {
    if (!open) return
    useAgentProgressPaneStore.getState().setCursor(0)
  }, [open, sortMode])

  // P1-4 修复:showArchived / activeColumn 切换后 cursor 超出可见范围时 clamp 到 max-1
  React.useEffect(() => {
    if (!open) return
    if (visibleCount <= 0) return
    const cur = useAgentProgressPaneStore.getState().cursorIndex
    if (cur >= visibleCount) {
      useAgentProgressPaneStore.getState().setCursor(visibleCount - 1)
    }
  }, [open, visibleCount, showArchived, activeColumn])
  ...
  return (
    ...
    >
      {/* Resize 手柄(drag 调节面板高度 + 双击重置) */}
      <ResizeHandle height={paneHeight} onHeightChange={setPaneHeight} onReset={handleResetHeight} />

      {/* Header:单行紧凑状态 + currentNode + 进度百分比 + 任务计时 + error */}
      <div className="flex items-center gap-2 border-b border-border px-2 py-1 text-xs">
        ...
        {/* P2-1 修复:移除冗余 ● 流式指示器(running 状态的 ⠋ spinner 已表示流式中) */}
        {/* SSE error 状态展示 */}
        {overview.error && (
          <span
          ...
          >
            ⚠ {overview.error.length > 30 ? `${overview.error.slice(0, 30)}…` : overview.error}
          </span>
        )}
        {/* 重连尝试指示器 */}
      ...
      {/* ThreadId 输入栏(仅 threadId 未设置时显示)+ 引导文案 */}
      {!threadId && (
        <div className="border-b border-border bg-muted/20 px-2 py-1">
          <div className="mb-1 text-[10px] text-muted-foreground/70" data-testid="thread-id-hint">
            输入 Agent threadId 查看 SSE 流式进度(Tasks / Subagents / Terminals 三栏)
          </div>
      ...
      {searchMode && (
        <SearchBar
          query={searchQuery}
          matchCount={searchMatchCount.match}
          totalCount={searchMatchCount.total}
      ...
      <div className="flex items-center gap-3 border-t border-border px-2 py-0.5 text-[10px] text-muted-foreground/70">
        ...
        <span>
          <span className="text-muted-foreground">PgUp</span> pgup
        </span>
        <span>
