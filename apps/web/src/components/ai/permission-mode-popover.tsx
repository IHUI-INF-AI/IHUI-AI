    ...
    <Popover
      content={
        ...
        >
          ...
          {/* "查看历史"按钮(2026-07-25 立,深度对标 Codex CLI 审计能力):
              - 通过 window.__IHUI_OPEN_HISTORY__?.() 触发 PermissionHistoryPanel
              - 关闭当前 popover(避免两层浮层叠加)
              - 图标 + 小字极简风格,与"完全访问"深色卡片形成视觉对比 */}
          <button
            type="button"
            onClick={() => {
              if (typeof window !== 'undefined') {
              ...
              // 关闭当前 popover(用户期望点"查看历史"后跳出本 popover 看历史面板)
              setIsOpen(false)
            }}
            data-testid="permission-mode-popover-history-trigger"
            className={cn(
              'mt-1 flex w-full items-center gap-2 rounded-md px-2 py-1 text-left transition-colors',
              'text-muted-foreground hover:bg-muted/50 hover:text-foreground',
            )}
          >
            <Clock4 className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
            <span className="flex-1 text-[11px] font-medium">{t('historyOpenExternal')}</span>
          </button>
