...
export function AISidePanel() {
  ...
  // 宽度闪变修复(2026-09-08 用户反馈"刷新时先窄过一会又变宽"):
  // SSR 用 noopStorage 以默认宽度 300 渲染,客户端 zustand persist rehydrate 恢复
  // 用户拖拽偏好宽度,React 水合时补丁 style 触发宽度跳变;而 transition 会把这次
  // 跳变渲染成 0.2s 动画,被感知为"慢慢变宽"。首帧挂载前禁用所有宽度过渡,
  // 让水合宽度瞬时落定,挂载后(setMounted)再恢复过渡动画。
  const [mounted, setMounted] = React.useState(false)
  React.useEffect(() => setMounted(true), [])
  const dockedTransition = mounted && !isResizing ? 'width 0.2s cubic-bezier(0.4,0,0.2,1)' : 'none'
  ...
  // - 2026-08-02 修复:768-1023px(平板/小笔记本窗口)不再自动全屏,保持 docked 400px,
        ...
        <button
          ...
          className={cn(
            ...
            isMobileSmall
              ...
              : // 桌面/平板端:48px FAB,位置由 floatPosition 控制
                'h-12 w-12',
          )}
          style={
            isMobileSmall
  ...
  // 2026-07-31 移动端适配:加 hidden min-[768px]:block 让 docked 关闭态(手柄)在 < 768px 隐藏,
  // 避免 AISidePanel 在手机视口下占 400px 宽把 work-area 推到 viewport 外。
  // 2026-08-02 阈值 1024→768:768-1023px(平板/小笔记本)保持 docked 可见,不全屏。
  ...
  if (!open) {
    return (
      <>
        {workspaceNameSync}
        <div
          className="relative hidden h-full shrink-0 py-2 mr-1.5 min-[768px]:block"
          style={{ width: 0 }}
        >
