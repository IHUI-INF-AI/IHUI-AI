...
function HistoryList({ entries, now }: HistoryListProps) {
  ...
  if (entries.length === 0) {
    return (
      <div className="py-6 text-center text-xs text-muted-foreground">{t('historyEmpty')}</div>
    )
  }
  return (
...
function StatsFooter() {
  const t = useTranslations('chat.permission')
  const stats: { mode: WorkspacePermissionMode; labelKey: 'mode.ask' | 'mode.auto' | 'mode.full' }[] = [
