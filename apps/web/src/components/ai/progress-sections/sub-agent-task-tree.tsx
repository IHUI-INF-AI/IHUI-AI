...
import { SUBAGENT_COLOR_CLASS, type Subagent, type SubagentStatus } from '@/hooks/use-agent-progress'
import { formatDuration } from './foldable-section'

/**
 * SubAgentTaskTree — Subagent 任务树形嵌套(2026-07-28 立,Trae Work 对齐)
 *
 * 设计:
 * - 批次(可选,顶层)→ subagent → 子操作 3 层缩进结构
 * - 每层支持折叠/展开(本组件只渲染单棵 subagent 树,批次由 BatchHeader 负责)
 * - 状态点:running=脉冲,completed=绿,failed=红
 * - 工具调用:内嵌 checklist 展示
 * - memo 化
 */
...
export const SubAgentTaskTree = React.memo(function SubAgentTaskTree({
  ...
  return (
    <div
      className={cn('relative space-y-0.5 rounded-md border border-border/40 bg-card/30 p-1.5', className)}
    ...
    >
      ...
      >
        ...
        <span className="shrink-0 text-[11px] font-medium text-foreground/90">{subagent.nickname}</span>
