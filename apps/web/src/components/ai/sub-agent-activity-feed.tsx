...
export function SubAgentActivityFeed({
  ...
  // Phase 20 升级:autoTree 默认开启 — 2+ activities 时自动启用树形展示
  // (批次派发场景天然是树形结构,扁平列表信息密度低,树形更直观)
  const autoTree = activities.length >= 2
  const useTree = treeMode || autoTree || (batches !== undefined && batches.length > 0)
