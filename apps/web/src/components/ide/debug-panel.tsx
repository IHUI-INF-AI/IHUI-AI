...
function VariableRow({ v, depth = 0 }: { v: LocalVariable; depth?: number }) {
  ...
  return (
    <div>
      ...
      {hasChildren &&
        expanded &&
        v.children!.map((c, i) => <VariableRow key={i} v={c} depth={depth + 1} />)}
    </div>
  )
}
...
export function DebugPanel() {
  ...
  const [breakpoints, setBreakpoints] = React.useState<Breakpoint[]>(() => {
    if (typeof window === 'undefined') return []
    try {
      return JSON.parse(localStorage.getItem(BREAKPOINTS_KEY) ?? '[]')
    } catch {
      return []
    }
  })
  ...
  React.useEffect(() => {
    if (!sessionId || currentFrameId === null || currentFrameId === undefined) {
      setVariables([])
      return
    }
    let cancelled = false
    getVariables(sessionId, currentFrameId)
      .then((res) => {
        if (!cancelled) setVariables(res.variables.map(adaptVar))
      })
      .catch((e: unknown) => {
        if (!cancelled) {
          setVariables([])
  ...
  const toggleBreakpoint = (id: string) =>
    setBreakpoints((p) => p.map((b) => (b.id === id ? { ...b, enabled: !b.enabled } : b)))
  const removeBreakpoint = (id: string) => setBreakpoints((p) => p.filter((b) => b.id !== id))
  const addWatch = () => {
    if (watchInput.trim()) {
      setWatches((p) => [...p, watchInput.trim()])
      setWatchInput('')
    }
  }
  ...
  const language =
    activeTab?.language ?? LANG_MAP[program.split('.').pop()?.toLowerCase() ?? ''] ?? 'typescript'
            ...
            <div className="mb-2">
              <div className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-muted-foreground">
                ...
                <span className="ml-auto text-muted-foreground">
                  {enabledBreaks}/{breakpoints.length}
                </span>
              </div>
              {breakpoints.map((b) => (
                <div
                  key={b.id}
                  className="group flex items-center gap-1.5 px-2 py-0.5 text-xs hover:bg-muted/30"
                >
                  <button
                    onClick={() => toggleBreakpoint(b.id)}
