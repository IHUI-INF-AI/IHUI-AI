...
function parseGrepResults(raw: unknown): SearchResult[] {
  ...
  for (const item of raw) {
    ...
    const line =
      typeof r.line === 'number' ? r.line : typeof r.lineNumber === 'number' ? r.lineNumber : 0
    const content =
    ...
    <div className="flex w-72 shrink-0 flex-col bg-muted/20">
      <div className="flex items-center gap-1 px-2 py-1.5">
        ...
        <button
          ...
          aria-label={t('searchPanel.wholeWord')}
        >
      ...
      <div className="flex-1 overflow-auto">
        {!workspacePath ? (
          <div className="px-3 py-2 text-xs text-muted-foreground">
            {t('searchPanel.openWorkspaceHint')}
          </div>
        ) : loading ? (
          <div className="px-3 py-2 text-xs text-muted-foreground">
              ...
              return (
                <div key={filename} className="mb-0.5">
                  ...
                  >
                    ...
                    <span className="ml-auto rounded bg-muted px-1 text-muted-foreground">
                      {items.length}
                    </span>
                  </button>
                  {!collapsed && (
                    <div className="ml-4">
                      {items.map((r, i) => (
                        <div
                          key={i}
                          className="cursor-pointer rounded px-1.5 py-0.5 text-xs hover:bg-muted/30"
                        >
