...
export function TiptapToolbar({ editor }: { editor: Editor }) {
  ...
  const handleSetLink = async () => {
    const url = await prompt({ title: t('linkPrompt'), defaultValue: 'https://' })
    if (url === null) return
    if (url === '') {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const fallbackUrl = await prompt({
          title: t('imageUploadFailedPrompt'),
          defaultValue: 'https://',
        })
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-1 border-b bg-muted/30 p-2">
        ...
        />
        <ToolbarBtn icon={ImageIcon} title={t('image')} onClick={() => void handleSetImage()} />
        <Sep />
        <ToolbarBtn
          icon={AlignLeft}
          title={t('alignLeft')}
