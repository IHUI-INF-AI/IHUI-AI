 ...
 *   - 实际写入复用 workspace-tool-executor.ts 的 writeFileToHandle(同 splitPath/ensureDirHandle
 *     / getFileHandle / createWritable 路径),确保人工保存与 AI 写文件行为一致,不另起炉灶。
 *
...
import { writeFileToHandle } from '@/lib/workspace-tool-executor'
...
export async function saveWorkspaceTextFile(
  ...
  try {
    const { bytes } = await writeFileToHandle(handle, rel, content)
