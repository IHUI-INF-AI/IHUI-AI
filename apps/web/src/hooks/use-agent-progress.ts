...
function extractPlanFromEvents(events: SSEEvent[]): PlanStep[] {
  ...
  if (planSnapshots.length > 0) {
    ...
    const data = lastSnapshot.data as { explanation?: string; plan?: Array<{ step: string; status: PlanStepStatus }> } | undefined
...
function extractSubagentsFromEvents(events: SSEEvent[]): Subagent[] {
  ...
  for (const evt of events) {
    if ((evt.type as string) === 'subagent_spawn') {
      ...
      const data = evt.data as { id?: string; threadId?: string; status?: 'done' | 'failed' } | undefined
      ...
      const data = evt.data as { id?: string; threadId?: string; status?: SubagentStatus; task?: string; pendingApproval?: boolean } | undefined
      const id = data?.id ?? data?.threadId ?? ''
      const existing = id ? map.get(id) : undefined
      if (existing) {
...
export function useAgentProgress(threadId: string | null): UseAgentProgressReturn {
  ...
  // 聚合 terminals
  const terminals = React.useMemo<TerminalTask[]>(() => extractTerminalsFromEvents(events), [events])
