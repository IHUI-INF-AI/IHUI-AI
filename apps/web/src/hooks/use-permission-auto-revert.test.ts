...
describe('usePermissionAutoRevert', () => {
  ...
  it('场景 3: expired record + 刷新场景 → 自动切回 default + toast + 清除 localStorage', async () => {
    ...
    // 2026-07-25: 首次渲染时 auto-switch effect 因 record===null 把 watchedRecordRef 重置为 null,
    // hydration effect 在同一轮把 record 设为 expired record,但 watchedRecordRef 未同步更新,
    // 导致第二轮渲染时 watchedRecordRef !== record → 跳过自动切回。
    // 触发 storage 事件让 storage-sync effect 把 watchedRecordRef 同步到 record,
    // 使 auto-switch effect 能通过 watchedRecordRef === record 校验。
    await act(async () => {
