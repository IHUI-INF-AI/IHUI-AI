...
describe('useWorkPanelStore - 代理导航历史语义(2026-09-02 fix)', () => {
  ...
  it('loaded 广播(302 落点)修正当前条目为真实 URL,不压栈', () => {
    ...
    expect(tab().history).toEqual(['https://a.com/', 'http://www.iana.org/help/example-domains'])
  ...
  it('proxy 模式 reload 保持代理通道(带 cache-buster,不回落 iframe 重探测)', () => {
    seed(['https://a.com', 'https://b.com'], 1, 'proxy')
    useWorkPanelStore.getState().reload()
    const t = tab()
    expect(t.state.mode).toBe('proxy')
    expect(t.state.status).toBe('loading')
    expect(t.state.proxyUrl).toContain('/api/embed-proxy/raw?url=https%3A%2F%2Fb.com')
    expect(t.state.proxyUrl).toMatch(/&_ihui_reload=\d+$/)
    expect(t.url).toBe('https://b.com') // 不动 tab.url / 历史
    expect(t.historyIndex).toBe(1)
    expect(probeEmbed).not.toHaveBeenCalled()
  })
})
