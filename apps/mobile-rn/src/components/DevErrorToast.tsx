 ...
 * 与项目统一悬浮条(FloatBox)风格不符且无样式 API → 自研替换:
 * - App.tsx 调 LogBox.ignoreAllLogs() 抑制内核 UI(metro 终端日志不受影响)
 * - 包装 console.error 收集报错:未处理错误/未处理 promise 拒绝经
 *   ExceptionsManager 最终都走 console.error,统一在此捕获
 * - 样式对齐 FloatBox:深色半透明底 + 圆角 + danger 色左边条;
 *   新增投影(elevation/shadow)与左右内收(16),折叠态即显示错误文字
 * - 点击条目展开完整 message + 堆栈(可选中复制);右侧 X 清除全部
...
const ICON_SIZE = 16
const BADGE_SIZE = 18
/** 样式常量:深底白字对齐 FloatBox(半透明黑底 + surface.light 文字) */
...
const styles = StyleSheet.create({
  ...
  bar: {
    ...
    borderLeftColor: tokens.danger.DEFAULT,
    paddingHorizontal: BAR_PADDING_H,
    paddingVertical: BAR_PADDING_V,
    // 项目统一悬浮条投影(Android elevation / iOS shadow)
    elevation: 8,
    shadowColor: '#000000',
    shadowOpacity: 0.35,
    shadowRadius: 8,
