...
interface Cert { id: string; certNo: string; title: string; issuer: string; holder: string; issuedAt: string; expiredAt?: string; score: number; verifyUrl: string }
...
export function CertDetailScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const res = await fetchApi<Cert>(`/api/certs/${encodeURIComponent(id)}`)
      if (cancelled) return
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.certCard}>
        ...
        <Text style={styles.certNo}>{t('certDetail.certNo')}:{cert.certNo}</Text>
        ...
        {cert.expiredAt ? <><Text style={styles.label}>{t('certDetail.expiredAt')}</Text><Text style={styles.value}>{cert.expiredAt}</Text></> : null}
      </View>
      <TouchableOpacity style={styles.verifyBtn} onPress={() => navigation.navigate('CertVerify', { certNo: cert.certNo })}>
...
const styles = StyleSheet.create({
  ...
  certCard: { padding: 16, borderRadius: 8, borderWidth: 2, borderColor: PRIMARY, backgroundColor: '#ecfdf5' },
  ...
  verifyBtn: { marginTop: 16, paddingVertical: 12, borderRadius: 8, borderWidth: 1, borderColor: PRIMARY, alignItems: 'center' },
