...
export function PointRuleScreen() {
  ...
  const load = useCallback(async () => {
    ...
    else setError(t('pointRule.loadFailed'))
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        ...
        <Text style={s.title}>{t('pointRule.title')}</Text>
      </View>
      ...
      {loading && items.length === 0 ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={s.center}><Text style={s.muted}>{t('pointRule.empty')}</Text></View>}
          renderItem={({ item }) => (
            <View style={s.card}>
