...
export function PostCreateScreen() {
  ...
  const onSubmit = async () => {
    if (!title.trim() || !content.trim()) { setError(t('postCreate.required')); return }
    setSaving(true); setError('')
    const res = await fetchApi<{ id: string }>('/api/posts', {
      method: 'POST',
      body: JSON.stringify({ title: title.trim(), content: content.trim(), circleId, tags: tags.split(',').map((s) => s.trim()).filter(Boolean) }),
    })
    setSaving(false)
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder={t('postCreate.titlePlaceholder')} placeholderTextColor="#9ca3af" />
      <Text style={styles.label}>{t('postCreate.contentLabel')}</Text>
      <TextInput style={[styles.input, styles.textarea]} value={content} onChangeText={setContent} placeholder={t('postCreate.contentPlaceholder')} placeholderTextColor="#9ca3af" multiline textAlignVertical="top" />
      <Text style={styles.label}>{t('postCreate.tagsLabel')}</Text>
      <TextInput style={styles.input} value={tags} onChangeText={setTags} placeholder={t('postCreate.tagsPlaceholder')} placeholderTextColor="#9ca3af" />
