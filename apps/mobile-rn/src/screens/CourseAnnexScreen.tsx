  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        ...
        <Text style={s.title}>{t('courseAnnex.title')}</Text>
      </View>
      ...
      {loading && items.length === 0 ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={s.center}><Text style={s.muted}>{t('courseAnnex.empty')}</Text></View>}
          renderItem={({ item }) => (
            <View style={s.card}>
              <Text style={s.cardTitle} numberOfLines={1}>{item.name}</Text>
              <View style={s.metaRow}>
                <Text style={s.cardSize}>{t('courseAnnex.size')}: {fmtSize(item.size)}</Text>
                <Text style={s.cardAction}>{t('courseAnnex.download')}</Text>
              </View>
            </View>
          )}
        />
