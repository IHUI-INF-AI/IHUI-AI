    ...
    <View className="flex-1 bg-card">
      ...
      <View className="p-4">
        ...
        <Input
          className="mt-1"
          value={setting.model}
          onChangeText={(v) => setSetting({ ...setting, model: v })}
        />
        ...
        <Input
          ...
          onChangeText={(v) => setSetting({ ...setting, temperature: Number(v) || 0 })}
        />
        <View className="mt-3 flex-row items-center justify-between">
          <Text className="text-xs text-muted-foreground">{t('agentSetting.enabled')}</Text>
          <Switch
            value={setting.enabled}
            onValueChange={(v) => setSetting({ ...setting, enabled: v })}
            thumbColor={tokens.brand.DEFAULT}
          />
        </View>
        ...
        <TouchableOpacity
          className={`mt-4 items-center rounded-lg bg-primary py-2.5 ${saving ? 'opacity-50' : ''}`}
          onPress={save}
          disabled={saving}
        >
