...
interface ExamResult { id: string; examTitle: string; score: number; totalScore: number; passed: boolean; correctCount: number; totalCount: number; duration: number; submittedAt: string; wrongQuestions: Array<{ index: number; question: string; yourAnswer: string; correctAnswer: string }> }
...
export function ExamResultScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const res = await fetchApi<ExamResult>(`/api/exams/results/${encodeURIComponent(id)}`)
      if (cancelled) return
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={[styles.scoreCard, result.passed && styles.passedCard]}>
        <Text style={styles.scoreValue}>{result.score}/{result.totalScore}</Text>
        <Text style={[styles.passedLabel, result.passed ? styles.passed : styles.failed]}>{result.passed ? t('examResult.passed') : t('examResult.failed')}</Text>
      </View>
      <View style={styles.statRow}>
        <Text style={styles.stat}>{t('examResult.correct', { count: result.correctCount, total: result.totalCount })}</Text>
        ...
        <>
          ...
          {result.wrongQuestions.map((q) => (
            <View key={q.index} style={styles.card}>
              <Text style={styles.qIdx}>{q.index + 1}. {q.question}</Text>
              <Text style={styles.wrongAns}>{t('examResult.yourAnswer')}:{q.yourAnswer}</Text>
              <Text style={styles.correctAns}>{t('examResult.correctAnswer')}:{q.correctAnswer}</Text>
            </View>
          ))}
        </>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  scoreCard: { padding: 16, borderRadius: 8, borderWidth: 1, borderColor: '#e5e7eb', alignItems: 'center', marginBottom: 12 },
