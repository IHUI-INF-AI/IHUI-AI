  ...
  return (
    <View className="flex-1 bg-card px-4 pt-12">
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center py-10">
            <Text className="text-[13px] text-muted-foreground">{t('agentChat.empty')}</Text>
          </View>
        }
        renderItem={({ item }) => (
          <View
            className={`mb-2 max-w-[85%] rounded-lg p-2.5 ${item.role === 'user' ? 'self-end bg-primary' : 'self-start bg-muted'}`}
          >
            <Text
              className={`text-sm ${item.role === 'user' ? 'text-primary-foreground' : 'text-foreground'}`}
            >
              {item.content}
            </Text>
          </View>
      ...
      <View className="flex-row items-center gap-2 py-2">
        ...
        />
        <TouchableOpacity
          className={`rounded-lg bg-primary px-3.5 py-2 ${!input.trim() || sending ? 'opacity-50' : ''}`}
          onPress={onSend}
