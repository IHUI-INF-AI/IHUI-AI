...
interface ExamHistory { id: string; examTitle: string; score: number; totalScore: number; passed: boolean; submittedAt: string }
...
export function ExamHistoryScreen() {
  ...
  const load = async (refresh = false) => {
    ...
    setError('')
    const res = await fetchApi<ExamHistory[]>('/api/exams/history')
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View style={styles.empty}><Text style={styles.muted}>{t('examHistory.empty')}</Text></View>}
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('ExamResult', { id: item.id })}>
            <View style={styles.cardHead}>
              <Text style={styles.cardTitle} numberOfLines={1}>{item.examTitle}</Text>
              <Text style={[styles.status, item.passed ? styles.statusPass : styles.statusFail]}>{item.passed ? t('examHistory.passed') : t('examHistory.failed')}</Text>
            </View>
            <View style={styles.cardFoot}>
              <Text style={styles.score}>{item.score}/{item.totalScore}</Text>
...
const styles = StyleSheet.create({
  ...
  status: { fontSize: 11, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, marginLeft: 8 },
  statusPass: { color: PRIMARY, backgroundColor: '#ecfdf5' },
  statusFail: { color: '#dc2626', backgroundColor: '#fef2f2' },
  cardFoot: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 6 },
  score: { fontSize: 13, color: PRIMARY, fontWeight: '600' },
