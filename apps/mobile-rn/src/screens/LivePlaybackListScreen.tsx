...
interface Item { id: string; title: string; lecturer: string; duration: number; viewerCount: number; createdAt: string }
...
export function LivePlaybackListScreen() {
  ...
  const load = useCallback(async () => {
    setError('')
    try {
      const r = await fetch(`${API_BASE_URL}/api/live/playback`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      if (!r.ok) throw new Error()
      const d = (await r.json()) as { data?: Item[] }
      setItems(d.data ?? [])
    } catch { setError(t('livePlaybackList.loadFailed')) } finally { setLoading(false); setRefreshing(false) }
  }, [token, t])
    ...
    <View style={s.container}>
      ...
      {loading && items.length === 0 ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={s.center}><Text style={s.muted}>{t('livePlaybackList.empty')}</Text></View>}
          renderItem={({ item }) => (
            <View style={s.card}>
              <Text style={s.cardTitle} numberOfLines={1}>{item.title}</Text>
              <Text style={s.cardMeta}>{t('livePlaybackList.lecturer')}: {item.lecturer}</Text>
              <View style={s.metaRow}>
                <Text style={s.cardMeta}>{t('livePlaybackList.duration')}: {fmtDur(item.duration)} · {t('livePlaybackList.viewerCount')}: {item.viewerCount}</Text>
