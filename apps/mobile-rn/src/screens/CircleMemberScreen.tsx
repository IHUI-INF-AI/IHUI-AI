...
export function CircleMemberScreen() {
  ...
  return (
    <View className="flex-1 bg-card px-4 pt-12">
      ...
      <Text className="mb-3 mt-2 text-[22px] font-semibold text-foreground">{t('circleMember.title')}</Text>
      <FlatList
        data={members}
        ...
        renderItem={({ item }) => (
          <Card className="mb-2 flex-row items-center p-3">
            ...
            <View className="flex-1">
              <Text className="text-sm font-medium text-foreground" numberOfLines={1}>{item.nickname}</Text>
              <Text className="mt-0.5 text-[11px] text-muted-foreground">{item.joinedAt}</Text>
            </View>
            <Badge
              variant={item.role === 'owner' ? 'default' : 'secondary'}
              label={t(`circleMember.${item.role}`)}
            />
          </Card>
        )}
      />
