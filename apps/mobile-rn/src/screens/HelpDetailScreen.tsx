...
interface Detail { id: string; question: string; answer: string; category: string }
...
export function HelpDetailScreen() {
  ...
  const load = useCallback(async () => {
    setError('')
    try {
      const r = await fetch(`${API_BASE_URL}/api/help/faq/${id}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      if (!r.ok) throw new Error()
      const d = (await r.json()) as { data?: Detail }
      setItem(d.data ?? null)
    } catch { setError(t('helpDetail.loadFailed')) } finally { setLoading(false) }
  }, [id, token, t])
...
const s = StyleSheet.create({
  ...
  categoryBadge: { fontSize: 11, color: '#FFFFFF', backgroundColor: '#10B981', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8, overflow: 'hidden' },
