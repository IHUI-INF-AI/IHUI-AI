...
const MENU: MenuItem[] = [
  { key: 'Order', labelKey: 'profile.orders', icon: '🧾' },
  { key: 'Favorites', labelKey: 'profile.favorites', icon: '★' },
  { key: 'Following', labelKey: 'profile.following', icon: '👤' },
  { key: 'Subscriptions', labelKey: 'profile.subscriptions', icon: '🔁' },
  { key: 'Wallet', labelKey: 'profile.wallet', icon: '💰' },
  { key: 'Agent', labelKey: 'profile.agent', icon: '🤖' },
  { key: 'Settings', labelKey: 'profile.settings', icon: '⚙' },
]
...
export function ProfileScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-white dark:bg-black" contentContainerStyle={{ paddingBottom: 32 }}>
      ...
      <View className="px-4">
        <Card>
          ...
          {user?.id ? (
            <Text className="mt-1 text-xs text-neutral-500">ID: {user.id}</Text>
          ) : null}
      ...
      <View className="mt-4 px-4">
        {MENU.map((m) => (
          <TouchableOpacity
            key={m.key}
            onPress={() => navigation.navigate(m.key as never)}
            activeOpacity={0.7}
