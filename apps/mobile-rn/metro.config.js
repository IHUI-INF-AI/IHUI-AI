      ...
      if (!entry) {
        for (const key of Object.keys(pkgJson.exports)) {
          if (key.includes('*')) {
            ...
            if (exportKey.startsWith(prefix) && exportKey.endsWith(suffix) && exportKey.length >= prefix.length + suffix.length) {
              const wildcardMatch = exportKey.substring(prefix.length, exportKey.length - suffix.length)
