...
// 2026-07-29 修复:APK 里的 Hermes 运行时版本与 Metro 0.81 的 Hermes 编译器版本不匹配,
// 导致 "bad application bundle" 错误(JS 模块零注册)。
// 临时方案:重写 bundle 请求 URL,把 transform.bytecode=1 改成 0,让 Metro 返回 JS 文本
// 而不是 Hermes 字节码。JS 文本由 Hermes 运行时直接解析,不依赖字节码版本匹配。
// 根本方案:重新构建 APK(expo run:android)让 Hermes 版本一致后移除此重写。
config.server = {
  ...config.server,
  rewriteRequestUrl: (url) => {
    return url.replace('transform.bytecode=1', 'transform.bytecode=0')
  },
}
