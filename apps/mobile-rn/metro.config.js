...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  try {
    ...
    // 加 conditions 支持 ESM 包(如 @ihui/shared type:module 只有 import 条件)
    try {
      const originRealPath = fs.realpathSync(context.originModulePath)
      const resolved = require.resolve(moduleName, {
        paths: [path.dirname(originRealPath)],
        conditions: ['import', 'require', 'default', 'node', 'types'],
      })
      return { type: 'sourceFile', filePath: resolved }
    } catch (_e2) {
      // 3. 最终 fallback:手动解析包名 + 子路径 + 扩展名
      const originRealPath = fs.realpathSync(context.originModulePath)
...
function resolveManual(moduleName, originDir, platform) {
  ...
  if (path.isAbsolute(moduleName)) {
    ...
    let pkgJson
    try { pkgJson = JSON.parse(fs.readFileSync(pkgJsonPath, 'utf8')) } catch { return null }
    // 优先用 exports 解析子路径
    if (pkgJson.exports) {
      const exportKey = subPath ? `./${subPath}` : '.'
      const entry = pkgJson.exports[exportKey] || pkgJson.exports['./*'] || pkgJson.exports['*']
      if (entry) {
        // entry 可能是字符串或对象 { import: '...', require: '...' }
        const target = typeof entry === 'string' ? entry : (entry.import || entry.require || entry.default)
        if (target) {
