...
// pnpm isolated linker 兼容:watchFolders 只用 projectRoot(2026-07-29 修复)
// 根因:Windows 无 watchman,Metro FileMap 用 FallbackWatcher(sane)监视多 root 时
// 240s 超时卡死。packages/ 共享代码通过 nodeModulesPaths + unstable_enableSymlinks
// 由 resolver 解析,不需要在 FileMap 初始扫描范围内。
// 2026-07-29 补丁:NativeWind 的 react-native-css-interop/.cache/ 目录必须加入
// watchFolders,否则 forceWriteFileSystem=true 写入的 android.js 等虚拟 CSS 模块
// 不在 FileMap 扫描范围,Metro 报 "Failed to get the SHA-1" 错误。
const fs = require('fs')
...
// 确保 cache 目录存在(NativeWind 初始化时也会创建,提前创建避免竞态)
try {
  fs.mkdirSync(cssInteropCacheDir, { recursive: true })
} catch {}
config.watchFolders = [__dirname, cssInteropCacheDir]
