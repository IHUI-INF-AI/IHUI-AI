...
// rpx → px 间距基准(对齐历史 Uniapp 750 设计宽,1rpx = 0.5px @375pt)。
// 用法:p-r40 = 20px、m-r20 = 10px、w-r750 = 375px(=屏宽)。仅对与原稿 rpx 数值对齐的场景启用,
// 存量 screen 的 p-4 等默认 tailwind 间距保持不变,逐步审计替换为 r* 类以消除系统性间距漂移。
const _rpxScale = {}
for (let i = 1; i <= 400; i++) {
  _rpxScale['r' + i] = i / 2
}
