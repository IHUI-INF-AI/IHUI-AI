...
import {
  ...
  CarrierLoginError,
  CarrierErrorCode,
} from '../src/services/carrier-login.js'
...
const endpoint =
  process.env.FLASHVERIFY_API_URL || 'https://wsflash.253.com/open/flashsdk/mobile-query'
...
try {
  ...
  if (code !== null && code !== '200000') {
    console.log(
      `ℹ️ 网关已受理请求(appid+签名校验通过),返回业务码=${code}。占位 token 得不到真机号属预期。`,
    )
  }
} catch (err: unknown) {
  ok('网关可达', false)
...
function decrypt(name: string, key: string) {
  ...
  return Buffer.concat([d.update(Buffer.from(name, 'hex')), d.final()])
    .toString('utf8')
    .trim()
}
const enc = createCipheriv(
  'aes-128-cbc',
  createHash('md5').update(appKey!, 'utf8').digest('hex').slice(0, 16),
  createHash('md5').update(appKey!, 'utf8').digest('hex').slice(16, 32),
)
