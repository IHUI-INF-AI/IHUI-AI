...
async function main() {
  ...
  await exec(
    `ALTER TABLE "ai_world_categories" ADD COLUMN IF NOT EXISTS "description" varchar(500)`,
  )
  await exec(
    `ALTER TABLE "ai_world_categories" ADD COLUMN IF NOT EXISTS "created_at" timestamp with time zone DEFAULT now() NOT NULL`,
  )
  await exec(
    `ALTER TABLE "ai_world_categories" ADD COLUMN IF NOT EXISTS "updated_at" timestamp with time zone DEFAULT now() NOT NULL`,
  )
  ...
  await exec(
    `ALTER TABLE "ai_world_items" ADD COLUMN IF NOT EXISTS "published_at" timestamp with time zone`,
  )
  await exec(
    `ALTER TABLE "ai_world_items" ADD COLUMN IF NOT EXISTS "fetched_at" timestamp with time zone DEFAULT now() NOT NULL`,
  )
  await exec(
    `ALTER TABLE "ai_world_items" ADD COLUMN IF NOT EXISTS "metadata" jsonb DEFAULT '{}'::jsonb NOT NULL`,
  )
  await exec(
    `ALTER TABLE "ai_world_items" ADD COLUMN IF NOT EXISTS "like_count" integer DEFAULT 0 NOT NULL`,
  )

  // 4. 索引
  await exec(
    `CREATE INDEX IF NOT EXISTS "ix_ai_world_sync_log_source" ON "ai_world_sync_log" USING btree ("source")`,
  )
  await exec(
    `CREATE INDEX IF NOT EXISTS "ix_ai_world_sync_log_started_at" ON "ai_world_sync_log" USING btree ("started_at")`,
  )
  await exec(
    `CREATE INDEX IF NOT EXISTS "ix_ai_world_items_kind" ON "ai_world_items" USING btree ("kind")`,
  )
  await exec(
    `CREATE INDEX IF NOT EXISTS "ix_ai_world_items_source" ON "ai_world_items" USING btree ("source")`,
  )

  // 5. 唯一约束
  await exec(
    `ALTER TABLE "ai_world_categories" DROP CONSTRAINT IF EXISTS "ai_world_categories_slug_unique"`,
  )
  await exec(
    `ALTER TABLE "ai_world_categories" ADD CONSTRAINT "ai_world_categories_slug_unique" UNIQUE("slug")`,
  )
  await exec(
    `ALTER TABLE "ai_world_items" DROP CONSTRAINT IF EXISTS "uq_ai_world_items_kind_source_url"`,
  )
  await exec(
