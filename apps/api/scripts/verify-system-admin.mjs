...
try {
  const admin = await sql`
    ...
    FROM users
    WHERE username = 'admin' OR email = '[REDACTED-EMAIL]' OR phone = '[REDACTED-PHONE]'
  `
  console.log('Found', admin.length, 'admin-related rows:')
  for (const u of admin) {
    const ok = await verifyPassword('[REDACTED-PW]', u.password_hash)
    console.log(JSON.stringify({ ...u, password_hash: '[REDACTED]', password_[REDACTED-PW]: ok }, null, 2))
  }
