...
describe('paypal service', () => {
    ...
    it('配置 PAYPAL_WEBHOOK_ID 时调用 Verify-API 返回 SUCCESS 则验签通过', async () => {
      ...
      const mockFetch = vi.fn()
        .mockResolvedValueOnce(
          new Response(
            JSON.stringify({ access_token: 'token-xxx', expires_in: 32400 }),
  ...
  describe('getAccessToken', () => {
    ...
    it('成功获取 token 后缓存命中,第二次不调 fetch', async () => {
      ...
      const mockFetch = vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({ access_token: 'cached-token', expires_in: 32400 }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        ),
      )
      global.fetch = mockFetch
    ...
    it('token 过期后重新获取(模拟过期)', async () => {
      ...
      const mockFetch = vi.fn()
        ...
        .mockResolvedValueOnce(
          new Response(
            JSON.stringify({ access_token: 'token-2', expires_in: 32400 }),
            { status: 200, headers: { 'Content-Type': 'application/json' } },
          ),
        )
      global.fetch = mockFetch
  ...
  describe('captureOrder', () => {
    it('无配置时抛错', async () => {
      await expect(captureOrder('PAYID-XXX')).rejects.toThrow(
        /PayPal OAuth2 token failed/,
      )
    })
  ...
  describe('getOrderStatus', () => {
    it('无配置时抛错', async () => {
      await expect(getOrderStatus('PAYID-XXX')).rejects.toThrow(
        /PayPal OAuth2 token failed/,
      )
    })
