...
async function createRun(
  ...
  const res = await app.inject({
    method: 'POST',
    url: '/',
    headers: { 'idempotency-key': key, ...(over.user ? { 'x-test-user': over.user } : {}) },
...
describe('① 幂等 run 创建', () => {
  ...
  it('并发同 key → 一个建成、一个 409 IN_PROGRESS,创建只发生一次', async () => {
    ...
    expect(both.map((item) => item.status).sort()).toEqual([201, 409])
    const rejected = both.find((item) => item.status === 409)
    expect(rejected?.body.errorCode).toBe(IDEMPOTENCY_IN_PROGRESS_CODE)
    expect(created).toBe(1)
  })
