...
const rows = (from: number, to: number): Row[] =>
  Array.from({ length: to - from + 1 }, (_unused, idx) => ({
    id: `r${from + idx}`,
    seq: from + idx,
  }))
...
describe('buildCursorPage', () => {
  it('信封形状是本仓 {code,message,data},data 带 items/has_more/next_cursor', () => {
    ...
    expect(Object.keys(res.envelope.data).sort()).toEqual(['has_more', 'items', 'next_cursor'])
  ...
  it('追加新数据后,旧游标仍指向同一处:已交付的不重发、新增的排其后', () => {
    ...
    // 旧断言拿"从 r4 续翻那一页的 next_cursor"当参照,但那一页只剩 r5 一条、已无下一页 ⇒
    // 参照值恒为 null,比的根本不是同一件事。改成直接验证游标可续:用它翻第 3 页必须正好拿到 r5。
    const c3 = page2After.envelope.data.next_cursor
    expect(typeof c3).toBe('string')
    const p3 = readPageQuery({
      query: { limit: 2, after: c3 },
      binding: bindingFor(1),
      secret: SECRET,
    })
    expect(p3.ok).toBe(true)
    if (!p3.ok) throw new Error('第 2 页末尾游标应可被解析')
    expect(page(rows(1, 5), p3.request).envelope.data.items.map((row) => row.id)).toEqual(['r5'])
  })
