...
describe('plot-advisor-service 剧情顾问', () => {
  ...
  describe('addPlotPoint', () => {
    ...
    it('resolvedForeshadowIds 回收伏笔', async () => {
  ...
  describe('checkConsistency 一致性检查', () => {
    it('无问题时返回空数组', async () => {
      expect(await checkConsistency(SID)).toEqual([])
    })

    it('未回收伏笔产生 warning', async () => {
      addPlotPoint(SID, { id: 'f1', chapter: 1, type: 'foreshadow', description: '伏笔1' })
      const issues = await checkConsistency(SID)
    ...
    it('关系对称检查（father vs son 合法）', async () => {
      addCharacter(SID, {
        id: 'father',
        name: '父亲',
        traits: [],
    ...
    it('关系不对称检查（father vs spouse 不合法）', async () => {
      addCharacter(SID, {
        id: 'a',
        name: 'A',
        traits: [],
    ...
    it('故事不存在返回空数组', async () => {
      expect(await checkConsistency('no-story')).toEqual([])
    })
  })

  describe('analyzePacing 节奏分析', () => {
    it('故事不存在返回 0 分', async () => {
      const r = await analyzePacing('no-story')
    ...
    it('空故事给出缺失建议', async () => {
      const r = await analyzePacing(SID)
  ...
  describe('suggestChapterOutline 章节大纲', () => {
    it('故事不存在返回空数组', async () => {
      expect(await suggestChapterOutline('no-story', 1)).toEqual([])
    })

    it('返回章节建议列表', async () => {
      const outline = await suggestChapterOutline(SID, 1)
      expect(outline.length).toBeGreaterThan(0)
      expect(outline[0]).toContain('第 1 章建议聚焦')
    })

    it('第 5 章后无高潮时建议进入高潮', async () => {
      addPlotPoint(SID, { id: 's1', chapter: 1, type: 'setup', description: 's' })
      addPlotPoint(SID, { id: 's2', chapter: 1, type: 'setup', description: 's' })
      addPlotPoint(SID, { id: 's3', chapter: 1, type: 'setup', description: 's' })
      const outline = await suggestChapterOutline(SID, 6)
      expect(outline.some((s) => s.includes('高潮'))).toBe(true)
    })

    it('无特殊建议时给出默认推进主线建议', async () => {
      addPlotPoint(SID, { id: 's1', chapter: 1, type: 'setup', description: 's' })
      addPlotPoint(SID, { id: 's2', chapter: 1, type: 'setup', description: 's' })
      addPlotPoint(SID, { id: 'c1', chapter: 2, type: 'conflict', description: 'c' })
      ...
      addPlotPoint(SID, { id: 'cl1', chapter: 3, type: 'climax', description: 'cl' })
      addPlotPoint(SID, { id: 'r1', chapter: 4, type: 'resolution', description: 'r' })
      const outline = await suggestChapterOutline(SID, 3)
