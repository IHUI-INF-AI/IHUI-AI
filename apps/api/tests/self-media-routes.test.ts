...
describe('self-media routes — POST /api/self-media/koubo/generate', () => {
  ...
  describe('代理行为', () => {
    ...
    it('请求体为空时转发 {}(body ?? {} 兜底)', async () => {
      mockRegularUser()
      fetchMock.mockResolvedValue(
        makeUpstreamResponse(JSON.stringify({ code: 0, message: 'success', data: {} }), 200, 'application/json'),
      )
    ...
    it('转发 Authorization header 到 ai-service(共享鉴权上下文)', async () => {
      mockRegularUser()
      fetchMock.mockResolvedValue(
        makeUpstreamResponse('{"ok":true}', 200, 'application/json'),
      )
  ...
  describe('错误处理', () => {
    ...
    it('上游 JSON 解析失败时透传原始文本(content-type 声明 JSON 但 body 非法)', async () => {
      mockRegularUser()
      fetchMock.mockResolvedValue(
        makeUpstreamResponse('{not valid json', 200, 'application/json'),
      )
