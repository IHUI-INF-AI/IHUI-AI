...
describe('TenantAuditor check — SELECT 缺 tenant_id', () => {
  it('业务表 select 无 tenant_id → 违规', () => {
    const a = new TenantAuditor()
    const v = a.check('SELECT * FROM zhs_order WHERE status = 1', [], null, 'u1')
    expect(v).not.toBeNull()
    expect(v?.table).toBe('zhs_order')
  ...
  it('select 含 tenant_id → 通过', () => {
    const a = new TenantAuditor()
    expect(a.check('SELECT * FROM zhs_order WHERE tenant_id = 1', [], 1, 'u1')).toBeNull()
  })

  it('select 参数首位是数字 → 启发式通过', () => {
    const a = new TenantAuditor()
    expect(a.check('SELECT * FROM zhs_order WHERE id = $1', [42], 1, 'u1')).toBeNull()
  })

  it('select 参数首位是过大数字(>100000)→ 启发式不通过', () => {
    const a = new TenantAuditor()
    const v = a.check('SELECT * FROM zhs_order WHERE id = $1', [999999])
    expect(v).not.toBeNull()
  })
})
...
describe('TenantAuditor check — UPDATE/DELETE', () => {
  it('update 缺 tenant_id → 违规', () => {
    const a = new TenantAuditor()
    const v = a.check('UPDATE zhs_order SET status = 0 WHERE id = 1', [], 1, 'u1')
    expect(v).not.toBeNull()
    expect(v?.kind).toBe('missing_tenant_filter')
  })
  ...
  it('delete 缺 tenant_id → 违规', () => {
    const a = new TenantAuditor()
    expect(a.check('DELETE FROM zhs_order WHERE id = 1', [], 1, 'u1')).not.toBeNull()
  })
})
...
describe('TenantAuditor check — INSERT', () => {
  it('insert 无 tenant_id → 违规 (insert_no_tenant)', () => {
    const a = new TenantAuditor()
    const v = a.check("INSERT INTO zhs_order (name, amount) VALUES ('x', 1)", [], 1, 'u1')
    expect(v).not.toBeNull()
  ...
  it('insert 含 tenant_id → 通过', () => {
    const a = new TenantAuditor()
    expect(
      a.check("INSERT INTO zhs_order (name, tenant_id) VALUES ('x', 1)", [], 1, 'u1'),
    ).toBeNull()
  })
})
...
describe('TenantAuditor 配置', () => {
  ...
  it('whitelistTable 移动表到跳过集合', () => {
    const a = new TenantAuditor()
    a.whitelistTable('zhs_order')
    expect(a.check('SELECT * FROM zhs_order', [], 1, 'u1')).toBeNull()
  })
...
describe('TenantAuditor stats/recentViolations/reset', () => {
  it('stats 准确计数 byTable/byTableViolations', () => {
    const a = new TenantAuditor()
    a.check('SELECT * FROM zhs_order WHERE id = 1', [], 1, 'u1')
    a.check('SELECT * FROM zhs_order WHERE id = 2', [], 1, 'u1')
    a.check('SELECT * FROM user_vip WHERE id = 1', [], 1, 'u1')
    const s = a.stats()
    expect(s.auditedQueries).toBe(3)
    expect(s.violations).toBe(3)
    expect(s.byTable.zhs_order).toBe(2)
    expect(s.byTableViolations.zhs_order).toBe(2)
  })

  it('violationRate 保留 4 位小数', () => {
    const a = new TenantAuditor()
    a.check('SELECT * FROM zhs_order WHERE id = 1', [], 1, 'u1')
    a.check('SELECT * FROM zhs_order WHERE tenant_id = 1', [], 1, 'u1')
    a.check('SELECT * FROM zhs_order WHERE tenant_id = 1', [], 1, 'u1')
    const s = a.stats()
  ...
  it('recentViolations 限制返回数量', () => {
    const a = new TenantAuditor()
    for (let i = 0; i < 5; i++) a.check('SELECT * FROM zhs_order WHERE id = 1', [], 1, 'u1')
...
describe('TenantAuditor 告警回调', () => {
  it('违规率超阈值且达最小审计量时触发 alertFn', () => {
    ...
    expect(fn.mock.calls[0][0]).toBe('tenant_audit_violation:zhs_order')
  })
  ...
  it('未达 minAuditsForAlert 时不告警', () => {
    ...
    const fn = vi.fn()
    a.setAlertFn(fn)
    a.check('SELECT * FROM zhs_order WHERE id = 1', [], 1, 'u1') // 1/1=100% 但未达 50
...
describe('TenantAuditor 大小写与表名变体', () => {
  it('表名大小写不敏感', () => {
    const a = new TenantAuditor()
    expect(a.check('SELECT * FROM ZHS_ORDER WHERE id = 1', [], 1, 'u1')).not.toBeNull()
  })
})
