...
export async function findPublishedPapers(opts: {
  ...
  const [rows, totalRows] = await Promise.all([
    ...
    db.select({ count: sql<number>`COUNT(*)` }).from(examPapers).where(where),
  ]);
...
export async function updatePaper(id: string, data: UpdatePaperInput): Promise<ExamPaper | undefined> {
  const rows = await db
    .update(examPapers)
    .set({
...
export async function createQuestion(paperId: string, data: CreateQuestionInput): Promise<ExamQuestion> {
  const rows = await db
...
export async function updateQuestion(id: string, data: UpdateQuestionInput): Promise<ExamQuestion | undefined> {
  const rows = await db
...
async function syncQuestionCount(paperId: string): Promise<void> {
  ...
  await db.update(examPapers).set({ questionCount: count, updatedAt: new Date() }).where(eq(examPapers.id, paperId));
}
 ...
 * 管理员查询全站答题记录(分页,支持搜索)。
 */
export async function findAdminExamRecords(
...
export async function findAdminExamRecordsRich(
  opts: { page: number; pageSize: number; search?: string; paperId?: string; status?: string },
