...
export async function ensureMargin(userId: string): Promise<UserMargin> {
  const existing = await db.select().from(userMargins).where(eq(userMargins.userId, userId)).limit(1);
...
export async function refundToken(userId: string, quantity: number, remark?: string): Promise<number> {
  return rechargeToken(userId, quantity, undefined, remark ?? '退款');
}

export async function expireToken(userId: string, quantity: number, source?: string): Promise<number> {
  const margin = await ensureMargin(userId);
...
export async function listTokenFlows(
  userId: string,
  page: number,
  limit: number,
  opType?: number,
...
export async function listCommissionFlows(
...
/** 递归查父链（最多 2 级） */
export async function getParentUsers(userId: string): Promise<
  Array<{ userId: string; isVip: number; level: number }>
> {
...
export async function withdrawalSummary(userId: string) {
  ...
  const pending = await db
    ...
    .from(withdrawalFlows)
    .where(
      and(eq(withdrawalFlows.userId, userId), inArray(withdrawalFlows.status, [0, 1])),
    );
  return {
