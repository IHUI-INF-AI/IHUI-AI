...
export async function createChapter(lessonId: string, data: CreateChapterInput): Promise<LessonChapter> {
  const rows = await db
...
export async function createSection(chapterId: string, data: CreateSectionInput): Promise<LessonChapterSection> {
  const rows = await db
...
export async function isSignedUp(lessonId: string, userId: string): Promise<boolean> {
  const rows = await db
    ...
    .where(
      and(
        eq(lessonSignUps.lessonId, lessonId),
 ...
 * 查询用户报名记录。
 */
export async function findSignUp(lessonId: string, userId: string): Promise<LessonSignUp | undefined> {
  const rows = await db
    .select()
    .from(lessonSignUps)
    .where(
 ...
 */
export async function findLessonStudyReport(
 ...
 */
export async function findSignupReport(
 ...
 */
export async function findMemberStudyReport(
