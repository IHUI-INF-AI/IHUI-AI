...
export async function findFollowing(
  ...
  const [list, totalRows] = await Promise.all([
    ...
    db.select({ count: sql<number>`COUNT(*)` }).from(userFollows).where(where),
  ]);
...
export async function findFavorites(
  ...
  const [list, totalRows] = await Promise.all([
    db
      ...
      .limit(opts.pageSize)
      .offset((opts.page - 1) * opts.pageSize),
    db.select({ count: sql<number>`COUNT(*)` }).from(userFavorites).where(where),
  ]);
  return { list, total: Number(totalRows[0]?.count ?? 0) };
}
 ...
 */
export async function findTagsByTarget(
  resourceType: string,
  resourceId: string,
): Promise<Tag[]> {
  return db
    ...
    .where(
      and(
        eq(tagRelations.resourceType, resourceType),
        eq(tagRelations.resourceId, resourceId),
      ),
    )
...
export async function findTagResources(
  ...
  const [list, totalRows] = await Promise.all([
    db
      ...
      .limit(opts.pageSize)
      .offset((opts.page - 1) * opts.pageSize),
    db.select({ count: sql<number>`COUNT(*)` }).from(tagRelations).where(where),
  ]);
  return { list, total: Number(totalRows[0]?.count ?? 0) };
}
