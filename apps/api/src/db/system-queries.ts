...
export async function updateIntegration(
  ...
  const encryptedCredentials =
    patch.credentials !== undefined ? encryptJSON(patch.credentials) : undefined
  const rows = await db
...
export async function deleteIntegration(id: string): Promise<IntegrationConfig | undefined> {
  const rows = await db.delete(integrationConfigs).where(eq(integrationConfigs.id, id)).returning()
  return rows[0]
}
...
export async function updateSystemEvent(
  ...
  const rows = await db.update(systemEvents).set(set).where(eq(systemEvents.id, id)).returning()
  return rows[0]
}
