...
export async function findAllLiveCategories(): Promise<LiveCategory[]> {
  return db.select().from(liveCategories).orderBy(asc(liveCategories.sort), asc(liveCategories.id))
}
...
export async function updateLiveCategory(
  ...
  const rows = await db.update(liveCategories).set(set).where(eq(liveCategories.id, id)).returning()
  return rows[0]
}
...
export async function updateLiveChannel(
  ...
  if (data.sort !== undefined) set.sort = data.sort
  if (data.status !== undefined) set.status = data.status
  const rows = await db.update(liveChannels).set(set).where(eq(liveChannels.id, id)).returning()
  return rows[0]
}
...
export async function updateLecturer(
  ...
  if (data.sort !== undefined) set.sort = data.sort
  if (data.status !== undefined) set.status = data.status
  const rows = await db.update(liveLecturers).set(set).where(eq(liveLecturers.id, id)).returning()
  return rows[0]
}
...
export async function subscribeLiveChannel(userId: string, channelId: string): Promise<void> {
  await db.insert(liveSubscribe).values({ userId, channelId }).onConflictDoNothing()
}

export async function unsubscribeLiveChannel(userId: string, channelId: string): Promise<void> {
  await db
    .delete(liveSubscribe)
    .where(and(eq(liveSubscribe.userId, userId), eq(liveSubscribe.channelId, channelId)))
}

export async function findUserSubscriptions(userId: string): Promise<LiveChannel[]> {
  const rows = await db
    .select({ channel: liveChannels })
    .from(liveSubscribe)
    .innerJoin(liveChannels, eq(liveSubscribe.channelId, liveChannels.id))
    .where(and(eq(liveSubscribe.userId, userId), eq(liveChannels.status, 1)))
    .orderBy(desc(liveChannels.id))
