...
export async function findTemplates(opts: {
  ...
  const [list, totalRows] = await Promise.all([
    ...
    db.select({ count: sql<number>`count(*)::int` }).from(certificateTemplates).where(where),
  ]);
...
export async function findCertificates(opts: {
  ...
  if (opts.search) {
    conds.push(
      ilike(certificates.certificateNo, `%${opts.search}%`),
    );
  }
  const where = conds.length ? and(...conds) : undefined;
...
export async function findCertificateByNo(certificateNo: string): Promise<CertificateWithUser | undefined> {
  const rows = await db
    .select({
      cert: certificates,
      nickname: users.nickname,
...
export async function updateCertificateStatus(id: string, status: number): Promise<Certificate | undefined> {
  const rows = await db
    .update(certificates)
    .set({ status })
    .where(eq(certificates.id, id))
