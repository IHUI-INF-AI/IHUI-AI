...
export async function deletePermission(id: string): Promise<Permission | undefined> {
  const rows = await db.delete(permissions).where(eq(permissions.id, id)).returning()
  return rows[0]
}
