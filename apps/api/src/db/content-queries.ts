...
export async function findAnnouncementById(id: string): Promise<Announcement | undefined> {
  const rows = await dbRead.select().from(announcements).where(eq(announcements.id, id)).limit(1)
  return rows[0]
}
...
export async function findHelpCategoryById(id: string): Promise<HelpCategory | undefined> {
  const rows = await dbRead.select().from(helpCategories).where(eq(helpCategories.id, id)).limit(1)
  return rows[0]
}
...
export async function findHelpArticleBySlug(slug: string): Promise<HelpArticle | undefined> {
  const rows = await dbRead.select().from(helpArticles).where(eq(helpArticles.slug, slug)).limit(1)
  return rows[0]
}

export async function findHelpArticleById(id: string): Promise<HelpArticle | undefined> {
  const rows = await dbRead.select().from(helpArticles).where(eq(helpArticles.id, id)).limit(1)
  return rows[0]
}
...
export async function findAllDocs(category?: string): Promise<Doc[]> {
  ...
  return dbRead.select().from(docs).where(where).orderBy(asc(docs.sortOrder), desc(docs.createdAt))
}

export async function findDocBySlug(slug: string): Promise<Doc | undefined> {
  const rows = await dbRead.select().from(docs).where(eq(docs.slug, slug)).limit(1)
  return rows[0]
}

export async function findDocById(id: string): Promise<Doc | undefined> {
  const rows = await dbRead.select().from(docs).where(eq(docs.id, id)).limit(1)
  return rows[0]
}
...
export async function countUnreadAnnouncements(
  userId: string,
): Promise<{ total: number; unread: number; unreadIds: string[] }> {
  const now = new Date()
  // 已发布公告
  const published = await dbRead
