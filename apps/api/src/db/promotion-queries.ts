...
function generateInvitationCode(): string {
  let code = ''
  // 2026-07-21 安全审计加固:用 CSPRNG 替换 Math.random 生成优惠码,
  // 优惠码可预测 -> 攻击者可猜测并滥用其他用户未使用的优惠码
...
export async function findCouponByCode(code: string): Promise<Coupon | undefined> {
  const rows = await db.select(couponFields).from(coupons).where(eq(coupons.code, code)).limit(1)
  return rows[0]
}
