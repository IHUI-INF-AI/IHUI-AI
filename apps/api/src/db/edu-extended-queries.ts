...
export async function createOfflineRecord(
  data: CreateOfflineRecordInput,
): Promise<EduOfflineRecord> {
  const rows = await db
...
export async function createUploadedPaper(
  data: CreateUploadedPaperInput,
): Promise<EduUploadedPaper> {
  const rows = await db
