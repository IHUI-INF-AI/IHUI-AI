...
export async function searchFiles(opts: SearchFilesOpts): Promise<File[]> {
  ...
  const conds = [or(eq(files.uploadedBy, opts.userId), inArray(files.projectId, userProjectIds))]
  ...
  if (opts.tag) {
    const taggedFileIds = db
      ...
      .from(tagRelations)
      .where(and(eq(tagRelations.tagId, opts.tag), eq(tagRelations.resourceType, 'file')))
...
export async function findShareByToken(token: string): Promise<FileShare | undefined> {
  const rows = await db.select().from(fileShares).where(eq(fileShares.shareToken, token)).limit(1)
  const row = rows[0]
