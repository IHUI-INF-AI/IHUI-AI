...
export async function findCategoryBySlug(
  slug: string,
): Promise<CustomerServiceCategory | undefined> {
  const rows = await db
    .select()
    .from(customerServiceCategories)
...
export async function updateTicket(
  id: string,
  data: Partial<
    Pick<CustomerServiceTicket, 'title' | 'description' | 'priority' | 'categoryId' | 'status'>
  >,
...
export async function updateAgentStatus(
  id: string,
  status: string,
): Promise<CustomerServiceAgent | undefined> {
  const rows = await db
...
export async function findSessionBySessionId(
  sessionId: string,
): Promise<CustomerServiceSession | undefined> {
  const rows = await db
    .select()
...
export async function findActiveSessionsByAgent(
  agentId: string,
  ...
  return db
    ...
    .where(
      and(
        eq(customerServiceSessions.agentId, agentId),
        eq(customerServiceSessions.status, 'active'),
      ),
    )
}
...
export async function findRatingByTicket(
  ticketId: string,
): Promise<CustomerServiceRating | undefined> {
  const rows = await db
    .select()
