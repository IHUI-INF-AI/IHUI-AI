  ...
  server.get(
    ...
    async () => {
      ...
      for (const b of userBudgets) {
        ...
        const [todayRow] = await db
          ...
          .where(
            and(
              ...
              gte(aiCostRecords.createdAt, todayStart),
            ),
          )
        const [monthRow] = await db
          ...
          .where(
            and(
              eq(aiCostRecords.userId, userId),
              gte(aiCostRecords.createdAt, monthStart),
            ),
          )
        ...
        const dailyTokenPercent =
          b.dailyTokenLimit > 0
            ? Math.round((dailyTokenUsed / b.dailyTokenLimit) * 10000) / 100
            : 0
      ...
      alerts.sort((a, b) => {
        ...
        return Math.max(b.dailyTokenPercent, b.monthlyCostPercent) - Math.max(a.dailyTokenPercent, a.monthlyCostPercent)
      })
