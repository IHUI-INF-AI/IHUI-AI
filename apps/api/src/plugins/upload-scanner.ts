...
// 已知魔数签名：[magic bytes, MIME]
const MAGIC_SIGNATURES: ReadonlyArray<readonly [Buffer, string]> = [
  [Buffer.from([0xff, 0xd8, 0xff]), 'image/jpeg'],
  [Buffer.from('\x89PNG\r\n\x1a\n', 'latin1'), 'image/png'],
  [Buffer.from('GIF87a', 'latin1'), 'image/gif'],
  [Buffer.from('GIF89a', 'latin1'), 'image/gif'],
  [Buffer.from('RIFF', 'latin1'), 'image/webp'],
  [Buffer.from('%PDF-', 'latin1'), 'application/pdf'],
  [Buffer.from('PK\x03\x04', 'latin1'), 'application/zip'],
  [Buffer.from([0x1f, 0x8b]), 'application/gzip'],
  [Buffer.from('BM', 'latin1'), 'image/bmp'],
  [Buffer.from([0x00, 0x00, 0x01, 0xba]), 'video/mpeg'],
  [Buffer.from('ftyp', 'latin1'), 'video/mp4'],
];
...
export function hasDangerousSignature(data: Buffer): boolean {
  ...
  return DANGEROUS_SIGNATURES.some((sig) => head.includes(sig));
}
...
export function scanFileBuffer(
  ...
  if (requireMagic) {
    ...
    // 扩展名与魔数类型必须一致（宽松匹配：图片扩展名需对应 image/ 魔数）
    const extIsImage = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(ext);
    if (extIsImage && !realMime.startsWith('image/')) {
 ...
 */
export function createUploadPreHandler(
  opts: UploadScanOptions = {},
): preHandlerHookHandler {
  const allowedExts = opts.allowedExts ?? DEFAULT_ALLOWED_EXTS;
  const maxSize = opts.maxSize ?? DEFAULT_MAX_SIZE;
