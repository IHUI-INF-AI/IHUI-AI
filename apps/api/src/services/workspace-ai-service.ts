  ...
  async run(params: {
    ...
    try {
      for (let i = 0; i < maxIter; i++) {
        ...
        task.iterations = i + 1;
        // LLM 思考步骤（简化：无真实 LLM 调用，返回 stub）
        const llmContent = `Agent 第 ${i + 1} 轮思考: ${params.goal}`;
...
const NETWORK_COMMANDS = [
  ...
  'rsync', 'telnet', 'ftp', 'Invoke-WebRequest', 'Invoke-RestMethod', 'iwr', 'irm',
];

const SENSITIVE_ENV_KEYS = [
  'API_KEY', 'SECRET', 'TOKEN', 'PASSWORD', 'PASSWD', 'CREDENTIAL',
  'PRIVATE_KEY', 'AWS_', 'AZURE_', 'GCP_', 'OPENAI', 'ANTHROPIC',
  'DATABASE_URL', 'REDIS_URL', 'SSH_AUTH',
];
...
class ComputerUseService {
  ...
  async mouseClick(params: { x: number; y: number; button?: 'left' | 'right' | 'double' }): Promise<void> {
    this.checkEnabled();
...
const SYMBOL_PATTERNS: Record<string, RegExp[]> = {
  ...
  java: [/(?:public|private|protected|static)\s+(?:class|interface|void|int|String|boolean)\s+(\w+)/g],
};
...
class VectorIndexer {
  ...
  async search(params: {
    ...
    return chunks
      .map((c) => ({ filePath: c.filePath, content: c.content, score: this.cosine(qVec, c.vector) }))
      .sort((a, b) => b.score - a.score)
      .slice(0, topK);
  }
...
class CheckpointManager {
  ...
  snapshot(workspacePath: string, params: {
    tool: string;
    description: string;
    files: string[];
  }): Checkpoint {
...
class PermissionManager {
  ...
  loadRules(workspacePath: string): PermissionRule[] {
    ...
    const data = readJson<{ permissions?: { allow?: string[]; ask?: string[]; deny?: string[] } }>(file, {});
...
const BUILTIN_PERSONAS: Persona[] = [
  {
    ...
    systemPrompt: '你是一位资深的代码审查专家。从正确性/性能/安全/可读性/可测试性维度评估，给出可执行的修改建议。',
    tools: [], examples: ['review this PR', '检查代码质量'], tags: ['code', 'review'],
    enabled: true, builtin: true, createdAt: 0, updatedAt: 0,
  },
  {
    id: 'refactor-expert', name: 'Refactor Expert', category: 'engineering',
    description: '在不改变行为前提下重构代码',
    systemPrompt: '你是 Martin Fowler 风格的重构专家。每次只做一种重构，完成后保持测试通过，用 SOLID 原则评估。',
    tools: [], examples: ['重构这段代码'], tags: ['refactor'],
    enabled: true, builtin: true, createdAt: 0, updatedAt: 0,
  },
  {
    ...
    tools: [], examples: ['为这段代码写测试'], tags: ['test'],
    enabled: true, builtin: true, createdAt: 0, updatedAt: 0,
  },
  {
    ...
    tools: [], examples: ['这个 bug 怎么修'], tags: ['debug'],
    enabled: true, builtin: true, createdAt: 0, updatedAt: 0,
  },
  {
    ...
    tools: [], examples: ['帮我设计组件'], tags: ['frontend'],
    enabled: true, builtin: true, createdAt: 0, updatedAt: 0,
  },
  {
    id: 'backend-pro', name: 'Backend Pro', category: 'engineering',
    description: '后端架构、API 设计、数据库优化',
    systemPrompt: '你是后端架构师。精通 Fastify/Go/Node。RESTful 规范、索引策略、缓存层级、鉴权限流。',
    tools: [], examples: ['设计 API'], tags: ['backend'],
    enabled: true, builtin: true, createdAt: 0, updatedAt: 0,
  },
];
...
class PersonaRegistry {
  ...
  update(id: string, updates: Partial<Omit<Persona, 'id' | 'builtin' | 'createdAt'>>): Persona | null {
    const p = this.personas.get(id);
    if (!p) return null;
    const updated = { ...p, ...updates, id: p.id, builtin: p.builtin, createdAt: p.createdAt, updatedAt: nowSec() };
...
class RoutineManager {
  ...
  private nextRunTime(cron: string): number {
    ...
    for (let i = 0; i < 10080; i++) {
      ...
      if (this.cronMatch(min, t.getMinutes(), 0, 59) &&
...
class GitHubClient {
  ...
  async addPRComment(params: {
    ...
    return this.api(`/repos/${params.owner}/${params.repo}/issues/${params.number}/comments`, 'POST', {
      body: params.body,
    });
  }
  ...
  async requestReview(params: {
    ...
    return this.api(`/repos/${params.owner}/${params.repo}/pulls/${params.number}/requested_reviewers`, 'POST', {
...
class FSBridge {
    ...
    if (!path) {
      if (platform() === 'win32') {
        ...
        for (let i = 65; i <= 90; i++) {
          ...
          if (existsSync(drive)) {
            entries.push({ name: `${String.fromCharCode(i)}:`, path: drive, isDir: true, size: 0, modified: 0 });
          }
        }
        return entries;
      }
  ...
  detectTechStack(workspacePath: string): string[] {
    ...
    if (existsSync(join(workspacePath, 'pyproject.toml')) || existsSync(join(workspacePath, 'requirements.txt'))) stack.push('python');
    if (existsSync(join(workspacePath, 'go.mod'))) stack.push('go');
    if (existsSync(join(workspacePath, 'Cargo.toml'))) stack.push('rust');
    if (existsSync(join(workspacePath, 'pom.xml')) || existsSync(join(workspacePath, 'build.gradle'))) stack.push('java');
    return stack;
  }
