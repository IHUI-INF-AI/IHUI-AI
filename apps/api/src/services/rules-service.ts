...
class RulesService {
   ...
   */
  async autoGenerateRules(userId: string, authHeader?: string): Promise<AutoGenerateRulesDto> {
    try {
      const raw = await this.request<unknown>('/auto-generate', {
        method: 'POST',
        // 上游字段名是 user_id(AutoGenerateBody 无 alias_generator/populate_by_name),
        // 旧写法发的是驼峰 userId ⇒ 从未通过校验。
        body: JSON.stringify({ user_id: userId }),
        // /api/rules 不在 ai-service 的 PUBLIC_PATHS:缺这个头就是 401,而 401 会被下面的
        // catch 收成 degraded:true —— 现象是"生成不出草稿"而不是"报错了"。
