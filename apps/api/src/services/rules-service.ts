 ...
 *  - POST   /rules/auto-generate           → autoGenerateRules(LLM 行为模式提炼候选规则)
 *  - POST   /rules/resolve-conflicts       → resolveConflicts(LLM 仲裁冲突规则)
 *  - POST   /rules/:id/predict-effect      → predictEffect(dry-run 效果预测)
 *  - GET    /rules/knowledge-graph         → getKnowledgeGraph(规则关系图谱)
...
// ── 超越创新 DTO(2026-07-23,对标 Trae/Cursor Rules 超越)──────
...
export interface RuleAutoGenerateDto {
...
/** 冲突协商结果 */
export interface RuleResolveConflictsDto {
  winningRule: RuleDto | null
  reason: string
...
/** 知识图谱节点 */
export interface RuleGraphNodeDto {
  ruleId: string
  name: string
  scope: 'global' | 'workspace' | 'agent'
  matchCount: number
...
/** 知识图谱边 */
export interface RuleGraphEdgeDto {
  source: string
  target: string
...
export interface RuleKnowledgeGraphDto {
  nodes: RuleGraphNodeDto[]
  edges: RuleGraphEdgeDto[]
}

/** 学习反馈 */
export type RuleLearnFeedbackType = 'helpful' | 'unhelpful' | 'harmful'

export interface RuleLearnFeedbackDto {
  success: boolean
}
...
class RulesService {
  ...
  // ── 超越创新方法(2026-07-23,转发到 ai-service,失败降级)──────

  /**
   * 自动生成规则 — 从用户行为模式 LLM 提炼候选规则。
   *
   * 转发到 ai-service POST /api/rules/auto-generate。
   * 失败时降级为空候选列表(无法本地 LLM 分析)。
   */
  async autoGenerateRules(userId?: string): Promise<RuleAutoGenerateDto> {
    try {
      return await this.request<RuleAutoGenerateDto>('/auto-generate', {
        method: 'POST',
        body: JSON.stringify({ userId: userId ?? 'default' }),
      })
    } catch (e) {
      console.warn(
        '[rules-service] autoGenerateRules 降级返回空(需 ai-service LLM 支持):',
        (e as Error).message,
      )
      return {
        candidates: [],
        behaviorCount: 0,
        degraded: true,
        message: '自动生成需要 ai-service LLM 支持,当前降级为空候选列表',
      }
    }
  }

  /**
   * 冲突自动协商 — LLM 仲裁冲突规则,返回获胜规则 + 理由。
   *
   * 转发到 ai-service POST /api/rules/resolve-conflicts。
   * 失败时降级为本地优先级仲裁(priority 高者胜出)。
   */
  async resolveConflicts(
    context: string,
    conflictingRuleIds: string[],
  ): Promise<RuleResolveConflictsDto> {
    try {
      return await this.request<RuleResolveConflictsDto>(
        '/resolve-conflicts',
        {
          method: 'POST',
          body: JSON.stringify({ context, conflictingRules: conflictingRuleIds }),
        },
      )
    } catch (e) {
      console.warn(
        '[rules-service] resolveConflicts 降级为本地优先级仲裁:',
        (e as Error).message,
      )
      return this._resolveConflictsLocal(context, conflictingRuleIds)
    }
  }

  /**
   * 效果预测 — dry-run 对比应用规则 vs 不应用规则的 LLM 输出。
   *
   * 转发到 ai-service POST /api/rules/:id/predict-effect。
   * 失败时降级为基本规则信息(无法本地 LLM dry-run)。
   */
  async predictEffect(
    ruleId: string,
    testPrompt: string,
    ...
    try {
      return await this.request<RulePredictEffectDto>(
        `/${encodeURIComponent(ruleId)}/predict-effect`,
        {
          method: 'POST',
          body: JSON.stringify({ testPrompt, context: context ?? '' }),
        },
      )
    } catch (e) {
      console.warn(
        '[rules-service] predictEffect 降级为基本信息(需 ai-service LLM 支持):',
        (e as Error).message,
      )
      return {
        withRule: '',
        ...
        recommendation: '中性',
        degraded: true,
        message: '效果预测需要 ai-service LLM 支持,当前降级为空结果',
      }
    }
  }

  /**
   * 获取规则知识图谱 — 规则间依赖/冲突/互补关系图。
   *
   * 转发到 ai-service GET /api/rules/knowledge-graph。
   * 失败时降级为本地计算(基于规则元数据的 Jaccard 文本相似度)。
   */
  async getKnowledgeGraph(): Promise<RuleKnowledgeGraphDto> {
    try {
      return await this.request<RuleKnowledgeGraphDto>('/knowledge-graph')
    } catch (e) {
      console.warn(
        '[rules-service] getKnowledgeGraph 降级为本地计算:',
        (e as Error).message,
      )
      return this._getKnowledgeGraphLocal()
    }
  }

  /**
   * 记录学习反馈 — 规则应用后用户反馈(helpful/unhelpful/harmful)。
   *
  ...
  async recordLearnFeedback(
    ruleId: string,
    feedback: RuleLearnFeedbackType,
    context?: string,
  ): Promise<RuleLearnFeedbackDto> {
    try {
      return await this.request<RuleLearnFeedbackDto>(
        `/${encodeURIComponent(ruleId)}/learn-feedback`,
        {
          method: 'POST',
  ...
  /**
   * 本地冲突仲裁降级(不依赖 ai-service LLM)。
   *
   * 按 priority 高者胜出,同 priority 时 scope 更具体者胜出。
   */
  private async _resolveConflictsLocal(
    _context: string,
    conflictingRuleIds: string[],
  ): Promise<RuleResolveConflictsDto> {
    const { rules } = await this.listRules()
    const conflicting = rules.filter((r) => conflictingRuleIds.includes(r.id))
    ...
    const winner = conflicting.reduce((best, r) => {
      const bestScore = best.priority + (scopeBoost[best.scope] ?? 0)
      const rScore = r.priority + (scopeBoost[r.scope] ?? 0)
      return rScore > bestScore ? r : best
    })
    return {
      winningRule: winner,
      reason: `本地降级:优先级仲裁(优先级 ${winner.priority},作用域 ${winner.scope} 胜出)`,
      alternative: null,
      degraded: true,
    }
  }
  ...
  /**
   * 本地知识图谱降级(不依赖 ai-service embedding)。
   *
   * 基于 name + content 的 Jaccard 文本相似度计算规则关系。
   */
  private async _getKnowledgeGraphLocal(): Promise<RuleKnowledgeGraphDto> {
    const { rules } = await this.listRules()
    const nodes: RuleGraphNodeDto[] = rules.map((r) => ({
      ruleId: r.id,
      name: r.name,
      scope: r.scope,
      matchCount: r.matchCount ?? 0,
    }))
    const edges: RuleGraphEdgeDto[] = []
    const KG_DUPLICATE = 0.9
    const KG_COMPLEMENTARY = 0.7
    const KG_CONFLICT = 0.3

    for (let i = 0; i < rules.length; i++) {
      for (let j = i + 1; j < rules.length; j++) {
        const r1 = rules[i]!
        const r2 = rules[j]!
        const sim = this._jaccard(
          `${r1.name} ${r1.description ?? ''} ${r1.content.slice(0, 200)}`,
          `${r2.name} ${r2.description ?? ''} ${r2.content.slice(0, 200)}`,
        )
        let type: RuleGraphEdgeDto['type'] | null = null
        if (sim > KG_DUPLICATE) type = 'duplicate'
        else if (sim >= KG_COMPLEMENTARY) type = 'complementary'
        else if (
          sim < KG_CONFLICT &&
          r1.scope === r2.scope &&
          Math.abs(r1.priority - r2.priority) < 20
        ) {
          type = 'conflict'
        }
        if (type) {
  ...
  /** Jaccard 文本相似度(本地降级用)。 */
  private _jaccard(a: string, b: string): number {
