 ...
 */
export async function publishAgent(agentId: string): Promise<{ success: boolean; reason?: string }> {
  const agent = await findAgentById(agentId);
  if (!agent) return { success: false, reason: 'Agent 不存在' };
 ...
 * 状态机：下架 Agent。
 */
export async function offlineAgent(agentId: string): Promise<{ success: boolean; reason?: string }> {
  const agent = await findAgentById(agentId);
  if (!agent) return { success: false, reason: 'Agent 不存在' };
...
/**
 * 执行 Agent：记录调用次数（agents 表无 usage_count，此处用 remark 计数占位）。
 * 实际 AI 调用由 agentic-service 路由转发到 AI 服务，本函数仅做前置校验。
 */
export async function executeAgent(
  agentId: string,
): Promise<{ success: boolean; agent?: NonNullable<Awaited<ReturnType<typeof findAgentById>>>; reason?: string }> {
  const agent = await findAgentById(agentId);
  if (!agent) return { success: false, reason: 'Agent 不存在' };
