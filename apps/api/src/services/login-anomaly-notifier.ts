...
async function sendEmailNotification(
  ...
  try {
    await sendEmail({
      to: input.userEmail,
      subject: title,
      html: `<div style="font-family:sans-serif;line-height:1.6"><h2>${escapeHtml(title)}</h2><pre style="white-space:pre-wrap;font-family:inherit">${escapeHtml(content)}</pre><p style="color:${DOC_TEXT_MUTED};font-size:12px">若非本人操作,请立即修改密码并检查登录设备。</p></div>`,
      scene: 'notification',
      userId: input.userId,
    })
  } catch (e) {
