...
export async function getAccessToken(): Promise<string> {
  ...
  if (!resp.ok) {
    ...
    throw new Error(
      `PayPal OAuth2 token failed: ${resp.status} ${text.slice(0, 200)}`,
    )
  }
...
export async function captureOrder(orderId: string): Promise<CaptureResult> {
  return paypalRequest<CaptureResult>(
    'POST',
...
export async function verifyWebhookSignature(
  ...
  if (!resp.ok) {
    const text = await resp.text().catch(() => '')
    throw new Error(
      `PayPal verify-webhook-signature failed: ${resp.status} ${text.slice(0, 200)}`,
    )
  }
