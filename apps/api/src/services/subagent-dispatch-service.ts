...
// ---------------------------------------------------------------------------
// 超越 v3 类型(自演化 + 拓扑自优化 + 角色自动生成 + 协作协议增强)
// ---------------------------------------------------------------------------

/** 用户反馈 */
export type UserFeedback = 'positive' | 'negative' | 'neutral'

/** Agent 演化跟踪记录(单次任务完成时写入) */
...
/** 5 默认角色的初始 prompt(演化基线) */
const DEFAULT_ROLE_PROMPTS: Record<SubagentRole, string> = {
  researcher: '你是一名研究助手,负责调研技术方案、收集信息、分析可行性。',
  coder: '你是一名代码助手,负责按照任务要求编写高质量代码,遵循项目代码风格。',
  reviewer: '你是一名审查助手,负责审查代码质量、安全性、可维护性,提出改进建议。',
  architect: '你是一名架构师,负责设计系统架构、拆分模块、定义接口契约。',
  debugger: '你是一名调试助手,负责定位和修复 bug,分析错误根因。',
}

/**
 * 通过 ai-service /api/v1/ai/agent/run-decomposed 转发 LLM 分析。
 * 失败时返回空字符串(调用方降级处理)。
 */
async function callAiServiceAnalyze(analysisPrompt: string): Promise<string> {
  try {
    ...
    const resp = await fetch(
      `${AI_SERVICE_URL}/api/v1/ai/agent/run-decomposed`,
      {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          affectedFiles: [],
          verifyCommands: [],
          constraints: '返回纯 JSON,不要 markdown 代码块',
          deliverables: 'JSON 分析结果',
        }),
    ...
    clearTimeout(timer)
    if (!resp.ok) return ''
    const body = (await resp.json()) as AiServiceResponse
    return body?.data?.final_output ?? ''
  } catch {
    return ''
  }
}

/** 容错 JSON 解析(支持 ```json 代码块、首尾 { }、首尾 [ ]) */
...
class SubagentDispatchService {
  ...
  private async _recordCompletion(id: string): Promise<void> {
    ...
    const retryCount = runtime.steps.reduce(
      (acc, s) => acc + Math.max(0, s.attempt - 1),
      0,
    )
    ...
    records.push(record)
    if (records.length > EVOLUTION_MAX_RECORDS) {
      records.splice(0, records.length - EVOLUTION_MAX_RECORDS)
    }
    ...
    if (this.redisClient) {
      try {
        const key = `${REDIS_KEY_EVOLUTION}${role}`
        await this.redisClient.lpush(key, JSON.stringify(record))
        await this.redisClient.ltrim(key, 0, EVOLUTION_MAX_RECORDS - 1)
      } catch {
        // Redis 不可用 → 内存降级
      }
    }
    ...
    // 更新拓扑统计
    this._updateTopologyStats(mode, success, durationMs, tokens)
  }

  /** 更新编排模式统计 */
  private _updateTopologyStats(
    mode: OrchestrationMode,
    success: boolean,
    durationMs: number,
    tokens: number,
    ...
    existing.avgDurationMs =
      Math.round(
        ((existing.avgDurationMs * (existing.totalRuns - 1) + durationMs) /
          existing.totalRuns) *
          10,
      ) / 10
    existing.avgTokens =
      Math.round(
        ((existing.avgTokens * (existing.totalRuns - 1) + tokens) /
          existing.totalRuns) *
          10,
      ) / 10
  ...
  async getEvolutionRecords(role: string): Promise<AgentEvolutionRecord[]> {
    // 内存优先
    const cached = this._evolutionCache.get(role)
  ...
  private async _getEvolutionVersions(role: string): Promise<EvolutionVersion[]> {
    const cached = this._evolutionVersionsCache.get(role)
    if (cached && cached.length > 0) return cached
    if (this.redisClient) {
      try {
  ...
  async getEvolutionHistory(role: string): Promise<EvolutionHistory> {
    const [records, versions] = await Promise.all([
      this.getEvolutionRecords(role),
      this._getEvolutionVersions(role),
    ])
    const currentPrompt =
      versions.length > 0
        ? versions[versions.length - 1]!.prompt
        : DEFAULT_ROLE_PROMPTS[role as SubagentRole] ?? `你是 ${role} 助手。`
    return {
      agentRole: role,
      currentPrompt,
      versions,
  ...
  async evolveAgentPrompt(role: string): Promise<EvolutionAnalysis> {
    ...
    // 判断是否需要演化
    const needsEvolution = recentRecords.some(
      (r) => r.retryCount > 1 || r.userFeedback === 'negative' || !r.success,
    )

    if (!needsEvolution && recentRecords.length > 0) {
      return {
        agentRole: role,
    ...
    const analysisPrompt = [
      `你是 Agent Prompt 演化分析器。分析以下 ${role} 角色的最近任务记录,判断是否需要改进 prompt。`,
      '',
      '当前 Prompt:',
      currentPrompt,
      '',
      ...
      '',
      '请返回 JSON,格式:',
      '{ "needsEvolution": boolean, "patches": [{ "originalText": "原文本", "suggestedReplacement": "替换文本", "reason": "原因" }], "summary": "总结" }',
    ].join('\n')
    ...
    return {
      agentRole: role,
      scannedRecords: recentRecords.length,
      needsEvolution: parsed?.needsEvolution ?? false,
      patches: parsed?.patches ?? [],
      summary: parsed?.summary ?? 'LLM 分析失败,无补丁建议。',
      analyzedAt: nowIso(),
    }
  }
  ...
  async recordUserFeedback(
    ...
    const runtime = this.runtimes.get(dispatchId)
    if (!runtime) return false
    const role = runtime.dispatch.agentRole ?? 'coder'
    const records = this._evolutionCache.get(role) ?? []
    // 找到对应记录并更新
    for (let i = records.length - 1; i >= 0; i--) {
      if (records[i]!.dispatchId === dispatchId) {
        records[i]!.userFeedback = feedback
        break
      }
    }
    this._evolutionCache.set(role, records)
  ...
  async getAllTopologyStats(): Promise<TopologyStats[]> {
    ...
    for (const mode of modes) {
      const cached = this._topologyStatsCache.get(mode)
      if (cached) {
        result.push(cached)
        continue
      }
      if (this.redisClient) {
        try {
          ...
          if (raw) {
            const stats = JSON.parse(raw) as TopologyStats
            this._topologyStatsCache.set(mode, stats)
            result.push(stats)
            continue
          }
        } catch {
  ...
  async autoPlan(request: AutoPlanRequest): Promise<AutoPlanResult> {
    ...
    const statsContext = allStats
      .filter((s) => s.totalRuns > 0)
      .map(
        (s) =>
          `${s.orchestration}: 成功率 ${Math.round(s.successRate * 100)}% (${s.totalRuns} 次, 平均 ${Math.round(s.avgDurationMs / 1000)}s)`,
      )
      .join('\n')

    const planPrompt = [
      '你是 Subagent 编排规划器。根据任务推荐最佳编排模式和 agent 组合。',
      '',
      `任务: ${request.task}`,
      '',
      '可用编排模式: pipeline(串行) / parallel(并行) / debate(辩论) / vote(投票) / critique(批判) / decomposed(分解) / with_communication(协作通信)',
      '',
      '可用角色: researcher / coder / reviewer / architect / debugger',
      '',
      statsContext
        ? `历史编排统计:\n${statsContext}`
        : '暂无历史统计',
      '',
      request.constraints?.maxAgents
        ? `约束: 最多 ${request.constraints.maxAgents} 个 agent`
        : '',
      '',
      '请返回 JSON,格式:',
      '{ "orchestration": "模式", "agents": [{ "role": "角色", "task": "任务", "depends_on": ["依赖角色"] }], "estimatedDuration": "预估耗时", "estimatedCost": "预估成本", "reasoning": "推理过程" }',
    ].join('\n')
    ...
    return {
      orchestration: parsed?.orchestration ?? 'parallel',
      agents: parsed?.agents ?? [
        { role: 'coder', task: request.task, depends_on: [] },
      ],
      estimatedDuration: parsed?.estimatedDuration ?? '未知',
      estimatedCost: parsed?.estimatedCost ?? '未知',
      reasoning: parsed?.reasoning ?? 'LLM 规划失败,使用默认 parallel + coder。',
      topologyStats: allStats
        .filter((s) => s.totalRuns > 0)
        .map((s) => ({
          orchestration: s.orchestration,
          successRate: s.successRate,
          sampleSize: s.totalRuns,
        })),
      generatedAt: nowIso(),
    }
  }
  ...
  async createCustomRole(input: CustomRoleInput): Promise<CustomRole> {
    ...
    this._customRolesCache.push(role)
    if (this.redisClient) {
      try {
        await this.redisClient.hset(
          REDIS_KEY_CUSTOM_ROLES,
          role.id,
          JSON.stringify(role),
        )
      } catch {
  ...
  async updateCustomRole(
    ...
    const existing = this._customRolesCache[idx]!
    const updated: CustomRole = {
      ...existing,
      ...input,
      skills: input.skills ?? existing.skills,
      recommendedTasks: input.recommendedTasks ?? existing.recommendedTasks,
      updatedAt: nowIso(),
    }
    this._customRolesCache[idx] = updated
    if (this.redisClient) {
      try {
        await this.redisClient.hset(
          REDIS_KEY_CUSTOM_ROLES,
  ...
  async autoGenerateRole(
    request: AutoGenerateRoleRequest,
  ): Promise<AutoGeneratedRole> {
    const genPrompt = [
      '你是 Subagent 角色生成器。根据任务需求生成一个定制 agent 角色。',
      '',
      `任务: ${request.task}`,
      '',
      request.existingRoles && request.existingRoles.length > 0
        ? `已有角色: ${request.existingRoles.join(', ')}`
        : '',
      '',
      '请返回 JSON,格式:',
      '{ "role": "kebab-case技术名", "displayName": "中文显示名", "systemPrompt": "详细 system prompt", "skills": ["技能1","技能2"], "recommendedTasks": ["推荐任务1"], "reasoning": "推理过程" }',
    ].join('\n')
    ...
    return {
      role: parsed?.role ?? 'custom-agent',
      displayName: parsed?.displayName ?? '自定义角色',
      systemPrompt:
        parsed?.systemPrompt ?? `你是 ${request.task} 的专业助手。`,
      skills: parsed?.skills ?? [],
      recommendedTasks: parsed?.recommendedTasks ?? [],
      reasoning:
        parsed?.reasoning ?? 'LLM 生成失败,使用默认角色定义。',
    }
  }
  ...
  async recordCollaboration(
    ...
    const existing = this._collaborationCache.get(dispatchId) ?? []
    const round = msg.round ?? existing.length + 1
    ...
    this._collaborationCache.set(dispatchId, existing)
    if (this.redisClient) {
      try {
        const key = `${REDIS_KEY_COLLABORATION}${dispatchId}`
        await this.redisClient.lpush(key, JSON.stringify(fullMsg))
      } catch {
        // Redis 不可用
      }
    }
  ...
  async getCollaboration(
      ...
      if (this.redisClient) {
        try {
          ...
          if (messages.length > 0) {
            this._collaborationCache.set(dispatchId, messages)
          }
        } catch {
          // Redis 不可用
        }
    ...
    const relationMap = new Map<
      string,
      { from: string; to: string; count: number; types: Set<CollaborationMessageType> }
    >()
    for (const msg of messages) {
      ...
      const existing = relationMap.get(key) ?? {
        ...
        to: msg.to,
        count: 0,
        types: new Set<CollaborationMessageType>(),
      }
      existing.count++
