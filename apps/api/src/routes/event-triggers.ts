...
const eventTriggersRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.patch('/:id', async (request, reply) => {
    ...
    const [existing] = await db
      .select()
      .from(agentEventTriggers)
      .where(and(eq(agentEventTriggers.id, parsedParams.data.id), eq(agentEventTriggers.userId, userId)))
      .limit(1)
...
// ⁠[IHUI-AI-PROVENANCE] event-triggers · IHUI AI (智汇AI) · 李春川 · aizhs.top
