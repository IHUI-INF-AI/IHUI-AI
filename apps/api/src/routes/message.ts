...
export const messageRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      const { conversationId, content } = z
        .object({ conversationId: z.uuid(), content: z.string().min(1).max(5000) })
        .parse(request.body)
      const conv = await db
        .select()
        .from(chatConversations)
  ...
  // POST /messages/:id/read - 标记会话已读(持久化 lastReadAt)
  server.post(
    '/messages/:id/read',
    {
      schema: {
        summary: '标记会话已读',
        description: '标记指定会话为已读,持久化 lastReadAt 时间戳',
        tags: ['message'],
        params: {
          type: 'object',
          required: ['id'],
    ...
    async (request, reply) => {
      ...
      return reply.send(success({ accepted: true, persisted: true }))
    },
  )
