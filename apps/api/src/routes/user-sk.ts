...
export const userSkRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post(`${PREFIX}/create`, async (request, reply) => {
    try {
      ...
      const { apiKey, secret } = await apiKeysService.createKey(user_uuid, {
        name,
        permissions: [],
        rateLimit: max ?? 60,
      })
      ...
      return reply.send(
        success({
          id: apiKey.id,
          user_uuid: apiKey.userId,
          key: secret,
          status: 0,
          type,
          max: apiKey.rateLimit,
          out_time: out_time ?? null,
          created_time: toIsoOrNull(apiKey.createdAt),
          updated_time: toIsoOrNull(apiKey.updatedAt),
        }),
      )
    } catch (e) {
  ...
  server.delete(`${PREFIX}/delete/:sk_id`, async (request, reply) => {
    try {
      ...
      const ok = await apiKeysService.deleteKey(sk_id, request.userId!)
      if (!ok) return reply.status(404).send(error(404, 'SK 密钥不存在或无权操作'))
  ...
  server.put(`${PREFIX}/update/:sk_id`, async (request, reply) => {
    try {
      ...
      const patch: apiKeysService.UpdateKeyPatch = {}
      if (parsed.data.status !== undefined) {
        patch.status = parsed.data.status === 0 ? 'active' : 'revoked'
      }
      if (parsed.data.max !== undefined) {
        patch.rateLimit = parsed.data.max
      }
      // out_time 在 G 盘 schema 中无对应字段,忽略;实际项目可考虑扩展 schema

      const updated = await apiKeysService.updateKey(sk_id, request.userId!, patch)
      if (!updated) return reply.status(404).send(error(404, 'SK 密钥不存在或无权操作'))
