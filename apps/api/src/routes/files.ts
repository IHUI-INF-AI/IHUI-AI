...
export const fileRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post('/files/upload/base64', async (request, reply) => {
   ...
   */
  server.post('/files/upload/form', async (request, reply) => {
    await requireAuth(request, reply)
    if (!request.userId) return
   ...
   * @returns { file: { id, name, size, mimeType, path, uploadedBy } }
   */
  server.post('/files/upload/octet', async (request, reply) => {
    await requireAuth(request, reply)
    if (!request.userId) return
