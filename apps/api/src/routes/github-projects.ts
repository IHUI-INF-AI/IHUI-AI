...
const githubProjectRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
  ...
  }>('/github-projects', async (request, reply) => {
    ...
    const items = await db
      ...
      .limit(limit)
      .offset((page - 1) * limit)
    const [countRow] = await db.select({ value: count() }).from(resourceGithubProjects).where(where)
    return reply.send(success({ items, total: Number(countRow?.value ?? 0) }))
  })

  // GitHub 项目详情
  fastify.get<{ Params: { id: number } }>('/github-projects/:id', async (request, reply) => {
    const [item] = await db
      .select()
      .from(resourceGithubProjects)
