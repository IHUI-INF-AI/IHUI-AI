...
export const adminVipRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post('/vip/levels', async (request, reply) => {
    const { levelName, levelValue = 1, price, durationDays = 30, benefits = [] } = request.body as {
      levelName: string;
      levelValue?: number;
      price: number;
      durationDays?: number;
  ...
  server.put('/vip/levels/:id', async (request, reply) => {
    ...
    const body = request.body as { levelName?: string; price?: number; durationDays?: number; status?: number };
  ...
  server.get('/vip/users', async (request, reply) => {
    const { page = '1', limit = '20', userId } = request.query as {
      page: string;
      limit: string;
      userId?: string;
    };
