...
export const workspaceAiRoutes: FastifyPluginAsync = async (server) => {
  ...
  // 1.3 GitHub OAuth Device Flow — 环境信息弹窗「连接 GitHub」免粘贴 token(2026-08-17 立)
  //
  //   - POST /github/device-code  → 请求设备码(跳转 GitHub 授权页)
  //   - POST /github/device-token → 前端轮询换取 access token,成功即自动保存
  //
  // 协议(官方 Device Flow):
  //   请求设备码  POST https://github.com/login/device/code
  //                body: client_id + scope=repo read:org
  //   轮询换取     POST https://github.com/login/oauth/access_token
  //                body: client_id + client_secret + device_code + grant_type
  //   GitHub 换取失败时返回 200 + {error},不是 4xx,必须按响应体 error 字段判断。
  // 返回给前端的顶层字段均避开 "token" 子串(响应脱敏器会把含 "token" 字段值脱成 "***")。
  // ===========================================================================
  ...
  server.post('/github/device-code', async (request, reply) => {
    ...
    try {
      const body = z.object({ workspacePath: z.string().min(1) }).parse(request.body ?? {})
      void body.workspacePath // 设备码获取与 workspace 无关,仅按约定接收参数
  ...
  server.post('/github/device-token', async (request, reply) => {
    ...
    try {
      ...
      if (json.access_token) {
        // 成功:保存到 ~/.ihui/github_token(与 githubClient.loadToken 读取路径一致)
        const { homedir } = await import('node:os')
        const { mkdirSync, writeFileSync } = await import('node:fs')
        const { join } = await import('node:path')
        const dir = join(homedir(), '.ihui')
