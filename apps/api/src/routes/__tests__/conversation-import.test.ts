...
describe('外部会话导入路由(D28)', () => {
  ...
  describe('POST /api/conversation-import/commit', () => {
    ...
    it('消息写入失败 → 500(事务回滚)但仍写 failed 批次留痕', async () => {
      store.insertQueue.push([{ id: CONV_ID }])
      ...
      // values() 在 await 前已记录:会话 + 消息两次插入尝试,加上失败留痕第 3 次。
      // 不写这行,status 的 partial|failed 两态永不可达,而三端都在渲染失败徽标。
      expect(store.insertValues).toHaveLength(3)
      expect(store.insertValues[1]).toBeInstanceOf(Array) // 消息为批量数组
      const failure = store.insertValues[2] as Record<string, unknown>
      expect(failure.status).toBe('failed')
      expect(failure.conversationId).toBeNull()
      expect(failure.importedCount).toBe(0)
      expect(failure.failedCount).toBeGreaterThan(0)
      expect(String(failure.errorMessage)).toBeTruthy()
    })
