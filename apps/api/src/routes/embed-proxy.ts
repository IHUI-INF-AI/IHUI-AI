...
const BRIDGE_SCRIPT = [
  '(function(){try{',
  "var post=function(t,u,x){try{parent.postMessage({type:t,url:u,title:x||null},'*')}catch(e){}};",
  ...
  "post('ihui-embed-nav',abs,document.title)};",
  "window.addEventListener('popstate',function(){post('ihui-embed-nav',cur(),document.title)});",
    ...
    handler: async (request, reply) => {
      ...
      if (isPost) {
        ...
        if (ctype.startsWith('application/x-www-form-urlencoded')) {
          ...
          return reply.send(
            errorPage('该页面表单含文件上传(multipart),嵌入代理暂不支持,请用外部浏览器打开'),
          )
        } else {
