 ...
 *   GET    /debug/sessions/:sessionId/scopes          获取 scope 分组
 *   GET    /debug/sessions/:sessionId/variables       获取变量(frameId/scope/variablesReference 查询参数)
...
export const debugRoutes: FastifyPluginAsync = async (app) => {
  ...
  // 获取变量:?frameId=X&scope=local 或 ?variablesReference=N(子树) → {variables}
