...
const COSYVOICE_VOICES = [
  { voice_id: 'longxiaochun', name: '龙小春', language: 'zh-CN', gender: 'male', description: '年轻男声,自然亲切' },
  { voice_id: 'longxiaoxia', name: '龙小夏', language: 'zh-CN', gender: 'female', description: '年轻女声,温柔甜美' },
  { voice_id: 'longlaotie', name: '龙老铁', language: 'zh-CN', gender: 'male', description: '中年男声,沉稳有力' },
  { voice_id: 'longyuan', name: '龙媛', language: 'zh-CN', gender: 'female', description: '成熟女声,优雅知性' },
  { voice_id: 'longshu', name: '龙书', language: 'zh-CN', gender: 'male', description: '书生男声,温文尔雅' },
  { voice_id: 'longcheng', name: '龙诚', language: 'zh-CN', gender: 'male', description: '播音男声,专业标准' },
  { voice_id: 'longwan', name: '龙婉', language: 'zh-CN', gender: 'female', description: '播音女声,字正腔圆' },
  { voice_id: 'longhua', name: '龙华', language: 'zh-CN', gender: 'male', description: '东北男声,幽默风趣' },
  { voice_id: 'longxiaobei', name: '龙小贝', language: 'zh-CN', gender: 'female', description: '童声女声,活泼可爱' },
];
...
export const aiAudioRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post('/audio/speech', async (request, reply) => {
    ...
    try {
      ...
      const msg = (e as Error).name === 'AbortError' ? '语音合成超时,请缩短文本后重试' : (e as Error).message;
  ...
  server.post('/audio/recognize', async (request, reply) => {
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        reply.send(success({ results: transcripts, request_id: (data as { request_id?: string }).request_id ?? '' }));
        return;
      }
      ...
      if (taskId) {
        ...
        try {
          ...
          const pollOutput = (pollData as {
            output?: { task_status?: string; results?: Array<{ transcription_text?: string }>; message?: string };
            request_id?: string;
          }).output ?? {};
          const status = pollOutput.task_status ?? '';
          if (status === 'SUCCEEDED') {
            const transcripts = (pollOutput.results ?? []).map((r) => ({ transcription: r.transcription_text ?? '' }));
            reply.send(success({ task_id: taskId, status: 'SUCCEEDED', results: transcripts, request_id: (pollData as { request_id?: string }).request_id ?? '' }));
            return;
          }
          if (status === 'FAILED') {
          ...
          reply.send(success({ task_id: taskId, status, msg: '任务处理中,请稍后使用 task_id 查询结果' }));
        } catch {
          reply.send(success({ task_id: taskId, status: output.task_status ?? '', msg: '任务处理中,请稍后使用 task_id 查询结果' }));
        }
        return;
      }
  ...
  server.post('/audio/chat', async (request, reply) => {
    ...
    if (!userText && (body?.audio_base64 || body?.audio_url)) {
      ...
      try {
        ...
        const asrOutput = (asrData as { output?: { results?: Array<{ transcription_text?: string }> } }).output ?? {};
  ...
  server.post('/audio/voiceprint/groups/:groupId/users', async (request, reply) => {
    ...
    const body = request.body as { name?: string; desc?: string; audio_url?: string; audio_base64?: string };
    if (!body?.name) {
      reply.status(400).send(error(400, '请提供 name'));
      return;
    }
  ...
  // 12. POST /audio/voiceprint/identify — 声纹识别(占位实现)
  server.post('/audio/voiceprint/identify', async (request, reply) => {
    const body = request.body as { group_id?: string; audio_url?: string; audio_base64?: string };
    ...
    // 占位:返回已注册声纹列表,实际比对需集成第三方声纹服务
    reply.send(success({
      group_id: body.group_id,
      registered_speakers: features.map((f) => ({ feature_id: f.featureId, name: f.name })),
      matched_speaker: null,
      confidence: 0,
      msg: '声纹比对功能需集成第三方声纹识别服务后启用',
    }));
  });

  // ==========================================================================
  // 13. WS /audio/realtime — 实时语音识别 WebSocket(占位:功能暂不可用)
  // ==========================================================================
  server.get('/audio/realtime', { websocket: true }, (socket, request) => {
    ...
    (async () => {
      ...
      socket.send(JSON.stringify({
        ...
        content: 'WebSocket连接已建立,实时语音识别功能暂不可用,请使用HTTP /audio/recognize 接口',
      }));
      socket.on('message', (data: Buffer) => {
        ...
        // 任意消息(含二进制音频)回执暂不可用提示
        socket.send(JSON.stringify({
          type: 'error',
          content: '实时语音识别功能暂不可用,请使用HTTP /audio/recognize 接口',
        }));
      });
    })();
  });
