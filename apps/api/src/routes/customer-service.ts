...
const createCategorySchema = z.object({
  ...
  slug: z.string().min(1).max(64).regex(/^[a-z0-9-]+$/, 'slug 仅允许小写字母、数字与连字符'),
...
export const customerServiceRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.put(
    ...
    async (request, reply) => {
      ...
      if (existing.status !== 'pending') return reply.status(400).send(error(400, '工单状态不允许修改'));
  ...
  server.delete(
    ...
    async (request, reply) => {
      ...
      if (!existing) return reply.status(404).send(error(404, '工单不存在'));
      if (existing.userId !== request.userId) return reply.status(403).send(error(403, '无权操作'));
      if (existing.status !== 'pending') return reply.status(400).send(error(400, '工单状态不允许撤销'));
