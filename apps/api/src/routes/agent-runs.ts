...
import { RUN_HANDLE_REJECT_STATUS, issueRunHandle, parseRunHandle } from '../services/run-handle.js'
import {
  PAGE_LIMITS,
  buildCursorPage,
  cursorBinding,
  ...
  const routes: FastifyPluginAsync = async (app) => {
    ...
    app.post('/', async (request, reply) => {
      ...
      if (!body.success) {
        return reply
          .code(400)
          .send({
            ...error(400, body.error.issues[0]?.message ?? 'invalid body'),
    ...
    app.get('/list', async (request, reply) => {
      ...
      if (!parsed.ok) {
        return reply
          .code(400)
          .send({ ...error(400, parsed.message), errorCode: INVALID_CURSOR_CODE })
      }
