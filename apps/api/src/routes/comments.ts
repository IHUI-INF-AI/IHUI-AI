...
export const commentRoutes: FastifyPluginAsync = async (server) => {
  ...
  // POST /feedbacks/:id/reply - 用户补充回复（追加到 adminReply 字段或更新 status）
  server.post('/feedbacks/:id/reply', async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params)
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'))
    }
    const body = z.object({ reply: z.string().min(1).max(2000) }).safeParse(request.body)
    if (!body.success) {
      return reply.status(400).send(error(400, body.error.issues[0]?.message ?? '参数错误'))
    }
    ...
    if (existing.userId !== request.userId) {
      return reply.status(403).send(error(403, '无权回复该反馈'))
    }
    const updated = await updateFeedback(parsed.data.id, {
  ...
  // PUT /feedbacks/:id/status - 用户更新反馈状态（如关闭）
  server.put('/feedbacks/:id/status', async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params)
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'))
    }
    const body = z
      .object({ status: z.enum(['pending', 'reviewing', 'resolved', 'closed']) })
