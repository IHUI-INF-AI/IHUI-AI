...
const githubWebhookRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post('/github', async (request, reply) => {
    ...
    const candidates = await db
      ...
      .where(and(eq(agentEventTriggers.repoFullName, repo), eq(agentEventTriggers.event, eventName)))
    const matched = matchTriggers(repo, eventName, candidates)
    if (matched.length === 0) {
      return reply.send(success({ accepted: false, reason: 'no_matching_trigger', repo, event: eventName }))
    }
    ...
    for (const trigger of matched) {
      const rendered = { ...trigger, action: { ...trigger.action, prompt: buildTriggerPrompt(trigger.action, ctx) } }
