...
const userIdParam = z.object({ userId: z.uuid({ error: '无效的用户 ID' }) })
const tagIdParam = z.object({ id: z.uuid({ error: '无效的标签 ID' }) })
...
export const socialRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return reply
          .status(statusCode as 201 | 400 | 401 | 404 | 409)
          .send(error(statusCode, (e as Error).message ?? '关注失败'))
      }
      // 关注成功后，检查互关并发送通知
  ...
  server.get('/follows/following', async (request, reply) => {
    ...
    return reply.send(
      success({ list, total, page: parsed.data.page, pageSize: parsed.data.pageSize }),
    )
  })
  ...
  server.get('/tags/:id/resources', async (request, reply) => {
    ...
    return reply.send(
      success({ list, total, page: parsedQ.data.page, pageSize: parsedQ.data.pageSize }),
    )
  })
}
