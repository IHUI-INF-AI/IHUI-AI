...
export const refundAuditRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get('/refunds', { schema: { summary: '退款列表(审核管理)', tags: ['refund-audit'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' }, userId: { type: 'string' }, orderId: { type: 'string' }, orderNo: { type: 'string' } } } } }, async (request, reply) => {
  ...
  server.post('/refunds/:id/audit', { schema: { summary: '审核退款(通过/拒绝)', tags: ['refund-audit'], body: { type: 'object', additionalProperties: true } } }, async (request, reply) => {
  ...
  server.post('/refunds/:id/reject', { schema: { summary: '驳回退款', tags: ['refund-audit'], body: { type: 'object', additionalProperties: true } } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
...
export const adminRefundAuditRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get('/refunds/:id', { schema: { summary: '退款详情(含审核记录)', tags: ['refund-audit'] } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/refunds/stats', { schema: { summary: '退款统计', tags: ['refund-audit'] } }, async (_request, reply) => {
