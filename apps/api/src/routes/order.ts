...
export const orderRoutes: FastifyPluginAsync = async (server) => {
  ...
  // POST /orders - 创建订单
  server.post('/orders', { schema: { summary: '创建订单', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: createdResponse } }, async (request, reply) => {
  ...
  // GET /orders/me - 我的订单
  server.get('/orders/me', { schema: { summary: '我的订单', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' }, orderType: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
  ...
  server.get('/orders/:id', { schema: { summary: '订单详情', tags: ['order'], response: okResponse } }, async (request, reply) => {
  ...
  server.post('/orders/:id/cancel', { schema: { summary: '取消订单', tags: ['order'], response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
    ...
    if (existing.status !== 'pending') return reply.status(400).send(error(400, '订单状态不允许取消'));
    const order = await cancelOrder(parsed.data.id);
    return reply.send(success({ order }));
  });

  // POST /orders/:id/refund - 申请退款
  server.post('/orders/:id/refund', { schema: { summary: '申请退款', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
    ...
    const res = await applyRefund({ orderId: parsed.data.id, userId: request.userId!, ...body.data });
    if (res.reason === 'order_not_found') return reply.status(404).send(error(404, '订单不存在'));
    if (res.reason === 'order_not_paid') return reply.status(400).send(error(400, '订单未支付, 无法退款'));
  ...
  server.post('/orders/:id/payment', { schema: { summary: '创建支付', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
    ...
    const res = await createPayment({ orderId: parsed.data.id, userId: request.userId!, ...body.data });
    if (res.reason === 'order_not_found') return reply.status(404).send(error(404, '订单不存在'));
    if (res.reason === 'order_not_pending') return reply.status(400).send(error(400, '订单状态不允许支付'));
    return reply.send(success({ payment: res.payment }));
  });

  // GET /payments/me - 我的支付记录
  server.get('/payments/me', { schema: { summary: '我的支付记录', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
  ...
  server.get('/payments/:id', { schema: { summary: '支付状态', tags: ['order'], response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.post('/payments/:id/cancel', { schema: { summary: '取消支付', tags: ['order'], response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/refunds/me', { schema: { summary: '我的退款记录', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
  ...
  // GET /refunds/:id - 退款详情
  server.get('/refunds/:id', { schema: { summary: '退款详情', tags: ['order'], response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/invoices/titles', { schema: { summary: '发票抬头列表', tags: ['order'], querystring: { type: 'object', properties: { titleType: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
  ...
  server.post('/invoices/titles', { schema: { summary: '创建发票抬头', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: createdResponse } }, async (request, reply) => {
  ...
  server.put('/invoices/titles/:id', { schema: { summary: '更新发票抬头', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.delete('/invoices/titles/:id', { schema: { summary: '删除发票抬头', tags: ['order'], response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/invoices/applications', { schema: { summary: '我的发票申请', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
  ...
  server.post('/invoices/applications', { schema: { summary: '创建发票申请', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: createdResponse } }, async (request, reply) => {
    const parsed = createInvoiceAppSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
    const application = await createInvoiceApplication({ userId: request.userId!, ...parsed.data });
    return reply.status(201).send(success({ application }));
  });

  // PUT /invoices/applications/:id - 更新发票申请
  server.put('/invoices/applications/:id', { schema: { summary: '更新发票申请', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.delete('/invoices/applications/:id', { schema: { summary: '删除发票申请', tags: ['order'], response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
...
export const adminOrderRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get('/orders', { schema: { summary: '订单列表', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' }, orderType: { type: 'string' }, userId: { type: 'string' }, orderNo: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
    const parsed = listOrdersQuery.extend({ userId: z.preprocess(emptyToUndefined, z.string().uuid().optional()) }).safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/payments', { schema: { summary: '支付记录列表', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' }, userId: { type: 'string' }, orderId: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
    const parsed = listPaymentsQuery.extend({ userId: z.preprocess(emptyToUndefined, z.string().uuid().optional()) }).safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/refunds', { schema: { summary: '退款列表', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' }, userId: { type: 'string' }, orderId: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
    const parsed = listRefundsQuery.extend({ userId: z.preprocess(emptyToUndefined, z.string().uuid().optional()) }).safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.put('/refunds/:id/process', { schema: { summary: '审核退款', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.put('/refunds/:id/handle', { schema: { summary: '处理退款', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.get('/invoices/applications', { schema: { summary: '发票申请列表', tags: ['order'], querystring: { type: 'object', properties: { page: { type: 'integer', minimum: 1, default: 1 }, pageSize: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, status: { type: 'string' }, userId: { type: 'string' } } }, response: okResponse } }, async (request, reply) => {
    const parsed = listInvoiceAppsQuery.extend({ userId: z.preprocess(emptyToUndefined, z.string().uuid().optional()) }).safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
  ...
  server.put('/invoices/applications/:id/status', { schema: { summary: '变更发票申请状态', tags: ['order'], body: { type: 'object', additionalProperties: true }, response: okResponse } }, async (request, reply) => {
    const parsed = idParamSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send(error(400, parsed.error.issues[0]?.message ?? '参数错误'));
    }
    ...
    const existing = await findInvoiceApplicationById(parsed.data.id);
    if (!existing) return reply.status(404).send(error(404, '发票申请不存在'));
    const application = await updateInvoiceApplication(parsed.data.id, { status: body.data.status });
    return reply.send(success({ application }));
  });
};
