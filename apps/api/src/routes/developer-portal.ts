 ...
 *
 * 端点:
 * - GET /api/developer/portal  公开,返回开发者门户元信息(API host/文档/定价/限流/SDK)
 *
 * 注意:路径使用 /developer/portal 而非 /developer/info,避免与
 * routes/user/developer-routes.ts 的 GET /developer/info(用户开发者档案,需鉴权)冲突。
 *
...
export const developerPortalRoutes: FastifyPluginAsync = async (server) => {
  server.get('/developer/portal', async (_request, reply) => {
