...
const BUSINESS_LINES = ['after_school_care', 'kindergarten', 'academic', 'ai_course', 'other'] as const
...
const eduAiManagementRoutes: FastifyPluginAsync = async (server) => {
  ...
  async function trySendWechatReminder(
    ...
    try {
      ...
      const res = await sendSubscribeMessage(openId, buildFeeReminderData(studentName, dueAmount), '/pkg-user/bill/index')
  ...
  server.post('/fee-reminder', async (request, reply) => {
    ...
    const [enrollment] = await db
      ...
      .from(eduEnrollment)
      .where(
        and(eq(eduEnrollment.id, parsed.data.enrollmentId), isNull(eduEnrollment.deletedAt)),
      )
      .limit(1)
    ...
    const [student] = await db
      ...
      .limit(1)
    const message = parsed.data.message ?? defaultReminderMessage(student?.nickname ?? '同学', dueAmount)
    ...
    if (parsed.data.channel === 'wechat') {
      const wx = await trySendWechatReminder(enrollment.studentId, student?.nickname ?? '同学', dueAmount)
      wxSent = wx.sent
    ...
    try {
      await createNotification({
        ...
        title: '学费催缴通知',
        content: message,
        data: { reminderId: row.id, enrollmentId: enrollment.id, dueAmount, channel: parsed.data.channel },
      })
    } catch (_err) {
      // 通知失败不回滚催费留痕,仅标记发送失败
      const [failed] = await db
        ...
        .returning()
      return reply.status(201).send(success({ reminder: failed, notified: false, wxSent, wxFailReason }))
    }
  ...
  server.post('/fee-reminder/batch', async (request, reply) => {
    ...
    const enrollments = await db
      ...
      .where(
        and(
          inArray(eduEnrollment.id, parsed.data.enrollmentIds),
    ...
    const dueList = parsed.data.enrollmentIds
      ...
      .filter((v): v is { enrollment: typeof enrollments[number]; dueAmount: number } => v !== null)
    ...
    for (const row of rows) {
      ...
      try {
        await createNotification({
          ...
          title: '学费催缴通知',
          content: row.message ?? '',
          data: { reminderId: row.id, enrollmentId: row.enrollmentId, dueAmount: row.dueAmount, channel: row.channel },
        })
        notified += 1
      } catch (_err) {
        await db
    ...
    return reply.status(201).send(success({ sent: notified, total: rows.length, wxSent: wxSentCount, skipped }))
  })
  ...
  // 学员登录后自查:我的报名(应缴/已缴/欠费) + 最近缴费记录。authenticate 即可,非管理端权限。
