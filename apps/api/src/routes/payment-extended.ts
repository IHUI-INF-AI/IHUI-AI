...
export const paymentExtendedRoutes: FastifyPluginAsync = async (server) => {
  ...
  // ==========================================================================
  server.post('/payments/withdrawal/notify', async (request, reply) => {
    try {
      // 提取微信支付回调 Headers
  ...
  // ==========================================================================
  server.get('/payments/sync-return', async (request, reply) => {
    ...
    try {
      ...
      // 查询订单确认存在
      const [order] = await db.select().from(orders).where(eq(orders.orderNo, outTradeNo)).limit(1)
  ...
  // ==========================================================================
  server.post('/payments/subscription/renew', async (request, reply) => {
    ...
    const [vip] = await db
      ...
      .from(userVips)
      .where(
        and(eq(userVips.userId, userId), eq(userVips.status, 1), gte(userVips.endTime, new Date())),
      )
      .limit(1)
  ...
  // ==========================================================================
  server.get('/payments/subscription/status', async (request, reply) => {
    await authenticate(request)
    const userId = request.userId!
