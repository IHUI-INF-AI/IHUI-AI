...
def _validate_xy(payload: Any) -> dict[str, Any]:
    ...
    for idx, item in enumerate(series):
        ...
        for j, v in enumerate(values):
            num, err = _to_float(v)
            if err is not None or num is None:
                raise ValueError(f"series[{idx}].values 第 {j + 1} 个值{err or '无法转为数值'}")
            nums.append(num)
...
def _check_points(points: Any) -> list[list[float]]:
    ...
    for i, pt in enumerate(points):
        ...
        if err_x is not None or err_y is not None or x is None or y is None:
            raise ValueError(f"points 第 {i + 1} 个点的坐标{err_x or err_y or '无法转为数值'}")
        norm.append([x, y])
    return norm
...
async def generate_chart(arguments: dict[str, Any]) -> dict[str, Any]:
