...
model 前缀: openrouter/* (openrouter/auto, openrouter/optimus-alpha, 以及聚合的 anthropic/claude-3.5-sonnet 等)
协议: OpenAI 兼容(OpenRouter 对外暴露 OpenAI 兼容 endpoint,聚合 100+ 厂商模型)
注: HTTP-Referer / X-Title 请求头可选(用于 OpenRouter 排行榜归因),此处暂不传。
"""
...
class OpenrouterProvider(OpenAIProvider):
    """OpenRouter 聚合平台适配器:OpenAI 兼容协议,仅覆写 base_url 与 URL 构造。"""

    def __init__(self, api_key: str, api_base: str | None = None, timeout: float = 60.0):
        # api_base 已含 /api/v1,不能再加 /v1
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                async with client.stream(
                    ...
                    if resp.status_code >= 400:
                        ...
                        raise ProviderError(
                            f"OpenRouter 流式调用失败: {resp.status_code} {body[:300]!r}",
                            resp.status_code,
                        )
            ...
            yield {"type": "error", "message": f"OpenRouter 流式网络异常: {e}"}
