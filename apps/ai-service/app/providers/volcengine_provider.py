"""Volcengine(字节火山引擎)适配器。

api_base: https://ark.cn-beijing.volces.com/api/v3
model 前缀: volcengine-* / 字节其他模型(doubao / skylark / chatglm 等通过 Ark 接入)
协议: OpenAI 兼容(火山引擎方舟 Ark 提供 OpenAI 兼容 endpoint)
注: 与 doubao_provider 同源(均走 Ark api_base),doubao 专用于豆包系列,
    volcengine 用于火山引擎其他模型(Skylark / 自定义 endpoint)。
"""
...
class VolcengineProvider(OpenAIProvider):
    """火山引擎适配器:OpenAI 兼容协议(Ark api_base),仅覆写 base_url 与 URL 构造。"""
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                async with client.stream(
                    ...
                    if resp.status_code >= 400:
                        ...
                        raise ProviderError(
                            f"Volcengine 流式调用失败: {resp.status_code} {body[:300]!r}",
                            resp.status_code,
                        )
            ...
            yield {"type": "error", "message": f"Volcengine 流式网络异常: {e}"}
