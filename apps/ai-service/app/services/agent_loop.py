            ...
            for i in range(max_iter):
                ...
                for tc in tool_calls:
                    ...
                    if tool_name not in tools:
                        ...
                        continue
                    pending_tools.append((tool_name, tool_args))

                tool_outcomes: list[tuple[str, str]] = []
                for b in range(0, len(pending_tools), MAX_PARALLEL_TOOL_CALLS):
                    batch = pending_tools[b:b + MAX_PARALLEL_TOOL_CALLS]
                    gathered = await asyncio.gather(
                        *(_execute_tool_call(name, args) for name, args in batch),
                        return_exceptions=True,
                    )
                    for (name, _args), outcome in zip(batch, gathered, strict=True):
                ...
                for (tool_name, tool_args), (tool_content, step_status) in zip(
                    pending_tools, tool_outcomes, strict=True
                ):
                    steps.append({
                        "iteration": i + 1,
                        "type": "tool",
