...
class RulesEngine:
    ...
    def update(self, rule_id: str, data: dict[str, Any]) -> Optional[Rule]:
        """更新规则(部分字段) → 重写 .md 文件。"""
        with self._lock:
    ...
    def _match_semantic(self, rule: Rule, message: str) -> bool:
        ...
        try:
            ...
            if loop.is_running():
                # 已在事件循环中 → 用 run_coroutine_threadsafe 同步等待
                import concurrent.futures

                with concurrent.futures.ThreadPoolExecutor() as pool:
                    msg_emb = pool.submit(
                        asyncio.run, llm_gateway.embed(message[:8000])
                    ).result(timeout=15)
                    rule_emb = pool.submit(
                        asyncio.run, llm_gateway.embed(rule.match_pattern[:8000])
                    ).result(timeout=15)
            else:
                msg_emb = loop.run_until_complete(
                    llm_gateway.embed(message[:8000])
                )
                rule_emb = loop.run_until_complete(
                    llm_gateway.embed(rule.match_pattern[:8000])
                )
    ...
    async def _match_semantic_async(self, rule: Rule, message: str) -> bool:
        ...
        try:
            ...
            msg_emb = await llm_gateway.embed(message[:8000])
            rule_emb = await llm_gateway.embed(rule.match_pattern[:8000])
            if not msg_emb or not rule_emb:
                return self._fallback_keyword(rule, message)
            score = _cosine_similarity(msg_emb, rule_emb)
            return score >= SEMANTIC_THRESHOLD
    ...
    def test(self, rule_id: str, message: str) -> dict[str, Any]:
        """测试单个规则是否匹配某消息。
