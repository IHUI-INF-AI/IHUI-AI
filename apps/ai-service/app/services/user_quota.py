...
class UserTrialQuota:
    ...
    async def check(self, user_id: str | None) -> dict[str, Any]:
        """查询剩余额度。返回 {allowed, used, limit, remaining}。

        user_id 允许为空(未登录/无 owner 身份):此时不消耗 per-user 额度,直接放行,
        额度约束交由上层鉴权处理。"""
    ...
    async def consume(self, user_id: str | None, total_tokens: int) -> None:
        """累加用量(fire-and-forget 调用;Redis 故障静默)。user_id 为空时跳过。"""
