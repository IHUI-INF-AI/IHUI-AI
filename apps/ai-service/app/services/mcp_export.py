...
# 协议版本单一来源: 复用 app/services/mcp_client.py 的 SUPORT 常量,消除双轨漂移风险。
SUPPORTED_PROTOCOL_VERSIONS: tuple[str, ...] = _mcp_client.SUPPORTED_PROTOCOL_VERSIONS
...
LATEST_INTERNAL_VERSION = _mcp_client.SUPPORTED_PROTOCOL_VERSIONS[-1]
        ...
        async with self._manager_lock:
            ...
            if self._manager_task is None or self._manager_task.done():
                ...
                async def _hold() -> None:
                    async with mgr.run():  # type: ignore[attr-defined]
