...
搜狐号是搜狐系内容创作平台(mp.sohu.com),风控较严,本适配器完整接入反风控五层防线:
  stealth 反检测 / 指纹隔离 / 行为人类化 / 代理池 / 账号 profile 持久化。

凭证:{ SUV, IPLOC, sct }  — 均为 .sohu.com 域 cookie(sct 为登录态核心)
...
_CREATOR_URL = "https://mp.sohu.com"
_EDIT_URL = "https://mp.sohu.com/mpfe/article/edit"
_TITLE_SELECTOR = "input.article-title"
_EDITOR_SELECTOR = ".ql-editor, [contenteditable=\"true\"]"
...
class SohuAdapter(BasePlatformAdapter):
    """搜狐号适配器。"""
    ...
    def _cookies(self, credentials: dict[str, Any]) -> list[SetCookieParam]:
        return [
            ...
            {
                "name": "IPLOC",
                "value": credentials.get("IPLOC", ""),
                "domain": ".sohu.com",
                "path": "/",
            },
            {
                "name": "sct",
                "value": credentials.get("sct", ""),
                "domain": ".sohu.com",
                "path": "/",
                "httpOnly": True,
            },
    ...
    async def verify_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str]:
        ...
        suv = credentials.get("SUV", "").strip()
        if not suv:
            return False, "missing SUV cookie"

        try:
            async with async_playwright() as p:
                browser, context = await create_stealth_browser_context(
                    account_id=self._account_id(credentials, suv),
                ...
                try:
                    ...
                    return True, "connected (SUV valid)"
                finally:
    ...
    async def publish(
        ...
        if not suv:
            return PublishResult(
                success=False,
                platform=self.platform_id,
                error_message="missing SUV cookie",
            )
                ...
                try:
                    ...
                    if "login" in page.url.lower():
                        return PublishResult(
                            success=False,
                            platform=self.platform_id,
                            error_message="cookie expired, please refresh SUV/IPLOC/sct",
                        )
                    ...
                    published = False
                    if "/mpfe/article/edit" not in page.url and "login" not in page.url.lower():
                        published = True
