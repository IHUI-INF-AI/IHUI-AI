...
企鹅号是腾讯系内容创作平台(om.qq.com),风控较严,本适配器完整接入反风控五层防线:
  stealth 反检测 / 指纹隔离 / 行为人类化 / 代理池 / 账号 profile 持久化。

凭证:{ RK, ptcz, pgv_pvid }  — 均为 .qq.com 域 cookie(QQ 登录态)
...
_CREATOR_URL = "https://om.qq.com"
_EDIT_URL = "https://om.qq.com/article/articlepublish"
_TITLE_SELECTOR = "input.title-input, #title"
_EDITOR_SELECTOR = ".article-content [contenteditable=\"true\"], [contenteditable=\"true\"]"
_CATEGORY_SELECTOR = "select.category-select"
...
class QqAdapter(BasePlatformAdapter):
    ...
    def _cookies(self, credentials: dict[str, Any]) -> list[SetCookieParam]:
        return [
            ...
            {
                "name": "ptcz",
                "value": credentials.get("ptcz", ""),
                "domain": ".qq.com",
                "path": "/",
            },
            {
                "name": "pgv_pvid",
                "value": credentials.get("pgv_pvid", ""),
                "domain": ".qq.com",
                "path": "/",
            },
        ]
    ...
    async def verify_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str]:
        ...
        rk = credentials.get("RK", "").strip()
        if not rk:
            return False, "missing RK cookie"

        try:
            async with async_playwright() as p:
                browser, context = await create_stealth_browser_context(
                    account_id=self._account_id(credentials, rk),
                ...
                try:
                    ...
                    return True, "connected (RK valid)"
                finally:
    ...
    async def publish(
        ...
        if not rk:
            return PublishResult(
                success=False,
                platform=self.platform_id,
                error_message="missing RK cookie",
            )
                ...
                try:
                    ...
                    if "login" in page.url.lower():
                        return PublishResult(
                            success=False,
                            platform=self.platform_id,
                            error_message="cookie expired, please refresh RK/ptcz/pgv_pvid",
                        )
                    ...
                    published = False
                    if "/articlepublish" not in page.url and "login" not in page.url.lower():
                        published = True
