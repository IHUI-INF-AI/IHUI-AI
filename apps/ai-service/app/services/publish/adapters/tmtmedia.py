...
钛媒体是科技商业媒体平台,内容偏向 TMT(科技/媒体/通信)领域深度分析。
...
_HOME_URL = "https://www.tmtpost.com"
_CREATE_URL = "https://www.tmtpost.com/article/write"
...
class TmtmediaAdapter(BasePlatformAdapter):
    """钛媒体适配器。"""
    ...
    def _cookies(self, credentials: dict[str, Any]) -> list[SetCookieParam]:
        return [
            {
                "name": "tmt_session",
                "value": credentials.get("tmt_session", ""),
                "domain": ".tmtpost.com",
                "path": "/",
                "httpOnly": True,
            },
            {
                "name": "tmt_token",
                "value": credentials.get("tmt_token", ""),
                "domain": ".tmtpost.com",
                "path": "/",
            },
        ]
    ...
    async def verify_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str]:
        ...
        session = credentials.get("tmt_session", "").strip()
        if not session:
            return False, "missing tmt_session cookie"
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    ...
                    return True, "connected (tmt_session valid)"
                finally:
    ...
    async def publish(
        ...
        if not session:
            return PublishResult(
                success=False,
                platform=self.platform_id,
                error_message="missing tmt_session cookie",
            )
                ...
                try:
                    ...
                    if "login" in page.url.lower():
                        return PublishResult(
                            success=False,
                            platform=self.platform_id,
                            error_message="cookie expired, please refresh tmt_session",
                        )
