...
人民网是中央重点新闻网站,内容偏向时政/民生/深度报道,适配创作者投稿通道。
...
_HOME_URL = "https://www.people.com.cn"
_CREATE_URL = "https://www.people.com.cn/creator/write"
...
class PeopleAdapter(BasePlatformAdapter):
    """人民网适配器。"""
    ...
    def _cookies(self, credentials: dict[str, Any]) -> list[SetCookieParam]:
        return [
            {
                "name": "people_session",
                "value": credentials.get("people_session", ""),
            ...
            {
                "name": "people_token",
                "value": credentials.get("people_token", ""),
                "domain": ".people.com.cn",
                "path": "/",
            },
        ]
    ...
    async def verify_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str]:
        ...
        session = credentials.get("people_session", "").strip()
        if not session:
            return False, "missing people_session cookie"
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    ...
                    return True, "connected (people_session valid)"
                finally:
    ...
    async def publish(
        ...
        if not session:
            return PublishResult(
                success=False,
                platform=self.platform_id,
                error_message="missing people_session cookie",
            )
                ...
                try:
                    ...
                    if "login" in page.url.lower():
                        return PublishResult(
                            success=False,
                            platform=self.platform_id,
                            error_message="cookie expired, please refresh people_session",
                        )
