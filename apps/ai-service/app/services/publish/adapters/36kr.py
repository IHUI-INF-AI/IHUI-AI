...
_HOME_URL = "https://www.36kr.com"
_CREATE_URL = "https://write.36kr.com/posts/create"
...
_PUBLISH_SELECTOR = 'button:has-text("发布"), button:has-text("发表"), .publish-btn'
_SUCCESS_SELECTOR = (
...
class Kr36Adapter(BasePlatformAdapter):
    """36氪适配器。"""
    ...
    def _cookies(self, credentials: dict[str, Any]) -> list[SetCookieParam]:
        return [
            {
                "name": "_36kr_session",
                "value": credentials.get("_36kr_session", ""),
                "domain": ".36kr.com",
                "path": "/",
                "httpOnly": True,
            },
    ...
    async def verify_credentials(self, credentials: dict[str, Any]) -> tuple[bool, str]:
        ...
        session = credentials.get("_36kr_session", "").strip()
        if not session:
            return False, "missing _36kr_session cookie"
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    ...
                    return True, "connected (_36kr_session valid)"
                finally:
    ...
    async def publish(
        ...
        if not session:
            return PublishResult(
                success=False,
                platform=self.platform_id,
                error_message="missing _36kr_session cookie",
            )

        title = (content.title or "Untitled")[:50]
        body_text = content.html or content.text or ""
        tags = [str(t) for t in platform_config.get("tags", [])][:3]
                ...
                try:
                    ...
                    if "login" in page.url.lower():
                        return PublishResult(
                            success=False,
                            platform=self.platform_id,
                            error_message="cookie expired, please refresh _36kr_session",
                        )
                    ...
                    published = False
                    if "/posts/" in page.url and "create" not in page.url:
                        published = True
