...
_REGISTRY: list[FreeProvider] = [
    ...
    # ---------------- 对齐 Node 端 VENDORS 事实源补齐(2026-09-21 立) ----------------
    # 依据 apps/api/src/routes/ai-vendors/_shared.ts VENDORS(113 家)逐家核对,补齐本注册表缺失的
    # 对话 LLM provider。provider_code 与网关 _PREFIX_TO_PROVIDER_CODE / key_env_vars 与 Node keyEnv
    # 命名对齐,便于双端 key 状态一致。注意:
    # - 既有 48 条一律不动,本区块纯追加。
    # - 同端点但 Node 端 env 名不同的走别名条目(step/baidu/vercel),备注已说明。
    # - Node VENDORS 中的专有 REST 媒体/向量/平台类(suno/sora2/coze/jimeng4/n8n/tencent(3D)/
    #   volcengine(visual)/stability/flux/kling/runway/luma/elevenlabs/ideogram/recraft/vidu/
    #   pixverse/fishaudio/leonardo/firefly/deepgram/cartesia/azure-speech/voyage/jina/assemblyai/
    #   replicate/mureka/playai/hume/inworld/soniox/gladia/getimg/bria/freepik)无对话 chat 模型,
    #   不属本注册表语义(聊天模型选择器),由 Node 代理直连,fal 除外(任务显式要求收录)。
    # - default_models 为官方真实模型 ID 的保守稳型(旗舰+轻量),最终清单以 model_sync 从
    #   {api_base}/models 实时同步为准。
    FreeProvider(
        provider_code="openai",
        display_name="OpenAI(GPT 官方)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://platform.openai.com/api-keys",
        free_quota="无免费层(按量计费)",
        rate_limit="按 tier 分级",
        default_base_url="https://api.openai.com/v1",
        key_env_vars=["OPENAI_API_KEY"],
        default_models=["openai/gpt-4o", "openai/gpt-4o-mini", "openai/gpt-4.1-mini"],
        docs_url="https://platform.openai.com/docs",
        notes="官方 API,OpenAI 协议本源;gpt-* / o1-* / o3-* 前缀默认路由到此 provider",
    ),
    FreeProvider(
        ...
        notes="官方 Messages API(x-api-key + anthropic-version 头);claude-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="xai",
        display_name="xAI(Grok 官方)",
        category=ProviderCategory.INTERNATIONAL,
        ...
        default_models=["grok-4", "grok-4-fast", "grok-3-mini"],
        docs_url="https://docs.x.ai",
        notes="OpenAI 兼容;grok-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="azure",
        display_name="Azure OpenAI(企业版)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://portal.azure.com",
        free_quota="无免费层(Azure 订阅按量计费)",
        rate_limit="按 deployment 配额",
        default_base_url="",  # 端点含资源名(https://{resource}.openai.azure.com),须逐配置填写
        key_env_vars=["AZURE_OPENAI_API_KEY", "AZURE_API_KEY"],
        default_models=["azure/gpt-4o", "azure/gpt-4o-mini", "azure/gpt-4.1-mini"],
        docs_url="https://learn.microsoft.com/azure/ai-services/openai/",
        notes="企业版;需自建 deployment,api_base 填资源级端点(可含 /openai/deployments/{id}),api-version 由配置带",
    ),
    FreeProvider(
        ...
        free_quota="无免费层(AWS 按量计费)",
        rate_limit="按 region 配额",
        default_base_url="https://bedrock-runtime.us-east-1.amazonaws.com/openai/v1",
        key_env_vars=["BEDROCK_API_KEY", "AWS_ACCESS_KEY_ID", "AWS_BEDROCK_API_KEY"],
        default_models=[
        ...
        notes="AWS 托管多模型;Bedrock OpenAI 兼容端点或 LiteLLM 原生(AWS_ACCESS_KEY_ID/SECRET)",
    ),
    FreeProvider(
        provider_code="spark",
        ...
        free_quota="spark lite 限免(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://spark-api-open.xf-yun.com/v1",
        key_env_vars=["SPARK_API_KEY"],
        default_models=["4.0Ultra", "max-32k", "lite"],
        docs_url="https://www.xfyun.cn/doc/spark/HTTP调用文档.html",
        notes="OpenAI 兼容 HTTP 端点;spark-* 前缀路由到此;模型名官方大小写敏感(4.0Ultra)",
    ),
    FreeProvider(
        provider_code="baichuan",
        ...
        key_env_vars=["BAICHUAN_API_KEY"],
        default_models=["Baichuan4", "Baichuan4-Turbo", "Baichuan4-Air"],
        docs_url="https://platform.baichuan-ai.com/docs",
        notes="OpenAI 兼容;baichuan-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="sensenova",
        display_name="商汤日日新 SenseNova",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://console.sensecore.cn",
        free_quota="新用户试用额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.sensenova.cn/compatible-mode/v1",
        key_env_vars=["SENSENOVA_API_KEY"],
        default_models=["SenseChat-5", "SenseChat-Turbo", "SenseChat-Lite"],
        docs_url="https://console.sensecore.cn/help/docs/model-as-a-service/nova/",
        notes="OpenAI 兼容模式;sensenova-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="internlm",
        display_name="书生·浦语 InternLM",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://open.intern-ai.org.cn",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.intern-ai.org.cn/v1",
        key_env_vars=["INTERNLM_API_KEY"],
        default_models=["internlm2.5-latest", "internlm3-8b-instruct"],
        docs_url="https://open.intern-ai.org.cn/docs",
        notes="上海AI实验室;OpenAI 兼容;internlm 前缀路由到此",
    ),
    FreeProvider(
        provider_code="byteplus",
        display_name="BytePlus(字节海外 Model Engine)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://console.byteplus.com",
        free_quota="新用户试用额度(以官网为准)",
        rate_limit="按配额",
        default_base_url="https://ark.ap-southeast.bytepluses.com/api/v3",
        key_env_vars=["BYTEPLUS_API_KEY"],
        default_models=["doubao-seed-1-6", "doubao-pro-32k", "doubao-lite-32k"],
        docs_url="https://docs.byteplus.com/en/docs/Model-Engine/",
        notes="火山方舟海外版,OpenAI 兼容;与国内 doubao(ark.cn-beijing)互补",
    ),
    FreeProvider(
        provider_code="pangu",
        display_name="华为盘古 Pangu(ModelArts MaaS)",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://console.huaweicloud.com/modelarts",
        free_quota="无免费层(按量计费)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.modelarts-maas.com/v1",
        key_env_vars=["PANGU_API_KEY"],
        default_models=["pangu-nlp-n2-324k", "pangu-nlp-n1-32k"],
        docs_url="https://www.huaweicloud.com/product/pangu.html",
        notes="OpenAI 兼容 MaaS 端点;实际可用以 ModelArts 控制台开通的模型/服务为准",
    ),
    FreeProvider(
        provider_code="infini",
        display_name="无问芯穹 Infini-AI",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://cloud.infini-ai.com",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://cloud.infini-ai.com/maas/v1",
        key_env_vars=["INFINI_API_KEY"],
        default_models=["deepseek-r1", "deepseek-v3", "qwen2.5-72b-instruct", "glm-4-9b-chat"],
        docs_url="https://cloud.infini-ai.com/docs",
        notes="OpenAI 兼容 MaaS,聚合 DeepSeek/Qwen/GLM/Llama 等",
    ),
    FreeProvider(
        provider_code="mimo",
        ...
        free_quota="plan 套餐/试用(以官网为准)",
        rate_limit="按套餐",
        # 2026-09-21 实测纠正:token-plan-cn 域是套餐(plan)专用端点,只认 tp- 前缀凭据;
        # 本项目 MIMO_API_KEY 为 sk- 前缀,走 plan 域必 401 Invalid API Key(曾被误判为 key 失效)。
        default_base_url="https://api.xiaomimimo.com/v1",
        key_env_vars=["MIMO_API_KEY"],
        default_models=["mimo-v2.5", "mimo-v2.5-pro", "mimo-v2.5-asr", "mimo-v2.5-tts"],
        docs_url="https://xiaomimimo.com",
        notes="小米 MiMo 官方端点;2026-09-21 /v1/models 实测 6 个(v2.5/pro/asr/tts/tts-voiceclone/tts-voicedesign),完整清单以同步为准",
    ),
    FreeProvider(
        provider_code="ai360",
        display_name="360智脑 Ai360",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://ai.360.com/platform",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.360.cn/v1",
        key_env_vars=["AI360_API_KEY"],
        default_models=["360gpt2-pro", "360gpt-pro", "360gpt-turbo"],
        docs_url="https://ai.360.com/platform/docs",
        notes="OpenAI 兼容",
    ),
    FreeProvider(
        provider_code="longcat",
        display_name="美团 LongCat",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://longcat.ai",
        free_quota="公测期免费额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.longcat.chat/openai/v1",
        key_env_vars=["LONGCAT_API_KEY"],
        default_models=["LongCat-Flash-Chat", "LongCat-Flash-Thinking"],
        docs_url="https://longcat.ai/docs",
        notes="美团龙猫大模型,OpenAI 兼容",
    ),
    FreeProvider(
        provider_code="skywork",
        display_name="昆仑万维天工 Skywork",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://www.skywork.ai",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.skywork.ai/v1",
        key_env_vars=["SKYWORK_API_KEY"],
        default_models=["skywork-reasoner", "skywork-chat"],
        docs_url="https://www.skywork.ai/docs",
        notes="OpenAI 兼容;skywork-* 前缀路由到此;模型清单以 /v1/models 同步为准",
    ),
    FreeProvider(
        provider_code="openbmb",
        display_name="面壁 OpenBMB(MiniCPM)",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://openbmb.cn",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.openbmb.cn/v1",
        key_env_vars=["OPENBMB_API_KEY"],
        default_models=["MiniCPM4-8B", "MiniCPM-o-2.6"],
        docs_url="https://openbmb.cn/docs",
        notes="OpenAI 兼容;模型清单以 /v1/models 同步为准",
    ),
    FreeProvider(
        provider_code="bailian",
        display_name="阿里云百炼 Bailian",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://bailian.console.aliyun.com",
        free_quota="新模型限时免费额度(以控制台为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",
        key_env_vars=["BAILIAN_API_KEY", "DASHSCOPE_API_KEY"],
        default_models=["qwen-max", "qwen-plus", "qwen3-coder-plus"],
        docs_url="https://help.aliyun.com/zh/model-studio/",
        notes="与 qwen(dashscope) 同端点;对齐 Node VENDORS bailian(BAILIAN_API_KEY);bailian/ 前缀已入网关前缀表",
    ),
    FreeProvider(
        provider_code="siliconflow",
        display_name="SiliconFlow(硅基流动,对齐命名)",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://cloud.siliconflow.cn",
        free_quota="小模型永久免费 + 新用户额度(同 siliconcloud)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.siliconflow.cn/v1",
        key_env_vars=["SILICONFLOW_API_KEY", "SILICONCLOUD_API_KEY"],
        default_models=[
        ...
        docs_url="https://docs.siliconflow.cn",
        notes="与 siliconcloud 同平台同端点;对齐 Node VENDORS siliconflow(SILICONFLOW_API_KEY);siliconflow/ 前缀已入网关前缀表",
    ),
    FreeProvider(
        provider_code="step",
        display_name="Step(阶跃星辰,对齐命名)",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://platform.stepfun.com",
        free_quota="plan 套餐(同 stepfun)",
        rate_limit="无限制(plan 套餐)",
        default_base_url="https://api.stepfun.com/step_plan/v1",
        key_env_vars=["STEP_API_KEY", "STEPFUN_API_KEY"],
        default_models=["stepfun/step-3.7-flash", "stepfun/step-3.5-flash"],
        docs_url="https://platform.stepfun.com/docs",
        notes="与 stepfun 同端点;对齐 Node VENDORS step(STEP_API_KEY);模型复用 stepfun/ 前缀路由",
    ),
    FreeProvider(
        provider_code="baidu",
        display_name="百度千帆(对齐命名)",
        category=ProviderCategory.DOMESTIC,
        signup_url="https://console.bce.baidu.com/qianfan",
        free_quota="ERNIE-Speed/Lite 限免(同 wenxin)",
        rate_limit="按套餐 RPM",
        default_base_url="https://qianfan.baidubce.com/v2",
        key_env_vars=["BAIDU_API_KEY", "QIANFAN_API_KEY"],
        default_models=["ernie-4.5-turbo-128k", "ernie-speed-8k", "ernie-lite-8k"],
        docs_url="https://cloud.baidu.com/doc/WENXINWORKSHOP",
        notes="与 wenxin 同端点;对齐 Node VENDORS baidu(BAIDU_API_KEY);ernie-* 前缀路由到 wenxin",
    ),
    FreeProvider(
        provider_code="zai",
        display_name="Z.ai(智谱海外 GLM)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://z.ai",
        free_quota="glm-4.5-flash 免费(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.z.ai/api/paas/v4",
        key_env_vars=["ZAI_API_KEY"],
        default_models=["zai/glm-4.6", "zai/glm-4.5-flash", "zai/glm-4.5-air"],
        docs_url="https://docs.z.ai",
        notes="智谱海外 OpenAI 兼容端点;zai/ 前缀已入网关前缀表,与国内 zhipu(open.bigmodel.cn)互补",
    ),
    FreeProvider(
        provider_code="minimaxintl",
        display_name="MiniMax International(海外)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://platform.minimax.io",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.minimax.io/v1",
        key_env_vars=["MINIMAXINTL_API_KEY"],
        default_models=["MiniMax-M2", "MiniMax-Text-01", "MiniMax-M1-80k"],
        docs_url="https://platform.minimax.io/docs",
        notes="MiniMax 海外 OpenAI 兼容端点,与国内 minimax(api.minimax.chat)互补",
    ),
    FreeProvider(
        provider_code="dashscopeintl",
        display_name="DashScope International(阿里海外)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://www.alibabacloud.com/en/en/dashscope",
        free_quota="1M tokens/模型/月免费(同 alibaba_intl)",
        rate_limit="60 RPM",
        default_base_url="https://dashscope-intl.aliyuncs.com/compatible-mode/v1",
        key_env_vars=["DASHSCOPEINTL_API_KEY", "DASHSCOPE_API_KEY"],
        default_models=["qwen3-235b-a22b", "qwen-plus", "qwen-turbo"],
        docs_url="https://www.alibabacloud.com/help/en/dashscope/",
        notes="与 alibaba_intl 同端点;对齐 Node VENDORS dashscopeintl(DASHSCOPEINTL_API_KEY)",
    ),
    FreeProvider(
        provider_code="vercel",
        display_name="Vercel AI Gateway(对齐命名)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://vercel.com/dashboard",
        free_quota="Hobby plan 免费额度(同 vercel_ai_gateway)",
        rate_limit="按套餐",
        default_base_url="https://ai-gateway.vercel.sh/v1",
        key_env_vars=["VERCEL_API_KEY", "VERCEL_AI_GATEWAY_API_KEY"],
        default_models=["vercel/openai/gpt-4o-mini", "vercel/anthropic/claude-3.5-haiku"],
        docs_url="https://vercel.com/docs/ai-gateway",
        notes="与 vercel_ai_gateway 同端点;对齐 Node VENDORS vercel(VERCEL_API_KEY);vercel/ 前缀已入网关解析表",
    ),
    FreeProvider(
        provider_code="perplexity",
        display_name="Perplexity(Sonar 在线搜索)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://www.perplexity.ai/settings/api",
        free_quota="无免费层(按量计费)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.perplexity.ai",
        key_env_vars=["PERPLEXITY_API_KEY"],
        default_models=["sonar", "sonar-pro", "sonar-reasoning-pro"],
        docs_url="https://docs.perplexity.ai",
        notes="在线 LLM(带搜索引用);sonar-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="lingyiwanwu",
        display_name="零一万物 Yi(LingYi)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://platform.lingyiwanwu.com",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.lingyiwanwu.com/v1",
        key_env_vars=["LINGYIWANWU_API_KEY", "YI_API_KEY"],
        default_models=["yi-lightning", "yi-large"],
        docs_url="https://platform.lingyiwanwu.com/docs",
        notes="OpenAI 兼容;yi-* 前缀路由到 yi 配置(YI_API_KEY),本条对齐 Node LINGYIWANWU_API_KEY",
    ),
    FreeProvider(
        provider_code="meta",
        display_name="Meta(Llama API 官方)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://llama.developer.meta.com",
        free_quota="预览期免费额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.llama.com/compat/v1",
        key_env_vars=["META_API_KEY"],
        default_models=[
        ...
        notes="Llama API 官方 OpenAI 兼容端点;llama-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="ai21",
        ...
        free_quota="新用户试用 credits(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.ai21.com/v1",
        key_env_vars=["AI21_API_KEY"],
        default_models=["jamba-1.6-large", "jamba-1.6-mini"],
        docs_url="https://docs.ai21.com",
        notes="OpenAI 兼容;jamba-* 前缀路由到此",
    ),
    FreeProvider(
        provider_code="writer",
        display_name="Writer(Palmyra)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://dev.writer.com",
        free_quota="试用期额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.writer.com/v1",
        key_env_vars=["WRITER_API_KEY"],
        default_models=["palmyra-x-004", "palmyra-med", "palmyra-creative"],
        docs_url="https://dev.writer.com/docs",
        notes="企业级 Palmyra 系列,OpenAI 兼容",
    ),
    FreeProvider(
        ...
        free_quota="新用户 $10 credits(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.upstage.ai/v1/solar",
        key_env_vars=["UPSTAGE_API_KEY"],
        default_models=["upstage/solar-pro", "upstage/solar-mini"],
        docs_url="https://developers.upstage.ai",
        notes="OpenAI 兼容;upstage/ 前缀已入网关前缀表",
    ),
    FreeProvider(
        provider_code="deepinfra",
        display_name="DeepInfra",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://deepinfra.com/dash",
        free_quota="无免费层(按量计费,价格低)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.deepinfra.com/v1/openai",
        key_env_vars=["DEEPINFRA_API_KEY"],
        default_models=[
        ...
        notes="开源模型托管,OpenAI 兼容;deepinfra/ 前缀已入网关前缀表",
    ),
    FreeProvider(
        provider_code="novita",
        display_name="Novita AI",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://novita.ai/dashboard",
        free_quota="新用户赠送额度(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.novita.ai/v3/openai",
        key_env_vars=["NOVITA_API_KEY"],
        default_models=[
        ...
        notes="开源模型聚合,OpenAI 兼容;novita/ 前缀已入网关前缀表",
    ),
    FreeProvider(
    ...
    FreeProvider(
        ...
        notes="OpenAI 兼容,Turbo 系列性价比高",
    ),
    FreeProvider(
        provider_code="chutes",
        display_name="Chutes",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://chutes.ai",
        free_quota="存入 $5 解锁(无纯免费层)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.chutes.ai/v1",
        key_env_vars=["CHUTES_API_KEY"],
        default_models=["deepseek-ai/DeepSeek-V3-0324", "Qwen/Qwen3-235B-A22B", "zai-org/glm-4.5-air"],
        docs_url="https://chutes.ai/docs",
        notes="去中心化推理网络(基于 Bittensor),OpenAI 兼容",
    ),
    FreeProvider(
        provider_code="parasail",
        display_name="Parasail AI",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://www.parasail.io",
        free_quota="无免费层(按量计费)",
        rate_limit="按套餐 RPM",
        default_base_url="https://api.parasail.io/v1",
        key_env_vars=["PARASAIL_API_KEY"],
        default_models=["deepseek-ai/DeepSeek-V3-0324", "deepseek-ai/DeepSeek-R1-0528"],
        docs_url="https://docs.parasail.io",
        notes="开源模型推理,OpenAI 兼容;parasail/ 前缀已入网关前缀表",
    ),
    FreeProvider(
        provider_code="friendli",
        display_name="FriendliAI",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://friendli.ai",
        free_quota="按量计费(以官网为准)",
        rate_limit="按套餐 RPM",
        default_base_url="https://inference.friendli.ai/v1",
        key_env_vars=["FRIENDLI_API_KEY"],
        default_models=[
            "friendli/deepseek-ai/DeepSeek-R1",
            "friendli/meta-llama-3.3-70b-instruct",
            "friendli/qwen2.5-72b-instruct",
        ],
        docs_url="https://friendli.ai/docs",
        notes="OpenAI 兼容;friendli/ 前缀已入网关前缀表",
    ),
    FreeProvider(
        provider_code="nscale",
        display_name="Nscale",
        category=ProviderCategory.INTERNATIONAL,
        ...
        key_env_vars=["NSCALE_API_KEY"],
        default_models=["deepseek-ai/DeepSeek-R1-0528", "meta-llama/Llama-3.3-70B-Instruct", "Qwen/Qwen3-235B-A22B"],
        docs_url="https://docs.nscale.com",
        notes="欧洲 GPU 云,OpenAI 兼容",
    ),
    FreeProvider(
        provider_code="gmi",
        ...
        key_env_vars=["GMI_API_KEY"],
        default_models=["deepseek-ai/DeepSeek-R1", "meta-llama/Llama-3.3-70B-Instruct"],
        docs_url="https://docs.gmicloud.ai",
        notes="GPU 云推理,OpenAI 兼容",
    ),
    FreeProvider(
        provider_code="ubicloud",
        display_name="Ubicloud(AI 推理)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://www.ubicloud.com",
        free_quota="按量计费,价格低(开源透明定价)",
        rate_limit="按套餐 RPM",
        default_base_url="https://ai.ubicloud.com/v1",
        key_env_vars=["UBICLOUD_API_KEY"],
        default_models=["meta-llama/Llama-3.1-8B-Instruct", "mistralai/Mixtral-8x7B-Instruct-v0.1"],
        docs_url="https://www.ubicloud.com/docs",
        notes="开源云平台 AI 推理,OpenAI 兼容",
    ),
    FreeProvider(
        provider_code="inferencenet",
        ...
        key_env_vars=["INFERENCENET_API_KEY", "INFERENCE_NET_API_KEY"],
        default_models=[
            "inferencenet/deepseek-ai/deepseek-r1",
            "inferencenet/meta-llama/llama-3.3-70b-instruct",
        ],
        docs_url="https://inference.net/docs",
        notes="OpenAI 兼容;inferencenet/ 前缀已入网关解析表(配置名 inference_net,读 INFERENCE_NET_API_KEY)",
    ),
    FreeProvider(
        provider_code="xinference",
        display_name="Xinference(自托管)",
        category=ProviderCategory.LOCAL,
        ...
        key_env_vars=[],
        default_models=["xinference/llama-3.3-70b-instruct", "xinference/qwen2.5-72b-instruct"],
        docs_url="https://inference.readthedocs.io",
        notes="xorbitsai 开源推理框架,OpenAI 兼容;模型以本地启动列表为准,对齐 Node VENDORS xinference",
    ),
    FreeProvider(
        provider_code="fal",
        display_name="Fal.ai(媒体生成聚合)",
        category=ProviderCategory.INTERNATIONAL,
        signup_url="https://fal.ai/dashboard/keys",
        free_quota="无免费层(按量计费,按秒计 GPU 费)",
        rate_limit="按套餐 RPM",
        default_base_url="https://queue.fal.run",
        key_env_vars=["FAL_API_KEY"],
        default_models=[
            "fal-ai/flux/dev",
        ...
        notes=(
            "⚠️ 非对话 LLM:图像/视频/音频生成聚合队列(异步提交 + 轮询),不走 openai_chat 协议,"
            "不会进聊天模型选择器(无 chat 模型端点,可用性过滤自动剔除)。收录以对齐 Node VENDORS fal; "
            "模型经 MCP 媒体工具链调用,以 fal.ai 模型广场实际 ID 为准。"
        ),
    ),
