...
def check_article(art: Article, all_articles: list[Article]) -> list[tuple[str, bool, str]]:
    ...
    for marker, limit in NORTHEAST_MARKERS.items():
        cnt = count_in_text(art.body, marker)
        if cnt > limit:
            marker_issues.append(f'{marker}={cnt}/{limit}')
    ...
    found_pat = [_bm.group() for pat in BANNED_SENTENCE_PATTERNS if (_bm := pat.search(art.body))]
    ...
    for anc in HOLLOW_POINT_ANCHORS:
        ...
        if pos >= 0:
            ...
            for w in HOLLOW_JUDGMENT:
                _jpos = tail_clause.find(w)
                if _jpos >= 0:
                    jpos = _jpos
                    break
            apos = 10**9
            for w in HOLLOW_ACTION:
                _apos = tail_clause.find(w)
                if _apos >= 0:
                    apos = _apos
                    break
            if apos < jpos:
