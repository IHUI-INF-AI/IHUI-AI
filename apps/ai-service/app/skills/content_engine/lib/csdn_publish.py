...
def _upload_to_oss(params: dict[str, Any], image_path: str) -> str | None:
    ...
    fields = [
        ('OSSAccessKeyId', str(params['accessId'])),
        ('policy', str(params['policy'])),
        ('key', str(params['filePath'])),
    ...
    try:
        ...
        if 'data' in resp_json and 'imageUrl' in resp_json['data']:
            return str(resp_json['data']['imageUrl'])
        elif 'imageUrl' in resp_json:
            return str(resp_json['imageUrl'])
        # 有些情况URL在filePath拼接
...
def list_articles(page: int = 1, size: int = 10) -> dict[str, Any] | None:
    ...
    if code == 200:
        data = cast(dict[str, Any], result.get('data', {}))
        articles = cast(list[dict[str, Any]], data.get('list', []))
