    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def _mcp_result(self, method: str) -> dict[str, Any]:
            if method == "initialize":
                return {
                    "protocolVersion": state.server_protocol_version or "2025-03-26",
                    "capabilities": state.capabilities if state.capabilities is not None else {},
                    "serverInfo": state.server_info
                    or {"name": "mock-mcp", "version": "1.0.0"},
                }
...
async def test_protocol_negotiation_default_backward_compat(mock_servers) -> None:
    """默认握手:客户端发 2025-03-26,服务器也回 2025-03-26 → 协商采用该版本。"""
    state = MockState(mode="post")  # 默认 server_protocol_version -> 2025-03-26
    mock_servers(state)
    client = _new_client(_config(state.base_url + "/mcp"))
    try:
        await client.connect()
        ...
        # 能力/身份探测也拿到默认 server 返回
        assert client.server_info() == {"name": "mock-mcp", "version": "1.0.0"}
        assert client.capabilities() == {}
    finally:
        await _disconnect(client)


async def test_protocol_negotiation_adopts_higher_known_version(mock_servers) -> None:
    """服务器回告一个客户端已知且比发送更高的版本(2025-11-25)→ 采纳该版本。"""
    state = MockState(mode="post", server_protocol_version="2025-11-25")
    mock_servers(state)
    client = _new_client(_config(state.base_url + "/mcp"))
    try:
        await client.connect()
...
async def test_protocol_negotiation_unknown_newer_clamps_to_max(mock_servers) -> None:
    """服务器要求客户端未知的更新协议(2027-01-01)→ 退回本地最高支持版本。"""
    state = MockState(mode="post", server_protocol_version="2027-01-01")
    mock_servers(state)
    client = _new_client(_config(state.base_url + "/mcp"))
    try:
        await client.connect()
        assert client.negotiated_protocol() == SUPPORTED_PROTOCOL_VERSIONS[-1]
        assert client.negotiated_protocol() == "2025-11-25"
    finally:
        await _disconnect(client)


async def test_protocol_negotiation_unknown_garbage_falls_back(mock_servers) -> None:
    """服务器回告无法解析的版本串(不 mock 成功)→ 明确降级回退到初始版本。"""
    state = MockState(mode="post", server_protocol_version="bogus")
    mock_servers(state)
    client = _new_client(_config(state.base_url + "/mcp"))
    try:
        await client.connect()
...
async def test_server_info_and_capabilities_parsed(mock_servers) -> None:
    """initialize result 的 capabilities(布尔结构+扩展字段)与 serverInfo 被结构化保存。"""
    caps = {
        "experimental": {"kit": {"proto": "v1"}},
        "tools": {"listChanged": True},
        "prompts": {"listChanged": False},
        "resources": {"subscribe": True, "listChanged": True},
        "logging": {},
        "extensions": {"x-sep": {"mode": "on"}},
    }
    info = {"name": "rich-server", "version": "2.3.4", "weird_extra": 1}
    state = MockState(mode="post", capabilities=caps, server_info=info)
    mock_servers(state)
    client = _new_client(_config(state.base_url + "/mcp"))
    try:
        await client.connect()
        assert client.capabilities() == caps
        assert client.capabilities()["experimental"]["kit"] == {"proto": "v1"}
        assert client.capabilities()["extensions"] == {"x-sep": {"mode": "on"}}
        assert client.server_info() == info
        assert client.server_info()["name"] == "rich-server"
    finally:
        await _disconnect(client)


async def test_config_protocol_version_override(mock_servers) -> None:
    """客户端可通过 config.protocol_version 主动打招呼为更高版本。"""
    state = MockState(mode="post", server_protocol_version="2025-06-18")
    mock_servers(state)
    client = _new_client(
        _config(state.base_url + "/mcp", protocol_version="2025-06-18")
    )
    try:
        await client.connect()
        assert client.negotiated_protocol() == "2025-06-18"
    finally:
        await _disconnect(client)
