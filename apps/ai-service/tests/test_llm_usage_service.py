...
def test_old_records_excluded_by_days(service):
    ...
    2026-09-10 修复:原 wide 查询用 days=366 与旧记录时间戳(恰好 366 天前)零余量
    贴边 —— Linux 上 time.time() 纳秒级分辨率,第二次取 cutoff 必然更晚,记录恰好
    落在边界外被排除;Windows 分辨率粗才侥幸通过。改用 days=400 留出 34 天余量,
    两个断言在任意时钟分辨率下都确定。
    """
    r = service.record_usage("openai", "gpt-4o", "user1", 100, 50)
    # 强制修改 timestamp 为 366 天前
    ...
    # 扩大范围应包含(400 天窗口 > 366 天前,余量 34 天,不受取时延影响)
    stats_wide = service.get_user_stats("user1", days=400)
