/**
 * ModelsPage — AI 模型广场(2026-07-25 立)。
 *
 * 数据源:getAiModels() → GET /api/ai/models(分页 PageData<AiModel>)。
 * 列表项:模型名 + 厂商 + 描述 + 类型标签,点击 chrome.tabs.create 跳 web 详情。
 */
...
export default function ModelsPage() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const res = await getAiModels({ page: 1, pageSize: 30 })
  ...
  return (
    <div className="p-3 md:p-4 flex flex-col gap-2.5">
      <div className="flex items-center justify-between pb-2 border-b border-border">
        <h3 className="m-0 text-sm font-semibold">{t('apps.models')}</h3>
      </div>
      {items.length === 0 ? (
        <div className="text-center text-muted-foreground py-8 px-4 text-sm">
          {t('common.empty')}
            ...
            >
              ...
              <CardContent className="px-3 pb-2 -mt-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs text-muted-foreground truncate">{m.provider}</span>
                  {m.type ? (
