/**
 * AiNewsPage — AI 资讯列表(2026-07-25 立)。
 *
 * 数据源:getAiFeeds() → GET /api/ai-feed(分页 PageData<AiFeedItem>)。
 * 列表项:标题 + 摘要 + 来源 + 时间,点击 chrome.tabs.create 跳 web 详情。
 */
...
export default function AiNewsPage() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const res = await getAiFeeds({ page: 1, pageSize: 20 })
  ...
  return (
    <div className="p-3 md:p-4 flex flex-col gap-2.5">
      <div className="flex items-center justify-between pb-2 border-b border-border">
        <h3 className="m-0 text-sm font-semibold">{t('apps.aiNews')}</h3>
      </div>
      {items.length === 0 ? (
        <div className="text-center text-muted-foreground py-8 px-4 text-sm">
          {t('common.empty')}
        ...
        <div className="flex flex-col gap-2">
          {items.map((n) => (
            <Card
              ...
              onClick={() => openItemInWeb(`/ai-news/${encodeURIComponent(n.id)}`)}
            >
              ...
              <CardContent className="px-3 pb-2 -mt-1">
                {n.content ? (
                  <p className="m-0 text-xs text-muted-foreground line-clamp-2">{n.content}</p>
                ) : null}
                <div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground gap-2">
                  ...
                  <span className="whitespace-nowrap">{fmtDate(n.publishTime)}</span>
                </div>
              </CardContent>
            </Card>
          ))}
