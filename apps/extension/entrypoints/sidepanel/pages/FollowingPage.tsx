/**
 * FollowingPage — 个人中心 / 关注列表(2026-07-25 立)。
 *
 * 数据源:getFollowing() → GET /api/follows/following(分页 PageData<FollowUser>)。
 * 列表项:头像(initials)+ 昵称 + 简介 + 关注时间 + "取消关注"按钮(unfollowUser)。
 */
...
export default function FollowingPage() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const res = await getFollowing({ page: 1, pageSize: 30 })
  ...
  return (
    <div className="p-3 md:p-4 flex flex-col gap-2.5">
      <div className="flex items-center justify-between pb-2 border-b border-border">
        <h3 className="m-0 text-sm font-semibold">{t('apps.following')}</h3>
