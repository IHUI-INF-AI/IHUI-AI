...
describe('isPathAllowedWithRealpath(symlink 逃逸防护)', () => {
  ...
  it('白名单内路径的真实路径仍需落白名单', () => {
    ...
    const fakeRealpath = (p: string) => (p.includes('data') ? p.replace('data', 'etc') : p);
    expect(
...
describe('appendSandboxAuditLog(jsonl 审计)', () => {
  ...
  it('同步执行写入 jsonl(含执行与拦截两类记录)', async () => {
    ...
    try {
      ...
      const okRec = records.find((r) => String(r.command).includes('audit-test-ok'));
      const blockedRec = records.find((r) => r.blocked === true);
      expect(okRec).toBeDefined();
      expect(okRec!.blocked).toBe(false);
      expect(typeof okRec!.durationMs).toBe('number');
      expect(blockedRec).toBeDefined();
      expect(String(blockedRec!.blockReason)).toContain('command_not_allowed');
    } finally {
      delete process.env.IHUI_SANDBOX_AUDIT_LOG;
