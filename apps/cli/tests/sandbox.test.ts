...
describe('execSandboxed 端到端(真实执行)', () => {
  // 2026-09-14:超时预算 25s→60s + retry:2——turbo 全量并行时机器高负载,真实沙箱
  // spawn 偶发超时/进程树异常(exitCode 124);单独运行 3.4s 内稳定完成,属负载
  // 敏感而非功能缺陷,真实执行类 E2E 按 e2e 惯例给重试
  it('Windows 本机:沙箱内真实执行 echo 命令', { timeout: 60_000, retry: 2 }, async () => {
    if (process.platform !== 'win32') return;
