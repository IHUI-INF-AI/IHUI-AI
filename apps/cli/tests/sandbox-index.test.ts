...
// 唯一临时目录:tests/sandbox/index.test.ts 与本文件是同源套件,若共用固定路径,
// vitest 并行 worker 下一个套件的 afterAll rmSync 会与另一个真实执行用例
// (spawn 子进程 CWD=该目录)竞争句柄 → EPERM 套件级失败;mkdtemp 隔离根治
let CWD = '';
