/**
 * cli 守门脚本自测(check-cli-integration-completeness.mjs)。
 *
 * 设计:
...
const SCRIPT_PATH = path.resolve(__dirname, '../../../scripts/check-cli-integration-completeness.mjs')
...
describe('场景 1: 全 8 项 INTEGRATED_CLAIMS 都有真实接入 → 守门通过', () => {
  ...
  beforeAll(() => {
    root = setupProject({
      ...
      'scripts/audit-cli-crates.md': `# Audit: 不融合 crate 决策记录

## cli-markdown (checkpoint-based freezing)
...
describe('场景 2: CLAIMED 项 1 个文件缺失 → 守门失败', () => {
  ...
  beforeAll(() => {
    root = setupProject({
      ...
      'apps/cli/src/plugins/registry.ts': 'export class PluginRegistry { plugins = [] }',
      // NI-1/2/3/4 证据文件
      'scripts/audit-cli-crates.md': `# Audit

## cli-markdown 不融合 理念借鉴
 ...
 * 必须同步修改 scripts/check-cli-integration-completeness.mjs 的
