  ...
  for (const m of messages) {
    if (m.role === 'assistant' && Array.isArray(m.tool_calls)) {
      for (const tc of m.tool_calls) {
        const fn = typeof tc === 'object' && tc ? (tc as { function?: { name?: unknown } }).function : undefined;
...
function renderTable(rows: EvalRow[]) {
  const keys = Object.keys(rows[0]);
  const widths = keys.map(k => Math.max(k.length, ...rows.map(r => String(r[k] ?? '').length)));
  const line = (cells: string[]) => cells.map((c, i) => c.padEnd(widths[i])).join(' | ');
  console.warn(line(keys));
  console.warn(line(widths.map(w => '-'.repeat(w))));
  for (const r of rows) {
    console.warn(line(keys.map(k => String(r[k] ?? ''))));
  }
}
...
function main() {
  ...
  if (live) {
    console.warn('--live 模式依赖 CLI 登录态与 streamChat，请在 CLI 会话内评测（例如在 agent.ts decideCompaction 中插入评测钩子）。');
    console.warn('本次以 dry-run 结果退出。\n');
  }
