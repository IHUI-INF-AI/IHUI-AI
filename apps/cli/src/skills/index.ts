 ...
 *
 * Skill 文件格式(对齐 grok-build Skills frontmatter 规范):
 *   ---
 *   name: <skill 名>(可选,覆盖文件名 stem)
 *   description: <一句话描述>(可选)
 *   allowed-tools: [tool-a, tool-b](可选,工具白名单)
 *   tools: [tool-c](可选,等价于 allowed-tools,grok-build 兼容字段)
 *   model: <模型名>(可选)
...
/** Skill frontmatter 元信息(对齐 grok-build Skills 规范) */
export interface SkillFrontmatter {
  ...
  allowedTools?: string[];
  /** 工具白名单(grok-build 兼容字段,等价于 allowedTools,对应 frontmatter 的 tools) */
  tools?: string[];
  /** 指定模型(可选) */
  model?: string;
  /** 分类标签(可选) */
