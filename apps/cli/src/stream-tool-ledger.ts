...
export class StreamToolLedger {
  ...
  /**
   * 流内即时登记。同一个 toolCallId + 同一指纹的重复登记不造第二条(幂等回放);
   * 同 id 不同指纹是两个调用,不得并入。markEndOfStream 之后再登记:带 run 的
   * 大声抛错(收尾后派发没有可对账的时机);裸登记直接丢弃(不入册、不进快照)。
   *
  ...
  register(
    ...
    // 收尾之后再带 run 登记 = 协议层不该发生的迟到派发。静默不跑或悄悄跑一次
    // 都造得出"执行过却无人记账"这一型,宁可当场大声炸,逼调用方修时序。
    if (this.ended && run) {
      throw new Error(
        'stream-tool-ledger: refusing to register a callable run after end-of-stream',
      )
    }
    const fingerprint = computeFingerprint(this.turn, call.toolName, call.args)
    if (call.toolCallId) {
      // 幂等回放只认"同 id 且同指纹"。同 id 而指纹不同是两个不同的调用,
      // 按 id 并入会让第二个静默蒸发 —— 那是改语义,不是去重。
      const existing = this.list.find(
        (e) => e.toolCallId === call.toolCallId && e.anchor.fingerprint === fingerprint,
      )
      if (existing) return existing
    }
    if (this.ended) {
      // (带 run 的迟到登记已在函数开头抛响。)迟到的裸登记既没有执行机会也没有
      // 对账价值,直接丢弃:不入册、不占 occurrence、不进快照。此前"记成 lost +
      // skipReason"已撤销 —— 账本只记还有完整生命周期可观测的调用,伪造一条
      // 永远走不完的 lifecycle 恰恰违背"对账要能对上"的初衷。
      return {
        anchor: {
          turn: this.turn,
        ...
        dedupeKey: `${fingerprint}#discarded`,
