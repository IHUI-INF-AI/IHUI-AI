 ...
 *
 * 灵感来源:grok-build 的 `xai-grok-tools/src/reminders/` crate(task_completion / skill_discovery / lsp_diagnostics)。
 * 问题:LLM 在长任务中可能"迷失方向"——不知道上下文快满了 / 迭代预算快用完了 / 连续失败需要换方案。
 * grok-build 在工具执行后自动向 prompt 注入 system reminders,让 LLM 被动接收关键状态信息。
 *
 * 简化策略(做减法):
