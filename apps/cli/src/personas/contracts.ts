/**
 * Subagent Personas Input/Output Contracts — 对齐 cli Personas 规范。
 *
...
export const PERSONAS_CONTRACTS: Readonly<PersonaContracts> = Object.freeze({
  researcher: {
    ...
    output_schema: {
      ...
      properties: {
        researchSummary: { type: 'string', description: '调研结论摘要(事实陈述,非代码)' },
...
export function getPersonaContract(persona: string): PersonaContract | undefined {
  return PERSONAS_CONTRACTS[persona];
}

export function listPersonaContractNames(): string[] {
  return Object.keys(PERSONAS_CONTRACTS);
}
