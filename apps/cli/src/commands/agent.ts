...
// 灵感来源:grok-build 的 sampler actor(指数退避重试 + 死循环检测)。
...
export async function runToolLoop(opts: RunToolLoopOptions): Promise<RunToolLoopResult> {
  ...
  // 灵感来源:grok-build doom_loop 理念,简化为客户端工具调用层滑动窗口检测
  ...
  try {
    for (let i = 0; i < opts.maxIterations; i++) {
      ...
      // 灵感来源:grok-build doom_loop 理念,客户端工具调用层滑动窗口检测
      ...
      // 灵感来源:grok-build 的 reminders crate,让 LLM 被动接收关键状态信息
