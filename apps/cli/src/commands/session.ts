 ...
 *
 * P0-2 融合:对齐 cli 的 `x.ai/session/repair` + `x.ai/rewind/*` ACP 扩展方法。
 ...
 *
 * 灵感来源:cli 的 `x.ai/session/repair` 扩展方法。
...
/**
 * P0-2 结构化 Repair 响应 — 对齐 cli `extensions/repair.rs::RepairSessionResponse`。
 ...
 * 对齐 cli 的双轨(居民 actor vs on-disk)+ dry_run 模式。
 *
...
/** Rewind 模式 — 对齐 cli `extensions/rewind.rs::RewindMode` */
 ...
 * 对齐 cli `extensions/rewind.rs::rewind/points`。
 */
...
/**
 * P0-2 回退 history 到指定 prompt 索引 — 对齐 cli `extensions/rewind.rs::rewind/execute`。
 *
