<template>
  <view class="page">
    ...
    <button class="btn" @tap="onSubmit" :disabled="!email || !code">绑定邮箱</button>
  </view>
</template>
...
onShow(async () => { try { email.value = (await getProfile()).email || '' } catch (e) {} })

function sendCode() {
  if (counting.value) return
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) return uni.showToast({ title: '邮箱格式错误', icon: 'none' })
...
async function onSubmit() {
  try {
    await bindEmail(email.value, code.value)
