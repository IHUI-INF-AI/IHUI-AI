  ...
  <view class="page">
    ...
    <view class="top3" v-if="list.length >= 3">
      <view class="top-item top2">
        ...
        <text class="commission">¥{{ list[1].commission }}</text>
      ...
      <view class="top-item top1">
        ...
        <text class="commission">¥{{ list[0].commission }}</text>
      ...
      <view class="top-item top3">
        ...
        <text class="commission">¥{{ list[2].commission }}</text>
    ...
    <view class="list">
      <view class="item" v-for="(u, i) in list.slice(3)" :key="u.id">
        ...
        <text class="commission">¥{{ u.commission }}</text>
      </view>
    </view>
...
const list = ref<Array<{ id: string; nickname: string; avatar?: string; commission: number }>>([])
const loading = ref(true)

async function load() {
  try { list.value = (await getDistributionRank()).list || [] } finally { loading.value = false }
}
onShow(load)
</script>
...
.header { padding: 60rpx 0; text-align: center; background: linear-gradient(135deg, #ff6e3c, #ff9a3c); }
.title { color: #fff; font-size: 36rpx; font-weight: 700; }
...
.commission { font-size: 26rpx; color: #ff6e3c; font-weight: 600; margin-top: 4rpx; }
...
.commission { font-size: 28rpx; color: #ff6e3c; font-weight: 600; }
