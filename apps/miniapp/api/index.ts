...
export const getVipPrivilege = () => get<{ list: Array<{ id: string; title: string; desc: string }> }>('/vip/privilege')
...
export const getMemberBenefits = () => get<{ list: Array<{ id: string; title: string; desc: string; icon?: string }> }>('/member/benefits')
export const getIntegral = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: Array<{ id: string; type: string; amount: number; time: string }>; total: number }>('/member/integral', params)
export const getCouponList = (params?: { status?: string }) =>
  get<{ list: Array<{ id: string; title: string; amount: number; threshold: number; expireTime: string; status: string }> }>('/member/coupon', params)

/* ============ 支付 / 订单 ============ */

export interface PayParams { orderNo: string; payType: 'wechat' | 'balance' | 'alipay' }
export const pay = (data: PayParams) => post<{ success: boolean; payUrl?: string }>('/pay', data)
export const getPayResult = (orderNo: string) => get<{ status: 'pending' | 'paid' | 'failed'; amount: number }>('/pay/result', { orderNo })
...
export const getDistributionTeam = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: Array<{ id: string; nickname: string; avatar?: string; joinTime: string; level: number }>; total: number }>('/distribution/team', params)
export const getCommissionRecords = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: Array<{ id: string; amount: number; type: string; time: string; nickname?: string }>; total: number }>('/distribution/commission', params)
export const withdraw = (data: { amount: number; type: string }) => post('/distribution/withdraw', data)
export const getDistributionRank = () => get<{ list: Array<{ id: string; nickname: string; avatar?: string; commission: number }>; }>('/distribution/rank')
...
export const updatePassword = (oldPwd: string, newPwd: string) => post('/user/password', { oldPwd, newPwd })
export const bindEmail = (email: string, code: string) => post('/user/email/bind', { email, code })
export const realNameAuth = (data: { realName: string; idCard: string }) => post('/user/realname', data)
export const submitFeedback = (data: { content: string; contact?: string; images?: string[] }) => post('/user/feedback', data)

/* ============ 内容 / 社区 ============ */

export interface News { id: string | number; title: string; coverUrl?: string; summary?: string; content?: string; createTime: string; views?: number }
...
export interface Circle { id: string | number; title: string; content: string; images?: string[]; author?: string; avatar?: string; createTime: string; likes?: number; comments?: number }
...
export const createCircle = (data: { title: string; content: string; images?: string[] }) => post('/community/circle', data)

export interface Ask { id: string | number; title: string; content: string; author?: string; avatar?: string; createTime: string; answers?: number; adopted?: boolean }
...
export const getTopicList = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: Array<{ id: string; name: string; count: number; coverUrl?: string }>; total: number }>('/community/topic/list', params)
export const getTopicDetail = (id: string | number) => get<{ id: string; name: string; posts: Circle[] }>(`/community/topic/${id}`)

/* ============ 教育扩展 ============ */

export interface StudyRecord { id: string; courseId: string; courseTitle: string; progress: number; duration: number; time: string }
export const getStudyInfo = () => get<{ todayMinutes: number; totalMinutes: number; continuousDays: number; courses: number }>('/study/info')
export const getStudyRecords = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: StudyRecord[]; total: number }>('/study/records', params)
export const getStudyPlan = () => get<{ list: Array<{ id: string; title: string; target: number; progress: number }> }>('/study/plan')
export const getStudyRank = () => get<{ list: Array<{ id: string; nickname: string; avatar?: string; minutes: number }> }>('/study/rank')

export interface Exam { id: string | number; title: string; duration: number; questions: number; passScore: number; startTime?: string; endTime?: string; status?: string }
export const getExamList = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: Exam[]; total: number }>('/exam/list', params)
export const getExamDetail = (id: string | number) => get<Exam & { questions: Array<{ id: string; title: string; options: string[] }> }>(`/exam/${id}`)
export const submitExam = (data: { examId: string; answers: Record<string, number> }) =>
  post<{ score: number; pass: boolean }>('/exam/submit', data)
export const getExamResult = (id: string | number) => get<{ score: number; pass: boolean; rank?: number; total?: number }>(`/exam/${id}/result`)

/* ============ AI 模块 ============ */

export const getChatHistory = (params?: { page?: number; pageSize?: number }) =>
  get<{ list: Array<{ id: string; title: string; time: string; messages: ChatMessage[] }>; total: number }>('/ai/history', params)
export const generateImage = (data: { prompt: string; size?: string }) =>
  post<{ url: string }>('/ai/image', data)
export const voiceChat = (data: { audio: string }) => post<{ reply: string; audio?: string }>('/ai/voice', data)
export const getAgentList = () => get<{ list: Array<{ id: string; name: string; desc: string; avatar?: string; uses: number }> }>('/ai/agent/list')
export const getAgentDetail = (id: string | number) => get<{ id: string; name: string; desc: string; avatar?: string; prompt: string; config?: any }>(`/ai/agent/${id}`)
...
export interface Teacher { id: string | number; name: string; avatar?: string; title?: string; intro?: string; courses?: number; students?: number }
...
export const getAbout = () => get<{ name: string; version: string; intro: string; logo?: string }>('/about')
export const getHelp = () => get<{ list: Array<{ id: string; title: string; content: string }> }>('/help')
export const getProtocol = () => get<{ content: string }>('/protocol')
export const getPrivacy = () => get<{ content: string }>('/privacy')
export const getContact = () => get<{ phone: string; email: string; address: string; qq?: string }>('/contact')
export const getNotificationSettings = () => get<{ list: Array<{ key: string; title: string; enabled: boolean }> }>('/settings/notification')
export const updateNotificationSettings = (data: Record<string, boolean>) => put('/settings/notification', data)
