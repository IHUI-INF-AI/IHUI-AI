...
/// 接收方向字符串(n/s/e/w/ne/nw/se/sw),映射到 Tauri 原生 ResizeDirection,
/// 调用 window.start_resize_dragging 让 OS 接管 resize 手势(边框隐藏时必需)。
#[tauri::command]
fn start_resize(direction: String, window: tauri::WebviewWindow) -> Result<(), String> {
    use tauri::ResizeDirection;
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // P1-6: Windows 平台启用系统原生窗口阴影(decorations=false 时默认无阴影,
            // 用 SystemShadow effect 让无边框窗口也有 OS 级投阴影,提升立体感)
            #[cfg(target_os = "windows")]
            {
                use tauri::WindowEffects;
                if let Some(window) = app.get_webview_window("main") {
                    let _ = window.set_effects(WindowEffects {
                        effects: vec![tauri::WindowEffect::SystemShadow],
                        state: None,
                        color: None,
                        radius: None,
                    });
