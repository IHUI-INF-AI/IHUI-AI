...
/// - promoted_identities:已处理过的托盘身份清单(dev/安装版 exe 文件名小写)。
///   每个身份只在首次出现时自动写一次 IsPromoted=1;之后用户在任务栏设置里
///   手动隐藏该图标的选择会被永久尊重(不再每次启动强制拉回)。
#[cfg(target_os = "windows")]
#[derive(serde::Deserialize, serde::Serialize, Clone, Default)]
struct TraySettings {
    #[serde(default = "tray_settings_default_visible")]
    always_visible: bool,
    #[serde(default)]
    promoted_identities: Vec<String>,
}

#[cfg(target_os = "windows")]
fn tray_settings_default_visible() -> bool {
    true
}
...
///
/// force=false(启动自动路径):仅处理 promoted_identities 里未记录的新身份,
///   每个 身份+target 只写一次,此后尊重用户手动调整(含手动隐藏)。
/// force=true(设置开关显式操作):忽略记录,全部身份强制写 target。
///
...
fn apply_tray_promotion(app: &tauri::AppHandle, target: u32, force: bool) -> bool {
    ...
    for sub in root.enum_keys().flatten() {
        ...
        let Some(identity) = candidates.iter().find(|c| **c == last_segment) else {
            continue;
        };
        // 尊重用户选择:该身份已处理过且非显式强制 → 跳过
        if !force && settings.promoted_identities.iter().any(|i| i == identity) {
            continue;
        }
        ...
        // 记录已处理身份(幂等去重)
        if !settings.promoted_identities.iter().any(|i| i == identity) {
            settings.promoted_identities.push(identity.clone());
        }
    }

    if !settings.promoted_identities.is_empty() {
