    ...
    Win32::{
        ...
        UI::{
            Shell::{
                ...
                NIF_STATE, NIF_TIP, NIM_ADD, NIM_DELETE, NIM_MODIFY, NIS_HIDDEN,
...
/// 为托盘图标派生一个**稳定 GUID**,使 Windows 通知区(系统托盘)按 GUID 记忆图标的
/// 显示/隐藏状态,而不是按 exe 路径(开发版/安装版/更新版路径不同 → 被当成不同图标 →
/// 显隐记忆每次重置、图标被丢进隐藏溢出区)。
///
/// 根因:Tauri/tray-icon 默认只以 (hWnd, 自增计数器 uID) 标识图标,不设置 guidItem,
/// 因此 Windows 退化为按 exe 路径识别。Microsoft 官方明确要求用 GUID 才能保证
/// "跨重装/更新持久化"。这里对同一个 tray id(如 "main")散列出确定性的 u128 → GUID,
/// 每次启动、每次更新都得到完全相同的 GUID。
fn tray_guid(id: &TrayIconId) -> GUID {
    // FNV-1a (128-bit) 对 id 字符串做确定性散列;结果稳定、与构建/路径无关。
    const FNV_OFFSET: u128 = 0x6c62_272e_07bb_0142_62b8_2175_6295_c58d;
    const FNV_PRIME: u128 = 0x0000_0000_0100_0000_0000_0000_0000_013b;
    let mut hash: u128 = FNV_OFFSET;
    for b in id.as_ref().as_bytes() {
        hash ^= *b as u128;
...
impl TrayIcon {
    pub fn new(id: TrayIconId, attrs: TrayIconAttributes) -> crate::Result<Self> {
        ...
        // 为托盘图标派生稳定 GUID,使 Windows 通知区按 GUID 记忆显隐(跨重装/更新持久化)。
        let guid = tray_guid(&id);
        ...
        unsafe {
            ...
            if !register_tray_icon(hwnd, internal_id, &hicon, &attrs.tooltip, true, guid) {
    ...
    pub fn show_menu(&self) {
        if let Some(menu) = &self.menu {
            unsafe {
                if let Some(rect) = get_tray_rect(self.guid) {
    ...
    pub fn rect(&self) -> Option<Rect> {
        get_tray_rect(self.guid).map(Into::into)
    }
...
impl Drop for TrayIcon {
    fn drop(&mut self) {
        unsafe {
            remove_tray_icon(self.hwnd, self.guid);
...
unsafe extern "system" fn tray_proc(
    ...
    match msg {
        ...
        _ if msg == *S_U_TASKBAR_RESTART => {
            remove_tray_icon(userdata.hwnd, userdata.guid);
            register_tray_icon(
                userdata.hwnd,
            ...
            ) =>
        {
            ...
            let rect = match get_tray_rect(userdata.guid) {
...
#[inline]
unsafe fn remove_tray_icon(hwnd: HWND, guid: GUID) {
    let nid = NOTIFYICONDATAW {
        uFlags: NIF_ICON | NIF_GUID,
        hWnd: hwnd,
        uID: 0,
...
#[inline]
fn get_tray_rect(guid: GUID) -> Option<RECT> {
