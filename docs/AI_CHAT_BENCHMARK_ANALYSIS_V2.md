...
> 证据基线:2 路代码全链路摸排(前端消息渲染 10 维 + 后端传输/编排/存储 10 维,均为 2026-09-15 现场核对,E1)+ 4 路竞品联网调研(OpenAI Codex app 官方公告 2026-03,E3;Trae 官网与中文社区知识库,E3+E4;Qoder 官网与 docs.qoder.com Quest 文档,E3;WorkBuddy 产品能力,E5·二手·不可核证,见 §一.4 说明)。
>
> 证据等级(本轮统一口径,D57 补):E1 本仓代码/命令实测;E2 竞品本机安装包/运行时一手反编译或取证;E3 竞品官方文档/公告;E4 联网二手(社区/媒体);E5 推测/不可核证。以下各节各表均已标注等级;凡标 E5 不得当一手结论引用。
>
> WorkBuddy 特别声明(D57 卫生项):WorkBuddy 本机无本体、无安装包、无运行时取证物;本仓 `.workbuddy/` 系我方 git-push-guard 自建状态目录,与竞品 WorkBuddy 无关。故本文档所有 WorkBuddy 条目均为 E5 二手·不可核证,仅作方向参考,不支撑差距结论的"有/无"判定;下游引用必须保留该限定。V1–V3 核查:docs/ 下仅存在本 V2 文件,无其他 V1/V3 报告文件,故仅在本文件内声明修正,不改他处。

---

## 一、四竞品对话界面元素基线(2026-09 实况,证据等级见各小节)

### 1. OpenAI Codex(app / CLI / IDE 扩展,GPT-5.2/5.3-Codex)【E3:竞品官方公告/文档】
...
### 2. Trae(Chat / Builder / SOLO)【E3+E4:官网(E3)+中文社区知识库(E4)】
...
### 3. Qoder(Desktop / CLI / JetBrains)【E3:官网与 docs.qoder.com Quest 文档】
...
### 4. WorkBuddy(桌面 Agent 工作台)【E5·二手·不可核证:本机无本体,无一手取证】

> ⚠️ 本节 E5·二手·不可核证:WorkBuddy 本机无本体、无安装包、无运行时取证物;本仓 `.workbuddy/` 系我方 git-push-guard 自建状态目录,与该竞品无关。本节做法均转述自联网二手材料,不可当一手引用,不支撑差距"有/无"判定。

| 对话流元素 | 做法 | 证据 |
|---|---|---|
| 模式 | Agent / Plan / Ask 三模式 | E5·二手·不可核证 |
| 执行可视化 | 任务清单 UI(TaskCreate/List,状态流转呈现);后台任务(run_in_background + 完成通知);内联可视化 widget(SVG 图表/交互组件随对话流出) | E5·二手·不可核证 |
| 上下文 | 项目布局快照、身份文件注入、三层记忆(云端画像 / 用户级 / 工作区日志);MCP 连接器生态 | E5·二手·不可核证 |
| 团队 | 多 agent 团队(TeamCreate / 消息协作 / 共享任务看板) | E5·二手·不可核证 |
| 其他 | Skills 体系(SKILL.md 触发词路由)、定时自动化、浏览器实时预览、结果文件卡片(present_files)、检查点回退 | E5·二手·不可核证 |

---

## 二、本项目对话全链路现状速写(文件证据)【E1:本仓代码/命令实测】
...
## 三、逐项差距矩阵(本轮核心产出)【口径见下,不单列等级】

> 证据口径:竞品做法列等级继承 §一(Codex E3;Trae E3+E4;Qoder E3;WorkBuddy E5·二手·不可核证);本项目现状列均为 E1(文件证据);结论/建议为基于前两者的工程推导,不单独定级。其中引用 WorkBuddy 的仅第 27 项一处,已标 E5 限定;第 15/17/32 项原"四家/各家"表述已修正为"可核证三家+E5 另注",不把 WorkBuddy 当一手计入判定。

图例:🔴P0=用户每次对话都感知;🟡P1=一周内应跟上;⚪P2=拉开身位/长尾。`无`/`部分`/`有` 为与竞品基线对照结论。

### A. 对话流显示层【竞品做法等级继承 §一,本项目现状 E1】

| # | 项目 | 竞品做法 | 本项目现状(证据) | 结论 | 建议方案 |
|---|---|---|---|---|---|
| 15 | 会话标题自动生成 | 三家可核证全员(Codex/Trae/Qoder,E3/E4):首条消息自动起标题;WorkBuddy 二手亦称有〔E5·二手·不可核证,不纳入判定〕 | `chat.ts` schema 无标题生成链路,仅前端手填/默认 | 🔴 无 | 首轮回复 done 后异步 LLM 生成(截断首条 user 消息+摘要),失败静默回退现有命名;i18n 5 语言 |
...
| 17 | 代码块行号 | Codex diff 带行号〔E3〕、可核证各家代码块普遍行号〔E3+E4〕(WorkBuddy 是否有行号不可核证,E5,不纳入) | `markdown-stream.tsx` 代码块无行号(高亮/复制/折叠/应用齐备) | 🔴 无 | react-syntax-highlighter gutter 行号 + `showLineNumbers` 开关;diff 卡行号已有则对齐样式 |
...
### B. 传输与会话健壮性【竞品做法多为工程标配,本项目现状 E1】
...
| 23 | chat 多端/多标签实时同步 | Codex app 多线程并行可见;可核证各家账号级同步(WorkBuddy 是否同步不可核证,E5,不纳入) | chat 不走 WS(`create-websocket-hook` 仅 question 广播),多端打开不同步 | 🟡 无 | chat-server 增加 conversation 房间:消息落库后广播 `message:created/patched`,他端增量拉取;先做多标签同端,再扩移动端 |
...
### C. 智能层(上下文与记忆的「可见性」)【本项目现状 E1;第 27 项竞品做法含 E5,见行内标注】
...
| 27 | 记忆更新可视化 | WorkBuddy 三层记忆可感〔E5·二手·不可核证,不支撑判定〕;Qoder Memory 沉淀可见〔E3〕 | LTM/画像后台静默写,对话流无任何提示 | 🟡 部分 | 记忆写入时插入轻提示条「已记住:…(管理)」跳设置页;避免喧宾夺主,每轮限 1 条 |
...
### D. 长尾与质量【竞品做法 E3/E4 为主,第 32 项见行内限定;本项目现状 E1】

| # | 项目 | 竞品 | 现状 | 结论 | 建议方案 |
|---|---|---|---|---|---|
| 32 | PDF 等文件消息内富预览 | 可核证三家有不同程度支持〔E3/E4〕(WorkBuddy 二手,E5,不纳入判定) | 输出文件无内嵌预览 | ⚪ 无 | 常见格式(PDF/CSV)iframe/表格化预览条 |
...
## 四、已达标、无需立项项(审计确认,避免误伤)【E1:本仓实测审计结论】
...
## 五、与第一轮批次的关系【综合推导:第一轮 E1 结论 + 本轮 §一–§四等级继承】
...
- **最大病根沿用第一轮判断并新增**:第一轮病根是「能力库存 > 实际生效」;本轮新增病根是「**智能在后台发生,但对话流里看不见**」——RAG/记忆/turn 变更/usage 都真实存在,只是没有以用户可感知的形式呈现,恰是 Codex/Trae/Qoder 对话体验的主观差距来源(WorkBuddy 条目为 E5,不参与本病根判定)。

## 六、排期建议【工程排期推导,不单独定级;WorkBuddy 相关仅第 27 项记忆提示一条,且为 E5 参考】
