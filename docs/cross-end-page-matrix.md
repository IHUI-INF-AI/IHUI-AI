...
| --- | --- | --- | --- |
| ✅ 两端一致 | 126（含多对一映射；历轮 P0-P2 补齐核销 + 收尾轮 pay/result 收敛、following↔Follow 对齐） | 105 个 screen 被映射 | 126 对 |
| 🟡 非必需差异 | 3（topic×2、circle/index：Screen+深链已落地，待双端回归转 ✅） | 52（含 Points×3 复核转入） | 55 |
| 🔴 真缺失 | 8（RN 缺：forgot-password/email/live-calendar/dev-subscribe/dist-rank/exam-detail/study-rank/ai-circle；基线 15 → 历轮核销 7） | 18（小程序缺，基线 31 → 历轮核销 13） | 26 |
...
> 校验（收尾轮刷新）：miniapp 126+3+8+0+25 = 162 ✓（域表实际条目数）；RN 105+52+18+0+6 = 181 ✓（删 Following/Finance 2 项）。
...
| `pages/teacher/list` ↔ TeacherListScreen | TeacherListScreen | ✅ | ✅ | 讲师列表（第 7 轮 P0 补齐） |
| `pages/teacher/detail` ↔ TeacherDetailScreen | TeacherDetailScreen | ✅ | ✅ | 讲师详情（第 7 轮 P0 补齐） |
...
| `pages/topic/list` ↔ TopicListScreen | TopicListScreen | ✅ | 🟡 | 收尾轮：TopicListScreen + linking 深链已落地，待双端回归后转 ✅ |
| `pages/topic/detail` ↔ TopicDetailScreen | TopicDetailScreen | ✅ | 🟡 | 同上 |
...
| `pages/circle/index` ↔ CircleIndexScreen | CircleIndexScreen | ✅ | ✅ | 圈子首页/列表（第 8 轮补齐 + linking 深链） |
...
| — ↔ PointsMallScreen | PointsMallScreen | — | 🟡 | 积分商城，小程序由 `member/integral` 承接积分场景，细分页属 RN 独立实现 |
| — ↔ PointRuleScreen | PointRuleScreen | — | 🟡 | 积分规则，同上（收尾轮复核：小程序 member/integral 已含规则说明） |
| — ↔ PointHistoryScreen | PointHistoryScreen | — | 🟡 | 积分流水，同上（收尾轮复核：member/integral 已含流水） |
...
| `pages/cart/index` ↔ CartScreen | CartScreen | ✅ | ✅ | 购物车（第 6 轮补齐，交易闭环） |
...
### 🔴 真缺失清单（终态：RN 缺 8 / miniapp 缺 18 = 26 项）

**RN 端缺失（小程序有 → RN，8 项；核销前基线 15 项）**

| # | 页面 | 域 | 说明 |
| --- | --- | --- | --- |
| 1 | `pages/forgot-password/index` | 账号 | 找回密码（RN 登录页缺入口，属功能缺口） |
| 2 | `pages/user/email` | 账号 | 绑定邮箱 |
| 3 | `pages/live/calendar` | 直播 | 直播日历 |
| 4 | `pages/developer/subscribe` | 开发者 | 开发者订阅 |
| 5 | `pages/distribution/rank` | 分销 | 分销排行 |
| 6 | `pages/exam/detail` | 教育 | 考试详情 |
| 7 | `pages/study/rank` | 教育 | 学习排行 |
| 8 | `pages/ai-circle/index` | AI | AI 圈 |

> 🟡 观察项（不计入 🔴）：`topic/list`、`topic/detail`、`circle/index` —— 收尾轮 TopicListScreen/TopicDetailScreen/CircleIndexScreen + linking 深链已落地并通过全量守门，建议双端真机回归后转 ✅。
> 历轮核销：`cart`（第 6 轮 CartScreen）、`teacher/list`+`teacher/detail`（第 7 轮）、`pay/result`（第 8 轮 PayResultScreen 承接 + 小程序三套结果页收敛）。

**小程序端缺失（RN 有 → miniapp，18 项；核销前基线 31 项，P0/P1/P2 补齐 10 + 复核转 🟡 3）**

| # | 页面 | 域 | 说明 |
| --- | --- | --- | --- |
| 1 | CertificateScreen | 教育 | 证书中心 |
| 2 | CertListScreen | 教育 | 证书列表 |
| 3 | CertDetailScreen | 教育 | 证书详情 |
| 4 | CertApplyScreen | 教育 | 证书申请 |
| 5 | CertVerifyScreen | 教育 | 证书核验 |
| 6 | ExamHistoryScreen | 教育 | 考试记录 |
| 7 | KnowledgeBaseScreen | 知识库 | 知识库 |
| 8 | KnowledgeDocScreen | 知识库 | 知识库文档 |
| 9 | KnowledgeCreateScreen | 知识库 | 知识库创建 |
| 10 | KnowledgeRagScreen | 知识库 | 知识库检索 |
| 11 | KnowledgePlanetScreen | 知识库 | 知识星球 |
| 12 | NoteScreen | 笔记 | 笔记 |
| 13 | NoteListScreen | 笔记 | 笔记列表 |
| 14 | NoteDetailScreen | 笔记 | 笔记详情 |
| 15 | NoteCreateScreen | 笔记 | 笔记创建 |
| 16 | BookmarkScreen | 内容 | 书签/浏览记录 |
| 17 | MemoryScreen | AI | AI 记忆管理 |
| 18 | AiWorldScreen | AI | AI 世界 |

> 历轮核销：Search/CheckIn/TaskCenter（第 6 轮 P0）、PostDetail→plaza/detail、PostCreate→community/create（第 7 轮 P1）、Announcement×2/Activity/AiSkill×2（第 7 轮 P2）；PointsMall/PointRule/PointHistory 复核转 🟡（member/integral 承接）。
...
- **P1 — 内容与社区生态**：~~Announcement+AnnouncementDetail、AiSkill+AiSkillDetail、Activity、PostDetailScreen+PostCreateScreen（→miniapp）~~ ✅ 补齐完成；~~`topic/list`+`topic/detail`、`circle/index`（→RN）~~ ✅ 第 8 轮 Screen + linking 深链完成；`ai-circle`（→RN）仍缺。
