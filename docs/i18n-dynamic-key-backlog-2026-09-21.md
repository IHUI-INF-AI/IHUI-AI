...
这些值 HEAD 就在渲染,与动态键无关,判据也无法靠"键是否存在"发现。
**本轮进展**:先修掉其中"根本没翻译"的确定性子集 —— 判据 = ja/ko 值与同键 zh-CN **逐字节相同**
且不含假名/谚文 —— ja 55 处 + ko 9 处(commit `9814f32b6d`),`web/ko` 中文残留守门 9 → 0。
再按 **(中文值, ja 残片值) 组合词表** 精确替换 41 组高频 chrome 标签,web/ja 残片
**784 → 235 处**(549 处替换,commit 见本次;词表 + 纯平假名 ≤4 双条件,片假名词如
`ソート`/`エラー` 一律不碰 —— 第一版误把 `ソート` 当残片改掉,已收紧后重跑)。
剩余 235 处 / 194 组合基本是 `新建X ⇒ しい`、`请输入X ⇒ を` 长尾(每个 2 处),
续跑方式:往 `PAIRS` 里加组合,或为 `新建/新增/请输入 + 名词` 建一张名词映射表后生成式替换 ——
名词表未覆盖的**不要**自动生成。
两条负面结论,不要重复踩:
1. **`opencc-js` 的 `cn2jp` 不能用来自动修 ja** —— 实测把正当日语改写坏(`注文`→`註文`、`連携`→`連攜`),
   622 个"命中"里大量是误报,已丢弃该方案,未据此改任何值。
2. **"ja 值不含假名"不是缺陷判据** —— `操作`/`保存`/`内容`/`名称`/`最小化`/`商品管理`/`評価` 都是正当和语汉字词
   (全库 781 个"与中文同形无假名"的值里绝大多数属此类),所以只改"与中文逐字相同"的确定集合,
   绝不做启发式批量改写。

### 候选但未校准的一类(计数**不等于**缺陷数,别直接开单)

映射表属性值形态(`labelKey: 'a.b'` / `xxxKey: 'a.b'` → 再 `t(cfg.labelKey)`)是本档案与
`check-i18n-keys` 共同的**检测盲区**:档案抽取器只吃 ``t(`p.${x}`)`` 模板前缀与 `?? 'x'` 兜底字面量,
`useT`/解构扩展也只覆盖到"键出现在 `t(...)` 参数位"。用 `[Kk]ey[s]?: 'a.b'` 粗扫全仓得到
`web 286 / miniapp-taro 127 / mobile-rn 93 / api 6 / cli 2` 共 514 条 —— **但这条判据未经校准**:
`apps/api` 的消息包本就不装界面标签、Taro 的 `tt(key, 中文兜底)` 在键缺失时渲染兜底文案(表现是
"没翻译"而不是"回显键名",严重度完全不同)。所以这些数字**只能当作待分拣的候选池**,
真要开单必须逐条回到调用点确认:① 该值最终是否进 `t()`;② 有无第二参兜底;③ 该端词典是否本就不含此 ns。
