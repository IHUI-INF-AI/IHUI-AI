...
- **搜索关键词高亮**(2026-07-22 立升级):搜索时模型名/厂商名中匹配的关键词用 `<mark>` 高亮显示(黄色背景,dark mode 适配),大小写不敏感,支持多次匹配,正则特殊字符自动转义
- **空状态优化**(2026-07-22 立升级):搜索/筛选无结果时显示"未找到匹配的模型"提示 + "清空筛选"按钮(一键清空搜索词和厂商筛选),与无搜索时的"暂无排行数据"区分
...
- **搜索关键词高亮**(2026-07-22 立升级):搜索时平台名/特点/计费/厂商标签中匹配的关键词用 `<mark>` 高亮显示(黄色背景 dark mode 适配),复用 Leaderboard 的 `highlight` 函数(提取到 `text-utils.tsx` 共享)
...
| 子命名空间 | 键数 | 用途 |
|---|---|---|
| `leaderboard` | 9 | 排行榜标题/副标题/空态/排序提示/搜索/全部厂商/未匹配/清空筛选 |
