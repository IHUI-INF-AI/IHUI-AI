...
def replace_gitee_feed(latest_data):
    ...
    if rel and rel.get("id"):
        api(f"/repos/{OWNER}/{REPO}/releases/{rel['id']}", "DELETE")
        print("[gitee] 旧 feed release 已删除(连带旧附件)")
    # 删 release 后 tag 仍残留 → 同 tag 重建会被拒("该标签已经存在发行版"),先删 tag
    # (tag 由 GitHub 侧镜像同步,下次镜像会自动恢复,无副作用)
    api(f"/repos/{OWNER}/{REPO}/tags/{FEED_TAG}", "DELETE")
