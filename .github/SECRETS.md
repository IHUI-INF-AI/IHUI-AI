...
> **说明**：下方「必配 Secrets（生产环境）」部分为应用运行时所需的配置项（通常通过 `.env` 注入到运行中的服务），并非全部以 `${{ secrets.* }}` 形式直接出现在工作流 YAML 中。CI 流水线中测试多使用本地服务容器（如 postgres:18）与硬编码测试凭据，因此这些生产密钥主要用于部署后的运行环境。「CI/CD 专用 Secrets」则为工作流中通过 `${{ secrets.* }}` 直接引用的变量。
